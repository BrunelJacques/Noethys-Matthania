...
DICT_LABELS_ACTIONS = {
    "enregistrer" : _("Enregistrer une consommation"),
    "reserver" : _("Réserver des consommations"),
    "message" : _("Afficher des messages"),
    }
...
def GetDetailAction(dictDonnees):
    ...
    if action == "enregistrer" :
        ...
        if len(listeDonnees) > 0 :
            ...
            texte = "%s (%s)" % (nomUnite, nomActivite)
        else :
            texte = _("Unité inconnue")
    ...
    if action == "reserver" :
        ...
        texte = "%s (%s)" % (", ".join(listeUnites), nomActivite)

    # Message
    if action == "message" :
        # Message unique
        if dictDonnees["action_message"] != None :
            texte = _("1 message unique : '%s'") % dictDonnees["action_message"]
        else :
            listeMessages = [] 
            for IDmessage, message in dictDonnees["action_messages"] :
                listeMessages.append("'%s'" % message)
            if len(listeMessages) == 1 :
                texte = _("1 message aléatoire : %s") % listeMessages[0]
            else :
                texte = _("%d messages aléatoires : %s") % (len(listeMessages), ", ".join(listeMessages))
...
class CTRL_Icone(BitmapComboBox):
    def __init__(self, parent, size=(-1,  -1)):
        ...
        # MAJ
        self.listeIcones = [
            ("commentaire", _("Commentaire"), "Images/Badgeage/Commentaire_24x24.png"),
            ("information", _("Information"), "Images/Badgeage/Information_24x24.png"),
            ("exclamation", _("Exclamation"), "Images/Badgeage/Exclamation_24x24.png"),
            ("question", _("Question"), "Images/Badgeage/Question_24x24.png"),
            ("erreur", _("Erreur"), "Images/Badgeage/Erreur_24x24.png"),
        ]
...
class CTRL_Liste_messages(wx.ListBox):
    ...
    def Ajouter(self):
        dlg = wx.TextEntryDialog(self, _("Saisissez un message :"), _("Saisie"), "")
        if dlg.ShowModal() == wx.ID_OK:
            nom = dlg.GetValue()
            if nom == "":
                dlg = wx.MessageDialog(self, _("Le message que vous avez saisi n'est pas valide."), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return
            else:
    ...
    def Modifier(self):
        ...
        if index == -1 :
            ...
            dlg.Destroy()
            return
        dlg = wx.TextEntryDialog(self, _("Modifiez le message :"), _("Modification"), valeur)
        if dlg.ShowModal() == wx.ID_OK:
            nom = dlg.GetValue()
            if nom == "":
                dlg = wx.MessageDialog(self, _("Le message que vous avez saisi n'est pas valide."), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
...
class CTRL_Message(wx.Panel):
    def __init__(self, parent):
        ...
        self.label_messages = wx.StaticText(self, -1, _("Messages :"))
        
        self.radio_unique = wx.RadioButton(self, -1, _("Message unique :"), style=wx.RB_GROUP)
        self.ctrl_unique = wx.TextCtrl(self, -1, "")
        
        self.radio_aleatoires = wx.RadioButton(self, -1, _("Messages aléatoires :"))
        ...
        self.label_icone = wx.StaticText(self, -1, _("Icône :"))
        self.ctrl_icone = CTRL_Icone(self)

        self.label_duree = wx.StaticText(self, -1, _("Durée :"))
        self.ctrl_duree = wx.SpinCtrl(self, -1, "", min=1, max=60, initial=2, size=(80, -1))
        
        self.label_frequence = wx.StaticText(self, -1, _("Fréquence :"))
        self.label_pourcentage = wx.StaticText(self, -1, "100 %")
        
        self.label_pourcentage.SetMinSize((40, -1))
        self.ctrl_frequence = wx.Slider(self, -1, 100, 1, 100, size=(-1, 28), style=wx.SL_HORIZONTAL)
        self.label_audio = wx.StaticText(self, -1, _("Audio :"))
        self.ctrl_vocal = wx.CheckBox(self, -1, _("Activer la synthèse vocale"))
    ...
    def __set_properties(self):
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter un message")))
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier le message sélectionné")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer le message sélectionné")))
        self.ctrl_icone.SetToolTip(wx.ToolTip(_("Sélectionnez l'icône à afficher dans la boîte de dialogue")))
        self.ctrl_duree.SetToolTip(wx.ToolTip(_("Sélectionnez la durée d'affichage du message (en secondes)")))
        self.ctrl_frequence.SetToolTip(wx.ToolTip(_("Sélectionnez la fréquence de diffusion (en %)")))
        self.ctrl_vocal.SetToolTip(wx.ToolTip(_("Cochez cette case pour activer la synthèse vocale")))
    ...
    def Validation(self):
        if self.radio_aleatoires.GetValue() == True :
            if len(self.ctrl_aleatoires.GetValeur()) == 0 :
                dlg = wx.MessageDialog(self, _("Vous devez saisir au moins un message !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        ...
        if self.radio_unique.GetValue() == True :
            if len(self.ctrl_unique.GetValue()) == 0 :
                dlg = wx.MessageDialog(self, _("Vous devez saisir un message !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
...
class CTRL_Choix_activite(wx.Choice):
    ...
    def GetListeDonnees(self):
        ...
        for IDactivite, nom in listeDonnees :
            if nom == None : nom = _("Activité inconnue")
...
class CTRL_Choix_etat(wx.Choice):
    def __init__(self, parent):
        ...
        self.parent = parent
        self.listeEtats = [
            #(None, _("Ne pas modifier")),
            ("reservation", _("Pointage en attente")),
            ("present", _("Présence")),
            ("absentj", _("Absence justifiée")),
            ("absenti", _("Absence injustifiée")),
            ]
        self.MAJ() 
...
class CTRL_Enregistrer(wx.Panel):
    def __init__(self, parent):
        ...
        # Unité
        self.box_unite_staticbox = wx.StaticBox(self, -1, _("Unité de consommation"))
        ...
        # Demander
        self.check_demander = wx.CheckBox(self, -1, _("Demander s'il s'agit du début ou de la fin"))

        # Début
        self.box_debut_staticbox = wx.StaticBox(self, -1, _("Heure de début"))
        self.radio_debut_pointee = wx.RadioButton(self, -1, _("Heure du badgeage"), style=wx.RB_GROUP)
        self.radio_debut_defaut = wx.RadioButton(self, -1, _("Heure par défaut"))
        self.radio_debut_autre = wx.RadioButton(self, -1, _("Autre heure :"))
        self.ctrl_debut_autre = CTRL_Saisie_heure.Heure(self)

        # Fin
        self.box_fin_staticbox = wx.StaticBox(self, -1, _("Heure de fin"))
        self.radio_fin_pointee = wx.RadioButton(self, -1, _("Heure du badgeage"), style=wx.RB_GROUP)
        self.radio_fin_defaut = wx.RadioButton(self, -1, _("Heure par défaut"))
        self.radio_fin_autre = wx.RadioButton(self, -1, _("Autre heure :"))
        self.ctrl_fin_autre = CTRL_Saisie_heure.Heure(self)
        
        # Message de confirmation
        self.box_message_staticbox = wx.StaticBox(self, -1, _("Message de confirmation"))
        self.label_message_actif = wx.StaticText(self, -1, _("Activé :"))
        self.check_message_actif = wx.CheckBox(self, -1, "")
        self.label_message_texte = wx.StaticText(self, -1, _("Texte :"))
        self.ctrl_message_texte = wx.TextCtrl(self, -1, "")
        self.label_audio = wx.StaticText(self, -1, _("Audio :"))
        self.ctrl_vocal = wx.CheckBox(self, -1, _("Activer la synthèse vocale"))

        # Impression de ticket
        self.box_ticket_staticbox = wx.StaticBox(self, -1, _("Impression d'un ticket"))
        self.label_ticket_actif = wx.StaticText(self, -1, _("Activé :"))
        self.check_ticket_actif = wx.CheckBox(self, -1, "")
        self.label_ticket_modele = wx.StaticText(self, -1, _("Modèle :"))
    ...
    def __set_properties(self):
        self.ctrl_activite.SetToolTip(wx.ToolTip(_("Sélectionnez une activité")))
        self.ctrl_unite.SetToolTip(wx.ToolTip(_("Sélectionnez une unité de consommation")))
        self.ctrl_etat.SetToolTip(wx.ToolTip(_("Sélectionnez un état")))
        self.check_demander.SetToolTip(wx.ToolTip(_("[OPTION OBSOLETE] Cochez cette case pour que Noethys demande s'il s'agit du début ou de la fin de la consommation.")))
        self.radio_debut_defaut.SetToolTip(wx.ToolTip(_("Heure par défaut définie dans le paramétrage de l'unité")))
        self.radio_debut_pointee.SetToolTip(wx.ToolTip(_("Heure badgée")))
        self.radio_debut_autre.SetToolTip(wx.ToolTip(_("Autre heure")))
        self.ctrl_debut_autre.SetToolTip(wx.ToolTip(_("Saisissez une heure")))
        self.radio_fin_defaut.SetToolTip(wx.ToolTip(_("Heure par défaut définie dans le paramétrage de l'unité")))
        self.radio_fin_pointee.SetToolTip(wx.ToolTip(_("Heure badgée")))
        self.radio_fin_autre.SetToolTip(wx.ToolTip(_("Autre heure")))
        self.ctrl_fin_autre.SetToolTip(wx.ToolTip(_("Saisissez une heure")))
        self.check_message_actif.SetToolTip(wx.ToolTip(_("Cochez cette case pour activer l'affichage d'un message de confirmation")))
        self.ctrl_message_texte.SetToolTip(wx.ToolTip(_("Saisissez un message de confirmation")))
        self.ctrl_vocal.SetToolTip(wx.ToolTip(_("Cochez cette case pour activer la synthèse vocale")))
        self.check_ticket_actif.SetToolTip(wx.ToolTip(_("Cochez cette case pour activer l'impression d'un ticket")))
        self.ctrl_ticket_modele.SetToolTip(wx.ToolTip(_("Sélectionnez le modèle de ticket à utiliser")))
    ...
    def OnCheckDemander(self, event): 
        ...
        if etat == True :
            self.box_debut_staticbox.SetLabel(_("Heure de début par défaut"))
            self.box_fin_staticbox.SetLabel(_("Heure de fin par défaut"))
        else :
            self.box_debut_staticbox.SetLabel(_("Heure de début "))
            self.box_fin_staticbox.SetLabel(_("Heure de fin"))
    ...
    def Validation(self):
        ...
        if self.ctrl_unite.GetID() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner une unité de consommation !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False

        if self.radio_debut_autre.GetValue() == True and self.ctrl_debut_autre.GetHeure() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une heure de début !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.radio_fin_autre.GetValue() == True and self.ctrl_fin_autre.GetHeure() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une heure de fin !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.check_message_actif.GetValue() and len(self.ctrl_message_texte.GetValue()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez saisi aucun texte de confirmation !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.check_ticket_actif.GetValue() and self.ctrl_ticket_modele.GetID() == None :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun modèle de ticket dans la liste !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
...
class CTRL_Choix_date(wx.Choice):
    def __init__(self, parent):
        ...
        self.parent = parent
        self.listeEtats = [
            ("date_actuelle", _("Le jour-même")),
            ("prochaine_ouverture", _("La prochaine date d'ouverture de l'activité")),
            ]
        self.MAJ() 
...
class CTRL_Reserver(wx.Panel):
    def __init__(self, parent):
        ...
        # Unité
        self.box_unite_staticbox = wx.StaticBox(self, -1, _("Unités de consommation à proposer"))
        self.label_activite = wx.StaticText(self, -1, _("Activité :"))
        self.ctrl_activite = CTRL_Choix_activite(self)
        self.label_unite = wx.StaticText(self, -1, _("Unités :"))
        self.ctrl_unite = CTRL_Liste_unites(self)
        self.label_etat = wx.StaticText(self, -1, _("Etat :"))
        self.ctrl_etat = CTRL_Choix_etat(self)
        self.label_attente = wx.StaticText(self, -1, _("Attente :"))
        self.check_attente = wx.CheckBox(self, -1, _("Proposer place sur liste d'attente si complet"))
        
        self.label_attente.Show(False)
        self.check_attente.Show(False)
        
        # Date
        self.box_date_staticbox = wx.StaticBox(self, -1, _("Date à proposer"))
        self.label_date = wx.StaticText(self, -1, _("Date :"))
        self.ctrl_date = CTRL_Choix_date(self)

        # Messages
        self.box_message_staticbox = wx.StaticBox(self, -1, _("Messages"))
        self.label_question = wx.StaticText(self, -1, _("Question :"))
        self.ctrl_question = wx.TextCtrl(self, -1, "")
        self.label_confirmation = wx.StaticText(self, -1, _("Confirmation :"))
        self.check_confirmation_actif = wx.CheckBox(self, -1, "")
        self.ctrl_confirmation = wx.TextCtrl(self, -1, "")
        self.label_audio = wx.StaticText(self, -1, _("Audio :"))
        self.ctrl_vocal = wx.CheckBox(self, -1, _("Activer la synthèse vocale"))
    ...
    def __set_properties(self):
        self.ctrl_activite.SetToolTip(wx.ToolTip(_("Sélectionnez une activité")))
        self.ctrl_unite.SetToolTip(wx.ToolTip(_("Sélectionnez les unités de consommation à proposer")))
        self.ctrl_etat.SetToolTip(wx.ToolTip(_("Sélectionnez un état")))
        self.check_attente.SetToolTip(wx.ToolTip(_("Cochez cette case pour que Noethys propose une place sur liste d'attente sur il n'y a plus de places")))
        self.ctrl_date.SetToolTip(wx.ToolTip(_("Sélectionnez la date à proposer")))
        self.ctrl_question.SetToolTip(wx.ToolTip(_("Saisissez le texte de la question")))
        self.check_confirmation_actif.SetToolTip(wx.ToolTip(_("Cochez cette case pour activer l'affichage d'un message de confirmation")))
        self.ctrl_confirmation.SetToolTip(wx.ToolTip(_("Saisissez un message de confirmation")))
        self.ctrl_vocal.SetToolTip(wx.ToolTip(_("Cochez cette case pour activer la synthèse vocale")))
    ...
    def Validation(self):
        ...
        if len(self.ctrl_unite.GetIDcoches()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez cocher au moins une unité de consommation !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False

        if len(self.ctrl_unite.GetIDcoches()) > 9 :
            dlg = wx.MessageDialog(self, _("Vous ne pouvez pas proposer plus de 9 unités de consommation !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False

        if len(self.ctrl_question.GetValue()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une question !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
...
##        self.ctrl.SetValeur("reserver", {"activite":"1", "unite":"1", "etat":"present", "demande":"0", "heure_debut":"pointee", "heure_fin":"16:30", "message":_("coucou"), "vocal":"1"})
