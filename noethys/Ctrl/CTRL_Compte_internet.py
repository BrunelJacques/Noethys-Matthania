...
class CTRL(html.HtmlWindow):
    ...
    def MAJ(self):
        if self.dictDonnees["internet_actif"] == 1 :
            activation = _("Compte internet activé")
            image = "Ok4"
        else :
            activation = _("Compte internet désactivé")
            image = "Interdit2"
        ...
        if mdp.startswith("custom"):
            mdp = _("********<BR><FONT SIZE=1>(Mot de passe personnalisé)</FONT>")
        self.SetPage(u"""
        <FONT SIZE=2>
        <BR><BR>
    ...
    def Envoyer_pressepapiers(self, event):
        ...
        codes = _("Identifiant : %s / Mot de passe : %s") % (self.GetIdentifiant(), self.GetMdp())
        ...
        dlg = wx.MessageDialog(self, _("Les codes ont été copiés dans le presse-papiers."), "Presse-papiers", wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
