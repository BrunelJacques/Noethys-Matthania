...
class ObjectListView(OLV.ObjectListView):
    ...
    def MenuConfigurerListe(self, event=None):
        ...
        from Dlg import DLG_Configuration_listes
        dlg = DLG_Configuration_listes.Dialog(self, colonnes_dispo=colonnes_dispo,
                                              colonnes_defaut=colonnes_defaut,
                                              colonnes_selection=colonnes_selection)
        if dlg.ShowModal() == wx.ID_OK:
            listeColonnesSelection = dlg.GetSelections(mode="dict")
            dlg.Destroy()
            if not listeColonnesSelection:
                return
        else :
            dlg.Destroy()
            return
...
class CTRL_Outils(wx.Panel):
    def __init__(self, parent, listview=None, texteDefaut=u"Rechercher...",
        ...
        item = wx.MenuItem(menu, 10,
                           ...
                           "Cliquez ici pour accéder à la gestion des filtres de listes")
        item.SetBitmap(wx.Bitmap(
        ...
        if afficherCocher == True:
            ...
            item = wx.MenuItem(menu, 20, "Tout cocher",
                               "Cliquez ici pour cocher tous les éléments de la liste")
            item.SetBitmap(wx.Bitmap(
                Chemins.GetStaticPath("Images/16x16/Cocher.png"),
                wx.BITMAP_TYPE_ANY))
            menu.Append(item)
            item = wx.MenuItem(menu, 21, "Tout décocher",
                               "Cliquez ici pour décocher tous les éléments de la liste")
            item.SetBitmap(wx.Bitmap(
