...
def PeriodeComplete(mois, annee):
    listeMois = (_("Jan"), _("Fév"), _("Mars"), _("Avr"), _("Mai"), _("Juin"), _("Juil"), _("Août"), _("Sept"), _("Oct"), _("Nov"), _("Déc"))
...
class CTRL_Parametres(DLG_Options_impression_pdf.CTRL_Parametres):
    ...
    def Remplissage(self):
        ...
        # Couleur 3
        propriete = wxpg.ColourProperty(label=_("Fond ligne entêtes"), name="couleur_fond_entetes", value=wx.Colour(204, 204, 255))
        propriete.SetHelpString(_("Sélectionnez une couleur"))
        propriete.SetAttribute("obligatoire", True)
        self.Append(propriete)

        # Couleur 1
        propriete = wxpg.ColourProperty(label=_("Fond ligne dépôt"), name="couleur_fond_depot", value=wx.Colour(230, 230, 255))
        propriete.SetHelpString(_("Sélectionnez une couleur"))
        propriete.SetAttribute("obligatoire", True)
        self.Append(propriete)

        # Couleur 2
        propriete = wxpg.ColourProperty(label=_("Fond ligne total"), name="couleur_fond_total", value=wx.Colour(204, 204, 255))
        propriete.SetHelpString(_("Sélectionnez une couleur"))
        propriete.SetAttribute("obligatoire", True)
        self.Append(propriete)
        ...
        propriete = wxpg.IntProperty(label=_("Largeur colonne label"), name="largeur_colonne_labels", value=170)
        propriete.SetHelpString(_("Saisissez la largeur pour la colonne label (170 par défaut)"))
        propriete.SetAttribute("obligatoire", True)
        self.Append(propriete)
        self.SetPropertyEditor("largeur_colonne_labels", "SpinCtrl")

        # Largeur colonne valeurs
        propriete = wxpg.IntProperty(label=_("Largeur colonne valeur"), name="largeur_colonne_valeurs", value=45)
        propriete.SetHelpString(_("Saisissez la largeur pour la colonne valeur (45 par défaut)"))
        propriete.SetAttribute("obligatoire", True)
        self.Append(propriete)
...
##        self.AssignImageList(il)
    ...
    def Importation_ventilation(self):
        ...
        for IDventilation, IDreglement, IDprestation, montantVentilation, dateReglement, dateSaisieReglement, IDdepot, dateDepotReglement, datePrestation, labelPrestation, IDactivite, nomActivite, abregeActivite in listeVentilation :
            ...
            if nomActivite != None :
                labelPrestation = "%s - %s" % (nomActivite, labelPrestation)
    ...
    def CreationColonnes(self, listePeriodes=[]):
        ...
        if self.type == "depots" :
            self.AddColumn(_("Non ventilé"))
            self.SetColumnWidth(numColonne, 65)
            self.SetColumnAlignment(numColonne, wx.ALIGN_CENTRE)
            numColonne += 1

        # Création de la colonne Total
        self.AddColumn(_("Total"))
    ...
    def MAJ(self):        
        ...
        if self.type == "prestations" and None in dictVentilation :
            dictDepots[None] = {"date":datetime.date(1977, 1, 1), "nom":_("----- %d règlements non déposés -----") % len(dictIDreglements[None]), "verrouillage":False, "IDcompte":None, "montantTotal":0.0}
        ...
        dictColonnes = {}
        index = 1
        self.dictImpression["entete"].append(_("Dépôts"))
        ...
        if self.type == "depots" :
            ...
            self.dictImpression["entete"].append(_("Non ventilé"))
            index += 1
        dictColonnes["total"] = index
        self.dictImpression["entete"].append(_("Total"))
        ...
        for dateDepot, IDdepot, dictDepot in listeDepotsTemp :
            if dateDepot == datetime.date(1977, 1, 1) : 
                dateStr = _("Sans date de dépôt")
            else:
                dateStr = "%02d/%02d/%04d" % (dateDepot.day, dateDepot.month, dateDepot.year)
            label = "%s (%s - %.2f %s)" % (dictDepot["nom"], dateStr, dictDepot["montantTotal"], SYMBOLE)
            ...
            if self.type == "depots" :
                ...
                if sansVentilation != 0.0 :
                    ...
                    texte = "%.2f %s" % (sansVentilation, SYMBOLE)
                    ...
                    if IDdepot in dictVentilation :
                        if periode in dictVentilation[IDdepot]:
                            if label in dictVentilation[IDdepot][periode]:
                                ...
                                totalColonne += valeur
            texte = "%.2f %s" % (totalColonne, SYMBOLE)
        ...
        if self.type == "depots" :
            texte = "%.2f %s" % (totalSansVentilation, SYMBOLE)
    ...
    def Imprimer(self):
        ...
        def Header():
            ...
            dataTableau.append( (_("Analyse croisée ventilation/dépôts"), _("%s\nEdité le %s") % (UTILS_Organisateur.GetNom(), dateDuJour)) )
            style = TableStyle([
        ...
        for rubrique in listeRubriques :
            ...
            for ligne in listeLignes :
                ...
                for texte in ligne :
                    ...
                    if indexColonne > 0 :
                        texte = _("<para align='right'>%s</para>") % texte
