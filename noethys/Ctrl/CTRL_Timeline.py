...
class ToolBar(UTILS_Adaptations.ToolBar):
    def __init__(self, *args, **kwds):
        ...
        # Boutons
        self.AddLabelTool(ID_IMPRIMER,           _("Imprimer"), wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Imprimante.png"), wx.BITMAP_TYPE_ANY), wx.NullBitmap, wx.ITEM_NORMAL, _("Imprimer"), "")
        self.AddLabelTool(ID_APERCU,              _("Aperçu"), wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Apercu.png"), wx.BITMAP_TYPE_ANY), wx.NullBitmap, wx.ITEM_NORMAL, _("Aperçu avant impression"), "")
        self.AddLabelTool(ID_IMAGE,                 _("Enreg."), wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Sauvegarder.png"), wx.BITMAP_TYPE_ANY), wx.NullBitmap, wx.ITEM_NORMAL, _("Exporter au format image"), "")
        self.AddSeparator()
        self.AddLabelTool(ID_GO_TODAY,         _("Aujourd."), wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Jour.png"), wx.BITMAP_TYPE_ANY), wx.NullBitmap, wx.ITEM_NORMAL, _("Atteindre aujourd'hui"), "")
        self.AddLabelTool(ID_GO_DATE,            _("Trouver"), wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Recherche_date.png"), wx.BITMAP_TYPE_ANY), wx.NullBitmap, wx.ITEM_NORMAL, _("Atteindre une date"), "")
        self.AddSeparator()
        self.AddLabelTool(ID_AFFICHE_ANNEE, _("Année"), wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Calendrier.png"), wx.BITMAP_TYPE_ANY), wx.NullBitmap, wx.ITEM_NORMAL, _("Afficher l'année"), "")
        self.AddLabelTool(ID_AFFICHE_MOIS,    _("Mois"), wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Calendrier.png"), wx.BITMAP_TYPE_ANY), wx.NullBitmap, wx.ITEM_NORMAL, _("Afficher le mois"), "")
        self.AddLabelTool(ID_AFFICHE_JOUR,    _("Jour"), wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Calendrier.png"), wx.BITMAP_TYPE_ANY), wx.NullBitmap, wx.ITEM_NORMAL, _("Afficher le jour"), "")
        self.AddSeparator()
        self.AddLabelTool(ID_GO_ARRIERE,      _("Reculer"), wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Reculer.png"), wx.BITMAP_TYPE_ANY), wx.NullBitmap, wx.ITEM_NORMAL, _("Reculer"), "")
        self.AddLabelTool(ID_GO_AVANT,          _("Avancer"), wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Avancer.png"), wx.BITMAP_TYPE_ANY), wx.NullBitmap, wx.ITEM_NORMAL, _("Avancer"), "")
...
##        self.drawing_area.print_setup(event)
    ...
    def ExportImage(self, event):
        ...
        dialog = wx.FileDialog(self, message=_("Exporter au format image"),
                               wildcard=wildcard, style=wx.FD_SAVE)
        if dialog.ShowModal() == wx.ID_OK:
            path, extension = _extend_path(dialog.GetPath(), extensions, "png")
            if os.path.exists(path) :
                reponse = _ask_question(_("Le fichier existe déjà. Voulez-vous l'écraser ?"), self) 
                if reponse == False :
                    return
...
class DrawingArea(wx.Panel):
    ...
    def print_timeline(self, event):
        ...
        if not printer.Print(frame, printout, True):
            if printer.GetLastError() == wx.PRINTER_ERROR:
                wx.MessageBox(_("Problème d'impression. Peut-être votre imprimante n'est-elle pas configurée correctement ?"), "Impression", wx.OK)
        else:
    ...
    def print_preview(self, event):
        ...
        if not etat:
            logging.debug(_("Impossible d'afficher l'aperçu avant impression...\n"))
            return
        frame = wx.GetApp().GetTopWindow()
        pfrm = wx.PreviewFrame(self.preview, frame, _("Aperçu avant impression"))
        pfrm.Initialize()
...
##            print e.message
    ...
    def _window_on_right_down(self, evt):
        ...
        if self.context_menu_event == None or self.lectureSeule == True :
            return
        menu_definitions = (
            (_("Modifier"), self._context_menu_on_edit_event),
            (_("Supprimer"), self._context_menu_on_delete_event),
        )
...
####        self.show_balloons_on_hover = config.get_balloon_on_hover()
    ...
    def _delete_selected_events(self):
        ...
        if nbr_of_selected_events > 1:
            text = _("Confirmez-vous la suppression de %d évènements ?") % nbr_of_selected_events
        else:
            text = _("Confirmez-vous la suppression de l'évènement ?")
...
class GotoDateDialog(wx.Dialog):
    ...
    def __init__(self, parent, time):
        wx.Dialog.__init__(self, parent, title=_("Atteindre une date"))
        self._create_gui()
        self.dtpc.set_value(time)

    def _create_gui(self):
        self.dtpc = DateTimePicker(self)
        checkbox = wx.CheckBox(self, label=_("Afficher l'heure"))
...
class MyFrame(wx.Frame):
    ...
    def create_new_event(self, start=None, end=None):
        dialog = EventEditor(self, "Create Event", self.timeline, start, end)
        dialog.ShowModal() 
        dialog.Destroy()

    def edit_event(self, event):
        dialog = EventEditor(self, _("Edit Event"), self.timeline, event=event)
        dialog.ShowModal() 
        dialog.Destroy()
