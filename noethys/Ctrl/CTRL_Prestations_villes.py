...
class CTRL(HTL.HyperTreeList):
    def __init__(self, parent): 
        ...
        listeColonnes = [
            ( _("Famille/Individu/Prestations"), 250, wx.ALIGN_LEFT),
            ( _("Détail"), 250, wx.ALIGN_LEFT),
            ( _("Montant"), 70, wx.ALIGN_RIGHT),
            ( _("Qté"), 60, wx.ALIGN_CENTER),
            ]
        numColonne = 0
    ...
    def Importation(self, listeActivites=[], presents=None, listeVilles=None):
        ...
        for IDprestation, date, categorie, label, montant, IDactivite, IDfamille, IDindividu, IDcategorie_tarif, nom, prenom, date_naiss, nomActivite, IDcivilite in listeDonnees :
            ...
            if nomActivite == None : nomActivite = _("Nom d'activité inconnu")
    ...
    def Remplissage(self, listeActivites=[], presents=None, listeVilles=None):
        ...
        for nomTitulaires, IDfamille in listeFamilles :
            ...
            adresse = "%s %s %s" % (dictResultats[IDfamille]["rue"], dictResultats[IDfamille]["cp"], dictResultats[IDfamille]["ville"])
            montantStr = "%.2f %s" % (dictResultats[IDfamille]["montant"], SYMBOLE)
            ...
            for IDindividu, dictIndividu in dictResultats[IDfamille]["individus"].items() :
                ...
                if dictIndividu["date_naiss"] == None :
                    datenaiss_str = _("Sans date de naissance")
                else:
                    if DICT_CIVILITES[dictIndividu["IDcivilite"]]["sexe"] == "M" :
                        datenaiss_str = _("né le %s") % DateEngFr(dictIndividu["date_naiss"])
                    else:
                        datenaiss_str = _("née le %s") % DateEngFr(dictIndividu["date_naiss"])
                
                if dictIndividu["nom"] != None :
                    labelIndividu = "%s (%s)" % (nomIndividu, datenaiss_str)
                else :
                    labelIndividu = _("Prestations diverses")
                ...
                for label, dictPrestation in listeLabels :
                    ...
                    montantStr = "%.2f %s" % (dictPrestation["montant"], SYMBOLE)
        ...
        # Stats
        niveauStats = self.AppendItem(self.root, _("TOTAL"))
        ...
        detail = _("%d familles et %s individus") % (dictStats["familles"], dictStats["individus"])
        montantStr = "%.2f %s" % (dictStats["montant"], SYMBOLE)
        ...
        self.listeImpression["totaux"].append((_("TOTAL"), detail, montantStr, dictStats["nombre"]))
    ...
    def Imprimer(self, event=None):
        ...
        dataTableau.append( (_("Liste des prestations"), _("%s\nEdité le %s") % (UTILS_Organisateur.GetNom(), dateDuJour)) )
        style = TableStyle([
