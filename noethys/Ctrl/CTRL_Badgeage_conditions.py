...
class CTRL_Activite(wx.CheckListBox):
    ...
    def SetData(self, listeValeurs=[]):
        ...
        for ID, label, checked in listeValeurs:
            if label == None : label = "Activité inconnue"
...
class CTRL_Heure(wx.Panel):
    ...
    def Validation(self):
        ...
        if criteres == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une heure !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        return True
...
class CTRL_Poste(wx.ListBox):
    ...
    def Ajouter(self):
        dlg = wx.TextEntryDialog(self, _("Saisissez le nom du poste réseau :"), _("Saisie"), "")
    ...
    def Modifier(self):
        ...
        if index == -1 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun poste à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        dlg = wx.TextEntryDialog(self, _("Modifiez le nom du poste réseau :"), _("Modification"), valeur)
        if dlg.ShowModal() == wx.ID_OK:
            nom = dlg.GetValue()
            if nom == "":
                dlg = wx.MessageDialog(self, _("Le nom que vous avez saisi n'est pas valide."), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
    ...
    def Supprimer(self):
        ...
        index = self.GetSelection()
        if index == -1 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun poste à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer ce poste ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
...
class Page_Activite(wx.Panel):
    def __init__(self, parent):
        ...
        # Groupes
        self.check = wx.CheckBox(self, -1, _("Uniquement si l'individu est inscrit à l'une des activités cochées :"))
    ...
    def __set_properties(self):
        self.check.SetToolTip(wx.ToolTip(_("Cochez cette case si vous souhaitez appliquer un filtre sur les activités")))
    ...
    def Validation(self):
        ...
        if self.check.GetValue() == True :
            if self.GetValeur() == None :
                dlg = wx.MessageDialog(self, _("Vous n'avez coché aucune activité !"), "Erreur", wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        return True
...
class Page_Heure(wx.Panel):
    def __init__(self, parent):
        ...
        self.check = wx.CheckBox(self, -1, _("Uniquement si l'heure de badgeage..."))
    ...
    def __set_properties(self):
        self.check.SetToolTip(wx.ToolTip(_("Cochez cette case si vous souhaitez appliquer un filtre sur l'heure de badgeage")))
...
class Page_Periode(wx.Panel):
    def __init__(self, parent):
        ...
        # Caisses
        self.check = wx.CheckBox(self, -1, _("Uniquement sur les jours cochés :"))
    ...
    def __set_properties(self):
        self.check.SetToolTip(wx.ToolTip(_("Cochez cette case si vous souhaitez appliquer un filtre de périodes")))
    ...
    def GetValeur(self):
        if self.check.GetValue() == True :
            ...
            valeur = "%s-%s" % (jours_scolaires, jours_vacances)
        else:
            valeur = None
        return valeur
    ...
    def Validation(self):
        ...
        if self.check.GetValue() == True :
            ...
            if valeur == "None-None" :
                dlg = wx.MessageDialog(self, _("Vous n'avez coché aucun jour !"), "Erreur", wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        return True
...
class Page_Poste(wx.Panel):
    def __init__(self, parent):
        ...
        self.check = wx.CheckBox(self, -1, _("Uniquement sur les postes réseau suivants :"))
    ...
    def __set_properties(self):
        self.check.SetToolTip(wx.ToolTip(_("Cochez cette case pour appliquer un filtre sur les noms de postes réseau")))
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter un nom de poste")))
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier le nom de poste sélectionné dans la liste")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer le nom de poste sélectionné dans la liste")))
    ...
    def Validation(self):
        ...
        if self.check.GetValue() == True and self.GetValeur() == None :
            dlg = wx.MessageDialog(self, _("Vous avez coché la condition 'Poste' mais sans saisir de noms de poste !"), "Erreur", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        return True
...
class Page_Questionnaire(wx.Panel):
    def __init__(self, parent):
        ...
        # Cotisations
        self.check_filtres = wx.CheckBox(self, -1, _("Uniquement selon les filtres questionnaires suivants :"))
    ...
    def __set_properties(self):
        self.check_filtres.SetToolTip(wx.ToolTip(_("Cochez cette case pour appliquer des filtres sur les questionnaires individuels ")))
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter un filtre")))
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier le filtre sélectionné dans la liste")))
    ...
    def Validation(self):
        ...
        if self.check_filtres.GetValue() == True and len(self.ctrl_filtres.GetDonnees()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous avez coché la condition 'Filtres de questionnaire' mais sans saisir de filtre !"), "Erreur", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        return True
...
class Panel(wx.Panel):
    ...
    def CreationPages(self):
        ...
        self.listePages = [
            {"index" : 0, "code" : "activite", "ctrl" : Page_Activite(self), "label" : _("Activité")},
            {"index" : 1, "code" : "heure", "ctrl" : Page_Heure(self), "label" : _("Heure")},
            {"index" : 2, "code" : "periode", "ctrl" : Page_Periode(self), "label" : _("Période")},
            {"index" : 3, "code" : "poste", "ctrl" : Page_Poste(self), "label" : _("Poste")},
            {"index" : 4, "code" : "questionnaire", "ctrl" : Page_Questionnaire(self), "label" : _("Questionnaire")},
            ]
