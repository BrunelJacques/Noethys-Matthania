...
class Date(wx.TextCtrl):
    ...
    def OnContextMenu(self, event):
        ...
        listeFonctions = [
            ...
            (ID_SEMAINE_PRECEDENTE, "Semaine précédente"),
            (ID_SEMAINE_SUIVANTE, "Semaine suivante"),
            (None, None),
            (ID_MOIS_ACTUEL, "Mois actuel"),
            (ID_MOIS_PRECEDENT, "Mois précédent"),
            (ID_MOIS_SUIVANT, "Mois suivant"),
            (None, None),
            (ID_ANNEE_ACTUELLE, "Année actuelle"),
            (ID_ANNEE_PRECEDENTE, "Année précédente"),
            (ID_ANNEE_SUIVANTE, "Année suivante"),
        ]
        for id, label in listeFonctions:
            if id == None:
                ...
                sousMenu.AppendItem(wx.MenuItem(menuPop, id + 1, "Date de début"))
...
class Date2(wx.Panel):
    def __init__(self, parent, date_min="01/01/1910", date_max="01/01/2030",
        ...
        self.bouton_calendrier.SetToolTip(
            wx.ToolTip("Cliquez ici pour sélectionner la date dans le calendrier"))
