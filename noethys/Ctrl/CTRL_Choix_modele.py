...
def zzzDemandeModele(categorie="",table="documents_modeles"):
    ...
    for IDmodele, nom, defaut in listeDonnees :
        ...
        if defaut == 1 :
            indexDefaut = index
        index += 1
    dlg = wx.SingleChoiceDialog(None, _("Veuillez sélectionner un modèle dans la liste :"), _("Sélection d'un modèle"), listeLabels, wx.CHOICEDLG_STYLE)
    if indexDefaut != None :
    ...
    if dlg.ShowModal() == wx.ID_OK:
        ...
        dlg.Destroy()
        dlg = wx.MessageDialog(None, _("Sans modèle, l'édition est annulée !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
        dlg.ShowModal()
        dlg.Destroy()
        return None
    return IDmodele
