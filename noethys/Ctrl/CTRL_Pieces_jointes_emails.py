...
class CTRL(ULC.UltimateListCtrl):
    ...
    def Supprimer(self, event):
        if self.GetSelectedItemCount() == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune pièce jointe à enlever de la liste !"), "Erreur", wx.OK| wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        index = self.GetFirstSelected()
        if self.listePieces[index]["obligatoire"] == True :
            dlg = wx.MessageDialog(self, _("Vous ne pouvez pas désélectionner cette pièce !"), "Erreur", wx.OK| wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
...
class MyFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        for x in range(0, 1) :
            listePieces.append(
                {"nom":_("Facture"), "extension":"pdf", "taille":None, "obligatoire":True},
                )
