...
class CTRL_Question(wx.Choice):
    ...
    def SetListe(self):
        ...
        self.liste_questions = self.UtilsQuestionnaires.GetQuestions(type="location_demande", avec_filtre=False)
        self.Clear()
        self.dictDonnees = {}
        index = 0
...
class CTRL(wx.Panel):
    def __init__(self, parent):
        ...
        self.ctrl_modele = CTRL_Choix_modele.CTRL_Choice(self, categorie="location_demande")
        ...
        self.label_questionnaire = wx.StaticText(self, -1, _("Stockage :"))
        self.checkbox_questionnaire = wx.CheckBox(self, -1, _("Enregistrer une copie unique dans un porte-documents :"))
        ...
        param = UTILS_Config.GetParametre("impression_locations_demandes_repertoire", defaut="")
        if param != "" :
            self.checkbox_repertoire.SetValue(True)
            self.ctrl_repertoire.SetValue(param)

        param = UTILS_Config.GetParametre("impression_locations_demandes_questionnaire", defaut="")
        if param != "" :
    ...
    def __set_properties(self):
        self.ctrl_modele.SetToolTip(wx.ToolTip(_("Sélectionnez le modèle")))
        self.bouton_modele.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à la gestion des modèles")))
        self.checkbox_repertoire.SetToolTip(wx.ToolTip(_("Cochez cette case pour enregistrer un exemplaire de chaque document au format PDF dans le répertoire indiqué")))
        self.bouton_repertoire.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner un répertoire de destination")))
        self.checkbox_questionnaire.SetToolTip(wx.ToolTip(_("Cochez cette case pour enregistrer un exemplaire de chaque document au format PDF dans un porte-document du questionnaire")))
        self.ctrl_questionnaire.SetToolTip(wx.ToolTip(_("Sélectionnez la question de type 'porte-document' dans laquelle sera stocké le document")))
    ...
    def OnBoutonModele(self, event): 
        ...
        dlg = DLG_Modeles_docs.Dialog(self, categorie="location_demande")
        dlg.ShowModal() 
        dlg.Destroy()
    ...
    def MemoriserParametres(self):
        if self.checkbox_repertoire.GetValue() == True :
            UTILS_Config.SetParametre("impression_locations_demandes_repertoire", self.ctrl_repertoire.GetValue())
        else :
            UTILS_Config.SetParametre("impression_locations_demandes_repertoire", "")
        if self.checkbox_questionnaire.GetValue() == True :
            UTILS_Config.SetParametre("impression_locations_demandes_questionnaire", self.ctrl_questionnaire.GetID())
        else :
            UTILS_Config.SetParametre("impression_locations_demandes_questionnaire", "")
    ...
    def GetOptions(self):
        ...
        if self.checkbox_questionnaire.GetValue() == True :
            ...
            if questionnaire == None :
                dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner une question !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
