...
class CTRL_Affichage(wx.Choice):
    def __init__(self, parent, id=-1, item=None, track=None):
        ...
        if self.track.type == "conso" :
            self.SetItems([_("Toujours afficher"), _("Ne jamais afficher"), _("Afficher si ouvert")])
        else:
            self.SetItems([_("Toujours afficher"), _("Ne jamais afficher")])
        self.SetToolTip(wx.ToolTip(_("Sélectionnez un type d'affichage pour cette unité")))
        # Defaut
        self.SetDefaut() 
        # Bind
...
class CTRL(HTL.HyperTreeList):
    def __init__(self, parent): 
        ...
        listeColonnes = [
            ( _("Activité / Unité"), 180, wx.ALIGN_LEFT),
            ( _("Abrégé"), 50, wx.ALIGN_LEFT),
            ( _("Type"), 100, wx.ALIGN_LEFT),
            ( _("Affichage"), LARGEUR_COLONNE_AFFICHAGE, wx.ALIGN_LEFT),
##            ( _("Ordre"), 40, wx.ALIGN_CENTER),
            ]
        numColonne = 0
    ...
    def OnContextMenu(self, event):
        ...
        # Item Descendre
        item = wx.MenuItem(menuPop, 30, _("Réinitialiser"))
...
##                index += 1
    ...
    def Monter(self, event):
        ...
        if track == None or type(track) == int :
            dlg = wx.MessageDialog(self, _("Nous n'avez sélectionné aucune unité à déplacer !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        if track.position == 0 :
            dlg = wx.MessageDialog(self, _("Cette unité est déjà en première position !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Descendre(self, event):
        ...
        if track.position == len(self.dictTracksFinal[track.IDactivite])-1 :
            dlg = wx.MessageDialog(self, _("Cette unité est déjà en dernière position !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        listeTemp = self.dictTracksFinal[track.IDactivite]
    ...
    def Reinit(self, event):
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment réinitialiser la liste des unités ?"), _("Réinitialisation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
