...
class Ligne_prestation(object):
    def __init__(self, grid=None, donnees={}):
        ...
        if self.IDfacture == None or self.IDfacture == "" :
            self.label_facture = _("Non facturé")
        else:
...
class EditeurMontant(gridlib.PyGridCellEditor):
    ...
    def EndEdit(self, row, col, grid, oldVal):
        ...
        if valeur != None :
            ...
            if resteAVentiler < 0 :
                dlg = wx.MessageDialog(grid, _("Le montant saisi ne peut pas être supérieur au montant à ventiler !"), _("Erreur"), wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                valeur = None
        # Renvoie la valeur
...
class CTRL(wx.Panel):
    def __init__(self, parent, IDcompte_payeur=None, IDreglement=None):
        ...
        self.hyper_automatique = Hyperlien(self, label=_("automatiquement"), infobulle=_("Cliquez ici pour ventiler automatiquement le crédit restant"), URL="automatique")
    ...
    def MAJinfos(self, erreur=None):
        ...
        if self.montant_reglement == Ftd(0.0) :
            ...
            self.ctrl_info.SetLabel(_("Le montant de ce règlement est Nul !"))
            #return
        ...
        if self.negatif == False:
            ...
            if creditAVentiler == Ftd(0.0) :
                self.validation = "ok"
                label = _("Ventilation complète !")
            elif creditAVentiler > Ftd(0.0) :
                self.validation = "addition"
                label = _("Reste à ventiler %.2f %s !") % (creditAVentiler, SYMBOLE)
            elif creditAVentiler < Ftd(0.0) :
    ...
    def Validation(self):
        ...
        if self.validation == "addition" :
            ...
            if creditAVentiler > totalRestePrestationsAVentiler :
                creditAVentiler = totalRestePrestationsAVentiler
            if creditAVentiler > Ftd(0.0) :
                dlg = wx.MessageDialog(self, _("Vous devez encore ventiler %.2f %s.\n\nEtes-vous sûr de quand même vouloir valider et fermer ?") % (creditAVentiler, SYMBOLE), _("Confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
                if self.negatif: dlg = wx.MessageDialog(self, _("Vous avez ventilé %.2f %s en trop !\n\nEtes-vous sûr de quand même vouloir valider et fermer ?") % (creditAVentiler, SYMBOLE), _("Confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
                reponse = dlg.ShowModal()
                dlg.Destroy()
                if reponse !=  wx.ID_YES :
                    return False
        if self.validation == "trop" :
            dlg = wx.MessageDialog(self, _("Vous avez ventilé %.2f %s en trop !\n\nEtes-vous sûr de quand même vouloir valider et fermer ?") % (creditAVentiler, SYMBOLE), _("Confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
        ...
        if self.validation == "erreur" :
            dlg = wx.MessageDialog(self, _("La ventilation n'est pas valide. Veuillez la vérifier..."), _("Erreur de saisie"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        return True
    ...
    def VentilationAuto(self):
        ...
        if presenceNegatif:
                dlg = wx.MessageDialog(None, _("Présence de prestations aux valeurs négatives !\n\nLa ventilation automatique affecte prioritairement tous les montants négatifs !"), _("Information"), wx.OK | wx.ICON_INFORMATION)
                dlg.ShowModal()
                dlg.Destroy()
