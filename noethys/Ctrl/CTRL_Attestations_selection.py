    ...
    for IDprestation, IDcompte_payeur, date, categorie, label, montant_initial, montant, tva, IDactivite, nomActivite, abregeActivite, IDtarif, nomTarif, nomCategorieTarif, IDfacture, IDindividu, IDfamille, forfait_date_debut, forfait_date_fin in listePrestationsTemp :
        ...
        if isPrestationSelection(label, IDactivite) == True :
            ...
            if (IDcompte_payeur in dictComptes) == False :
                ...
                destinataire_ville = "%s %s" % (cp_resid, ville_resid)
...
class CTRL(HTL.HyperTreeList):
    def __init__(self, parent, date_debut=None, date_fin=None, dateNaiss=None, listeActivites=[], listePrestations=[], typeLabel=0): 
        ...
        listeColonnes = [
            ( _("Famille/Individu"), 270, wx.ALIGN_LEFT),
            ( _("Total"), 80, wx.ALIGN_RIGHT),
            ( _("Réglé"), 80, wx.ALIGN_RIGHT),
            ( _("Solde"), 80, wx.ALIGN_RIGHT),
            ]
        numColonne = 0
    ...
    def AfficheNbreComptes(self, nbreComptes=0):
        if self.parent.GetName() == "DLG_Attestations_selection" :
            if nbreComptes == 0 : label = _("Aucune attestation sélectionnée")
            elif nbreComptes == 1 : label = _("1 attestation sélectionnée")
            else: label = _("%d attestations sélectionnées") % nbreComptes
    ...
    def GetDonnees(self, dictOptions={}, infosSignataire={}):
        ...
        for IDcompte_payeur, dictCompte in self.dictComptes.items() :
            if IDcompte_payeur in dictCoches :
                ...
                dictCompte["numero"] = _("Attestation n°%06d") % num_attestation
                dictCompte["num_attestation"] = num_attestation
                dictCompte["{NUM_ATTESTATION}"] = "%06d" % num_attestation
                ...
                if infosSignataire["sexe"] == "H" :
                    dictCompte["{SIGNATAIRE_GENRE}"] = ""
                else:
                    dictCompte["{SIGNATAIRE_GENRE}"] = "e"
    ...
    def Remplissage(self):
        ...
        for nomSansCivilite, IDcompte_payeur in listeNomsSansCivilite :
            ...
            self.SetItemText(niveauCompte, "%.02f %s" % (total, SYMBOLE), 1)
            self.SetItemText(niveauCompte, "%.02f %s" % (ventilation, SYMBOLE), 2)
            self.SetItemText(niveauCompte, "%.02f %s" % (total-ventilation, SYMBOLE), 3)
            ...
            for nomIndividu, IDindividu in listeIndividus :
                ...
                self.SetItemText(niveauIndividu, "%.02f %s" % (total, SYMBOLE), 1)
                self.SetItemText(niveauIndividu, "%.02f %s" % (ventilation, SYMBOLE), 2)
                self.SetItemText(niveauIndividu, "%.02f %s" % (total-ventilation, SYMBOLE), 3)
    ...
    def OnContextMenu(self, event):
        ...
        item = wx.MenuItem(menuPop, 10, _("Ouvrir la fiche famille de %s") % nomIndividu)
    ...
    def OuvrirFicheFamille(self, event):
        ...
        dictItem = self.GetMainWindow().GetItemPyData(item)
        if dictItem == None :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune famille dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        type = dictItem["type"]
    ...
    def ImpressionListe(self):
        ...
        def Header():
            ...
            dataTableau.append( (_("Liste des attestations"), _("%s\nEdité le %s") % (UTILS_Organisateur.GetNom(), dateDuJour)) )
            style = TableStyle([
