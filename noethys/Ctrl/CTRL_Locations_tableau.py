...
class Barre(object):
    ...
    def GetTexteStatusBar(self):
        ...
        texte = _("%s -> %s (de %s à %s)") % (self.nomTitulaires, self.parent.dict_produits[self.IDproduit]["nom"], debut, fin)
        return texte
...
class CTRL_Tableau(wx.Panel):
    ...
    def MajTexteInfos2(self):
        if self.barre_selectionnee != None:
            ...
            if self.barre_selectionnee.date_fin.year == 2999 :
                date_fin = _("Illimitée")
                duree = ""
            else :
                date_fin = self.barre_selectionnee.date_fin.strftime("%d/%m/%Y %Hh%M")
                duree = "(%s) " % UTILS_Dates.FormatDelta(self.barre_selectionnee.date_fin - self.barre_selectionnee.date_debut)
            ...
            texte_infos = "%s > %s %s: %s" % (date_debut, date_fin, duree, nom_produit)
        else :
            texte_infos = ""
    ...
    def OnRightClick(self, event):
        ...
        if case != None :
            ...
            if barre != None:
                ...
                # Item Dupliquer
                item = wx.MenuItem(menuPop, 40, _("Dupliquer"))
                ...
                # Item Recopier
                item = wx.MenuItem(menuPop, 50, _("Recopier sur plusieurs dates"))
                bmp = wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Dupliquer.png"), wx.BITMAP_TYPE_PNG)
                item.SetBitmap(bmp)
                menuPop.AppendItem(item)
    ...
    def OnMotion(self, event=None):
        ...
        if case != None :
            ...
            texte_infos = "%s : %s" % (heure.strftime("%d/%m/%Y %Hh%M"), nomProduit)
    ...
    def AfficheTooltip(self):
        ...
        if barre != None:
            texte = ""
            texte += "Description : %s" % (barre.description or "")
            ...
            texte = _("Aucune location n'est enregistrée à cette date.\n\n")
        self.tip.SetMessage(texte)

        # Pied du tooltip
        if barre != None:
            pied = _("Double-cliquez pour modifier")
        else :
            pied = _("Double-cliquez pour saisir une nouvelle location")
    ...
    def SaveBarre(self, barre=None):
        ...
        label_produit = "%s (%s)" % (self.dict_produits[barre.IDproduit]["nom"], self.dict_produits[barre.IDproduit]["nom_categorie"])
        periode = _("%s - %s") % (barre.date_debut.strftime("%d/%m/%Y %H:%M:%S"), barre.date_fin.strftime("%d/%m/%Y %H:%M:%S") if (barre.date_fin and barre.date_fin.year != 2999) else _("Illimitée"))
        texte_historique = _("Modification de la location ID%d : %s %s") % (barre.IDlocation, label_produit, periode)
