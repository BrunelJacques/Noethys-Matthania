...
class CTRL(wx.StaticBitmap):
    ...
    def Supprimer(self, sauvegarder=True):
        ...
        if self.modeDefaut == True :
            dlg = wx.MessageDialog(self, _("Aucune image n'est enregistrée !"), _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
            
        txtMessage = _("Souhaitez-vous vraiment supprimer cette image ?")
