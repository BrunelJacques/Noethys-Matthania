...
class TextCtrlCp(masked.TextCtrl):
    ...
    def ChoixVilles(self, cp, listeReponses):
        ...
        resultat = ""
        titre = _("Sélection d'une ville")
        nbreReponses = len(listeReponses)
        listeReponses.sort()
        message = str(nbreReponses) + _(" villes possèdent le code postal ") + str(cp) + _(". Double-cliquez sur\nle nom d'une ville pour la sélectionner :")
    ...
    def SetInfobulleVille(self):
        ...
        if cp == "" or cp == "     " :
            ...
            try :
                ...
                texte = _("Département : %s (%s)\nRégion : %s") % (nomDepartement, num_dep, nomRegion)
                self.SetToolTip(wx.ToolTip(texte))
            except :
                self.SetToolTip(wx.ToolTip(_("Le code postal saisi ne figure pas dans la base de données")))
...
class TextCtrlVille(wx.TextCtrl):
    ...
    def OnKillFocus(self, event):
        ...
        if nbreCodes == 0:
            dlg = wx.MessageDialog(self, _("Cette ville n'est pas répertoriée dans la base de données. \nVérifiez que vous n'avez pas fait d'erreur de saisie."), "Information", wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
    ...
    def ChoixCodes(self, ville, listeReponses):
        ...
        nbreReponses = len(listeReponses)
        listeReponses.sort()
        message = str(nbreReponses) + _(" villes portent le nom ") + str(ville) + _(". Double-cliquez sur\nle code postal d'une ville pour la sélectionner :")
        dlg = wx.SingleChoiceDialog(self, message, titre, listeReponses, wx.CHOICEDLG_STYLE)
        if dlg.ShowModal() == wx.ID_OK:
            resultat = dlg.GetStringSelection()
        dlg.Destroy()
...
class Adresse(wx.Panel):
    ...
    def __set_properties(self):
        ...
        self.ctrl_cp.SetToolTip(wx.ToolTip(_("Saisissez ici le code postal de la ville")))
        self.ctrl_ville.SetToolTip(wx.ToolTip(_("Saisissez ici le nom de la ville")))
        self.bouton_options.SetToolTip(wx.ToolTip(_("Cliquez ici pour rechercher une ville ou pour saisir \nmanuellement une ville non présente dans la base\nde données du logiciel")))
