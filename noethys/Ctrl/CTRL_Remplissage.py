...
class Case():
    ...
    def GetTexteInfobulle(self):
        ...
        if self.etat in ("reservation", "attente", "present") :
            ...
            if self.etat == "reservation" or self.etat == "present" : texte += _("Consommation réservée le %s\n") % date_saisie_FR
...
##            item = wx.MenuItem(menuPop, 10, _("Déverrouiller cette consommation"))
...
##        prt.SetHeader("Le ", type = "Date & Time", align=wx.ALIGN_RIGHT, indent = -1, colour = wx.NamedColour('BLACK'))
    ...
    def Apercu(self):
        ...
        if not preview.Ok():
            wx.MessageBox(_("Désolé, un problème a été rencontré dans l'aperçu avant impression..."), "Erreur", wx.OK)
            return
##        frm = UTILS_Printer.PreviewFrame(preview, None)
        frame = wx.GetApp().GetTopWindow() 
        frm = wx.PreviewFrame(preview, None, _("Aperçu avant impression"))
        frm.Initialize()
        frm.MakeModal(False)
