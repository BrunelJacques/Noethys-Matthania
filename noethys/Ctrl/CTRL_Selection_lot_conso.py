...
class CTRL_Individus(wx.CheckListBox):
    ...
    def MAJ(self):
        ...
        AND ((inscriptions.statut = 'ok') OR (inscriptions.statut Is Null)) AND inscriptions.IDactivite IS NOT NULL
        ...
        for IDindividu, nom, prenom in listeDonnees :
            ...
            listeValeurs.append((IDindividu, "%s %s" % (nom, prenom), checked))
...
class CTRL(wx.Panel):
    def __init__(self, parent, IDfamille=None, mode="ajout", listeDates=[], selectionIndividus=[], selectionActivite=None):
        ...
        self.radio_periode_1 = wx.RadioButton(self, -1, "")
        self.label_periode = wx.StaticText(self, -1, _("La période affichée dans la grille"))
        self.radio_periode_2 = wx.RadioButton(self, -1, "")
        ...
        # Individus
        self.staticbox_individus_staticbox = wx.StaticBox(self, -1, _("Individus"))
        self.ctrl_individus = CTRL_Individus(self, self.IDfamille, self.selectionIndividus)
        
        # Activité
        self.staticbox_activite_staticbox = wx.StaticBox(self, -1, _("Activité"))
        ...
        # Unités
        self.staticbox_unites_staticbox = wx.StaticBox(self, -1, _("Unités"))
        self.ctrl_unites = CTRL_Unites(self, self.selectionActivite)
        
        # Modes
        self.staticbox_modes_staticbox = wx.StaticBox(self, -1, _("Modes"))
        self.ctrl_modes = CTRL_Modes(self)
        self.ctrl_modes.SetMinSize((60, 40))
        
        # Etats
        self.staticbox_etats_staticbox = wx.StaticBox(self, -1, _("Etats"))
    ...
    def __set_properties(self):
        self.radio_periode_1.SetToolTip(wx.ToolTip(_("Cochez ici pour sélectionner la période affichée dans la grille des consommations")))
        self.radio_periode_2.SetToolTip(wx.ToolTip(_("Cochez ici pour sélectionner une autre période")))
        self.ctrl_individus.SetToolTip(wx.ToolTip(_("Cochez les individus souhaités")))
        self.ctrl_activite.SetToolTip(wx.ToolTip(_("Selectionnez l'activité souhaitée")))
        self.ctrl_groupes.SetToolTip(wx.ToolTip(_("Cochez les groupes souhaités")))
        self.ctrl_unites.SetToolTip(wx.ToolTip(_("Cochez les unités de consommations souhaitées")))
        self.ctrl_modes.SetToolTip(wx.ToolTip(_("Cochez les modes souhaités")))
        self.ctrl_etats.SetToolTip(wx.ToolTip(_("Cochez les etats souhaités")))
...
class MyFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        self.bouton_ok = CTRL_Bouton_image.CTRL(panel, texte=_("Ok"), cheminImage="Images/32x32/Valider.png")
