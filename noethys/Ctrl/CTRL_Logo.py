...
class CTRL(wx.StaticBitmap):
    ...
    def Menu(self, event):
        ...
        if self.mode == "ecriture" :
            ...
            # Item Ajouter
            item = wx.MenuItem(menuPop, 10, _("Importer une image"))
        ...
        # Item Visualiser
        item = wx.MenuItem(menuPop, 40, _("Visualiser"))
    ...
    def Ajouter(self, event=None):
        ...
        dlg = wx.FileDialog(
            self, message=_("Sélectionnez une image"),
...
##            imgPIL = imgPIL.resize((largeur, hauteur), Image.ANTIALIAS)
    ...
    def MAJ(self, event=None):
        ...
        position = (int(((largeurCadre/2.0) - (largeurImage/2.0))), int((hauteurCadre/2.0) - (hauteurImage/2.0)))
    ...
    def Supprimer(self, event=None):
        if self.imagewx == None :
            dlg = wx.MessageDialog(self, _("Il n'y a aucune image à supprimer !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        # Confirmation
        dlg = wx.MessageDialog(self, _("Confirmez-vous la suppression de cette image ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
    ...
    def Visualiser(self, event=None):
        if self.imagewx == None :
            dlg = wx.MessageDialog(self, _("Il n'y a aucune image à visualiser !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
