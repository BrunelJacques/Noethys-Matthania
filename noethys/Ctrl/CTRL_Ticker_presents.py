...
class CTRL(wx.Panel):
    ...
    def JoinListe(self, listeTemp=[]):
        if len(listeTemp) > 2 :
            return _("%s et %s") % (u", ".join(listeTemp[:-1]), listeTemp[-1])
        else :
            return _(" et ").join(listeTemp)
    ...
    def GetTexte(self, listeActivites=[]):
        ...
        for IDactivite in listeActivites :
            ...
            for IDgroupe, dictGroupe in dictTemp[IDactivite]["groupes"].items() :
                label = "%d %s" % (dictGroupe["nbre"], dictGroupe["nom"])
            ...
            if nbre == 1 :
                ...
                temp = _("individus")
            listeTextes.append(_("%d %s sur l'activité %s (%s)") % (nbre, temp, nomActivite, self.JoinListe(groupes)))
        
        texte = _("Il y a actuellement %s") % self.JoinListe(listeTextes)
        return texte
