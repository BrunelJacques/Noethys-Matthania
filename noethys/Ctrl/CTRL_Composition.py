    ...
    def GetInfosIndividus(self):
        ...
        for IDindividu in listeIDindividus :
            ...
            if tel_domicile != None :
                dictInfos[IDindividu]["tel_domicile_complet"] = _("Tél. domicile : %s") % tel_domicile
            else:
                dictInfos[IDindividu]["tel_domicile_complet"] = None
            tel_mobile = dictInfos[IDindividu]["tel_mobile"] 
            if tel_mobile != None :
                dictInfos[IDindividu]["tel_mobile_complet"] = _("Tél. mobile : %s") % tel_mobile
            else:
            ...
            if travail_tel != None :
                dictInfos[IDindividu]["travail_tel_complet"] = _("Tél. travail : %s") % travail_tel
            else:
        ...
        if datenaiss != None :
            try :
                ...
                if dictInfos[IDindividu]["genre"] == "M" :
                    txtDatenaiss = _("Né le %s (%d ans)") % (datenaissFR, age)
                else:
                    txtDatenaiss = _("Née le %s (%d ans)") % (datenaissFR, age)
            except :
                pass
        return txtDatenaiss
...
##        self.DoDrawing(self.pdc)
    ...
    def Calendrier_selection(self):
        ...
        if IDindividu == None :
            dlg = wx.MessageDialog(self, _("Vous devez d'abord sélectionner un individu dans le cadre Composition !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return   

        if self.dictValeurs.dictInfosIndividus[IDindividu]["inscriptions"] == False :
            dlg = wx.MessageDialog(self, _("L'individu sélectionné n'est inscrit à aucune activité !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return   
    ...
    def OnContextMenu(self, event):
        ...
        if IDindividu != None :
            ...
            id = wx.Window.NewControlId()
            item = wx.MenuItem(menu, id, _("Détacher ou supprimer"))
            ...
            item = wx.MenuItem(sousMenuCategorie, 601, _("Représentant"), kind=wx.ITEM_RADIO)
            ...
            if self.dictCadres[self.IDindividu_menu]["categorie"] == 1 :
                id = wx.Window.NewControlId()
                item = wx.MenuItem(menu, id, _("Définir comme titulaire"), kind=wx.ITEM_CHECK)
                menu.Append(item)
            ...
            if self.dictCadres[self.IDindividu_menu]["categorie"] == 1 :
                id = wx.Window.NewControlId()
                item = wx.MenuItem(menu, id, _("Définir correspondant famille"), kind=wx.ITEM_CHECK)
                menu.Append(item)
    ...
    def On_SetCorrespondant(self, event):
        if self.dictCadres[self.IDindividu_menu]["correspondant"]:
            ...
            dlg = wx.MessageDialog(self, _("Déjà le correspondant de cette famille !\npour changer choisissez un autre titulaire"),
...
##                    self.SetItemBackgroundColour(brancheIndividu, wx.Colour(251, 212, 239))
    ...
    def OnContextMenu(self, event):
        ...
        if IDindividu != None :
            ...
            menu.AppendMenu(wx.Window.NewControlId(), _("Changer de catégorie"), sousMenuCategorie)
    ...
    def OuvrirCalendrier(self, IDindividu=None):
        ...
        dictItem = self.GetMainWindow().GetItemPyData(item)
        if dictItem == None :
            dlg = wx.MessageDialog(self, _("Vous devez d'abord sélectionner un individu dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return   
        type = dictItem["type"]
