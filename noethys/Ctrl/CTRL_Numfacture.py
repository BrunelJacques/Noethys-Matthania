...
class CTRL(wx.SearchCtrl):
    def __init__(self, parent, size=(-1,20), IDfamille=None):
        ...
        self.SetDescriptiveText(_("N° de facture"))
    ...
    def Recherche(self, event):
        ...
        # except :
        #     dlg = wx.MessageDialog(self, _("Ce numéro de facture n'est pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
        #     dlg.ShowModal()
        #     dlg.Destroy()
        #     return
        self.ReglerFacture(numFacture)
    ...
    def ReglerFacture(self, numFacture=None):
        if self.IDfamille != None :
            texteSupp = _("pour cette famille ")
        ...
        try :
            if "-" in numFacture :
                ...
                numero = int(numero)
                conditionNumero = "WHERE factures_prefixes.prefixe='%s' AND factures.numero=%d" % (prefixe, numero)
            else :
                numero = int(numFacture)
                conditionNumero = "WHERE factures.numero=%d" % numero
        except :
            conditionNumero = None

        if conditionNumero == None :
            dlg = wx.MessageDialog(self, _("Ce numéro de facture ne semble pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
...
##                dictPrestations[IDfacture] = totalPrestations
        ...
        if len(listeDonnees) == 0 :
            dlg = wx.MessageDialog(self, _("Ce numéro ne correspond à aucune facture existante %s!") % texteSupp, _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if etat == "annulation" :
            dlg = wx.MessageDialog(self, _("La facture n°%s a été annulée !") % numFacture, _("Facture annulée"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if totalActuel - regleActuel == 0.0 :
            dlg = wx.MessageDialog(self, _("La facture n°%s a déjà été réglée en intégralité !") % numFacture, _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
...
class Dialog(wx.Dialog):
    def __init__(self, parent, id=-1, title=_("Régler une facture"), IDfamille=None):
        ...
        self.label = wx.StaticText(self, -1, _("Veuillez saisir le numéro de la facture à régler ou\nscannez directement le code-barre sur la facture :"))
