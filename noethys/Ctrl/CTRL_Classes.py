...
class CTRL(HTL.HyperTreeList):
    def __init__(self, parent, id=wx.ID_ANY, pos=wx.DefaultPosition,
        ...
        self.AddColumn(_("Saison / Classe"))
        self.SetColumnWidth(0, 370)
        self.AddColumn(_("Niveaux scolaires"))
        self.SetColumnWidth(1, 120)
        self.SetMainColumn(0)
                                
        # Création des branches
        self.root = self.AddRoot(_("Classes"))
    ...
    def CreationBranches(self):
        ...
        for saison in listeSaisons :
            nomSaison = _("Du %s au %s") % (DateEngFr(str(saison[0])), DateEngFr(str(saison[1])) )
    ...
    def Modifier(self, event=None):
        ...
        if type != "classe" : 
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune classe à modifier !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return        
    ...
    def Supprimer(self, event=None):
        ...
        if type != "classe" : 
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune classe à supprimer !"), _("Erreur"), wx.OK | wx.ICON_QUESTION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        DB.Close()
        if nbre > 0 :
            dlg = wx.MessageDialog(self, _("Cette classe a déjà été attribuée %d fois.\n\nVous ne pouvez donc pas la supprimer !") % nbre, _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return

        # Confirmation de suppression
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cette classe ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
