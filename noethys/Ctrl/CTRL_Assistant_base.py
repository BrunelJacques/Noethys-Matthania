...
class CTRL(object):
    ...
    def Validation(self):
        if self.obligatoire == True and self.GetValeur() in (None, "", 0) :
            dlg = wx.MessageDialog(self, _("Merci de répondre à la question suivante :\n\n%s") % self.titre, _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.SetFocus()
            return False
...
class CTRL_Oui_non(CTRL, wx.Panel):
    def __init__(self, parent, *args, **kwds):
        CTRL.__init__(self, parent, *args, **kwds)
        wx.Panel.__init__(self, parent, id=-1, style=wx.TAB_TRAVERSAL)
        self.radio_oui = wx.RadioButton(self, -1, _("Oui"), style=wx.RB_GROUP)
        self.radio_non = wx.RadioButton(self, -1, _("Non"))
...
class Page_exemple(Page):
    def __init__(self, parent):
        ...
        self.Ajouter_question(code="nom", titre=_("Quel est le nom du séjour ?"), commentaire=_("Exemple : 'Séjour neige - Février 2018'"), ctrl=CTRL_Texte)
        self.Ajouter_question(code="groupes", titre=_("Le séjour est-il constitué de groupes d'âge ?"), commentaire=_("Commentaire"), ctrl=CTRL_Oui_non)
        self.Ajouter_question(code="date_debut", titre=_("Quelle est la date de début du séjour ?"), commentaire=None, ctrl=CTRL_Date)
        self.Ajouter_rubrique(titre=_("Inscriptions"))
        self.Ajouter_question(code="nbre_inscrits_max", titre=_("Quel est le nombre maximal d'inscrits ?"), commentaire=_("S'il n'y aucune limite, laissez sur 0"), ctrl=CTRL_Nombre)
        self.Ajouter_question(ctrl=CTRL_Html, texte=_("Ceci est un texte HTML"), size=(-1, 50))


    def MAJ(self):
        dict_valeurs = {
            "nom" : _("Séjour aventure - Juillet 2018"),
            }
...
class Page_responsable(Page):
    def __init__(self, parent):
        ...
        self.Ajouter_rubrique(titre=_("Le responsable du séjour"))
        self.Ajouter_question(code="responsable_nom", titre=_("Quel est le nom complet du responsable de l'activité (Prénom et nom) ?"), commentaire=_("Exemple : 'Jean-Louis DUPOND'"), ctrl=CTRL_Texte, obligatoire=False)
        self.Ajouter_question(code="responsable_fonction", titre=_("Quelle est sa fonction ?"), commentaire=_("Exemple : 'Directeur'"), ctrl=CTRL_Texte, obligatoire=False)
        self.Ajouter_question(code="responsable_sexe", titre=_("Homme ou femme ?"), choix=[("H", _("Homme")), ("F", _("Femme"))], ctrl=CTRL_Radio, obligatoire=False)
...
class Page_renseignements(Page):
    def __init__(self, parent):
        Page.__init__(self, parent)
        self.Ajouter_rubrique(titre=_("Renseignements"))
        self.Ajouter_question(code="pieces", titre=_("Quelles sont les pièces à fournir par l'individu inscrit ?"), commentaire=_("Si la pièce souhaitée n'apparaît pas, vous devez d'abord la paramétrer dans le menu Paramétrage."), ctrl=CTRL_Pieces)
        self.Ajouter_question(code="cotisations", titre=_("Si l'individu doit être à jour de l'une des cotisations suivantes, cochez-les ci-dessous :"), commentaire=_("Si la cotisation souhaitée n'apparaît pas, vous devez d'abord la paramétrer dans le menu Paramétrage."), ctrl=CTRL_Cotisations)
        self.Ajouter_question(code="renseignements", titre=_("Quels renseignements doivent être fournis par l'individu inscrit ?"), commentaire=None, ctrl=CTRL_Renseignements)
...
class Page_recopier_tarifs(Page):
    def __init__(self, parent):
        ...
        self.Ajouter_question(code="recopier_tarifs", titre=_("Souhaitez-vous recopier la tarification d'une activité existante ?"), commentaire=_("Sélectionnez une activité du même type dans la liste déroulante, sa tarification sera récupérée pour votre nouvelle activité."), choix=liste_choix, ctrl=CTRL_Choix, defaut=None)
...
class Dialog(wx.Dialog):
    def __init__(self, parent, page_introduction=None):
        ...
        intro = _("Cette fonctionnalité vous permet de bénéficier d'un paramétrage semi-automatisé d'une activité d'un type donné. Vous n'avez qu'à simplement répondre au questionnaire proposé pour générer facilement votre nouvelle activité.")
        titre = _("Assistant de paramétrage d'une activité")
        self.SetTitle(titre)
        ...
        #self.bouton_options = CTRL_Bouton_image.CTRL(self, texte=_("Paramètres avancés"), cheminImage="Images/32x32/Configuration2.png")
    ...
    def __set_properties(self):
        ...
        #self.bouton_options.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder aux paramètres avancés")))
    ...
    def Afficher_page(self, ctrl=None):
        ...
        if ctrl.__name__ == "Page_conclusion" :
            self.bouton_suite.SetImageEtTexte(Chemins.GetStaticPath("Images/32x32/Valider.png"), _("Valider"))
        else:
            self.bouton_suite.SetImageEtTexte(Chemins.GetStaticPath("Images/32x32/Fleche_droite.png"), _("Suite"))
    ...
    def Sauvegarde_standard(self, DB=None):
        ...
        if "has_groupes" in self.dict_valeurs:
            ...
            if self.dict_valeurs["has_groupes"] == False:
                listeDonnees = [("IDactivite", IDactivite), ("nom", _("Groupe unique")), ("ordre", 1), ("abrege", _("UNIQ"))]
            ...
            if self.dict_valeurs["has_groupes"] == True:
                ...
                for index in range(1, nbreGroupes+1):
                    ...
                    if "capacite_max_groupe#%d" % index in self.dict_valeurs:
                        nbrMaxi_groupe = self.dict_valeurs["capacite_max_groupe#%d" % index]
                    else :
                        nbrMaxi_groupe = None
                    listeDonnees = [("IDactivite", IDactivite), ("nom", nom_groupe), ("ordre", index+1), ("abrege", abrege_groupe), ("nbrMaxi", nbrMaxi_groupe)]
                    IDgroupe = DB.ReqInsert("groupes", listeDonnees)
                    listeIDgroupe.append(IDgroupe)
