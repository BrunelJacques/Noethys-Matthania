...
class CTRL(TC.ThumbnailCtrl):
    ...
    def SaisirLabel(self, label=u"", nomFichier=None):
        if nomFichier == None :
            message = _("Vous pouvez saisir un titre pour ce document (optionnel) :")
        else:
            message = _("Vous pouvez saisir un titre pour le document '%s' (optionnel) :") % nomFichier
        dlg = wx.TextEntryDialog(self, message, _("Titre du document"), label)
...
Tous les fichiers (*.*)|*.*"
        ...
        dlg = wx.FileDialog(
            self, message=_("Sélectionnez un ou plusieurs documents"),
    ...
    def SupprimerPage(self, event):
        ...
        if index == -1 : 
            dlg = wx.MessageDialog(self, _("Vous devez d'abord sélectionner un document !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        # Confirmation
        dlg = wx.MessageDialog(self, _("Confirmez-vous la suppression de ce document ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
        reponse = dlg.ShowModal()
        dlg.Destroy()
    ...
    def ContextMenu(self):
        ...
        item = wx.MenuItem(menu, ID_AJOUTER, _("Ajouter des documents"))
        ...
        item = wx.MenuItem(menu, ID_ROTATION_GAUCHE, _("Pivoter à gauche"))
        ...
        item = wx.MenuItem(menu, ID_ROTATION_DROITE, _("Pivoter à droite"))
        ...
        item = wx.MenuItem(menu, ID_VISUALISER, _("Visualiser"))
        ...
        item = wx.MenuItem(menu, ID_MODIFIER_LABEL, _("Modifier le titre"))
        ...
        item = wx.MenuItem(menu, ID_SUPPRIMER, _("Supprimer"))
    ...
    def RotationGauche(self, event=None):
        index = self.GetSelection()
        if index == -1 : 
            dlg = wx.MessageDialog(self, _("Vous devez d'abord sélectionner une image !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        thumb = self.GetItem(index)
        ...
        if track.isImage == False : 
            dlg = wx.MessageDialog(self, _("Ce document n'est pas une image !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def VisualiserPage(self, event=None):
        ...
        if track.isImage == True :
            ...
            try:
                ...
                print(err)
                dlg = wx.MessageDialog(self, _("Ouverture du document impossible !\n\nVérifiez si le document n'est pas déjà ouvert en arrière-plan."), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return
...
class MyFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        self.bouton_1 = CTRL_Bouton_image.CTRL(panel, texte=_("Ajouter"), cheminImage="Images/32x32/Valider.png")
        self.bouton_2 = CTRL_Bouton_image.CTRL(panel, texte=_("Bouton 2"), cheminImage="Images/32x32/Valider.png")
