...
class ListBoxCombinaisons(wx.ListBox):
    ...
    def Saisie(self, listeAnciennesIDunites=None):                
        ...
        dlg = wx.MultiChoiceDialog(self, _("Cochez les unités à combiner :"), _("Combinaisons d'unités"), listeItems)
...
class Panel(wx.Panel):
    def __init__(self, parent, IDactivite=None, IDtarif=None, nouveauTarif=True):
        ...
        self.label_combinaisons = wx.StaticText(self, -1, _("Saisissez les combinaisons conditionnelles :"))
        ...
        # Etats
        self.label_etats = wx.StaticText(self, -1, _("Cochez les états conditionnels :"))
    ...
    def __set_properties(self):
        self.ctrl_etats.SetToolTip(wx.ToolTip(_("Cochez les états de consommation associées à ce tarif")))
    ...
    def Validation(self):
        if len(self.ctrl_combinaisons.listeDonnees) == 0 :
            dlg = wx.MessageDialog(self, _("Aucune combinaison conditionnelle n'a été indiquée, confirmez-vous ?"), _("Information"), wx.YES_NO | wx.NO_DEFAULT | wx.CANCEL | wx.ICON_INFORMATION)
            if dlg.ShowModal() != wx.ID_YES:
                dlg.Destroy()
                return False
            else:
