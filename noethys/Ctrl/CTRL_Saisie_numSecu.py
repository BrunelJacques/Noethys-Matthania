...
GG : Clé
    ...
    def TestValidite(self, avecMessagesErreur=True):
        ...
        if validation == False :
            ...
            if self.remplissageEnCours == False and avecMessagesErreur == True:
                wx.MessageBox(message, _("Numéro de sécurité sociale erroné"))
    ...
    def ValideNumSecu(self, texte, sexe, date_naiss, cp_naiss):
        ...
        if nbreChiffres == 0 :
            return None, ""
        if nbreChiffres < 15 :
            message = _("Il manque ") + str(15 - nbreChiffres) + _(" chiffre(s) au numéro de sécurité sociale que vous venez de saisir. Veuillez le vérifier.")
        ...
        if nbreChiffres == 15:
            ...
            if sexe == "M" :
                if int(texteSansEsp[0]) != 1:
                    message = _("Le numéro de sécurité sociale ne correspond pas à la civilité de la personne (le premier chiffre devrait être 1).")
                    return False, message
            if sexe == "F" :
                if int(texteSansEsp[0]) != 2:
                    message = _("Le numéro de sécurité sociale ne correspond pas à la civilité de la personne (le premier chiffre devrait être 2).")
                    return False, message
            ...
            if date_naiss != "  /  /    " :
                ...
                if annee != str(texteSansEsp[1:3]):
                    message = _("Le numéro de sécurité sociale ne correspond pas à l'année de naissance de la personne.")
                    return False, message
                elif mois != str(texteSansEsp[3:5]):
                    message = _("Le numéro de sécurité sociale ne correspond pas au mois de naissance de la personne.")
                    return False, message
                        
            # Vérification avec le département de naissance
            if cp_naiss != "     " and cp_naiss != None :
                dep = cp_naiss[0:2]
                if str(dep) != str(texteSansEsp[5:7]):
                    message = _("Le numéro de sécurité sociale ne correspond pas au lieu de naissance de la personne.")
                    return False, message
...
class MyFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        self.bouton = wx.Button(panel, -1, _("Test"))
