...
class CTRL_Date_facturation(wx.Panel):
    def __init__(self, parent, listeChoix=[]):
        ...
        self.listeChoix.append(("date:", _("La date suivante")))
        
        choices = []
        for code, label in self.listeChoix :
            choices.append(label)
        self.ctrl_choix = wx.Choice(self, -1, choices=choices)
        self.ctrl_choix.SetToolTip(wx.ToolTip(_("Sélectionnez la date de facturation de la prestation")))
    ...
    def Validation(self):
        if self.GetCodeSelection() == "date:None" :
            dlg = wx.MessageDialog(self, _("Vous avez sélectionné la date de facturation 'La date suivante' mais sans sélectionner de date valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_date.SetFocus()
            return False
...
class Panel_type_sans_parametres(wx.Panel):
    def __init__(self, parent, nouveauTarif=False, texte=_("Aucun paramètre à renseigner.")):
        wx.Panel.__init__(self, parent, id=-1, style=wx.TAB_TRAVERSAL)
        self.parent = parent
...
##        liste_pages = [
##            ("SIMPLE", _("Prestation simple"), Panel_type_sans_parametres(self, self.nouveauTarif)),
##            ("JOURN", _("Prestation journalière"), CTRL_Tarification_journ.Panel(self, self.IDactivite, self.IDtarif, self.nouveauTarif)),
##            ("FORFAIT", _("Forfait daté"), CTRL_Tarification_forfait.Panel(self, self.IDactivite, self.IDtarif, self.nouveauTarif)),
##            ("CREDIT", _("Forfait crédit"), CTRL_Tarification_credit.Panel(self, self.IDactivite, self.IDtarif, self.nouveauTarif)),
##            ]
##        self.dictPages = {}
##        index = 0
...
class CTRL_Parametres(wx.Panel):
    def __init__(self, parent, IDactivite=None, IDtarif=None, nouveauTarif=False):
        ...
        self.liste_pages = [
            ("JOURN", _("Prestation journalière"), CTRL_Tarification_journ.Panel(self, IDactivite, IDtarif, nouveauTarif)),
            ("FORFAIT", _("Forfait daté"), CTRL_Tarification_forfait.Panel(self, IDactivite, IDtarif, nouveauTarif)),
            ("CREDIT", _("Forfait crédit"), CTRL_Tarification_credit.Panel(self, IDactivite, IDtarif, nouveauTarif)),
            ("BAREME", _("Barême de contrat"), Panel_type_sans_parametres(self, nouveauTarif)),
            ]
...
class Panel(wx.Panel):
    def __init__(self, parent, IDactivite=None, IDtarif=None, nouveauTarif=False):
        ...
        self.label_type = wx.StaticText(self, -1, _("Type de tarif :"))
