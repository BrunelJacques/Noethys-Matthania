...
class CTRL_Type(wx.Choice):
    def __init__(self, parent, id=-1, item=None, track=None):
        ...
        self.item = item
        self.track = track
        self.SetItems([_("Nombre d'unités consommées"), _("Temps réél de présence"), _("Temps de présence facturé"), _("Formule")])
        self.SetToolTip(wx.ToolTip(_("Sélectionnez le type de calcul à appliquer à cette unité de consommation")))
...
class CTRL_Arrondi(wx.Choice):
    def __init__(self, parent, id=-1, item=None, track=None):
        ...
        self.listeValeurs = [
            (_("Aucun"), None),
            (_("Durée : 5 min. sup."), ("duree", 5)),
            (_("Durée : 10 min. sup."), ("duree", 10)),
            (_("Durée : 15 min. sup."), ("duree", 15)),
            (_("Durée : 30 min. sup."), ("duree", 30)),
            (_("Durée : 60 min. sup."), ("duree", 60)),
            (_("Horaire : 5 min."), ("tranche_horaire", 5)),
            (_("Horaire : 10 min."), ("tranche_horaire", 10)),
            (_("Horaire : 15 min."), ("tranche_horaire", 15)),
            (_("Horaire : 30 min."), ("tranche_horaire", 30)),
            (_("Horaire : 60 min."), ("tranche_horaire", 60)),
            ]
...
class CTRL_Coeff(wx.TextCtrl):
    def __init__(self, parent, id=-1, item=None, track=None):
        ...
        self.item = item
        self.track = track
        self.SetToolTip(wx.ToolTip(_("Saisissez le coefficient multiplicateur à appliquer")))
...
class DLG_Saisie_formule(wx.Dialog):
    def __init__(self, parent, texte=""):
        ...
        # Texte
        self.staticbox_texte_staticbox = wx.StaticBox(self, -1, _("Formule python"))
        self.ctrl_texte = wx.TextCtrl(self, -1, "", style=wx.TE_MULTILINE )
        self.ctrl_texte.SetMinSize((600, 200))
        self.hyper_formule = Hyperlien(self, label=_("Insérer une unité"), infobulle=_("Cliquez ici pour insérer une unité de consommation"), URL="")

        formule_exemple = """Exemples de variables et de fonctions spéciales :
...
Exemples de formule :
        ...
        self.bouton_supprimer = CTRL_Bouton_image.CTRL(self, texte=_("Effacer"), cheminImage="Images/32x32/Gomme.png")
    ...
    def __set_properties(self):
        self.SetTitle(_("Saisie d'une formule"))
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à l'aide")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour effacer le contenu du texte")))
        self.bouton_ok.SetToolTip(wx.ToolTip(_("Cliquez ici pour valider ou tapez sur la touche Entrée du clavier")))
    ...
    def OnLeftLink(self, event):
        ...
        dlg = wx.SingleChoiceDialog(None, _("Sélectionnez une unité de consommation à insérer :"), _("Insérer une unité"), listeLabels, wx.CHOICEDLG_STYLE)
        dlg.SetSize((580, 450))
        dlg.CenterOnScreen()
        if dlg.ShowModal() == wx.ID_OK:
...
class CTRL(HTL.HyperTreeList):
    def __init__(self, parent): 
        ...
        listeColonnes = [
            ( _("Unité de consommation"), 225, wx.ALIGN_LEFT),
            ( _("Type de calcul"), 210, wx.ALIGN_LEFT),
            ( _("Coefficient"), 80, wx.ALIGN_LEFT),
            ( _("Formule"), 100, wx.ALIGN_LEFT),
            ( _("Arrondi"), 150, wx.ALIGN_LEFT),
            ( _("Durée seuil"), 90, wx.ALIGN_LEFT),
            ( _("Durée plafond"), 90, wx.ALIGN_LEFT),
            ( _("Heure seuil"), 90, wx.ALIGN_LEFT),
            ( _("Heure plafond"), 90, wx.ALIGN_LEFT),
            ]
        numColonne = 0
        ...
        for nomActivite, IDactivite in listeKeys :
            ...
            for track in listeTracks :
                ...
                if track.IDactivite == IDactivite :
                    ...
                    # CTRL durée seuil
                    ctrl_duree_seuil = CTRL_Heure(self.GetMainWindow(), item=brancheUnite, track=track, tooltip=_("Saisissez la durée seuil pour chaque consommation : La durée de chaque consommation ne pourra être inférieure à cette valeur"))
                    self.SetItemWindow(brancheUnite, ctrl_duree_seuil, 5)
                    track.ctrl_duree_seuil = ctrl_duree_seuil

                    # CTRL durée plafond
                    ctrl_duree_plafond = CTRL_Heure(self.GetMainWindow(), item=brancheUnite, track=track, tooltip=_("Saisissez la durée plafond pour chaque consommation : La durée de chaque consommation ne pourra être supérieure à cette valeur"))
                    self.SetItemWindow(brancheUnite, ctrl_duree_plafond, 6)
                    track.ctrl_duree_plafond = ctrl_duree_plafond

                    # CTRL heure seuil
                    ctrl_heure_seuil = CTRL_Heure(self.GetMainWindow(), item=brancheUnite, track=track, tooltip=_("Saisissez une heure seuil pour chaque consommation : La durée sera calculée uniquement à partir de cette heure-là"))
                    self.SetItemWindow(brancheUnite, ctrl_heure_seuil, 7)
                    track.ctrl_heure_seuil = ctrl_heure_seuil

                    # CTRL heure plafond
                    ctrl_heure_plafond = CTRL_Heure(self.GetMainWindow(), item=brancheUnite, track=track, tooltip=_("Saisissez une heure plafond pour chaque consommation : La durée sera calculée uniquement jusqu'à cette heure-là"))
            ...
            if self.IsItemChecked(track.item) :
                ...
                if typeCalcul == 0 :
                    ...
                    if track.ValidationCoeff() == False :
                        dlg = wx.MessageDialog(self, _("Le coefficient de l'unité '%s' semble incorrecte !") % track.nomUnite, _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                        dlg.ShowModal()
                        dlg.Destroy()
                        return False
                    ...
                    if track.GetDureeSeuil() == False :
                        dlg = wx.MessageDialog(self, _("Le seuil de l'unité '%s' semble incorrecte !") % track.nomUnite, _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                        dlg.ShowModal()
                        dlg.Destroy()
                        return False

                    if track.GetDureePlafond() == False :
                        dlg = wx.MessageDialog(self, _("Le plafond de l'unité '%s' semble incorrecte !") % track.nomUnite, _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                        dlg.ShowModal()
                        dlg.Destroy()
                        return False

                    if track.GetHeureSeuil() == False :
                        dlg = wx.MessageDialog(self, _("L'heure seuil de l'unité '%s' semble incorrecte !") % track.nomUnite, _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                        dlg.ShowModal()
                        dlg.Destroy()
                        return False

                    if track.GetHeurePlafond() == False :
                        dlg = wx.MessageDialog(self, _("L'heure plafond de l'unité '%s' semble incorrecte !") % track.nomUnite, _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                        dlg.ShowModal()
                        dlg.Destroy()
                        return False

                    if track.GetHeurePlafond() != None and track.GetHeureSeuil() != None and track.GetHeurePlafond() < track.GetHeureSeuil() :
                        dlg = wx.MessageDialog(self, _("L'heure plafond de l'unité '%s' doit obligatoirement être supérieure à l'heure seuil !") % track.nomUnite, _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                        dlg.ShowModal()
                        dlg.Destroy()
                        return False
...
class MyFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        self.boutonTest = wx.Button(panel, -1, _("Test"))
