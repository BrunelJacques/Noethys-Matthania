...
def Concatenation(listeTextes=[]):
    if len(listeTextes) == 0 : 
        ...
        return _("%s et %s") % (listeTextes[0], listeTextes[1])
    else :
        premiers = ", ".join(listeTextes[:-1])
        return _("%s et %s") % (premiers, listeTextes[-1])
...
##                listePages.append(texte)
    ...
    def GetGPSOrganisateur(self, cp="", ville=""):
        ...
        if dictGPS == None : 
            ...
            self.DB.ReqMAJ("organisateur", [("gps", "%s;%s" % (str(lat), str(long)) ),], "IDorganisateur", 1)
        return lat, long
    ...
    def GetCelebrations(self):
        ...
        try :
            ...
            texte = ""
            intro = _("<t>LES CELEBRATIONS DU JOUR</t>")
            if len(texteFetes) > 0 and len(texteCelebrations) > 0 : texte = _("%sNous fêtons aujourd'hui les %s et célébrons %s.") % (intro, texteFetes, texteCelebrations)
            if len(texteFetes) > 0 and len(texteCelebrations) == 0 : texte = _("%sNous fêtons aujourd'hui les %s.") % (intro, texteFetes)
            if len(texteFetes) == 0 and len(texteCelebrations) > 0 : texte = _("%sNous célébrons aujourd'hui %s.") % (intro, texteCelebrations)
            if texte == "" : 
                return None
            return texte
    ...
    def GetAnniversaires(self):
        ...
        try :
            condInscription = "%4d-01-01" % (self.dateJour.year-3) # participants depuis 3 ans
            ...
            WHERE inscriptions.date_inscription > '%s' AND date_naiss LIKE '%%%s' AND IDinscription IS NOT NULL
            GROUP BY individus.IDindividu
            ORDER BY date_naiss DESC
            ;""" %(condInscription, conditionJour)
            self.DB.ExecuterReq(req,MsgBox="ExecuterReq")
            listeDonnees = self.DB.ResultatReq()
            if len(listeDonnees) == 0 : return None
            listeTextes = []
            for IDindividu, nom, prenom, date_naiss in listeDonnees :
                ...
                listeTextes.append(_("%s %s (%d ans)") % (prenom, nom, age))
            texte = _("<t>LES ANNIVERSAIRES DU JOUR</t>Joyeux anniversaire à %s.") % Concatenation(listeTextes)
            return texte
    ...
    def GetCitation(self):
        ...
        texte = _("<t>LA CITATION DU JOUR</t>%s (%s)") % (citation, auteur)
        return texte
    ...
    def GetSoleil(self):
        ...
        try :
            ...
            texte = _("<t>HORAIRES DU SOLEIL</t>Aujourd'hui à %s, le soleil se lève à %dh%02d et se couche à %dh%02d.") % (ville.capitalize(), heureLever.hour, heureLever.minute, heureCoucher.hour, heureCoucher.minute)
            return texte
