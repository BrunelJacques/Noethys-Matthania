...
##                        pass
    ...
    def ModifierFichier(self, titre=""):
        ...
        if self.mode == "reseau" :
            dlg = wx.MessageDialog(self, _("Il est impossible de modifier le nom d'un fichier réseau !"), _("Désolé"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return

        # Demande de confirmation 1
        dlg = wx.MessageDialog(None, _("Souhaitez-vous vraiment modifier le nom du fichier '%s' ?") % titre, _("Modifier un fichier"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        ...
        dlg = wx.TextEntryDialog(self, _("Saisissez un nouveau nom pour le fichier '%s' :") % titre, _("Modifier le nom"), titre)
        ...
        if nouveauTitre == "" :
            dlg = wx.MessageDialog(self, _("Le nom que vous avez saisi ne semble pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return

        # Demande de confirmation 2
        dlg = wx.MessageDialog(None, _("Vous êtes vraiment sûr de vouloir changer le nom du fichier '%s' en '%s' ?") % (titre, nouveauTitre), _("Modifier un fichier"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
        reponse = dlg.ShowModal()
        dlg.Destroy()
        if reponse != wx.ID_YES :
            return 
    ...
    def SupprimerFichier(self, titre=""):
        ...
        dlg = wx.MessageDialog(None, _("Souhaitez-vous vraiment supprimer le fichier '%s' ?") % titre, _("Supprimer un fichier"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        reponse = dlg.ShowModal()
        dlg.Destroy()
        if reponse != wx.ID_YES :
            return 
        
        dlg = wx.MessageDialog(None, _("Attention, la suppression est irreversible !!! \n\n Vous êtes vraiment sûr de vouloir supprimer le fichier '%s' ?") % titre, _("Supprimer un fichier"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
        reponse = dlg.ShowModal()
        dlg.Destroy()
        if reponse != wx.ID_YES :
            return 
        ...
        if self.mode == "reseau" :
            ...
            if DB.echec == 1 :
                dlg = wx.MessageDialog(self, _("Erreur de connexion MySQL !\n\n%s") % DB.erreur, _("Erreur de connexion"), wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                return
