...
class CTRL(HTL.HyperTreeList):
    ...
    def MAJ(self):
        ...
        liste_colonnes = [
            {"code": "prestations", "label": _("Activité/Prestation"), "largeur": 280, "alignement": wx.ALIGN_LEFT},
            {"code": "tarif_unitaire", "label": _("Tarif unitaire"), "largeur": 100, "alignement": wx.ALIGN_CENTRE},
            {"code": "quantite", "label": _("Quantité"), "largeur": 90, "alignement": wx.ALIGN_CENTRE},
            {"code": "total", "label": _("Total"), "largeur": 100, "alignement": wx.ALIGN_CENTRE},
        ]
        numColonne = 0
        ...
        for IDactivite, dictActivite in dictResultats.items():
            if IDactivite in self.dictActivites :
                ...
                nomActivite = _("Cotisations")
            elif IDactivite == 90002 :
                nomActivite = _("Locations")
            elif IDactivite == 80000:
                nomActivite = _("Avoirs")
            else :
        ...
        for nomActivite, IDactivite, dictActivite in listeLabels :
            ...
            self.SetItemText(niveauActivite, "%.2f %s" % (dictActivite["total"], SYMBOLE), 3)
            ...
            impression_ligne = [nomActivite, str(quantite_activite), "%.2f %s" % (dictActivite["total"], SYMBOLE)]
            ...
            for label in liste_labels:
                ...
                if self.afficher_detail == True:
                    ...
                    for montant in liste_montants :
                        ...
                        self.SetItemText(niveauPrestation, "%.2f %s" % (montant or 0.0, SYMBOLE), 1)
                        self.SetItemText(niveauPrestation, str(dict_montant["quantite"]), 2)
                        self.SetItemText(niveauPrestation, "%.2f %s" % (dict_montant["total"] or 0.0, SYMBOLE), 3)
                        ...
                        impression_ligne = [label, str(dict_montant["quantite"]), "%.2f %s" % (dict_montant["total"], SYMBOLE)]
                        if self.afficher_detail:
                            impression_ligne.insert(1, "%.2f %s" % (montant or 0.0, SYMBOLE))
                        self.dictImpression["contenu"].append(impression_ligne)
                    ...
                    self.SetItemText(niveauPrestation, "%.2f %s" % (dict_label["total"], SYMBOLE), 3)
                    self.SetItemFont(niveauPrestation, wx.Font(7, wx.SWISS, wx.NORMAL, wx.NORMAL))
                    self.SetItemTextColour(niveauPrestation, wx.Colour(160, 160, 160))

                    impression_ligne = [label, str(dict_label["quantite"]), "%.2f %s" % (dict_label["total"], SYMBOLE)]
                    if self.afficher_detail:
                        impression_ligne.insert(1, "")
                    self.dictImpression["contenu"].append(impression_ligne)
        ...
        self.SetItemText(niveauTotal, "%.2f %s" % (total, SYMBOLE), 3)

        impression_ligne = [_("Total"), str(quantite), "%.2f %s" % (total, SYMBOLE)]
        if self.afficher_detail:
            impression_ligne.insert(1, "")
    ...
    def GetLabelParametres(self):
        return _("Dépôt : %s") % self.GetParent().ctrl_infos.GetLabel()
    ...
    def Imprimer(self):
        ...
        dataTableau.append( (_("Détail des prestations d'un dépôt"), _("%s\nEdité le %s") % (UTILS_Organisateur.GetNom(), dateDuJour)) )
        style = TableStyle([
    ...
    def ExportExcel(self):
        """ Export Excel """
        titre = _("Détail des prestations d'un dépôt")
