    ...
    def Initialisation(self):               
        ...
        for IDfamille, dictFamille in dictDonnees.items() :
            ...
            if len(list(dictDonnees.keys())) > 1 or len(dictFamille["forfaits"]) > 0 :
                ...
                label = _("Famille %s") % dictTitulaires[IDfamille]["titulairesSansCivilite"]
...
##        dc.DrawCircle(5, 9, 4)
    ...
    def Ajouter(self, event=None, date_debut=None, IDfamille=None):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            if nbreConso > 0 :
                dlg2 = wx.MessageDialog(None, _("Souhaitez-vous recalculer les prestations affichées (conseillé) ?"), _("Recalcul"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
    ...
    def Modifier(self, event=None):
        ...
        if dataItem == None or dataItem["type"] != "forfait" :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun forfait à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if donnees["IDfacture"] != None :
            dlg = wx.MessageDialog(self, _("Ce forfait apparaît déjà sur une facture. Il n'est pas possible de le modifier !"), _("Attention"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
...
##        dlg.ctrl_forfaits.Activation(False)
    ...
    def Supprimer(self, event=None):
        ...
        dataItem = self.GetItemPyData(item)
        if dataItem == None or dataItem["type"] != "forfait" :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun forfait à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        # Vérifie si le forfait est déjà facturé
        if donnees["IDfacture"] != None :
            dlg = wx.MessageDialog(self, _("Ce forfait apparaît déjà sur une facture. Il n'est pas possible de le supprimer !"), _("Attention"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if nbreConsoNonAffichees > 0 :
            dlg = wx.MessageDialog(self, _("Ce forfait contient %d consommation(s) qui ne sont pas affichées ici. Il n'est donc pas possible de le supprimer maintenant !") % nbreConsoNonAffichees, _("Attention"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        
        # Demande la confirmation de la suppression du forfait
        dlg = wx.MessageDialog(None, _("Souhaitez-vous vraiment supprimer ce forfait ?"), _("Suppression"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
...
class CTRL(wx.Panel):
    def __init__(self, parent, grille=None, listePrestations=[]):
        ...
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour saisir un forfait crédit")))
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier le forfait sélectionné dans la liste")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer le forfait sélectionné dans la liste")))
