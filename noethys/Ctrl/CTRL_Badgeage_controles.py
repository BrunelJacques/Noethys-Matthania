...
class CTRL_Barre_numerique(wx.SearchCtrl):
                ...
                if sw1 == 144 :
                    ...
                    if self.dernierRFID != IDbadge :
                        ...
                        if IDindividu != None :
                            ...
                            DIALOGUES.DLG_Message(self.interface, message=_("Ce numéro de badge RFID n'est pas répertorié !"), icone="erreur")
                            self.Start() 
    ...
    def OnEnter(self, evt):
        ...
        try :
            ...
            DIALOGUES.DLG_Message(self.interface, message=_("Ce numéro de badge ne semble pas valide !"), icone="erreur")
            return False
...
##            self.ValidationIdentification(IDindividu, mode="CB")
    ...
    def ValidationIdentification(self, IDindividu=None, mode="CLAVIER"):
        ...
        if resultat == False :
            DIALOGUES.DLG_Message(self.interface, message=_("Ce numéro de badge n'existe pas !"), icone="erreur")
            return False
        else :
            # Vérification que l'individu est inscrit
            if self.listeIndividus != None :
                if IDindividu not in self.listeIndividus :
                    DIALOGUES.DLG_Message(self.interface, message=_("Il n'y a pas d'inscription valide !"), icone="erreur")
                    return False
            # Lance la procédure
...
class CTRL_Liste_individus(ULC.UltimateListCtrl):
    ...
    def Initialisation(self):
        ...
        if listeActivites != None :
            ...
            req = """SELECT individus.IDindividu, IDcivilite, nom, prenom 
                        FROM individus 
                        LEFT JOIN inscriptions ON inscriptions.IDindividu = individus.IDindividu
                        WHERE (NOT inscriptions.statut LIKE 'ko%%') AND inscriptions.IDactivite IN %s
                        AND (date_desinscription IS NULL OR date_desinscription>='%s')
                        GROUP BY individus.IDindividu
                        ORDER BY nom, prenom;""" % (conditionActivites, self.interface.date)
        else :
        ...
        self.InsertColumn(0, _("Nom de l'individu"), width=400, format=ULC.ULC_FORMAT_LEFT)
    ...
    def Valider(self, event=None):
        ...
        if len(selections) == 0 :
            DIALOGUES.DLG_Message(self.interface, message=_("Vous devez d'abord sélectionner un individu dans la liste !"), icone="exclamation")
            return
...
class CTRL_Importation(wx.Panel):
    def __init__(self, interface):
        ...
        # Contrôles
        self.boutonTest = wx.Button(self, -1, _("Test"))
