...
class CTRL_Question(wx.Choice):
    ...
    def SetListe(self):
        ...
        self.liste_questions = self.UtilsQuestionnaires.GetQuestions(type="inscription", avec_filtre=False)
        self.Clear()
        self.dictDonnees = {}
        index = 0
...
class CTRL(wx.Panel):
    def __init__(self, parent):
        ...
        param = UTILS_Config.GetParametre("impression_inscriptions_repertoire", defaut="")
        if param != "" :
            self.checkbox_repertoire.SetValue(True)
            self.ctrl_repertoire.SetValue(param)

        param = UTILS_Config.GetParametre("impression_inscriptions_questionnaire", defaut="")
        if param != "" :
    ...
    def __set_properties(self):
        ...
        self.checkbox_repertoire.SetToolTip(wx.ToolTip(_("Cochez cette case pour enregistrer un exemplaire de chaque inscription au format PDF dans le répertoire indiqué")))
    ...
    def MemoriserParametres(self):
        if self.checkbox_repertoire.GetValue() == True :
            UTILS_Config.SetParametre("impression_inscriptions_repertoire", self.ctrl_repertoire.GetValue())
        else :
            UTILS_Config.SetParametre("impression_inscriptions_repertoire", "")
        if self.checkbox_questionnaire.GetValue() == True :
            UTILS_Config.SetParametre("impression_inscriptions_questionnaire", self.ctrl_questionnaire.GetID())
        else :
            UTILS_Config.SetParametre("impression_inscriptions_questionnaire", "")
