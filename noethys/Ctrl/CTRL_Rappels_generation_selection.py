...
class CTRL(HTL.HyperTreeList):
    ...
    def AfficheNbreComptes(self, nbreComptes=0):
        if self.parent.GetName() == "DLG_Rappels_generation_selection" :
            if nbreComptes == 0 : label = _("Aucune lettre de rappel sélectionnée")
            elif nbreComptes == 1 : label = _("1 lettre de rappel sélectionnée")
            else: label = _("%d lettres de rappel sélectionnées") % nbreComptes
    ...
    def Remplissage(self):
        ...
        dlgAttente = PBI.PyBusyInfo(_("Recherche des impayés en cours..."), parent=None, title=_("Veuillez patienter..."), icon=wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Logo.png"), wx.BITMAP_TYPE_ANY))
        try :
            ...
            dlg = wx.MessageDialog(self, _("Désolé, le problème suivant a été rencontré dans la recherche des rappels : \n\n%s") % err, _("Erreur"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        dlgAttente = PBI.PyBusyInfo(_("Calcul des selectionnés en cours..."), parent=None, title=_("Veuillez patienter..."), icon=wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Logo.png"), wx.BITMAP_TYPE_ANY))
        wx.Yield()
        index = 0
        for nomSansCivilite, IDcompte_payeur in listeNomsSansCivilite :
            ...
            ctrl_document = CTRL_document(self.GetMainWindow(), -1, branche=niveauCompte, IDcompte_payeur=IDcompte_payeur, nbreJoursRetard=nbreJoursRetard, infobulle=_("Sélectionnez un document"))
    ...
    def OnContextMenu(self, event):
        ...
        item = wx.MenuItem(menuPop, 10, _("Afficher un aperçu PDF"))
    ...
    def AfficherApercu(self, event=None):
        ...
        dictItem = self.GetMainWindow().GetItemPyData(item)
        if dictItem == None :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune lettre dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        type = dictItem["type"]
        ...
        if dictDocument["IDtexte"] == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement attribuer un texte à cette lettre !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        # Fabrication du PDF
        dlgAttente = PBI.PyBusyInfo(_("Création de l'aperçu au format PDF..."), parent=None, title=_("Veuillez patienter..."), icon=wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Logo.png"), wx.BITMAP_TYPE_ANY))
        wx.Yield() 
        try :
            ...
            del dlgAttente
            traceback.print_exc(file=sys.stdout)
            dlg = wx.MessageDialog(self, _("Désolé, le problème suivant a été rencontré dans la création de l'aperçu de la lettre de rappel : \n\n%s") % err, _("Erreur"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return False
