...
class Page_introduction(Assistant.Page):
    def __init__(self, parent):
        ...
        self.Ajouter_question(titre=_("Bienvenue dans l'assistant de génération d'une activité annuelle de type culturelle ou sportive (gym, yoga, art floral, foot, etc...)"))
        self.Ajouter_question(titre=_("Cliquez sur le bouton Suite pour commencer la saisie des données..."))
...
class Page_generalites(Assistant.Page):
    def __init__(self, parent):
        Assistant.Page.__init__(self, parent)
        self.Ajouter_rubrique(titre=_("Généralités"))
        self.Ajouter_question(code="nom", titre=_("Quel est le nom de l'activité ?"), commentaire=_("Exemple : 'Yoga - Saison 2017-18'"), ctrl=Assistant.CTRL_Texte, obligatoire=True)
        self.Ajouter_question(code="date_debut", titre=_("Quelle est la date de début de l'activité ?"), commentaire=None, ctrl=Assistant.CTRL_Date, obligatoire=True)
        self.Ajouter_question(code="date_fin", titre=_("Quelle est la date de fin de l'activité ?"), commentaire=None, ctrl=Assistant.CTRL_Date, obligatoire=True)
        self.Ajouter_question(code="nbre_inscrits_max", titre=_("Quel est le nombre maximal d'inscrits sur la saison ?"), commentaire=_("S'il n'y aucune limitation du nombre d'inscrits global, conservez la valeur 0."), ctrl=Assistant.CTRL_Nombre)
        self.Ajouter_question(code="groupes_activites", titre=_("Cochez les groupes d'activités associés à cette activité :"), commentaire=_("Les groupes d'activités permettent une sélection plus rapide dans certaines fenêtres de Noethys."), ctrl=Assistant.CTRL_Groupes_activite)

    def Suite(self):
        # Validation des dates du séjour
        if self.parent.dict_valeurs["date_debut"] > self.parent.dict_valeurs["date_fin"]:
            dlg = wx.MessageDialog(self, _("La date de fin de l'activité doit être supérieure à la date du début !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
...
class Page_groupes(Assistant.Page):
    def __init__(self, parent):
        ...
        self.Ajouter_question(code="has_groupes", titre=_("Cette activité est-elle composée de plusieurs groupes ou plusieurs séances ?"), commentaire=_("Exemples : Groupe du 'lundi soir', 'jeudi 18h15', 'Séniors', etc..."), ctrl=Assistant.CTRL_Oui_non, defaut=False)
        self.Ajouter_question(code="has_consommations", titre=_("Souhaitez-vous pouvoir faire du pointage à chaque séance ?"), commentaire=_("Noethys enregistrera alors des consommations pour chaque séance. Si vous ne savez pas, sélectionnez Non."), ctrl=Assistant.CTRL_Oui_non, defaut=False)
        ...
        if self.parent.dict_valeurs["has_consommations"] == True:
            if (self.parent.dict_valeurs["date_fin"] - self.parent.dict_valeurs["date_debut"]).days > 50 :
                ...
                if len(listeDonnees) == 0 :
                    dlg = wx.MessageDialog(self, _("Attention, il semblerait que les périodes de vacances n'aient pas été paramétrées !\n\nSouhaitez-vous quand même continuer ? \n\nSinon, cliquez sur Non et allez dans Menu Paramétrage > Calendrier > Vacances."), _("Avertissement"), wx.YES_NO | wx.NO_DEFAULT | wx.CANCEL | wx.ICON_EXCLAMATION)
...
class Page_jours(Assistant.Page):
    def __init__(self, parent):
        Assistant.Page.__init__(self, parent)
        self.Ajouter_rubrique(titre=_("Groupes ou séances"))
        self.Ajouter_question(code="jours_groupe#1", titre=_("La séance a lieu quel jour de la semaine ?"), commentaire=_("Noethys va créer des consommations sur chaque jour d'ouverture de la séance durant toute la durée de l'activité."), ctrl=Assistant.CTRL_Jours, obligatoire=False)
...
class Page_groupes_nombre(Assistant.Page):
    def __init__(self, parent):
        Assistant.Page.__init__(self, parent)
        self.Ajouter_rubrique(titre=_("Groupes ou séances"))
        self.Ajouter_question(code="nbre_groupes", titre=_("Quel est le nombre de groupes ou de séances ?"), commentaire=None, ctrl=Assistant.CTRL_Nombre, obligatoire=True)
...
class Page_groupes_liste(Assistant.Page):
    def __init__(self, parent):
        ...
        for index in range(1, self.parent.dict_valeurs["nbre_groupes"]+1) :
            self.Ajouter_question(code="nom_groupe#%d" % index, titre=_("Quel est le nom du groupe ou de la séances n°%d ?") % index, commentaire=_("Exemples : 'Lundi 18h15', 'Samedi 10h', 'Séniors', etc..."), ctrl=Assistant.CTRL_Texte, obligatoire=True)
            self.Ajouter_question(code="capacite_max_groupe#%d" % index, titre=_("Quel est le nombre d'inscrits maximal du groupe ou de la séance n°%d ?") % index, commentaire=_("S'il n'y aucune limitation du nombre d'inscrits sur le groupe ou la séance, conservez la valeur 0."), ctrl=Assistant.CTRL_Nombre, obligatoire=False)
            if self.parent.dict_valeurs["has_consommations"] == True:
                self.Ajouter_question(code="jours_groupe#%d" % index, titre=_("La séance n°%d a lieu quel jour de la semaine ?") % index, commentaire=_("Noethys va créer des consommations sur chaque jour d'ouverture de la séance durant toute la durée de l'activité."), ctrl=Assistant.CTRL_Jours, obligatoire=False)
    ...
    def Suite(self):
        ...
        if len(listeDonnees) > 0 :
            ...
            for IDactivite, nom, date_debut, date_fin in listeDonnees :
                ...
                label = "%s - Du %s au %s" % (nom, UTILS_Dates.DateDDEnFr(date_debut), UTILS_Dates.DateDDEnFr(date_fin))
...
class Page_categories_tarifs(Assistant.Page):
    def __init__(self, parent):
        ...
        self.Ajouter_question(code="gratuit", titre=_("Cette activité est-elle gratuite ?"), ctrl=Assistant.CTRL_Oui_non, defaut=False)
        self.Ajouter_question(code="has_categories_tarifs", titre=_("Avez-vous plusieurs catégories de tarifs ?"), commentaire=_("On retrouve par exemple souvent 'Commune' et 'Hors commune'."), ctrl=Assistant.CTRL_Oui_non, defaut=False)
...
class Page_categories_tarifs_nombre(Assistant.Page):
    def __init__(self, parent):
        Assistant.Page.__init__(self, parent)
        self.Ajouter_rubrique(titre=_("Tarifs"))
        self.Ajouter_question(code="nbre_categories_tarifs", titre=_("Quel est le nombre de catégories de tarifs ?"), commentaire=None, ctrl=Assistant.CTRL_Nombre, obligatoire=True)

    def Suite(self):
        if self.parent.dict_valeurs["nbre_categories_tarifs"] < 2 :
            dlg = wx.MessageDialog(self, _("Le nombre de catégories doit être supérieur à 1 !\n\nSinon sélectionnez Non à la question précédente."), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
...
class Page_tarifs(Assistant.Page):
    def __init__(self, parent):
        ...
        if self.parent.dict_valeurs["has_categories_tarifs"] == False :
            ...
            self.Ajouter_question(code="tarif", titre=_("Quel est le tarif à appliquer ?"), commentaire=_("Sélectionnez une méthode puis saisissez les paramètres demandés."), ctrl=Assistant.CTRL_Tarif)
        # Si plusieurs catégories de tarifs
            ...
            for index in range(1, self.parent.dict_valeurs["nbre_categories_tarifs"]+1):
                ...
                self.Ajouter_question(code="nom_categorie_tarif#%d" % index, titre=_("Quel est le nom de la catégorie de tarifs n°%d ?") % index, commentaire=_("Exemples : 'Commune' ou 'Hors commune'."), ctrl=Assistant.CTRL_Texte, obligatoire=False)
                self.Ajouter_question(code="tarif#%d" % index, titre=_("Quel est le tarif à appliquer à la catégorie n°%d ?") % index, commentaire=_("Sélectionnez une méthode puis saisissez les paramètres demandés."), ctrl=Assistant.CTRL_Tarif)
...
class Page_conclusion(Assistant.Page):
    def __init__(self, parent):
        Assistant.Page.__init__(self, parent)
        self.Ajouter_question(titre=_("Félicitations, vous avez terminé de paramétrer votre activité annuelle !"))
        self.Ajouter_question(titre=_("Cliquez maintenant sur le bouton Valider pour générer cette activité."))
    ...
    def Suite(self):
        ...
        if self.parent.dict_valeurs["recopier_tarifs"] == None :
            ...
            # Si catégorie unique
            if self.parent.dict_valeurs["has_categories_tarifs"] == False:
                listeDonnees = [("IDactivite", IDactivite), ("nom", _("Catégorie unique"))]
