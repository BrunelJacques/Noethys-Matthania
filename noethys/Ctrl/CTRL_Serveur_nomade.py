...
class GenerationFichier(Thread): 
    ...
    def run(self): 
        ...
        self.parent.EcritLog(_("Génération du fichier de données"))
        try: 
...
class Echo(Protocol):
    ...
    def connectionMade(self):
        ...
        if IP_AUTORISEES not in (None, "") :
            if self.IsIPinListe(IP_AUTORISEES, ip) == False :
                self.EcritLog(_("Adresse IP non autorisée"))
                self.transport.loseConnection()

        if IP_INTERDITES not in (None, "") :
            if self.IsIPinListe(IP_INTERDITES, ip) == True :
                self.EcritLog(_("Adresse IP interdite"))
                self.transport.loseConnection()
    ...
    def EnvoyerInfosSurFichierAEnvoyer(self, nomFichier=""):
        ...
        self.EcritLog(_("Envoi du fichier (%s)") % FonctionsPerso.Formate_taille_octets(tailleFichier))
        self.log.SetImage("on")
    ...
    def dataReceived(self, data):
        ...
        if self.IdentificationJSON(data) :
            ...
            if message["action"] == "envoyer" :
                ...
                self.EcritLog(_("Prêt à recevoir de l'appareil ") + nom_appareil + " le fichier " + nomInitial + " (" + FonctionsPerso.Formate_taille_octets(tailleFichier) + ")")
                ...
                if six.PY3:
                    texte = texte.encode('utf-8')
                self.transport.write(texte)
                self.EcritLog(_("Réception en cours"))
        ...
        if self.dictFichierReception != None :
            ...
            #self.EcritLog(_("Réception en cours...  ") + str(pourcentage) + " %")
            self.log.SetGauge(pourcentage)
            return

        # Si aucune data valide, on déconnecte
        self.EcritLog(_("Aucune donnée valide reçue"))
        self.transport.loseConnection()
    ...
    def Envoyer(self):
        self.log.SetImage("upload")
        self.EcritLog(_("Envoi en cours"))
        f = open(self.nomFichierAEnvoyer, "rb")
        self.transport.write(f.read())
        f.close()
        
    def FinEnvoi(self):
        self.EcritLog(_("Fin de l'envoi"))
        self.log.SetImage("on")
        self.log.SetGauge(0)
        self.transport.loseConnection()
        
    def connectionLost(self, reason):
        self.EcritLog(_("Fin de connexion du client ") + self.transport.getPeer().host)
        if self.dictFichierReception != None:
            wx.CallLater(1000, self.log.SetGauge, 0)
            self.log.SetImage("on")
            self.EcritLog(_("Clôture du fichier de réception"))
...
def StartServer(log=None):
    if IMPORT_TWISTED == False :
        log.EcritLog(_("Erreur : Problème d'importation de Twisted"))
        return
    ...
    try :
        ...
        log.EcritLog(_("Erreur dans le lancement du serveur Nomadhys [factory] :") )
        log.EcritLog(err)
    ...
    try :
        ...
        s.close()
        log.EcritLog(_("IP locale : %s") % ip_local)
    except Exception as err:
        print(("Erreur lancement serveur Nomadhys :", err))
        log.EcritLog(_("Erreur dans le lancement du serveur Nomadhys [IP locale] :"))
        log.EcritLog(err)

    try :
        # IP internet
        ip_internet = json.loads(urlopen("https://jsonip.com", timeout=1).read())["ip"]
        log.EcritLog(_("IP internet : %s") % ip_internet)
    except Exception as err:
        print(("Erreur lancement serveur Nomadhys :", err))
        log.EcritLog(_("Erreur dans le lancement du serveur Nomadhys [IP internet] :"))
        log.EcritLog(err)

    log.EcritLog(_("Serveur prêt sur le port %d") % port)
    log.SetImage("on")
    ...
    try :
        ...
        print(("Erreur lancement serveur Nomadhys :", err))
        log.EcritLog(_("Erreur dans le lancement du serveur Nomadhys [reactor] :") )
        log.EcritLog(err)
...
class Panel(wx.Panel):
    def __init__(self, parent):
        ...
        self.bouton_analyse = wx.Button(self, -1, _("Analyser"))
    ...
    def MAJ(self):
        ...
        # MAJ du bouton Analyser
        if nbreFichiers == 0 :
            self.bouton_analyse.SetToolTip(wx.ToolTip(_("Aucun fichier à importer")))
            self.bouton_analyse.SetBackgroundColour(self.couleur_defaut)
            self.bouton_analyse.Enable(False)            
        elif nbreFichiers == 1 :
            self.bouton_analyse.SetToolTip(wx.ToolTip(_("1 fichier à importer")))
            self.bouton_analyse.SetBackgroundColour((150, 255, 150))
            self.bouton_analyse.Enable(True)
        else :
            self.bouton_analyse.SetToolTip(wx.ToolTip(_("%d fichiers à importer") % nbreFichiers))
            self.bouton_analyse.SetBackgroundColour((150, 255, 150))
            self.bouton_analyse.Enable(True)
