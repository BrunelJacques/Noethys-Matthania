...
##        self.listeConso = []
        ...
        if modeLabel == "nom" : 
            if FORMAT_LABEL_LIGNE == "nom_prenom" :
                nomIndividu = "%s %s" % (self.dictInfosIndividu["nom"], self.dictInfosIndividu["prenom"])
            if FORMAT_LABEL_LIGNE == "prenom_nom" :
                nomIndividu = "%s %s" % (self.dictInfosIndividu["prenom"], self.dictInfosIndividu["nom"])
            if FORMAT_LABEL_LIGNE == "nom_prenom_id" :
                nomIndividu = "%s %s (%s)" % (self.dictInfosIndividu["nom"], self.dictInfosIndividu["prenom"], self.IDindividu)
            if FORMAT_LABEL_LIGNE == "prenom_nom_id" :
                nomIndividu = "%s %s (%s)" % (self.dictInfosIndividu["prenom"], self.dictInfosIndividu["nom"], self.IDindividu)
...
##        
##        item = wx.MenuItem(menuPop, 20, _("Déverrouiller toutes les consommations"))
...
##        self.grid.Bind(wx.EVT_MENU, self.Deverrouillage, id=20)
        ...
        if nbreCasesReservations > 0 :
            ...
            item = wx.MenuItem(menuPop, 30, _("Définir toutes les pointages de la ligne comme 'Pointage en attente'"))
            menuPop.AppendItem(item)
            self.grid.Bind(wx.EVT_MENU, self.SetPresentAbsent, id=30)
            
            item = wx.MenuItem(menuPop, 40, _("Pointer toutes les consommations de la ligne sur 'Présent'"))
            ...
            item = wx.MenuItem(menuPop, 50, _("Pointer toutes les consommations de la ligne sur 'Absence justifiée'"))
            ...
            item = wx.MenuItem(menuPop, 60, _("Pointer toutes les consommations de la ligne sur 'Absence injustifiée'"))
    ...
    def SetPresentAbsent(self, event):
        ...
        for numColonne, case in self.dictCases.items() :
            if case.typeCase == "consommation" :
                ...
                case.renderer.MAJ()
        self.MAJctrl_totaux()
        self.grid.listeHistorique.append((self.IDindividu, self.date, None, _("Modification de l'état de toutes les consommations de la ligne")))
...
##        self.DisableDragRowSize()
    ...
    def InitGrid(self):
        ...
        if AFFICHE_COLONNE_MEMO == True :
            ...
            self.SetColLabelValue(numColonne, _("Mémo journalier"))
            numColonne += 1
        ...
        if AFFICHE_COLONNE_TRANSPORTS == True :
            ...
            self.SetColLabelValue(numColonne, _("Transports"))
        ...
        if self.mode == "individu" :
            ...
            for IDindividu, dictInfos in self.dictInfosIndividus.items() :
                if IDindividu in self.listeSelectionIndividus :
                    nomIndividu = "%s %s" % (dictInfos["nom"], dictInfos["prenom"])
        ...
        if self.mode == "date" :
            ...
            for IDindividu, dictInfos in self.dictInfosIndividus.items() :
                ...
                if FORMAT_LABEL_LIGNE.startswith("prenom_nom") : 
                    nomIndividu = "%s %s" % (dictInfos["prenom"], dictInfos["nom"])
                listeNomsIndividus.append( (nomIndividu, IDindividu) )
            listeNomsIndividus.sort()
    ...
    def Importation_prestations(self, listeComptesPayeurs=[]):
        ...
        for IDprestation, IDcompte_payeur, date, categorie, label, montant_initial, montant, IDactivite, IDtarif, IDfacture, IDfamille, IDindividu, forfait, temps_facture, IDcategorie_tarif, forfait_date_debut, forfait_date_fin, code_compta, code_produit_local, tva in listeDonnees:
            ...
            if IDindividu != None and IDindividu != 0 and IDindividu in self.dictIndividus:
                nomIndividu = "%s %s" % (self.dictIndividus[IDindividu]["nom"], self.dictIndividus[IDindividu]["prenom"])
            else:
                nomIndividu = ""
                        ...
                        if quantite_max != None :
                            ...
                            if IDprestationTemp != IDprestationForfait and case.IDunite in combiRetenue and nbre + 1 > quantite_max :
                                ...
                                if "blocage_plafond" in dictTarif["options"] :
                                    ...
                                    case.Supprimer()
                                    if modeSilencieux == False :
                                        dlg = wx.MessageDialog(self, _("Il n'y a plus de crédit disponible dans le forfait !"), _("Crédit épuisé"), wx.OK | wx.ICON_EXCLAMATION)
                                        dlg.ShowModal()
                                        dlg.Destroy()
                                        return
                        ...
                        for IDunite, listeConso in dictUnites.items() :
                            for conso in listeConso :
                                ...
                                if IDactivite in self.listeActivites and conso.IDfamille == self.IDfamille:
                                    ...
                                    if agrement != None :
                                        agrement = _(" - n° agrément : %s") % agrement
                                    ...
                                    etat = conso.etat
                                    if etat == "reservation" : etat = _("Réservation")
                                    if etat == "absenti" : etat = _("Absence injustifiée")
                                    if etat == "absentj" : etat = _("Absence justifiée")
                                    if etat == "present" : etat = _("Présent")
                                    if etat == "attente" : etat = _("Attente")
                                    if etat == "refus" : etat = _("Refus")
...
##            }
    ...
    def ConvertirEtat(self, event=None):
        ...
        if len(listeConso) == 0 :
            dlg = wx.MessageDialog(self, _("Il n'y a aucune consommation ayant cet état !"), _("Annulation"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        # Confirmation
        dlg = wx.MessageDialog(self, _("Confirmez-vous le changement d'état '%s' en '%s' pour %d consommations ?") % (dictDonnees["label_etat_avant"], dictDonnees["label_etat_apres"], len(listeConso)), _("Changement d'état"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
    ...
    def RecalculerToutesPrestations(self, modeSilencieux=True):
        ...
        if len(listeDejaFactures) > 0 and modeSilencieux == False :
            dlg = wx.MessageDialog(self, _("Notez que %d prestations n'ont pas été recalculés car \ncelles-ci apparaissent déjà sur des factures ou sont verrouillées !") % len(listeDejaFactures), _("Information"), wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
    ...
    def TraitementLot(self):
        ...
        try :
            # dlgAttente = wx.BusyInfo(_("Veuillez patienter durant la procédure..."), None)
            # wx.Yield()
            ...
            dlg = wx.MessageDialog(self, _("Désolé, le problème suivant a été rencontré dans le traitement par lot des consommations : \n\n%s") % err, _("Erreur"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return

        # Formatage du texte de résultats
        texte = _("<B>La procédure de traitement par lot est terminée mais les incidents suivants ont été rencontrés :</B><BR><BR>")
        ...
        for IDindividu, listeActions in journal.items() :
            if len(listeActions) > 0 :
                ...
                texte += "<U>%s :</U><BR><BR>" % nomIndividu
                
                texte += "<UL>"
                for date, nomUnite, action in listeActions :
                    texte += "<LI>%s - %s : %s.</LI>" % (DateEngFr(str(date)), nomUnite, action)
                texte += "</UL><BR><BR>"
            
        # Affichage des résultats
        if afficher == True :
            from Dlg import DLG_Message_html
            dlg = DLG_Message_html.Dialog(self, texte=u"<FONT SIZE=2>%s</FONT>" % texte, titre=_("Résultats du traitement par lot"), size=(630, 450))
            dlg.ShowModal()
            dlg.Destroy()
                                ...
                                for dictUnite in resultats["unites"] :
                                    ...
                                    if dictUnite["IDunite"] == case.IDunite :
                                        ...
                                        if dictUnite["type"] != "Evenement" and case.IsCaseDisponible(heure_debut, heure_fin) == False :
                                            journal[case.IDindividu].append((case.date, dictUnite["nom"], _("Une consommation est déjà enregistrée")))
                                            valide = False
                                        ...
                                        if dictUnite["type"] != "Evenement" and case.HasPlaceDisponible(heure_debut, heure_fin) == False :
                                            journal[case.IDindividu].append((case.date, dictUnite["nom"], _("Plus de places")))
                                            valide = False
                                        ...
                                        if incompatibilite != None :
                                            ...
                                            journal[case.IDindividu].append((case.date, dictUnite["nom"], _("Incompatibilité avec l'unité '%s' déjà enregistrée") %  nomUniteIncompatible))
                                            valide = False
                                    ...
                                    if dictUnite["IDunite"] == case.IDunite :
                                        ...
                                        if index <= len(listeConso) - 1 :
                                            ...
                                            if conso.etat != None :
                                                ...
                                                if conso.IDfacture != None :
                                                    journal[case.IDindividu].append((case.date, dictUnite["nom"], _("Interdit de modifier une consommation déjà facturée")))
                                                    
                                                elif conso.etat in ("present", "absentj", "absenti") :
                                                    journal[case.IDindividu].append((case.date, dictUnite["nom"], _("Interdit de modifier une consommation déjà pointée")))
                                                
                                                elif conso.forfait != None :
                                                    journal[case.IDindividu].append((case.date, dictUnite["nom"], _("Interdit de modifier un forfait")))
                                        ...
                                        for conso in case.GetListeConso() :
                                            ...
                                            if len(resultats["etiquettes"]) == 0 or len(set(conso.etiquettes) & set(resultats["etiquettes"])) > 0 :
                                                ...
                                                if conso.IDfacture != None :
                                                    ...
                                                    journal[case.IDindividu].append((case.date, dictUnite["nom"], _("Impossible de modifier un forfait")))
                                                else :
                                                    if dictUnite["type"] == "Multihoraires" :
                                            ...
                                            if len(resultats["etiquettes"]) == 0 or len(set(conso.etiquettes) & set(resultats["etiquettes"])) > 0 :
                                                ...
                                                if conso.etat != None :
                                                    if resultats["etat"] != None and conso.etat != resultats["etat"] :
                                                        if conso.IDfacture != None :
                                                            journal[case.IDindividu].append((case.date, dictUnite["nom"], _("Impossible de supprimer une consommation déjà facturée")))
                                                        else :
                ...
                for codeCategorie in ("suppr", "modif", "ajout") :
                    ...
                    if len(dictDates) > 0 :
                        ...
                        for date in listeDates :
                            ...
                            dateFr = "%s/%s/%s" % (str(date)[8:10], str(date)[5:7], str(date)[2:4])
...
####AFFICHAGE
    ...
    def Parametres(self):
        ...
        # Hauteur
        listeDonnees.append(
            (_("Hauteur de ligne"), [{"label" : _("Hauteur"), "code" : "hauteur", "valeur" : self.dictParametres["hauteur"], "defaut" : HAUTEUR_LIGNE},] )
            )
        listeActivites = []
        ...
        for IDactivite in self.listeActivites :
            ...
            if IDactivite in self.dictListeUnites:
                for dictUnite in self.dictListeUnites[IDactivite] :
                    IDunite = dictUnite["IDunite"]
                    nomUnite = "%s (%s)" % (dictUnite["nom"], dictUnite["abrege"])
                    if dictUnite["type"] == "Multihoraires" :
            ...
            listeDonnees.append((_("Colonnes %s") % nomActivite, listeUnites))

        listeDonnees.append(
            (_("Autres colonnes"), [
                {"label" : _("Mémo journalier"), "code" : "memo", "valeur" : self.dictParametres["largeurs"]["memo"], "defaut" : LARGEUR_COLONNE_MEMO},
                {"label" : _("Transports"), "code" : "transports", "valeur" : self.dictParametres["largeurs"]["transports"], "defaut" : LARGEUR_COLONNE_TRANSPORTS},
                ] )
            )
...
##                ligne.Flash()
    ...
    def Recopier(self, event=None):
        ...
        # Formatage du texte de résultats
        texte = _("<B>La procédure de recopiage est terminée mais les incidents suivants ont été rencontrés :</B><BR><BR>")
        ...
        for case, conso, journal in listeJournaux :
            for IDindividu, listeActions in journal.items() :
                if len(listeActions) > 0 :
                    afficher = True
                    for date, nomUnite, action in listeActions :
                        texte += "<LI>%s - %s : %s.</LI>" % (case.ligne.labelLigne, nomUnite, action)
        texte += "</UL><BR><BR>"

        # Affichage des résultats
        if afficher == True :
            from Dlg import DLG_Message_html
            dlg = DLG_Message_html.Dialog(self, texte=u"<FONT SIZE=2>%s</FONT>" % texte, titre=_("Résultats du recopiage"), size=(630, 450))
            dlg.ShowModal()
            dlg.Destroy()
