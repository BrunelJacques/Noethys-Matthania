...
class Panel(wx.Panel):
    def __init__(self, parent, IDactivite=None, IDtarif=None, nouveauTarif=True):
        ...
        # Intro
        self.label_intro = wx.StaticText(self, -1, _("Saisissez les unités associables au forfait :"))
        ...
        # Durée
        self.label_duree_forfait = wx.StaticText(self, -1, _("Durée par défaut :"))
        self.ctrl_duree_forfait = wx.CheckBox(self, -1, _(""))
        ...
        self.label_annees = wx.StaticText(self, -1, _("Années :"))
        self.spin_annees = wx.SpinCtrl(self, -1, "", min=0, max=500)

        # Blocage
        self.label_blocage_plafond = wx.StaticText(self, -1, _("Blocage plafond :"))
        self.ctrl_blocage_plafond = wx.CheckBox(self, -1, _("Bloquer si la quantité maximale de consommations est atteinte"))

        # Bénéficiaire
        self.label_type_forfait = wx.StaticText(self, -1, _("Type de forfait :"))
        self.ctrl_beneficiaire_individu = wx.RadioButton(self, -1, _("Forfait individuel"), style = wx.RB_GROUP)
        self.ctrl_beneficiaire_famille = wx.RadioButton(self, -1, _("Forfait familial"))
        
        # Date de facturation
        self.label_date_facturation = wx.StaticText(self, -1, _("Date de facturation :"))
        listeChoix = [
            ("date_debut_forfait", _("Date de début du forfait")),
            ("date_fin_forfait", _("Date de fin du forfait")),
    ...
    def __set_properties(self):
        self.bouton_ajouter_combi.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter une combinaison d'unités")))
        self.bouton_modifier_combi.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier la combinaison d'unités selectionnée dans la liste")))
        self.bouton_supprimer_combi.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer la combinaison d'unités selectionnée dans la liste")))
        self.ctrl_duree_forfait.SetToolTip(wx.ToolTip(_("Vous pouvez définir une durée de validité à partir de la date de début de forfait saisie par l'utilisateur")))
        self.ctrl_blocage_plafond.SetToolTip(wx.ToolTip(_("Cliquez ici pour empêcher la saisie d'une consommation si la quantité maximale est atteinte")))
    ...
    def Validation(self):
        if self.ctrl_duree_forfait.GetValue() == True :
            ...
            if jours == 0 and mois == 0 and annees == 0 :
                dlg = wx.MessageDialog(self, _("Vous avez coché une durée limitée. \nVous devez donc saisir un nombre de jours et/ou de mois et/ou d'années."), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
