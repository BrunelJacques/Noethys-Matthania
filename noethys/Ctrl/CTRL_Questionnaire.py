...
LISTE_CONTROLES = [
    {"code" : "ligne_texte", "label" : _("Ligne de texte"), "image" : "Texte_ligne.png", "filtre" : "texte"},
    {"code" : "bloc_texte", "label" : _("Bloc de texte multiligne"), "image" : "Texte_bloc.png", "options" : {"hauteur":60}, "filtre" : "texte" },
    {"code" : "entier", "label" : _("Nombre entier"), "image" : "Ctrl_nombre.png", "options" : {"min":0, "max":99999}, "filtre" : "entier" },
    {"code" : "decimal", "label" : _("Nombre décimal"), "image" : "Ctrl_decimal.png", "options" : {"min":0, "max":99999}, "filtre" : "decimal" },
    {"code" : "montant", "label" : _("Montant"), "image" : "Euro.png", "filtre" : "montant" },
    {"code" : "liste_deroulante", "label" : _("Liste déroulante"), "image" : "Ctrl_choice.png", "options":{"choix":None}, "filtre" : "choix" },
    {"code" : "liste_coches", "label" : _("Liste à cocher"), "image" : "Coches.png", "options" : {"hauteur":-1, "choix":None} , "filtre" : "choix"},
    {"code" : "case_coche", "label" : _("Case à cocher"), "image" : "Ctrl_coche.png" , "filtre" : "coche"},
    {"code" : "date", "label" : _("Date"), "image" : "Jour.png" , "filtre" : "date"},
    {"code" : "slider", "label" : _("Réglette"), "image" : "Reglette.png", "options" : {"hauteur":-1, "min":0, "max":100}, "filtre" : "entier" },
    {"code" : "couleur", "label" : _("Couleur"), "image" : "Ctrl_couleur.png", "options" : {"hauteur":20}, "filtre" : None},
##    {"code" : "potentiometre", "label" : _("Potentiomère"), "image" : "Potentiometre.png", "options" : {"hauteur":100, "min":0, "max":100} },
    {"code" : "documents", "label" : _("Porte-documents"), "image" : "Document.png", "options" : {"hauteur":60}, "filtre" : None},
    {"code" : "codebarres", "label" : _("Code-barres"), "image" : "Codebarres.png", "options" : {"norme":"39"}, "filtre" : "texte" },
    {"code" : "rfid", "label" : _("Badge RFID"), "image" : "Rfid.png" , "filtre" : "texte"},
    ] 
...
class DLG_Choix_creation(wx.Dialog):
    ...
    def __set_properties(self):
        self.SetTitle(_("Choix de l'élément à créer"))
        self.bouton_categorie.SetToolTip(wx.ToolTip(_("Créer une nouvelle catégorie")))
        self.bouton_question.SetToolTip(wx.ToolTip(_("Créer une nouvelle question")))
...
class CTRL_montant(CTRL_Saisie_euros.CTRL):
    def __init__(self, parent, item=None, track=None):
        ...
        self.SetToolTip(wx.ToolTip(_("Saisissez un montant")))
...
class CTRL_liste_coches(wx.CheckListBox):
    ...
    def OnContextMenu(self, event):
        ...
        # Item Tout cocher
        item = wx.MenuItem(menuPop, 10, _("Tout cocher"))
        ...
        # Item Tout décocher
        item = wx.MenuItem(menuPop, 20, _("Tout décocher"))
...
class CTRL_date(CTRL_Saisie_date.Date2):
    def __init__(self, parent, item=None, track=None):
        ...
        self.track = track
        self.SetBackgroundColour((255, 255, 255))
        self.ctrl_date.SetToolTip(wx.ToolTip(_("Saisissez une date")))
...
class CTRL_slider(wx.Panel):
    def __init__(self, parent, item=None, track=None):
        ...
        self.ctrl_slider.SetToolTip(wx.ToolTip(_("Faites glisser la glissière sur la valeur de votre choix")))
...
class CTRL_couleur(wx.lib.colourselect.ColourSelect):
    def __init__(self, parent, item=None, track=None):
        ...
        self.track = track
        self.SetBackgroundColour((255, 255, 255))
        self.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner une couleur")))
...
class CTRL_documents(wx.Panel):
    def __init__(self, parent, item=None, track=None):
        ...
        self.bouton_outils.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder aux commandes disponibles")))
...
class CTRL_rfid(wx.Panel):
    def __init__(self, parent, item=None, track=None):
        ...
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour scanner un badge")))
    ...
    def OnBoutonModifier(self, event=None):
        ...
        if DLG_Saisie_rfid.CheckLecteurs() == False :
                dlg = wx.MessageDialog(self, _("Aucun lecteur RFID connecté."), "Erreur", wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                return 
...
class CTRL(HTL.HyperTreeList):
    def __init__(self, parent, type="individu", IDdonnee=None, mode="normal",
        ...
        listeColonnes = [
            ( _("Question"), self.largeurQuestion, wx.ALIGN_LEFT),
            ( _("Réponse"), self.largeurReponse, wx.ALIGN_LEFT),
            ]
        numColonne = 0
...
##                        if track.controle == "potentiometre" : ctrl = CTRL_potentiometre(self.GetMainWindow(), item=brancheQuestion, track=track) # size=(largeurControle, 100) )
    ...
    def OnContextMenu(self, event):
        ...
        # Item Monter
        item = wx.MenuItem(menuPop, 40, _("Monter"))
        ...
        # Item Descendre
        item = wx.MenuItem(menuPop, 50, _("Descendre"))
    ...
    def Ajouter(self, event=None):
        ...
        if reponse == 200 : 
            ...
            if len(self.dictCategories) == 0 :
                dlg = wx.MessageDialog(self, _("Vous devez d'abord créer au moins une catégorie !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return
    ...
    def Modifier(self, event=None):
        ...
        resultat = self.IdentificationBranche(item) 
        if resultat == None :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune ligne !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        # Catégorie
    ...
    def Supprimer(self, event=None):
        ...
        if resultat["type"] == "categorie" :
            ...
            if len(listeQuestionRattachees) > 0 :
                dlg = wx.MessageDialog(self, _("Cette catégorie comporte déjà %d question(s).\n\nVous devez déjà les supprimer avant de supprimer la catégorie.") % len(listeQuestionRattachees), _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return
            # Confirmation
            dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cette catégorie ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
            reponse = dlg.ShowModal()
            dlg.Destroy()
        ...
        if resultat["type"] == "question" :
            ...
            if len(listeReponses) > 0 :
                dlg = wx.MessageDialog(self, _("%d filtres sont déjà associés à cette question.\n\nVous devez déjà les supprimer avant de supprimer la question.") % len(listeReponses), _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return
            ...
            DB.Close() 
            if len(listeReponses) > 0 :
                dlg = wx.MessageDialog(self, _("%d fiches comportent une réponse à cette question.\nSi vous supprimez cette question, les réponses le seront également.\n\nSouhaitez-vous vraiment supprimer cette question ?") % len(listeReponses), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
                reponse = dlg.ShowModal()
                dlg.Destroy()
                if reponse != wx.ID_YES :
                    return 
            ...
            # Confirmation
            dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cette question ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
            reponse = dlg.ShowModal()
            dlg.Destroy()
            if reponse != wx.ID_YES :
                return 
