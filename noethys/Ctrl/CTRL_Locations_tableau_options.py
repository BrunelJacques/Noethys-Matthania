...
class CTRL_Parametres(CTRL_Propertygrid.CTRL):
    ...
    def Remplissage(self):
        # Heure min affichée
        propriete = wxpg.StringProperty(label=_("Heure minimale affichée"), name="heure_min", value="08:30")
        propriete.SetHelpString(_("Saisissez une heure"))
        propriete.SetEditor("EditeurHeure")
        propriete.SetAttribute("obligatoire", True)
        self.Append(propriete)

        # Heure max affichée
        propriete = wxpg.StringProperty(label=_("Heure maximale affichée"), name="heure_max", value="22:30")
        propriete.SetHelpString(_("Saisissez une heure"))
        propriete.SetEditor("EditeurHeure")
        propriete.SetAttribute("obligatoire", True)
        self.Append(propriete)
        ...
        # Intervalle souris
        propriete = wxpg.StringProperty(label=_("Pas de la souris"), name="temps_arrondi", value="00:30")
        propriete.SetHelpString(_("Saisissez un temps"))
        propriete.SetEditor("EditeurHeure")
        propriete.SetAttribute("obligatoire", True)
        self.Append(propriete)

        # Durée minimale barre
        propriete = wxpg.StringProperty(label=_("Durée minimale location"), name="barre_duree_minimale", value="00:30")
        propriete.SetHelpString(_("Saisissez une durée"))
        propriete.SetEditor("EditeurHeure")
        propriete.SetAttribute("obligatoire", True)
        self.Append(propriete)

        # Hauteur de ligne
        propriete = wxpg.IntProperty(label=_("Hauteur de ligne"), name="case_hauteur", value=50)
        propriete.SetHelpString(_("Saisissez une hauteur de ligne (50 par défaut)"))
        propriete.SetAttribute("obligatoire", True)
        self.Append(propriete)
        self.SetPropertyEditor("case_hauteur", "SpinCtrl")

        # Autoriser changement de ligne
        propriete = wxpg.BoolProperty(label=_("Autoriser le changement de ligne"), name="autoriser_changement_ligne", value=True)
        propriete.SetHelpString(_("Cochez cette case si vous souhaitez pouvoir changer de ligne"))
...
class CTRL(wx.Panel):
    def __init__(self, parent, ctrl_tableau=None):
        ...
        # Période
        self.staticBox_periode = wx.StaticBox(self, -1, _("Sélection de la période"))
        self.ctrl_periode = CTRL_Selection_periode_simple.CTRL(self, callback=self.OnChangePeriode)

        # Catégories de produits
        self.staticBox_categories = wx.StaticBox(self, -1, _("Catégories de produits"))
