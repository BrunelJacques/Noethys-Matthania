...
class Page_introduction(Assistant.Page):
    def __init__(self, parent):
        ...
        self.Ajouter_question(titre=_("Bienvenue dans l'assistant de génération d'une activité de type stage (stage de théâtre, de danse, de gym, etc...)"))
...
class Page_generalites(Assistant.Page):
    def __init__(self, parent):
        ...
        self.Ajouter_question(code="nom", titre=_("Quel est le nom du stage ?"), commentaire=_("Exemple : 'Stage de théâtre - Février 2018'"), ctrl=Assistant.CTRL_Texte, obligatoire=True)
        self.Ajouter_question(code="date_debut", titre=_("Quelle est la date de début du stage ?"), commentaire=None, ctrl=Assistant.CTRL_Date, obligatoire=True)
        self.Ajouter_question(code="date_fin", titre=_("Quelle est la date de fin du stage ?"), commentaire=None, ctrl=Assistant.CTRL_Date, obligatoire=True)
        ...
        self.Ajouter_question(code="groupes_activites", titre=_("Cochez les groupes d'activités associés à ce stage :"), commentaire=_("Les groupes d'activités permettent une sélection plus rapide dans certaines fenêtres de Noethys."), ctrl=Assistant.CTRL_Groupes_activite)

    def Suite(self):
        # Validation des dates du stage
        if self.parent.dict_valeurs["date_debut"] > self.parent.dict_valeurs["date_fin"]:
            dlg = wx.MessageDialog(self, _("La date de fin du stage doit être supérieure à la date du début !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
...
class Page_groupes(Assistant.Page):
    def __init__(self, parent):
        ...
        self.Ajouter_question(code="has_groupes", titre=_("Le stage est-il constitué de groupes distincts ?"), commentaire=_("Exemple : Débutants et confirmés. Si vous n'êtes pas sûr, laissez non."), ctrl=Assistant.CTRL_Oui_non, defaut=False)
...
class Page_conclusion(Assistant.Page):
    def __init__(self, parent):
        Assistant.Page.__init__(self, parent)
        self.Ajouter_question(titre=_("Félicitations, vous avez terminé de paramétrer votre stage !"))
