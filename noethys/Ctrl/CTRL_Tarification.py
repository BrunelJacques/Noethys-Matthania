...
class CTRL(HTL.HyperTreeList):
    def __init__(self, parent, id=wx.ID_ANY, pos=wx.DefaultPosition,
        ...
        self.AddColumn(_("Prestations / Tarifs"))
        self.SetColumnWidth(0, 250)
        self.AddColumn(_("Nom du tarif"))
        self.SetColumnWidth(1, 230)
        self.AddColumn(_("Catégories de tarifs"))
        self.SetColumnWidth(2, 180)
        self.AddColumn(_("Méthode de calcul"))
        self.SetColumnWidth(3, 230)
        self.SetMainColumn(0)
                                
        # Création des branches
        self.root = self.AddRoot(_("Tarifs"))
        self.SetItemData(self.root, {"type" : "root", "ID" : None} )
        self.SetAGWWindowStyleFlag(HTL.TR_COLUMN_LINES)
    ...
    def CreationBranches(self):
        ...
        for IDnom_tarif, nom in listeNoms :
            ...
            self.SetItemData(brancheNom, {"type" : "nom", "ID" : IDnom_tarif, "nom":nom} )
            ...
            if IDnom_tarif in dictTarifs :
                for dictTarifTemp in dictTarifs[IDnom_tarif] :
                    ...
                    if dictTarifTemp["date_debut"] != None :
                        ...
                        date_debut = "?"
                    label = _("A partir du %s") % date_debut
                    if dictTarifTemp["date_fin"] != None :
                        date_fin = DateEngFr(dictTarifTemp["date_fin"])
                        label = _("Du %s au %s") % (date_debut, date_fin)
                        
                    brancheTarif = self.AppendItem(brancheNom, label)
                    self.SetItemData(brancheTarif, {"type" : "tarif", "ID" : IDtarif, "IDnom_tarif":IDnom_tarif})
...
##            self.Bind(wx.EVT_MENU, self.Ajouter, id=10)
        ...
        if type == "root" :
            # Item Ajouter
            item = wx.MenuItem(menuPop, 10, _("Ajouter un nom de prestation"))
            bmp = wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Ajouter.png"), wx.BITMAP_TYPE_PNG)
            item.SetBitmap(bmp)
            menuPop.Append(item)
            self.Bind(wx.EVT_MENU, self.Ajouter, id=10)
...
##            # Item Supprimer
##            item = wx.MenuItem(menuPop, 30, _("Supprimer la catégorie de tarif"))
...
##            menuPop.AppendSeparator()
##            # Item Dupliquer
##            item = wx.MenuItem(menuPop, 40, _("Dupliquer cette catégorie de tarif"))
...
##            self.Bind(wx.EVT_MENU, self.Dupliquer, id=40)
        ...
        if type == "nom" :
            # Item Ajouter
            item = wx.MenuItem(menuPop, 10, _("Ajouter un tarif pour cette prestation"))
            bmp = wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Ajouter.png"), wx.BITMAP_TYPE_PNG)
            item.SetBitmap(bmp)
            menuPop.Append(item)
            self.Bind(wx.EVT_MENU, self.Ajouter, id=10)
            menuPop.AppendSeparator()
            # Item Modifier
            item = wx.MenuItem(menuPop, 20, _("Modifier le nom de la prestation"))
            bmp = wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Modifier.png"), wx.BITMAP_TYPE_PNG)
            item.SetBitmap(bmp)
            menuPop.Append(item)
            self.Bind(wx.EVT_MENU, self.Modifier, id=20)
            # Item Supprimer
            item = wx.MenuItem(menuPop, 30, _("Supprimer ce nom de prestation"))
            bmp = wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Supprimer.png"), wx.BITMAP_TYPE_PNG)
            item.SetBitmap(bmp)
            menuPop.Append(item)
            self.Bind(wx.EVT_MENU, self.Supprimer, id=30)
            menuPop.AppendSeparator()
            # Item Dupliquer
            item = wx.MenuItem(menuPop, 40, _("Dupliquer ce nom de prestation"))
            bmp = wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Dupliquer.png"), wx.BITMAP_TYPE_PNG)
            item.SetBitmap(bmp)
            menuPop.Append(item)
            self.Bind(wx.EVT_MENU, self.Dupliquer, id=40)
        ...
        if type == "tarif" :
            # Item Modifier
            item = wx.MenuItem(menuPop, 20, _("Modifier ce tarif"))
            bmp = wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Modifier.png"), wx.BITMAP_TYPE_PNG)
            item.SetBitmap(bmp)
            menuPop.Append(item)
            self.Bind(wx.EVT_MENU, self.Modifier, id=20)
            # Item Supprimer
            item = wx.MenuItem(menuPop, 30, _("Supprimer ce tarif"))
            bmp = wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Supprimer.png"), wx.BITMAP_TYPE_PNG)
            item.SetBitmap(bmp)
            menuPop.Append(item)
            self.Bind(wx.EVT_MENU, self.Supprimer, id=30)
            menuPop.AppendSeparator()
            # Item Dupliquer
            item = wx.MenuItem(menuPop, 40, _("Dupliquer ce tarif"))
            bmp = wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Dupliquer.png"), wx.BITMAP_TYPE_PNG)
            item.SetBitmap(bmp)
            menuPop.Append(item)
            self.Bind(wx.EVT_MENU, self.Dupliquer, id=40)
...
##            dlg = wx.MessageDialog(self, _("Il est impossible de supprimer une catégorie qui comporte au moins un tarif !"), _("Suppression impossible"), wx.OK | wx.ICON_INFORMATION)
##            dlg.ShowModal()
##            dlg.Destroy()
##            return
##        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cette catégorie ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
...
##        dlg = wx.MessageDialog(self, _("Souhaitez-vous dupliquer également les tarifs et paramétrages de cette catégorie ?"), _("Duplication"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
...
##        self.ModifierCategorie(IDcategorie_tarif=IDcategorie_tarif_nouveau)
    ...
    def AjouterNom(self):
        dlg = wx.TextEntryDialog(self, _("Saisissez le nouveau nom de prestation :"), _("Saisie d'un nouveau nom de prestation"), "")
        if dlg.ShowModal() == wx.ID_OK:
            nom = dlg.GetValue()
            if nom == "":
                dlg2 = wx.MessageDialog(self, _("Le nom que vous avez saisi n'est pas valide."), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg2.ShowModal()
                dlg2.Destroy()
                dlg.Destroy()
                return
    ...
    def ModifierNom(self, IDnom_tarif=None):
        ...
        DB.Close()
        dlg = wx.TextEntryDialog(self, _("Modifiez le nom de la prestation :"), _("Modification d'un nom de prestation"), nom)
        if dlg.ShowModal() == wx.ID_OK:
            nom = dlg.GetValue()
            if nom == "":
    ...
    def SupprimerNom(self, IDnom_tarif=None):
        ...
        DB.Close()
        if len(listeDonnees) > 0 :
            dlg = wx.MessageDialog(self, _("Il est impossible de supprimer un tarif qui comporte au moins un paramétrage !"), _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def SupprimerTarif(self, IDtarif=None):
        ...
        if nbrePrestations > 0 :
            dlg = wx.MessageDialog(self, _("Ce tarif a déjà été attribué à %d prestations.\nIl est donc impossible de le supprimer !") % nbrePrestations, _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def DupliquerNom(self, IDnom_tarif, nomTarif):
        ...
        dlg = wx.MessageDialog(self, _("Souhaitez-vous dupliquer également les tarifs ?"), _("Duplication"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        reponse = dlg.ShowModal() 
        dlg.Destroy()
        if reponse == wx.ID_YES :
            dupliqueEnfants = True
        ...
        # Duplication des noms de tarifs
        newIDnom_tarif = DB.Dupliquer(nomTable="noms_tarifs", nomChampCle="IDnom_tarif", conditions="IDnom_tarif=%d" % IDnom_tarif, dictModifications={"nom":_("Copie de %s") % nomTarif})
