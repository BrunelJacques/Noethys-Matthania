...
class CTRL_Label_prestation(wx.Panel):
    def __init__(self, parent, listeChoix=[]):
        ...
        self.listeChoix.append(("autre:", _("Le label suivant")))

        choices = []
        for code, label in self.listeChoix:
            choices.append(label)
        self.ctrl_choix = wx.Choice(self, -1, choices=choices)
        self.ctrl_choix.SetToolTip(wx.ToolTip(_("Sélectionnez le label de la prestation")))
    ...
    def Validation(self):
        if self.GetCodeSelection() == "autre:":
            dlg = wx.MessageDialog(self, _("Vous avez sélectionné un label de prestation personnalisé mais sans saisir le label souhaité !"),_("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_autre.SetFocus()
            return False
...
class Panel(wx.Panel):
    def __init__(self, parent, IDactivite=None, IDtarif=None, cacher_dates=False):
        ...
        self.check_date_fin = wx.CheckBox(self, -1, _("Jusqu'au"))
        ...
        self.label_label_prestation = wx.StaticText(self, -1, _("Label prestation :"))
        listeChoix = [
            ("nom_tarif", _("Nom du tarif (Par défaut)")),
            ("description_tarif", _("Description du tarif")),
            ]
        ...
        # TVA
        self.label_tva = wx.StaticText(self, -1, _("Taux TVA :"))
        ...
        self.ctrl_description.SetToolTip(wx.ToolTip(_("Saisissez une description explicite pour ce tarif [Optionnel]")))
        self.ctrl_categories.SetToolTip(wx.ToolTip(_("Cochez les catégories de tarifs à rattacher à ce tarif")))
        self.ctrl_tva.SetToolTip(wx.ToolTip(_("Saisissez le taux de TVA inclus [Optionnel]")))
        self.ctrl_code_comptable.SetToolTip(wx.ToolTip(_("Saisissez le code comptable de cette prestation si vous souhaitez utiliser l'export vers les logiciels de comptabilité [Optionnel]")))
    ...
    def Validation(self):
        ...
        if not self.cacher_dates :
            ...
            if self.ctrl_date_debut.GetDate() == None :
                dlg = wx.MessageDialog(self, _("Vous devez saisir une date de début de validité !"), "Erreur", wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
            ...
            if self.check_date_fin.GetValue() == True :
                ...
                if validation == False :
                    return False
                if self.ctrl_date_fin.GetDate() == None :
                    dlg = wx.MessageDialog(self, _("Vous devez saisir une date de fin de validité !"), "Erreur", wx.OK | wx.ICON_EXCLAMATION)
                    dlg.ShowModal()
                    dlg.Destroy()
                    self.ctrl_date_fin.SetFocus()
                    return False
                if self.ctrl_date_fin.GetDate() < self.ctrl_date_debut.GetDate() :
                    dlg = wx.MessageDialog(self, _("Vous devez saisir une date de fin supérieure à la date de début !"), "Erreur", wx.OK | wx.ICON_EXCLAMATION)
                    dlg.ShowModal()
                    dlg.Destroy()
                    self.ctrl_date_fin.SetFocus()
                    return False
        ...
        if self.ctrl_categories.IsShown() and len(listeCategories) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez coché aucune catégorie de tarifs.\nCe tarif sera donc inactif pour le moment...\n\nVoulez-vous quand-même continuer ?") , _("Erreur"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
        ...
        if self.GetLabelPrestation() == "description_tarif" and self.GetDescription() == "" :
            dlg = wx.MessageDialog(self, _("Vous avez sélectionné un label de prestation 'Description du tarif' mais sans saisir de description !"), "Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
