...
class ListBoxCombinaisons_ARCHIVE(wx.ListBox):
    ...
    def Modifier(self):
        ...
        if index == -1 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune combinaison à modifier dans la liste !"), "Information", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self):
        index = self.GetSelection()
        if index == -1 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune combinaison à supprimer dans la liste !"), "Information", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDcombi_tarif = self.GetClientData(index)
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cette combinaison ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
    ...
    def Saisie(self, date=None, listeAnciennesIDunites=None):     
        ...
        if len(listeUnites) == 0 :
            dlg = wx.MessageDialog(self, _("Il n'existe aucune unité pour cette activité !"), "Information", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return None, None
        ...
        if dlg.ShowModal() == wx.ID_OK :
            ...
            if len(listeSelections) == 0 :
                dlg = wx.MessageDialog(self, _("Vous n'avez coché aucune unité !"), "Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return None, None
        ...
        for IDunite in listeIDunites :
            ...
            if len(listeOuvertures) == 0 :
                dlg = wx.MessageDialog(self, _("Une des unités de cette combinaison n'est pas ouverte à la date sélectionnée.\nVous ne pouvez donc pas valider cette combinaison."), "Erreur", wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                DB.Close()
                return None, None
        ...
        if listeAnciennesIDunites != None :
            if listeAnciennesIDunites == listeIDunites and ancienneDate == date :
                dlg = wx.MessageDialog(self, _("Vous n'avez effectué aucune modification !"), "Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return None, None
...
##                dlg = wx.MessageDialog(self, _("Cette combinaison existe déjà !"), "Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
##                dlg.ShowModal()
##                dlg.Destroy()
##                return None, None
...
class Panel(wx.Panel):
    def __init__(self, parent, IDactivite=None, IDtarif=None, nouveauTarif=True):
        ...
        # Intro
        self.radio_conso_sans = wx.RadioButton(self, -1, _("Sans consommations"), style=wx.RB_GROUP)
        self.radio_conso_ouvertures = wx.RadioButton(self, -1, _("Créer les consommations selon le calendrier des ouvertures"))
        self.radio_conso_perso = wx.RadioButton(self, -1, _("Créer les consommations suivantes :"))
        ...
        # Autres paramètres
        self.ctrl_saisie_manuelle = wx.CheckBox(self, -1, _("Ce forfait peut être saisi manuellement depuis la grille des consommations"))
        self.ctrl_saisie_auto = wx.CheckBox(self, -1, _("Ce forfait est automatiquement créé lors de l'inscription à l'activité"))
        self.ctrl_suppression_auto = wx.CheckBox(self, -1, _("Ce forfait ne peut être supprimé que lors de la désinscription à l'activité"))

        # Date de facturation
        self.label_date_facturation = wx.StaticText(self, -1, _("Date de facturation :"))
        listeChoix = [
            ("date_debut_forfait", _("Date de début du forfait")),
            ("date_saisie", _("Date de la saisie du forfait")),
            ("date_debut_activite", _("Date de début de l'activité")),
            ]
    ...
    def __set_properties(self):
        self.radio_conso_ouvertures.SetToolTip(wx.ToolTip(_("Cochez ce choix pour qu'aucune consommation ne soit créée")))
        self.radio_conso_ouvertures.SetToolTip(wx.ToolTip(_("Cochez ce choix pour que le forfait soit associé au calendrier des ouvertures")))
        self.radio_conso_perso.SetToolTip(wx.ToolTip(_("Cochez ce choix pour que le forfait soit basé sur les dates et unités saisies")))
        self.bouton_modifier_combi.SetToolTip(wx.ToolTip(_("Cliquez ici pour saisir ou modifier des combinaisons d'unités pour ce forfait")))
        self.bouton_supprimer_combi.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer en un clic TOUTES les combinaisons d'unités de cette liste")))
        self.ctrl_saisie_manuelle.SetToolTip(wx.ToolTip(_("Ce forfait peut être saisi manuellement dans la grille des consommations grâce à la commande dédiée")))
        self.ctrl_saisie_auto.SetToolTip(wx.ToolTip(_("Ce forfait est automatiquement saisi lorsqu'un individu s'inscrit à l'activité")))
        self.ctrl_suppression_auto.SetToolTip(wx.ToolTip(_("La suppression manuelle est impossible. Le forfait peut être uniquement supprimé lors d'un désincription à l'activité")))
    ...
    def OnSupprimerCombi(self, event): 
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer TOUTES les combinaisons de cette liste ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        if dlg.ShowModal() == wx.ID_YES :
    ...
    def Validation(self):
        if self.radio_conso_perso.GetValue() == True and len(self.ctrl_combinaisons.listeDonnees) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez saisi aucune consommation pour ce forfait daté !"), "Information", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
