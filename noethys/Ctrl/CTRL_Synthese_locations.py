...
class CTRL(gridlib.Grid, glr.GridWithLabelRenderersMixin): 
    ...
    def MAJ(self, date_debut=None, date_fin=None, listeCategories=[], affichage_donnees="quantite",
        ...
        try :
            ...
            dlg = wx.MessageDialog(self, _("Désolé, le problème suivant a été rencontré dans la recherche des données de la synthèse des locations : \n\n%s") % err, _("Erreur"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return False
    ...
    def Apercu(self):
        ...
        dataTableau.append( (_("Synthèse des locations"), _("%s\nEdité le %s") % (UTILS_Organisateur.GetNom(), dateDuJour)) )
        style = TableStyle([
    ...
    def ExportTexte(self, event=None):
        ...
        UTILS_Export.ExportTexte(grid=self, titre=_("Synthèse des locations"))
        
    def ExportExcel(self, event=None):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(grid=self, titre=_("Synthèse des locations"))
