...
class CTRL(gridlib.Grid, glr.GridWithLabelRenderersMixin): 
    ...
    def MAJ(self, date_debut=None, date_fin=None, listeActivites=[], affichage_regroupement="jour", affichage_caisse=None, labelParametres=u""):
        ...
        # init grid
        try :
            dlgAttente = wx.BusyInfo(_("Veuillez patienter durant la recherche des données..."), None)
            ...
            dlg = wx.MessageDialog(self, _("Désolé, le problème suivant a été rencontré dans la recherche des données de la synthèse des déductions : \n\n%s") % err, _("Erreur"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return False
    ...
    def Importation(self):
        ...
        for IDdeduction, IDprestation, IDcompte_payeur, date, montant, label, IDaide, nomIndividu, prenomIndividu, date_naiss, labelPrestation, montantPrestation, montantInitialPrestation, IDfamille, IDactivite, abregeActivite, IDindividu, datePrestation, IDfacture, IDcaisse, num_allocataire, nomCaisse, nomAide in listeDeductions:
            ...
            mois = date.month
            annee = date.year
            nom_complet_individu = "%s %s" % (nomIndividu, prenomIndividu)
    ...
    def InitGrid(self):
        ...
        listeColonnes = [
            ("montant_initial", _("Total initial")),
            ("montant_deduction", _("Total déduit")),
            ("montant_final", _("Total final")),
            ("date_min", _("Date min")),
            ("date_max", _("Date max")),
            ("nbre_dates", _("Nbre dates")),
        ]
        if self.affichage_regroupement == "individu":
            listeColonnes.insert(0, ("num_allocataire", _("N° Allocataire")))
            listeColonnes.insert(0, ("famille", _("Famille")))
    ...
    def Apercu(self):
        ...
        dataTableau.append( (_("Synthèse des déductions"), _("%s\nEdité le %s") % (UTILS_Organisateur.GetNom(), dateDuJour)) )
        style = TableStyle([
    ...
    def ExportTexte(self, event=None):
        ...
        UTILS_Export.ExportTexte(grid=self, titre=_("Synthèse des déductions"))
        
    def ExportExcel(self, event=None):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(grid=self, titre=_("Synthèse des déductions"))
...
class MyFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        self.bouton_test = wx.Button(panel, -1, "Test export Excel")
