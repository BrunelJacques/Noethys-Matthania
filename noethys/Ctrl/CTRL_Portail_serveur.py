...
class Panel(wx.Panel):
    def __init__(self, parent):
        ...
        self.bouton_traiter = wx.Button(self, -1, _("Traiter les\ndemandes"))
        self.bouton_traiter.SetToolTip(wx.ToolTip(_("traiter les demandes")))
        ...
        self.bouton_synchroniser.SetToolTip(wx.ToolTip(_("Synchroniser maintenant les données entre Noethys et Connecthys")))

        self.bouton_outils = wx.BitmapButton(self, -1, wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Outils.png"), wx.BITMAP_TYPE_ANY))
        self.bouton_outils.SetToolTip(wx.ToolTip(_("Outils")))
    ...
    def StartServeur(self, block=True):
        if not hasattr(self, "serveur") :
            if self.lock.acquire(block) == True:
                ...
                return False
            self.serveur.Start()
            return self.EcritLog(_("Client de synchronisation prêt"))
        else :
            self.serveur.Start()
    ...
    def OnBoutonOutils(self, event=None):
        ...
        texte_next_synchro = _("Prochaine synchronisation prévue à %s") % self.serveur.GetHeureProchaineSynchro().strftime("%Hh%M")
        ...
        id = wx.Window.NewControlId()
        item = wx.MenuItem(menuPop, id, _("Synchroniser maintenant"), _("Synchroniser maintenant les données entre Noethys et Connecthys"))
        ...
        id = wx.Window.NewControlId()
        item = wx.MenuItem(menuPop, id, _("Configuration"), _("Accéder à la configuration de Connecthys"))
