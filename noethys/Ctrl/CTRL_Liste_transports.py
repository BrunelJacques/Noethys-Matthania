                ...
                for label, IDindividu, dictDates in listeIndividusTemp :
                    ...
                    for date, sens in dictDates.items() :
                        ...
                        if date in self.dictIndexColonnes :
                            indexColonne = self.dictIndexColonnes[date]
                            if sens == "depart" : label = _("Départ")
                            elif sens == "arrivee" : label = _("Arrivée")
                            else : label = ""
...
##        self.MAJ() 
    ...
    def Imprimer(self, event=None):
        ...
        if len(listeDates) > 26 :
            dlg = wx.MessageDialog(self, _("Désolé mais vous ne pouvez pas imprimer plus de 26 jours sur une feuille !"), _("Information"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        def Header():
            ...
            dataTableau.append( (_("Liste des transports"), _("%s\nEdité le %s") % (UTILS_Organisateur.GetNom(), dateDuJour)) )
            style = TableStyle([
        ...
        for dictCategorie in self.dictImpression["donnees"] :
            ...
            for element in elements :
                ...
                label = element["texte"]
                if "marge" in element :
                    label = "%s%s" % ((element["marge"]-1) * "      ", label)
                ligne = [label,]
...
class MyFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        boutonTest = wx.Button(panel, -1, _("Test d'impression PDF"))
