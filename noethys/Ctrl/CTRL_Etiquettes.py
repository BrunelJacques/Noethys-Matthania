...
##        gc.DrawRectangle(0, 0, tailleImages[0], tailleImages[1])
    ...
    def OnContextMenu(self, event):
        ...
        itemx = wx.MenuItem(menuPop, 60, _("Trier par ordre alphabétique"))
    ...
    def Modifier(self, event):
        ...
        if dictData == None or ("label" in dictData) == False :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner une étiquette à modifier !"), "Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if dlg.ShowModal() == wx.ID_OK :
            ...
            if dictData["IDetiquette"] == IDparent :
                dlg2 = wx.MessageDialog(self, _("Vous ne pouvez pas sélectionner une étiquette parente qui est l'étiquette à modifier !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg2.ShowModal()
                dlg2.Destroy()
                dlg.Destroy()
                return
    ...
    def Supprimer(self, event):
        ...
        if dictData == None or dictData["IDetiquette"] == None:
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner une étiquette à supprimer !"), "Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
            ...
            for dictDataTemp in listeTousEnfants :
                if dictDataTemp["IDetiquette"] in dictConsoAssociees :
                    if dictConsoAssociees[dictDataTemp["IDetiquette"]] > 0 :
                        dlg = wx.MessageDialog(self, _("L'étiquette enfant '%s' qui dépend de l'étiquette sélectionnée a déjà été associée à %d consommation(s) !\n\nVous ne pouvez donc pas supprimer l'étiquette parent." % (dictDataTemp["label"], dictDataTemp["IDetiquette"])), "Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
                        dlg.ShowModal()
                        dlg.Destroy()
                        return
            
            # Demande de confirmation
            dlg = wx.MessageDialog(self, _("Attention, cette étiquette comporte des étiquettes enfants.\n\nSouhaitez-vous vraiment supprimer cette étiquette ? Les étiquettes enfants seront également supprimées !"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
        ...
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cette étiquette ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
...
class DialogSelection(wx.Dialog):
    def __init__(self, parent, listeActivites=[], nomActivite=None):
        ...
        self.parent = parent      
        self.listeActivites = listeActivites
        self.SetTitle(_("Sélectionner des étiquettes"))
