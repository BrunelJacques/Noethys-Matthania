...
##        IDaide, IDfamille, IDactivite, nom, date_debut, date_fin, IDcaisse, montant_max, nbre_dates_max
        ...
        if self.montant_max != None :
            self.texte_montant_max = _("%.2f %s (%.2f %s max)") % (self.total_deductions, SYMBOLE, self.montant_max, SYMBOLE)
        else:
            self.texte_montant_max = "%.2f %s" % (self.total_deductions, SYMBOLE)
            
        if self.nbre_dates_max != None :
            self.texte_dates_max = _("%d dates (%d dates max)") % (self.nbre_deductions, self.nbre_dates_max)
        else:
            self.texte_dates_max = _("%d dates") % self.nbre_deductions
...
##            criteresAides = "WHERE aides.IDfamille IS NOT NULL"
    ...
    def InitObjectListView(self):            
        ...
        if self.IDfamille == None :
            # Version MODELES
            liste_Colonnes = [
                ColumnDefn(_("ID"), "left", 0, "IDaide", typeDonnee="entier"),
                ColumnDefn(u"Du", 'left', 75, "date_debut", typeDonnee="date", stringConverter=FormateDateCourt),
                ColumnDefn(_("Au"), 'left', 75, "date_fin", typeDonnee="date", stringConverter=FormateDateCourt),
                ColumnDefn(_("Nom"), 'left', 160, "nom", typeDonnee="texte"),
                ColumnDefn(_("Activité"), 'left', 60, "nom_activite", typeDonnee="texte"),
                ColumnDefn(_("Caisse"), 'left', 140, "nom_caisse", typeDonnee="texte"),
                ]
        else:
            # Version FAMILLE
            liste_Colonnes = [
                ...
                ColumnDefn(u"Du", 'left', 75, "date_debut", typeDonnee="date", stringConverter=FormateDateCourt),
                ColumnDefn(_("Au"), 'left', 75, "date_fin", typeDonnee="date", stringConverter=FormateDateCourt),
                ColumnDefn(_("Nom"), 'left', 130, "nom", typeDonnee="texte"),
                ColumnDefn(_("Activité"), 'left', 60, "nom_activite", typeDonnee="texte"),
                ColumnDefn(_("Caisse"), 'left', 80, "nom_caisse", typeDonnee="texte"),
                ColumnDefn(_("Bénéficiaires"), 'left', 130, "texteBeneficiaires", typeDonnee="texte"),
                ColumnDefn(_("Total des déductions"), 'left', 140, "texte_montant_max", typeDonnee="texte"),
                ColumnDefn(_("Nbre de dates"), 'left', 130, "texte_dates_max", typeDonnee="texte"),
                ]
        
        self.rowFormatter = rowFormatter
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucune aide journalière"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des aides journalières"), format="A", orientation=wx.LANDSCAPE)
        prt.Preview()
    ...
    def ExportTexte(self, event):
        ...
        UTILS_Export.ExportTexte(self, titre=_("Liste des aides journalières"))
        
    def ExportExcel(self, event):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=_("Liste des aides journalières"))
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune aide à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune aide à supprimer dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDaide = self.Selection()[0].IDaide
        ...
        if len(listeDeductions) > 0 :
            dlg = wx.MessageDialog(self, _("Cette aide a déjà été attribuée à %s déductions. Il est donc impossible de la supprimer !") % len(listeDeductions), _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cette aide ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
