...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        self.SetEmptyListMsg(_("Aucun jour férié %s") % self.type)
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des jours fériés"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun jour férié à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun jour férié à supprimer dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer ce jour férié ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
...
class Saisie(wx.Dialog):
    def __init__(self, parent, type="variable", IDferie=None):
        ...
        self.text_ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez un nom pour ce jour férié (ex: 'Noël', 'Ascension', 'Jour de l'an', etc...)")))
        self.choice_jour_fixe.SetToolTip(wx.ToolTip(_("Sélectionnez le jour")))
        self.choice_mois_fixe.SetToolTip(wx.ToolTip(_("Sélectionnez le mois")))
        self.datepicker_date_variable.SetToolTip(wx.ToolTip(_("Saisissez ou sélectionnez une date pour ce jour férié")))
        ...
        if IDferie == None :
            if self.type == "fixe" :
                self.SetTitle(_("Saisie d'un jour férié fixe"))
            else:
                self.SetTitle(_("Saisie d'un jour férié variable"))
        else:
            if self.type == "fixe" :
                self.SetTitle(_("Modification d'un jour férié fixe"))
            else:
                self.SetTitle(_("Modification d'un jour férié variable"))
    ...
    def OnBoutonOk(self, event):
        if self.GetNom() == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un nom pour ce jour férié !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.type == "fixe" :
            # Variable
            if self.GetJour() == 0 :
                dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner un jour !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.choice_jour_fixe.SetFocus()
                return
            if self.GetMois() == 0 :
                dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner un mois !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
...
class BarreRecherche(wx.SearchCtrl):
    def __init__(self, parent):
        ...
        self.SetDescriptiveText(_("Rechercher un jour férié %s...") % self.parent.type)
