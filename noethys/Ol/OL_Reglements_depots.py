...
class ListView(FastObjectListView):
    ...
    def Deplacer(self):
        if len(self.Selection()) == 0 :
            if self.inclus == True :
                message = _("Vous devez d'abord sélectionner un règlement à retirer du dépôt !")
            else:
                message = _("Vous devez d'abord sélectionner un règlement disponible à ajouter au dépôt !")
    ...
    def GetLabelListe(self, texte=_("règlements")):
    ...
    def InitObjectListView(self):            
        ...
        if self.inclus == True :
            self.SetEmptyListMsg(_("Aucun règlement dans ce dépôt"))
        else:
...
##            self.DefileDernier() 
    ...
    def Supprimer(self, event):
        ...
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer ce règlement ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
