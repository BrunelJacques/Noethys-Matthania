...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):
        ...
        if self.mode == "familles" :
            liste_Colonnes = [
                ...
                ColumnDefn(_("Solde"), 'right', 110, "solde", typeDonnee="montant", stringConverter=FormateSolde),
                ]
        else :
            liste_Colonnes = [
                ColumnDefn(_("ID"), "left", 0, "IDindividu", typeDonnee="entier"),
                ColumnDefn(_("Nom"), 'left', 180, "nom", typeDonnee="texte"),
                ColumnDefn(_("Prénom"), 'left', 180, "prenom", typeDonnee="texte"),
                ColumnDefn(_("Date naiss."), "left", 100, "date_naiss", typeDonnee="date", stringConverter=FormateDate),
                ]

        liste_Colonnes.append(ColumnDefn(_("Etat"), "left", 120, "etat", typeDonnee="texte", stringConverter=FormateEtat, imageGetter=GetImageEtat))
        ...
        if self.mode == "familles":
            ...
            self.SetEmptyListMsg(_("Aucune individu"))
    ...
    def GetParametresImpression(self):
        dictParametres = {
            "titre": _("Liste des %s") % self.mode,
            "total": _("> %s %s") % (len(self.donnees), self.mode),
    ...
    def ExportTexte(self, event):
        ...
        UTILS_Export.ExportTexte(self, titre=_("Liste des %s") % self.mode)
        
    def ExportExcel(self, event):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=_("Liste des %s") % self.mode)
    ...
    def ModifierArchivage(self, etat="archiver"):
        ...
        if len(listeID) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez cocher au moins une ligne !"), _("Action impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
...
class ListviewAvecFooter(PanelAvecFooter):
    def __init__(self, parent, kwargs={}):
        if kwargs["mode"] == "familles" :
            dictColonnes = {"nomTitulaires": {"mode": "nombre", "singulier": _("famille"), "pluriel": _("familles"), "alignement": wx.ALIGN_CENTER},}
        else :
            dictColonnes = {"nom": {"mode": "nombre", "singulier": _("individu"), "pluriel": _("individus"), "alignement": wx.ALIGN_CENTER}, }
