...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ColumnDefn(_("ID"), "left", 0, "IDniveau", typeDonnee="entier"),
            ColumnDefn(_("Ordre"), "center", 0, "ordre", typeDonnee="entier"),
            ColumnDefn(_("Nom du niveau"), "left", 290, "nom", typeDonnee="texte", isSpaceFilling=True),
            ColumnDefn(_("Abrégé"), "left", 110, "abrege", typeDonnee="texte"),
            ]
        
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucun niveau scolaire"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des niveaux scolaires"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun niveau scolaire à modifier dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun niveau scolaire à supprimer dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDniveau = self.Selection()[0].IDniveau
        ...
        if nbreClasses > 0 :
            dlg = wx.MessageDialog(self, _("Ce niveau scolaire a déjà été rattaché à %d classes.\nVous ne pouvez donc le supprimer.") % nbreClasses, _("Avertissement"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        DB.Close()
        if nbreIndividus > 0 :
            dlg = wx.MessageDialog(self, _("Ce niveau scolaire a déjà été rattaché à %d individus.\nVous ne pouvez donc le supprimer.") % nbreIndividus, _("Avertissement"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return

        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer ce niveau scolaire ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
    ...
    def Monter(self, event):
        if UTILS_Utilisateurs.VerificationDroitsUtilisateurActuel("parametrage_niveaux_scolaires", "modifier") == False : return
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun niveau scolaire dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDniveau = self.Selection()[0].IDniveau
...
class DLG_Saisie(wx.Dialog):
    def __init__(self, parent, nom=None, abrege=None):
        ...
        if nom == None :
            self.SetTitle(_("Saisie d'un niveau scolaire"))
        else:
            self.SetTitle(_("Modification d'un niveau scolaire"))
    ...
    def OnBoutonOk(self, event):
        ...
        if nom == "" :
            dlg = wx.MessageDialog(self, _("Vous n'avez saisi aucun nom de niveau !\nExemple : 'Cours préparatoire'"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_nom.SetFocus()
            return
        if abrege == "" :
            dlg = wx.MessageDialog(self, _("Vous n'avez saisi aucun abrégé de niveau !\nExemple : 'CP'"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
...
class BarreRecherche(wx.SearchCtrl):
    def __init__(self, parent):
        ...
        self.SetDescriptiveText(_("Rechercher un niveau..."))
