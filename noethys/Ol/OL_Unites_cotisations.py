...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        def FormateDate(dateDD):
            ...
            if dateDD == datetime.date(2999, 1, 1) : return _("Illimitée")
            date = str(dateDD)
        ...
        def FormateValidite(validite):
            if validite[0] == "periode" :
                return _("Du %s au %s") % (UTILS_Dates.DateDDEnFr(validite[1]), UTILS_Dates.DateDDEnFr(validite[2]))

            if validite[0] == "duree" :
                return _("%d jours / %d mois / %s années") % (validite[3], validite[2], validite[1])
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Validité"), 'left', 175, "validite", typeDonnee="texte", stringConverter=FormateValidite),
            ColumnDefn(_("Montant"), 'right', 70, "montant", typeDonnee="montant", stringConverter=FormateMontant),
            ]
        
        self.rowFormatter = rowFormatter
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucune unité de cotisation"))
    ...
    def OnContextMenu(self, event):
        ...
        # Item Par défaut
        item = wx.MenuItem(menuPop, 60, _("Définir comme unité par défaut"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des unités de cotisations"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Modifier(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune unité de cotisation à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        nomTypeCotisation = self.GetParent().ctrl_nom.GetValue()
    ...
    def Supprimer(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune unité de cotisation à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if IDunite_cotisation:
            ...
            if nbreCotisations > 0 :
                dlg = wx.MessageDialog(self, _("Cette unité de cotisation a déjà été attribuée à %d cotisations(s).\n\nVous ne pouvez donc pas la supprimer !") % nbreCotisations, _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return
        
        # Confirmation de suppression
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cette unité de cotisation ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
    ...
    def SetDefaut(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune unité de cotisation dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
