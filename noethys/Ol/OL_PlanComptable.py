...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        self.SetEmptyListMsg(_("Aucun bloc défini"))
    ...
    def PrepareEnreg(self, listeDonnees, mode):
        if UTILS_Utilisateurs.VerificationDroitsUtilisateurActuel("parametrage_activites", "creer") == False :
            GestionDB.MessageBox(self,"Vous n'avez pas les droits de paramétrage activité")
        else :
    ...
    def Modifier(self, event):
        if len(self.Selection()) == 0 :
            GestionDB.MessageBox(self, "Vous n'avez sélectionné aucun bloc dans la liste")
            return
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun bloc dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
...
class MyFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        self.myOlv = ListView(panel, panel, id=-1, name="OL_test",
