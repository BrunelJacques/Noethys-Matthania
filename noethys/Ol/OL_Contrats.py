...
class Track(object):
    def __init__(self, parent, donnees):
        ...
        if self.type_contrat in (None, "classique") :
            self.type_contrat_str = _("Classique")
        if self.type_contrat == "psu" :
            self.type_contrat_str = _("P.S.U.")
...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ColumnDefn(_("IDcontrat"), "left", 0, "IDcontrat", typeDonnee="entier"),
            ColumnDefn(_("Du"), 'center', 70, "date_debut", typeDonnee="date", stringConverter=DateEngFr),
            ColumnDefn(_("Au"), 'center', 70, "date_fin", typeDonnee="date", stringConverter=DateEngFr),
            ColumnDefn(_("Type"), 'center', 70, "type_contrat_str", typeDonnee="texte"),
        ...
        if self.IDindividu == None :
            liste_Colonnes.insert(4, ColumnDefn(_("Individu"), 'left', 200, "nomCompletIndividu", typeDonnee="texte"))
            
        self.rowFormatter = rowFormatter
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucun contrat"))
    ...
    def OnContextMenu(self, event):
        ...
        self.GenerationContextMenu(menuPop, titre=_("Liste des contrats"))
    ...
    def Modifier(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun contrat à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun contrat à supprimer dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        track = self.Selection()[0]
...
##            return
        ...
        if len(listePrestationsFacturees) > 0 :
            dlg = wx.MessageDialog(self, _("Il est impossible de supprimer ce contrat directement car %d prestations associées apparaissent déjà sur une facture !") % len(listePrestationsFacturees), _("Suppression impossible"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            DB.Close() 
            return
        ...
        if len(listeConsoPointees) > 0 :
            dlg = wx.MessageDialog(self, _("Il est impossible de supprimer ce contrat directement car %d consommations associées sont déjà pointées !") % len(listeConsoPointees), _("Suppression impossible"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return
        
        # Demande de confirmation
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer ce contrat ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
...
class ListviewAvecFooter(PanelAvecFooter):
    def __init__(self, parent, kwargs={}):
        dictColonnes = {
            "date_fin" : {"mode" : "nombre", "singulier" : _("contrat"), "pluriel" : _("contrats"), "alignement" : wx.ALIGN_LEFT},
            "montant" : {"mode" : "total"},
            }
