...
##        self.ctrl_soldes.Layout() 
    ...
    def InitObjectListView(self):            
        ...
        self.SetEmptyListMsg(_("Aucune opération"))
    ...
    def MAJInformations(self):
        ...
        if dictDonnees["nbreOperationsRapprochees"] == 0 :
            texte = _("Aucune opération rapprochée")
        elif dictDonnees["nbreOperationsRapprochees"] == 1 :
            texte = _("1 opération rapprochée")
        else :
            texte = _("%d opérations rapprochées") % dictDonnees["nbreOperationsRapprochees"]
        
        # Opérations au débit
        if dictDonnees["nbreOperationsDebit"] > 0 :
            if dictDonnees["nbreOperationsDebit"] == 1 :
                texte += _("<BR>1 opération au débit : %.2f %s") % (dictDonnees["totalOperationsDebit"], SYMBOLE)
            else :
                texte += _("<BR>%d opérations au débit : %.2f %s") % (dictDonnees["nbreOperationsDebit"], dictDonnees["totalOperationsDebit"], SYMBOLE)
                
        # Opérations au crédit
        if dictDonnees["nbreOperationsCredit"] > 0 :
            if dictDonnees["nbreOperationsCredit"] == 1 :
                texte += _("<BR>1 opération au crédit : %.2f %s") % (dictDonnees["totalOperationsCredit"], SYMBOLE)
            else :
                texte += _("<BR>%d opérations au crédit : %.2f %s") % (dictDonnees["nbreOperationsCredit"], dictDonnees["totalOperationsCredit"], SYMBOLE)
        
        # Modes
        for IDmode, donnees in dictDonnees["modes"].items() :
            if donnees["nbre"] == 1 :
                texte += _("<BR>1 opération de type '%s' : %.2f %s") % (donnees["nom"], donnees["total"], SYMBOLE)
            else :
                texte += _("<BR>%d opérations de type '%s' : %.2f %s") % (donnees["nbre"], donnees["nom"], donnees["total"], SYMBOLE)
