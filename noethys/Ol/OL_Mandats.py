...
class Track(object):
    def __init__(self, donnees, dictTitulaires):
        ...
        if self.typeMandat == "recurrent" :
            self.typeMandatStr = _("Récurrent")
        else :
...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Séquence suiv."), 'left', 100, "sequenceStr", typeDonnee="texte"),
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun mandat à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if track.nbrePrelevements > 0 :
            dlg = wx.MessageDialog(self, _("Ce mandat a déjà été utilisé pour %d prélèvements. Souhaitez-vous tout de même l'ouvrir ?\n\nAttention, il est déconseillé de modifier un mandat déjà utilisé.") % track.nbrePrelevements, _("Avertissement"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun mandat à supprimer dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        track = self.Selection()[0]
        ...
        if track.nbrePrelevements > 0 :
            dlg = wx.MessageDialog(self, _("Suppression impossible ! \n\nCe mandat est déjà rattaché à %d prélèvements.") % track.nbrePrelevements, _("Suppression impossible"), wx.OK|wx.ICON_ERROR)
            dlg.ShowModal() 
            dlg.Destroy()
            return
