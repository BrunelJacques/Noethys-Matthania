...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Nom de la colonne"), "left", 180, "nom", typeDonnee="texte"),
            ColumnDefn(_("Catégories associées"), "left", 110, "texte_categories", typeDonnee="texte", isSpaceFilling=True),
            ]
...
class CTRL_Categories(wx.CheckListBox):
    ...
    def Importation(self):
        ...
        self.Append(_("Toutes les catégories inutilisées"))
        index = 1
...
class DLG_Saisie(wx.Dialog):
    ...
    def OnBoutonOk(self, event):
        ...
        if nom == "" :
            dlg = wx.MessageDialog(self, _("Vous devez saisir un nom pour cette colonne !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_nom.SetFocus()
            return
        if categories == "" :
            dlg = wx.MessageDialog(self, _("Vous devez sélectionner au moins une catégorie !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
