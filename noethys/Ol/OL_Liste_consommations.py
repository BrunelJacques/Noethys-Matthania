...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        def FormateEtat(etat):
            if etat == "reservation": return _("Réservation")
            if etat == "attente": return _("Attente")
            if etat == "refus": return _("Refus")
            if etat == "present": return _("Présent")
            if etat == "absentj": return _("Absence justifiée")
            if etat == "absenti": return _("Absence injustifiée")
        ...
        liste_Colonnes = [
            ColumnDefn(_("ID"), "left", 0, "IDconso", typeDonnee="entier"),
            ColumnDefn(_("Date"), 'left', 90, "date", typeDonnee="date", stringConverter=FormateDateCourt),
            ColumnDefn(_("Activité"), 'left', 120, "nom_activite", typeDonnee="texte"),
            ColumnDefn(_("Unité"), 'left', 100, "nom_unite", typeDonnee="texte"),
            ColumnDefn(_("Etat"), 'left', 100, "etat", typeDonnee="texte", stringConverter=FormateEtat),
            ColumnDefn(_("Groupe"), 'left', 100, "nom_groupe", typeDonnee="texte"),
            ColumnDefn(_("Nom"), 'left', 120, "nom", typeDonnee="texte"),
            ColumnDefn(_("Prénom"), 'left', 120, "prenom", typeDonnee="texte"),
            ColumnDefn(_("Famille"), 'left', 160, "nomTitulaires", typeDonnee="texte"),
            ColumnDefn(_("Date de saisie"), 'left', 90, "date", typeDonnee="date_saisie", stringConverter=FormateDateCourt),
            ColumnDefn(_("Début"), 'left', 60, "heure_debut", typeDonnee="texte"),
            ColumnDefn(_("Fin"), 'left', 60, "heure_fin", typeDonnee="texte"),
            ColumnDefn(_("Forfait"), 'left', 100, "forfait", typeDonnee="texte", stringConverter=Formate_forfait),
            ColumnDefn(_("Evénement"), 'left', 110, "evenement", typeDonnee="texte"),
            ColumnDefn(_("IDprestation"), "left", 80, "IDprestation", typeDonnee="entier"),
            ColumnDefn(_("Label prestation"), 'left', 140, "label_prestation", typeDonnee="texte"),
            ColumnDefn(_("Montant prestation"), 'left', 120, "montant_prestation", typeDonnee="montant", stringConverter=FormateMontant),
        ]
    ...
    def GetParametresImpression(self):
        ...
        dictParametres = {
            "titre" : _("Liste des consommations"),
            "intro" : intro,
    ...
    def Supprimer(self, event):
        ...
        if track.etat in ("present", "absenti", "absentj"):
            dlg = wx.MessageDialog(self, _("Cette consommation ne peut pas être supprimée car elle a déjà été pointée !"), _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if track.IDprestation:
            dlg = wx.MessageDialog(self, _("Attention, cette consommation est associée à la prestation ID%d %s. Cette prestation ne sera pas supprimée automatiquement. Souhaitez-vous tout de même continuer ?") % (track.IDprestation, track.label_prestation), _("Avertissement"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
            if dlg.ShowModal() != wx.ID_YES:
                return False

        # Demande de confirmation de suppression
        dlg = wx.MessageDialog(self, _("Confirmez-vous la suppression de cette consommation ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
        if dlg.ShowModal() == wx.ID_YES :
            DB = GestionDB.DB()
            ...
            UTILS_Historique.InsertActions([{
                ...
                "IDcategorie": 10,
                "action": _("Suppression de la consommation ID%d : %s le %s pour %s %s") % (track.IDconso, track.nom_unite, track.date, track.nom, track.prenom),
                },])
