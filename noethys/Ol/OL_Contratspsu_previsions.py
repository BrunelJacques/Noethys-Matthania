...
class Track(object):
    ...
    def MAJ(self):
        ...
        if self.quantite != None :
            self.texteDetail += _(" Qté=%d") % self.quantite
        
        if self.etat == "reservation" : 
            self.texteEtat = _("Réservation")
        elif self.etat == "present" : 
            self.texteEtat = _("Présent")
        elif self.etat == "attente" : 
            self.texteEtat = _("Attente")
        elif self.etat == "absenti" : 
            self.texteEtat = _("Absence injustifiée")
        elif self.etat == "absentj" : 
            self.texteEtat = _("Absence justifiée")
...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            #ColumnDefn(_("Unité"), 'left', 70, "nomUnite", typeDonnee="texte", isSpaceFilling=True),
            #ColumnDefn(_("Etat"), 'left', 50, "texteEtat", typeDonnee="texte"),
            ColumnDefn(_("Détail"), 'left', 100, "texteDetail", typeDonnee="texte"),
            ColumnDefn(_("Durée réelle"), 'center', 90, "duree_reelle", typeDonnee="texte", stringConverter=FormateDuree),
            ColumnDefn(_("Durée retenue"), 'center', 90, "duree_arrondie", typeDonnee="texte", stringConverter=FormateDuree),
            ]
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des prévisions"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def ExportTexte(self, event):
        ...
        UTILS_Export.ExportTexte(self, titre=_("Liste des prévisions"), autoriseSelections=False)
        
    def ExportExcel(self, event):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=_("Liste des prévisions"), autoriseSelections=False)
    ...
    def Modifier(self, event):  
        if len(self.Selection()) == 0 :
           dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune consommation à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
           dlg.ShowModal()
           dlg.Destroy()
           return
    ...
    def Supprimer(self, event):  
        if len(self.Selection()) == 0 and len(self.GetTracksCoches()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune consommation à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if len(self.GetTracksCoches()) > 0 :
            ...
            dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer les consommations cochées ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
            ...
            dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer la consommation sélectionnée ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
            reponse = dlg.ShowModal() 
            dlg.Destroy()
            if reponse != wx.ID_YES :
                return
        ...
        for track in listeSelections :
            if track.etat in ("present", "absenti", "absentj") :
                dlg = wx.MessageDialog(self, _("Vous ne pouvez pas supprimer la consommation '%s' du %s car elle est déjà pointée !") % (track.nomUnite, UTILS_Dates.DateDDEnFr(track.date)) , _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return
    ...
    def Generation(self, listeConso=[], IDconso=None):
        ...
        for dictConso in listeConso :
            ...
            if IDgroupe != None and ((dictConso["date"], dictConso["IDunite"], IDgroupe) in self.dictOuvertures) == False :
                listeAnomalies.append(_("%s : Unité %s fermée") % (dateFr, self.dictUnites[dictConso["IDunite"]]["nom"]))
                valide = False
        ...
        if len(listeAnomalies) :
            message1 = _("Les %d anomalies suivantes ont été trouvées.\n\nSouhaitez-vous tout de même générer les %d autres consommations ?") % (len(listeAnomalies), nbreConsoValides)
            message2 = "\n".join(listeAnomalies)
            dlg = dialogs.MultiMessageDialog(self, message1, caption = _("Génération"), msg2=message2, style = wx.ICON_EXCLAMATION | wx.YES|wx.CANCEL|wx.YES_DEFAULT, btnLabels={wx.ID_YES : _("Oui"), wx.ID_CANCEL : _("Annuler")})
            reponse = dlg.ShowModal()
            dlg.Destroy()
            if reponse != wx.ID_YES :
                return False
        ...
        if nbreConsoValides == 0 :
            dlg = wx.MessageDialog(self, _("Il n'y a aucune consommation à générer !"), _("Génération"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if IDconso == None :
            dlg = wx.MessageDialog(self, _("Confirmez-vous la génération de %d consommations ?") % nbreConsoValides, _("Génération"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
            reponse = dlg.ShowModal()
            dlg.Destroy()
            if reponse != wx.ID_YES :
                return False
