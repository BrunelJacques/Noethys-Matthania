...
class Analyse():
    ...
    def GetValeurs(self):
        ...
        listeCategories.append({
            ...
            "IDcategorie" : None, "valeur" : None, "nomCategorie" : _("Total"),
    ...
    def InitObjectListView(self):            
        ...
        def FormateMontant(valeur):
            ...
            try :
                ...
                return "%.2f %s" % (float(valeur), SYMBOLE)
            except :
                return valeur
        ...
        def FormateType(valeur):
            if valeur == "credit" : return _("Crédit")
            if valeur == "debit" : return _("Débit")
            if valeur == "total" : return _("Total")
            
            
        liste_Colonnes = [
##            ColumnDefn(u"", "left", 0, "IDcategorie_budget"),
            ColumnDefn(_("Catégorie budgétaire"), "left", 200, "nomCategorie", typeDonnee="texte", isSpaceFilling=True),
            ColumnDefn(_("Type"), "left", 50, "typeCategorie", typeDonnee="texte", stringConverter=FormateType),
            ColumnDefn(_("Réel"), "right", 80, "realise", typeDonnee="montant", stringConverter=FormateMontant),
            ColumnDefn(_("Budgété"), "right", 80, "plafond", typeDonnee="montant", stringConverter=FormateMontant),
            ColumnDefn(_("Pourcent."), "right", 80, "pourcentage", stringConverter=FormatePourcentage),
            ColumnDefn(_("Ecart"), "right", 80, "solde", typeDonnee="montant", stringConverter=FormateEcart),
            ]
        ...
        self.SetEmptyListMsg(_("Aucune catégorie budgétaire"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des catégories budgétaires"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def ExportTexte(self, event):
        ...
        UTILS_Export.ExportTexte(self, titre=_("Liste des catégories budgétaires"))
        
    def ExportExcel(self, event):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=_("Liste des catégories budgétaires"))
