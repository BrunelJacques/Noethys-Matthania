    ...
    def __init__(self, donnees, dictModeles):
        ...
        if self.profil.startswith("modele:") :
            ...
            if IDmodele in dictModeles :
                ...
                self.texteDroits = _("Modèle de droits inconnu")

        elif self.profil == "perso" :
            self.texteDroits = _("Droits personnalisés")
        
        else :
            self.texteDroits = _("Administrateur")
        
        # Image
        if self.nomImage == None or self.nomImage == "Automatique" :
            if self.sexe == "M" : 
                self.image = wx.Bitmap(Chemins.GetStaticPath("Images/Avatars/16x16/Homme.png"), wx.BITMAP_TYPE_PNG)
            else:
                self.image = wx.Bitmap(Chemins.GetStaticPath("Images/Avatars/16x16/Femme.png"), wx.BITMAP_TYPE_PNG)
        else :
            self.image = wx.Bitmap(Chemins.GetStaticPath("Images/Avatars/16x16/%s.png" % self.nomImage), wx.BITMAP_TYPE_PNG)
...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Nom"), 'left', 150, "nom", typeDonnee="texte"),
            ColumnDefn(_("Prénom"), 'left', 150, "prenom", typeDonnee="texte"),
            ColumnDefn(_("Actif"), "left", 60, "texteActif", typeDonnee="texte", imageGetter=GetImageActif),
            ColumnDefn(_("Droits"), 'left', 200, "texteDroits", typeDonnee="texte"),
            ]
        
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucun utilisateur"))
    ...
    def OnContextMenu(self, event):
        ...
        # Item Historique
        item = wx.MenuItem(menuPop, 60, _("Historique"))
        bmp = wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Horloge3.png"), wx.BITMAP_TYPE_PNG)
        item.SetBitmap(bmp)
        menuPop.AppendItem(item)
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des utilisateurs"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Modifier(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun utilisateur à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun utilisateur à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        DB.Close()
        if nbreConso > 0 :
            dlg = wx.MessageDialog(self, _("Cet utilisateur a déjà été attribué à %d consommation(s).\n\nVous ne pouvez donc pas le supprimer !") % nbreConso, _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if profil == "administrateur" :
            ...
            for track in self.donnees :
                if track.profil == "administrateur" and track.IDutilisateur != IDutilisateur : 
                    nbreAdmin += 1
            if nbreAdmin == 0 :
                dlg = wx.MessageDialog(self, _("Vous ne pouvez pas supprimer le seul administrateur !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return
            
        # Suppression
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cet utilisateur ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
    ...
    def Historique(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun utilisateur dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDutilisateur = self.Selection()[0].IDutilisateur
...
class BarreRecherche(wx.SearchCtrl):
    def __init__(self, parent):
        ...
        self.SetDescriptiveText(_("Rechercher un utilisateur..."))
