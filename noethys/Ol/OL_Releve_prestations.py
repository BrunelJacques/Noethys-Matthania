...
class Track(object):
    def __init__(self, periode={}, index=None):
        ...
        if "impayes" in self.dictOptions and self.dictOptions["impayes"] == True :
            self.listeOptions.append(_("Uniquement les impayés"))
        ...
        if "regroupement" in self.dictOptions :
            ...
            if self.dictOptions["regroupement"] == "annee" :
                self.listeOptions.append(_("Regroupement par année"))
                
        if "conso" in self.dictOptions and self.dictOptions["conso"] == True :
            self.listeOptions.append(_("Détail des consommations"))
...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):        
        ...
        liste_Colonnes = [
            ColumnDefn(_("Période"), 'left', 180, "label_periode", typeDonnee="entier"),
            ColumnDefn(_("Type"), "left", 100, "label_type", typeDonnee="texte"),
            ColumnDefn(_("Options"), "left", 230, "label_options", typeDonnee="texte"),
            ColumnDefn(_("Date de début"), "left", 0, "date_debut", typeDonnee="date"),
    ...
    def OnContextMenu(self, event):
        ...
        # Tout sélectionner
        item = wx.MenuItem(menuPop, 20, _("Tout sélectionner"))
        menuPop.AppendItem(item)
        self.Bind(wx.EVT_MENU, self.CocheTout, id=20)

        # Tout dé-sélectionner
        item = wx.MenuItem(menuPop, 30, _("Tout dé-sélectionner"))
        menuPop.AppendItem(item)
    ...
    def Impression(self, mode="preview"):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des périodes"), intro="", total="", format="A", orientation=wx.LANDSCAPE)
        if mode == "preview" :
            prt.Preview()
        else:
            prt.Print()
    ...
    def ExportTexte(self, event):
        ...
        UTILS_Export.ExportTexte(self, titre=_("Liste des périodes"))
        
    def ExportExcel(self, event):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=_("Liste des périodes"))
    ...
    def Modifier(self, event=None):
        ...
        dlg = DLG_Releve_prestations_saisie.Dialog(self)
        dlg.SetTitle(_("Modification d'une période"))
