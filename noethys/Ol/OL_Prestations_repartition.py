...
class Track(object):
    def __init__(self, donnees):
        ...
        if self.IDfacture == None :
            ...
            if num_facture != None :
                self.label_facture = "n°%d" % num_facture
            else :
                self.label_facture = ""
...
##            dictVentilation[IDprestation] = montantVentilation
    ...
    def InitObjectListView(self):
        ...
        self.SetColumns([
            ...
            ColumnDefn(_("Date"), "left", 160, "date", typeDonnee="date", stringConverter=FormateDate),
##            ColumnDefn(_("Catégorie"), "left", 100, "categorie"),
            ColumnDefn(_("Individu"), "left", 75, "prenomIndividu", typeDonnee="texte"),
            ColumnDefn(_("Activité"), "left", 55, "nomAbregeActivite", typeDonnee="texte"),
            ColumnDefn(_("Label"), "left", 155, "label", typeDonnee="texte"),
            ColumnDefn(_("Montant"), "right", 65, "montant", typeDonnee="montant", stringConverter=FormateMontant),
            ColumnDefn(_("Réglé"), "right", 72, "montant_ventilation", typeDonnee="montant", stringConverter=FormateMontant, imageGetter=GetImageVentilation),
            ColumnDefn(_("Déduc."), "right", 55, "montant_deduction", typeDonnee="montant", stringConverter=FormateMontant),
##            ColumnDefn(_("Tarif"), "left", 140, "nomTarif"),
            ColumnDefn(_("Catégorie de tarif"), "left", 100, "nomCategorieTarif", typeDonnee="texte"),
            ColumnDefn(_("N° Facture"), "left", 70, "label_facture", typeDonnee="texte"),
        ])
##        self.SetShowGroups(False)
...
##        self.SetSortColumn(self.columns[indexColonne-1], resortNow=True)
    ...
    def Supprimer(self, event):
        ...
        if len(self.GetTracksCoches()) > 0 :
            ...
            dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer les %d prestations cochées ?") % len(listeSelections), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
            ...
            dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer la prestation n°%d ?") % listeSelections[0].IDprestation, _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
            reponse = dlg.ShowModal() 
            dlg.Destroy()
            if reponse != wx.ID_YES :
                return
        ...
        for track in listeSelections :
            ...
            if track.forfait == 2 :
                dlg = wx.MessageDialog(self, _("La prestation n°%d est un forfait non supprimable !\n\n(Pour le supprimer, vous devez obligatoirement désinscrire l'individu de l'activité)") % track.IDprestation, _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                valide = False
            ...
            if valide == True and track.IDfacture != None :
                dlg = wx.MessageDialog(self, _("La prestation n°%d est déjà facturée. Vous ne pouvez donc pas la supprimer !") % track.IDprestation, _("Suppression impossible"), wx.OK | wx.ICON_INFORMATION)
                dlg.ShowModal()
                dlg.Destroy()
                valide = False
            ...
            if len(listeConsommations) > 0 and valide == True :
                message = _("Attention, la prestation n°%d est rattachée aux %d consommation(s) suivantes :\n\n") % (track.IDprestation, len(listeConsommations))
                ...
                dlg = wx.MessageDialog(self, message, _("Suppression"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
                reponse = dlg.ShowModal() 
                dlg.Destroy()
        ...
        dlg = wx.MessageDialog(self, _("%d prestation(s) ont été supprimée(s) avec succès.") % len(listeSuppressions), _("Suppression"), wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
