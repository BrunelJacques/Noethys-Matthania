...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Préfixe"), 'center', 150, "prefixe", typeDonnee="texte"),
            ColumnDefn(_("Nbre factures associées"), 'center', 150, "nbre_factures", typeDonnee="entier"),
            ]

        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucun préfixe"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des préfixes de factures"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun préfixe à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if track.nbre_factures > 0 :
            dlg = wx.MessageDialog(self, _("Ce préfixe est déjà associé à %d factures.\n\nIl sera donc uniquement possible de modifier le nom du préfixe.") % track.nbre_factures, _("Avertissement"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun préfixe à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDprefixe = self.Selection()[0].IDprefixe
        
        # Vérifie que ce préfixe n'a pas déjà été attribué à une facture
        DB = GestionDB.DB()
        ...
        DB.Close()
        if nbreFactures > 0 :
            dlg = wx.MessageDialog(self, _("Ce préfixe a déjà été attribué à %d facture(s).\n\nVous ne pouvez donc pas le supprimer !") % nbreFactures, _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return

        # Suppression
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer ce préfixe ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
