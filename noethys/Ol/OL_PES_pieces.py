...
class Track(object):
    def __init__(self, donnees, dictTitulaires={}, dictIndividus={}):
        ...
        self.code_tiers = "FAM%06d" % self.IDfamille
    ...
    def InitTitulaireHelios(self):
        if self.titulaire_helios in self.dictIndividus :
            ...
            if self.titulaireCivilite == None :
                self.titulaireNomComplet = "%s %s" % (self.titulaireNom, self.titulairePrenom)
            else :
                self.titulaireNomComplet = "%s %s %s" % (self.titulaireCivilite, self.titulaireNom, self.titulairePrenom)
            self.titulaireNomPrenom = "%s %s" % (self.titulaireNom, self.titulairePrenom)
            ...
            self.titulaireAdresse = "%s %s %s" % (self.titulaireRue, self.titulaireCP, self.titulaireVille)
        else :
    ...
    def AnalysePiece(self):
        ...
        if self.titulaire_helios in (None, "") : 
            listeProblemes.append(_("Titulaire Hélios manquant"))
        else :
            if self.titulaireRue in (None, "") or self.titulaireCP in (None, "") or self.titulaireVille in (None, "") : 
                listeProblemes.append(_("Adresse du titulaire Hélios incomplète"))
        ...
        if self.prelevement == 1 :
            if self.prelevement_iban in (None, "") or self.prelevement_bic in (None, "") or self.prelevement_titulaire in (None, "") :
                listeProblemes.append(_("Coordonnées bancaires incomplètes"))
            if self.prelevement_rum in (None, "") or self.prelevement_IDmandat in (None, "") or self.prelevement_date_mandat in (None, "") :
                listeProblemes.append(_("Mandat SEPA manquant"))
            if self.prelevement_sequence in (None, "") :
                listeProblemes.append(_("Séquence SEPA non renseignée"))
        
        if len(listeProblemes) > 0 :
            self.analysePiece = False
            self.analysePieceTexte = ", ".join(listeProblemes)
        else :
            self.analysePiece = True
            self.analysePieceTexte = _("Pièce valide")
...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Analyse pièce"), 'left', 120, "analysePieceTexte", typeDonnee="texte", imageGetter=GetImageAnalysePiece),
            ColumnDefn(_("Famille"), 'left', 230, "titulaires", typeDonnee="texte"),
##            ColumnDefn(_("Type"), 'left', 70, "type", stringConverter=FormateType),
            ColumnDefn(_("Libellé"), 'left', 110, "libelle", typeDonnee="texte"),
            ColumnDefn(_("Montant"), 'right', 70, "montant", typeDonnee="montant", stringConverter=FormateMontant),
            ColumnDefn(_("Statut"), 'left', 80, "prelevement_statut", typeDonnee="texte", stringConverter=FormateStatut, imageGetter=GetImageStatut),
            ColumnDefn(_("Règlement"), 'left', 70, "reglement", typeDonnee="bool", stringConverter=FormateReglement, imageGetter=GetImageReglement),
            ColumnDefn(_("Prélèvt"), 'left', 55, "prelevement", typeDonnee="bool", stringConverter=FormatePrelevement, imageGetter=GetImagePrelevement),
            ColumnDefn(_("Séquence"), 'left', 70, "prelevement_sequence", typeDonnee="texte"),
            ColumnDefn(_("IBAN"), 'left', 190, "prelevement_iban", typeDonnee="texte"),
            ColumnDefn(_("BIC"), 'left', 100, "prelevement_bic", typeDonnee="texte"),
            ColumnDefn(_("Titulaire compte bancaire"), 'left', 160, "prelevement_titulaire", typeDonnee="texte"),
            ColumnDefn(_("Ref. mandat"), 'left', 90, "prelevement_rum", typeDonnee="texte"),
            ColumnDefn(_("Date mandat"), 'left', 80, "prelevement_date_mandat", typeDonnee="date", stringConverter=FormateDateCourt),
            ColumnDefn(_("Titulaire Hélios"), 'left', 150, "titulaireNomComplet", typeDonnee="texte"),
            ColumnDefn(_("Adresse"), 'left', 220, "titulaireAdresse", typeDonnee="texte"),
            ColumnDefn(_("ID Tiers"), 'left', 70, "idtiers_helios", typeDonnee="texte"),
            ColumnDefn(_("Type IDTiers"), 'left', 70, "natidtiers_helios", typeDonnee="texte"),
            ColumnDefn(_("Ref. Tiers"), 'left', 70, "reftiers_helios", typeDonnee="texte"),
            ColumnDefn(_("Cat. Tiers"), 'left', 70, "cattiers_helios", typeDonnee="texte"),
            ColumnDefn(_("Nat. Jur."), 'left', 70, "natjur_helios", typeDonnee="texte"),
            ColumnDefn(_("Code compta."), 'left', 95, "code_compta", typeDonnee="texte"),
            ]
        

        self.SetColumns(liste_Colonnes)
        self.CreateCheckStateColumn(0)
        self.SetEmptyListMsg(_("Aucune pièce"))
...
##        self.Bind(wx.EVT_MENU, self.Saisie_manuelle, id=10)
        ...
        # Item Supprimer
        item = wx.MenuItem(menuPop, 30, _("Retirer"))
        ...
        item = wx.MenuItem(menuPop, 800, _("Activer le prélèvement"))
        ...
        # Item Activer le prélèvement
        item = wx.MenuItem(menuPop, 801, _("Désactiver le prélèvement"))
        item.SetBitmap(wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Prelevement.png"), wx.BITMAP_TYPE_PNG))
        menuPop.AppendItem(item)
    ...
    def Impression(self):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des pièces du bordereau"), intro=txtIntro, total=txtTotal, format="A", orientation=wx.LANDSCAPE)
        return prt
    ...
    def ExportTexte(self, event=None):
        ...
        UTILS_Export.ExportTexte(self, titre=_("Liste des pièces du bordereau"))
        
    def ExportExcel(self, event=None):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=_("Liste des pièces du bordereau"))
    ...
    def Saisie_factures(self, event=None):
        ...
        if self.GetParent().GetVerrouillage() == True :
            dlg = wx.MessageDialog(self, _("Vous ne pouvez pas effectuer cette action car le lot est verrouillé !"), _("Verrouillage"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def AjoutFactures(self, tracks=[]):
        ...
        for track in tracks :
            ...
            if IDmandat == None :
                ...
                listeMandatsNonValides.append(_("Facture n°%s de la famille %s") % (track.numero, nomTitulaires))
            ...
            dictTemp = {
                ...
                "libelle" : _("FACT%s") % track.numero, "montant" : montant, "prelevement_statut" : "attente", "IDlot" : self.IDlot, "etat" : "ajout", "IDreglement" : None, "dateReglement" : None,
...
##            message1 = _("Les factures suivantes n'ont pas été intégrées car les mandats des familles sont indisponibles ou non valides :")
##            message2 = "\n".join(listeMandatsNonValides)
##            dlg = dialogs.MultiMessageDialog(self, message1, caption=_("Avertissement"), msg2=message2, style = wx.ICON_EXCLAMATION |wx.OK, icon=None, btnLabels={wx.ID_OK : _("Ok")})
##            reponse = dlg.ShowModal() 
##            dlg.Destroy() 
    ...
    def Modifier(self, event=None):
        if self.GetParent().GetVerrouillage() == True :
            ...
            return
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune pièce à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event=None):
        ...
        if len(self.Selection()) == 0 and len(self.GetTracksCoches()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune pièce à retirer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if len(self.GetTracksCoches()) > 0 :
            ...
            dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment retirer les %d pièces cochées ?") % len(listeSelections), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
            reponse = dlg.ShowModal() 
            dlg.Destroy()
            ...
            dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment retirer la pièce sélectionnée ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
            reponse = dlg.ShowModal() 
            dlg.Destroy()
            if reponse != wx.ID_YES :
                return
        ...
        if len(listeDepots) > 0 :
            dlg = wx.MessageDialog(self, _("Suppression interdite car %d pièce(s) de la sélection ont déjà un règlement attribué à un dépôt de règlement !") % len(listeDepots), _("Remarque"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def ActiverPrelevement(self, event):
        ...
        if len(self.Selection()) == 0 and len(self.GetTracksCoches()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune pièce dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if len(self.GetTracksCoches()) > 0 :
            # Sélection multiple
            listeSelections = self.GetTracksCoches()
            dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment modifier le prélèvement des %d pièces cochées ?") % len(listeSelections), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
            reponse = dlg.ShowModal()
            dlg.Destroy()
            if reponse != wx.ID_YES :
                return
            ...
            # Sélection unique
            listeSelections = self.Selection()
            dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment modifier le prélèvement de la pièce sélectionnée ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
            reponse = dlg.ShowModal()
            dlg.Destroy()
            if reponse != wx.ID_YES :
                return
    ...
    def SetStatut(self, statut="attente") :
        ...
        if len(listeDepots) > 0 :
            dlg = wx.MessageDialog(self, _("Notez que Noethys n'a pas procédé changement de statut de %d pièce(s) car le règlement correspondant appartient déjà à un dépôt de règlement !") % len(listeDepots), _("Remarque"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def SetRegle(self, etat=False) :
        ...
        if len(listeAnomalies) > 0 :
            dlg = wx.MessageDialog(self, _("Notez que Noethys n'a pas procédé au règlement de %d pièce(s) en raison de leur statut 'Refus'.") % len(listeAnomalies), _("Remarque"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return

        if len(listeDepots) > 0 :
            dlg = wx.MessageDialog(self, _("Notez que Noethys n'a pas procédé à la suppression de %d pièce(s) car le règlement correspondant appartient déjà à un dépôt de règlement !") % len(listeDepots), _("Remarque"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def GetTexteTotaux(self):
        ...
        # Création du texte
        if nbreTotal == 0 :
            texte = _("<B>Aucune pièce   </B>")
        elif nbreTotal == 1 :
            texte = _("<B>%d pièce (%.2f %s) : </B>") % (nbreTotal, montantTotal, SYMBOLE)
        else :
            texte = _("<B>%d pièces (%.2f %s) : </B>") % (nbreTotal, montantTotal, SYMBOLE)
        ...
        for key in ("attente", "valide", "refus", "regle", "pasregle") :
            if key in dictDetails :
                ...
                if dictDetail["nbre"] == 1 :
                    ...
                    if key == "valide" : label = _("validé")
                    if key == "refus" : label = _("refusé")
                    if key == "regle" : label = _("réglé")
                    if key == "pasregle" : label = _("non réglé")
                else :
                    if key == "attente" : label = _("en attente")
                    if key == "valide" : label = _("validés")
                    if key == "refus" : label = _("refusés")
                    if key == "regle" : label = _("réglés")
                    if key == "pasregle" : label = _("non réglés")
