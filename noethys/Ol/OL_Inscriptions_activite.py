    ...
    def __init__(self, donnees=[], dictGroupes={}, dictInscriptions={}):
        ...
        for dictGroupe in self.liste_groupes :
            if dictGroupe["nbre_inscrits_max"] not in (None, 0) :
                ...
                label = _("%s : %d/%d" % (dictGroupe["nom"], nbre_inscrits_groupes, dictGroupe["nbre_inscrits_max"]))
                liste_labels_groupes.append(label)

        self.label_places = ", ".join(liste_labels_groupes)

        if len(self.label_places) == 0 :
            self.label_places = _("Places illimitées")
        ...
        if self.date_debut == "1977-01-01" and self.date_fin == "2999-01-01" :
            ...
            if self.date_debut != None and self.date_fin != None :
                ...
                #self.labelPeriode = _("Du %s au %s") % (UTILS_Dates.DateEngFr(self.date_debut), UTILS_Dates.DateEngFr(self.date_fin))
            else:
                self.periode = None
...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        def FormatePeriode(periode):
            ...
            return _("Du %s au %s") % (UTILS_Dates.DateEngFr(date_debut), UTILS_Dates.DateEngFr(date_fin))

        liste_Colonnes = [
            ColumnDefn(_("IDactivite"), "left", 0, "IDactivite", typeDonnee="entier"),
            ColumnDefn(_("Nom de l'activité"), 'left', 220, "nom", typeDonnee="texte", isSpaceFilling=True),
            ColumnDefn(_("Période de validité"), 'left', 180, "periode", typeDonnee="texte", stringConverter=FormatePeriode),
            ColumnDefn(_("Etat des places"), 'left', 250, "label_places", typeDonnee="texte"),
            ]
