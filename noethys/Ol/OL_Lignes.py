...
class ListView(FastObjectListView):
    def __init__(self, *args, **kwds):
        ...
        self.categorieSingulier = kwds.pop("categorieSingulier", "") # "ligne de bus"
        self.categoriePluriel = kwds.pop("categoriePluriel", "") # "lignes de bus"
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ColumnDefn(_("ID"), "left", 0, "IDligne", typeDonnee="entier"),
            ColumnDefn(_("Nom"), 'left', 430, "nom", typeDonnee="texte"),
            ColumnDefn(_("Nbre arrêts"), "center", 100, "nbreArrets", typeDonnee="entier"),
            ]
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune %s à modifier !") % self.categorieSingulier, _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune %s à supprimer !") % self.categorieSingulier, _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        if self.Selection()[0].nbreArrets > 0 :
            dlg = wx.MessageDialog(self, _("Il est impossible de supprimer une ligne qui a déjà un ou plusieurs arrêts !"), _("Suppression impossible"), wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cette %s ?") % self.categorieSingulier, _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
...
class BarreRecherche(wx.SearchCtrl):
    def __init__(self, parent):
        ...
        self.SetDescriptiveText(_("Rechercher une ligne..."))
...
class MyFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        self.myOlv = ListView(panel, id=-1, categorie="bus", categorieSingulier=_("ligne de bus"), categoriePluriel=_("lignes de bus"), name="OL_test", style=wx.LC_REPORT|wx.SUNKEN_BORDER|wx.LC_SINGLE_SEL|wx.LC_HRULES|wx.LC_VRULES)
        self.myOlv.MAJ() 
