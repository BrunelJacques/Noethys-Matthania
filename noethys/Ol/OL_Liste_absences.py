...
class Track(object):
    def __init__(self, donnees):
        ...
        if self.prenom == None : 
            self.prenom = ""
        self.nomComplet = "%s %s" % (self.nom, self.prenom)
...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Individu"), 'left', 200, "nomComplet", typeDonnee="texte"),
            ColumnDefn(_("Nbre"), 'center', 70, "nbreDates", typeDonnee="entier"),
            ColumnDefn(_("Absences"), "left", 450, "labelConsommations", typeDonnee="texte"),
            ]        
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucune absence"))
    ...
    def Impression(self, mode="preview"):
        ...
        total = _("> %d individus") % len(self.donnees)
        from Utils import UTILS_Printer
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des absences"), intro=intro, total=total, format="A", orientation=wx.PORTRAIT)
        if mode == "preview" :
            prt.Preview()
        else:
            prt.Print()
    ...
    def ExportTexte(self, event):
        ...
        UTILS_Export.ExportTexte(self, titre=_("Liste des absences"))
        
    def ExportExcel(self, event):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=_("Liste des absences"))
