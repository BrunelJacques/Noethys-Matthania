...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Ordre"), "center", 70, "index", typeDonnee="entier", stringConverter=FormateIndex),
            ColumnDefn(_("Titre"), "left", 230, "titre", typeDonnee="texte", isSpaceFilling=True),
            ]
        
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucun onglet"))
    ...
    def Modifier(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun onglet  à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun onglet à supprimer dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        track = self.Selection()[0]
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cet onglet ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
    ...
    def Monter(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun onglet dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        track = self.Selection()[0]
...
class DLG_Saisie_element(wx.Dialog):
    def __init__(self, parent):
        ...
        self.label_titre = wx.StaticText(self, -1, _("Titre de l'onglet :"))
    ...
    def __set_properties(self):
        self.SetTitle(_("Saisie d'un onglet"))
        self.ctrl_titre.SetToolTip(wx.ToolTip(_("Saisissez ici le titre de l'onglet")))
    ...
    def OnBoutonOk(self, event):
        ...
        if len(self.ctrl_titre.GetValue()) == 0:
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un titre pour cet onglet !"), "Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
    ...
    def SetDonnees(self, dictDonnees={}):
        ...
        self.SetTitle(_("Modification d'un onglet"))
...
class MyFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        listeParametres = [
            {"IDelement" : 1, "titre" : "Onglet 1", "texte_xml" : "", "texte_html" : ""},
            {"IDelement" : 2, "titre" : "Onglet 2", "texte_xml" : "", "texte_html" : ""},
        ]
