...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Nom"), "left", 290, "nom", typeDonnee="texte", isSpaceFilling=True),
            ]
        
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucune catégorie"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des catégories de menus"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Ajouter(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            IDcategorie = DB.ReqInsert("menus_categories", listeDonnees)
            DB.Close()
    ...
    def Modifier(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            DB.ReqMAJ("menus_categories", listeDonnees, "IDcategorie", IDcategorie)
            DB.Close()
            self.MAJ(IDcategorie)
        dlg.Destroy()
    ...
    def Supprimer(self, event):
        ...
        FROM menus
        WHERE IDcategorie=%s;""" % IDcategorie
        DB.ExecuterReq(req,MsgBox="ExecuterReq")
        listeMenus = DB.ResultatReq()
        DB.Close()
        if len(listeMenus) > 0 :
            dlg = wx.MessageDialog(self, _("Cette catégorie a déjà été rattachée à %d menus.\nVous ne pouvez donc la supprimer.") % len(listeMenus), _("Avertissement"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if dlg.ShowModal() == wx.ID_YES :
            ...
            DB.Close() 
            self.MAJordre(IDcategorie)
            self.MAJ()
        dlg.Destroy()

    def MAJordre(self, IDcategorie=None):
        DB = GestionDB.DB()
        ordre = 1
        for index in range(0, len(self.donnees)) :
            objet = self.GetObjectAt(index)
            if objet.IDcategorie != IDcategorie :
                DB.ReqMAJ("menus_categories", [("ordre", ordre),], "IDcategorie", objet.IDcategorie)
                ordre += 1
        DB.Close()
    ...
    def Monter(self, event):
        if UTILS_Utilisateurs.VerificationDroitsUtilisateurActuel("parametrage_menus_categories", "modifier") == False : return
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune catégorie dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDcategorie = self.Selection()[0].IDcategorie
        ...
        DB.ReqMAJ("menus_categories", [("ordre", ordre),], "IDcategorie", IDcategorie2)
        DB.Close()
        self.MAJ(IDcategorie)
...
class DLG_Saisie(wx.Dialog):
    def __init__(self, parent, nom=None):
        ...
        if nom == None :
            self.SetTitle(_("Saisie d'une catégorie"))
        else:
            self.SetTitle(_("Modification d'une catégorie"))
    ...
    def OnBoutonOk(self, event):
        ...
        if nom == "" :
            dlg = wx.MessageDialog(self, _("Vous n'avez saisi aucun nom de catégorie !\nExemples : 'Midi', 'goûter', 'Dessert', etc..."), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
    ...
    def OnBoutonAide(self, event): 
        ...
        UTILS_Aide.Aide("Categoriesdemenus")
