...
##            ColumnDefn(_("Type"), 'left', 70, "type", stringConverter=FormateType),
            ColumnDefn(_("Libellé"), 'left', 110, "libelle"),
##            ColumnDefn(_("Banque"), 'left', 120, "nomBanque"),
            ...
            ColumnDefn(_("Règlement"), 'left', 70, "reglement", stringConverter=FormateReglement, imageGetter=GetImageReglement),
...
##        self.Bind(wx.EVT_MENU, self.Saisie_manuelle, id=10)
    ...
    def Impression(self):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des prélèvements"), intro=txtIntro, total=txtTotal, format="A", orientation=wx.LANDSCAPE)
        return prt
    ...
    def ExportTexte(self, event=None):
        ...
        UTILS_Export.ExportTexte(self, titre=_("Liste des prélèvements"))
        
    def ExportExcel(self, event=None):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=_("Liste des prélèvements"))
    ...
    def Supprimer(self, event=None):
        if len(self.Selection()) == 0 and len(self.GetTracksCoches()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun prélèvement à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if len(self.GetTracksCoches()) > 0 :
            ...
            dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer les %d prélèvements cochés ?") % len(listeSelections), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
            reponse = dlg.ShowModal() 
            dlg.Destroy()
            ...
            dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer le prélèvement sélectionné ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
            reponse = dlg.ShowModal() 
            dlg.Destroy()
            if reponse != wx.ID_YES :
                return
