...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ColumnDefn(_("ID"), "left", 0, "IDtype_groupe_activite", typeDonnee="entier"),
            ColumnDefn(_("Nom"), "left", 300, "nom", typeDonnee="texte"), 
            ColumnDefn(_("Nbre d'activités rattachées"), "left", 150, "nbreActivitesRattachees", typeDonnee="entier"), 
            ]
        
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucun groupe d'activités"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des groupes d'activités"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Ajouter(self, event):
        ...
        dlg = wx.TextEntryDialog(self, _("Saisissez le nom du nouveau groupe d'activités :"), _("Saisie d'un nouveau groupe"), "")
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun groupe d'activités à modifier dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDtype_groupe_activite = self.Selection()[0].IDtype_groupe_activite
        nom = self.Selection()[0].nom
        dlg = wx.TextEntryDialog(self, _("Modifiez le nom du groupe d'activités :"), _("Modification d'un groupe d'activités"), nom)
        if dlg.ShowModal() == wx.ID_OK:
            nom = dlg.GetValue()
            if nom == "":
                dlg = wx.MessageDialog(self, _("Le nom que vous avez saisi n'est pas valide."), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun groupe d'activités à supprimer dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if nbreActivitesRattachees > 0 :
            dlg = wx.MessageDialog(self, _("Une ou plusieurs activités sont déjà rattachées à ce groupe. Vous ne pouvez donc pas le supprimer !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer ce groupe d'activités ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
...
class BarreRecherche(wx.SearchCtrl):
    def __init__(self, parent):
        ...
        self.SetDescriptiveText(_("Rechercher un groupe d'activités..."))
