...
class Track(object):
    def __init__(self, parent, donnees):
        ...
        self.nomComplet_utilisateur = "%s %s" % (self.nom_utilisateur, self.prenom_utilisateur)
        # Famille
        ...
        if self.IDfamille != None :
            if self.IDfamille in parent.titulaires :
                ...
                self.nomTitulaires = _("Aucun titulaire")
        else:
            self.nomTitulaires = ""
        # Individu
        ...
        if self.prenom_individu == None : 
            self.prenom_individu = ""
        if self.IDindividu != None :
            self.nomComplet_individu = "%s %s" % (self.nom_individu, self.prenom_individu)
        else:
            self.nomComplet_individu = ""
        # Catégorie
...
##        self.Bind(wx.EVT_LIST_ITEM_ACTIVATED, self.OnItemActivated)
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ColumnDefn(_("ID"), "left", 0, "IDaction", typeDonnee="entier"),
            ColumnDefn(_("Date"), 'center', 70, "dateHeure", typeDonnee="date", stringConverter=FormateDate),
            ColumnDefn(_("Heure"), 'center', 60, "heure", typeDonnee="texte"),
            ColumnDefn(_("Utilisateur"), 'left', 130, "nomComplet_utilisateur", typeDonnee="texte"),
            ColumnDefn(_("Famille"), 'left', 120, "nomTitulaires", typeDonnee="texte"),
            ColumnDefn(_("Individu"), 'left', 120, "nomComplet_individu", typeDonnee="texte"),
            ColumnDefn(_("Catégorie"), 'left', 150, "nomCategorie", typeDonnee="texte"),
            ColumnDefn(_("Action"), 'left', 700, "action", typeDonnee="texte"),
            ]
        
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucun historique"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Historique"), format="A", orientation=wx.LANDSCAPE)
        prt.Preview()
    ...
    def ExportTexte(self, event):
        ...
        UTILS_Export.ExportTexte(self, titre=_("Historique"))
        
    def ExportExcel(self, event):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=_("Historique"))
