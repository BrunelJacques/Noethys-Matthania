...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        def FormatePeriode(periode):
            if periode == None or periode == "" : return ""
            ...
            return "%s %s" % (listeMois[int(mois)], annee)

        liste_Colonnes = [
            ColumnDefn(_("ID"), "left", 42, "IDlot", typeDonnee="entier", imageGetter=GetImageVerrouillage),
            ColumnDefn(_("Période"), "left", 150, "periode", typeDonnee="texte", stringConverter=FormatePeriode),
            ColumnDefn(_("Nom du lot"), "left", 190, "nom", typeDonnee="texte"),
            ColumnDefn(_("Format"), "left", 170, "nom_format", typeDonnee="texte"),
            ColumnDefn(_("Nbre pièces"), "center", 80, "nbrePieces", typeDonnee="entier"),
            ColumnDefn(_("Observations"), "left", 200, "observations", typeDonnee="texte"),
            ]
        
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucun lot"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des bordereaux PES ORMC"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun bordereau à modifier dans la liste !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun bordereau à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        track = self.Selection()[0]
        if track.verrouillage == 1 :
            dlg = wx.MessageDialog(self, _("Il est impossible de supprimer ce bordereau car il est verrouillé."), _("Suppression impossible"), wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        nbrePieces = track.nbrePieces
        if nbrePieces > 0 :
            dlg = wx.MessageDialog(self, _("Il est impossible de supprimer ce bordereau puisqu'il est déjà constitué de %d pièce(s) !") % nbrePieces, _("Suppression impossible"), wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer ce bordereau ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
