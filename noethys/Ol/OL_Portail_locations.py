...
class Track(object):
    def __init__(self, parent, donnees):
        ...
        if self.etat == "ajouter":
            self.action = "Ajouter %s de %s à %s" % (self.nom_produit, self.date_debut.strftime("%d/%m/%Y-%H:%M"), self.date_fin.strftime("%d/%m/%Y-%H:%M"))
            liste_periodes_non_dispo = Verif_dispo_produit(self)
            if len(liste_periodes_non_dispo) > 0:
                self.action_possible = False
                self.statut = _("Produit non disponible sur la période demandée")
            else:
                self.action_possible = True
                self.statut = _("Ajout possible")
        ...
        if self.etat == "modifier":
            if self.IDlocation in parent.dict_locations_existantes:
                ...
                self.action = "Modifier %s de %s à %s > %s de %s à %s" % (
                ...
                self.action_possible = True
                self.statut = _("Modification possible")
                ...
                self.action_possible = False
                self.statut = _("La location à modifier est inexistante")
                self.action = _("Modifier : Location initiale inexistante !")

        if self.etat == "supprimer":
            self.action = "Supprimer %s de %s à %s" % (self.nom_produit, self.date_debut.strftime("%d/%m/%Y-%H:%M"), self.date_fin.strftime("%d/%m/%Y-%H:%M"))
            # Recherche la location à supprimer
            if self.IDlocation in parent.dict_locations_existantes:
                self.action_possible = True
                self.statut = _("Suppression possible")
            else:
                self.action_possible = False
                self.statut = _("La location à supprimer est inexistante")

        if self.partage:
            self.action += " (Partage autorisé)"

        if self.resultat == "ok":
            self.action_possible = False
            if self.etat == "ajouter": self.statut = _("Ajout effectué")
            if self.etat == "modifier": self.statut = _("Modification effectuée")
            if self.etat == "supprimer": self.statut = _("Suppression effectuée")

        if self.resultat == "refus":
            self.action_possible = True
            if self.etat == "ajouter": self.statut = _("Ajout refusé")
            if self.etat == "modifier": self.statut = _("Modification refusée")
            if self.etat == "supprimer": self.statut = _("Suppression refusée")
...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Action"), 'left', 500, "action", typeDonnee="texte", imageGetter=GetImageEtat),
            # ColumnDefn(_("Action"), 'left', 100, "etat", typeDonnee="texte", stringConverter=FormateEtat, imageGetter=GetImageEtat),
            # ColumnDefn(_("Produit"), 'left', 150, "nom_produit", typeDonnee="texte"),
            # ColumnDefn(_("Début"), 'centre', 110, "date_debut", typeDonnee="date", stringConverter=FormateDateDT),
            # ColumnDefn(_("Fin"), 'centre', 110, "date_fin", typeDonnee="date", stringConverter=FormateDateDT),
            ColumnDefn(_("Statut"), 'left', 280, "statut", typeDonnee="texte", imageGetter=GetImageStatut),
            ]

        self.SetColumns(liste_Colonnes)
        self.CreateCheckStateColumn(0)
        self.SetEmptyListMsg(_("Aucune action"))
    ...
    def OnContextMenu(self, event):
        ...
        self.GenerationContextMenu(menuPop, titre=_("Liste des actions"))
