...
def FormatDuree(validite):
    ...
    if validite != None and validite.startswith("j") :
        ...
        listItems = []
        if jours == 1:
            textJours = _("%d jour") % jours
            listItems.append(textJours)
        if jours > 1:
            textJours = _("%d jours") % jours
            listItems.append(textJours)
        if mois > 0:
            textMois = _("%d mois") % mois
            listItems.append(textMois)
        if annees == 1:
            textAnnees = _("%d année") % annees
            listItems.append(textAnnees)
        if annees > 1:
            textAnnees = _("%d années") % annees
            listItems.append(textAnnees)
    ...
    if validite != None and validite.startswith("d") :
        return _("Jusqu'au %s") % UTILS_Dates.DateEngFr(validite[1:])
...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ColumnDefn(_("ID"), "left", 0, "IDtype_piece", typeDonnee="entier"),
            ColumnDefn(_("Nom de la pièce"), 'left', 220, "nom", typeDonnee="texte"),
            ColumnDefn(_("Public"), "left", 70, "txt_public", typeDonnee="texte"),
            ColumnDefn(_("Validité"), "left", 140, "txt_duree_validite", typeDonnee="texte"),
            ColumnDefn(_("Rattachement valide"), "left", 120, "txt_rattachement", typeDonnee="texte"),
            ]
        
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucun type de pièce"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des types de pièces"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun type de pièce dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        ...
        DB.Close()
        if nbrePieces > 0 :
            dlg = wx.MessageDialog(self, _("Ce type de pièce a déjà été attribué à %d pièce(s).\n\nVous ne pouvez donc pas le supprimer !") % nbrePieces, _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        DB.Close()
        if nbreActivites > 0 :
            dlg = wx.MessageDialog(self, _("Ce type de pièce a déjà été attribué comme pièce obligatoire à %d activité(s).\n\nVous ne pouvez donc pas le supprimer !") % nbreActivites, _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        
        # Confirmation
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer ce type de pièce ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
...
class BarreRecherche(wx.SearchCtrl):
    def __init__(self, parent):
        ...
        self.SetDescriptiveText(_("Rechercher un type de pièce..."))
