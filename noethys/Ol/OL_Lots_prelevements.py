...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Nbre Prélèv."), "center", 80, "nbrePrelevements", typeDonnee="entier"),
        ...
        self.SetEmptyListMsg(_("Aucun lot de prélèvements"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des lots de prélèvements"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun lot à modifier dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun lot à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        nbrePrelevements = self.Selection()[0].nbrePrelevements
        if nbrePrelevements > 0 :
            dlg = wx.MessageDialog(self, _("Il est impossible de supprimer ce lot puisqu'il est déjà constitué de %d prélèvements(s) !") % nbrePrelevements, _("Suppression impossible"), wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer ce lot de prélèvements ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
    ...
    def DemanderTypePrelevement(self):
        ...
        # listeBoutons = [
        #     (_("Prélèvement SEPA"), _("Nouvelle norme de prélèvements bancaires valables dans toute l'Europe à partir du 1er février 2014. Veuillez à saisir les mandats de vos usagers avant d'utiliser ce nouveau type de prélèvement.")),
        #     (_("Prélèvement National"), _("Type de prélèvement valable uniquement jusqu'au 1er février 2014 en France. Ce type de prélèvement sera impossible au-delà de cette date.")),
        #     ]
...
class BarreRecherche(wx.SearchCtrl):
    def __init__(self, parent):
        ...
        self.SetDescriptiveText(_("Rechercher un lot de prélèvements..."))
