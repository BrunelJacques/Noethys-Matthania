...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        self.SetEmptyListMsg(_("Aucun émetteur"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des émetteurs"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Ajouter(self, event):
        ...
        if self.IDmode == None :
            dlg = wx.MessageDialog(self, _("Vous devez d'abord sélectionner un mode de règlement !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun émetteur à modifier dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun émetteur à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDemetteur = self.Selection()[0].IDemetteur
        ...
        DB.Close()
        if nbreReglements > 0 :
            message = _("Attention, cet émetteur a déjà été attribué à %d règlement(s). Si vous le supprimez, il sera également dissocié des règlements associés !\n\nSouhaitez-vous tout de même le supprimer ?") % nbreReglements
        ...
        # Suppression
        dlg = wx.MessageDialog(self, _("Confirmez-vous vraiment la suppression de cet émetteur ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
