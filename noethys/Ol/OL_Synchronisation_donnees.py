...
class Track(object):
    def __init__(self, donnees={}, dictIndividus=None, dictUnites=None, dictParametres=None, nomFichier=""):
        ...
        self.statut = None
        self.appareil = "%s (%s)" % (self.nom_appareil, self.ID_appareil)
        ...
        if self.categorie == "consommation" :
            ...
            self.nomUnite = dictUnites.get(self.IDunite, "<Unité supprimée>")
            if self.IDindividu in dictIndividus :
                self.nomIndividu = dictIndividus[self.IDindividu]
            else :
                self.nomIndividu = _("Individu ID%d inexistant") % self.IDindividu
            self.detail = _("%s %s le %s pour %s") % (self.action.capitalize(), self.nomUnite, UTILS_Dates.DateDDEnFr(self.date), self.nomIndividu)
        ...
        if self.categorie == "memo_journee" :
            ...
            if self.IDindividu in dictIndividus :
                ...
                self.nomIndividu = _("Individu ID%d inexistant") % self.IDindividu
            self.detail = _("%s le %s pour %s le texte '%s'") % (self.action.capitalize(), UTILS_Dates.DateDDEnFr(self.date), self.nomIndividu, self.texte)
    
        # Recherche anomalie
        self.anomalie = False
        if (self.IDindividu in dictIndividus) == False :
            self.anomalie = _("Individu ID%d inexistant dans le fichier") % donnees["IDindividu"]
...
class ListView(GroupListView):
    ...
    def InitObjectListView(self):          
        ...
        def FormateCategorie(categorie):
            if categorie == "consommation" : return _("Consommation")
            if categorie == "memo_journee" : return _("Mémo journalier")
            return "?"
        
        liste_Colonnes = [
            ColumnDefn(_("Horodatage"), "left", 130, "horodatage", typeDonnee="dateheure", stringConverter=FormateHorodatage),
            ColumnDefn(_("Statut"), "left", 50, "statut", typeDonnee="texte", imageGetter=GetImageStatut),
            ColumnDefn(_("Catégorie"), "left", 120, "categorie", typeDonnee="texte", stringConverter=FormateCategorie),
            ColumnDefn(_("Action"), "left", 100, "action", typeDonnee="texte", stringConverter=Capitalize),
            ColumnDefn(_("Individu"), "left", 150, "nomIndividu", typeDonnee="texte"),
            ColumnDefn(_("Détail"), "left", 300, "detail", typeDonnee="texte"),
            ColumnDefn(_("Appareil (ID)"), "left", 130, "appareil", typeDonnee="texte"),
            ColumnDefn(_("Nom du fichier"), "left", 290, "nomFichier", typeDonnee="texte"),
            ]
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des données à synchroniser"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def ExportTexte(self, event):
        ...
        UTILS_Export.ExportTexte(self, titre=_("Liste des données à synchroniser"))
        
    def ExportExcel(self, event):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=_("Liste des données à synchroniser"))
