...
class ListView(FastObjectListView):
    ...
    def Impression(self, mode="preview"):
        if self.donnees == None or len(self.donnees) == 0 :
            ...
            dlg.Destroy()
            return
        total = _("> %d lignes") % len(self.donnees)
        from Utils import UTILS_Printer
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste personnalisée"), intro=total, total=total, format="A", orientation=wx.PORTRAIT)
        if mode == "preview" :
            prt.Preview()
        else:
            prt.Print()
    ...
    def ExportTexte(self, event):
        ...
        UTILS_Export.ExportTexte(self, titre=_("Liste personnalisée"))
        
    def ExportExcel(self, event):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=_("Liste personnalisée"))
