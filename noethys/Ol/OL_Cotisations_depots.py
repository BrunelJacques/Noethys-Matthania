...
class ListView(FastObjectListView):
    ...
    def Deplacer(self):
        if len(self.Selection()) == 0 :
            if self.inclus == True :
                message = _("Vous devez d'abord sélectionner une cotisation à retirer du dépôt !")
            else:
                message = _("Vous devez d'abord sélectionner une cotisation disponible à ajouter au dépôt !")
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("au"), 'left', 70, "date_fin", typeDonnee="date", stringConverter=FormateDate),
            ColumnDefn(_("Type"), 'left', 150, "typeStr", typeDonnee="texte"),
            ColumnDefn(_("Famille"), 'left', 130, "nomTitulaires", typeDonnee="texte"),
            ColumnDefn(_("Individu"), 'left', 130, "individu", typeDonnee="texte"),
            ColumnDefn(_("Nom"), 'left', 270, "nomCotisation", typeDonnee="texte"),
            ColumnDefn(_("Numéro"), 'left', 80, "numero", typeDonnee="texte"),
            ColumnDefn(_("Création carte"), 'left', 100, "date_creation_carte", typeDonnee="date", stringConverter=FormateDate, imageGetter=GetImageCreation),
            ColumnDefn(_("Activités"), 'left', 150, "activitesStr"),
            ColumnDefn(_("Notes"), 'left', 150, "observations"),
            ]
        
        self.SetColumns(liste_Colonnes)
        if self.inclus == True :
            self.SetEmptyListMsg(_("Aucune cotisation dans ce dépôt"))
        else:
            self.SetEmptyListMsg(_("Aucune cotisation disponible"))
...
##                
##        # Item Supprimer
##        item = wx.MenuItem(menuPop, 20, _("Supprimer"))
...
##        menuPop.AppendSeparator()
    ...
    def Impression(self, mode="preview"):
        ...
        txtTotal = _("%d cotisations") % len(self.donnees)
        from Utils import UTILS_Printer
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des cotisations"), intro=txtIntro, total=txtTotal, format="A", orientation=wx.LANDSCAPE)
        if mode == "preview" :
            prt.Preview()
        else:
            prt.Print()
    ...
    def ExportTexte(self, event):
        ...
        UTILS_Export.ExportTexte(self, titre=_("Liste des cotisations"))
        
    def ExportExcel(self, event):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=_("Liste des cotisations"))
    ...
    def Supprimer(self, event):
        ...
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cette cotisation ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
