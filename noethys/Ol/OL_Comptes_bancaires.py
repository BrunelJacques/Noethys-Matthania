...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Intitulé"), 'left', 200, "nom", typeDonnee="texte"),
            ColumnDefn(_("Numéro"), "left", 100, "numero", typeDonnee="texte"),
            ColumnDefn(_("Raison sociale"), "left", 170, "raison", typeDonnee="texte"),
            ColumnDefn(_("Code Etab."), "left", 80, "code_etab", typeDonnee="texte"),
            ColumnDefn(_("Code Guichet"), "left", 90, "code_guichet", typeDonnee="texte"),
            ColumnDefn(_("N° NNE"), "left", 80, "code_nne", typeDonnee="texte"),
            ]

        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucun compte bancaire"))
    ...
    def OnContextMenu(self, event):
        ...
        # Item Par défaut
        item = wx.MenuItem(menuPop, 60, _("Définir comme compte par défaut"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des comptes bancaires"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun compte à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun compte à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        DB.Close()
        if nbre > 0 :
            dlg = wx.MessageDialog(self, _("Ce compte a déjà été attribué à %d règlement(s).\n\nVous ne pouvez donc pas le supprimer !") % nbre, _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        DB.Close()
        if nbre > 0 :
            dlg = wx.MessageDialog(self, _("Ce compte a déjà été attribué à %d dépôt(s) de règlements.\n\nVous ne pouvez donc pas le supprimer !") % nbre, _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        DB.Close()
        if nbre > 0 :
            dlg = wx.MessageDialog(self, _("Ce compte a déjà été attribué à %d opérations comptables.\n\nVous ne pouvez donc pas le supprimer !") % nbre, _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        DB.Close()
        if nbre > 0 :
            dlg = wx.MessageDialog(self, _("Ce compte a déjà été attribué à %d relevés de compte.\n\nVous ne pouvez donc pas le supprimer !") % nbre, _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return

        # Suppression
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer ce compte bancaire ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
...
class Saisie(wx.Dialog):
    def __init__(self, parent, nom=None, numero=None):
        ...
        self.label_nom = wx.StaticText(self, -1, _("Intitulé du compte :"))
        self.ctrl_nom = wx.TextCtrl(self, -1, "")
        if nom != None :
            self.ctrl_nom.SetValue(nom)
        self.label_numero = wx.StaticText(self, -1, _("Numéro de compte :"))
        ...
        if nom == None :
            self.SetTitle(_("Saisie d'un compte"))
        else:
            self.SetTitle(_("Modification d'un compte"))
        self.SetMinSize((450, -1))
        
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez ici l'intitulé du compte")))
        self.ctrl_numero.SetToolTip(wx.ToolTip(_("Saisissez ici le numéro du compte")))
    ...
    def OnBoutonOk(self, event):
        ...
        if nom == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un nom de compte !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_nom.SetFocus()
            return
        if numero == "" :
            dlg = wx.MessageDialog(self, _("Etes-vous sûr de ne pas saisir de numéro de compte ?"), _("Confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
...
class BarreRecherche(wx.SearchCtrl):
    def __init__(self, parent):
        ...
        self.SetDescriptiveText(_("Rechercher un compte bancaire..."))
