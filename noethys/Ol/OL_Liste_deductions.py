...
##        self.Bind(wx.EVT_LIST_ITEM_ACTIVATED, self.OnItemActivated)
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Label déduction"), 'left', 220, "label", typeDonnee="texte"),
            ...
            ColumnDefn(_("Activité"), 'left', 70, "abregeActivite", typeDonnee="texte"),
            ...
            ColumnDefn(_("Facturé"), 'left', 60, "IDfacture", typeDonnee="texte", stringConverter=FormateFacture),
            ]

        self.SetColumns(liste_Colonnes)
        self.CreateCheckStateColumn(0)
        self.SetEmptyListMsg(_("Aucune déduction"))
    ...
    def GetParametresImpression(self):
        dictParametres = {
            ...
            "total" : _("%s déductions") % len(self.donnees),
    ...
    def Modifier(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune déduction à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        if len(self.Selection()) == 0 and len(self.GetTracksCoches()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune déduction à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if len(self.GetTracksCoches()) > 0 :
            ...
            dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer les %d déductions cochées ?") % len(listeSelections), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
            ...
            dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer la déduction n°%d ?") % listeSelections[0].IDdeduction, _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
            reponse = dlg.ShowModal() 
            dlg.Destroy()
            if reponse != wx.ID_YES :
                return
        ...
        for track in listeSelections :
            if track.IDfacture != None :
                nbreFactures += 1
        if nbreFactures > 0 :
            dlg = wx.MessageDialog(self, _("Suppression impossible : %d déductions sont associées à des prestations déjà facturées !") % nbreFactures, _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False                
...
class ListviewAvecFooter(PanelAvecFooter):
    def __init__(self, parent, kwargs={}):
        dictColonnes = {
            "nomTitulaires" : {"mode" : "nombre", "singulier" : _("déduction"), "pluriel" : _("déductions"), "alignement" : wx.ALIGN_CENTER},
            "montant" : {"mode" : "total"},
