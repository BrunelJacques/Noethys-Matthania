...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        def FormateValidite(nbreJoursRestants):
            if nbreJoursRestants == None :
                ...
                return _("Expire aujourd'hui")
            elif nbreJoursRestants < 0 :
                return _("Vaccin périmé")
            else:
                return _("Expire dans %d jours") % nbreJoursRestants
        ...
        liste_Colonnes = [
            ColumnDefn(_("ID"), "left", 0, "IDvaccin", typeDonnee="entier"),
            ColumnDefn(_("Nom du vaccin"), 'left', 140, "nom", typeDonnee="texte"),
            ColumnDefn(_("Validité"), "left", 120, "nbreJoursRestants", typeDonnee="texte", stringConverter=FormateValidite),
            ColumnDefn(_("Maladies associées"), "left", 110, "txt_maladies_associees", typeDonnee="texte", isSpaceFilling=True),
            ]
        
        self.rowFormatter = rowFormatter
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucun vaccin"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des vaccins"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun vaccin à modifier dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun vaccin à supprimer dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer ce vaccin ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
...
class Choice(wx.Choice):
    def __init__(self, parent):
        ...
        self.parent = parent
        self.listeData = []
        self.SetToolTip(wx.ToolTip(_("Sélectionnez le vaccin")))
...
class Saisie(wx.Dialog):
    def __init__(self, parent, IDtype_vaccin=None, date=None):
        ...
        self.label_vaccin = wx.StaticText(self, -1, _("Vaccin :"))
        ...
        if IDtype_vaccin == None :
            self.SetTitle(_("Saisie d'un vaccin"))
        else:
            self.SetTitle(_("Modification d'un vaccin"))
    ...
    def OnBoutonOk(self, event):
        if self.ctrl_vaccin.GetID() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner un vaccin dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.ctrl_date.GetDate() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date pour ce vaccin !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_date.SetFocus()
            return
...
class BarreRecherche(wx.SearchCtrl):
    def __init__(self, parent):
        ...
        self.SetDescriptiveText(_("Rechercher un vaccin..."))
