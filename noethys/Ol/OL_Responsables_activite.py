...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Nom du responsable"), 'left', 190, "nom", imageGetter=GetImageSexe),
            ColumnDefn(_("Fonction"), "left", 130, "fonction"),
            ]
        
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucun responsable"))
    ...
    def OnContextMenu(self, event):
        ...
        # Item Par défaut
        item = wx.MenuItem(menuPop, 40, _("Définir comme responsable par défaut"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des responsables d'activités"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Modifier(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun responsable dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        if len(self.Selection()) == 0 :
            ...
            dlg.Destroy()
            return
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer ce responsable ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
...
class Saisie(wx.Dialog):
    def __init__(self, parent, sexe="H", nom=None, fonction=None):
        ...
        self.label_sexe = wx.StaticText(self, -1, _("Genre du responsable :"))
        self.ctrl_sexe = wx.Choice(self, -1, choices=[_("Homme"), _("Femme")])
        if sexe == "H" :
            self.ctrl_sexe.Select(0)
        else:
            self.ctrl_sexe.Select(1)
        self.label_nom = wx.StaticText(self, -1, _("Nom du responsable :"))
        self.ctrl_nom = wx.TextCtrl(self, -1, "")
        if nom != None :
            self.ctrl_nom.SetValue(nom)
        self.label_fonction = wx.StaticText(self, -1, _("Fonction :"))
        ...
        if nom == None :
            self.SetTitle(_("Saisie d'un responsable"))
        else:
            self.SetTitle(_("Modification d'un responsable"))
        self.SetMinSize((400, -1))
        
        self.ctrl_sexe.SetToolTip(wx.ToolTip(_("Sélectionnez ici le genre du responsable.")))
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez ici le nom du responsable. Ex.: 'David DUPOND' ")))
        self.ctrl_fonction.SetToolTip(wx.ToolTip(_("Saisissez ici la fonction du responsable. Ex. : 'Directeur'")))
    ...
    def OnBoutonOk(self, event):
        ...
        if nom == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un nom de responsable !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_nom.SetFocus()
            return
        if fonction == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une fonction pour ce responsable !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
...
class BarreRecherche(wx.SearchCtrl):
    def __init__(self, parent):
        ...
        self.SetDescriptiveText(_("Rechercher un responsable..."))
