...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ColumnDefn(_("ID"), "left", 0, "IDtype_vaccin", typeDonnee="entier"),
            ColumnDefn(_("Nom du vaccin"), 'left', 200, "nom", typeDonnee="texte"),
            ColumnDefn(_("Durée de validité"), "left", 140, "txt_duree_validite", typeDonnee="texte"),
            ColumnDefn(_("Maladies associées"), "left", 220, "txt_maladies_associees", typeDonnee="texte"),
            ]
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des types de vaccins"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Ajouter(self, event):
        ...
        from Dlg import DLG_Saisie_typesVaccins
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun vaccin dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        ...
        DB.Close()
        if nbreVaccins > 0 :
            dlg = wx.MessageDialog(self, _("Ce vaccin a déjà été attribué %d fois.\n\nVous ne pouvez donc pas le supprimer !") % nbreVaccins, _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
