...
def VentilationAuto(IDcompte_payeur=None, IDreglement=None):
    ...
    for dictPrestation in listePrestations : 
        ...
        if ResteAVentiler < FloatToDecimal(0.0) :
            dlg = wx.MessageDialog(None, _("Ventilation automatique impossible !\n\nLa ventilation automatique n'est pas compatible avec les prestations comportant un montant négatif ! Vous devez donc effectuer une ventilation manuelle."), _("Information"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            DB.Close()
            return False
...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Règlements"), 'right', 80, "total_reglements", typeDonnee="montant", stringConverter=FormateMontant),
            ColumnDefn(_("Total ventilé"), 'right', 80, "total_ventilations", typeDonnee="montant", stringConverter=FormateMontant),
        ...
        self.SetEmptyListMsg(_("Aucun problème de ventilation"))
    ...
    def OnContextMenu(self, event):
        ...
        item = wx.MenuItem(sousMenuVentilation, 201, _("Uniquement la ligne sélectionnée"))
        ...
        item = wx.MenuItem(sousMenuVentilation, 202, _("Uniquement les lignes cochées"))
        item.SetBitmap(wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Magique.png"), wx.BITMAP_TYPE_PNG))
        sousMenuVentilation.AppendItem(item)
        ...
        menuPop.AppendMenu( sousMenuVentilation, "Ventilation automatique")
    ...
    def VentilationAuto(self, event=None, action=None):
        ...
        if ID == 202 :
            ...
            if len(listeTracks) == 0 :
                dlg = wx.MessageDialog(self, _("Vous n'avez coché aucune ligne !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return
