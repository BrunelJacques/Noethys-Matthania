...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ColumnDefn(_("ID"), "left", 0, "IDcaisse", typeDonnee="entier"),
            ColumnDefn(_("Nom"), 'left', 230, "nom", typeDonnee="texte"),
            ColumnDefn(_("Régime social associé"), "left", 140, "nom_regime", typeDonnee="texte"),
            ]
        
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucune caisse"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des caisses"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune caisse dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        ...
        DB.Close()
        if nbreFamilles > 0 :
            dlg = wx.MessageDialog(self, _("Cette caisse a déjà été attribuée à %d famille(s).\n\nVous ne pouvez donc pas la supprimer !") % nbreFamilles, _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        DB.Close()
        if nbreAides > 0 :
            dlg = wx.MessageDialog(self, _("Cette caisse a déjà été attribuée à %d aide(s).\n\nVous ne pouvez donc pas la supprimer !") % nbreAides, _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        
        # Confirmation de suppression
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cette caisse ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
...
class Saisie(wx.Dialog):
    def __init__(self, parent, nom=None, IDregime=None):
        ...
        self.label_nom = wx.StaticText(self, -1, _("Nom de la caisse :"))
        self.ctrl_nom = wx.TextCtrl(self, -1, "")
        if nom != None :
            self.ctrl_nom.SetValue(nom)
            
        self.label_regime = wx.StaticText(self, -1, _("Régime social associé :"))
        ...
        if nom == None :
            self.SetTitle(_("Saisie d'une caisse"))
        else:
            self.SetTitle(_("Modification d'une caisse"))
    ...
    def OnBoutonOk(self, event):
        ...
        if nom == "" :
            dlg = wx.MessageDialog(self, _("Vous n'avez saisi aucun nom de caisse !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
...
class BarreRecherche(wx.SearchCtrl):
    def __init__(self, parent):
        ...
        self.SetDescriptiveText(_("Rechercher une caisse..."))
