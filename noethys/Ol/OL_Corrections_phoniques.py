...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ColumnDefn(_("ID"), "left", 0, "IDcorrection"),
            ColumnDefn(_("Mot"), "left", 200, "mot"),
            ColumnDefn(_("Correction"), 'left', 300, "correction", isSpaceFilling=True),
            ]
        
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucune correction"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des corrections phonétiques"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Modifier(self, event=None):
        if UTILS_Utilisateurs.VerificationDroitsUtilisateurActuel("parametrage_vocal", "modifier") == False : return
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune correction à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event=None):
        if UTILS_Utilisateurs.VerificationDroitsUtilisateurActuel("parametrage_vocal", "modifier") == False : return
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune correction à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDcorrection = self.Selection()[0].IDcorrection
        ...
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cette correction ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
...
class Saisie(wx.Dialog):
    def __init__(self, parent, mot="", correction=""):
        ...
        self.label_mot = wx.StaticText(self, -1, _("Mot :"))
        self.ctrl_mot = wx.TextCtrl(self, -1, mot)
            
        self.label_correction = wx.StaticText(self, -1, _("Correction :"))
        ...
        if mot == "" :
            self.SetTitle(_("Saisie d'une correction"))
        else:
            self.SetTitle(_("Modification d'une correction"))
        self.SetMinSize((500, -1))
        
        self.ctrl_mot.SetToolTip(wx.ToolTip(_("Saisissez ici le mot à corriger")))
        self.ctrl_correction.SetToolTip(wx.ToolTip(_("Saisissez ici la correction du mot")))
    ...
    def OnBoutonOk(self, event):
        ...
        if mot == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un mot !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_mot.SetFocus()
            return
        if correction == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une correction !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_mot.SetFocus()
            return
