...
class ListView(FastObjectListView):
    ...
    def OnItemActivated(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune ligne dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        # Demande le numéro
        dlg = wx.TextEntryDialog(self, _("Veuillez saisir un numéro pour la cotisation à créer :"), _("Numéro de cotisation"))
    ...
    def InitObjectListView(self):
        ...
        if self.categorie == "individu" :
            ...
            liste_Colonnes = [
                ...
                ColumnDefn(_("Prénom"), "left", 100, "prenom", typeDonnee="texte"),
                ...
                ColumnDefn(_("Famille"), 'left', 200, "nomTitulaires", typeDonnee="texte"),
                ColumnDefn(_("Rue"), "left", 150, "rue_resid", typeDonnee="texte"),
                ColumnDefn(_("C.P."), "left", 50, "cp_resid", typeDonnee="texte"),
                ColumnDefn(_("Ville"), "left", 120, "ville_resid", typeDonnee="texte"),
                ]
            ...
            liste_Colonnes = [
                ...
                ColumnDefn(_("Famille"), 'left', 200, "nomTitulaires", typeDonnee="texte"),
                ColumnDefn(_("Rue"), "left", 160, "rue", typeDonnee="texte"),
                ...
                ColumnDefn(_("Régime"), "left", 130, "regime", typeDonnee="texte"),
                ColumnDefn(_("Caisse"), "left", 130, "caisse", typeDonnee="texte"),
                ColumnDefn(_("Numéro Alloc."), "left", 120, "numAlloc", typeDonnee="texte"),
                ]        

        if self.afficher_colonne_numero == True :
            liste_Colonnes.insert(1, ColumnDefn(_("Numéro"), "left", 90, "numero", typeDonnee="texte"))
    ...
    def MAJlabel(self):
        ...
        label = "%ss" % self.categorie.capitalize()
        nbreCoches = len(self.GetCheckedObjects())
        if nbreCoches == 1 :
            label += _(" (1 sélection)")
        elif nbreCoches > 1 :
            label += _(" (%d sélections)") % nbreCoches
    ...
    def ExportTexte(self, event):
        ...
        UTILS_Export.ExportTexte(self, titre=_("Liste des %ss") % self.categorie)
        
    def ExportExcel(self, event):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=_("Liste des %ss") % self.categorie)

    def Validation(self):
        if len(self.GetTracksCoches()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez cocher au moins une ligne dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        for track in self.GetTracksCoches() :
            if self.afficher_colonne_numero == True and track.numero in (None, ""):
                if self.categorie == "individu" :
                    label = "%s, %s" % (track.nom, track.prenom)
                else :
                    label = track.nomTitulaires
                dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un numéro de cotisation pour %s") % label, _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        return True
