...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Nom"), "left", 460, "label", typeDonnee="texte"),
            ]
        
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucun profil"))
    ...
    def OnContextMenu(self, event):
        ...
        # Item Par défaut
        item = wx.MenuItem(menuPop, 70, _("Définir comme profil par défaut"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Listes des profils de configuration"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Ajouter(self, event):
        dlg = wx.TextEntryDialog(self, _("Saisissez le nom du nouveau profil de configuration :"), _("Saisie d'un profil de configuration"), "")
    ...
    def Modifier(self, event):
        ...
        dlg = wx.TextEntryDialog(self, _("Modifiez le profil de configuration :"), _("Modification d'un profil"), label)
        if dlg.ShowModal() == wx.ID_OK:
            label = dlg.GetValue()
            dlg.Destroy()
        else :
    ...
    def Dupliquer(self, event):
        """ Dupliquer un modèle """
        if len(self.Selection()) == 0:
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun profil à dupliquer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDprofil = self.Selection()[0].IDprofil
        ...
        dictModifications = {"label": _("Copie de %s") % label, "defaut" : 0}
    ...
    def SetDefaut(self, event=None):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun profil dans la liste !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDprofil = self.Selection()[0].IDprofil
