...
class Track(object):
    def __init__(self, donnees, dictTitulaires):
        ...
        if self.iban == None or self.iban == "" : informationsManquantes.append(_("IBAN"))
        if self.bic == None or self.bic == "" : informationsManquantes.append(_("BIC"))
        if self.individu_nom_complet == None or self.individu_nom_complet == "" : informationsManquantes.append(_("nom du titulaire"))
        if self.rum == None or self.rum == "" : informationsManquantes.append(_("RUM"))
        if self.date == None or self.date == "" : informationsManquantes.append(_("date du mandat"))
        
        if self.IDmandat != None :
            self.analyse = _("Mandat déjà créé")
            self.valide = False
        elif len(informationsManquantes) > 0 :
            self.analyse = _("Infos manquantes : %s") % ", ".join(informationsManquantes)
            self.valide = False
        else :
            self.analyse = _("Prêt pour transfert")
            self.valide = True
...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ColumnDefn(_("ID"), "left", 0, "IDfamille"),
            ColumnDefn(_("Analyse"), 'left', 250, "analyse", imageGetter=GetImageAnalyse),
            ColumnDefn(_("Famille"), 'left', 200, "titulairesFamille"),
            ColumnDefn(_("IBAN"), 'left', 180, "iban"),
            ColumnDefn(_("BIC"), 'left', 100, "bic"),
            ColumnDefn(_("RUM"), 'left', 60, "rum"),
            ColumnDefn(_("Date sign."), 'left', 75, "date", stringConverter=FormateDateCourt),
            ColumnDefn(_("Titulaire"), 'left', 110, "individu_nom_complet"),
            ColumnDefn(_("Observations"), 'left', 120, "memo"),
            ]
    ...
    def OnContextMenu(self, event):
        ...
        item = wx.MenuItem(menuPop, 60, _("Cocher uniquement les transferts valides"))
    ...
    def Impression(self, mode="preview"):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des mandats"), format="A", orientation=wx.LANDSCAPE)
        if mode == "preview" :
            prt.Preview()
        else:
            prt.Print()
    ...
    def ExportTexte(self, event):
        ...
        UTILS_Export.ExportTexte(self, titre=_("Liste des RIB"))
        
    def ExportExcel(self, event):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=_("Liste des RIB"))
    ...
    def Conversion(self):
        ...
        for track in tracks :
            if track.valide == False :
                dlg = wx.MessageDialog(self, _("Vous ne pouvez sélectionner que les lignes valides !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        
        # Demande de confirmation
        dlg = wx.MessageDialog(self, _("Confirmez-vous la création de %d mandats ?\n\nCette opération peut prendre quelques minutes...") % len(tracks), _("Confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        ...
        index = 0
        for track in tracks :
            self.EcritStatusbar(_("Veuillez patienter...  Création du mandat %d / %d") % (index, len(tracks)))
            listeDonnees = [    
        ...
        dlg = wx.MessageDialog(self, _("%d mandats ont été créés avec succès.") % len(tracks), _("Fin"), wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
        return True
