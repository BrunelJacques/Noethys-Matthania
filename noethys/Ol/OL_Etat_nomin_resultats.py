...
##            dictDeductions[IDprestation].append({"IDdeduction":IDdeduction, "IDprestation":IDprestation, "date":date, "montant":montant, "label":label, "IDaide":IDaide})
        ...
        for IDfamille, IDcaisse, nomCaisse, num_allocataire, allocataire in listeFamilles :
            ...
            if allocataire != None and allocataire in DICT_INDIVIDUS :
                nomAllocataire = "%s %s" % (DICT_INDIVIDUS[allocataire]["nom"], DICT_INDIVIDUS[allocataire]["prenom"])
            else :
                nomAllocataire = None
...
##                deductions = []
        ...
        if len(listeFamillesSansQF) > 0 :
            dlg = wx.MessageDialog(self, _("Attention, veuillez noter que %d familles ne disposant de quotient familial sur la période sélectionnée ont été exclus des résultats !") % len(listeFamillesSansQF), _("Avertissement"), wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
    ...
    def AfficheTotaux(self):
        ...
        try :
            texte = _("---- TOTAUX pour les %d individus de la liste ----\n%s") % (len(listeTracks), ", ".join(listeTextes))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Etat nominatif des consommations"), total=txtTotal, format="A", orientation=wx.PORTRAIT)
        prt.Preview()
