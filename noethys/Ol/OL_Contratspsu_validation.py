...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("H. prévues"), 'center', 80, "heures_prevues_mois", typeDonnee="duree", stringConverter=FormateDuree, isEditable=False),
            ColumnDefn(_("Présences"), 'center', 80, "heures_presences", typeDonnee="duree", stringConverter=FormateDuree, isEditable=False),
            ColumnDefn(_("Abs déduc."), 'center', 80, "heures_absences_deductibles", typeDonnee="duree", stringConverter=FormateDuree, isEditable=False),
            ColumnDefn(_("Abs non déduc."), 'center', 95, "heures_absences_non_deductibles", typeDonnee="duree", stringConverter=FormateDuree, isEditable=False),
            ...
            ColumnDefn(_("H. régular."), 'center', 80, "heures_regularisation", typeDonnee="duree", stringConverter=FormateDuree, isEditable=True),
            ...
            ColumnDefn(_("H. facturées"), 'center', 80, "heures_facturees", typeDonnee="duree", stringConverter=FormateDuree, isEditable=False),
        ...
        self.SetEmptyListMsg(_("Aucune mensualité"))
    ...
    def Apercu(self, event):
        if self.IDactivite == None :
            dlg = wx.MessageDialog(self, _("Vous devez commencer par sélectionner une activité !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        from Utils import UTILS_Printer
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des mensualités"), intro=self.GetIntro(), format="A", orientation=wx.LANDSCAPE)
        prt.Preview()
    ...
    def ExportTexte(self, event):
        ...
        UTILS_Export.ExportTexte(self, titre=_("Liste des mensualités"), autoriseSelections=False)
    ...
    def ExportExcel(self, event):
        if self.IDactivite == None :
            ...
            return False
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=_("Liste des mensualités"), autoriseSelections=False)

    def GetIntro(self):
        return "Mensualités de %s %d de l'activité %s" % (LISTE_MOIS[self.mois-1], self.annee, self.nomActivite)

    def Supprimer(self, event=None):
        if len(self.Selection()) == 0 and len(self.GetCheckedObjects()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune aucune ligne dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if len(self.GetCheckedObjects()) > 0 :
            ...
            dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer les prestations des mensualités cochées ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
            ...
            dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer la prestation de la mensualité sélectionnée ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
            reponse = dlg.ShowModal()
            dlg.Destroy()
            if reponse != wx.ID_YES :
                return
        ...
        for track in listeSelections :
            if track.IDfacture != None :
                dlg = wx.MessageDialog(self, _("Vous ne pouvez pas supprimer la prestation de %s car elle apparaît déjà sur une facture !") % track.individu_nom_complet, _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return
        ...
        # Confirmation succès
        dlg = wx.MessageDialog(self, _("%d prestations ont été supprimées avec succès !") % len(listeSuppressions), _("Suppression terminée"), wx.OK | wx.ICON_EXCLAMATION)
        dlg.ShowModal()
        dlg.Destroy()

    def Detail(self, event=None):
        if len(self.Selection()) == 0 :
           dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune mensualité dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
           dlg.ShowModal()
           dlg.Destroy()
           return False
    ...
    def Valider(self):
        ...
        if len(listeTracks) == 0 :
           dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune mensualité à générer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
           dlg.ShowModal()
           dlg.Destroy()
           return False
        ...
        for track in listeTracks :
            if track.IDprestation != None :
                dlg = wx.MessageDialog(self, _("Vous ne pouvez pas sélectionner des lignes dont les mensualités ont déjà été générées !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False

        # Demande de confirmation
        dlg = wx.MessageDialog(self, _("Confirmez-vous la validation des %d mensualités sélectionnées sur le mois de %s %d ?" % (len(listeTracks), LISTE_MOIS[self.mois-1], self.annee)), _("Confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        reponse = dlg.ShowModal()
        dlg.Destroy()
        if reponse != wx.ID_YES :
            return False
        ...
        # Confirmation succès
        dlg = wx.MessageDialog(self, _("Les mensualités ont été générées avec succès !"), _("Génération terminée"), wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
        return True
