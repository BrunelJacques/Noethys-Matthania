...
class ListView(FastObjectListView):
    def __init__(self, *args, **kwds):
        ...
        self.categorieSingulier = kwds.pop("categorieSingulier", "") # "arrêt de bus"
        self.categoriePluriel = kwds.pop("categoriePluriel", "") # "arrêts de bus"
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ColumnDefn(_("ID"), "left", 0, "IDarret", typeDonnee="entier"),
            ColumnDefn(_("Ordre"), "center", 0, "ordre", typeDonnee="entier"),
            ColumnDefn(_("Nom"), 'left', 430, "nom", typeDonnee="texte", isSpaceFilling=True),
            ]
        
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucun %s") % self.categorieSingulier)
    ...
    def OnContextMenu(self, event):
        ...
        item = wx.MenuItem(menuPop, 80, _("Aperçu avant impression"))
        ...
        # Item Imprimer
        item = wx.MenuItem(menuPop, 90, _("Imprimer"))
    ...
    def Ajouter(self, event):
        ...
        dlg = wx.TextEntryDialog(self, _("Saisissez le nom du nouvel %s :") % self.categorieSingulier, _("Saisie d'un nouvel %s") % self.categorieSingulier, "")
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun %s à modifier !") % self.categorieSingulier, _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDarret = self.Selection()[0].IDarret
        nom = self.Selection()[0].nom
        dlg = wx.TextEntryDialog(self, _("Modifiez le nom de l'%s :") % self.categorieSingulier, _("Modification d'un %s") % self.categorieSingulier, nom)
        if dlg.ShowModal() == wx.ID_OK:
            nom = dlg.GetValue()
            if nom == "":
                dlg = wx.MessageDialog(self, _("Le nom que vous avez saisi n'est pas valide."), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun %s à supprimer !") % self.categorieSingulier, _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cet %s ?") % self.categorieSingulier, _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
    ...
    def Monter(self, event):
        if UTILS_Utilisateurs.VerificationDroitsUtilisateurActuel("parametrage_arrets", "modifier") == False : return
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun arrêt dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDarret = self.Selection()[0].IDarret
...
class BarreRecherche(wx.SearchCtrl):
    def __init__(self, parent):
        ...
        self.SetDescriptiveText(_("Rechercher un arrêt..."))
...
class MyFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        self.myOlv = ListView(panel, id=-1, categorie="bus", IDligne=8, categorieSingulier=_("arrêt de bus"), categoriePluriel=_("arrêts de bus"), name="OL_test", style=wx.LC_REPORT|wx.SUNKEN_BORDER|wx.LC_SINGLE_SEL|wx.LC_HRULES|wx.LC_VRULES)
        self.myOlv.MAJ() 
