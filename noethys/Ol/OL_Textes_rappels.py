...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        def FormateJours(nbreJours):
            texte = _("%d jours") % nbreJours
            return texte

        liste_Colonnes = [
            ColumnDefn(_("ID"), "left", 0, "IDtexte", typeDonnee="entier"),
            ColumnDefn(_("Intitulé"), 'left', 200, "label", typeDonnee="texte", imageGetter=GetImage),
            ColumnDefn(_("Titre"), 'left', 180, "titre", typeDonnee="texte"),
            ColumnDefn(_("Retard Min."), 'centre', 110, "retard_min", typeDonnee="entier", stringConverter=FormateJours),
            ColumnDefn(_("Retard Max."), 'centre', 110, "retard_max", typeDonnee="entier", stringConverter=FormateJours),
            ]
        
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucun texte de rappel"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des textes de rappel"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Modifier(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun texte à modifier dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun texte à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer ce texte ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
