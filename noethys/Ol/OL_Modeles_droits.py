...
class Track(object):
    def __init__(self, donnees, dictDroits):
        ...
        listeTemp = []
        if nbreAutorisations > 0 : listeTemp.append(_("%d autorisations") % nbreAutorisations)
        if nbreInterdictions > 0 : listeTemp.append(_("%d interdictions") % nbreInterdictions)
        if nbreRestrictions > 0 : listeTemp.append(_("%d restrictions") % nbreRestrictions)
        
        if len(listeTemp) > 0 :
            self.details = " - ".join(listeTemp)
        else :
            self.details = _("Aucune information")
...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Détails"), 'left', 250, "details", typeDonnee="texte"),
            ColumnDefn(_("Observations"), 'left', 200, "observations", typeDonnee="texte", isSpaceFilling=True),
            ]
        
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucun modèle de droits"))
    ...
    def OnContextMenu(self, event):
        ...
        # Item Dupliquer
        item = wx.MenuItem(menuPop, 70, _("Dupliquer"))
        ...
        # Item Par défaut
        item = wx.MenuItem(menuPop, 60, _("Définir comme modèle par défaut"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des modèle de droits"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()

    def Imprimer(self, event):
        from Utils import UTILS_Printer
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des modèles de droits"), format="A", orientation=wx.PORTRAIT)
        prt.Print()
    ...
    def Supprimer(self, event):
        ...
        if len(listeDonnees) > 0 :
            dlg = wx.MessageDialog(self, _("Vous ne pouvez pas supprimer ce modèle de droits.\n\nIl a déjà été attribué à %d utilisateur(s) !") % len(listeDonnees), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return

        # Demande confirmation suppression
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer ce modèle ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
    ...
    def Dupliquer(self, event):
        ...
        dlg = wx.MessageDialog(None, _("Confirmez-vous la duplication du modèle '%s' ?") % nom, _("Duplication"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
    ...
    def SetDefaut(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun modèle dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        if UTILS_Utilisateurs.VerificationDroitsUtilisateurActuel("parametrage_modeles_droits", "modifier") == False : return
