...
class Track(object):
    def __init__(self, donnees):
        ...
        if self.nbre == None or self.nbre == 0 :
            self.detail = _("Aucune cotisation")
        else:
            self.detail = _("%d cotisations") % self.nbre
...
##                    texteDetail = "%d %s (%.2f €), " % (dictDetail["nbre"], dictDetail["nom"], dictDetail["montant"])
##                    texte += texteDetail
...
##                DICT_DETAILS_DEPOTS[IDdepot]["montant"] += montant
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ColumnDefn(_("ID"), "left", 42, "IDdepot_cotisation", typeDonnee="entier", imageGetter=GetImageVerrouillage),
            ColumnDefn(_("Date"), 'left', 160, "date", typeDonnee="date", stringConverter=FormateDateLong),
            ColumnDefn(_("Nom"), 'left', 250, "nom", typeDonnee="texte"),
            ColumnDefn(_("Observations"), 'left', 250, "observations", typeDonnee="texte"),
            ColumnDefn(_("Nbre cotisations"), 'centre', 100, "nbre", typeDonnee="entier"),
##            ColumnDefn(_("Total"), 'right', 65, "total", stringConverter=FormateMontant),
##            ColumnDefn(_("Détail"), 'left', 210, "detail"),
            ]
        
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucun dépôt de cotisations"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des dépôts de cotisations"), format="A", orientation=wx.LANDSCAPE)
        prt.Preview()
    ...
    def ExportTexte(self, event):
        ...
        UTILS_Export.ExportTexte(self, titre=_("Liste des dépôts de cotisations"))
        
    def ExportExcel(self, event):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=_("Liste des dépôts de cotisations"))

    def Ajouter(self, event):
        if UTILS_Utilisateurs.VerificationDroitsUtilisateurActuel("cotisations_depots", "creer") == False : return
        from Dlg import DLG_Saisie_depot_cotisation
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun dépôt de cotisations à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun dépôt de cotisations à supprimer dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDdepot_cotisation = self.Selection()[0].IDdepot_cotisation
        ...
        if nbre_cotisations > 0 :
            dlg = wx.MessageDialog(self, _("Des cotisations sont déjà associées à ce dépôt. Vous ne pouvez donc pas le supprimer !"), _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer ce dépôt de cotisations ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
...
class BarreRecherche(wx.SearchCtrl):
    def __init__(self, parent):
        ...
        self.SetDescriptiveText(_("Rechercher un dépôt de cotisations..."))
