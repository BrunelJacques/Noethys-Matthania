...
import GestionDB
from Dlg import DLG_Saisie_releve_compta
...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Date début"), "left", 100, "date_debut", typeDonnee="date", stringConverter=FormateDate),
            ColumnDefn(_("Date fin"), "left", 100, "date_fin", typeDonnee="date", stringConverter=FormateDate),
            ColumnDefn(_("Compte bancaire"), "left", 200, "nomCompteBancaire", typeDonnee="texte"),
            ColumnDefn(_("Nbre opérations"), "center", 100, "nbreOperations", typeDonnee="entier"),
            ]

        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucun relevé bancaire"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des relevés bancaires"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun relevé bancaire à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun relevé bancaire à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        DB.Close()
        if nbreOperations > 0 :
            dlg = wx.MessageDialog(self, _("Ce relevé a déjà été associé à %d opération(s).\n\nVous ne pouvez donc pas le supprimer !") % nbreOperations, _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return

        # Suppression
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer ce relevé bancaire ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
