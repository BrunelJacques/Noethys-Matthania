...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ColumnDefn(_("ID"), "left", 0, "IDcontact", typeDonnee="entier"),
            ColumnDefn(_("Nom"), 'left', 160, "nom", typeDonnee="texte"),
            ColumnDefn(_("Prénom"), 'left', 160, "prenom", typeDonnee="texte"),
            ColumnDefn(_("Rue"), "left", 140, "rue_resid", typeDonnee="texte"),
            ColumnDefn(_("C.P."), "left", 45, "cp_resid", typeDonnee="texte"),
            ColumnDefn(_("Ville"), "left", 110, "ville_resid", typeDonnee="texte"),
            ColumnDefn(_("Fixe"), "left", 100, "tel_domicile", typeDonnee="texte"),
            ColumnDefn(_("Portable"), "left", 100, "tel_mobile", typeDonnee="texte"),
            ColumnDefn(_("Email"), "left", 200, "mail", typeDonnee="texte"),
            ]
        
        self.SetColumns(liste_Colonnes)
        if self.mode_coche:
            self.CreateCheckStateColumn(0)
        self.SetEmptyListMsg(_("Aucun contact"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des contacts"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun contact à modifier dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun contact dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDcontact = self.Selection()[0].IDcontact
        ...
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer ce contact ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
