...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Donnée associée"), "left", 200, "nomDonnee", typeDonnee="texte"),
            ColumnDefn(_("Observations"), "left", 120, "observations", typeDonnee="texte", isSpaceFilling=True),
            ]
        
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucune image"))
    ...
    def OnContextMenu(self, event):
        ...
        # Item Visualiser
        item = wx.MenuItem(menuPop, 32, _("Visualiser"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des images interactives"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune image à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune image à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if self.Selection()[0].supprimable == 0 :
            dlg = wx.MessageDialog(self, _("Cette image ne peut pas être supprimé !"), _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        # Suppression
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cette image ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
    ...
    def Dupliquer(self, event):
        ...
        if UTILS_Utilisateurs.VerificationDroitsUtilisateurActuel("parametrage_images_interactives", "creer") == False : return
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune image à dupliquer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDmodele = self.Selection()[0].IDmodele
    ...
    def Importer(self, event):
        ...
        wildcard = "Image interactive Noethys (*.ndi)|*.ndi|Tous les fichiers (*.*)|*.*"
        ...
        message = _("Confirmez-vous l'importation de l'image suivante ?\n\nNom : %s\nCatégorie : %s\n") % (nom , labelCategorie)
        dlg = wx.MessageDialog(self, message, _("Importer une image interactive"), wx.YES_NO|wx.YES_DEFAULT|wx.ICON_QUESTION)
        ...
        # Confirmation
        dlg = wx.MessageDialog(self, _("L'image interactive a été importée avec succès dans la catégorie '%s' !") % labelCategorie, _("Importation"), wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
    ...
    def Exporter(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune image dans la liste !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        track = self.Selection()[0]
        ...
        dlg = wx.FileDialog(self, message=_("Enregistrer l'image interactive sous..."),
        ...
        # Confirmation
        dlg = wx.MessageDialog(self, _("L'image interactive a été exportée avec succès !"), _("Exportation"), wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
        
    def Visualiser(self, event=None):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune image à visualiser dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        track = self.Selection()[0]
