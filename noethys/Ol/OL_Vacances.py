...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ColumnDefn(_("ID"), "left", 0, "IDvacance", typeDonnee="entier"),
            ColumnDefn(_("Année"), 'left', 50, "annee", typeDonnee="texte"),
            ColumnDefn(_("Nom"), "left", 120, "nom", typeDonnee="texte"),
            ColumnDefn(_("Date de début"), "left", 190, "date_debut", typeDonnee="date", stringConverter=FormateDate),
            ColumnDefn(_("Date de fin"), "left", 190, "date_fin", typeDonnee="date", stringConverter=FormateDate),
            ]
    ...
    def OnContextMenu(self, event):
        ...
        # Item Supprimer
        item = wx.MenuItem(menuPop, 70, _("Importer depuis Internet"))
        bmp = wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Updater.png"), wx.BITMAP_TYPE_PNG)
        item.SetBitmap(bmp)
        menuPop.AppendItem(item)
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des vacances"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune période de vacances à modifier dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune période de vacances à supprimer dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cette période de vacances ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
...
class Saisie(wx.Dialog):
    def __init__(self, parent, IDvacance=None):
        ...
        # Propriétés
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Choisissez ici le nom de la période")))
        self.ctrl_annee.SetToolTip(wx.ToolTip(_("Saisissez ici l'année de la période. Ex. : '2011'")))
        self.ctrl_dateDebut.SetToolTip(wx.ToolTip(_("Saisissez ici la date de début de la période")))
        self.ctrl_dateFin.SetToolTip(wx.ToolTip(_("Saisissez ici la date de fin de la période")))
        ...
        if IDvacance == None :
            self.SetTitle(_("Saisie d'une période de vacances"))
        else:
            self.SetTitle(_("Modification d'une période de vacances"))
    ...
    def OnBoutonOk(self, event):
        if self.ctrl_nom.GetSelection == -1 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner un nom de période !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if date_debut > date_fin :
            dlg = wx.MessageDialog(self, _("La date de fin de vacances doit être supérieure à la date de début !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
...
class BarreRecherche(wx.SearchCtrl):
    def __init__(self, parent):
        ...
        self.SetDescriptiveText(_("Rechercher une période de vacances..."))
