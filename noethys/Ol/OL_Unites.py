...
class Track(object):
    def __init__(self, donnees):
        ...
        if self.type == "Unitaire" : self.type = _("Standard")
        if self.type == "Quantite" : self.type = _("Quantité")
        if self.type == "Evenement": self.type = _("Evènementiel")
        ...
        if self.autogen_active == 1 :
            self.autogen_active_str = _("Oui")
        else :
            self.autogen_active_str = ""

        if self.date_debut == "1977-01-01" and self.date_fin == "2999-01-01" :
            self.periode_validite = _("Illimitée")
        else:
            self.periode_validite = _("Du %s au %s") % (DateEngFr(self.date_debut), DateEngFr(self.date_fin))
...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ColumnDefn(_("ID"), "left", 0, "IDunite", typeDonnee="entier"),
            ColumnDefn(_("Ordre"), "left", 70, "ordre", typeDonnee="entier"),
            ColumnDefn(_("Nom"), 'left', 190, "nom", typeDonnee="texte"),
            ColumnDefn(_("Abrégé"), "left", 80, "abrege", typeDonnee="texte"),
            ColumnDefn(_("Type"), "left", 120, "type", typeDonnee="texte"),
            ColumnDefn(_("Période de validité"), "left", 210, "periode_validite", typeDonnee="texte"),
            ColumnDefn(_("Auto-gen."), "left", 70, "autogen_active_str", typeDonnee="texte"),
            ]
        
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucune unité de consommation"))
    ...
    def OnContextMenu(self, event):
        ...
        item = wx.MenuItem(menuPop, 40, _("Déplacer vers le haut"))
        ...
        item = wx.MenuItem(menuPop, 50, _("Déplacer vers le bas"))
        ...
        item = wx.MenuItem(menuPop, 60, _("Aperçu avant impression"))
        ...
        # Item Imprimer
        item = wx.MenuItem(menuPop, 70, _("Imprimer"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des unités"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Ajouter(self, event):
        from Dlg import DLG_Saisie_unite
    ...
    def Modifier(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune unité à modifier dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune unité à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDunite = self.Selection()[0].IDunite
        ...
        DB.Close()
        if nbreUnites > 0 :
            dlg = wx.MessageDialog(self, _("Cette unité de consommation a déjà été attribuée à %d unité(s) de remplissage.\n\nVous ne pouvez donc pas la supprimer !") % nbreUnites, _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        DB.Close()
        if nbreOuvertures > 0 :
            dlg = wx.MessageDialog(self, _("Cette unité de consommation a déjà été attribuée à %d ouverture(s).\n\nVous ne pouvez donc pas la supprimer !") % nbreOuvertures, _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if nbreCombiTarifs > 0 :
            dlg = wx.MessageDialog(self, _("Cette unité de consommation a déjà été attribuée à une ou plusieurs combinaisons de tarifs.\n\nVous ne pouvez donc pas la supprimer !"), _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        DB.Close()
        if nbreCombiAides > 0 :
            dlg = wx.MessageDialog(self, _("Cette unité de consommation a déjà été attribuée à une ou plusieurs combinaisons d'aides journalières.\n\nVous ne pouvez donc pas la supprimer !"), _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return

        # Confirmation de suppression
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cette unité ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
    ...
    def Monter(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune unité dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDunite = self.Selection()[0].IDunite
...
class BarreRecherche(wx.SearchCtrl):
    def __init__(self, parent):
        ...
        self.SetDescriptiveText(_("Rechercher une unité"))
