...
class Track(object):
    def __init__(self, label, dictValeurs):
        ...
        self.total_str = "%.2f %s (%d)" % (self.montant_total, SYMBOLE, self.nombre_total)
        if self.nombre_total == 0 : self.total_str = ""
        self.regle_str = "%.2f %s (%d)" % (self.montant_regle, SYMBOLE, self.nombre_regle)
        if self.nombre_regle == 0 : self.regle_str = ""
        self.impaye_str = "%.2f %s (%d)" % (self.montant_impaye, SYMBOLE, self.nombre_impaye)
        if self.nombre_impaye == 0 : self.impaye_str = ""
...
##        ;""" % (conditionActivites, self.date_debut, self.date_fin, conditionDateNaiss)
    ...
    def InitObjectListView(self):
        ...
        liste_Colonnes = [
            ColumnDefn(_("Prestation"), 'left', 190, "label", typeDonnee="texte", isEditable=False),
            ColumnDefn(_("Ajustement"), "center", 80, "ajustement", typeDonnee="texte", isEditable=True), 
            ColumnDefn(_("Activité"), "left", 90, "nomActivite", typeDonnee="texte", isEditable=False), 
            ColumnDefn(_("Total"), "left", 110, "total_str", typeDonnee="texte", isEditable=False),
            ColumnDefn(_("Réglé"), "left", 110, "regle_str", typeDonnee="texte", isEditable=False),
            ColumnDefn(_("Impayé"), "left", 110, "impaye_str", typeDonnee="texte", isEditable=False),
            ]
