...
class DLG_Supprimer_occurences(wx.Dialog):
    def __init__(self, parent, nbre_occurences=0):
        ...
        # Intro
        self.label_intro = wx.StaticText(self, -1, _("Cette location fait partie d'une série de %d occurences.\n\nQue souhaitez-vous supprimer ?") % nbre_occurences)

        # Options
        self.radio_selection = wx.RadioButton(self, -1, _("Uniquement l'occurence sélectionnée"), style=wx.RB_GROUP)
        self.radio_toutes = wx.RadioButton(self, -1, _("Toutes les occurences de la série"))
        self.radio_periode = wx.RadioButton(self, -1, _("Les occurences de la série du"))
    ...
    def __set_properties(self):
        self.SetTitle(_("Supprimer des locations"))
...
def Supprimer_location(parent, IDlocation=None):
    ...
    def GetLocations(condition=""):
        ...
        for IDlocation, IDfamille, date_debut, date_fin, serie, nom_produit, nom_categorie in listeDonnees:
            ...
            label_produit = "%s (%s)" % (nom_produit, nom_categorie)
    ...
    if len(liste_anomalies) > 0:
        ...
        if len(liste_valides) > 0:
            conclusion = _("Souhaitez-vous continuer avec les %d autres locations ?") % len(liste_valides)
            dlg = DLG_Messagebox.Dialog(None, titre=_("Anomalies"), introduction=introduction, detail=u"\n".join(liste_anomalies), conclusion=conclusion, icone=wx.ICON_EXCLAMATION, boutons=[_("Oui"), _("Non"), _("Annuler")])
    ...
    if len(liste_valides) == 1:
        dlg = wx.MessageDialog(parent, _("Souhaitez-vous vraiment supprimer la location ?"), _("Confirmation de suppression"), wx.YES_NO | wx.NO_DEFAULT | wx.CANCEL | wx.ICON_INFORMATION)
        reponse = dlg.ShowModal()
        dlg.Destroy()
    else:
        introduction = _("Confirmez-vous la suppression des %d locations suivantes :") % len(liste_valides)
        liste_locations_temp = [dict_location["periode"] for dict_location in liste_valides]
        dlg = DLG_Messagebox.Dialog(None, titre=_("Confirmation de suppression"), introduction=introduction, detail=u"\n".join(liste_locations_temp), icone=wx.ICON_EXCLAMATION, boutons=[_("Oui"), _("Non"), _("Annuler")])
        reponse = dlg.ShowModal()
        dlg.Destroy()
    ...
    for dict_location in liste_valides:
        ...
        # Historique
        texte_historique = _("Suppression de la location ID%d : %s %s") % (dict_location["IDlocation"], dict_location["label_produit"], dict_location["periode"])
...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        dict_colonnes = {
            ...
            "date_debut" : ColumnDefn(_("Début"), "left", 130, "date_debut", typeDonnee="date", stringConverter=FormateDate),
            "date_fin" : ColumnDefn(_("Fin"), "left", 130, "date_fin", typeDonnee="date", stringConverter=FormateDate),
            "nomProduit" : ColumnDefn(_("Nom du produit"), 'left', 200, "nomProduit", typeDonnee="texte"),
            "nomCategorie" : ColumnDefn(_("Catégorie du produit"), 'left', 200, "nomCategorie", typeDonnee="texte"),
            "quantite": ColumnDefn(u"Qté", "left", 60, "quantite", typeDonnee="entier"),
            "nomTitulaires" : ColumnDefn(_("Loueur"), 'left', 270, "nomTitulaires", typeDonnee="texte"),
            "rue" : ColumnDefn(_("Rue"), 'left', 200, "rue", typeDonnee="texte"),
            "cp" : ColumnDefn(_("C.P."), 'left', 70, "cp", typeDonnee="texte"),
            "ville" : ColumnDefn(_("Ville"), 'left', 150, "ville", typeDonnee="texte"),
        ...
        self.SetEmptyListMsg(_("Aucune location"))
    ...
    def OnContextMenu(self, event):
        ...
        # Item Imprimer
        item = wx.MenuItem(menuPop, 100, _("Imprimer la location"))
        ...
        item = wx.MenuItem(menuPop, 110, _("Envoyer la location par Email"))
        ...
        self.GenerationContextMenu(menuPop, titre=_("Liste des locations"))
    ...
    def Modifier(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune location à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune location à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        track = self.Selection()[0]
    ...
    def ImprimerPDF(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune location à imprimer !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def EnvoyerEmail(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune location à envoyer par Email !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        track = self.Selection()[0]
        ...
        UTILS_Envoi_email.EnvoiEmailFamilles(parent=self, IDfamille=track.IDfamille, nomDoc=FonctionsPerso.GenerationNomDoc("LOCATION", "pdf"), categorie="location")
...
class ListviewAvecFooter(PanelAvecFooter):
    def __init__(self, parent, kwargs={}):
        dictColonnes = {
            "date_debut": {"mode": "nombre", "singulier": _("location"), "pluriel": _("locations"), "alignement": wx.ALIGN_CENTER},
        }
