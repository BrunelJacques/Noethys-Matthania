...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ColumnDefn(_("ID"), "center", 50, "IDlegende", typeDonnee="entier", imageGetter=GetImage),
            ColumnDefn(_("Nom"), "left", 290, "nom", typeDonnee="texte", isSpaceFilling=True),
        ]
        
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucune légende"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des légendes de menus"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune légende à modifier dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune légende à supprimer dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDlegende = self.Selection()[0].IDlegende

        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cette légende ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
...
class DLG_Saisie(wx.Dialog):
    def __init__(self, parent, nom=None, couleur=None):
        ...
        if nom == None :
            self.SetTitle(_("Saisie d'une légende"))
        else:
            self.SetTitle(_("Modification d'une légende"))
    ...
    def OnBoutonOk(self, event):
        ...
        if nom == "" :
            dlg = wx.MessageDialog(self, _("Vous n'avez saisi aucun nom de légende !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
