...
class Track(object):
    def __init__(self, donnees):
        ...
        if self.IDfamille != None and self.nom != None :
            self.texte = _("Famille de %s : %s") % (self.nom, self.texte)
        
        if self.IDindividu != None and self.nom != None :
            self.texte = "%s : %s" % (self.nom, self.texte)
...
class ListView(FastObjectListView):
    ...
    def GetTracks(self):
        ...
        ;"""
        DB.ExecuterReq(req,MsgBox="OL_Liste_messages")
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Date"), 'centre', 75, "date_parution", typeDonnee="date", stringConverter=FormateDateCourt),
            ColumnDefn(_("Catégorie"), 'left', 160, "nomCategorie", typeDonnee="texte"),
            ColumnDefn(_("Texte"), 'left', 420, "texte", typeDonnee="texte", imageGetter=GetImagePriorite, isSpaceFilling=True),
            ]
        
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucun message"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des messages"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Supprimer(self, event):
        ...
        if dlg.ShowModal() == wx.ID_YES :
            ...
            if len(texte) > 450 : texte = texte[450:] + "..."
            UTILS_Historique.InsertActions([{
                "IDindividu" : IDindividu,
                "IDfamille" : IDfamille,
                "IDcategorie" : 26, 
                "action" : _("Suppression du message ID%d : '%s'") % (IDmessage, texte)
            },])
...
class BarreRecherche(wx.SearchCtrl):
    def __init__(self, parent):
        ...
        self.SetDescriptiveText(_("Rechercher un message..."))
