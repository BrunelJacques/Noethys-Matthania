...
class ListView(FastObjectListView):
        ...
        for nomFichier in listeFichiers :
            if nomFichier.startswith("actions_%s" % IDfichier) and (nomFichier.endswith(".dat") or (nomFichier.endswith(".archive") and self.inclure_archives == True)) :
                ...
                if len(listeConsommations) > 0 :
                    liste_details_actions.append(_("%d actions sur les consommations") % len(listeConsommations))
                ...
                if len(listeMemosJournees) > 0 :
                    liste_details_actions.append(_("%d actions sur les mémos journaliers") % len(listeMemosJournees))

                DB.Close() 
                
                detail_actions = ", ".join(liste_details_actions)
                if detail_actions == "" :
                    detail_actions = _("Aucune action")
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ColumnDefn(_("Date génération"), "left", 130, "horodatage", typeDonnee="dateheure", stringConverter=FormateHorodatage),
            ColumnDefn(_("Appareil (ID)"), "left", 130, "appareil", typeDonnee="texte"),
            ColumnDefn(_("Contenu du fichier"), "left", 285, "detail_actions", typeDonnee="texte"),
            ColumnDefn(_("Nom du fichier"), "left", 285, "nom_fichier", typeDonnee="texte"),
            ColumnDefn(_("Taille"), "left", 70, "taille_fichier", typeDonnee="entier", stringConverter=FormateTailleFichier),
            ]

        self.rowFormatter = rowFormatter
        self.SetColumns(liste_Colonnes)
        self.CreateCheckStateColumn(0)
        self.SetEmptyListMsg(_("Aucun fichier"))
    ...
    def OnContextMenu(self, event):
        ...
        # Item Archiver
        item = wx.MenuItem(menuPop, 10, _("Archiver le fichier"))
    ...
    def Archiver(self, event):
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment archiver ce fichier ?"), _("Archivage"), wx.YES_NO|wx.CANCEL|wx.NO_DEFAULT|wx.ICON_QUESTION)
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des fichiers à synchroniser"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def ExportTexte(self, event):
        ...
        UTILS_Export.ExportTexte(self, titre=_("Liste des fichiers à synchroniser"))
        
    def ExportExcel(self, event):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=_("Liste des fichiers à synchroniser"))
