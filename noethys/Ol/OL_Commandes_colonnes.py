...
class Track(object):
    def __init__(self, parent, index=None, donnees=None):
        ...
        if self.categorie in DLG_Saisie_commandes_colonne.DICT_LABELS_CATEGORIES:
            ...
            self.categorie_texte = _("Catégorie inconnue")
...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ColumnDefn(_("ID"), "left", 0, "IDcolonne", typeDonnee="texte"),
            ColumnDefn(_("Ordre"), "center", 70, "index", typeDonnee="entier", stringConverter=FormateIndex),
            ColumnDefn(_("Nom de la colonne"), "left", 230, "nom", typeDonnee="texte"),
            ColumnDefn(_("Type de colonne"), "left", 110, "categorie_texte", typeDonnee="texte", isSpaceFilling=True),
            ]
        
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucune colonne"))
    ...
    def Modifier(self, event):
        ...
        if track.nbre_valeurs > 0 :
            dlg = wx.MessageDialog(self, _("Attention, cette colonne est déjà associée à %d valeurs dans une ou plusieurs commandes !\n\nSouhaitez-vous tout de même la modifier ?") % track.nbre_valeurs, _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
    ...
    def Supprimer(self, event):
        ...
        if track.nbre_valeurs > 0 :
            dlg = wx.MessageDialog(self, _("Attention, cette colonne est déjà associée à %d valeurs dans une ou plusieurs commandes ! Si vous supprimez cette colonne, les valeurs associées seront également supprimées.\n\nSouhaitez-vous tout de même la supprimer ?") % track.nbre_valeurs, _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
            reponse = dlg.ShowModal()
            dlg.Destroy()
            if reponse != wx.ID_YES :
                return False
    ...
    def Monter(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune colonne dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        track = self.Selection()[0]
