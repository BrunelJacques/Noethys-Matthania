...
class ListView(FastObjectListView):
    ...
    def GetTraductionsExistantes(self):
        ...
        for extension, rep, dictTemp in [("lang", Chemins.GetStaticPath("Lang"), dictTraductionsInitiales), ("xlang", UTILS_Fichiers.GetRepLang(), dictTraductionsPerso)] :
            nomFichier = os.path.join(rep, "%s.%s" % (self.code, extension))
    ...
    def InitObjectListView(self):
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Texte original"), "left", 200, "texte", typeDonnee="texte", isSpaceFilling=True),
            ColumnDefn(_("Traduction"), "left", 200, "traduction", typeDonnee="texte", imageGetter=GetImageDirty, isSpaceFilling=True),
            ColumnDefn(_("Nbre Fichiers"), "left", 85, "nbreFichiers", typeDonnee="entier"),
            ColumnDefn(_("Fichiers associés"), "left", 200, "listeFichiersStr", typeDonnee="texte"),
            ]
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucun texte"))
    ...
    def Impression(self, mode="preview"):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des traductions"), intro="", total="", format="A", orientation=wx.LANDSCAPE)
        if mode == "preview" :
            prt.Preview()
        else:
            prt.Print()
    ...
    def ExportTexte(self, event):
        ...
        UTILS_Export.ExportTexte(self, titre=_("Liste des traductions"))
        
    def ExportExcel(self, event):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=_("Liste des traductions"))
