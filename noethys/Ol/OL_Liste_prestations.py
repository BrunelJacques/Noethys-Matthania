...
##        self.SetSortColumn(self.columns[indexColonne-1], resortNow=True)
    ...
    def Modifier(self, event):
        ...
        if track.categorie == "import" :
            dlg = wx.MessageDialog(self, _("Cette prestation est antérieure. Il est donc impossible de la modifier."), _("Modification impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return

        if track.IDfacture != None :
            dlg = wx.MessageDialog(self, _("Cette prestation apparaît déjà sur la facture %s. Il est donc impossible de la modifier.") , _("Modification impossible"), wx.OK | wx.ICON_EXCLAMATION)
