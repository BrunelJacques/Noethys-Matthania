...
class ListView(FastObjectListView):
    ...
    def GetTracks(self):
        ...
        self.ctrl_soldes.label_solde_pointe.SetLabel(_("Solde pointé : %.2f %s") % (soldePointe, SYMBOLE))
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Libellé"), 'left', 180, "libelle", typeDonnee="texte", isSpaceFilling=True),
            ColumnDefn(_("Tiers"), 'left', 130, "nomTiers", typeDonnee="texte"),
            ColumnDefn(_("Mode"), 'left', 90, "nomMode", typeDonnee="texte"),
            ColumnDefn(_("N° Chèque"), 'left', 80, "num_piece", typeDonnee="texte"),
##            ColumnDefn(_("Compte bancaire"), 'left', 120, "nomCompte"),
            ColumnDefn(_("Relevé"), 'left', 90, "nomReleve", typeDonnee="texte"),
            ColumnDefn(_("Débit"), "right", 80, "debit", typeDonnee="montant", stringConverter=FormateMontant),
            ColumnDefn(_("Crédit"), "right", 80, "credit", typeDonnee="montant", stringConverter=FormateMontant),
            ColumnDefn(_("Solde"), "right", 80, "solde", typeDonnee="montant", stringConverter=FormateMontant),
            ]

        self.rowFormatter = rowFormatter
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucune opération de trésorerie"))
    ...
    def OnContextMenu(self, event):
        ...
        # Item Ajouter
        item = wx.MenuItem(menuPop, 10, _("Ajouter une opération au débit"))
        ...
        item = wx.MenuItem(menuPop, 11, _("Ajouter une opération au crédit"))
        bmp = wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Addition.png"), wx.BITMAP_TYPE_PNG)
        item.SetBitmap(bmp)
        menuPop.AppendItem(item)
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des opérations"), format="A", orientation=wx.LANDSCAPE)
        prt.Preview()
    ...
    def ExportTexte(self, event):
        ...
        UTILS_Export.ExportTexte(self, titre=_("Liste des opérations"))
        
    def ExportExcel(self, event):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=_("Liste des opérations"))
...
##        if UTILS_Utilisateurs.VerificationDroitsUtilisateurActuel("parametrage_categories_comptables", "modifier") == False : return
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune opération à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
...
##        if UTILS_Utilisateurs.VerificationDroitsUtilisateurActuel("parametrage_categories_comptables", "supprimer") == False : return
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune opération à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        track = self.Selection()[0]
        ...
        if track.IDvirement == None and track.IDreleve != None :
            dlg = wx.MessageDialog(self, _("Vous ne pouvez pas supprimer une opération pointée sur un relevé bancaire !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if track.IDvirement != None :
            ...
            for IDoperation, IDreleve, nomReleve, nomCompte in listeDonnees :
                if IDreleve != None :
                    dlg = wx.MessageDialog(self, _("Vous ne pouvez pas supprimer ce virement car il a déjà été pointé sur le relevé '%s' du compte '%s' !") % (nomReleve, nomCompte), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                    dlg.ShowModal()
                    dlg.Destroy()
                    return
            
        # Suppression
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cette opération ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
...
##        dictColonnes = {
##            "nomTiers" : {"mode" : "nombre", "singulier" : _("opération"), "pluriel" : _("opérations"), "alignement" : wx.ALIGN_CENTER},
...
class BarreSoldes(wx.Panel):
    def __init__(self, parent, listview=None):
        ...
        self.label_solde_pointe = wx.StaticText(self, wx.ID_ANY, _("Solde pointé : 0.00 €"))
