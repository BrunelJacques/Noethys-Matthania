...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Nom"), "left", 230, "titre", typeDonnee="texte", isSpaceFilling=True),
            ]
    ...
    def Modifier(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun individu à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun individu à supprimer dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        track = self.Selection()[0]
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cet individu ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
...
class DLG_Saisie_element(wx.Dialog):
    def __init__(self, parent):
        ...
        self.label_nom = wx.StaticText(self, -1, _("Nom de l'individu :"))
        ...
        # Photo
        self.staticbox_photo_staticbox = wx.StaticBox(self, -1, _("Photo"))
    ...
    def __set_properties(self):
        self.SetTitle(_("Saisie d'un individu"))
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez ici le nom de l'individu")))
        self.ctrl_description.SetToolTip(wx.ToolTip(_("Saisissez ici la description de l'individu (exemple : fonction dans la structure)")))
    ...
    def OnBoutonOk(self, event):
        ...
        if len(self.ctrl_nom.GetValue()) == 0:
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un nom pour cet individu !"), "Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if len(self.ctrl_description.GetValue()) == 0:
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une description (Exemple : la fonction de l'individu dans la structure) !"), "Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
    ...
    def SetDonnees(self, dictDonnees={}):
        ...
        self.SetTitle(_("Modification d'un individu"))
