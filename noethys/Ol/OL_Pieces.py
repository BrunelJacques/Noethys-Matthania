...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        def FormateDate(dateStr):
            if dateStr == "" : return ""
            if dateStr == "2999-01-01" : return _("Illimitée")
        ...
        if self.IDindividu != None :
            ...
            if self.nbreFamilles < 2 :
                liste_Colonnes = [
                    ...
                    ColumnDefn(_("Nom de la pièce"), 'left', 190, "nom", typeDonnee="texte", imageGetter=GetImageDocument, isSpaceFilling=True),
                    ]
            else:
                liste_Colonnes = [
                    ColumnDefn(_("ID"), "left", 0, "IDpiece", typeDonnee="entier"),
                    ColumnDefn(u"Du", "left", 85, "date_debut", typeDonnee="date", stringConverter=FormateDate), 
                    ColumnDefn(_("au"), "left", 85, "date_fin", typeDonnee="date", stringConverter=FormateDate),
                    ColumnDefn(_("Nom de la pièce"), 'left', 165, "nom", typeDonnee="texte", imageGetter=GetImageDocument, isSpaceFilling=True),
            ...
            if self.nbreFamilles < 2 :
                liste_Colonnes = [
                    ...
                    ColumnDefn(u"Du", "left", 85, "date_debut", typeDonnee="date", stringConverter=FormateDate), 
                    ColumnDefn(_("au"), "left", 85, "date_fin", typeDonnee="date", stringConverter=FormateDate),
                    ColumnDefn(_("Nom de la pièce"), 'left', 340, "nom", typeDonnee="texte", imageGetter=GetImageDocument, isSpaceFilling=True),
                    ]
            else:
                liste_Colonnes = [
                    ColumnDefn(_("ID"), "left", 0, "IDpiece", typeDonnee="entier"),
    ...
    def ExportTexte(self, event):
        ...
        UTILS_Export.ExportTexte(self, titre=_("Liste des pièces"))
        
    def ExportExcel(self, event):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=_("Liste des pièces"))
        ...
        if self.dictFamillesRattachees != None :
            ...
            for IDfamilleTmp, dictFamille in self.dictFamillesRattachees.items() :
                if dictFamille["IDcategorie"] in (1, 2) :
                    valide = True
            if valide == False :
                dlg = wx.MessageDialog(self, _("Pour saisir une pièce, un individu doit obligatoirement être\nrattaché comme représentant ou enfant à une fiche famille !"), _("Saisie de pièce impossible"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune pièce à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDpiece = self.Selection()[0].IDpiece
        ...
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cette pièce ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
        if dlg.ShowModal() == wx.ID_YES :
            ...
            UTILS_Historique.InsertActions([{
                ...
                "IDfamille" : IDfamille,
                "IDcategorie" : 17, 
                "action" : _("Suppression de la pièce ID%d '%s'") % (IDpiece, nomPiece),
                },])
...
class BarreRecherche(wx.SearchCtrl):
    def __init__(self, parent):
        ...
        self.SetDescriptiveText(_("Rechercher une pièce..."))
