...
class DLG_Saisie(wx.Dialog):
    def __init__(self, parent, nom=None, cp=None, pays=None):
        ...
        self.label_nom = wx.StaticText(self, -1, _("Nom de la ville :"))
        self.ctrl_nom = wx.TextCtrl(self, -1, "", size=(280, -1))
        if nom != None :
            self.ctrl_nom.SetValue(nom)
            
        self.label_cp = wx.StaticText(self, -1, _("Code postal :"))
        self.ctrl_cp = wx.TextCtrl(self, -1, "", size=(80, -1))
        if cp !=None :
            self.ctrl_cp.SetValue(cp)

        self.label_pays = wx.StaticText(self, -1, _("Pays étranger :"))
        ...
        if nom == None :
            self.SetTitle(_("Saisie d'une ville"))
        else:
            self.SetTitle(_("Modification d'une ville"))
    ...
    def OnBoutonOk(self, event):
        ...
        if nom == "" :
            dlg = wx.MessageDialog(self, _("Vous n'avez saisi aucun nom de ville !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_nom.SetFocus()
            return
        if cp == "" :
            dlg = wx.MessageDialog(self, _("Vous n'avez saisi aucun code postal !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if pays:
            if len(pays)>0:
                testPays = UTILS_SaisieAdresse.GetOnePays(pays)
                pays = pays.upper()
                if testPays != pays:
                    dlg = wx.MessageDialog(self, _("Le pays '%s' n'est pas connu, veuillez le gérer !"%pays), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                    dlg.ShowModal()
                    dlg.Destroy()
                    return
...
class ListView(FastObjectListView):
    ...
    def GetTracks(self):
        ...
        DB.ExecuterReq(req,MsgBox="OL_Villes")
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ColumnDefn("", "left", 0, "IDville", typeDonnee="entier"),
            ColumnDefn(_("Nom de la ville"), 'left', 250, "nom", typeDonnee="texte"),
            ColumnDefn("Code postal", "left", 120, "cp", typeDonnee="texte"),
            ColumnDefn("Pays étranger", "left", 120, "pays", typeDonnee="texte"),
            ]
        
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucune ville"))
    ...
    def OnContextMenu(self, event):
        ...
        # Item Choisir
        item = wx.MenuItem(menuPop, 40, _("Choisir"))
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune ville à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune ville à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDville = self.Selection()[0].IDville
...
class BarreRecherche(wx.SearchCtrl):
    def __init__(self, parent):
        ...
        self.SetDescriptiveText(_("Rechercher une ville ou un code postal..."))
