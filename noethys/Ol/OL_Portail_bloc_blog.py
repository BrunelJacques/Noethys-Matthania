...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        self.SetEmptyListMsg(_("Aucun article"))
    ...
    def Modifier(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun article à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun article à supprimer dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        track = self.Selection()[0]
...
class DLG_Saisie_element(wx.Dialog):
    def __init__(self, parent):
        ...
        self.label_titre = wx.StaticText(self, -1, _("Titre de l'article :"))
        self.ctrl_titre = wx.TextCtrl(self, -1, "")

        # Dates
        self.label_date_debut = wx.StaticText(self, -1, _("Publication du :"))
    ...
    def __set_properties(self):
        self.SetTitle(_("Saisie d'un article"))
        self.ctrl_titre.SetToolTip(wx.ToolTip(_("Saisissez ici le titre de l'article")))
    ...
    def OnBoutonOk(self, event):
        ...
        if len(self.ctrl_titre.GetValue()) == 0:
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un titre pour cet article !"), "Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.ctrl_date_debut.Validation() == False or self.ctrl_date_debut.GetDate() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de début de publication !"), "Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.ctrl_date_fin.GetDate() == None :
            dlg = wx.MessageDialog(self, _("Etes-vous sûr de ne pas vouloir saisir de date de fin de publication ?"), _("Confirmation"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
...
class MyFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        listeParametres = [
            {"IDelement" : 1, "titre" : "Article 1", "date_debut" : None, "date_fin" : None, "texte_xml" : "", "texte_html" : ""},
            {"IDelement" : 2, "titre" : "Article 2", "date_debut" : None, "date_fin" : None, "texte_xml" : "", "texte_html" : ""},
        ]
