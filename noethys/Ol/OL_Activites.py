...
class Track(object):
    def __init__(self, donnees):
        ...
        if self.date_debut == "1977-01-01" and self.date_fin == "2999-01-01" :
            ...
            self.labelPeriode = _("Illimitée")
        else:
            if self.date_debut != None and self.date_fin != None :
                self.periode = "%s;%s" % (self.date_fin, self.date_debut)
                self.labelPeriode = _("Du %s au %s") % (DateEngFr(self.date_debut), DateEngFr(self.date_fin))
            else:
                self.periode = None
                self.labelPeriode = _("Pas de période")
...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        def FormatePeriode(periode):
            if periode == None :
                return _("Pas de période")
            if periode == "illimitee" :
                return _("Illimitée")
            else:
                date_fin, date_debut = periode.split(";")
            return _("Du %s au %s") % (DateEngFr(date_debut), DateEngFr(date_fin))

        liste_Colonnes = [
            ColumnDefn(_("ID"), "left", 0, "IDactivite", typeDonnee="entier"),
            ColumnDefn(_("Nom de l'activité"), 'left', 220, "nom", typeDonnee="texte", isSpaceFilling=True),
            ColumnDefn(_("Abrégé"), 'left', 80, "abrege", typeDonnee="texte"),
            ColumnDefn(_("Période de validité"), 'left', 200, "periode", typeDonnee="texte", stringConverter=FormatePeriode),
            ]
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des activités"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def ExportTexte(self, event):
        ...
        UTILS_Export.ExportTexte(self, titre=_("Liste des activités"))
        
    def ExportExcel(self, event):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=_("Liste des activités"))
    ...
    def Modifier(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune activité à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune activité à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDactivite = self.Selection()[0].IDactivite
    ...
    def Dupliquer(self, event):
        ...
        if UTILS_Utilisateurs.VerificationDroitsUtilisateurActuel("parametrage_activites", "creer") == False : return
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune activité à dupliquer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDactivite = self.Selection()[0].IDactivite
        ...
        dlg = wx.MessageDialog(None, _("Confirmez-vous la duplication de l'activité '%s' ?") % nom, _("Duplication"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        ...
        # Exportation
        exportation = Exporter(categorie="activite", nouveauNom=_("Copie de %s") % nom)
    ...
    def Importer(self, event):
        ...
        wildcard = _("Activité Noethys (*.nxa)|*.nxa|Tous les fichiers (*.*)|*.*")
        ...
        # Confirmation
        if nbreChoix > 0 :
            dlg = wx.MessageDialog(self, _("%d activité(s) ont été importées avec succès.") % nbreChoix, _("Importation"), wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
    ...
    def Exporter(self, event):
        ...
        for track in self.GetObjects() :
            label = "%s (%s)" % (track.nom, track.labelPeriode)
            listeLabels.append(label)
            listeActivites.append((track.IDactivite, track.nom))

        dlg = wx.MultiChoiceDialog(None, _("Sélectionnez les activités à exporter :"), _("Exportation"), listeLabels)
        dlg.SetSize((550, 500))
        if dlg.ShowModal() == wx.ID_OK :
        ...
        dlg.Destroy()
        if len(selections) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune activité à exporter !\n\nExportation annulée."), _("Exportation"), wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        dlg = wx.FileDialog(self, message=_("Enregistrer le fichier sous..."),
                            defaultDir = standardPath.GetDocumentsDir(), defaultFile="activites.nxa",
                            wildcard=_("Activité Noethys (*.nxa)|*.nxa"), style=wx.FD_SAVE)
        ...
        # Confirmation
        dlg = wx.MessageDialog(self, _("%d activité(s) ont été exportées avec succès.") % len(listeSelection), _("Exportation"), wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
...
class BarreRecherche(wx.SearchCtrl):
    def __init__(self, parent):
        ...
        self.SetDescriptiveText(_("Rechercher une activité..."))
