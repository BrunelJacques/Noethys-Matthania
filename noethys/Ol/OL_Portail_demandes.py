...
class Track(object):
    ...
    def FormateCategorie(self):
        if self.categorie == "factures" : self.categorie_label = _("Factures")
        elif self.categorie == "reglements" : self.categorie_label = _("Règlements")
        elif self.categorie == "inscriptions" : self.categorie_label = _("Inscriptions")
        elif self.categorie == "reservations" : self.categorie_label = _("Réservations")
        elif self.categorie == "renseignements": self.categorie_label = _("Renseignements")
        elif self.categorie == "locations": self.categorie_label = _("Locations")
        elif self.categorie == "pieces": self.categorie_label = _("Pièces")
        elif self.categorie == "compte":self.categorie_label = _("Compte")
        else : self.categorie_label = _("")

    def FormateEtat(self):
        if self.etat == "attente" : self.etat_label = _("En attente")
        elif self.etat == "validation" : self.etat_label = _("Traité")
        else : self.etat_label = _("")
...
class ListView(GroupListView):
    ...
    def GetTracks(self):
        ...
        for IDutilisateur, nom, prenom in listeDonnees :
            self.dictUtilisateurs[IDutilisateur] = "%s %s" % (prenom, nom)
    ...
    def InitObjectListView(self):          
        ...
        dictColonnes = {
            "IDaction" : ColumnDefn(_("IDaction"), "left", 0, "", typeDonnee="texte"),
            "horodatage" : ColumnDefn(_("Horodatage"), "left", 140, "horodatage", typeDonnee="dateheure", stringConverter=FormateHorodatage),
            "etat" : ColumnDefn(_("Etat"), "left", 90, "etat_label", typeDonnee="texte", imageGetter=GetImageEtat),
            "traitement_date" : ColumnDefn(_("Traitée le"), "left", 80, "traitement_date", typeDonnee="date", stringConverter=FormateDate),
            "categorie" : ColumnDefn(_("Catégorie"), "left", 120, "categorie_label", typeDonnee="texte", imageGetter=GetImageCategorie),
            "nom" : ColumnDefn(_("Nom"), "left", 180, "nom", typeDonnee="texte"),
            "description" : ColumnDefn(_("Description"), "left", 300, "description", typeDonnee="texte"),
            "periode" : ColumnDefn(_("Période"), "left", 200, "periode_nom", typeDonnee="texte"),
            "commentaire" : ColumnDefn(_("Commentaire"), "left", 200, "commentaire", typeDonnee="texte"),
            "email_date": ColumnDefn(_("Email"), "left", 95, "email_date", typeDonnee="date", stringConverter=FormateDate, imageGetter=GetImageEmail),
            "reponse" : ColumnDefn(_("Réponse"), "left", 200, "reponse", typeDonnee="texte"),
            "ville": ColumnDefn(_("Ville de résidence"), "left", 150, "ville", typeDonnee="texte"),
            "nom_groupe": ColumnDefn(_("Groupe"), "left", 120, "nom_groupe", typeDonnee="texte"),
            }
        ...
        self.SetEmptyListMsg(_("Aucune demande"))
    ...
    def OnContextMenu(self, event):
        ...
        # Item Traiter
        item = wx.MenuItem(menuPop, 10, _("Traiter la demande"))
        ...
        self.GenerationContextMenu(menuPop, titre=_("Liste des données à synchroniser"))
    ...
    def Commencer(self):
        ...
        if len(tracks) == 0 :
            dlg = wx.MessageDialog(self, _("Il n'y a aucune demande à traiter dans la liste !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Traiter(self, event=None):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune ligne à traiter dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
