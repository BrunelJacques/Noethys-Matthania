...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):           
        ...
        liste_Colonnes = [
            ColumnDefn(_("ID"), "left", 0, "IDselection"),
            ColumnDefn(_("Ordre"), "left", 0, "ordre"),
            ColumnDefn(_("Code"), "left", 200, "code", imageGetter=GetImageCode),
            ColumnDefn(_("Description"), 'left', 250, "label", isSpaceFilling=True),
            ]
    ...
    def OnContextMenu(self, event):
        ...
        # Item Modifier
        item = wx.MenuItem(menuPop, 20, _("Modifier la liste"))
    ...
    def Monter(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun champ dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
...
class MyFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        self.myOlv.SetParametres(IDprofil=3, dateMin=datetime.date(2012, 1, 1), dateMax=datetime.date(2012, 12, 31), listeActivites=[1, 2, 3, 4])
        self.myOlv.MAJ() 
