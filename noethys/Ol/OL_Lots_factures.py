...
class ListView(FastObjectListView):
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun lot de factures à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        nbreFactures = self.Selection()[0].nbreFactures
        if nbreFactures > 0 :
            dlg = wx.MessageDialog(self, _("Il est impossible de supprimer ce lot de factures puisqu'il est déjà constitué de %d facture(s) !") % nbreFactures, _("Suppression impossible"), wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
