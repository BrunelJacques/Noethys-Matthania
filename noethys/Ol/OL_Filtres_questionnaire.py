...
def GetCondition(filtre="", choix="", criteres=""):
    ...
    # TEXTE
    if filtre == "texte" :
        if choix == "EGAL" : description = _("Est égal à '%s'") % criteres
        if choix == "DIFFERENT" : description = _("Est différent de '%s'") % criteres
        if choix == "CONTIENT" : description = _("Contient '%s'") % criteres
        if choix == "CONTIENTPAS" : description = _("Ne contient pas '%s'") % criteres
        if choix == "VIDE" : description = _("Est vide")
        if choix == "PASVIDE" : description = _("N'est pas vide")
    
    # ENTIER
    if filtre == "entier" :
        if choix == "EGAL" : description = _("Est égal à '%s'") % criteres
        if choix == "DIFFERENT" : description = _("Est différent de '%s'") % criteres
        if choix == "SUP" : description = _("Est supérieur à '%s'") % criteres
        if choix == "SUPEGAL" : description = _("Est supérieur ou égal à '%s'") % criteres
        if choix == "INF" : description = _("Est inférieur à '%s'") % criteres
        if choix == "INFEGAL" : description = _("Est inférieur ou égal à '%s'") % criteres
        if choix == "COMPRIS" : description = _("Est compris entre '%s' et '%s'") % (criteres.split(";")[0], criteres.split(";")[1])

    # DATE
    if filtre == "date" :
        if choix == "EGAL" : description = _("Est égal au '%s'") % DateEngFr(criteres)
        if choix == "DIFFERENT" : description = _("Est différent du '%s'") % DateEngFr(criteres)
        if choix == "SUP" : description = _("Est supérieur au '%s'") % DateEngFr(criteres)
        if choix == "SUPEGAL" : description = _("Est supérieur ou égal au '%s'") % DateEngFr(criteres)
        if choix == "INF" : description = _("Est inférieur au '%s'") % DateEngFr(criteres)
        if choix == "INFEGAL" : description = _("Est inférieur ou égal au '%s'") % DateEngFr(criteres)
        if choix == "COMPRIS" : description = _("Est compris entre le '%s' et le '%s'") % (DateEngFr(criteres.split(";")[0]), DateEngFr(criteres.split(";")[1]))

    # COCHE
    if filtre == "coche" :
        if choix == "COCHE" : description = _("Est 'oui'")
        if choix == "DECOCHE" : description = _("Est 'non'")
    ...
    if filtre == "choix" :
        ...
        description = _("Doit être %s") % _(" ou ").join(listeLabelsChoix)
...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ColumnDefn(_("ID"), "left", 0, "IDfiltre"),
            ColumnDefn(_("Question"), "left", 120, "label"),
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des filtres"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def ExportTexte(self, event):
        ...
        UTILS_Export.ExportTexte(self, titre=_("Liste des filtres"))
        
    def ExportExcel(self, event):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=_("Liste des filtres"))
