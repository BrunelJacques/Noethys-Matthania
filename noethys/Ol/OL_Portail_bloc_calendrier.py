...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ColumnDefn(_("ID"), "left", 0, "IDelement", typeDonnee="texte"),
            ColumnDefn(_("Du"), "left", 120, "date_debut", typeDonnee="date", stringConverter=FormateDate),
            ColumnDefn(_("Au"), "left", 120, "date_fin", typeDonnee="date", stringConverter=FormateDate),
            ColumnDefn(_("Titre"), "left", 230, "titre", typeDonnee="texte", isSpaceFilling=True),
            ]
        
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucun évènement"))
    ...
    def Modifier(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun évènement à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun évènement à supprimer dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        track = self.Selection()[0]
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cet évènement ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
...
class DLG_Saisie_element(wx.Dialog):
    def __init__(self, parent):
        ...
        # Dates
        self.label_date_debut = wx.StaticText(self, -1, _("Du :"))
    ...
    def __set_properties(self):
        ...
        self.ctrl_titre.SetToolTip(wx.ToolTip(_("Saisissez ici le titre de l'évènement")))
        self.ctrl_description.SetToolTip(wx.ToolTip(_("[Optionnel] Ajoutez une description pour cet évènement")))
        self.ctrl_couleur.SetToolTip(wx.ToolTip(_("Sélectionnez une couleur pour cet évènement")))
    ...
    def OnBoutonOk(self, event):
        ...
        if len(self.ctrl_titre.GetValue()) == 0:
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un titre pour cet évènement !"), "Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.ctrl_date_debut.Validation() == False or self.ctrl_date_debut.GetDate() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de début valide pour cet évènement !"), "Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.ctrl_date_fin.Validation() == False or self.ctrl_date_fin.GetDate() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de fin valide pour cet évènement !"), "Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
    ...
    def GetDonnees(self):
        ...
        self.dictDonnees["parametres"] = "description:::%s###couleur:::%s" % (self.ctrl_description.GetValue(), self.ctrl_couleur.GetCouleur())
        return self.dictDonnees

    def SetDonnees(self, dictDonnees={}):
        self.dictDonnees = dictDonnees
        self.SetTitle(_("Modification d'un article"))
