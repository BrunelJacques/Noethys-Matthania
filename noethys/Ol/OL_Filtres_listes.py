...
def GetCondition(titre="", typeDonnee="", choix="", criteres=""):
    ...
    # TEXTE
    if typeDonnee == "texte" :
        if choix == "EGAL" : description = _("'%s' est égal à '%s'") % (titre, criteres)
    ...
    if typeDonnee == "entier" :
        if choix == "EGAL" : description = _("'%s' est égal à '%s'") % (titre, criteres)
        if choix == "DIFFERENT" : description = _("'%s' est différent de '%s'") % (titre, criteres)
        if choix == "SUP" : description = _("'%s' est supérieur à '%s'") % (titre, criteres)
        if choix == "SUPEGAL" : description = _("'%s' est supérieur ou égal à '%s'") % (titre, criteres)
    ...
    if typeDonnee == "montant" :
        if choix == "EGAL" : description = _("'%s' est égal à %.2f %s") % (titre, float(criteres), SYMBOLE)
        if choix == "DIFFERENT" : description = _("'%s' est différent de %.2f %s") % (titre, float(criteres), SYMBOLE)
        if choix == "SUP" : description = _("'%s' est supérieur à %.2f %s") % (titre, float(criteres), SYMBOLE)
        if choix == "SUPEGAL" : description = _("'%s' est supérieur ou égal à %.2f %s") % (titre, float(criteres), SYMBOLE)
    ...
    # DATE
    if typeDonnee == "date" :
        if choix == "EGAL" : description = _("'%s' est égal au '%s'") % (titre, UTILS_Dates.DateEngFr(criteres))
        if choix == "DIFFERENT" : description = _("'%s' est différent du '%s'") % (titre, UTILS_Dates.DateEngFr(criteres))
        if choix == "SUP" : description = _("'%s' est supérieur au '%s'") % (titre, UTILS_Dates.DateEngFr(criteres))
        if choix == "SUPEGAL" : description = _("'%s' est supérieur ou égal au '%s'") % (titre, UTILS_Dates.DateEngFr(criteres))
    ...
    if typeDonnee == "dateheure" :
        if choix == "EGAL" : description = _("'%s' est égal au '%s'") % (titre, UTILS_Dates.DatetimeEnFr(criteres))
        if choix == "DIFFERENT" : description = _("'%s' est différent du '%s'") % (titre, UTILS_Dates.DatetimeEnFr(criteres))
        if choix == "SUP" : description = _("'%s' est supérieur au '%s'") % (titre, UTILS_Dates.DatetimeEnFr(criteres))
        if choix == "SUPEGAL" : description = _("'%s' est supérieur ou égal au '%s'") % (titre, UTILS_Dates.DatetimeEnFr(criteres))
        if choix == "INF" : description = _("'%s' est inférieur au '%s'") % (titre, UTILS_Dates.DatetimeEnFr(criteres))
        if choix == "INFEGAL" : description = _("'%s' est inférieur ou égal au '%s'") % (titre, UTILS_Dates.DatetimeEnFr(criteres))
    ...
    if typeDonnee == "inscrits" :
        if choix == "INSCRITS" : description = _("L'individu est inscrit sur les activités sélectionnées")
        if choix == "PRESENTS" : description = _("L'individu est inscrit sur les activités sélectionnées et présent entre le %s et le %s") % (UTILS_Dates.DateDDEnFr(criteres["date_debut"]), UTILS_Dates.DateDDEnFr(criteres["date_fin"]))

    # COTISATIONS
    if typeDonnee == "cotisations" :
        if choix == "AJOUR" : description = _("Les cotisations sélectionnées sont à jour")
...
##            dictTemp["titre"] = dictColonnes[dictTemp["code"]]["titre"]
    ...
    def Modifier(self, event=None):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun filtre à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event=None):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun filtre à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        track = self.Selection()[0]
