...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        self.SetEmptyListMsg(_("Aucune période"))
    ...
    def OnContextMenu(self, event):
        ...
        # Item Assistant
        item = wx.MenuItem(menuPop, 100, _("Générer automatiquement des périodes"))
        ...
        item = wx.MenuItem(menuPop, 110, _("Supprimer les consommations de cette période"))
        bmp = wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Supprimer.png"), wx.BITMAP_TYPE_PNG)
        item.SetBitmap(bmp)
        menuPop.AppendItem(item)
        ...
        item = wx.MenuItem(menuPop, 120, _("Supprimer les consommations de toutes les périodes"))
        bmp = wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Supprimer.png"), wx.BITMAP_TYPE_PNG)
        item.SetBitmap(bmp)
        menuPop.AppendItem(item)
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des périodes de contrats"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def ExportTexte(self, event):
        ...
        UTILS_Export.ExportTexte(self, titre=_("Liste des périodes de contrats"), autoriseSelections=False)
        
    def ExportExcel(self, event):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=_("Liste des périodes de contrats"), autoriseSelections=False)
    ...
    def Modifier(self, event):  
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune période à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):  
        if len(self.Selection()) == 0 and len(self.GetTracksCoches()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune période à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if len(self.GetTracksCoches()) > 0 :
            ...
            dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer les périodes cochées ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
            ...
            dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer la période sélectionnée ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
            reponse = dlg.ShowModal() 
            dlg.Destroy()
            if reponse != wx.ID_YES :
                return
        ...
        for track in listeSelections :
            ...
            if track.IDfacture != None :
                dlg = wx.MessageDialog(self, _("Vous ne pouvez pas supprimer la prestation '%s' car elle apparaît déjà sur la facture n°%s !") % (track.label_prestation, track.numFacture) , _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return
            ...
            if track.nbreConso > 0 :
                dlg = wx.MessageDialog(self, _("Souhaitez-vous également supprimer les %d consommations associées à la période sélectionnée ?") % track.nbreConso, _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
                reponse = dlg.ShowModal() 
                dlg.Destroy()
                ...
                if reponse == wx.ID_YES :
                    for dictConso in track.listeConso :
                        if dictConso["etat"] in ("present", "absenti", "absentj") :
                            dlg = wx.MessageDialog(self, _("Procédure de suppression annulée :\n\nVous ne pouvez pas supprimer la consommation du %s car elle est déjà pointée !") % UTILS_Dates.DateDDEnFr(dictConso["date"]), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                            dlg.ShowModal()
                            dlg.Destroy()
                            return
    ...
    def SupprimerConsoPeriode(self, event):
        if len(self.Selection()) == 0 and len(self.GetTracksCoches()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune période dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        track = self.Selection()[0]
        if len(track.listeConso) == 0 :
            dlg = wx.MessageDialog(self, _("Il n'y a aucun consommation à supprimer dans cette période !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer les %d consommations associées à cette période ?") % len(track.listeConso), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        reponse = dlg.ShowModal() 
        dlg.Destroy()
        if reponse != wx.ID_YES :
            return
    ...
    def SupprimerConsoToutesPeriodes(self, event):
        ...
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer les %d consommations associées à toutes les périodes ?") % len(listeIDconso), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        reponse = dlg.ShowModal() 
        dlg.Destroy()
        if reponse != wx.ID_YES :
            return
...
class ListviewAvecFooter(PanelAvecFooter):
    def __init__(self, parent, kwargs={}):
        dictColonnes = {
            "date_fin" : {"mode" : "nombre", "singulier" : _("période"), "pluriel" : _("périodes"), "alignement" : wx.ALIGN_CENTER},
