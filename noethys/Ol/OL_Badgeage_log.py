...
def Purger():
    ...
    texte = _("La purge du journal de badgeage vous permet de réduire la taille de la base de données. Il est conseillé d'y procéder une fois que vous n'avez plus besoin de l'historique de badgeage (soit quelques mois après).\n\nCommencez par sélectionner une date maximale.")
    ...
    dlg = wx.MessageDialog(None, _("Confirmez-vous la purge de l'historique de badgeage jusqu'au %s inclus ?") % DateEngFr(str(date)), _("Purge du journal de badgeage"), wx.YES_NO|wx.NO_DEFAULT|wx.ICON_EXCLAMATION)
    reponse = dlg.ShowModal()
    dlg.Destroy()
    ...
    DB.Close() 
    # Fin
    dlg = wx.MessageDialog(None, _("Purge du journal de badgeage terminée."), _("Information"), wx.OK | wx.ICON_INFORMATION)
    dlg.ShowModal()
    dlg.Destroy()
...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        if self.modeHistorique == True :
            liste_Colonnes = [
                ColumnDefn(u"", "left", 0, ""),
                ColumnDefn(_("Date"), 'center', 88, "date", typeDonnee="date", stringConverter=FormateDate),
                ColumnDefn(_("Heure"), 'center', 60, "heure", typeDonnee="texte", stringConverter=FormateHeure),
                ColumnDefn(_("Action"), 'left', 270, "action", typeDonnee="texte"),
                ColumnDefn(_("Individu"), 'left', 150, "individu", typeDonnee="texte"),
                ColumnDefn(_("Résultat"), 'left', 150, "texteResultat", typeDonnee="texte", imageGetter=GetImageStatut, isSpaceFilling=True),
                ]
        else :
            liste_Colonnes = [
                ColumnDefn(u"", "left", 0, ""),
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Journal des actions"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def ExportTexte(self, event):
        ...
        UTILS_Export.ExportTexte(self, titre=_("Journal des actions"))
        
    def ExportExcel(self, event):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=_("Journal des actions"))

    def AjouterAction(self, individu=u"", IDindividu=None, heure=None, action="", resultat=None):
        """ Ajouter une action """
        """ heure="14:45" (Si heure=None : heure actuelle automatique), individu=_("DUPOND Kévin"), action="enregistrer", resultat=True """
...
class BarreRecherche(wx.SearchCtrl):
    def __init__(self, parent):
        ...
        self.SetDescriptiveText(_("Rechercher une action..."))
