...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Date Budget"), 'left', 120, "date_budget", typeDonnee="date", stringConverter=FormateDate),
            ColumnDefn(_("Analytique"), "left", 160, "label_analytique", typeDonnee="texte"),
            ColumnDefn(_("Catégorie"), "left", 160, "label_categorie", typeDonnee="texte"),
            ColumnDefn(_("Libellé"), "left", 120, "libelle", typeDonnee="texte", isSpaceFilling=True),
            #ColumnDefn(_("Montant"), "right", 100, "montant", typeDonnee="montant", stringConverter=FormateMontant),
        ...
        self.SetEmptyListMsg(_("Aucune opération budgétaire"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des opérations budgétaires"), format="A", orientation=wx.LANDSCAPE)
        prt.Preview()
    ...
    def ExportTexte(self, event):
        ...
        UTILS_Export.ExportTexte(self, titre=_("Liste des opérations budgétaires"))
        
    def ExportExcel(self, event):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=_("Liste des opérations budgétaires"))
...
##        if UTILS_Utilisateurs.VerificationDroitsUtilisateurActuel("parametrage_categories_comptables", "modifier") == False : return
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune opération budgétaire à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
...
##        if UTILS_Utilisateurs.VerificationDroitsUtilisateurActuel("parametrage_categories_comptables", "supprimer") == False : return
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune opération budgétaire à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        track = self.Selection()[0]            
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cette opération budgétaire ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
