...
LISTE_JOURS = [_("LU"), _("MA"), _("ME"), _("JE"), _("VE"), _("SA"), _("DI")]
...
class Track(object):
    def __init__(self, parent, dictValeurs={}, index=0):
        ...
        if self.feries == True : listeTemp.append(_("Fériés inclus"))
            ...
            if dictUnite["IDunite"] in self.parent.dictUnites :
                ...
                if "options" in dictUnite :
                    if "heure_debut" in dictUnite["options"] :
                        nomUnite += " (%s-%s)" % (dictUnite["options"]["heure_debut"].replace(":", "h"), dictUnite["options"]["heure_fin"].replace(":", "h"))
...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Unités"), 'left', 150, "unites_txt", typeDonnee="texte"),
            ColumnDefn(_("Conditions"), 'left', 200, "criteres_txt", typeDonnee="texte", isSpaceFilling=True),
            ]
        
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucun paramètre de planning"))
    ...
    def Modifier(self, event):  
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun élément de planning à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):  
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun élément de planning à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return

        track = self.Selection()[0]     
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer l'élément de planning sélectionné ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
...
class MyFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        self.bouton_test = wx.Button(panel, -1, _("Test de sauvegarde"))
