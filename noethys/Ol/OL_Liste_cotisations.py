...
class Track(object):
    def __init__(self, parent, donnees, dictFacturation):
        ...
        # Nom cotisation
        self.nomCotisation = "%s - %s" % (self.nomTypeCotisation, self.nomUniteCotisation)
        ...
        if self.typeTypeCotisation == "famille" :
            self.typeStr = _("Cotisation familiale")
        else:
            self.typeStr = _("Cotisation individuelle")
        ...
        if self.IDdepot_cotisation == None :
            self.depotStr = _("Non déposée")
        else:
            self.depotStr = _("Dépôt n°%d") % self.IDdepot_cotisation
        ...
        if self.IDfamille != None :
            self.beneficiaires = _("IDfamille n°%d") % self.IDfamille
...
class ListView(FastObjectListView):
    def __init__(self, *args, **kwds):
        ...
        self.titre = _("Liste des cotisations")
    ...
    def InitObjectListView(self):            
        ...
        def FormateDate(dateDD):
            ...
            if dateDD == "2999-01-01" : return _("Illimitée")
            date = str(dateDD)
        ...
        dictColonnes = {
            ...
            "date_fin" : ColumnDefn(_("Au"), 'left', 80, "date_fin", typeDonnee="date", stringConverter=FormateDate),
            "date_saisie" : ColumnDefn(_("Date saisie"), 'left', 80, "date_saisie", typeDonnee="date", stringConverter=FormateDate),
            "beneficiaires" : ColumnDefn(_("Bénéficiaires"), 'left', 150, "beneficiaires", typeDonnee="texte"),
            "rue" : ColumnDefn(_("Rue"), 'left', 120, "rue", typeDonnee="texte"),
            "cp" : ColumnDefn(_("CP"), 'left', 70, "cp", typeDonnee="texte"),
            "ville" : ColumnDefn(_("Ville"), 'left', 100, "ville", typeDonnee="texte"),
            "type" : ColumnDefn(_("Type"), 'left', 110, "typeStr", typeDonnee="texte"),
            "nom" : ColumnDefn(_("Nom"), 'left', 210, "nomCotisation", typeDonnee="texte"),
            "type_cotisation" : ColumnDefn(_("Type"), 'left', 210, "nomTypeCotisation", typeDonnee="texte"),
            "unite_cotisation" : ColumnDefn(_("Unité"), 'left', 90, "nomUniteCotisation", typeDonnee="texte"),
            "numero" : ColumnDefn(_("Numéro"), 'left', 70, "numero", typeDonnee="texte"),
            "montant" : ColumnDefn(_("Montant"), 'left', 70, "montant", typeDonnee="montant", stringConverter=FormateMontant),
            "regle" : ColumnDefn(_("Réglé"), 'left', 70, "ventilation", typeDonnee="montant", stringConverter=FormateMontant),
            "date_reglement" : ColumnDefn(_("Date réglement"), 'left', 80, "dateReglement", typeDonnee="date", stringConverter=FormateDate),
            "mode_reglement" : ColumnDefn(_("Mode réglement"), 'left', 80, "modeReglement", typeDonnee="texte"),
            "solde" : ColumnDefn(_("Solde"), 'left', 80, "solde", typeDonnee="montant", stringConverter=FormateMontant, imageGetter=GetImageVentilation),
            "date_creation_carte" : ColumnDefn(_("Création carte"), 'left', 100, "date_creation_carte", typeDonnee="date", stringConverter=FormateDate, imageGetter=GetImageCreation),
            "depot_nom" : ColumnDefn(_("Dépôt carte"), 'left', 100, "depotStr", typeDonnee="texte", imageGetter=GetImageDepot),
            "activites" : ColumnDefn(_("Activités"), 'left', 150, "activitesStr", typeDonnee="texte"),
            "observations" : ColumnDefn(_("Notes"), 'left', 150, "observations", typeDonnee="texte"),
            }
        ...
        if self.mode == "liste":
            ...
            for titre, exemple, code in listeChamps :
                if "n°" not in titre and "_x_" not in code:
        ...
        self.SetEmptyListMsg(_("Aucune cotisation"))
    ...
    def OnContextMenu(self, event):
        ...
        item = wx.MenuItem(menuPop, 100, _("Imprimer la cotisation"))
        ...
        item = wx.MenuItem(menuPop, 110, _("Envoyer la cotisation par Email"))
        ...
        item = wx.MenuItem(menuPop, 60, _("Editer un reçu Dons aux Oeuvres (PDF)"))
        bmp = wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Apercu.png"), wx.BITMAP_TYPE_PNG)
        item.SetBitmap(bmp)
        menuPop.AppendItem(item)
    ...
    def Reedition(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune cotisation à imprimer !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def RecuCotisation(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune cotisation dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDcotisation = self.Selection()[0].IDcotisation
    ...
    def EnvoyerEmail(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune cotisation à envoyer par Email !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        UTILS_Envoi_email.EnvoiEmailFamilles(parent=self, IDfamille=track.IDfamille,
                                             nomDoc=FonctionsPerso.GenerationNomDoc("COTISATION", "pdf"),
    ...
    def GetTextesImpression(self):
        total = _("%d cotisations. ") % len(self.donnees)
        if self.filtres != None :
        ...
        if self.dictFamillesRattachees != None :
            ...
            for IDfamille, dictFamille in self.dictFamillesRattachees.items() :
                if dictFamille["IDcategorie"] in (1, 2) :
                    valide = True
            if valide == False :
                dlg = wx.MessageDialog(self, _("Pour bénéficier d'une cotisation, un individu doit obligatoirement être\nrattaché comme représentant ou enfant à une fiche famille !"), _("Création de cotisation impossible"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune cotisation à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        track = self.Selection()[0]
    ...
    def Supprimer(self, event):
        if len(self.Selection()) == 0 and len(self.GetTracksCoches()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune cotisation à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if len(self.GetTracksCoches()) > 0 :
            ...
            dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer les %d cotisations cochées ?") % len(listeSelections), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
            ...
            dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer la cotisation n°%s ?") % listeSelections[0].numero, _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
            reponse = dlg.ShowModal() 
            dlg.Destroy()
            if reponse != wx.ID_YES :
                return
        ...
        if len(listeFactures) > 0 :
            ...
            if nbreCotisations > 0 :
                if nbreCotisations == 1 :
                    message = _("Cette cotisation apparaît déjà sur une facture. Il est donc impossible de la supprimer.")
                else :
                    message = _("%d de ces cotisations apparaissent déjà sur une ou plusieurs factures. Il est donc impossible d'effectuer la suppression.") % len(listeFactures)
                dlg = wx.MessageDialog(self, message, _("Suppression impossible"), wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                return
        ...
        for track in listeSelections :
            ...
            UTILS_Historique.InsertActions([{
                ...
                "IDcategorie" : 23, 
                "action" : _("Suppression de la cotisation ID%d '%s' pour la période du %s au %s") % (track.IDcotisation, track.nomCotisation, UTILS_Dates.DateEngFr(str(track.date_debut)), UTILS_Dates.DateEngFr(str(track.date_fin))),
                },])
...
class ListviewAvecFooter(PanelAvecFooter):
    def __init__(self, parent, kwargs={}):
        dictColonnes = {
            "beneficiaires" : {"mode" : "nombre", "singulier" : _("cotisation"), "pluriel" : _("cotisations"), "alignement" : wx.ALIGN_CENTER},
            "montant" : {"mode" : "total"},
