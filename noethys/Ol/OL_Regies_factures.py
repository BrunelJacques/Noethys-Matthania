...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Numéro Client TIPI"), 'center', 150, "numclitipi", typeDonnee="entier"),
            ColumnDefn(_("Nbre factures associées"), 'center', 150, "nbre_factures", typeDonnee="entier"),
            ]

        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucune régie"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des régies de facturation"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune régie à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if track.nbre_factures > 0 :
            dlg = wx.MessageDialog(self, _("Cette régie est déjà associée à %d factures.\n\nIl sera donc uniquement possible de modifier le nom de la régie.") % track.nbre_factures, _("Avertissement"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune régie à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDregie = self.Selection()[0].IDregie
        
        # Vérifie que cett réie n'a pas déjà été attribué à une facture
        DB = GestionDB.DB()
        ...
        DB.Close()
        if nbreFactures > 0 :
            dlg = wx.MessageDialog(self, _("Cette régie a déjà été attribuée à %d facture(s).\n\nVous ne pouvez donc pas la supprimer !") % nbreFactures, _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return

        # Suppression
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cette régie ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
