...
##            self.RefreshObject(track)
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Type"), 'left', 70, "type", typeDonnee="texte", stringConverter=FormateType),
...
##            ColumnDefn(_("Banque"), 'left', 120, "nomBanque"),
            ...
            ColumnDefn(_("Séquence"), 'left', 70, "sequence", typeDonnee="texte"),
...
##            ColumnDefn(_("Clé"), 'left', 30, "prelevement_cle"),
##            ColumnDefn(_("Banque"), 'left', 130, "nomBanque"),
        ...
        self.SetEmptyListMsg(_("Aucun prélèvement"))
...
##        self.SetSortColumn(self.columns[self.numColonneTri])
    ...
    def GetParametresImpression(self):
        ...
        dictParametres = {
            "titre" : _("Liste des prélèvements"),
            "intro" : intro,
            "total" : total,
    ...
    def Saisie_factures(self, event=None):
        ...
        if self.GetParent().GetVerrouillage() == True :
            dlg = wx.MessageDialog(self, _("Vous ne pouvez pas effectuer cette action car le lot de prélèvement est verrouillé !"), _("Verrouillage"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def AjoutFactures(self, tracks=[]):
        ...
        for track in tracks :
            ...
            if IDmandat == None :
                ...
                listeMandatsNonValides.append(_("Famille %s (ID%d)") % (nomTitulaires, track.IDfamille))
        ...
        if len(listeMandatsNonValides) > 0 :
            message1 = _("Les prélèvements suivants n'ont pas été intégrés car les mandats SEPA des familles sont indisponibles ou non valides :")
...
##        dlg.Destroy() 
    ...
    def Modifier(self, event=None):
        if self.GetParent().GetVerrouillage() == True :
            ...
            return
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun prélèvement à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
...
##        self.MAJnomsBanquesTracks()
    ...
    def Supprimer(self, event=None):
        ...
        if len(self.Selection()) == 0 and len(self.GetTracksCoches()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun prélèvement à retirer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if len(self.GetTracksCoches()) > 0 :
            ...
            dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment retirer les %d prélèvements cochés ?") % len(listeSelections), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
            reponse = dlg.ShowModal() 
            dlg.Destroy()
            ...
            dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment retirer le prélèvement sélectionné ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
            reponse = dlg.ShowModal() 
            dlg.Destroy()
            if reponse != wx.ID_YES :
                return
        ...
        if len(listeDepots) > 0 :
            dlg = wx.MessageDialog(self, _("Suppression interdite car %d prélèvement(s) de la sélection ont déjà un règlement attribué à un dépôt de règlement !") % len(listeDepots), _("Remarque"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def SetStatut(self, statut="attente") :
        ...
        if len(listeDepots) > 0 :
            dlg = wx.MessageDialog(self, _("Notez que Noethys n'a pas procédé changement de statut de %d prélèvement(s) car le règlement correspondant appartient déjà à un dépôt de règlement !") % len(listeDepots), _("Remarque"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def SetRegle(self, etat=False) :
        ...
        if len(listeAnomalies) > 0 :
            dlg = wx.MessageDialog(self, _("Notez que Noethys n'a pas procédé au règlement de %d prélèvement(s) en raison de leur statut 'Refus'.") % len(listeAnomalies), _("Remarque"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return

        if len(listeDepots) > 0 :
            dlg = wx.MessageDialog(self, _("Notez que Noethys n'a pas procédé à la suppression de %d prélèvement(s) car le règlement correspondant appartient déjà à un dépôt de règlement !") % len(listeDepots), _("Remarque"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def GetTexteTotaux(self):
        ...
        # Création du texte
        if nbreTotal == 0 :
            texte = _("<B>Aucun prélèvement.   </B>")
        elif nbreTotal == 1 :
            texte = _("<B>%d prélèvement (%.2f %s) : </B>") % (nbreTotal, montantTotal, SYMBOLE)
        else :
            texte = _("<B>%d prélèvements (%.2f %s) : </B>") % (nbreTotal, montantTotal, SYMBOLE)
    ...
    def MemoriseReglementHistorique(self, mode="saisie", IDfamille=None, IDreglement=None, montant=0.0):
        ...
        montantStr = "%.2f %s" % (montant, SYMBOLE)
        dictAction = {
            "IDfamille" : IDfamille,
            "IDcategorie" : IDcategorie, 
            "action" : _("%s du règlement ID%d : %s payés par prélèvement automatique SEPA") % (categorie, IDreglement, montantStr),
            }
        return dictAction
...
class BarreRecherche(wx.SearchCtrl):
    def __init__(self, parent):
        ...
        self.SetDescriptiveText(_("Rechercher un prélèvement..."))
