...
class Track(object):
    def __init__(self, donnees):
        ...
        self.label = "%s%s  -  %s" % (" "*len(self.numero)*4, self.numero, self.nom)
...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Nom"), 'left', 200, "label", typeDonnee="texte", isSpaceFilling=True),
            ColumnDefn(_("Numéro"), 'left', 0, "numero", typeDonnee="texte"),
            ]
        
        self.rowFormatter = rowFormatter
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(_("Aucun compte"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des comptes comptables"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Supprimer(self, event):
        ...
        # Vérifie que ce compte n'a pas déjà été attribué à une catégorie
        DB = GestionDB.DB()
        ...
        DB.Close()
        if nbreCategorie > 0 :
            dlg = wx.MessageDialog(self, _("Ce compte a déjà été attribué à %d catégorie(s).\n\nVous ne pouvez donc pas le supprimer !") % nbreCategorie, _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
