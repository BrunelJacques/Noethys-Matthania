...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Date de début"), 'left', 100, "date_debut", typeDonnee="date", isSpaceFilling=True, stringConverter=FormateDate),
            ...
            ColumnDefn(_("Tarif dépass."), 'center', 90, "tarif_depassement", typeDonnee="montant", stringConverter=FormateMontant2),
            ]
    ...
    def Modifier(self, event):  
        if len(self.Selection()) == 0 :
           dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun tarif à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
           dlg.ShowModal()
           dlg.Destroy()
           return
    ...
    def Supprimer(self, event):  
        ...
        #     dlg = wx.MessageDialog(self, _("Attention, si vous supprimez ce tarif, la période du contrat ne disposera pas de tarif disponible au premier jour du contrat !\n\nSouhaitez-vous tout de même supprimer le tarif sélectionné ?"), _("Avertissement"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        #     reponse = dlg.ShowModal()
        #     dlg.Destroy()
        #     if reponse != wx.ID_YES :
        #         return

        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer le tarif sélectionné ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        reponse = dlg.ShowModal()
        dlg.Destroy()
        if reponse != wx.ID_YES :
            return
