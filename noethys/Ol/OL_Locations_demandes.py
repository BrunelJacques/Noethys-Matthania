...
class Track(object):
    def __init__(self, listview=None, donnees=None):
        ...
        if self.statut == "refusee" :
            self.texte_statut = _("Refusée")
        elif self.statut == "attribuee" :
            self.texte_statut = _("Demande satisfaite")
            ...
            if self.IDdemande in listview.dictPropositions :
                ...
                if len(listeProduitsProposes) == 0 :
                    self.texte_statut = _("En attente")
                elif len(listeProduitsProposes) == 1 :
                    self.texte_statut = _("1 produit disponible")
                else :
                    self.texte_statut = _("%d produits disponibles") % len(listeProduitsProposes)
...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        def FormatePosition(position):
            if position == None :
                ...
                return _("%dème") % position

        liste_Colonnes = [
            ColumnDefn(u"", "left", 0, "IDdemande", typeDonnee="entier"),
            ColumnDefn(_("Date"), "left", 130, "date", typeDonnee="date", stringConverter=FormateDate),
            ]

        if self.IDfamille == None :
            liste_Colonnes.extend([
                ColumnDefn(_("Nom du demandeur"), 'left', 210, "nomTitulaires", typeDonnee="texte"),
                # ColumnDefn(_("Rue"), 'left', 200, "rue", typeDonnee="texte"),
                # ColumnDefn(_("C.P."), 'left', 70, "cp", typeDonnee="texte"),
                # ColumnDefn(_("Ville"), 'left', 150, "ville", typeDonnee="texte"),
                ])

        liste_Colonnes.extend([
            ColumnDefn(_("Statut de la demande"), 'left', 170, "texte_statut", typeDonnee="texte", imageGetter=GetImageStatut),
            ColumnDefn(_("Position"), 'left', 80, "position", typeDonnee="entier", stringConverter=FormatePosition),
            ColumnDefn(_("Catégories demandées"), 'left', 200, "texte_categories", typeDonnee="texte"),
            ColumnDefn(_("Produits demandés"), 'left', 200, "texte_produits", typeDonnee="texte"),
            ColumnDefn(_("Notes"), 'left', 200, "observations", typeDonnee="texte"),
            ])
        ...
        self.SetEmptyListMsg(_("Aucune demande de location"))
    ...
    def OnContextMenu(self, event):
        ...
        # Item Imprimer
        item = wx.MenuItem(menuPop, 100, _("Imprimer la demande"))
        ...
        item = wx.MenuItem(menuPop, 110, _("Envoyer la demande par Email"))
        ...
        self.GenerationContextMenu(menuPop, titre=_("Liste des demandes de locations"))
    ...
    def Modifier(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune demande à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune demande à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        track = self.Selection()[0]
        ...
        # Suppression
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cette demande de location ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
    ...
    def ImprimerPDF(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune demande à imprimer !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def EnvoyerEmail(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune demande à envoyer par Email !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        track = self.Selection()[0]
...
class ListviewAvecFooter(PanelAvecFooter):
    def __init__(self, parent, kwargs={}):
        dictColonnes = {
            "date": {"mode": "nombre", "singulier": _("demande"), "pluriel": _("demandes"), "alignement": wx.ALIGN_CENTER},
        }
