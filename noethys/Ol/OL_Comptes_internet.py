...
class Track(object):
    def __init__(self, parent, donnees):
        ...
        if self.IDfamille in parent.dict_titulaires:
            ...
            self.nomTitulaires = _("Titulaires inconnus")
...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):
        ...
        liste_Colonnes = [
            ColumnDefn(_("ID"), "left", 0, "IDfamille", typeDonnee="entier"),
            ColumnDefn(_("Famille"), 'left', 350, "nomTitulaires", typeDonnee="texte"),
            ColumnDefn(_("Activation"), "left", 120, "internet_actif", typeDonnee="bool", stringConverter=FormateActivation, imageGetter=GetImageActivation),
            ColumnDefn(_("Identifiant"), "left", 120, "internet_identifiant", typeDonnee="texte"),
            ColumnDefn(_("Mot de passe"), "left", 120, "internet_mdp", typeDonnee="texte"),
            ]
    ...
    def GetParametresImpression(self):
        dictParametres = {
            "titre": _("Liste des comptes internet"),
            "total": _("> %s familles") % len(self.donnees),
    ...
    def ExportTexte(self, event):
        ...
        UTILS_Export.ExportTexte(self, titre=_("Liste des comptes internet"))
        
    def ExportExcel(self, event):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=_("Liste des comptes internet"))
    ...
    def SetActivation(self, actif=1):
        ...
        if len(listeCoches) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez coché aucun compte internet à modifier !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if actif == True :
            ...
            texte = _("désactiver")
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment %s les %d comptes sélectionnés ?") % (texte, len(listeCoches)), _("Modification"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
    ...
    def ReinitPasswords(self, event=None):
        ...
        listeCoches = self.GetTracksCoches()
        if len(listeCoches) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez coché aucun compte internet à réinitialiser !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return

        # Demandes de confirmation
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment réinitialiser les mots de passe des %d comptes sélectionnés ?") % len(listeCoches), _("Modification"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        reponse = dlg.ShowModal()
        dlg.Destroy()
        if reponse != wx.ID_YES :
            return

        dlg = wx.MessageDialog(self, _("ATTENTION, TOUS LES MOTS DE PASSE SERONT REINITIALISES !\n\nSouhaitez-vous vraiment regénérer les mots de passe des %d comptes sélectionnés ?") % len(listeCoches), _("Avertissement"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
        reponse = dlg.ShowModal()
        dlg.Destroy()
        if reponse != wx.ID_YES :
            return
        ...
        dlg = wx.MessageDialog(self, _("Les mots de passe comporteront %d caractères.\n\nSi cela vous convient, cliquez sur Oui, sinon annulez et allez dans Menu Paramétrage > Préférences pour modifier la taille des mots de passe des comptes internet.") % taille, _("Avertissement"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        reponse = dlg.ShowModal()
        dlg.Destroy()
        if reponse != wx.ID_YES :
            return

        dlg = wx.MessageDialog(self, _("DERNIER AVERTISSEMENT !\n\nConfirmez-vous la réinitialisation des mots de passe ? Vous ne pourrez pas revenir en arrière..."), _("Avertissement"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
        reponse = dlg.ShowModal()
        dlg.Destroy()
        if reponse != wx.ID_YES :
            return
...
class ListviewAvecFooter(PanelAvecFooter):
    def __init__(self, parent, kwargs={}):
        dictColonnes = {
            "nomTitulaires": {"mode": "nombre", "singulier": _("famille"), "pluriel": _("familles"), "alignement": wx.ALIGN_CENTER},
        }
