...
class Track(object):
    def __init__(self, fichier=""):
        ...
        if self.taille != None :
            ...
            self.label = "%s (%s)" % (self.nomFichier, texteTaille)
        else :
...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Pièce"), 'left', 180, "label", imageGetter=GetImagePiece, isSpaceFilling=True),
            ]
...
##        self._ResizeSpaceFillingColumns() 
    ...
    def OnContextMenu(self, event):
        ...
        item = wx.MenuItem(menuPop, 20, _("Ajouter"))
        ...
        # Ouvrir pièce jointe
        item = wx.MenuItem(menuPop, 60, _("Ouvrir la pièce"))
    ...
    def Ajouter(self, event):
        ...
        dlg = wx.FileDialog(self, message=_("Veuillez sélectionner le ou les fichiers à joindre"), defaultDir=rep, defaultFile="", style=wx.FD_OPEN|wx.FD_MULTIPLE)
    ...
    def Retirer(self, event):
        """ Retirer pièces """
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez d'abord sélectionner une pièce à retirer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Ouvrir(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez d'abord sélectionner une pièce à ouvrir dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        track = self.Selection()[0]
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des pièces jointes"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
