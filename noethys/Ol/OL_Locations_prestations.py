...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Date"), 'left', 100, "date", typeDonnee="date", stringConverter=FormateDate),
            ColumnDefn(_("Label"), "left", 280, "label", typeDonnee="texte"),
            ColumnDefn(_("Montant"), "right", 100, "montant", typeDonnee="montant", stringConverter=FormateMontant),
            ColumnDefn(_("Facturé"), "right", 80, "IDfacture", typeDonnee="entier", stringConverter=FormateFacture),
            ]
    ...
    def Modifier(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune prestation à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if track_prestation.IDfacture != None :
            dlg = wx.MessageDialog(self, _("Vous ne pouvez pas modifier cette prestation car elle apparaît déjà sur une facture !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        ...
        if track_prestation.IDfacture != None :
            dlg = wx.MessageDialog(self, _("Vous ne pouvez pas supprimer cette prestation car elle apparaît déjà sur une facture !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cette prestation ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
    ...
    def MAJtexteOnglet(self):
        ...
        if total > 0.0 :
            texte = "Prestations (%.2f %s)" % (total, SYMBOLE)
