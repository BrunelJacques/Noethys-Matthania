...
class ListView(FastObjectListView):
    ...
    def Modifier(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune langue à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun fichier de traduction à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        track = self.Selection()[0]
        ...
        if track.perso == 0 :
            dlg = wx.MessageDialog(self, _("Il n'y a aucune traduction personnalisée à supprimer !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return

        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer ce fichier de traduction personnalisé (%d traductions) ?\n\nAttention, toute suppression est irréversible.") % track.perso, _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
    ...
    def Importer(self, event):
        ...
        dlg = wx.FileDialog(
            self, message=_("Choisissez un fichier de langue à importer"),
        ...
        if os.path.isfile(UTILS_Fichiers.GetRepLang(nomFichierCourt)) == False :
            ...
            dlg = DLG_Messagebox.Dialog(self, titre=_("Importer"), introduction=_("Ce fichier est déjà présent !"), detail=None, conclusion=_("Souhaitez-vous le remplacer ou les fusionner ?"), icone=wx.ICON_EXCLAMATION, boutons=[_("Fusionner"), _("Remplacer"), _("Annuler")])
            ...
            if reponse == 0 : 
                ...
                dlg = wx.MessageDialog(self, _("Le fichier a été importé avec succès !"), _("Confirmation"), wx.OK | wx.ICON_INFORMATION)
                dlg.ShowModal()
                dlg.Destroy()
    ...
    def Exporter(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune langue dans la liste !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        track = self.Selection()[0]
        ...
        if track.perso == 0 :
            dlg = wx.MessageDialog(self, _("Il n'y a aucune traduction personnalisée à exporter !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        dlg = wx.FileDialog(self, message=_("Envoyer le fichier de traduction personnalisé vers..."),
        ...
        # Confirmation
        dlg = wx.MessageDialog(self, _("Le fichier de traduction a été exporté avec succès !"), _("Exportation"), wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
...
class Saisie(wx.Dialog):
    def __init__(self, parent):
        ...
        self.label_langue = wx.StaticText(self, -1, _("Sélectionnez une langue :"))
    ...
    def OnBoutonOk(self, event):
        ...
        if index == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner une langue dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
