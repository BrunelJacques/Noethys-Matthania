...
class Track(object):
    ...
    def MAJ(self):
        ...
        self.label_prestation = "%s %s" % (LISTE_MOIS[self.mois-1], self.annee)
...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):
        ...
        def FormateMois(donnee):
            if donnee in ("", None):
                ...
                annee, mois = donnee
                return "%s %s" % (LISTE_MOIS[mois-1], annee)

        liste_Colonnes = [
            ColumnDefn(_("IDprestation"), "left", 0, "IDprestation", typeDonnee="entier"),
            ColumnDefn(_("Mois"), 'left', 110, "annee_mois", typeDonnee="texte", stringConverter=FormateMois),
            ColumnDefn(_("Heures prév."), 'center', 80, "heures_prevues", typeDonnee="duree", stringConverter=FormateDuree),
            ColumnDefn(_("Montant prév."), 'center', 90, "montant_prevu", typeDonnee="montant", stringConverter=FormateMontant),
            ColumnDefn(_("Date fact."), 'center', 80, "date_facturation", typeDonnee="date", stringConverter=FormateDate),
            ColumnDefn(_("Heures fact."), 'center', 80, "heures_facturees", typeDonnee="duree", stringConverter=FormateDuree),
            ColumnDefn(_("Montant fact."), 'center', 90, "montant_facture", typeDonnee="montant", stringConverter=FormateMontant),
            ColumnDefn(_("N° Facture"), 'center', 70, "num_facture", typeDonnee="entier"),
            ColumnDefn(_("Taux"), 'center', 80, "taux", typeDonnee="montant", stringConverter=FormateMontant2),
            ColumnDefn(_("Tarif de base"), 'center', 80, "tarif_base", typeDonnee="montant", stringConverter=FormateMontant2),
            ]
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des mensualités"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
...
class ListviewAvecFooter(PanelAvecFooter):
    def __init__(self, parent, kwargs={}):
        dictColonnes = {
            "annee_mois" : {"mode" : "nombre", "singulier" : _("mensualité"), "pluriel" : _("mensualités"), "alignement" : wx.ALIGN_CENTER},
