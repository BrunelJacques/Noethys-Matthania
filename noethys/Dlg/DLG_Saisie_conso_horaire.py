...
class Dialog(wx.Dialog):
    def __init__(self, parent, nouveau=False, heure_min=None, heure_max=None):
        ...
        if nouveau == True :
            intro = _("Vous pouvez saisir ici une consommation horaire. Cliquez sur le bouton Horloge pour insérer l'heure actuelle.")
            titre = _("Saisie d'un horaire")
        else:
            intro = _("Vous pouvez modifier ici une consommation horaire. Cliquez sur le bouton Horloge pour insérer l'heure actuelle.")
            titre = _("Modification d'un horaire")
        self.SetTitle(titre)
        ...
        self.label_heure_debut = wx.StaticText(self, -1, _("De"))
...
##        self.ctrl_heure_fin.Bind(wx.EVT_TEXT_ENTER, self.OnBoutonOk)
    ...
    def __set_properties(self):
        self.bouton_heure_debut_now.SetToolTip(wx.ToolTip(_("Cliquez ici pour appliquer l'heure actuelle à l'heure de début")))
        self.bouton_heure_fin_now.SetToolTip(wx.ToolTip(_("Cliquez ici pour appliquer l'heure actuelle à l'heure de fin")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer cette consommation")))
    ...
    def OnBoutonOk(self, event=None):
        ...
        if self.ctrl_heure_debut.GetHeure() == None or self.ctrl_heure_debut.Validation() == False :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une heure de début valide !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_heure_debut.SetFocus()
            return
        if self.ctrl_heure_fin.GetHeure() == None or self.ctrl_heure_fin.Validation() == False :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une heure de fin valide !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_heure_fin.SetFocus()
            return
        if self.ctrl_heure_debut.GetHeure() > self.ctrl_heure_fin.GetHeure() :
            dlg = wx.MessageDialog(self, _("L'heure de début ne peut pas être supérieure à l'heure de fin !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        if self.ctrl_heure_debut.GetHeure() == self.ctrl_heure_fin.GetHeure() :
            dlg = wx.MessageDialog(self, _("L'heure de début ne peut pas être égale à l'heure de fin !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
##        if self.heure_min != None and self.ctrl_heure_debut.GetHeure() < self.heure_min :
##            dlg = wx.MessageDialog(self, _("L'heure de début ne peut pas être inférieure à %s !") % self.heure_min, _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
##            dlg.ShowModal()
##            dlg.Destroy()
##            self.ctrl_heure_debut.SetFocus()
##            return
##        if self.heure_max != None and self.ctrl_heure_fin.GetHeure() < self.heure_max :
##            dlg = wx.MessageDialog(self, _("L'heure de fin ne peut pas être supérieure à %s !") % self.heure_max, _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
##            dlg.ShowModal()
##            dlg.Destroy()
##            self.ctrl_heure_fin.SetFocus()
##            return
