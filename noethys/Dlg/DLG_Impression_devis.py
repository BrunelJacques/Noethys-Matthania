...
class CTRL_Individus(wx.CheckListBox):
    ...
    def Importation(self):
        listeIndividus = [(_("Prestations familiales"), 0)]
        dictIndividus = {}
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDfamille=None, date_debut=None, date_fin=None):
        ...
        # Bandeau
        intro = _("Vous pouvez ici éditer un devis au format PDF. Saisissez une période, sélectionnez les éléments de votre choix, puis cliquez sur 'Aperçu'.")
        titre = _("Edition d'un devis")
    ...
    def __set_properties(self):
        self.SetTitle(_("Edition d'un devis"))
    ...
    def Sauvegarder(self):
        ...
        UTILS_Historique.InsertActions([{
                ...
                "IDcategorie" : 36,
                "action" : _("Edition d'un devis pour la période du %s au %s pour un total de %.02f € et un solde de %.02f €") % (UTILS_Dates.DateEngFr(self.dictSave["date_debut"]), UTILS_Dates.DateEngFr(self.dictSave["date_fin"]), self.dictSave["total"], self.dictSave["solde"] ),
                },])
    ...
    def OnBoutonEmail(self, event): 
        ...
        UTILS_Envoi_email.EnvoiEmailFamilles(parent=self, IDfamille=self.IDfamille, nomDoc=FonctionsPerso.GenerationNomDoc("DEVIS", "pdf"), categorie="devis")
    ...
    def CreationPDF(self, nomDoc=FonctionsPerso.GenerationNomDoc("DEVIS", "pdf") , afficherDoc=True):
        ...
        if dictOptions["IDmodele"] == -1:
            dlg = wx.MessageDialog(self, _("Vous devez sélectionner un modèle de document !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
