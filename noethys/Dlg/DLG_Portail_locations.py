...
class Dialog(wx.Dialog):
    def __init__(self, parent, track=None):
        ...
        # Bandeau
        intro = _("Vous pouvez gérer ici la demande de façon manuelle. Commencez par cliquer sur le bouton 'Appliquer la demande' pour voir apparaître les modifications demandées sur le portail. Vous pouvez alors effectuer manuellement d'éventuelles modifications avant de valider. Décochez les actions à refuser.")
        titre = _("Traitement manuel des locations")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Location.png")

        # Locations
        self.box_locations_staticbox = wx.StaticBox(self, wx.ID_ANY, _("Détail de la demande"))
    ...
    def OnBoutonTraiter(self, event=None):
        self.parent.EcritLog(_("Application de la demande de locations"), self.ctrl_log)
        ...
        for track in self.ctrl_locations.GetObjects():
            ...
            if track.resultat != "ok":
                ...
                if self.ctrl_locations.IsChecked(track):
                    if track.etat == "ajouter":
                        if track.action_possible == True:
                            ...
                            resultat = _("Ajout de la location %s du %s") % (track.nom_produit, track.date_debut_txt)
                            ...
                            resultat = _("%s du %s : %s") % (track.nom_produit, track.date_debut_txt, track.statut)
                    ...
                    if track.etat == "modifier":
                        if track.action_possible == True:
                            ...
                            resultat = _("Modification de la location %s du %s") % (track.nom_produit, track.date_debut_txt)
                            DB.ReqMAJ("portail_reservations_locations", [("resultat", "ok")], "IDreservation", track.IDreservation)
                    ...
                    if track.etat == "supprimer":
                        if track.action_possible == True:
                            ...
                            resultat = _("Suppression de la location %s du %s") % (track.nom_produit, track.date_debut_txt)
                            DB.ReqMAJ("portail_reservations_locations", [("resultat", "ok")], "IDreservation", track.IDreservation)
                ...
                if not self.ctrl_locations.IsChecked(track):
                    ...
                    self.parent.EcritLog(_("Refus de l'action : %s." % track.action), self.ctrl_log)
                    refus.append(track)
        ...
        for etat, valeurs in resultats.items():
            for succes, quantite in valeurs.items():
                if quantite:
                    if succes == True:
                        txt_validation = "validé"
                    else:
                        txt_validation = "refusé"
                    if quantite == 1:
                        pluriel = ""
                    else:
                        pluriel = "s"
        ...
        if liste_resultats:
            ...
            self.parent.EcritLog(_("Réponse : %s") % self.reponse, self.ctrl_log)
