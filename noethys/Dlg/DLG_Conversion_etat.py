...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        # Bandeau
        titre = _("Convertir l'état des consommations")
        self.SetTitle(titre)
        intro = _("Sélectionnez l'état à convertir puis l'état souhaité. Vous pouvez également préciser les lignes à impacter.")
        ...
        # Conversion
        self.staticbox_conversion = wx.StaticBox(self, -1, _("Etats"))
    ...
    def __set_properties(self):
        self.ctrl_etat_avant.SetToolTip(wx.ToolTip(_("Sélectionnez l'état à convertir. Seules les consommations ayant cet état seront converties.")))
        self.ctrl_etat_apres.SetToolTip(wx.ToolTip(_("Sélectionnez l'état souhaité")))
        self.radio_lignes_affichees.SetToolTip(wx.ToolTip(_("Sélectionnez cette option pour appliquer la conversion sur toutes les lignes affichées")))
        self.radio_lignes_selectionnees.SetToolTip(wx.ToolTip(_("Sélectionnez cette option pour appliquer la conversion uniquement sur les lignes sélectionnées")))
    ...
    def OnBoutonOk(self, event):
        ...
        if self.ctrl_etat_avant.GetValeur() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner un état à convertir !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        if self.ctrl_etat_apres.GetValeur() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner l'état souhaité !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        if self.ctrl_etat_avant.GetValeur() == self.ctrl_etat_apres.GetValeur()  :
            dlg = wx.MessageDialog(self, _("Les deux états sélectionnés doivent être différents !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
