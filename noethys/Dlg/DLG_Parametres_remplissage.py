...
# -*- coding: iso-8859-15 -*-
#------------------------------------------------------------------------
# Application :    Noethys, Matthania mofifié l'amplitude date pour liste activités
...
class CTRL_activites(wx.CheckListBox):
    ...
    def Importation(self):
        # Récupération des activités JB réduit la durée
        if self.parent.ctrl_masquer_activites.GetValue() == True :
            conditions = "WHERE date_fin>'%s' " % (datetime.date.today() - datetime.timedelta(days=180))
        else :
            conditions = ""
        DB = GestionDB.DB()
...
class Dialog(wx.Dialog):
    def __init__(self, parent, dictDonnees = {}, largeurColonneUnite=60, afficheLargeurColonneUnite=True, afficheAbregeGroupes=True, abregeGroupes=False, affichePresents=1):
        ...
        self.ctrl_masquer_activites = wx.CheckBox(self, -1, _("Masquer les anciennes activités"))
        ...
        self.label_largeurUnite = wx.StaticText(self, -1, _("Largeur des colonnes unités (60 par défaut) :"))
        self.ctrl_largeurUnite = wx.SpinCtrl(self, -1, str(largeurColonneUnite), size=(60, -1))
        self.ctrl_largeurUnite.SetRange(30, 300)

        self.label_abregeGroupes = wx.StaticText(self, -1, _("Utiliser le nom abrégé des groupes :"))
        self.ctrl_abregeGroupes_oui = wx.RadioButton(self, -1, _("Oui"), style = wx.RB_GROUP)
        self.ctrl_abregeGroupes_non = wx.RadioButton(self, -1, _("Non"))
        if abregeGroupes == True :
            self.ctrl_abregeGroupes_oui.SetValue(True)
        else:
            self.ctrl_abregeGroupes_non.SetValue(True)

        self.label_affichePresents = wx.StaticText(self, -1, _("Afficher les présents en temps réel :"))
        self.ctrl_affichePresents_oui = wx.RadioButton(self, -1, _("Oui"), style = wx.RB_GROUP)
        self.ctrl_affichePresents_non = wx.RadioButton(self, -1, _("Non"))
    ...
    def __set_properties(self):
        self.SetTitle(_("Paramètres d'affichage"))
        self.ctrl_masquer_activites.SetToolTip(wx.ToolTip(_("Cochez cette case pour masquer de cette liste les activités vieilles de plus de 2 ans")))
    ...
    def OnBoutonOk(self, event):
        ...
        try :
            ...
            dlg2 = wx.MessageDialog(self, _("La largeur des colonnes unités semble incorrecte !"), _("Erreur de saisie"), wx.OK | wx.ICON_INFORMATION)
            dlg2.ShowModal()
            dlg2.Destroy()
            dlg.Destroy()
            return
        if newLargeur < 30 or newLargeur > 300 :
            dlg2 = wx.MessageDialog(self, _("La largeur des colonnes unités doit être comprise entre 30 et 300 !"), _("Erreur de saisie"), wx.OK | wx.ICON_INFORMATION)
            dlg2.ShowModal()
            dlg2.Destroy()
            return
