...
class Hyperlien(Hyperlink.HyperLinkCtrl):
    ...
    def OnLeftLink(self, event):
        ...
        dlg = wx.SingleChoiceDialog(None, _("Sélectionnez un champ à insérer :"), _("Insérer un champ"), listeLabels, wx.CHOICEDLG_STYLE)
        dlg.SetSize((580, 500))
        dlg.CenterOnScreen()
...
class DLG_Saisie_formule(wx.Dialog):
    def __init__(self, parent, listeChamps=[], formule="", titre=_("Saisie d'une formule")):
        ...
        self.label_formule = wx.StaticText(self, -1, _("Saisissez une formule :"))
        self.ctrl_formule = wx.TextCtrl(self, -1, formule, style=wx.TE_MULTILINE)
        self.hyper_formule = Hyperlien(self, label=_("Insérer un champ"), infobulle=_("Cliquez ici pour insérer un champ"), URL="")
    ...
    def __set_properties(self):
        self.ctrl_formule.SetToolTip(wx.ToolTip(_("Saisissez une formule")))
    ...
    def OnBoutonOk(self, event):
        if self.GetFormule() == "" :
            dlg = wx.MessageDialog(self, _("Vous n'avez saisi aucune formule !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
...
class CTRL_Conditions(wx.ListBox):
    ...
    def Modifier(self, event=None):
        ...
        if index == -1 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune condition à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event=None):
        ...
        index = self.GetSelection()
        if index == -1 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune condition à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer cette condition ?"), _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDactivite=None, IDunite=None):
        ...
        # Conditions
        self.staticbox_conditions_staticbox = wx.StaticBox(self, -1, _("Conditions d'application"))
        ...
        self.staticbox_parametres_staticbox = wx.StaticBox(self, -1, _("Paramètres de la consommation"))

        self.label_heure_debut = wx.StaticText(self, -1, _("Heure de début :"))
        self.radio_heure_debut_fixe = wx.RadioButton(self, -1, _("Fixe"), style=wx.RB_GROUP)
        self.ctrl_heure_debut = CTRL_Saisie_heure.Heure(self)
        self.radio_heure_debut_formule = wx.RadioButton(self, -1, _("Formule"))
        self.ctrl_heure_debut_formule = wx.TextCtrl(self, -1, "")
        ...
        self.label_heure_fin = wx.StaticText(self, -1, _("Heure de fin :"))
        self.radio_heure_fin_fixe = wx.RadioButton(self, -1, _("Fixe"), style=wx.RB_GROUP)
        self.ctrl_heure_fin = CTRL_Saisie_heure.Heure(self)
        self.radio_heure_fin_formule = wx.RadioButton(self, -1, _("Formule"))
        self.ctrl_heure_fin_formule = wx.TextCtrl(self, -1, "")
    ...
    def __set_properties(self):
        self.SetTitle(_("Paramètres de l'auto-génération"))
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour créer une nouvelle condition")))
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier la condition sélectionnée dans la liste")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer la condition sélectionnée dans la liste")))
        self.bouton_heure_debut_formule.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier la formule")))
        self.bouton_heure_fin_formule.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier la formule")))
        self.ctrl_etat.SetToolTip(wx.ToolTip(_("Sélectionnez un état")))
        self.ctrl_quantite.SetToolTip(wx.ToolTip(_("Saisir une quantité (1 par défaut)")))
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez ici pour obtenir de l'aide")))
        self.bouton_ok.SetToolTip(wx.ToolTip(_("Cliquez ici pour valider")))
        self.bouton_annuler.SetToolTip(wx.ToolTip(_("Cliquez ici pour annuler")))
    ...
    def GetChamps(self):
        ...
        listeCodes = [
            ("DUREE", _("Durée")),
            ("HEUREDEBUT", _("Heure de début")),
            ("HEUREFIN", _("Heure de fin")),
            ("QUANTITE", _("Quantité")),
            ("ETAT", _("Etat")),
        ]
        listeChamps = []
        for dictUnite in self.listeUnites :
            for code, label in listeCodes :
                ...
                nomChamp = "%s de l'unité '%s'" % (label, dictUnite["nom"])
