...
from Ol import OL_Categories_budgetaires
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDbudget=None):
        ...
        # Postes analytiques
        self.box_analytiques_staticbox = wx.StaticBox(self, wx.ID_ANY, _("Postes analytiques"))
        self.ctrl_analytiques = CTRL_Analytiques(self)

        # Catégories
        self.box_categories_staticbox = wx.StaticBox(self, wx.ID_ANY, _("Catégories budgétaires"))
...
##        self.ctrl_categories_credit = OL_Categories_budgetaires.ListView(self.notebook, id=-1, typeCategorie="credit", style=wx.LC_REPORT|wx.SUNKEN_BORDER|wx.LC_SINGLE_SEL|wx.LC_HRULES|wx.LC_VRULES)
        self.notebook.AddPage(self.ctrl_categories_debit, _("Débit"))
        self.notebook.AddPage(self.ctrl_categories_credit, _("Crédit"))
        ...
        self.bouton_supprimer_categories = wx.BitmapButton(self, -1, wx.Bitmap(Chemins.GetStaticPath(u"Images/16x16/Supprimer.png"), wx.BITMAP_TYPE_ANY))
        ...
        if self.IDbudget != None :
            self.Importation()
            titre = _("Modification d'un budget")
        else :
            titre = _("Saisie d'un budget")
        self.SetTitle(titre)
    ...
    def __set_properties(self):
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez un nom pour ce budget (Ex : Année 2015)")))
        self.ctrl_date_debut.SetToolTip(wx.ToolTip(_("Saisissez la date de début de la période à budgéter")))
        self.ctrl_date_fin.SetToolTip(wx.ToolTip(_("Saisissez la date de fin de la période à budgéter")))
        self.ctrl_observations.SetToolTip(wx.ToolTip(_("Saisissez des observations")))
        self.bouton_ajouter_categories.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter une catégorie budgétaire")))
        self.bouton_modifier_categories.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier la catégorie budgétaire sélectionnée")))
        self.bouton_supprimer_categories.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer la catégorie budgétaire sélectionnée")))
...
##        self.ctrl_exercice.SetID(IDexercice)
    ...
    def Sauvegarde(self):
        ...
        if nom == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un nom pour ce budget !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if date_debut == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de début de période pour ce budget !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if date_fin == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de fin de période pour ce budget !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if date_fin < date_debut :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de fin supérieure à la date de début de période !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_date_fin.SetFocus()
            return False
        ...
        if analytiques == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement cocher au moins un poste analytique !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
