...
class Page_Saisie_manuelle(wx.Panel):
    def __init__(self, parent):
        ...
        self.ctrl.SetToolTip(wx.ToolTip(_("Saisissez manuellement des adresses emails en les séparant par des points-virgules (;)")))
...
class CTRL_Listes_diffusion(wx.CheckListBox):
    def __init__(self, parent):
        ...
        self.data = []
        self.date = None
        self.SetToolTip(wx.ToolTip(_("Cochez les listes de diffusion souhaitées")))
        self.listeDiff = []
        self.dictDiff = {}
    ...
    def Importation(self):
        ...
        for IDliste, nom in listeListes :
            ...
            if IDliste in dictAbonnements :
                txtAbonnements = _("(%d abonnés)") % len(dictAbonnements[IDliste])
            else:
                txtAbonnements = _("(Aucun abonné)")
            label = "%s %s" % (nom, txtAbonnements)
...
class Page_Familles_individus(wx.Panel):
    ...
    def Validation(self):
        ...
        if len(listeNonValides) > 0 :
            ...
            texteIntro = _("Attention, les %d destinataires sélectionnés suivants n'ont pas d'adresse valide :") % len(listeNonValides)
            texteDetail = "\n".join(listeNonValides)
            dlg = dialogs.MultiMessageDialog(self, texteIntro, caption=_("Avertissement"), msg2=texteDetail, style = wx.ICON_EXCLAMATION | wx.OK | wx.CANCEL, icon=None, btnLabels={wx.ID_OK : _("Continuer"), wx.ID_CANCEL:_("Annuler")})
...
class Page_Carnet_adresses(wx.Panel):
    ...
    def Validation(self):
        ...
        if len(listeNonValides) > 0:
            ...
            texteIntro = _("Attention, les %d destinataires sélectionnés suivants n'ont pas d'adresse valide :") % len(listeNonValides)
            texteDetail = "\n".join(listeNonValides)
            dlg = dialogs.MultiMessageDialog(self, texteIntro, caption=_("Avertissement"), msg2=texteDetail,
                                             style=wx.ICON_EXCLAMATION | wx.OK | wx.CANCEL, icon=None,
                                             btnLabels={wx.ID_OK: _("Continuer"), wx.ID_CANCEL: _("Annuler")})
            reponse = dlg.ShowModal()
            dlg.Destroy()
            if reponse == wx.ID_CANCEL:
                return False
...
class CTRL_Pages(wx.Notebook):
    def __init__(self, parent):
        ...
        self.listePages = [
            ...
            {"code" : "listes_diffusion", "label" : _("Listes de diffusion"), "page" : Page_Listes_diffusion(self), "image" : wx.Bitmap(Chemins.GetStaticPath(u"Images/32x32/Questionnaire.png"), wx.BITMAP_TYPE_PNG)},
            {"code" : "carnet_adresses", "label" : _("Carnet d'adresses"), "page" : Page_Carnet_adresses(self), "image" : wx.Bitmap(Chemins.GetStaticPath(u"Images/32x32/Carnet.png"), wx.BITMAP_TYPE_PNG)},
    ...
    def SetInfos(self, code="", dictDonnees={}):
        ...
        if len(listeAdressesUniques) == 0 :
            ...
            texte = _("Vous avez sélectionné 1 destinataire valide. Cliquez sur OK pour valider la sélection.")
        else :
            texte = _("Vous avez sélectionné %d destinataires valides. Cliquez sur OK pour valider la sélection.") % len(listeAdressesUniques)
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        self.texte_intro = _("Sélectionnez des adresses Emails grâce aux contrôles ci-dessous...")
        self.ctrl_intro = wx.StaticText(self, -1, self.texte_intro)
        
        self.ctrl_pages = CTRL_Pages(self)

        self.bouton_aide = CTRL_Bouton_image.CTRL(self, texte=_("Aide"), cheminImage="Images/32x32/Aide.png")
        self.bouton_detail = wx.Button(self, -1, _("Afficher la liste des adresses valides"))
    ...
    def __set_properties(self):
        self.SetTitle(_("Sélection des destinataires"))
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez ici pour obtenir de l'aide")))
        self.bouton_detail.SetToolTip(wx.ToolTip(_("Cliquez ici pour afficher la liste des adresses valides")))
    ...
    def OnBoutonDetail(self, event):
        ...
        if len(listeAdressesUniques) == 0 :
            texte = _("Aucune adresse valide")
        else :
            texte = "\n".join(listeAdressesUniques)
        dlg = wx.lib.dialogs.ScrolledMessageDialog(self, texte, _("Liste des adresses valides"))
        dlg.ShowModal()
        dlg.Destroy()
    ...
    def OnBoutonOk(self, event): 
        ...
        if len(listeAdressesUniques) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune adresse valide.\n\nSouhaitez-vous tout de même valider ?"), _("Information"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
            reponse = dlg.ShowModal() 
            dlg.Destroy()
