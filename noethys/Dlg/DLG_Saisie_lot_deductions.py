...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        # Bandeau
        titre = _("Saisir un lot de déductions")
        intro = _("Vous pouvez saisir ici un lot de déductions pour les prestations sélectionnées. Commencez par définir les caractéristiques de la déduction puis sélectionnez les prestations concernées.")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Impayes.png")

        # Déduction
        self.box_deduction_staticbox = wx.StaticBox(self, -1, _("Paramètres de la déduction"))
        ...
        # Prestations
        self.box_prestations_staticbox = wx.StaticBox(self, -1, _("Sélection des prestations"))
    ...
    def __set_properties(self):
        self.ctrl_label.SetToolTip(wx.ToolTip(_("Saisissez le label de la déduction")))
        self.radio_montant_fixe.SetToolTip(wx.ToolTip(_("Saisie d'un montant pour chaque déduction")))
    ...
    def OnBoutonOk(self, event): 
        ...
        if len(label) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un label pour la déduction !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        dlg = wx.MultiChoiceDialog( self, _("Cochez les prestations auxquelles vous souhaitez appliquer les déductions :"), _("Sélection des prestations"), listeLabelsPrestations)
        ...
        dlg = wx.MessageDialog(self, _("Confirmez-vous la création automatique de %d déductions ?") % nbrePrestations, _("Demande de confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        reponse = dlg.ShowModal() 
        dlg.Destroy()
        ...
        for dictPrestation in listePrestations :
            ...
            if dictPrestation["label"] in listeSelectionPrestations :
                ...
                if nouveauMontantPrestation < 0.0 :
                    listeAnomalies.append(labelPrestation + _("La déduction est plus élevée que le montant de la prestation !"))
                    valide = False

                if nouveauMontantPrestation < montantVentilation :
                    listeAnomalies.append(labelPrestation + _("La ventilation est plus élevée que le nouveau montant de la prestation !"))
                    valide = False

                if dictPrestation["IDfacture"] != None :
                    listeAnomalies.append(labelPrestation + _("La prestation ne peut être modifiée car elle apparaît sur une facture !"))
                    valide = False
        ...
        if len(listeAnomalies) > 0 :
            dlg = DLG_Messagebox.Dialog(self, titre=_("Anomalies"), introduction=_("Les %d anomalies suivantes ont été détectées :") % len(listeAnomalies), detail=u"\n".join(listeAnomalies), conclusion=_("Souhaitez-vous continuer pour les %d déductions valides ?") % len(listeAjouts), icone=wx.ICON_EXCLAMATION, boutons=[_("Oui"), _("Non"), _("Annuler")])
            reponse = dlg.ShowModal() 
            dlg.Destroy() 
            if reponse in (1, 2) :
                return False
        ...
        # Confirmation
        dlg = wx.MessageDialog(self, _("%d déductions ont été créées avec succès.") % len(listeAjouts), _("Confirmation"), wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
