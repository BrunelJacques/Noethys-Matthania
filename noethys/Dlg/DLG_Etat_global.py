...
class Parametres(wx.Panel):
    ...
    def GetLabelParametres(self):
        # Label Paramètres
        listeParametres = [ 
            _("Période du %s au %s") % (UTILS_Dates.DateEngFr(str(self.ctrl_date_debut.GetDate())), UTILS_Dates.DateEngFr(str(self.ctrl_date_fin.GetDate()))),
            _("Activités : %s") % self.GetNomsActivites(),
            ]
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        titre = _("Etat global des consommations")
        ...
        self.staticbox_parametres_staticbox = wx.StaticBox(self, -1, _("Paramètres de calcul"))
        self.ctrl_parametres = CTRL_Etat_global_parametres.CTRL(self)

        # Options
        self.staticbox_filtres_staticbox = wx.StaticBox(self, -1, _("Options"))
    ...
    def __set_properties(self):
        self.bouton_ok.SetToolTip(wx.ToolTip(_("Cliquez ici pour créer un aperçu des résultats (PDF)")))
    ...
    def Apercu(self, event):
        ...
        if len(listeActivites) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez sélectionner au moins une activité !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if dict_options["afficher_regime_inconnu"] == True :
            ...
            if len(listeFamillesSansCaisses) > 0 :
                ...
                messageDetail = "\n".join(listeTemp)
                dlg = dialogs.MultiMessageDialog(self, _("Attention, le régime d'appartenance n'a pas été renseigné pour les %d familles suivantes :") % len(listeTemp), caption=_("Régime d'appartenance"), msg2=messageDetail, style = wx.ICON_EXCLAMATION | wx.OK | wx.CANCEL, btnLabels={wx.ID_OK : _("Continuer quand même"), wx.ID_CANCEL : _("Annuler")})
        ...
        for dictTemp in listeVacances :
            ...
            # Vacances
            if dictTemp["nom"] == _("Février") :
                nom = "vacances_fevrier"
            elif dictTemp["nom"] == _("Pâques") :
                nom = "vacances_paques"
            elif dictTemp["nom"] == _("Eté") :
                nom = "vacances_ete"
            elif dictTemp["nom"] == _("Toussaint") :
                nom = "vacances_toussaint"
            elif dictTemp["nom"] == _("Noël") :
                nom = "vacances_noel"
            else :
                nom = "?"
            dictTemp["code"] = nom + "_%d" % dictTemp["annee"]
            dictTemp["label"] = _("Vacances %s %d") % (dictTemp["nom"], dictTemp["annee"])
            ...
            if len(listeVacances) > index + 1 :
                ...
                if dictTemp["nom"].startswith("F") : 
                    ...
                    nom = "?"
                label = _("Hors vacances %s-%s %d") % (LISTE_MOIS[date_debut_temp.month-1], LISTE_MOIS[date_fin_temp.month-1], annee)
        ...
        if len(liste_regroupements) == 0 :
            dict_tranches_age = { 0 : {"label" : "", "min" : -1, "max" : -1} }
        else :
            dict_tranches_age = {}
            indexRegroupement = 0
            for regroupement in liste_regroupements :
                if indexRegroupement == 0 :
                    dict_tranches_age[indexRegroupement] = {"label" : _("Âge < %d ans") % regroupement, "min" : -1, "max" : regroupement}
                else :
                    dict_tranches_age[indexRegroupement] = {"label" : _("Âge >= %d et < %d ans") % (liste_regroupements[indexRegroupement-1], regroupement), "min" : liste_regroupements[indexRegroupement-1], "max" : regroupement}
                indexRegroupement += 1
                dict_tranches_age[indexRegroupement] = {"label" : _("Âge >= %d ans") % regroupement, "min" : regroupement, "max" : -1}
        ...
        for IDconso, date, IDindividu, IDunite, IDgroupe, IDactivite, etiquettes, heure_debut, heure_fin, etat, quantite, IDevenement, IDprestation, temps_facture, IDfamille, nomActivite, nomGroupe, nomCategorie, IDcaisse, IDregime, date_naiss in listeDonnees:
            ...
            if IDunite in dictUnites :
                ...
                nom_unite_conso = _("Unité inconnue")
            ...
            try:
                ...
                if regroupement_principal == "etiquette":
                    ...
                    if len(etiquettes) > 0:
                        ...
                        regroupement = _("- Aucune étiquette -")
            ...
            if periode == "" :
                texte = _("Période inconnue pour la date du %s. Vérifiez que les périodes de vacances ont bien été paramétrées.") % UTILS_Dates.DateDDEnFr(date)
                ...
                if dictCalcul["typeCalcul"] == 0 :
                    ...
                    if heure_debut != None and heure_debut != "" and heure_fin != None and heure_fin != "" :
                        ...
                        if "day" in str(valeur) :
                            dlg = wx.MessageDialog(self, _("Les horaires de cette consommation sont incorrectes : IDconso=%d | IDindividu=%d | IDfamille=%d | date=%s.") % (IDconso, IDindividu, IDfamille, date), _("Erreur"), wx.OK | wx.ICON_ERROR)
                            dlg.ShowModal()
                            dlg.Destroy()
                            return False
                    ...
                    try :
                        ...
                        dlg = wx.MessageDialog(self, six.text_type(err), _("Erreur de formule"), wx.OK | wx.ICON_ERROR)
                        dlg.ShowModal()
                        dlg.Destroy()
                        return False
        ...
        if len(listeAnomalies) > 0 :
            messageDetail = "\n".join(listeAnomalies)
            dlg = dialogs.MultiMessageDialog(self, _("Les %d anomalies suivantes ont été trouvées :") % len(listeAnomalies), caption=_("Anomalies"), msg2=messageDetail, style = wx.ICON_ERROR | wx.OK, btnLabels={wx.ID_OK : _("Ok")})
            dlg.ShowModal() 
            dlg.Destroy() 
            return
        ...
        dataTableau.append( (_("Etat global des consommations"), _("%s\nEdité le %s") % (UTILS_Organisateur.GetNom(), dateDuJour)) )
        style = TableStyle([
        ...
        txt_stats_globales = "%d individus | %d familles" % (len(dict_stats["individus"]), len(dict_stats["familles"]))
        ...
        for regroupement in regroupements :
            ...
            for IDregime in listeRegimesUtilises :
                if IDregime in dictRegimes :
                    ...
                    nomRegime = _("Sans régime")
                listeColonnes.append((IDregime, nomRegime, largeurColonne))
            listeColonnes.append((2000, _("Total"), largeurColonne))
            ...
            if regroupement_principal == "aucun" :
                ...
                label_regroupement = "%s (%s)" % (dictEvenements[regroupement]["nom"], UTILS_Dates.DateDDEnFr(dictEvenements[regroupement]["date"]))
            elif regroupement_principal.startswith("qf") and type(regroupement) == tuple :
                label_regroupement = "%d-%d" % regroupement
            ...
            if regroupement_principal != "aucun" and label_regroupement in ("None", "") :
                label_regroupement = _("- Non renseigné -")
            ...
            for index_tranche_age, dict_resultats_periode in dict_resultats_age.items() :
                ...
                if index_tranche_age in dict_tranches_age :
                    ...
                    label_tranche_age = _("Sans date de naissance")
                ...
                if modePeriodesDetail == False :
                    listePeriodes = [
                        {"code" : "petitesVacs", "label" : _("Petites vacances")},
                        {"code" : "grandesVacs", "label" : _("Vacances d'été")},
                        {"code" : "horsVacs", "label" : _("Hors vacances")},
                        ]
                else :
                ...
                for dictPeriode in listePeriodes :
                    if dictPeriode["code"] in dict_resultats_periode :
                        ...
                        if modePeriodesDetail == False :
                            ...
                            label = _("<para align='center'>%s<br/><font size=5>Du %s au %s</font></para>") % (dictPeriode["label"], UTILS_Dates.DateEngFr(str(date_debut_temp)), UTILS_Dates.DateEngFr(str(date_fin_temp)))
