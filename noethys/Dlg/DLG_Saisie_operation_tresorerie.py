...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDcompte_bancaire=None, typeOperation="credit", IDoperation=None):
        ...
        if self.IDoperation != None :
            self.Importation()
            titre = _("Modification d'une opération")
        else :
            titre = _("Saisie d'une opération")
        if self.typeOperation == "credit" : titre += _(" au crédit")
        if self.typeOperation == "debit" : titre += _(" au débit")
        self.SetTitle(titre)
    ...
    def __set_properties(self):
        self.ctrl_date.SetToolTip(wx.ToolTip(_("Saisissez la date de l'opération")))
        self.ctrl_libelle.SetToolTip(wx.ToolTip(_("Saisissez un libellé")))
        self.ctrl_tiers.SetToolTip(wx.ToolTip(_("Sélectionnez un tiers")))
        self.bouton_tiers.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à la gestion des tiers")))
        self.ctrl_mode.SetToolTip(wx.ToolTip(_("Sélectionnez un mode")))
        self.bouton_mode.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à la gestion des modes")))
        self.ctrl_num_cheque.SetToolTip(wx.ToolTip(_("Saisissez le numéro de chèque")))
        self.ctrl_observations.SetToolTip(wx.ToolTip(_("Saisissez des observations")))
        self.ctrl_num_piece.SetToolTip(wx.ToolTip(_("Saisissez le numéro de pièce")))
        self.bouton_ajouter_ventilation.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter une ventilation")))
        self.bouton_modifier_ventilation.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier la ventilation sélectionnée")))
        self.bouton_supprimer_ventilation.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer la ventilation sélectionnée")))
    ...
    def Sauvegarde(self):
        ...
        if date == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date valide !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_date.SetFocus()
            return False
        ...
        if libelle == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un libellé pour cette opération !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if IDtiers == None :
            dlg = wx.MessageDialog(self, _("Vous n'avez pas renseigné le tiers.\n\nSouhaitez-vous tout de même valider ?"), _("Avertissement"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
        ...
        if IDmode == None :
            dlg = wx.MessageDialog(self, _("Vous n'avez pas renseigné le mode.\n\nSouhaitez-vous tout de même valider ?"), _("Avertissement"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
            reponse = dlg.ShowModal() 
            dlg.Destroy()
            if reponse != wx.ID_YES :
        ...
        if self.ctrl_num_cheque.IsEnabled() :
            if num_piece == "" :
                dlg = wx.MessageDialog(self, _("Vous n'avez pas renseigné le numéro de chèque.\n\nSouhaitez-vous tout de même valider ?"), _("Avertissement"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
                reponse = dlg.ShowModal()
                dlg.Destroy()
                if reponse !=  wx.ID_YES :
                    return False
        ...
        if totalVentilation == 0.0 :
            dlg = wx.MessageDialog(self, _("Le montant de cette opération est de 0.00 €.\n\nSouhaitez-vous tout de même valider ?"), _("Avertissement"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
            reponse = dlg.ShowModal()
            dlg.Destroy()
            if reponse !=  wx.ID_YES :
                return False
