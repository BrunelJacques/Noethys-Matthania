...
class Dialog(wx.Dialog):
    ...
    def __set_properties(self):
        self.SetTitle(_("Générer un remboursement"))
        self.bouton_ok.SetToolTip(wx.ToolTip(_("Cliquez ici pour valider et fermer")))
        self.bouton_annuler.SetToolTip(wx.ToolTip(_("Cliquez ici pour annuler la saisie et fermer")))
    ...
    def OnKillFocusSeances(self, event):
        ...
        try:
            ...
            wx.MessageBox(_("Le nombre de séances doit être un entier"))
        event.Skip()
    ...
    def OnBoutonOk(self, event):
        if self.ctrl_remboursement.IsChecked():
            if (self.ctrl_motif.IsEmpty() or self.ctrl_total.IsEmpty() or self.ctrl_nb_seances.IsEmpty() or self.ctrl_prix_seance.IsEmpty()):
                dlg = wx.MessageDialog(self, _("Tous les champs doivent être remplis !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False

            montant = self.GetMontant()
            dlg = wx.MessageDialog(None, _("Confirmez-vous la génération d'un remboursement de %.2f %s ?") % (montant, SYMBOLE), _("Confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
