...
LISTE_ITEMS_TESTS = [
    ...
    {"code" : "menu_fichier", "label" : _("Fichier"), "items" : [
            {"code" : "nouveau_fichier", "label" : _("Créer un nouveau fichier\tCtrl+N"), "infobulle" : _("Créer un nouveau fichier"), "image" : "Images/16x16/Fichier_nouveau.png", "action" : None},
            {"code" : "ouvrir_fichier", "label" : _("Ouvrir un fichier\tCtrl+O"), "infobulle" : _("Ouvrir un fichier existant"), "image" : "Images/16x16/Fichier_ouvrir.png", "action" : None},
            {"code" : "fermer_fichier", "label" : _("Fermer le fichier\tCtrl+F"), "infobulle" : _("Fermer le fichier ouvert"), "image" : "Images/16x16/Fichier_fermer.png", "action" : None, "actif" : False},
            ],
    },
    # Paramétrage
    {"code" : "menu_parametrage", "label" : _("Paramétrage"), "items" : [
            {"code" : "preferences", "label" : _("Préférences"), "infobulle" : _("Préférences"), "image" : "Images/16x16/Mecanisme.png", "action" : None},
            {"code" : "enregistrement", "label" : _("Enregistrement"), "infobulle" : _("Enregistrement"), "image" : "Images/16x16/Cle.png", "action" : None},
        ],
    },
    ]
...
class CTRL_elements(wx.TreeCtrl):
    def __init__(self, parent): 
        ...
        # Données
        self.listeElements = [
            {"code" : "elements_speciaux", "label" : _("Eléments spéciaux"), "elements" : [
                {"code" : "|", "infobulle" : "<Séparateur>", "image" : "Images/16x16/Barreoutils_separator.png"},
                {"code" : "-", "infobulle" : "<Espace>", "image" : "Images/16x16/Barreoutils_spacer.png"},
                ]},
            {"code" : "elements_menus", "label" : _("Eléments du menu"), "elements" : 
                GetListeItemsMenu()
                },
            ]
...
class CTRL_Style(wx.Choice):
    def __init__(self, parent):
        ...
        self.parent = parent
        self.listeStyles = [
            {"code" : "textedroite", "label" : _("Icône + texte à droite")},
            {"code" : "textedessous", "label" : _("Icône + texte dessous")},
            {"code" : "texteseul", "label" : _("Texte uniquement")},
            {"code" : "imageseule", "label" : _("Icône uniquement")},            
            ]
        self.MAJ() 
        self.SetSelection(0)
...
class Dialog(wx.Dialog):
    def __init__(self, parent, texte=None):
        ...
        self.label_style = wx.StaticText(self, wx.ID_ANY, _("Style :"))
        self.ctrl_style = CTRL_Style(self)
        self.ctrl_observations = wx.TextCtrl(self, wx.ID_ANY, "", style=wx.TE_MULTILINE)
        
        # Elements
        self.box_elements_staticbox = wx.StaticBox(self, wx.ID_ANY, _("Eléments"))

        self.label_elements_dispo = wx.StaticText(self, wx.ID_ANY, _("Eléments disponibles"))
        ...
        self.label_elements_barre = wx.StaticText(self, wx.ID_ANY, _("Eléments de la barre"))
...
##        self.Bind(wx.EVT_BUTTON, self.OnBoutonModifier, self.bouton_modifier)
        ...
        if texte == None :
            self.SetTitle(_("Saisie d'une barre d'outils"))
            self.code = self.GetCodeUnique() 
        else :
            self.SetTitle(_("Modification d'une barre d'outils"))
            self.Importation(texte)

    def __set_properties(self):
        self.ctrl_nom.SetMinSize((300, 21))
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez un nom pour cette barre d'outils")))
        self.ctrl_style.SetToolTip(wx.ToolTip(_("Sélectionnez un style d'affichage pour cette barre d'outils")))
        self.ctrl_observations.SetToolTip(wx.ToolTip(_("Saisissez des observations (optionnel)")))
        ...
        self.bouton_droite.SetToolTip(wx.ToolTip(_("Cliquez ici pour insérer l'élément sélectionné dans la barre d'outils (Vous pouvez également double-cliquer dessus)")))
        self.bouton_gauche.SetToolTip(wx.ToolTip(_("Cliquez ici pour retirer l'élément sélectionné de la barre d'outils (Vous pouvez également double-cliquer dessus)")))
        self.bouton_monter.SetToolTip(wx.ToolTip(_("Cliquez ici pour déplacer cet élément")))
        self.bouton_descendre.SetToolTip(wx.ToolTip(_("Cliquez ici pour déplacer cet élément")))
##        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier cet élément")))
...
##        grid_sizer_boutons_elements.Add((10, 10), 0, wx.EXPAND, 0)
    ...
    def OnBoutonOk(self, event):  
        nom = self.ctrl_nom.GetValue() 
        if nom == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement renseigner le nom de la barre d'outils !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if len(listeElements) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez insérer au moins un élément dans cette barre d'outils !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
