...
class CTRL_Semaines(wx.Choice):
    def __init__(self, parent):
        ...
        self.parent = parent
        self.listeEtats = [
            (1, _("Toutes les semaines")),
            (2, _("Une semaine sur deux")),
            (3, _("Une semaine sur trois")),
            (4, _("Une semaine sur quatre")),
            (5, _("Les semaines paires")),
            (6, _("Les semaines impaires")),
        ]
        self.MAJ()
...
class CTRL_Jours(wx.Panel):
    def __init__(self, parent, periode="scolaire"):
        ...
        self.hyper_tout = Hyperlien(self, label=_("Tout"), infobulle=_("Cliquez ici pour tout cocher"), URL="tout")
        self.label_separation = wx.StaticText(self, -1, "|")
        self.hyper_rien = Hyperlien(self, label=_("Rien"), infobulle=_("Cliquez ici pour tout décocher"), URL="rien")
...
class Page_dates(wx.Panel):
    def __init__(self, parent, afficheElements=True, IDactivite=None):
        ...
        self.radio_date = wx.RadioButton(self, -1, _("Copier le schéma de la date suivante :"), style=wx.RB_GROUP)
        ...
        self.radio_calendrier = wx.RadioButton(self, -1, _("Copier le schéma suivant :"))
        self.ctrl_calendrier = Calendrier(self, IDactivite=IDactivite)
        self.ctrl_calendrier.SetMinSize((20, 20))

        self.radio_renitialisation = wx.RadioButton(self, -1, _("Réinitialiser la date"))
    ...
    def __set_properties(self):
        self.ctrl_date.SetToolTip(wx.ToolTip(_("Sélectionnez une date modèle. Les éléments de cette date seront copiés vers les dates cibles")))
        self.radio_date.SetToolTip(wx.ToolTip(_("Sélectionnez ce mode pour copier les éléments d'une date donnée")))
        self.radio_renitialisation.SetToolTip(wx.ToolTip(_("Sélectionnez ce mode pour supprimer les ouvertures, évènements et remplissages des dates")))
        self.check_ouvertures.SetToolTip(wx.ToolTip(_("Cochez cette case pour modifier les ouvertures")))
        self.check_places.SetToolTip(wx.ToolTip(_("Cochez cette case pour modifier les nbres de places max. (remplissage)")))
        self.check_evenements.SetToolTip(wx.ToolTip(_("Cochez cette case pour modifier les évènements (uniquement pour les unités de type évènementielles")))
    ...
    def Validation(self, dictDonnees={}):
        ...
        if self.radio_date.GetValue() == True:
            ...
            if date == None:
                dlg = wx.MessageDialog(self, _("Vous devez sélectionner une date modèle !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_date.SetFocus()
                return False
            ...
            if len(elements) == 0:
                dlg = wx.MessageDialog(self, _("Vous devez cocher au moins un élément à modifier (ouvertures/places) !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
...
class Page_evenements(wx.Panel):
    def __init__(self, parent, IDactivite=None, ctrl_calendrier=None):
        ...
        # Ajouter
        self.radio_ajouter = wx.RadioButton(self, -1, _("Ajouter les évènements suivants :"), style=wx.RB_GROUP)
        ...
        # Supprimer
        self.radio_supprimer_expression = wx.RadioButton(self, -1, _("Supprimer les évènements dont le nom contient l'expression suivante :"))
        self.ctrl_expression = wx.TextCtrl(self, -1, "")

        self.radio_supprimer_tout = wx.RadioButton(self, -1, _("Supprimer tous les évènements:"))
    ...
    def __set_properties(self):
        self.radio_ajouter.SetToolTip(wx.ToolTip(_("Sélectionnez ce mode pour ajouter un ou plusieurs évènements")))
        self.radio_supprimer_expression.SetToolTip(wx.ToolTip(_("Sélectionnez ce mode pour supprimer un ou plusieurs évènements")))
        self.radio_supprimer_tout.SetToolTip(wx.ToolTip(_("Sélectionnez ce mode pour supprimer tous les évènements")))
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier l'évènement sélectionné dans la liste")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer l'évènement sélectionné dans la liste")))
        self.ctrl_unite.SetToolTip(wx.ToolTip(_("Sélectionnez l'unité de consommations de type évènementiel qui sera associée aux évènements à créer")))
        self.ctrl_groupe.SetToolTip(wx.ToolTip(_("Sélectionnez le groupe qui sera associé aux évènements à créer")))
        self.ctrl_expression.SetToolTip(wx.ToolTip(_("Saisissez un mot ou une expression qui se trouve dans le ou les évènements à supprimer (Exemples : Patinoire, piscine...")))
    ...
    def Validation(self, dictDonnees={}):
        ...
        if self.radio_ajouter.GetValue() == True:
            ...
            if len(liste_evenements) == 0:
                dlg = wx.MessageDialog(self, _("Vous devez saisir au moins un évènement à créer !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
            ...
            if IDunite == None:
                dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner une unité de consommations à associer à ces évènements !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
            ...
            if IDgroupe == None:
                dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner un groupe à associer à ces évènements !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
        ...
        if self.radio_supprimer_expression.GetValue() == True or self.radio_supprimer_tout.GetValue() == True :
            ...
            if self.radio_supprimer_expression.GetValue() == True :
                ...
                if len(expression) == 0:
                    dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une expression valide !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                    dlg.ShowModal()
                    dlg.Destroy()
            ...
            if len(listeLabelsEvenements) == 0:
                dlg = wx.MessageDialog(self, _("Aucun évènement supprimable dont le nom contient cette expression n'a été trouvé !"), _("Suppression impossible"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False

            dlg = wx.MultiChoiceDialog(self, _("Voici la liste des évènements supprimables. Cochez ceux que vous souhaitez supprimer :"), _("Sélection des évènements"), listeLabelsEvenements)
...
class Notebook(wx.Notebook):
    def __init__(self, parent, afficheElements=True, IDactivite=None, ctrl_calendrier=None):
        ...
        self.listePages = [
            (_("dates"), _("Dates"), _("Page_dates(self, afficheElements=afficheElements, IDactivite=IDactivite)"), "Calendrier_jour.png"),
            (_("evenements"), _("Evènements"), _("Page_evenements(self, IDactivite=IDactivite, ctrl_calendrier=ctrl_calendrier)"), "Evenement.png"),
        ]
...
class Dialog(wx.Dialog):
    def __init__(self, parent, afficheElements=True, IDactivite=None, ctrl_calendrier=None):
        ...
        self.ctrl_feries = wx.CheckBox(self, -1, _("Inclure les fériés"))
        ...
        self.label_semaines = wx.StaticText(self, -1, _("Fréquence :"))
    ...
    def __set_properties(self):
        self.SetTitle(_("Saisie et suppression par lot"))
        self.ctrl_date_debut.SetToolTip(wx.ToolTip(_("Sélectionnez une date de début de période cible")))
        self.ctrl_date_fin.SetToolTip(wx.ToolTip(_("Sélectionnez une date de fin de période cible")))
        self.ctrl_semaines.SetToolTip(wx.ToolTip(_("Sélectionnez une fréquence")))
        self.ctrl_feries.SetToolTip(wx.ToolTip(_("Cochez cette case pour modifier également les jours fériés")))
    ...
    def OnBoutonOk(self, event):
        ...
        if date_debut == None :
            dlg = wx.MessageDialog(self, _("Vous devez saisir une date de début de période !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_date_debut.SetFocus()
            return False
        if date_fin == None :
            dlg = wx.MessageDialog(self, _("Vous devez saisir une date de fin de période !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_date_fin.SetFocus()
            return False
        if date_debut > date_fin :
            dlg = wx.MessageDialog(self, _("La date de début ne peut pas être supérieure à la date de fin !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_date_debut.SetFocus()
            return False
        ...
        if len(jours_scolaires) == 0 and len(jours_vacances) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez cocher au moins un jour scolaire ou de vacances !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
