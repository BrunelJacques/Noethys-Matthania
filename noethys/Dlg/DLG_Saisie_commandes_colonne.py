...
DICT_LABELS_CATEGORIES = {
    "numerique_avec_suggestion" : _("Numérique (Avec suggestion)"),
    "numerique_sans_suggestion" : _("Numérique (Libre)"),
    "numerique_total" : _("Numérique (Total)"),
    "texte_infos" : _("Texte (Informations)"),
    "texte_libre" : _("Texte (Libre)"),
}
...
class CTRL_Unites(HTL.HyperTreeList):
    def __init__(self, parent):
        ...
        self.AddColumn(_("Activité/Unité/Groupe"))
...
class PAGE_Unites(wx.Panel):
    def __init__(self, parent):
        ...
        # Contrôles
        self.label_intro = wx.StaticText(self, -1, _("Cochez les unités à additionner :"))
        self.ctrl_unites = CTRL_Unites(self)
        self.ctrl_unites.SetMinSize((100, 50))
        self.check_repas = wx.CheckBox(self, -1, _("Afficher uniquement les unités avec repas inclus"))
    ...
    def Validation(self):
        if len(self.ctrl_unites.GetCoches()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez cocher au moins une unité !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        return True
...
class PAGE_Informations(wx.Panel):
    def __init__(self, parent):
        ...
        # Contrôles
        self.label_intro = wx.StaticText(self, -1, _("Cochez les informations à afficher :"))
        self.check_infos_medicales = wx.CheckBox(self, -1, _("Afficher les informations médicales"))
        self.check_messages = wx.CheckBox(self, -1, _("Afficher les messages individuels"))

        self.label_groupes = wx.StaticText(self, -1, _("Cochez les groupes concernés :"))
        self.radio_groupes_tous = wx.RadioButton(self, -1, _("Tous les groupes"), style=wx.RB_GROUP)
        self.radio_groupes_choix = wx.RadioButton(self, -1, _("Uniquement les groupes suivants :"))
        self.ctrl_groupes = CTRL_Groupes(self)
        self.ctrl_groupes.SetMinSize((100, 50))

        self.check_infos_medicales.SetToolTip(wx.ToolTip(_("Affiche les informations médicales. Attention, la case 'Afficher sur la commande des repas' doit avoir été cochée dans la fenêtre de saisie de l'information médicale.")))
        self.check_messages.SetToolTip(wx.ToolTip(_("Affiche les messages individuels. Attention, la case 'Afficher sur la commande des repas' doit avoir été cochée dans la fenêtre de saisie du message.")))
    ...
    def Validation(self):
        if self.check_infos_medicales.GetValue() == False and self.check_messages.GetValue() == False :
            dlg = wx.MessageDialog(self, _("Vous devez cocher au moins un élément !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        if self.radio_groupes_choix.GetValue() == True :
            if len(self.ctrl_groupes.GetCoches()) == 0 :
                dlg = wx.MessageDialog(self, _("Vous devez cocher au moins un groupe !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        return True
...
class PAGE_Total(wx.Panel):
    def __init__(self, parent):
        ...
        # Contrôles
        self.label_intro = wx.StaticText(self, -1, _("Précisez les données à totaliser :"))
        self.radio_colonnes_toutes = wx.RadioButton(self, -1, _("Toutes les colonnes numériques"), style=wx.RB_GROUP)
        self.radio_colonnes_choix = wx.RadioButton(self, -1, _("Uniquement les colonnes suivantes :"))
...
class PAGE_Vide(wx.Panel):
    def __init__(self, parent):
        ...
        # Contrôles
        self.label_intro = wx.StaticText(self, -1, _("Aucun paramètre à renseigner."))
...
class Dialog(wx.Dialog):
    ...
    def __set_properties(self):
        ...
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez ici le nom de la colonne (Ex : 'Enfants', 'Adultes'...)")))
        self.ctrl_largeur.SetToolTip(wx.ToolTip(_("Saisissez ici la largeur de la colonne")))
    ...
    def OnBoutonOk(self, event):
        ...
        if nom == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un nom pour cette colonne !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_nom.SetFocus()
            return False
