...
class Page_Message(wx.Panel):
    ...
    def __set_properties(self):
        self.ctrl_objet.SetToolTip(wx.ToolTip(_("Saisissez l'objet du message. Il s'agit d'une donnée interne qui permet la tracabilité du message envoyé.")))
        self.ctrl_message.SetToolTip(wx.ToolTip(_("Saisissez le texte du message")))
    ...
    def OnTexte(self, event=None):
        ...
        texte = _("%d / %d caractères") % (len(self.ctrl_message.GetValue()), nbre_caracteres_max)
    ...
    def Validation(self):
        if len(self.ctrl_objet.GetValue()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir l'objet du message !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if len(texte) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir le texte du message !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if len(texte) > nbre_caracteres_max :
            dlg = wx.MessageDialog(self, _("Vous avez dépassé le nombre de caractères autorisé !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False

        alphabet_gsm = " 0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZÄäàÅåÆæßÇèéÉìÖöòÑñÜüù#%&()*+,-./:;<>=§$!?£¿¡@_'"
        for caractere in texte :
            if caractere not in alphabet_gsm and caractere != "\n" :
                dlg = wx.MessageDialog(self, _("Le caractère '%s' n'est pas pris en charge dans les SMS !") % caractere, _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
...
class Page_Saisie_manuelle(wx.Panel):
    def __init__(self, parent):
        ...
        self.ctrl.SetToolTip(wx.ToolTip(_("Saisissez manuellement des numéros de téléphones en les séparant par des points-virgules (;)")))
...
class CTRL_Destinataires(wx.Notebook):
    def __init__(self, parent):
        ...
        self.listePages = [
            {"code": "familles", "label": _("Familles"), "page": Page_Familles_individus(self, "familles"), "image": wx.Bitmap(Chemins.GetStaticPath(u"Images/32x32/Famille.png"), wx.BITMAP_TYPE_PNG)},
            {"code": "individus", "label": _("Individus"), "page": Page_Familles_individus(self, "individus"), "image": wx.Bitmap(Chemins.GetStaticPath(u"Images/32x32/Personnes.png"), wx.BITMAP_TYPE_PNG)},
            {"code": "saisie_manuelle", "label": _("Saisie manuelle"), "page": Page_Saisie_manuelle(self), "image": wx.Bitmap(Chemins.GetStaticPath(u"Images/32x32/Contrat.png"), wx.BITMAP_TYPE_PNG)},
        ]
...
class Page_Destinataires(wx.Panel):
    ...
    def __set_properties(self):
        self.check_avec_sms.SetToolTip(wx.ToolTip(_("Cochez cette option pour afficher uniquemement les numéros de téléphones dont l'option SMS a été activée dans la fiche individuelle")))
    ...
    def Validation(self):
        ...
        if len(listeTelephones) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement cocher au moins un numéro de téléphone !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        return True
...
class CTRL_Parametres(CTRL_Propertygrid.CTRL):
    ...
    def Remplissage(self):
        # Plateforme
        self.Append(wxpg.PropertyCategory(_("Paramètres")))

        liste_choix = [
            ("contact_everyone", _("Contact Everyone By Orange Business")),
            ("cleversms", _("Clever SMS")),
            ("clevermultimedias", _("Clever Multimedias")),
            ("mailjet", _("Mailjet")),
            ]

        propriete = CTRL_Propertygrid.Propriete_choix(label=_("Plateforme"), name="plateforme", liste_choix=liste_choix, valeur=None)
        propriete.SetEditor("EditeurChoix")
        propriete.SetHelpString(_("Sélectionnez une plateforme"))
        ...
        DB.Close()
        liste_adresses.insert(0, (0, _("Aucune")))
        propriete = CTRL_Propertygrid.Propriete_choix(label=_("Adresse d'expédition d'email"), name="adresse_expedition_email", liste_choix=liste_adresses, valeur=0)
        propriete.SetEditor("EditeurChoix")
        propriete.SetHelpString(_("Sélectionnez l'adresse d'expédition de l'email. Cette adresse doit être référencée sur votre compte Contact Everyone."))
        propriete.SetAttribute("obligatoire", True)
        self.Append(propriete)

        # Adresse de destination
        propriete = wxpg.StringProperty(label=_("Adresse de destination"), name="orange_adresse_destination_email", value="diffusion@contact-everyone.fr")
        propriete.SetHelpString(_("Saisissez l'adresse de destination de l'email"))
        self.Append(propriete)

        # Adresse de destination
        propriete = wxpg.StringProperty(label=_("Adresse de destination"), name="cleversms_adresse_destination_email", value="cleversmslight@cleversaas.fr")
        propriete.SetHelpString(_("Saisissez l'adresse de destination de l'email"))
        self.Append(propriete)

        # Adresse de destination
        propriete = wxpg.StringProperty(label=_("Adresse de destination"), name="clevermultimedias_adresse_destination_email", value="multimediasattachedfile@cleversaas.fr")
        propriete.SetHelpString(_("Saisissez l'adresse de destination de l'email"))
        self.Append(propriete)

        # Token Mailjet
        propriete = wxpg.StringProperty(label=_("Token Mailjet"), name="token_sms_mailjet", value="")
        propriete.SetHelpString(_("Saisissez le token que vous avez généré sur votre compte Mailjet"))
        self.Append(propriete)

        # Sender ID Mailjet
        propriete = wxpg.StringProperty(label=_("Nom de l'expéditeur"), name="sender_sms_mailjet", value="")
        propriete.SetHelpString(_("Saisissez le nom de l'expéditeur. Exemples : 'MJC', 'ALSH', 'MAIRIE'..."))
        self.Append(propriete)

        # Nbre caractères max
        propriete = wxpg.IntProperty(label=_("Nombre maximal de caractères du message"), name="nbre_caracteres_max", value=160)
        propriete.SetHelpString(_("Nombre maximal de caractères du message"))
        propriete.SetAttribute("obligatoire", True)
        self.Append(propriete)
    ...
    def Validation(self):
        ...
        #     dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner une adresse d'expédition !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
        #     dlg.ShowModal()
        #     dlg.Destroy()
        #     return False
...
class Base(object) :
    ...
    def InitPages(self, parent=None):
        ...
        self.listePages = [
            {"code" : "parametres", "label" : _("Paramètres"), "ctrl" : Page_Parametres(parent, clsbase=self), "image" : "Maison.png"},
            {"code" : "destinataires", "label": _("Destinataires"), "ctrl": Page_Destinataires(parent, clsbase=self), "image": "Maison.png"},
            {"code" : "message", "label": _("Message"), "ctrl": Page_Message(parent, clsbase=self), "image": "Maison.png"},
        ]
...
class Dialog(wx.Dialog, Base):
    def __init__(self, parent):
        ...
        titre = _("Envoi de SMS")
        intro = _("Vous pouvez envoyer ici des SMS aux individus ou familles à condition que votre plateforme d'envoi ait été intégrée à cette fonctionnalité. Consultez la liste des prestataires pris en charge dans la liste déroulante Plateforme de la page des paramètres.")
        self.SetTitle(titre)
    ...
    def Onbouton_suite(self, event):
        ...
        if self.pageVisible == self.nbrePages-1 :
            self.bouton_suite.SetImage(Chemins.GetStaticPath("Images/32x32/Valider.png"))
            self.bouton_suite.SetTexte(_("Envoyer"))
    ...
    def Envoyer(self):
        ...
        dlg = wx.MessageDialog(self, _("Confirmez-vous l'envoi du message vers %d numéros ?") % len(self.dictDonnees["liste_telephones"]), _("Confirmation"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        ...
        if self.dictDonnees["plateforme"] == "contact_everyone" :
            ...
            message = UTILS_Envoi_email.Message(destinataires=[self.dictDonnees["orange_adresse_destination_email"],],
                                                sujet=self.dictDonnees["objet"], texte_html=_("Envoi de SMS"), fichiers=[cheminFichier,])
        ...
        if self.dictDonnees["plateforme"] == "mailjet":
            ...
            for numero in self.dictDonnees["liste_telephones"]:
                ...
                if reponse.ok:
                    ...
                    texte_erreur = "Code erreur : %s. Erreur : %s" % (dict_erreur["ErrorCode"], dict_erreur["ErrorMessage"])
                    dlg = DLG_Messagebox.Dialog(self, titre=_("Envoi de SMS"), introduction=_("L'envoi du SMS vers le numéro %s a rencontré une erreur :") % numero,
                                                detail=texte_erreur, conclusion=_("Que souhaitez-vous faire ?"),
                                                icone=wx.ICON_ERROR, boutons=[_("Continuer l'envoi"), _("Arrêter")])
                    reponse = dlg.ShowModal()
                    dlg.Destroy()
                    if reponse == 1:
                        return False
        ...
        dlg = wx.MessageDialog(self, _("Envoi des SMS terminé."), _("Confirmation"), wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
