...
DICT_RENSEIGNEMENTS = {"nom" : "Nom", "prenom" : "Prénom", "date_naiss" : "Date de naissance", "cp_naiss" : "CP de naissance", "ville_naiss" : "Ville de naissance", "rue_resid" : "Adresse - Rue", "cp_resid" : "Adresse - CP", "ville_resid" : "Adresse - Ville",
                    "tel_domicile" : "Tél. Domicile", "tel_mobile" : "Tél. Mobile", "mail" : "Email", "profession" : "Profession", "employeur" : "Employeur", "travail_tel" : "Tél. Pro.", "travail_mail" : "Email Pro."}
...
class Dialog(wx.Dialog):
    def __init__(self, parent, track=None, tracks=[]):
        ...
        self.label_action = wx.StaticText(self.panel_bandeau, -1, _("Réservation de dates"))
        ...
        self.radio_validation = wx.RadioButton(self, -1, _("Traité le"))
        ...
        self.label_reponse = wx.StaticText(self, -1, _("Réponse :"))
    ...
    def __set_properties(self):
        ...
        self.bouton_premier.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à la première demande de la liste")))
        self.bouton_precedent.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à la demande précédente dans la liste")))
        self.bouton_suivant.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à la demande suivante dans la liste")))
        self.bouton_dernier.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à la dernière demande de la liste")))
        ...
        self.radio_validation.SetToolTip(wx.ToolTip(_("Demande traitée")))
        self.ctrl_date_validation.SetToolTip(wx.ToolTip(_("Date de traitement de la demande")))
        self.ctrl_reponse.SetToolTip(wx.ToolTip(_("Cette réponse apparaîtra sur le portail et dans l'email de confirmation (si vous utilisez le mot-clé {DEMANDE_REPONSE} dans l'email). Cette réponse est générée automatiquement par Noethys mais vous pouvez la modifier librement.")))
        self.bouton_envoyer.SetToolTip(wx.ToolTip(_("Cliquez ici pour envoyer directement un email de réponse à la famille sans passer par l'éditeur d'Emails.")))
        self.bouton_editeur.SetToolTip(wx.ToolTip(_("Cliquez ici pour ouvrir l'éditeur d'Email afin d'envoyer un email de réponse à la famille")))
        self.ctrl_modele_email.SetToolTip(wx.ToolTip(_("Sélectionnez un modèle d'email")))
        self.bouton_gestion_modeles.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à la gestion des modèles d'emails")))
        ...
        self.bouton_fermer.SetToolTip(wx.ToolTip(_("Cliquez ici pour fermer la fenêtre")))
            ...
            for champ, valeur in listeDonnees :
                ...
                if champ == "adresse_auto" and valeur != None:
                    ...
                    if len(listeIndividus) > 0 :
                        ...
                        prenom = "?"
                    label = _("<li>Adresse associée à celle de %s</li>") % prenom
    ...
    def MAJ_email_date(self):
        if self.track.email_date != None :
            ...
            self.label_email_reponse.SetLabel(_("Email de réponse envoyé le %s") % UTILS_Dates.DateDDEnFr(self.track.email_date))
        else :
    ...
    def MAJ_informations(self):
        ...
        if self.track.categorie == "reservations":
            ...
            texte = _("Total pour la période : %.2f %s | Réglé : %.2f %s | Solde à régler : %s") % (montants["total"], SYMBOLE, montants["regle"], SYMBOLE, texte_solde)
        ...
        if self.track.categorie == "reglements" and self.track.action == "paiement_en_ligne":
            ...
            texte = _("En règlement de : %s") % ", ".join(liste_textes)
    ...
    def Envoyer(self, visible=True):
        ...
        if self.ctrl_reponse.GetValue() == "" :
            dlg = wx.MessageDialog(self, _("Vous n'avez saisi aucune réponse.\n\nSouhaitez-vous tout de même envoyer l'email ?"), _("Avertissement"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        ...
        if IDmodele == None :
            dlg = wx.MessageDialog(self, _("Vous devez sélectionner un modèle d'Email dans la liste proposée !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
...
class Traitement():
    ...
    def Traitement_recus(self):
        ...
        # Traitement manuel
        if self.mode == "manuel" :
            self.EcritLog(_("Ouverture de la fenêtre d'édition d'un reçu."))
            if self.dict_parametres["methode_envoi"] == "email" :
                self.EcritLog(_("Veuillez envoyer ce reçu de règlement par Email."))
                reponse = _("Reçu de règlement envoyé par Email.")
            elif self.dict_parametres["methode_envoi"] == "courrier" :
                self.EcritLog(_("Veuillez imprimer le reçu de règlement pour un envoi par courrier."))
                reponse = _("Reçu de règlement envoyé par courrier.")
            else :
                self.EcritLog(_("Veuillez imprimer le reçu de règlement pour un retrait sur site."))
                reponse = _("Reçu de règlement disponible au retrait.")
        ...
        if self.mode == "automatique" :
            ...
            if self.dict_parametres["methode_envoi"] != "email" :
                message = _("Le reçu de règlement va être généré au format PDF et ouvert dans votre lecteur de PDF.\n\n")
                ...
                dlg = wx.MessageDialog(self.parent, message, _("Impression d'un reçu"), wx.OK|wx.OK_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
                reponse = dlg.ShowModal()
                dlg.Destroy()
                ...
                if dictChamps == False :
                    dlg_impression.Destroy()
                    self.EcritLog(_("[ERREUR] La génération du reçu au format PDF a rencontré une erreur."))
                    return False

                self.EcritLog(_("La génération du reçu est terminée."))
                if self.dict_parametres["methode_envoi"] == "courrier" :
                    self.EcritLog(_("Veuillez imprimer le reçu de règlement pour un envoi par courrier."))
                    reponse = _("Reçu de règlement envoyé par courrier.")
                else :
    ...
    def Traitement_paiement_en_ligne(self):
        ...
        systeme_paiement = self.dict_parametres.get("systeme_paiement", "Système inconnu")
        ...
        if len(liste_paiements) > 1:
            dlg = wx.MessageDialog(None, _("Il s'agit d'un paiement en %d fois. Noethys va donc vous demander de saisir successivement %d règlements.") % (len(liste_paiements), len(liste_paiements)), _("Information"), wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
    ...
    def Traitement_factures(self):
        ...
        if self.mode == "manuel" :
            if self.dict_parametres["methode_envoi"] == "email" :
                ...
                if resultat == False :
                    self.EcritLog(_("La facture n'a pas été envoyée par Email."))
                    return False

                reponse = _("Facture envoyée par Email.")

            elif self.dict_parametres["methode_envoi"] == "courrier" :
                self.EcritLog(_("Veuillez imprimer la facture pour un envoi par courrier."))
                reponse = _("Facture envoyée par courrier.")
                edition.Reedition()
            else :
        ...
        if self.mode == "automatique" :
            ...
            # Affichage du PDF pour envoi par courrier ou retrait sur site
            if self.dict_parametres["methode_envoi"] != "email" :
                message = _("La facture va être générée au format PDF et ouverte dans votre lecteur de PDF.\n\n")
                if self.dict_parametres["methode_envoi"] == "courrier" :
                    message += _("Veuillez l'imprimer et l'envoyer par courrier.")
                else :
                    message += _("Veuillez l'imprimer et le conserver pour un retrait sur site.")
                ...
                self.EcritLog(_("La génération de la facture est terminée."))
                if self.dict_parametres["methode_envoi"] == "courrier" :
                    self.EcritLog(_("Veuillez imprimer la facture pour un envoi par courrier."))
                    reponse = _("Facture envoyée par courrier.")
                else :
            ...
            if self.dict_parametres["methode_envoi"] == "email" :
                ...
                #     self.EcritLog(_("Aucune adresse Email n'a été sélectionnée."))
                #     return False
        ...
        if self.mode == "manuel" or self.mode == "automatique" :
            ...
            if len(listeDonnees) > 0 :
                ...
                if date_naiss != None :
                    ...
                    age = _("Âge inconnu")
            intro = _("Confirmez l'inscription de %s (%s) à l'activité sélectionnée et sur le groupe demandé par la famille." % (prenom, age))
            ...
            dlg.Destroy()
            if reponse == wx.ID_OK :
                self.EcritLog(_("Inscription de %s enregistrée.") % prenom)
                if statut == "attente" :
                    reponse = _("Inscription de %s mise en attente.") % prenom
                elif statut == "refus" :
                    reponse = _("Inscription de %s refusée.") % prenom
                else :
                    reponse = _("Inscription de %s validée.") % prenom
                return {"etat" : True, "reponse" : reponse}
            else :
                self.EcritLog(_("Inscription de %s annulée.") % prenom)
                return False
    ...
    def Traitement_reservations(self):
        if self.mode == "manuel" :
            ...
            if reponse_modal == wx.ID_OK :
                ...
                self.EcritLog(_("Traitement annulé par l'utilisateur"))
                return {"etat" : False}
    ...
    def Traitement_pieces(self):
        ...
        titre_piece = self.track.description.replace(u"Envoi de la pièce ", "")

        # Téléchargement du fichier
        from Utils import UTILS_Portail_synchro
        dlgAttente = wx.BusyInfo(_("Téléchargement de la pièce en cours..."), None)
        ...
        if self.mode == "manuel":
            if dlg.ShowModal() == wx.ID_OK:
                ...
                dlg.Destroy()
                self.EcritLog(_("Enregistrement manuel de la pièce ID%d") % IDpiece)
                return {"etat": True, "reponse": _("La pièce %s a bien été enregistrée") % titre_piece}

        if self.mode == "automatique":
            if dlg.Sauvegarde() == True:
                IDpiece = dlg.GetIDpiece()
                dlg.Destroy()
                self.EcritLog(_("Enregistrement automatique de la pièce ID%d") % IDpiece)
                return {"etat": True, "reponse": _("La pièce %s a bien été enregistrée") % titre_piece}
            else:
                if dlg.ShowModal() == wx.ID_OK:
                    IDpiece = dlg.GetIDpiece()
    ...
    def Traitement_compte(self):
        ...
        if self.mode == "manuel" or self.mode == "automatique" :
            ...
            if self.mode == "manuel" :
                dlg = wx.MessageDialog(None, _("Confirmez-vous l'enregistrement du nouveau mot de passe personnalisé de %s ?") % self.track.nom, _("Mise à jour du mot de passe"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
                reponse = dlg.ShowModal()
                dlg.Destroy()
                if reponse != wx.ID_YES :
                    return False
            ...
            if self.track.action == "maj_password" :
                ...
                self.EcritLog(_("Mise à jour du mot de passe de %s") % self.track.nom)
    ...
    def Appliquer_reservations(self, ctrl_grille=None, log_jumeau=None):
        ...
        self.EcritLog(_("Traitement des réservations de %s sur la période du %s au %s") % (ctrl_grille.ctrl_titre.GetNom(), UTILS_Dates.DateDDEnFr(date_debut_periode), UTILS_Dates.DateDDEnFr(date_fin_periode)), log_jumeau)
        ...
        for reservation in listeReservations :
            ...
            if reservation["etat"] == 0 :
                ...
                for IDunite in liste_unites_conso :
                    ...
                    self.EcritLog(_("Suppression de l'unité %s du %s") % (nomUnite, UTILS_Dates.DateDDEnFr(date)), log_jumeau)
                ...
                for IDunite in liste_unites_conso :
                    if ctrl_grille.IsOuvert(IDunite=IDunite, date=date) :
                        ...
                        if IDunite in ctrl_grille.grille.dictUnites:
                            nomUnite = ctrl_grille.grille.dictUnites[IDunite]["nom"]
                            texte = _("Saisie de l'unité %s du %s en mode %s") % (nomUnite, UTILS_Dates.DateDDEnFr(date), mode_label)
                            self.EcritLog(texte, log_jumeau)
                            resultat = ctrl_grille.SaisieConso(IDunite=IDunite, date=date, mode=mode)
                        else :
                            resultat = _("L'unité ID%d est inconnue. Vérifiez le paramétrage des unités de réservation." % IDunite)
        ...
        if len(liste_erreurs) > 0 :
            ...
            if len(liste_erreurs) == 1 :
                introduction = _("Une anomalie a été détectée durant l'application de la demande :")
            else :
                introduction = _("%s anomalies ont été détectées durant l'application de la demande :") % len(liste_erreurs)
            detail = "\n".join(liste_erreurs)
            ...
            if nbre_suppressions == 1 :
                reponse_temp.append(_("1 suppression effectuée"))
            if nbre_suppressions > 1 :
                reponse_temp.append(_("%d suppressions effectuées") % nbre_suppressions)

            nbre_reservations = dict_reponses["reservation"]
            if nbre_reservations == 1 :
                reponse_temp.append(_("1 réservation validée"))
            if nbre_reservations > 1 :
                reponse_temp.append(_("%d réservations validées") % nbre_reservations)

            nbre_attentes = dict_reponses["attente"]
            if nbre_attentes == 1 :
                reponse_temp.append(_("1 réservation en attente"))
            if nbre_attentes > 1 :
                reponse_temp.append(_("%d réservations en attente") % nbre_attentes)
            ...
            self.EcritLog(_("Réponse : %s") % reponse, log_jumeau)
            return reponse
    ...
    def Verifier_ventilation(self):
        ...
        if len(tracks) > 0 :
            dlg = wx.MessageDialog(None, _("Un ou plusieurs règlements peuvent être ventilés.\n\nSouhaitez-vous le faire maintenant (conseillé) ?"), _("Ventilation"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
            reponse = dlg.ShowModal()
            dlg.Destroy()
            if reponse == wx.ID_YES :
