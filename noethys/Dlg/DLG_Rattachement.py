...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDfamille=None):
        ...
        intro = _("Commencez par sélectionner une catégorie de rattachement puis saisissez son nom et son prénom. Si l'individu apparait dans la liste, sélectionnez-le. Sinon créez une nouvelle fiche individuelle.")
        ...
        txtRemarque = """
        <IMG SRC="%s/16x16/Attention2.png">
        <FONT SIZE=-1>
    ...
    def __set_properties(self):
        ...
        self.bouton_categorie_1.SetToolTip(wx.ToolTip(_("Représentants")))
        self.bouton_categorie_2.SetToolTip(wx.ToolTip(_("Enfants")))
        self.bouton_categorie_3.SetToolTip(wx.ToolTip(_("Contacts")))
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez ici le nom de l'individu à rattacher (en majuscules et sans accents)")))
        self.ctrl_prenom.SetToolTip(wx.ToolTip(_("Saisissez ici le prénom de l'individu à rattacher")))
        self.ctrl_propositions.SetToolTip(wx.ToolTip(_("Double-cliquez sur le nom de l'individu à rattacher")))
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez ici pour obtenir de l'aide")))
        self.bouton_ok.SetToolTip(wx.ToolTip(_("Cliquez ici pour rattacher l'individu selectionné dans la liste")))
        self.bouton_annuler.SetToolTip(wx.ToolTip(_("Cliquez ici pour annuler et fermer")))
        self.ctrl_titulaire.SetToolTip(wx.ToolTip(_("Cochez cette case si l'individu doit être considéré comme titulaire du dossier")))
    ...
    def CreationIDindividu(self):
        nom = self.ctrl_nom.GetValue()
        if nom  == "" :
            dlg = wx.MessageDialog(self, _("Vous devez saisir le nom du nouvel individu à créer !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_nom.SetFocus()
            return
        ...
        if self.ctrl_propositions.donnees != None :
            for individu in self.ctrl_propositions.donnees :
                if Formate(individu.nom) == Formate(nom) and Formate(individu.prenom) == Formate(prenom) :
                    dlg = wx.MessageDialog(self, _("Un individu portant ce nom existe déjà dans la liste ! \n\nSi vous souhaitez quand même créer un nouvel individu avec ce nom, cliquez sur OUI."), _("Confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
                    reponse = dlg.ShowModal()
                    dlg.Destroy()
                    if reponse !=  wx.ID_YES :
                        return
    ...
    def OnBoutonOk(self, event):
        ...
        if self.IDfamille != None :
            ...
            if len(listeRattachements) > 0 :
                dlg = wx.MessageDialog(self, _("Rattachement Impossible : Cet individu est déjà rattaché à cette famille !"), _("Erreur de rattachement"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
