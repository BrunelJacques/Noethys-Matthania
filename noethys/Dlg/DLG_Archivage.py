...
class Page(wx.Panel):
    def __init__(self, parent, mode="familles"):
        ...
        if self.mode == "familles" :
            texte1 = _("Toutes les familles")
            texte2 = _("Les familles inactives depuis plus de")
            texte3 = _("Les familles actives depuis")
        else :
            texte1 = _("Tous les individus")
            texte2 = _("Les individus inactifs depuis plus de")
            texte3 = _("Les individus actifs depuis")
        ...
        # Properties
        self.bouton_ouvrir_fiche.SetToolTip(wx.ToolTip(_("Cliquez ici pour ouvrir la fiche famille correspondante")))
...
class CTRL_Notebook(wx.Notebook):
    def __init__(self, parent):
        ...
        self.listePages = [
            {"code": "familles", "ctrl": Page(self, mode="familles"), "label": _("Familles"), "image": "Famille.png"},
            {"code": "individus", "ctrl": Page(self, mode="individus"), "label": _("Individus"), "image": "Personnes.png"},
        ]
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        intro = _("Sélectionnez le mode Familles ou Individus, cochez les lignes à traiter, puis cliquez sur le bouton Action souhaité. L'archivage permet de cacher les familles ou individus de certaines listes. L'effacement est une action non réversible qui supprime toutes les données personnelles.")
        titre = _("Archiver et effacer")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Archiver.png")

        # Liste des données
        self.staticbox_donnees = wx.StaticBox(self, -1, _("Liste des données"))
        self.ctrl_notebook = CTRL_Notebook(self)

        # Boutons actions
        self.staticbox_actions = wx.StaticBox(self, -1, _("Actions sur les lignes cochées"))
        self.bouton_archiver = CTRL_Bouton_image.CTRL(self, texte=_("Archiver"), cheminImage="Images/32x32/Archiver.png")
        self.bouton_desarchiver = CTRL_Bouton_image.CTRL(self, texte=_("Désarchiver"), cheminImage="Images/32x32/Desarchiver.png")
        self.bouton_effacer = CTRL_Bouton_image.CTRL(self, texte=_("Effacer"), cheminImage="Images/32x32/Gomme.png")
    ...
    def __set_properties(self):
        ...
        self.bouton_archiver.SetToolTip(wx.ToolTip(_("Cliquez ici pour archiver les lignes cochées. L'archivage permet de cacher des individus ou des familles dans certaines listes du logiciel.")))
        self.bouton_desarchiver.SetToolTip(wx.ToolTip(_("Cliquez ici pour désarchiver les lignes cochées")))
        self.bouton_effacer.SetToolTip(wx.ToolTip(_("Cliquez ici pour effacer définitivement les lignes cochées. L'effacement est une action irréversible qui supprime les données personnelles (anonymisation) mais sans affecter certaines données telles que la facturation ou les consommations.")))
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez ici pour obtenir de l'aide")))
        self.bouton_fermer.SetToolTip(wx.ToolTip(_("Cliquez ici pour fermer")))
