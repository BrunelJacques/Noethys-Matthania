...
class CTRL_Medecin(html.HtmlWindow):
    def __init__(self, parent, IDindividu=None, hauteur=39):
        ...
        self.SetToolTip(wx.ToolTip(_("Cliquez sur le bouton Modifier pour sélectionner un médecin")))
...
##        self.SetBackgroundColour("#F0FBED" )
    ...
    def SetIDmedecin(self, IDmedecin=None):
        ...
        if len(listeMedecins) > 0 :
            ...
            tel = listeMedecins[0][6]
            if tel == None : tel = ""
            texteMedecin = _("Dr %s %s<BR>%s") % (nom, prenom, tel)
    ...
    def SetTexteMedecin(self, texteMedecin=""):
        ...
        if texteMedecin == None or texteMedecin == "" :
            # Si pas de médecin
            texte = """<CENTER><IMG SRC="%s/Special/Medecin_gris.png"></CENTER>"""%chemin
        else:
            # Si médecin sélectionné
            texte = """
            <CENTER>
            <IMG SRC="%s/48x48/Medecin-avecbords.png"><BR>
            <FONT SIZE=-2>
            %s
            </FONT> 
            </CENTER>
...
##        self.SetEtatBoutonDetacherMedecin()
    ...
    def __set_properties(self):
        self.ctrl_maladies.SetToolTip(wx.ToolTip(_("Liste des maladies dont le vaccin est obligatoire")))
        self.bouton_ajouter_vaccin.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter un vaccin")))
        self.bouton_modifier_vaccin.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier le vaccin sélectionné")))
        self.bouton_supprimer_vaccin.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer le vaccin sélectionné")))
        self.bouton_ajouter_probleme.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter une information médicale")))
        self.bouton_modifier_probleme.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier l'information médicale sélectionnée dans la liste")))
        self.bouton_supprimer_probleme.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer l'information médicale sélectionnée dans la liste")))
        self.bouton_medecin.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner un médecin traitant")))
        self.bouton_detacher_medecin.SetToolTip(wx.ToolTip(_("Cliquez ici pour détacher le médecin de cette fiche individuelle")))
...
##        grid_sizer_base.AddGrowableRow(0)
    ...
    def Detacher_medecin(self, event):
        if UTILS_Utilisateurs.VerificationDroitsUtilisateurActuel("individus_medecin", "modifier") == False : return
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment détacher ce médecin ?"), _("Détacher un médecin"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
