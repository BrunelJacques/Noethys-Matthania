...
CATEGORIES = [
    {"code" : "badgeage_enregistrer", "label" : _("Badgeage - Enregistrement d'une consommation"), "motscles" : ["{ID_INDIVIDU}",  "{NOM_INDIVIDU}", "{NOM_ACTIVITE}", "{NOM_UNITE}", "{NOM_GROUPE}", "{DATE}",  "{HEURE}", "{ACTION}"] },
...
class CTRL_Imprimante(wx.Choice):
    ...
    def MAJ(self):
        ...
        listeItems = [_("Imprimante par défaut"),]
        try :
...
class Dialog(wx.Dialog):
    def __init__(self, parent, categorie="", IDmodele=None):
        ...
        # texte
        self.staticbox_texte_staticbox = wx.StaticBox(self, -1, _("Ticket"))
        self.ctrl_editeur = adv.EditableListBox(self, -1, _("Saisissez ci-dessous les lignes à imprimer sur le ticket"))
        self.ctrl_editeur.GetDelButton().SetToolTip(wx.ToolTip(_("Supprimer la ligne sélectionnée")))
        self.ctrl_editeur.GetDownButton().SetToolTip(wx.ToolTip(_("Descendre la ligne sélectionnée")))
        self.ctrl_editeur.GetUpButton().SetToolTip(wx.ToolTip(_("Monter la ligne sélectionnée")))
        self.ctrl_editeur.GetEditButton().SetToolTip(wx.ToolTip(_("Editer la ligne sélectionnée")))
        self.ctrl_editeur.GetNewButton().SetToolTip(wx.ToolTip(_("Insérer une nouvelle ligne")))
        
        # Commandes
        self.bouton_aide = CTRL_Bouton_image.CTRL(self, texte=_("Aide"), cheminImage="Images/32x32/Aide.png")
        self.bouton_apercu = CTRL_Bouton_image.CTRL(self, texte=_("Imprimer un ticket de test"), cheminImage="Images/32x32/Imprimante.png")
...
##        self.Bind(wx.EVT_LISTBOX_DCLICK, self.OnInsertMotcle, self.ctrl_motscles)
        ...
        if self.IDmodele != None :
            self.SetTitle(_("Modification d'un modèle de ticket"))
            self.Importation()
        else:
            self.SetTitle(_("Saisie d'un modèle de ticket"))
        

    def __set_properties(self):
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez un nom pour ce modèle")))
        self.ctrl_description.SetToolTip(wx.ToolTip(_("Saisissez une description pour ce modèle")))
        self.ctrl_taille.SetToolTip(wx.ToolTip(_("Sélectionnez une taille de police")))
        self.ctrl_interligne.SetToolTip(wx.ToolTip(_("Sélectionnez la hauteur d'interligne")))
        self.ctrl_imprimante.SetToolTip(wx.ToolTip(_("Sélectionnez l'imprimante à utiliser")))
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez ici pour obtenir de l'aide")))
        self.bouton_apercu.SetToolTip(wx.ToolTip(_("Cliquez ici pour imprimer une page de test")))
...
##        self.ctrl_editeur.EcritTexte(motcle)
    ...
    def OnBoutonOk(self, event):     
        ...
        if nom == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un nom pour ce modèle !"), "Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if description == "" :
            dlg = wx.MessageDialog(self, _("Etes-vous sûr de ne pas vouloir saisir de description pour ce modèle ?"), _("Confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        ...
        if len(listeLignes) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une ligne !"), "Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_editeur.SetFocus()
            return     
        
        lignes = "@@@".join(listeLignes)
    ...
    def OnBoutonApercu(self, event=None):
        ...
        try :
            from Utils import UTILS_Ticket
            UTILS_Ticket.Impression(lignes=listeLignes, imprimante=imprimante, titre=_("Ticket"), nomPolice="Arial", taillePolice=taillePolice, interligne=interligne)
