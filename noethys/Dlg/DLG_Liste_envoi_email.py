...
class Dialog(wx.Dialog):
    def __init__(self, parent, listview=None):
        ...
        self.SetTitle(titre)
        intro = _("Sélectionnez les paramètres et cliquez sur Ok pour accéder à l'outil d'envoi des emails.")
        ...
        # Choix de l'adresse
        self.staticbox_adresse = wx.StaticBox(self, -1, _("Sélection de l'adresse"))
        ...
        # Lignes
        self.staticbox_lignes = wx.StaticBox(self, -1, _("Sélection des lignes"))
        self.radio_lignes_affichees = wx.RadioButton(self, -1, _("Toutes les lignes affichées"), style=wx.RB_GROUP)
        self.radio_lignes_cochees = wx.RadioButton(self, -1, _("Toutes les lignes cochées"))
        self.radio_ligne_selectionnee = wx.RadioButton(self, -1, _("La ligne sélectionnée"))
    ...
    def __set_properties(self):
        self.radio_famille.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner les adresses des familles")))
        self.radio_individu.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner les adresses des individus")))
        self.radio_lignes_affichees.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner toutes les lignes affichées dans la liste")))
        self.radio_lignes_cochees.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner toutes les lignes cochées affichées")))
        self.radio_ligne_selectionnee.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner la ligne sélectionnée")))
    ...
    def OnBoutonOk(self, event):
        ...
        if len(listeAnomalies) > 0:
            if self.radio_famille.GetValue() == True :
                introduction = _("%d des familles sélectionnées n'ont pas d'adresse Email :") % len(listeAnomalies)
                conclusion = _("Souhaitez-vous quand même continuer avec les %d autres familles ?") % len(listeDonnees)
            else :
                introduction = _("%d des individus sélectionnés n'ont pas d'adresse Email :") % len(listeAnomalies)
                conclusion = _("Souhaitez-vous quand même continuer avec les %d autres individus ?") % len(listeDonnees)
        ...
        if len(listeDonnees) == 0:
            dlg = wx.MessageDialog(self, _("Il ne reste finalement aucun email à envoyer !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
