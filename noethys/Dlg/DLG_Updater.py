...
class zipdirectory(Thread):
    ...
    def run(self):
        ...
        zfile.close()
        self.journal.WriteText(str(self.nbreFichiers) + _(" fichiers ont été sauvegardés avec succès."))
...
class Download(Thread): 
    ...
    def _hook(self, nb_blocs, taille_bloc, taille_fichier):
        ...
        if nb_blocs*taille_bloc >= taille_fichier:
            ...
            self.succes = True
            self.zoneTexte.SetLabel(_("Le téléchargement est terminé. Veuillez patienter..."))
            self.frameParente.SetTitle(_("Mises à jour Internet"))
            raise Abort
        if self.stop: 
            raise Abort
        ...
        if nb_blocs % 5 == 0 :
            if "linux" not in sys.platform :
                texteInfo = _("Téléchargement en cours...  ") + FormateTailleFichier(nb_blocs*taille_bloc)+" / "+FormateTailleFichier(taille_fichier)
                if texteInfo != self.zoneTexte.GetLabel() :
                    self.zoneTexte.SetLabel(texteInfo)
                self.frameParente.SetTitle(AffichePourcentage(nb_blocs*taille_bloc, taille_fichier) + _(" | Téléchargement d'une mise à jour"))

    def run(self): 
        #print "Telechargement de la nouvelle version : etape 5"
        try: 
            if "linux" in sys.platform :
                self.zoneTexte.SetLabel(_("Téléchargement en cours..."))
...
class Page_recherche(wx.Panel):
    ...
    def Recherche(self):
        """ Recherche internet """
        texteIntro = _("Recherche d'une mise à jour internet en cours...")
...
##        print "versionLogiciel=", versionLogiciel, " | versionInternet=", versionInternet
    ...
    def Suite(self, etat):
        ...
        if etat == "erreur" :
            ...
            self.label_introduction.SetLabel(_("Connexion au serveur de mise à jour impossible."))
            self.timer.Stop()
            self.Layout()
        if etat == "aucune" :
            # Aucune mise à jour n'a été trouvée
            self.label_introduction.SetLabel(_("Vous disposez déjà de la dernière version du logiciel."))
            self.timer.Stop()
...
class Page_disponible(wx.Panel):
    def __init__(self, parent, ID=-1):
        ...
        texteIntro2 = _("Souhaitez-vous la télécharger maintenant ?")
        ...
        # Boutons
        self.bouton_aide = CTRL_Bouton_image.CTRL(self, texte=_("Aide"), cheminImage="Images/32x32/Aide.png")
        self.bouton_ok = CTRL_Bouton_image.CTRL(self, texte=_("Télécharger"), cheminImage="Images/32x32/Telecharger.png")
        self.bouton_annuler = CTRL_Bouton_image.CTRL(self, texte=_("Annuler"), cheminImage="Images/32x32/Annuler.png")
...
class Page_telechargement(wx.Panel):
    def __init__(self, parent, ID=-1):
        ...
        # Création des widgets
        texteIntro = _("Le téléchargement va commencer dans quelques instants...")
        self.label_introduction = wx.StaticText(self, -1, texteIntro) # FonctionsPerso.StaticWrapText(self, -1, texteIntro)
        self.gauge = wx.Gauge(self, -1)
    ...
    def __set_properties(self):
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez ici pour obtenir de l'aide")))
        self.bouton_ok.SetToolTip(wx.ToolTip(_("Cliquez ici pour lancer le téléchargement")))
        self.bouton_annuler.SetToolTip(wx.ToolTip(_("Cliquez pour annuler et fermer")))
    ...
    def Suite(self, succes):
        ...
        if succes == True :
            ...
            self.label_introduction.SetLabel(_("Le téléchargement n'est pas complet. Voulez-vous recommencer ?"))
            self.bouton_ok.Show(True)
            self.Layout()
    ...
    def Arreter_telechargement(self):
        ...
        if downloadEnCours:
            ...
            dlgConfirm = wx.MessageDialog(self, _("Souhaitez-vous vraiment arrêter le téléchargement ?"), _("Confirmation d'arrêt"), wx.YES_NO|wx.NO_DEFAULT|wx.ICON_QUESTION)
            ...
            self.label_introduction.SetLabel(_("Vous avez interrompu le téléchargement."))
            self.bouton_ok.Show(True)
...
class Page_fin_telechargement(wx.Panel):
    def __init__(self, parent, ID=-1):
        ...
        # Création des widgets
        texteIntro1 = _("La mise à jour a été téléchargée avec succès.")
        self.label_introduction1 = wx.StaticText(self, -1, texteIntro1) # FonctionsPerso.StaticWrapText(self, -1, texteIntro1)
...
class Page_installation(wx.Panel):
    def __init__(self, parent, ID=-1):
        ...
        # Création des widgets
        texteIntro = _("Installation de la mise à jour en cours...")
        self.label_introduction = wx.StaticText(self, -1, texteIntro) # FonctionsPerso.StaticWrapText(self, -1, texteIntro)
    ...
    def _Sauvegarde(self):
        ...
        self.label_introduction.SetLabel(_("Sauvegarde des données locales en cours..."))
        if "linux" not in sys.platform :
            self.journal.WriteText(_("> Sauvegarde des données locales :\n\n"))
    ...
    def _Installation(self):
        ...
        if "linux" in sys.platform :
            ...
            try :
                ...
                dlg = wx.MessageDialog(self, _("Noethys n'a pas réussi à ouvrir l'installeur.\n\nCe blocage peut peut-être venir de votre antivirus. Essayez éventuellement de le désactiver le temps de l'installation de la mise à jour."), _("Erreur"), wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                return False
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        intro = _("Vous pouvez ici télécharger et installer une mise à jour pour Noethys. Ces mises à jour vous permettent bien-sûr de gagner en stabilité et en fonctionnalités.")
        titre = _("Mise à jour du logiciel")
