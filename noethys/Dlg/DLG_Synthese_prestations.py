...
class CTRL_Regroupement(wx.Choice):
    def __init__(self, parent, option_aucun=False):
        ...
        self.parent = parent
        self.listeDonnees = [
            {"label": _("Jour"), "code": "jour"},
            {"label": _("Mois"), "code": "mois"},
            {"label": _("Année"), "code": "annee"},
            {"label": _("Label de la prestation"), "code": "label_prestation"},
            {"label": _("Activité"), "code": "activite"},
            {"label": _("Catégorie de tarif"), "code": "categorie_tarif"},
            {"label": _("Famille"), "code": "famille"},
            {"label": _("Individu"), "code": "individu"},
            {"label": _("Ville de résidence"), "code": "ville_residence"},
            {"label": _("Secteur géographique"), "code": "secteur"},
            ...
            {"label": _("Régime social"), "code": "regime"},
            {"label": _("Caisse d'allocations"), "code": "caisse"},
            {"label": _("Quotient familial - Tranches tarifs"), "code": "qf_tarifs"},
            {"label": _("Quotient familial - Tranches de 100"), "code": "qf_100"},
        ]

        if option_aucun == True :
            self.listeDonnees.insert(0, {"label": _("Aucun"), "code": ""})
...
class CTRL_Mode(wx.Choice):
    def __init__(self, parent):
        ...
        self.parent = parent
        self.listeDonnees = [
            (_("Nombre de prestations"), "nbre"),
            (_("Montant des prestations"), "facture"),
            (_("Montant des prestations réglées"), "regle"),
            (_("Montant des prestations impayées"), "impaye"),
            (u"---------------------------------------------", None),
            (_("Nombre de prestations facturées"), "nbre_facturees"),
            (_("Montant des prestations facturées"), "facture_facturees"),
            (_("Montant des prestations réglées et facturées"), "regle_facturees"),
            (_("Montant des prestations impayées et facturées"), "impaye_facturees"),
            (u"---------------------------------------------", None),
            (_("Nombre de prestations non facturées"), "nbre_nonfacturees"),
            (_("Montant des prestations non facturées"), "facture_nonfacturees"),
            (_("Montant des prestations réglées et non facturées"), "regle_nonfacturees"),
            (_("Montant des prestations impayées et non facturées"), "impaye_nonfacturees"),
            ]
        self.Remplissage() 
...
class ChoixPeriode(wx.Panel):
    def __init__(self, parent, type="", label=u"", infobulle=u""):
        ...
        self.ctrl_date_debut.SetToolTip(wx.ToolTip(_("Saisissez la date de début")))
...
class Parametres(wx.Panel):
    def __init__(self, parent):
        ...
        # Filtres
        self.staticbox_affichage_staticbox = wx.StaticBox(self, -1, _("Types de prestations"))
...
##        self.options_cotisations = ChoixPeriode(self, type="cotisations", label=_("Uniquement les cotisations déposées"))
        self.options_reglements = ChoixPeriode(self, type="reglements", label=_("Uniquement les règlements saisis"), infobulle=_("Ne seront considérés dans le calcul que les règlements saisis sur la période indiquée"))
        self.options_depots = ChoixPeriode(self, type="depots", label=_("Uniquement les règlements déposés"), infobulle=_("Ne seront considérés dans le calcul que les règlements qui ont été déposés en banque sur la période indiquée"))
    ...
    def __set_properties(self):
        ...
        self.bouton_date_debut.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner la date de début dans un calendrier")))
        self.ctrl_date_fin.SetToolTip(wx.ToolTip(_("Saisissez la date de fin de période")))
        self.bouton_date_fin.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner la date de fin dans un calendrier")))
        self.radio_cotisations.SetToolTip(wx.ToolTip(_("Cochez cette case pour afficher les cotisations dans la synthèse")))
        self.radio_consommations.SetToolTip(wx.ToolTip(_("Cochez cette case pour afficher les consommations dans la synthèse")))
        self.radio_locations.SetToolTip(wx.ToolTip(_("Cochez cette case pour afficher les locations dans la synthèse")))
        self.radio_autres.SetToolTip(wx.ToolTip(_("Cochez cette case pour afficher les autres types de prestations dans la synthèse")))
        self.bouton_actualiser.SetToolTip(wx.ToolTip(_("Cliquez ici pour actualiser les résultats en fonction des paramètres sélectionnés")))
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        # Bandeau
        intro = _("Cette fonction vous permet de quantifier précisément les prestations facturées sur une période donnée. Commencez par saisir une période puis sélectionnez un groupe d'activités.")
        titre = _("Synthèse des prestations")
        ...
        # CTRL résultats
        self.staticbox_stats_staticbox = wx.StaticBox(self, -1, _("Résultats"))

        self.label_lignes_1 = wx.StaticText(self, -1, _("Ligne :"))
        self.ctrl_lignes_1 = CTRL_Regroupement(self)
        self.ctrl_lignes_1.SetMinSize((50, -1))

        self.label_lignes_2 = wx.StaticText(self, -1, _("Détail :"))
        ...
        self.label_mode_affichage = wx.StaticText(self, -1, _("Données :"))
        ...
        self.check_details_total = wx.CheckBox(self, -1, _("Afficher le détail de la ligne total"))
    ...
    def __set_properties(self):
        ...
        self.bouton_apercu.SetToolTip(wx.ToolTip(_("Cliquez ici pour créer un aperçu avant impression des résultats (PDF)")))
        self.bouton_excel.SetToolTip(wx.ToolTip(_("Cliquez ici pour exporter les résultats au format MS Excel")))
        self.ctrl_mode.SetToolTip(wx.ToolTip(_("Sélectionnez le mode d'affichage souhaité")))
        self.check_details_total.SetToolTip(wx.ToolTip(_("Cliquez ici pour afficher les détails de la ligne total")))
    ...
    def Build_Hyperlink_developper(self) :
        ...
        hyper.SetBold(False)
        hyper.SetToolTip(wx.ToolTip(_("Tout développer")))
        hyper.UpdateLink()
        hyper.DoPopup(False)
        return hyper
    ...
    def Build_Hyperlink_reduire(self) :
        ...
        hyper.SetUnderlines(False, False, True)
        hyper.SetBold(False)
        hyper.SetToolTip(wx.ToolTip(_("Tout réduire")))
        hyper.UpdateLink()
        hyper.DoPopup(False)
        return hyper
    ...
    def GetLabelParametres(self):
        ...
        listeParametres.append(_("Période du %s au %s") % (DateEngFr(str(date_debut)), DateEngFr(str(date_fin))))
        ...
        listeParametres.append(_("Eléments affichés : %s") % affichage)
        ...
        if self.ctrl_parametres.options_reglements.GetEtat() == True :
            ...
            listeParametres.append(_("Uniquement les règlements saisis du %s au %s") % (date_debut_reglement, date_fin_reglement))
        ...
        if self.ctrl_parametres.options_depots.GetEtat() == True :
            ...
            listeParametres.append(_("Uniquement les règlements déposés du %s au %s") % (date_debut_depot, date_fin_depot))
        
        # Mode d'affichage
        mode = self.ctrl_mode.GetLabel()
        listeParametres.append(_("Mode d'affichage : %s") % mode)
