...
class Dialog(wx.Dialog):
    def __init__(self, parent, filtres=[]):
        ...
        # Bandeau
        intro = _("Cochez les factures à envoyer par Email puis cliquez sur le bouton 'Transférer vers l'éditeur d'Emails'.")
        titre = _("Envoi de factures par Email")
        self.SetTitle(titre)
        ...
        # Options
        self.box_options_staticbox = wx.StaticBox(self, -1, _("Options des factures"))
    ...
    def __set_properties(self):
        ...
        self.bouton_ok.SetToolTip(wx.ToolTip(_("Cliquez ici pour transférer les factures vers l'éditeur d'Emails")))
    ...
    def OnBoutonOk(self, event): 
        ...
        tracks = self.ctrl_liste_factures.GetTracksCoches() 
        if len(tracks) == 0 : 
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune facture à envoyer par Email !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if len(listeEnvoiNonDemande) > 0 :
            ...
            texte += _("Souhaitez-vous quand même leur envoyer une facture ?")
            dlg = wx.MessageDialog(self, texte, _("Avertissement"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
            reponse = dlg.ShowModal() 
            dlg.Destroy()
            if reponse != wx.ID_YES :
        ...
        if len(listeDonnees) == 0 : 
            dlg = wx.MessageDialog(self, _("Il ne reste finalement aucune facture à envoyer par Email !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            SupprimerFichiersTemp()
            return
