...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDactivite=None, dictValeurs={}, listeTracks=[]):
        ...
        self.label_periodicite = wx.StaticText(self, wx.ID_ANY, _("Périodicité :"))
        self.ctrl_periodicite = wx.Choice(self, -1, choices=[_("Annuelle"), _("Mensuelle"), _("Hebdomadaire")])
        self.ctrl_periodicite.SetSelection(1) 
        self.label_date_prestation = wx.StaticText(self, wx.ID_ANY, _("Date :"))
        self.ctrl_date_prestation = wx.Choice(self, -1, choices=[_("Début de période"), _("Fin de période")])
        self.ctrl_date_prestation.SetSelection(0)
        self.label_label_prestation = wx.StaticText(self, wx.ID_ANY, _("Label :"))
        self.ctrl_label_prestation = wx.TextCtrl(self, wx.ID_ANY, "{LABEL_AUTO}")
    ...
    def __set_properties(self):
        self.SetTitle(_("Génération automatique de périodes"))
        ...
        self.ctrl_periodicite.SetToolTip(wx.ToolTip(_("Sélectionnez la périodicité à appliquer")))
        self.ctrl_label_prestation.SetToolTip(wx.ToolTip(_("Saisissez le label de la prestation. Le mot-clé {LABEL_AUTO} permet d'insérer un label automatiquement (Ex : 'Janvier 2014'). Les mots-clés suivants sont également disponibles : {ANNEE} {NUM_MOIS} {NOM_MOIS} {NUM_SEMAINE}.")))
        self.ctrl_date_prestation.SetToolTip(wx.ToolTip(_("Sélectionnez la date de la prestation")))
    ...
    def Generation(self):
        ...
        for date1, date2 in listeDates2 :
            ...
            if dictParametres["periodicite"] == 1 :
                nom_auto = "%s %d" % (LISTE_MOIS[date1.month-1].capitalize(), date1.year)
            if dictParametres["periodicite"] == 2 :
                nom_auto = _("Semaine %d") % date1.isocalendar()[1]
        ...
        for dictPeriode in listePeriodes :
            for track in self.listeTracks :
                if dictPeriode["date_fin"] >= track.date_debut and dictPeriode["date_debut"] <= track.date_fin :
                    dlg = wx.MessageDialog(self, _("Génération impossible !\n\nLa période '%s' chevauche la période existante '%s' (du %s au %s).") % (dictPeriode["label_prestation"], track.label_prestation, UTILS_Dates.DateDDEnFr(track.date_debut), UTILS_Dates.DateDDEnFr(track.date_fin)), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                    dlg.ShowModal()
                    dlg.Destroy()
                    return False
...
if __name__ == "__main__":
    ...
    dlg.ctrl_label_prestation.SetLabel(_("Centre de Loisirs - {LABEL_AUTO}"))
