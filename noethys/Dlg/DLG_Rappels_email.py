...
class Dialog(wx.Dialog):
    def __init__(self, parent, filtres=[]):
        ...
        # Bandeau
        intro = _("Cochez les lettres de rappel à envoyer par Email puis cliquez sur le bouton 'Transférer vers l'éditeur d'Emails'.")
        titre = _("Envoi de letttres de rappel par Email")
        self.SetTitle(titre)
        ...
        # rappels
        self.box_rappels_staticbox = wx.StaticBox(self, -1, _("Liste des lettres de rappel"))
        self.ctrl_liste_rappels = CTRL_Liste_rappels.CTRL(self, filtres=filtres)
        
        # Options
        self.box_options_staticbox = wx.StaticBox(self, -1, _("Options des lettres de rappel"))
        self.ctrl_options = CTRL_Rappels_options.CTRL(self)
        
        # Boutons
        self.bouton_aide = CTRL_Bouton_image.CTRL(self, texte=_("Aide"), cheminImage="Images/32x32/Aide.png")
        self.bouton_ok = CTRL_Bouton_image.CTRL(self, texte=_("Transférer vers l'éditeur d'Emails"), cheminImage="Images/32x32/Emails_piece.png")
    ...
    def __set_properties(self):
        ...
        self.bouton_ok.SetToolTip(wx.ToolTip(_("Cliquez ici pour transférer les rappels vers l'éditeur d'Emails")))
    ...
    def OnBoutonOk(self, event): 
        ...
        if len(tracks) == 0 : 
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune lettre de rappel à envoyer par Email !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if len(listeEnvoiNonDemande) > 0 :
            texte = _("%d des familles sélectionnées n'ont pas demandé d'envoi par Email de leur facture :\n\n") % len(listeEnvoiNonDemande)
            texte += _("Souhaitez-vous quand même leur envoyer une lettre de rappel ?")
            dlg = wx.MessageDialog(self, texte, _("Avertissement"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
            reponse = dlg.ShowModal() 
            dlg.Destroy()
            if reponse != wx.ID_YES :
        ...
        if len(listeDonnees) == 0 : 
            dlg = wx.MessageDialog(self, _("Il ne reste finalement aucune lettre de rappel à envoyer par Email !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            SupprimerFichiersTemp()
            return
