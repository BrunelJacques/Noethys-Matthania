...
class CTRL_Parametres(CTRL_Propertygrid.CTRL) :
    ...
    def Remplissage(self):
        # --------------------------- TYPE DE DOCUMENT------------------------------------------
        self.Append( wxpg.PropertyCategory(_("Type de document")) )
    
        # Type
        propriete = wxpg.EnumProperty(label=_("Type"), name="type_document", labels=[_("Détaillé"), _("Simplifié"), (u"Totaux")], values=[0, 1, 2], value=0)
        propriete.SetHelpString(_("Sélectionnez un type de document")) 
        propriete.SetAttribute("obligatoire", True)
        self.Append(propriete)

        # --------------------------- COULEURS DE FOND ------------------------------------------
        self.Append( wxpg.PropertyCategory(_("Couleurs de fond")) )
        ...
        # Couleur 2
        propriete = wxpg.ColourProperty(label=_("Couleur de fond 2"), name="couleur_fond_2", value=wx.Colour(230, 230, 255))
        propriete.SetHelpString(_("Sélectionnez une couleur")) 
        propriete.SetAttribute("obligatoire", True)
        self.Append(propriete)
        ...
        # Texte
        propriete = wxpg.StringProperty(label=_("Texte"), name="titre_texte", value=_("Récapitulatif des factures"))
        propriete.SetHelpString(_("Saisissez un texte"))
        self.Append(propriete)
...
##        self.Append(propriete)
        ...
        # --------------------------- FAMILLES ------------------------------------------
        self.Append( wxpg.PropertyCategory(_("Champs familiaux")) )

        liste_choix = [("non", _("Non")),]
        ...
        # Question 1
        propriete = CTRL_Propertygrid.Propriete_choix(label=_("Champ 1"), name="question_1", liste_choix=liste_choix, valeur="non")
        propriete.SetEditor("EditeurChoix")
        propriete.SetHelpString(_("Sélectionnez un item du questionnaire famille à inclure dans le document"))
        self.Append(propriete)

        # Question 2
        propriete = CTRL_Propertygrid.Propriete_choix(label=_("Champ 2"), name="question_2", liste_choix=liste_choix, valeur="non")
        propriete.SetEditor("EditeurChoix")
        propriete.SetHelpString(_("Sélectionnez un item du questionnaire famille à inclure dans le document"))
        self.Append(propriete)

        # Question 3
        propriete = CTRL_Propertygrid.Propriete_choix(label=_("Champ 3"), name="question_3", liste_choix=liste_choix, valeur="non")
        propriete.SetEditor("EditeurChoix")
        propriete.SetHelpString(_("Sélectionnez un item du questionnaire famille à inclure dans le document"))
        self.Append(propriete)

        # --------------------------- INDIVIDUS ------------------------------------------
        self.Append( wxpg.PropertyCategory(_("Champs individuels")) )

        liste_choix = [("non", _("Non")),
                       ("ecole_debut_facture", _("Ecole à la date de début de la facture")),
                       ("ecole_fin_facture", _("Ecole à la date de fin de la facture")),
                       ("classe_debut_facture", _("Classe à la date de début de la facture")),
                       ("classe_fin_facture", _("Classe à la date de fin de la facture")),
                       ("niveau_debut_facture", _("Niveau scolaire à la date de début de la facture")),
                       ("niveau_fin_facture", _("Niveau scolaire à la date de fin de la facture")),
                       ]

        # Champ individuel 1
        propriete = CTRL_Propertygrid.Propriete_choix(label=_("Champ 1"), name="champ_ind_1", liste_choix=liste_choix, valeur="non")
        propriete.SetEditor("EditeurChoix")
        propriete.SetHelpString(_("Sélectionnez un champ individuel à intégrer dans le document"))
        self.Append(propriete)

        # Champ individuel 2
        propriete = CTRL_Propertygrid.Propriete_choix(label=_("Champ 2"), name="champ_ind_2", liste_choix=liste_choix, valeur="non")
        propriete.SetEditor("EditeurChoix")
        propriete.SetHelpString(_("Sélectionnez un champ individuel à intégrer dans le document"))
        self.Append(propriete)

        # Champ individuel 3
        propriete = CTRL_Propertygrid.Propriete_choix(label=_("Champ 3"), name="champ_ind_3", liste_choix=liste_choix, valeur="non")
        propriete.SetEditor("EditeurChoix")
        propriete.SetHelpString(_("Sélectionnez un champ individuel à intégrer dans le document"))
        self.Append(propriete)
        ...
        # Texte
        propriete = wxpg.StringProperty(label=_("Texte"), name="intro_texte", value=u"")
        propriete.SetHelpString(_("Saisissez un texte"))
        self.Append(propriete)
...
##        self.Append(propriete)
        ...
        # Texte
        propriete = wxpg.StringProperty(label=_("Texte"), name="conclusion_texte", value=_("{NBRE_FACTURES} factures | Montant total : {TOTAL_FACTURES}"))
        propriete.SetHelpString(_("Saisissez un texte. Vous pouvez utiliser les mots-clés suivants : {NBRE_FACTURES}, {TOTAL_FACTURES}, {NBRE_FACT_PRELEV}, {TOTAL_FACT_PRELEV}"))
        self.Append(propriete)
...
class Dialog(wx.Dialog):
    def __init__(self, parent, dictOptions={}, tracks=[]):
        ...
        # Bandeau
        titre = _("Impression d'un récapitulatif des factures")
        intro = _("Vous pouvez ici modifier les paramètres d'impression du document. Cliquez sur le bouton 'Mémoriser les paramètres' pour réutiliser les mêmes paramètres pour les impressions suivantes.")
        self.SetTitle(titre)
    ...
    def OnBoutonOk(self, event): 
        ...
        for IDprestation, IDfacture, label, montant, IDindividu, nom, prenom, IDactivite, nomActivite in listePrestations :
            ...
            if IDindividu != None and (IDindividu in dictIndividus) == False and prenom != None :
                dictIndividus[IDindividu] = "%s %s" % (nom, prenom)
        ...
        def Header():
            ...
            dataTableau.append( (dictOptions["titre_texte"], _("%s\nEdité le %s") % (UTILS_Organisateur.GetNom(), dateDuJour)) )
            style = TableStyle([
        ...
        if dictOptions["type_document"] in (0, 1) :
            ...
            for track in self.tracks :
                ...
                dataTableau.append((
                    _("Facture n°%s") % numero, 
                    "%.2f %s" % (solde, SYMBOLE),
                    _("Edité le %s | Echéance le %s | Période du %s au %s") % (UTILS_Dates.DateDDEnFr(track.date_edition), UTILS_Dates.DateDDEnFr(track.date_echeance), UTILS_Dates.DateDDEnFr(track.date_debut), UTILS_Dates.DateDDEnFr(track.date_fin)),
                    ))
                tableau = Table(dataTableau, largeursColonnes)
                listeStyles = [
                ...
                if dictOptions["type_document"] == 0 and track.IDfacture in dictPrestations :
                    ...
                    if len(liste_champs_ind) == 0 :
                        dataTableau = [(Paragraph(_("Individu"), styleLabel), Paragraph(_("Activité"), styleLabel),
                                        Paragraph(_("Prestation"), styleLabel), Paragraph(_("Quantité"), styleLabel),
                                        Paragraph(_("Montant total"), styleLabel)), ]
                        largeursColonnes = [130, 120, 185, 35, 50]
                    else:
                        # On rajoute la colonne Infos individuelles :
                        dataTableau = [(Paragraph(_("Individu"), styleLabel), Paragraph(_("Informations"), styleLabel), Paragraph(_("Activité"), styleLabel),
                                        Paragraph(_("Prestation"), styleLabel), Paragraph(_("Quantité"), styleLabel),
                                        Paragraph(_("Montant total"), styleLabel)), ]
        ...
        # Totaux        
        dataTableau = [
            (_("Totaux par activités et prestations"), "", "", ""),
            (Paragraph(_("Activités"), styleLabel), Paragraph(_("Prestations"), styleLabel), Paragraph(_("Quantité"), styleLabel), Paragraph(_("Montant"), styleLabel)),
            ]
        ...
        for IDactivite, dictLabels in dictTotaux.items() :
            ...
            if IDactivite == None :
                nomActivite = _("Prestations diverses")
            else :
        ...
        conclusion_texte = conclusion_texte.replace("{TOTAL_FACTURES}", "%.2f %s" % (montantTotal, SYMBOLE))
        conclusion_texte = conclusion_texte.replace("{NBRE_FACT_PRELEV}", str(nbrFactPrelev))
        conclusion_texte = conclusion_texte.replace("{TOTAL_FACT_PRELEV}", "%.2f %s" % (montantTotalPrelev, SYMBOLE))
