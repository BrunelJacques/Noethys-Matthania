...
##        self.label_intro = wx.StaticText(self, -1, _("Remarque : Les prélèvements manuels ne peuvent pas être réglés automatiquement."))
##        self.label_intro.SetFont(wx.Font(7, wx.DEFAULT, wx.NORMAL, wx.NORMAL, 0, ""))
##        self.label_intro.SetForegroundColour((100, 100, 100)) 
        
        # Famille
        self.staticbox_famille_staticbox = wx.StaticBox(self, -1, _("Famille"))
        self.label_famille = wx.StaticText(self, -1, _("Titulaires :"))
    ...
    def __set_properties(self):
        self.SetTitle(_("Saisie d'un prélèvement manuel"))
        self.ctrl_famille.SetToolTip(wx.ToolTip(_("Sélectionnez la famille")))
        self.ctrl_libelle.SetToolTip(wx.ToolTip(_("Saisissez ici le libellé du prélèvement")))
        self.ctrl_montant.SetToolTip(wx.ToolTip(_("Saisissez ici le montant du prélèvement")))
...
##        grid_sizer_base.Add(self.label_intro, 1, wx.LEFT|wx.RIGHT|wx.TOP|wx.EXPAND, 10)
    ...
    def OnBoutonOk(self, event): 
        if self.GetIDfamille() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner une famille dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_famille.SetFocus()
            return
        if self.GetLibelle() == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un libellé pour ce prélèvement !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_libelle.SetFocus()
            return
        if self.GetMontant() == None or self.GetMontant() == 0.0 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un montant pour ce prélèvement !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
