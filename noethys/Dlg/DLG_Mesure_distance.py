...
class CTRL_Lieu(wx.Choice):
    ...
    def MAJ(self):
        ...
        if "IDfamille" in self.dictParametres and self.dictParametres["IDfamille"] != None :
            ...
            liste_champs.append({"code" : "adresse_famille", "label" : _("Lieu de résidence du loueur"), "adresse" : "%s %s %s" % (rue, cp, ville)})
        ...
        if "IDproduit" in self.dictParametres and self.dictParametres["IDproduit"] != None :
            ...
            for dictQuestion in questionnaires.GetDonnees(ID=IDproduit):
                liste_champs.append({"code" : dictQuestion["champ"], "label" : _("%s (produit)") % dictQuestion["label"], "adresse" : dictQuestion["reponse"]})
        ...
        if "IDcategorie" in self.dictParametres and self.dictParametres["IDcategorie"] != None:
            ...
            for dictQuestion in questionnaires.GetDonnees(ID=IDcategorie):
                liste_champs.append({"code": dictQuestion["champ"], "label": _("%s (catégorie de produits)") % dictQuestion["label"], "adresse": dictQuestion["reponse"]})
...
class Dialog(wx.Dialog):
    def __init__(self, parent, dictParametres={}):
        ...
        # Bandeau
        intro = _("Vous pouvez ici mesurer la distance entre deux lieux. Sélectionnez l'origine et la destination du trajet à mesurer. Ces paramètres sont mémorisés pour une utilisation ultérieure.")
        titre = _("Mesurer une distance")
        self.SetTitle(titre)
        ...
        self.label_origine = wx.StaticText(self, wx.ID_ANY, _("Origine :"))
        self.ctrl_origine = CTRL_Lieu(self, dictParametres=dictParametres)
        self.label_destination = wx.StaticText(self, wx.ID_ANY, _("Destination :"))
        self.ctrl_destination = CTRL_Lieu(self, dictParametres=dictParametres)
        self.label_moyen = wx.StaticText(self, wx.ID_ANY, _("Véhicule :"))
        self.radio_voiture = wx.RadioButton(self, -1, _("En voiture"), style=wx.RB_GROUP)
        self.radio_marche = wx.RadioButton(self, -1, _("A pied"))
        self.radio_velo = wx.RadioButton(self, -1, _("A vélo"))
    ...
    def __set_properties(self):
        self.ctrl_origine.SetToolTip(wx.ToolTip(_("Sélectionnez une origine dans la liste")))
        self.ctrl_destination.SetToolTip(wx.ToolTip(_("Sélectionnez une destination dans la liste")))
    ...
    def MAJ(self, event=None):
        ...
        if dictValeursOrigine == None or dictValeursOrigine["adresse"] in (None, ""):
            self.ctrl_resultats.SetTexte(_("Le lieu d'origine n'est pas valide."))
            return False
        ...
        if dictValeursDestination == None or dictValeursDestination["adresse"] in (None, ""):
            self.ctrl_resultats.SetTexte(_("Le lieu de destination n'est pas valide."))
            return False
        ...
        if dictResultats == {} :
            self.ctrl_resultats.SetTexte(_("Aucun résultat avec ces paramètres."))
            return False
