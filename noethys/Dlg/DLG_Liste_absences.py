...
class Parametres(wx.Panel):
    def __init__(self, parent):
        ...
        self.ctrl_affichage = wx.Choice(self, -1, choices=[_("Absences injustifiées"), _("Absences justifiées")])
    ...
    def OnBoutonAfficher(self, event):
        ...
        listeParametres.append(_("Période du %s au %s") % (UTILS_Dates.DateDDEnFr(date_debut), UTILS_Dates.DateDDEnFr(date_fin)))
        ...
        if self.ctrl_affichage.GetSelection() == 0 :
            affichage = "absenti"
            listeParametres.append(_("Uniquement les absences injustifiées"))
        else :
            affichage = "absentj"
            listeParametres.append(_("Uniquement les absences justifiées"))
