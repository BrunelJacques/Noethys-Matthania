...
class Dialog(wx.Dialog):
    def __init__(self, parent, mode="ajout", track_evenement=None):
        ...
        self.label_horaires = wx.StaticText(self, -1, _("Horaires :"))
        ...
        self.check_limitation_inscrits = wx.CheckBox(self, -1, _("Nombre d'inscrits max. :"))
        self.ctrl_limitation_inscrits = wx.SpinCtrl(self, -1, size=(80, -1), min=1, max=99999)

        # Tarification
        self.staticbox_tarification_staticbox = wx.StaticBox(self, -1, _("Tarification spécifique"))
        ...
        # Init
        if mode == "ajout" :
            self.SetTitle(_("Saisie d'un évènement"))
        else:
            self.SetTitle(_("Modification d'un évènement"))
            self.Importation()
    ...
    def __set_properties(self):
        ...
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez ici l'intitulé de l'évènement (Ex : 'Cinéma', 'Yoga'...)")))
        self.ctrl_description.SetToolTip(wx.ToolTip(_("[Optionnel] Saisissez ici une description pour cet évènement")))
        self.ctrl_heure_debut.SetToolTip(wx.ToolTip(_("[Optionnel] Saisissez l'heure de début de l'évènement (Ex : 08:30)")))
        self.ctrl_heure_fin.SetToolTip(wx.ToolTip(_("[Optionnel] Saisissez l'heure de fin de l'évènement (Ex : 09:30)")))
        self.radio_tarification_aucune.SetToolTip(wx.ToolTip(_("Aucune tarification spécifique pour cet évènement")))
        self.radio_tarification_montant.SetToolTip(wx.ToolTip(_("Un montant fixe est associé à cet évènement")))
        self.radio_tarification_tarif.SetToolTip(wx.ToolTip(_("Un ou plusieurs tarifs avancés sont associés à cet évènement")))
        self.ctrl_montant.SetToolTip(wx.ToolTip(_("Saisissez un montant fixe pour cet évènement")))
        ...
        self.check_limitation_inscrits.SetToolTip(wx.ToolTip(_("Cochez cette case pour définir un nombre d'inscrits maximal pour cet évènement")))
        self.ctrl_limitation_inscrits.SetToolTip(wx.ToolTip(_("Saisissez ici une nombre d'inscrits maximal pour cet évènement")))
    ...
    def OnBoutonOk(self, event):
        ...
        if nom == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un nom pour cet évènement !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if heure_debut != None and heure_fin != None and heure_debut > heure_fin :
            dlg = wx.MessageDialog(self, _("L'heure de début ne peut pas être supérieure à l'heure de fin !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
