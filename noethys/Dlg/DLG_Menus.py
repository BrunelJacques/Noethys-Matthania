...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        # Restaurateur
        self.staticbox_restaurateur_staticbox = wx.StaticBox(self, -1, _("Restaurateur"))
        ...
        # Menus
        self.box_menus_staticbox = wx.StaticBox(self, wx.ID_ANY, _("Menus"))
        self.ctrl_menus = CTRL_Menus.CTRL(self)
        self.ctrl_menus.SetMinSize((100, 380))

        # Légende
        self.listeLegende = [
            {"label": _("Cases avec contenu"), "couleur": CTRL_Menus.COULEUR_CASES_OUVERTES, "ctrl_label": None, "ctrl_img": None},
            {"label": _("Cases vides"), "couleur": CTRL_Menus.COULEUR_CASES_FERMEES, "ctrl_label": None, "ctrl_img": None},
            ]
        index = 0
    ...
    def Imprimer(self, event=None):
        if self.ctrl_restaurateur.GetID() == None :
            dlg = wx.MessageDialog(self, _("Vous devez d'abord sélectionner un restaurateur dans la liste !"), _("Avertissement"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
