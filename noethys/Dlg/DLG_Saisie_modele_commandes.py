...
class Page_Colonnes(wx.Panel):
    ...
    def Validation(self):
        if len(self.GetColonnes()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez créer au moins une colonne pour ce modèle !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        return True
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDmodele=None, premierModele=None):
        ...
        # Colonnes
        self.box_colonnes_staticbox = wx.StaticBox(self, wx.ID_ANY, _("Colonnes"))
        ...
        if self.IDmodele == None :
            self.SetTitle(_("Saisie d'un modèle de commandes de repas"))
        else :
            self.SetTitle(_("Modification d'un modèle de commandes de repas"))
            self.Importation()
    ...
    def OnBoutonOk(self, event):
        ...
        if nom == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un nom pour ce modèle !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_nom.SetFocus()
            return False
        ...
        if IDrestaurateur == None :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun restaurateur.\n\nEtes-vous sûr de vouloir continuer ?"), _("Information"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
