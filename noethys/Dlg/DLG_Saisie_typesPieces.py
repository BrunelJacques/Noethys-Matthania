...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDtype_piece=None):
        ...
        self.parent = parent      
        self.SetTitle(_("Saisie d'un type de pièce"))
        self.IDtype_piece = IDtype_piece
        
        self.sizer_nom_staticbox = wx.StaticBox(self, -1, _("Caractéristiques"))
        ...
        self.label_rattachement = wx.StaticText(self, -1, _("Cochez cette case si, lorsqu'un individu est rattaché à plusieurs \nfamilles, cette pièce est valable pour toutes les familles rattachées."))
        
        self.sizer_duree_staticbox = wx.StaticBox(self, -1, _("Validité par défaut"))
        
        self.radio_duree_1 = wx.RadioButton(self, -1, _("Validité illimitée"), style=wx.RB_GROUP)
        
        self.radio_duree_2 = wx.RadioButton(self, -1, _("La durée suivante : "))
    ...
    def __set_properties(self):
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez ici un nom de pièce. Par exemple : 'Fiche sanitaire'")))
        self.ctrl_public.SetToolTip(wx.ToolTip(_("Sélectionnez le public auquel cette pièce s'adresse")))
        self.radio_duree_1.SetToolTip(wx.ToolTip(_("Sélectionnez 'Illimitée' si la pièce est valable à vie")))
        self.radio_duree_2.SetToolTip(wx.ToolTip(_("Sélectionnez 'Durée' si vous souhaitez définir une durée de validité pour cette pièce")))
        self.radio_duree_3.SetToolTip(wx.ToolTip(_("Sélectionnez 'Date' si la pièce n'est valable que jusqu'à une date précise")))
    ...
    def OnBoutonOk(self, event):
        ...
        if nom == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement donner un nom à cette pièce !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.ctrl_public.GetSelection() == -1 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner le public !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if date == None and self.radio_duree_3.GetValue() == True:
            dlg = wx.MessageDialog(self, _("La date de fin de validité n'est pas valide !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
