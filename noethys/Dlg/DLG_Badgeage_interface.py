...
LISTE_STYLES = [
    {"code" : "crystal", "label" : _("Crystal")},
    ]
    
LISTE_THEMES = [
    {"code" : "defaut", "label" : _("Défaut"), "image" : "Theme_defaut.png", "dlg" : {"couleurClaire" : wx.Colour(206, 196, 190), "couleurFoncee" : wx.Colour(169, 156, 146)}, },
    {"code" : "newyork", "label" : _("New-York"), "image" : "Theme_newyork.jpg", "dlg" : {"couleurClaire" : wx.Colour(186, 186, 186), "couleurFoncee" : wx.Colour(60, 60, 60)}, },
    {"code" : "ocean", "label" : _("Océan"), "image" : "Theme_ocean.jpg", "dlg" : {"couleurClaire" : wx.Colour(229, 195, 149), "couleurFoncee" : wx.Colour(2, 134, 183)}, },
    {"code" : "bleu", "label" : _("Bleu métal"), "image" : "Theme_bleu.jpg", "dlg" : {"couleurClaire" : wx.Colour(164, 182, 193), "couleurFoncee" : wx.Colour(63, 79, 94)}, },
    {"code" : "vert", "label" : _("Vert pomme"), "image" : "Theme_vert.jpg", "dlg" : {"couleurClaire" : wx.Colour(212, 238, 115), "couleurFoncee" : wx.Colour(71, 85, 24)}, },
    {"code" : "sommets", "label" : _("Sommets enneigés"), "image" : "Theme_sommets.jpg", "dlg" : {"couleurClaire" : wx.Colour(186, 186, 186), "couleurFoncee" : wx.Colour(60, 60, 60)}, },
    {"code" : "hiver", "label" : _("Ciel d'hiver "), "image" : "Theme_hiver.jpg", "dlg" : {"couleurClaire" : wx.Colour(111, 151, 255), "couleurFoncee" : wx.Colour(36, 67, 148)}, },
    {"code" : "noel", "label" : _("Noël "), "image" : "Theme_noel.jpg", "dlg" : {"couleurClaire" : wx.Colour(255, 98, 89), "couleurFoncee" : wx.Colour(120, 7, 15)}, },
    {"code" : "personnalise", "label" : _("Personnalisé"), "dlg" : {"couleurClaire" : wx.Colour(206, 196, 190), "couleurFoncee" : wx.Colour(169, 156, 146)}, },
    ]
...
class CTRL_Interface(wx.Panel):
    ...
    def OnTimer(self, event):
        ...
        if self.date != datetime.date.today() :
            ...
            self.log.AjouterAction(action=_("Changement de date automatique : %s") % DateEngFr(str(self.date)))
    ...
    def Procedure(self, IDindividu=None, date=None, heure=None):
        ...
        if self.dictProcedure["parametres"]["confirmation"] == 1 and self.importationManuelle == False :
            if self.dictProcedure["parametres"]["tutoiement"] == 1 :
                message = _("Confirmes-tu être %s %s ?") % (prenom, nom)
            else :
                message = _("Confirmez-vous être %s %s ?") % (prenom, nom)
        ...
        self.log.AjouterAction(individu=nomIndividu, IDindividu=IDindividu, action=_("Identification de %s (ID%d)") % (nomIndividu, IDindividu), resultat=True)
...
##                    listeReponses = DB.ResultatReq()     
    ...
    def RechercheInscription(self, IDindividu, nomIndividu, IDactivite, dictActivite, nomAction=u""):
        ...
        if len(listeIDfamille) == 0 :
            ...
            self.log.AjouterAction(individu=nomIndividu, IDindividu=IDindividu, action=nomAction, resultat=_("Individu non inscrit à l'activité"))
            if self.infosIndividus.RechercheIndividu(IDindividu)["genre"] == "F" :
                feminin = "e"
            else :
                feminin = ""
            if self.dictProcedure["parametres"]["tutoiement"] == 1 :
                message = _("Tu n'es pas inscrit%s à l'activité '%s' !") % (feminin, dictActivite["nom"])
            else :
                message = _("Vous n'êtes pas inscrit%s à l'activité '%s' !") % (feminin, dictActivite["nom"])
            ...
            dlg = DIALOGUES.DLG_Choix(self, message=_("Sur quel dossier faut-il facturer l'activité '%s' ?") % dictActivite["nom"], listeItems=listeNomsTitulaires, multiSelection=False)
            reponse = dlg.ShowModal()
            dlg.Destroy()
            if reponse == wx.ID_YES :
                index = dlg.GetSelections()[0]
                IDfamille, IDgroupe = listeIDfamille[index]
                self.log.AjouterAction(individu=nomIndividu, IDindividu=IDindividu, action=_("Choix d'une famille à facturer sur l'activité '%s' : %s.") % (dictActivite["nom"], listeNomsTitulaires[index]), resultat=True)
            else :
                self.log.AjouterAction(individu=nomIndividu, IDindividu=IDindividu, action=_("Choix d'une famille à facturer sur l'activité '%s'.") % dictActivite["nom"], resultat=_("Annulation lors du choix de la famille rattachée."))
                return False, False
    ...
    def Procedure_enregistrer(self, dictAction, IDindividu, date, heure):
        ...
        nomIndividu = "%s %s" % (self.infosIndividus.RechercheIndividu(IDindividu)["nom"], self.infosIndividus.RechercheIndividu(IDindividu)["prenom"])
        nomAction = _("Enregistrement d'une consommation '%s'") % dictUnites[IDunite]["nom"]
        ...
        if (IDunite, IDgroupe) not in listeOuvertures :
            self.log.AjouterAction(individu=nomIndividu, IDindividu=IDindividu, action=nomAction, resultat=_("Unité '%s' fermée le %s") % (dictUnites[IDunite]["nom"], DateEngFr(str(date))))
            return False
        ...
        if demande == 1 :
            if self.dictProcedure["parametres"]["tutoiement"] == 1 :
                texte = _("Est-ce que tu arrives ? Ou est-ce que tu pars ?")
            else :
                texte = _("Est-ce que vous arrivez ? Ou est-ce que vous partez ?")
            listeChoix = [_("J'arrive"), _("Je pars")]
            ...
            if heure_debut == "pointee" and heure_fin == "pointee":
                ...
                if case != None:
                    for conso in case.GetListeConso():
                        ...
                        if case.GetTypeUnite() != "Multihoraires" and conso.badgeage_debut != None and conso.badgeage_fin != None:
                            DIALOGUES.DLG_Message(self, message=_("Les heures d'arrivée et de départ ont déjà été enregistrées !"), icone="erreur")
                            self.log.AjouterAction(individu=nomIndividu, IDindividu=IDindividu, action=nomAction, resultat=_("Unité '%s' déjà badgée à l'arrivée et au départ !") % dictUnites[IDunite]["nom"])
                            return False
                ...
                if conso_a_modifier == None :
                    ...
                    if UTILS_Dates.HeureStrEnDelta(heureFin) - UTILS_Dates.HeureStrEnDelta(heureDebut) < UTILS_Dates.HeureStrEnDelta("00:01"):
                        DIALOGUES.DLG_Message(self, message=_("L'heure d'arrivée a été enregistrée il y a moins d'une minute !"), icone="erreur")
                        self.log.AjouterAction(individu=nomIndividu, IDindividu=IDindividu, action=nomAction, resultat=_("Unité '%s' badgée il y a moins d'une minute!") % dictUnites[IDunite]["nom"])
                        return False
...
##            print case.heure_debut, case.heure_fin, case.etat
    ...
    def Procedure_reserver(self, dictAction, IDindividu, date, heure):
        ...
        if dateTmp == "date_actuelle" :
            ...
            if dateTmp == None :
                self.log.AjouterAction(action=_("Réservation de consommations"), resultat=_("Pas d'ouvertures futures pour cette activité"))
                return False
        else :
            return False
        ...
        nomIndividu = "%s %s" % (self.infosIndividus.RechercheIndividu(IDindividu)["nom"], self.infosIndividus.RechercheIndividu(IDindividu)["prenom"])
        nomAction = _("Réservation de consommations '%s'") % dictActivite["nom"]
        ...
        if len(listeUnitesOuvertes) == 0 :
            self.log.AjouterAction(individu=nomIndividu, IDindividu=IDindividu, action=nomAction, resultat=_("Aucune unité ouverte le %s") % DateEngFr(str(dateTmp)))
            return False
        ...
        if len(listeUnitesOuvertes) == 1 :
            ...
            if reponse == wx.ID_YES :
                ...
                self.log.AjouterAction(individu=nomIndividu, IDindividu=IDindividu, action=nomAction, resultat=_("Annulation par l'individu"))
                return False
        ...
        if len(listeUnitesCompletes) > 0 :
            texte = _("Désolé mais il est possible qu'il n'y ait plus de places. Contactez un responsable.")
            DIALOGUES.DLG_Message(self, message=texte, icone="exclamation")
            self.log.AjouterAction(individu=nomIndividu, IDindividu=IDindividu, action=nomAction, resultat=_("Plus de places sur les unités %s") % ", ".join(listeUnitesCompletes))
            return False
    ...
    def ImportationBadgeage(self):
        ...
        for track in self.importationManuelle :
            ...
            if IDindividu == None :
                self.log.AjouterAction(individu="", IDindividu=None, action=_("Identification d'un code-barres"), resultat=_("Code-barres inconnu ('%s')") % track.codebarres)
            else :
...
class Dialog(wx.Dialog):
    def __init__(self, parent, log=None, IDprocedure=None, date=None, dateauto=False, importationManuelle=False, style=wx.BORDER_NONE, mode_debug=False):
        ...
        try :
            if importationManuelle == False :
                texte = _("Veuillez patienter durant l'initialisation de l'interface de badgeage...")
            else :
                texte = _("Veuillez patienter durant l'importation des badgeages...")
            dlgAttente = wx.BusyInfo(_("Initialisation..."), None)
            ...
            dlg = wx.MessageDialog(self, _("Désolé, le problème suivant a été rencontré dans l'initialisation de l'interface de badgeage : \n\n%s") % err, _("Erreur"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            self.Destroy()
