...
from Ol import OL_Modeles_commandes


class Dialog(wx.Dialog):
    def __init__(self, parent):
        wx.Dialog.__init__(self, parent, -1, name="DLG_Modeles_commandes", style=wx.DEFAULT_DIALOG_STYLE|wx.RESIZE_BORDER|wx.MAXIMIZE_BOX|wx.MINIMIZE_BOX)
        self.parent = parent

        intro = _("Vous pouvez ici créer, modifier ou supprimer des modèles de commandes de repas. Ces modèles permettent de renseigner notamment l'identité du restaurateur et les différentes colonnes qui composeront le tableau des commandes.")
        titre = _("Modèles de commandes de repas")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Repas.png")
        self.ctrl_listview = OL_Modeles_commandes.ListView(self, id=-1, style=wx.LC_REPORT|wx.SUNKEN_BORDER|wx.LC_SINGLE_SEL|wx.LC_HRULES|wx.LC_VRULES)
        self.ctrl_listview.MAJ()
        self.ctrl_recherche = OL_Modeles_commandes.CTRL_Outils(self, listview=self.ctrl_listview)
    ...
    def __set_properties(self):
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter un modèle")))
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier le modèle sélectionné dans la liste")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer le modèle sélectionné dans la iste")))
        self.bouton_defaut.SetToolTip(wx.ToolTip(_("Cliquez ici pour définir le modèle sélectionné comme celui par défaut")))
