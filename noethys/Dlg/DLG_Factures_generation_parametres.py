...
class CTRL_Prefixe_factures(wx.Choice):
    ...
    def GetListeDonnees(self):
        ...
        listeDonnees = db.ResultatReq()
        db.Close()
        listeItems = [_("Aucun préfixe"),]
        self.dictDonnees = {}
        self.dictDonnees[0] = { "ID" : None, "nom" : _("Aucun préfixe"), "prefixe" : None}
        index = 1
        for IDprefixe, nom, prefixe in listeDonnees :
            self.dictDonnees[index] = { "ID" : IDprefixe, "nom" : nom, "prefixe" : prefixe}
            label = "%s - %s" % (prefixe, nom)
...
class CTRL_Famille(wx.Choice):
    ...
    def GetListeDonnees(self):
        ...
        listeItems = [u"",]
        self.dictDonnees = {}
        self.dictDonnees[0] = { "IDfamille" : 0, "nom" : _("Inconnue"), "IDcompte_payeur" : 0 }
        index = 1
...
class Panel(wx.Panel):
    def __init__(self, parent):
        ...
        # Période
        self.box_periode_staticbox = wx.StaticBox(self, -1, _("Période"))
        ...
        # Lot
        self.box_lot_staticbox = wx.StaticBox(self, -1, _("Lot de factures"))
        self.label_lot = wx.StaticText(self, -1, _("Lot :"))
        ...
        # Paramètres
        self.label_prefixe = wx.StaticText(self, -1, _("Préfixe de numéro :"))
        ...
        self.label_prochain_numero = wx.StaticText(self, -1, _("Prochain numéro :"))
        self.ctrl_prochain_numero = wx.TextCtrl(self, -1, "", size=(95, -1))
        ...
        self.check_numero_auto = wx.CheckBox(self, -1, _("Auto."))
        self.box_parametres_staticbox = wx.StaticBox(self, -1, _("Paramètres"))
        self.label_date_emission = wx.StaticText(self, -1, _("Date d'émission :"))
        self.ctrl_date_emission = CTRL_Saisie_date.Date2(self)
        self.label_date_echeance = wx.StaticText(self, -1, _("Date d'échéance :"))
        self.ctrl_date_echeance = CTRL_Saisie_date.Date2(self)
        self.label_mention1 = wx.StaticText(self, -1, _("Mention 1 :"))
        self.ctrl_mention1 = wx.TextCtrl(self, -1, "")
        self.label_mention2 = wx.StaticText(self, -1, _("Mention 2 :"))
        self.ctrl_mention2 = wx.TextCtrl(self, -1, "")
        self.label_mention3 = wx.StaticText(self, -1, _("Mention 3 :"))
        self.ctrl_mention3 = wx.TextCtrl(self, -1, "")

        # Elements
        self.box_elements_staticbox = wx.StaticBox(self, -1, _("Prestations à facturer"))
        self.check_consommations = wx.CheckBox(self, -1, _("Consommations"))
        self.check_cotisations = wx.CheckBox(self, -1, _("Cotisations"))
        self.check_locations = wx.CheckBox(self, -1, _("Locations"))
        self.check_autres = wx.CheckBox(self, -1, _("Autres"))
        
        # Familles
        self.box_familles_staticbox = wx.StaticBox(self, -1, _("Sélection des familles"))
        self.radio_familles_toutes = wx.RadioButton(self, -1, _("Toutes les familles"), style=wx.RB_GROUP)
        self.radio_familles_unique = wx.RadioButton(self, -1, _("Uniquement la famille suivante :"))
        ...
        self.check_prestations_anterieures = wx.CheckBox(self, -1, _("Inclure les prestations antérieures depuis le"))
    ...
    def __set_properties(self):
        self.ctrl_prefixe.SetToolTip(wx.ToolTip(_("Sélectionnez un préfixe de numéro de factures dans la liste proposée. Cet paramètre permet d'obtenir des numéros de facture de type 'ABC-00001'.")))
        self.bouton_prefixes.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à la gestion des préfixes de factures")))
        self.ctrl_lot.SetToolTip(wx.ToolTip(_("Sélectionnez un nom de lot à associer aux factures générées. Ex : Janvier 2013, Février, 2013, etc... Ce nom vous permettra de retrouver vos factures facilement [Optionnel]")))
        self.bouton_lots.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à la gestion des lots")))
        self.ctrl_prochain_numero.SetToolTip(wx.ToolTip(_("Numéro de la prochaine facture générée. Vous pouvez modifier ce numéro si vous souhaitez par exemple modifier la numérotation en début d'année")))
        self.bouton_prochain_numero.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner automatiquement le prochain numéro de facture")))
        self.check_numero_auto.SetToolTip(wx.ToolTip(_("Cochez cette case pour laisser Noethys sélectionner automatiquement le prochain numéro de facture")))
        self.ctrl_date_emission.SetToolTip(wx.ToolTip(_("Saisissez ici la date d'émission la date des factures (Par défaut la date du jour)")))
        self.ctrl_date_echeance.SetToolTip(wx.ToolTip(_("Saisissez la date d'échéance de paiement qui apparaîtra sur la facture [Optionnel]")))
        self.ctrl_mention1.SetToolTip(wx.ToolTip(_("Saisissez une mention libre")))
        self.ctrl_mention2.SetToolTip(wx.ToolTip(_("Saisissez une mention libre")))
        self.ctrl_mention3.SetToolTip(wx.ToolTip(_("Saisissez une mention libre")))
        self.check_consommations.SetToolTip(wx.ToolTip(_("Cochez cette case pour inclure les prestations de consommations")))
        self.check_cotisations.SetToolTip(wx.ToolTip(_("Cochez cette case pour inclure les prestations de cotisations")))
        self.check_locations.SetToolTip(wx.ToolTip(_("Cochez cette case pour inclure les prestations de locations")))
        self.check_autres.SetToolTip(wx.ToolTip(_("Cochez cette case pour inclure les autres types de prestations")))
        self.radio_familles_toutes.SetToolTip(wx.ToolTip(_("Cliquez ici pour rechercher les factures de toutes les familles (par défaut)")))
        self.radio_familles_unique.SetToolTip(wx.ToolTip(_("Cliquez ici pour rechercher les factures d'une seule famille")))
        self.ctrl_famille.SetToolTip(wx.ToolTip(_("Sélectionnez ici une famille")))
        self.check_prestations_anterieures.SetToolTip(wx.ToolTip(_("Cochez cette case pour inclure les prestations antérieures à la date de début non facturées depuis la date souhaitée")))
    ...
    def Validation(self):
        ...
        if IDlot == None :
            dlg = wx.MessageDialog(self, _("Vous n'avez pas sélectionné de lot de factures à associer.\n\nSouhaitez-vous quand même continuer ?"), _("Confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
        ...
        if self.check_numero_auto.GetValue() == True :
            ...
            if prochain_numero in (None, "") :
                dlg = wx.MessageDialog(self, _("Le prochain numéro de facture ne semble pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
            ...
            if prochain_numero < self.prochain_numero_defaut :
                dlg = wx.MessageDialog(self, _("Le prochain numéro de facture n'est pas valide : une facture générée porte déjà un numéro égal ou supérieur !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_prochain_numero.SetFocus()
                return False
        ...
        if self.ctrl_date_emission.FonctionValiderDate() == False or date_emission == None :
            dlg = wx.MessageDialog(self, _("La date d'émission ne semble pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if date_echeance == None :
            dlg = wx.MessageDialog(self, _("Vous n'avez pas saisi de date d'échéance. \n\nSouhaitez-vous quand même continuer ?"), _("Confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
            resultat = dlg.ShowModal()
            dlg.Destroy()
            if resultat != wx.ID_YES :
                return False
        ...
        if len(prestations) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement cocher au moins un type de prestation à facturer !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if self.radio_familles_toutes.GetValue() == True :
            ...
            if IDcompte_payeur_unique == None :
                dlg = wx.MessageDialog(self, _("Vous avez sélectionné l'option 'famille unique' mais sans sélectionner de famille dans la liste proposée !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        ...
        if len(listeActivites) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune activité.\n\nSouhaitez-vous quand même continuer ?"), _("Avertissement"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
            resultat = dlg.ShowModal()
            dlg.Destroy()
            if resultat != wx.ID_YES :
                return False
        ...
        if self.check_prestations_anterieures.GetValue() == True :
            ...
            if self.ctrl_date_anterieures.FonctionValiderDate() == False or date_anterieure == None:
                dlg = wx.MessageDialog(self, _("La date antérieure ne semble pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_date_anterieures.SetFocus()
                return False
        ...
        for IDactivite in listeActivites :
            if UTILS_Utilisateurs.VerificationDroitsUtilisateurActuel("facturation_factures", "creer", IDactivite=IDactivite, afficheMessage=False) == False : 
                dlg = wx.MessageDialog(self, _("Vous n'avez pas l'autorisation de générer des factures pour l'ensemble des activités sélectionnées !"), _("Action non autorisée"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        ...
        if len(listeActivites) >= 1 :
            ...
            if len(listeRegies) > 1 :
                dlg = wx.MessageDialog(self, _("Vous ne pouvez pas générer des factures pour l'ensemble des activités sélectionnées !\n\nCertaines activités sont liées à des régies différentes"), _("Régies différentes"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
            else :
