...
class Dialog(wx.Dialog):
    def __init__(self, parent, categorie=None):
        ...
        # Bandeau
        titre = _("Traductions")
        intro = _("Cette liste présente les langues disponibles sur cet ordinateur. Vous pouvez ici ajouter, modifier ou supprimer des traductions. Il est ensuite possible de partager vos traductions personnalisées avec la communauté en les envoyant au concepteur de Noethys.")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Traduction.png")
                
        # Modèles
        self.staticbox_modeles_staticbox = wx.StaticBox(self, -1, _("Langues disponibles"))
        ...
        self.hyper_proposer = Hyperlien(self, label=_("Partager un fichier de traduction avec la communauté"), infobulle=_("Partager un fichier de traduction avec la communauté"), URL="proposer")
        self.label_separation = wx.StaticText(self, -1, "|") 
        self.hyper_importer = Hyperlien(self, label=_("Importer/Exporter les textes au format TXT"), infobulle=_("Importer/Exporter les textes au format TXT"), URL="importer")
    ...
    def __set_properties(self):
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour créer une nouvelle traduction")))
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier la traduction sélectionnée dans la liste")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer la traduction sélectionnée dans la liste")))
        self.bouton_importer.SetToolTip(wx.ToolTip(_("Cliquez ici pour importer une traduction (.xlang)")))
        self.bouton_exporter.SetToolTip(wx.ToolTip(_("Cliquez ici pour exporter la traduction personnalisée sélectionnée dans la liste (.xlang)")))
...
class DLG_Envoi(wx.Dialog):
    def __init__(self, parent, texteRapport=u""):
        ...
        self.SetTitle(_("Envoyer un fichier de traduction"))
        self.label_ligne_1 = wx.StaticText(self, wx.ID_ANY, _("Vos fichiers seront envoyés pour validation au concepteur de Noethys\npuis intégrés dans la prochaine version du logiciel. En effectuant cette\ndémarche, vous acceptez que votre traduction soit diffusée auprès de\nla communauté sous la licence libre GNU GPL."))
        self.label_ligne_2 = wx.StaticText(self, wx.ID_ANY, _("Cochez le ou les fichiers de traduction à envoyer :"))
        ...
        self.label_ligne_3 = wx.StaticText(self, wx.ID_ANY, _("Vous pouvez ajouter ci-dessous des commentaires, remarques ou \ncompléments d'informations avant de l'envoyer au concepteur de Noethys."))
    ...
    def OnBoutonEnvoyer(self, event):  
        ...
        if len(listeFichiers) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement cocher un ou plusieurs fichiers de traduction personnalisés à envoyer !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        
        # Confirmation
        dlg = wx.MessageDialog(self, _("Confirmez-vous l'envoi de ce message ?"), _("Envoi d'une traduction"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
    ...
    def EnvoyerEmail(self, commentaires="", listeFichiers=[]):
        ...
        texteMail = _("<u>Envoi de %d fichier(s) de traduction pour Noethys :</u><br/><br/>%s<br/><br/><u>Commentaires :</u><br/><br/>%s") % (len(listeFichiers), ", ".join(listeFichiers), commentaires)
        ...
        if dictExp == None :
            dlg = wx.MessageDialog(self, _("Vous devez d'abord saisir une adresse d'expéditeur depuis le menu Paramétrage > Adresses d'expédition d'Emails."), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        msg['Subject'] = _("Envoi de fichiers de traduction Noethys")
        ...
        try :
            ...
            dlg = wx.MessageDialog(self, _("Le message n'a pas pu être envoyé.\n\nErreur : %s !") % err, _("Envoi impossible"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        
        # Message de confirmation
        dlg = wx.MessageDialog(self, _("Le message a été envoyé avec succès."), _("Traduction envoyée"), wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
