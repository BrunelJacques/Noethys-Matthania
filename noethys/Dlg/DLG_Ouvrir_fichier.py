...
class MyDialog(wx.Dialog):
    def __init__(self, parent, fichierOuvert=None):
        ...
        # Bandeau
        titre = _("Ouvrir un fichier")
        intro = _("Sélectionnez le mode Local pour afficher les fichiers disponibles sur cet ordinateur. Pour afficher les fichiers réseau, sélectionnez le mode Réseau, saisissez les codes d'accès MySQL puis cliquez sur le bouton Valider. Double-cliquez sur le nom d'un fichier dans la liste pour l'ouvrir.")
        ...
        # Mode
        self.stbMode = wx.StaticBox(self, -1, _("Mode"))
        self.radio_local = wx.RadioButton(self.stbMode, -1, _("Local"), style=wx.RB_GROUP)
        self.radio_reseau = wx.RadioButton(self.stbMode, -1, _("Réseau"))
        
        # Codes d'accès
        self.stbCodes = wx.StaticBox(self, -1, _("Codes d'accès réseau"))
        self.label_port = wx.StaticText(self.stbCodes, -1, _("Port :"))
        self.ctrl_port = wx.TextCtrl(self.stbCodes, -1, "3306", style=wx.TE_CENTRE)
        self.label_hote = wx.StaticText(self.stbCodes, -1, _("Hôte :"))
        self.ctrl_hote = wx.TextCtrl(self.stbCodes, -1, "")
        self.label_utilisateur = wx.StaticText(self.stbCodes, -1, _("Utilisateur :"))
        self.ctrl_utilisateur = wx.TextCtrl(self.stbCodes, -1, "")
        self.label_motdepasse = wx.StaticText(self.stbCodes, -1, _("Mot de passe :"))
        self.ctrl_motdepasse = wx.TextCtrl(self.stbCodes, -1, "", style=wx.TE_PASSWORD)
        self.bouton_valider_codes = wx.Button(self.stbCodes, -1, _("Valider"), style=wx.BU_EXACTFIT)
        self.bouton_importer_codes = wx.BitmapButton(self.stbCodes, -1, wx.Bitmap(Chemins.GetStaticPath(u"Images/16x16/Document_import.png"), wx.BITMAP_TYPE_ANY))
        self.bouton_exporter_codes = wx.BitmapButton(self.stbCodes, -1, wx.Bitmap(Chemins.GetStaticPath(u"Images/16x16/Document_export.png"), wx.BITMAP_TYPE_ANY))

        
        # Liste fichiers
        self.stbFichiers = wx.StaticBox(self, -1, _("Liste des fichiers"))
        self.ctrl_fichiers = CTRL_Liste_fichiers.CTRL(self.stbFichiers, mode="local")
        self.ctrl_fichiers.SetMinSize((-1, 400))
        self.bouton_modifier_fichier = wx.BitmapButton(self.stbFichiers, -1, wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Modifier.png"), wx.BITMAP_TYPE_ANY))
        self.bouton_supprimer_fichier = wx.BitmapButton(self.stbFichiers, -1, wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Supprimer.png"), wx.BITMAP_TYPE_ANY))
        self.ctrl_image = wx.StaticBitmap(self.stbFichiers, -1, wx.Bitmap(Chemins.GetStaticPath(u"Images/16x16/Updater.png"), wx.BITMAP_TYPE_ANY))
        self.hyper_telecharger = Hyperlien(self.stbFichiers, label=_("Télécharger d'autres fichiers exemples"), infobulle=_("Cliquez ici pour télécharger d'autres fichiers exemples sur internet"), URL="telecharger")
    ...
    def __set_properties(self):
        self.radio_local.SetToolTip(wx.ToolTip(_("Cliquez ici pour afficher les fichiers disponibles en mode local")))
        self.radio_reseau.SetToolTip(wx.ToolTip(_("Cliquez ici pour afficher les fichiers disponibles en mode réseau")))
        self.ctrl_port.SetMinSize((40, -1))
        self.ctrl_port.SetToolTip(wx.ToolTip(_("Le numéro de port est 3306 par défaut")))
        ...
        self.ctrl_utilisateur.SetToolTip(wx.ToolTip(_("Indiquez ici le nom de l'utilisateur")))
        self.ctrl_motdepasse.SetToolTip(wx.ToolTip(_("Indiquez ici le mot de passe nécessaire à la connexion à MySQL")))
        self.bouton_valider_codes.SetToolTip(wx.ToolTip(_("Cliquez ici pour valider les codes réseau et afficher la liste des fichiers disponibles")))
        self.bouton_importer_codes.SetToolTip(wx.ToolTip(_("Importer une configuration de fichier réseau")))
        self.bouton_exporter_codes.SetToolTip(wx.ToolTip(_("Exporter une configuration de fichier réseau")))
        self.bouton_modifier_fichier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier le nom du fichier sélectionné dans la liste")))
        self.bouton_supprimer_fichier.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer le fichier sélectionné dans la liste")))
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez ici pour obtenir de l'aide")))
        self.bouton_ok.SetToolTip(wx.ToolTip(_("Cliquez ici pour ouvrir le fichier sélectionné dans la liste")))
    ...
    def __do_layout(self):
        ...
        box_fichiers = wx.StaticBoxSizer(self.stbFichiers, wx.VERTICAL)
        ...
        box_codes = wx.StaticBoxSizer(self.stbCodes, wx.VERTICAL)
        grid_sizer_codes = wx.FlexGridSizer(1, 12, 5, 5)
        box_mode = wx.StaticBoxSizer(self.stbMode, wx.VERTICAL)
    ...
    def OnBoutonImporterCodes(self, event):
        wildcard = _("Paramétrage fichier réseau Noethys (*.nnc)|*.nnc|Tous les fichiers (*.*)|*.*")
        sp = wx.StandardPaths.Get()
        dlg = wx.FileDialog(
            self, message=_("Choisissez un fichier à importer"),
    ...
    def OnBoutonValiderCodes(self, event):
        ...
        if dictCodes["port"] == "" :
            dlg = wx.MessageDialog(self, _("Le numéro de port n'est pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if dictCodes["hote"] == "" :
            dlg = wx.MessageDialog(self, _("Vous devez saisir un nom pour le serveur hôte !"), _("Erreur de saisie"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if test != True  :
            dlg = wx.MessageDialog(self, _("Erreur de connexion MySQL :\n\n%s") % test, _("Erreur de connexion"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
    ...
    def OnBoutonModifierFichier(self, event):
        ...
        if index == -1 :
            dlg = wx.MessageDialog(self, _("Vous devez sélectionner un fichier à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return 
        titre = self.ctrl_fichiers.GetItemPyData(index)["titre"]
        if self.fichierOuvert == titre :
            dlg = wx.MessageDialog(self, _("Vous ne pouvez pas modifier un fichier déjà ouvert !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return 
    ...
    def OnBoutonSupprimerFichier(self, event):
        ...
        index = self.ctrl_fichiers.GetFirstSelected()
        if index == -1 :
            dlg = wx.MessageDialog(self, _("Vous devez sélectionner un fichier à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return 
        titre = self.ctrl_fichiers.GetItemPyData(index)["titre"]
        if self.fichierOuvert == titre :
            dlg = wx.MessageDialog(self, _("Vous ne pouvez pas supprimer un fichier déjà ouvert !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return 
    ...
    def GetNomFichier(self):
        ...
        if modeLocal == False :
            ...
            nomFichier = "%s;%s;%s;%s[RESEAU]%s" % (port, hote, utilisateur, motdepasse, fichier)
    ...
    def OnBoutonOk(self, event): 
        index = self.ctrl_fichiers.GetFirstSelected()
        if index == -1 :
            dlg = wx.MessageDialog(self, _("Vous devez sélectionner un fichier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return 
        ...
        titre = self.ctrl_fichiers.GetItemPyData(index)["titre"]
        if self.fichierOuvert == titre :
            dlg = wx.MessageDialog(self, _("Vous ne pouvez pas ouvrir un fichier déjà ouvert !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return 
