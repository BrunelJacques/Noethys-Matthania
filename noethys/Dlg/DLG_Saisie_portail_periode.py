...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDperiode=None, IDactivite=None):
        ...
        if self.IDperiode == None :
            titre = _("Saisie d'une nouvelle période de réservations")
        else :
            titre = _("Modification d'une période de réservations")
        self.SetTitle(titre)
        intro = _("Définissez ici une période en renseignant le nom de la période, la période correspondante dans le calendrier des ouvertures et une éventuelle période daffichage sur le portail.")
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Emails_exp.png")
        
        # Nom
        self.box_nom_staticbox = wx.StaticBox(self, -1, _("Nom de la période"))
        self.ctrl_nom = wx.TextCtrl(self, -1, "")

        # Introduction
        self.box_intro_staticbox = wx.StaticBox(self, -1, _("Texte d'introduction"))
        self.ctrl_intro = wx.TextCtrl(self, -1, "", style=wx.TE_MULTILINE)
        self.ctrl_intro.SetMinSize((10, 50))

        # Période
        self.box_periode_staticbox = wx.StaticBox(self, -1, _("Période de réservations"))
        ...
        # Affichage
        self.box_affichage_staticbox = wx.StaticBox(self, -1, _("Affichage sur le portail"))
        self.radio_oui = wx.RadioButton(self, -1, _("Toujours afficher"), style=wx.RB_GROUP)
        self.radio_dates = wx.RadioButton(self, -1, _("Afficher uniquement sur la période suivante :"))
        self.label_affichage_date_debut = wx.StaticText(self, -1, _("Du"))
        self.ctrl_affichage_date_debut = CTRL_Saisie_date.Date2(self)
        self.ctrl_affichage_heure_debut = CTRL_Saisie_heure.Heure(self)
        self.label_affichage_date_fin = wx.StaticText(self, -1, _("au"))
        self.ctrl_affichage_date_fin = CTRL_Saisie_date.Date2(self)
        self.ctrl_affichage_heure_fin = CTRL_Saisie_heure.Heure(self)
        self.radio_non = wx.RadioButton(self, -1, _("Ne pas afficher"))

        # Modèle d'Email de réponse associé
        self.box_modele_staticbox = wx.StaticBox(self, -1, _("Modèle d'Email de réponse"))
        self.radio_modele_defaut = wx.RadioButton(self, -1, _("Utiliser le modèle par défaut"), style=wx.RB_GROUP)
        self.radio_modele_choix = wx.RadioButton(self, -1, _("Utiliser le modèle suivant :"))
        ...
        # Préfacturation
        self.box_prefacturation_staticbox = wx.StaticBox(self, -1, _("Préfacturation"))
        self.ctrl_prefacturation = wx.CheckBox(self, -1, _("Activer la préfacturation pour cette période"))
    ...
    def __set_properties(self):
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez ici un nom pour cette période (Ex : 'Juillet 2016', 'Année 2016', etc...)")))
        self.ctrl_intro.SetToolTip(wx.ToolTip(_("Saisissez ici un texte d'introduction qui apparaîtra au-dessus du planning des réservations [Optionnel]")))
        self.radio_oui.SetToolTip(wx.ToolTip(_("Sélectionnez cette option pour afficher cette période sur le portail")))
        self.radio_dates.SetToolTip(wx.ToolTip(_("Sélectionnez cette option pour afficher cette période sur le portail uniquement entre les dates souhaitées")))
        self.radio_non.SetToolTip(wx.ToolTip(_("Sélectionnez cette option pour ne pas afficher cette période sur le portail")))
        self.radio_modele_defaut.SetToolTip(wx.ToolTip(_("Sélectionnez cette option pour utiliser le modèle d'Email par défaut pour l'envoi des réponses aux demandes")))
        self.radio_modele_choix.SetToolTip(wx.ToolTip(_("Sélectionnez cette option pour utiliser un modèle d'Email spécifique pour les réponses aux demandes")))
        self.ctrl_modele_email.SetToolTip(wx.ToolTip(_("Sélectionnez le modèle d'Email à associer à la période")))
        self.bouton_gestion_modeles.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à la gestion des modèles d'Emails")))
        self.ctrl_prefacturation.SetToolTip(wx.ToolTip(_("Cochez cette case pour que le solde à régler pour cette période apparaisse sur le portail")))
    ...
    def OnBoutonOk(self, event):
        ...
        if nom == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un nom pour cette période !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if date_debut == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de début pour cette période !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if date_fin == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de fin pour cette période !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.radio_dates.GetValue() == True :
            ...
            if affichage_date_debut == None :
                dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de début pour l'affichage !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
            ...
            if affichage_date_fin == None :
                dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de fin pour l'affichage !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
            ...
            if affichage_date_debut > affichage_date_fin :
                dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de fin supérieure à la date de début pour l'affichage !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_affichage_date_fin.SetFocus()
                return
        ...
        if self.radio_modele_choix.GetValue() == True :
            ...
            if IDmodele == None :
                dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun modèle d'Email dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return
        else :
