...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDlot=None):
        ...
        intro = _("Cochez les factures que vous souhaitez ajouter au lot de prélèvements puis cliquez sur le bouton 'Ok'.")
        titre = _("Ajouter des factures au prélèvement")
        self.SetTitle(titre)
    ...
    def OnBoutonOk(self, event):
        ...
        if len(tracks) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune facture à inclure dans votre lot de prélèvements !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        for track in tracks :
            ...
            if len(listeAutresLots) > 0 :
                message1 = _("La facture n°%s est déjà présente dans les autres lots de prélèvements suivants. Souhaitez-vous tout de même l'inclure de votre lot de prélèvements actuel ?") % track.numero
        ...
        if len(listeErreursSoldes) > 0 :
            dlg = wx.MessageDialog(self, _("Vous avez sélectionné %d factures qui ont déjà été payées !") % len(listeErreursSoldes), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
