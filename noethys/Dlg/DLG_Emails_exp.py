...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        intro = _("Vous pouvez ici créer, modifier ou supprimer les adresses d'expéditions d'Emails. Celles-ci sont obligatoires pour pouvoir envoyer des Emails à partir du système de messagerie intégré à Noethys.")
        titre = _("Gestion des adresses d'expédition d'Emails")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Emails_exp.png")

        self.ctrl_listview = OL_Emails_exp.ListView(self, id=-1, style=wx.LC_REPORT | wx.SUNKEN_BORDER | wx.LC_SINGLE_SEL | wx.LC_HRULES | wx.LC_VRULES)
        self.ctrl_listview.MAJ()
        self.ctrl_recherche = OL_Emails_exp.CTRL_Outils(self, listview=self.ctrl_listview)
    ...
    def __set_properties(self):
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter une adresse")))
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier l'adresse sélectionnée dans la liste")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer l'adresse sélectionnée dans la iste")))
        self.bouton_defaut.SetToolTip(wx.ToolTip(_("Cliquez ici pour définir l'adresse sélectionnée comme celle par défaut")))
