...
from Ol import OL_Niveaux_scolaires
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        intro = _("Vous pouvez ici saisir, modifier ou supprimer des niveaux scolaires. Ceux-ci peuvent ensuite être attribués à des classes.")
        titre = _("Gestion des niveaux scolaires")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Niveau_scolaire.png")
        self.ctrl_listview = OL_Niveaux_scolaires.ListView(self, id=-1, style=wx.LC_REPORT|wx.SUNKEN_BORDER|wx.LC_SINGLE_SEL|wx.LC_HRULES|wx.LC_VRULES)
        self.ctrl_listview.MAJ()
        self.ctrl_recherche = OL_Niveaux_scolaires.CTRL_Outils(self, listview=self.ctrl_listview)
    ...
    def __set_properties(self):
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter un niveau")))
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier le niveau sélectionné dans la liste")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer le niveau sélectionné dans la liste")))
        self.bouton_monter.SetToolTip(wx.ToolTip(_("Cliquez ici pour monter le niveau sélectionné dans la liste")))
        self.bouton_descendre.SetToolTip(wx.ToolTip(_("Cliquez ici pour descendre le niveau sélectionné dans la liste")))
    ...
    def Monter(self, event):
        self.ctrl_listview.Monter(None)
        
    def Descendre(self, event):
        self.ctrl_listview.Descendre(None)
