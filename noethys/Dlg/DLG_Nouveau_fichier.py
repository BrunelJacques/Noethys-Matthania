...
class PanelReseau(wx.Panel):
    def __init__(self, parent, ID=-1):
        ...
        self.label_hote = wx.StaticText(self, -1, _("Hôte :"), size=(-1, -1), style=wx.ALIGN_RIGHT)
...
class MyDialog(wx.Dialog):
    def __init__(self, parent):
        wx.Dialog.__init__(self, parent, -1, title=_("Création d'un fichier"))
        self.parent = parent
        
        self.listeTablesImportation = Tables.TABLES_IMPORTATION_OPTIONNELLES

        # Bandeau
        titre = _("Créer un nouveau fichier")
        intro = _("Sélectionnez le type de fichier à créer puis renseignez les paramètres demandés : le nom du fichier et l'identité de l'administrateur du fichier. Pour un fichier réseau, saisissez les codes MySQL.")
    ...
    def __set_properties(self):
        self.checkbox_details.SetToolTip(wx.ToolTip(_("Il est recommandé de conserver cette case\n cochée afin d'importer les données par défaut")))
        ...
        self.radio_local.SetToolTip(wx.ToolTip(_("Le mode local est utilisé pour une utilisation mono-poste")))
        self.radio_reseau.SetToolTip(wx.ToolTip(_("Le mode réseau est utilisateur pour une utilisation multipostes. \nMySQL doit être obligatoirement installé et configuré avant utilisation.")))
    ...
    def Build_Hyperlink(self) :
        ...
        hyper.SetBold(False)
        hyper.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner les données à importer")))
        hyper.UpdateLink()
        hyper.DoPopup(False)
        return hyper
    ...
    def OnBoutonOk(self, event):
        ...
        if (self.radio_reseau.GetValue() == True or self.radio_internet.GetValue() == True) and " " in nomFichier :
            dlg = wx.MessageDialog(self, _("Il est interdit d'utiliser des espaces dans les noms des fichiers réseau !"), _("Erreur de saisie"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            self.text_nomFichier.SetFocus()
            return
        ...
        for caract in nomFichier :
            if caract in "éèëäâïîùûüàôç'&%*,;!:?./§@€" :
                dlg = wx.MessageDialog(self, _("Il est interdit d'utiliser des accents ou autres caractères spéciaux dans le nom de fichier !"), _("Erreur de saisie"), wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                self.text_nomFichier.SetFocus()
                return
        ...
        if self.checkbox_details.GetValue() == False :
            ...
            dlg = wx.MessageDialog(self, _("Etes-vous sûr de ne pas vouloir importer les données par défaut ?"), _("Importation des données"), wx.YES_NO | wx.CANCEL | wx.ICON_QUESTION)
        ...
        if self.radio_reseau.GetValue() == True or self.radio_internet.GetValue() == True :
            ...
            try :
                ...
                dlg = wx.MessageDialog(self, _("Le numéro de port n'est pas valide. \n\nErreur : %s") % err, _("Erreur de saisie"), wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
            ...
            if dictResultats["connexion"][0] == False :
                ...
                dlg = wx.MessageDialog(self, _("La connexion au réseau MySQL est impossible. \n\nErreur : %s") % erreur, "Erreur de connexion", wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                return False
