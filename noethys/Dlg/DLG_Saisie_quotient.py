...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDfamille=None, IDquotient=None):
        ...
        # Dates
        self.staticbox_dates_staticbox = wx.StaticBox(self, -1, _("Dates de validité"))
        ...
        self.label_type_quotient = wx.StaticText(self, -1, _("Type de quotient :"))
        ...
        self.label_quotient = wx.StaticText(self, -1, _("Quotient familial :"))
        self.ctrl_quotient = wx.TextCtrl(self, -1, "")
    ...
    def __set_properties(self):
        ...
        self.ctrl_type_quotient.SetToolTip(wx.ToolTip(_("Sélectionnez un type de quotient")))
        self.bouton_types_quotients.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à la gestion des types de quotients")))
        self.ctrl_quotient.SetToolTip(wx.ToolTip(_("Saisissez ici le quotient familial")))
        self.ctrl_revenu.SetToolTip(wx.ToolTip(_("Saisissez ici le revenu")))
        self.ctrl_observations.SetToolTip(wx.ToolTip(_("[Optionnel] Saisissez ici des commentaires sur ce quotient/revenu")))
    ...
    def OnBoutonOk(self, event):
        ...
        if len(quotient) > 0 :
            try :
                ...
                dlg = wx.MessageDialog(self, _("Le quotient familial que vous avez saisi n'est pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
        ...
        if self.ctrl_revenu.Validation() == False :
            dlg = wx.MessageDialog(self, _("Le revenu que vous avez saisi n'est pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if quotient == None and revenu == 0.0 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un quotient familial ou un revenu !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if len(listeDonnees) > 0 :
            dlg = wx.MessageDialog(self, _("Il existe déjà %d quotient/revenu saisi sur cette période.\n\nSouhaitez-vous tout de même l'enregistrer ?") % len(listeDonnees), _("Anomalie"), wx.YES_NO | wx.NO_DEFAULT | wx.CANCEL | wx.ICON_EXCLAMATION)
    ...
    def Importation(self):
        if self.IDquotient == None :
            ...
            self.SetTitle(_("Saisie d'un quotient familial/revenu"))
            ...
            self.SetTitle(_("Modification d'un quotient familial/revenu"))
            DB = GestionDB.DB()
            req = """
