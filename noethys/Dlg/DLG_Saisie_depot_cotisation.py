...
class Track(object):
    def __init__(self, parent, donnees):
        ...
        if self.IDindividu != None :
            self.individu = "%s %s" % (self.individu_nom, self.individu_prenom)
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDdepot_cotisation=None):
        ...
        if self.IDdepot_cotisation != None :
            self.SetTitle(_("Modification d'un dépôt de cotisations"))
            self.Importation() 
            self.OnCheckVerrouillage(None)
        else:
            self.SetTitle(_("Saisie d'un dépôt de cotisations"))
    ...
    def __set_properties(self):
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez ici un nom")))
        self.ctrl_date.SetToolTip(wx.ToolTip(_("Saisissez la date de dépôt")))
        self.bouton_imprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour imprimer la liste des cotisations du dépôt")))
        self.ctrl_verrouillage.SetToolTip(wx.ToolTip(_("Cochez cette case si le dépôt doit être verrouillé. Dans ce cas, il devient impossible de modifier la liste des cotisations qui le contient !")))
        self.ctrl_observations.SetToolTip(wx.ToolTip(_("[Optionnel] Saisissez des commentaires")))
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter ou retirer des cotisations de ce dépôt")))
    ...
    def Sauvegarde_depot(self):
        ...
        if nom == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un nom. Exemple : 'Cotisations de Juillet 2010'... !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
    ...
    def MAJinfos(self):
        ...
        for track in self.tracks :
            if track.inclus == True :
                nbreTotal += 1
        if nbreTotal == 0 : 
            texte = _("Aucune cotisation")
        elif nbreTotal == 1 : 
            texte = _("1 cotisation")
        else:
            texte = _("%d cotisations") % nbreTotal
