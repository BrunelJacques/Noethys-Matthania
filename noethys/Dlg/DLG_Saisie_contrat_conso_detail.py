...
class CTRL_Unites(HTL.HyperTreeList):
    ...
    def GetDonnees(self):
            ...
            if typeUnite == "Unitaire" :
                ...
                for ctrl in dictItem["controles"] :
                    ...
                    if ctrl.Validation() == False :
                        dlg = wx.MessageDialog(self, _("Veuillez vérifier la saisie des paramètres des unités !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                        dlg.ShowModal()
                        dlg.Destroy()
                        return False
        ...
        if self.VerifieSiChevauchement(listeUnites) == True :
            dlg = wx.MessageDialog(self, _("Des unités multi-horaires se chevauchent !\n\nVérifiez les horaires..."), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDactivite=None, IDunite=None):
        ...
        # Jours
        self.box_jours_staticbox = wx.StaticBox(self, -1, _("Conditions"))
    ...
    def __set_properties(self):
        self.SetTitle(_("Saisie d'un paramètre de planning"))
        self.ctrl_semaines.SetToolTip(wx.ToolTip(_("Sélectionnez une fréquence")))
        self.ctrl_feries.SetToolTip(wx.ToolTip(_("Cochez cette case pour inclure les jours fériés")))
