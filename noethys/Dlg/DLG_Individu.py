...
class CTRL_header_rattachement(html.HtmlWindow):
    ...
    def SetLabel(self, texte=""):
        self.SetPage("""<BODY TEXT='#B2B2B2' LINK='#B2B2B2' VLINK='#B2B2B2' ALINK='#B2B2B2'><FONT SIZE=1 COLOR='#B2B2B2'>%s</FONT></BODY>""" % texte)
...
class Notebook(wx.Notebook):
    ...
    def __init__(self, parent, id=-1, IDindividu=None, IDfamille=None, dictFamillesRattachees=[]):
        ...
        self.parent = parent
        self.module = "DLG_INdividu.Notebook"
        self.dictFamillesRattachees = dictFamillesRattachees
        self.dictPages = {}
        self.listePages = [
            ("activites", _("Activités"), "DLG_Individu_inscriptions.Panel(self, IDindividu=IDindividu, dictFamillesRattachees=dictFamillesRattachees)", "Activite.png"),
...
class Dialog(wx.Dialog, GestCompo):
    def __init__(self, parent, IDindividu=None,IDfamille=None ,dictRattach={}):
        ...
        self.ctrl_adresse = wx.StaticText(self, -1, _("Lieu de résidence inconnu"))
        ...
        if self.nouvelleFiche == True :
            ...
            if dictRattach["IDcategorie"] == 3 :
                ...
                pageCoords.lstContacts[0].OnRadioAdresseManu(None)
    ...
    def __set_properties(self):
        if self.IDindividu != None :
            self.SetTitle(_("Fiche individu n°%d") % self.IDindividu)
        ...
        self.ctrl_photo.SetToolTip(_("Cliquez sur le bouton droit de la souris\npour accéder aux outils photo"))
        ...
        self.bouton_ok.SetToolTip(_("Cliquez ici pour valider et fermer la fiche"))
        self.bouton_annuler.SetToolTip(_("Cliquez ici pour annuler la\nsaisie et fermer la fiche"))
    ...
    def OnBoutonOutils(self, event):
        ...
        item = wx.MenuItem(menuPop, 30, _("Edition d'étiquettes et de badges"))
        ...
        # Item Historique
        item = wx.MenuItem(menuPop, 10, _("Historique"))
        ...
        # Item Chronologie
        item = wx.MenuItem(menuPop, 20, _("Chronologie"))
    ...
    def GetFamillesRattachees(self):
        ...
        for IDrattachement, IDindividu, IDfamille, IDcategorie, titulaire, nom, prenom in listeTitulaires :
            ...
            if nbreTitulaires == 2 : 
                dictFamilles[IDfamille]["nomsTitulaires"] = _("%s et %s") % (dictFamilles[IDfamille]["listeNomsTitulaires"][0], dictFamilles[IDfamille]["listeNomsTitulaires"][1])
            if nbreTitulaires > 2 :
                texteNoms = ""
                for nomTitulaire in dictFamilles[IDfamille]["listeNomsTitulaires"][:-1] :
                    texteNoms += "%s, " % nomTitulaire
                texteNoms = _("%s et %s") % (dictFamilles[IDfamille]["listeNomsTitulaires"][-2], dictFamilles[IDfamille]["listeNomsTitulaires"][-1])
                dictFamilles[IDfamille]["nomsTitulaires"] = texteNoms
        return dictFamilles
    
    def MAJtexteRattachementHeader(self):
        if len(self.dictFamillesRattachees) == 0 :
            texte = _("Aucun rattachement")
        else:
            ...
            for IDfamille, dictFamille in self.dictFamillesRattachees.items() :
                ...
                texte += _("Est rattaché à la famille de <A HREF='%s'>%s</A> en tant que %s.<BR>") % (str(IDfamille), nomsTitulaires, nomCategorie)
            if nbreFamilles > 0 :
                texte = texte[:-4]
    ...
    def Annuler(self):
        ...
        if self.nouvelleFiche == True and self.IDindividu != None :
            ...
            if len(listeDonnees) > 0 :
                dlg = wx.MessageDialog(self, _("Vous ne pouvez pas détacher cette fiche car %d pièce(s) existent déjà pour cet individu sur cette fiche famille.\n\nSupprimez les pièces avant la suppression de l'individu !") % len(listeDonnees), _("Détachement impossible"), wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                DB.Close()
                return False
            ...
            listeDonnees = DB.ResultatReq()
            if len(listeDonnees) > 0 :
                dlg = wx.MessageDialog(self, _("Vous ne pouvez pas détacher cette fiche car %d cotisation(s) individuelle(s) existe(nt) déjà pour cet individu sur cette fiche famille.\n\nSupprimez les cotisations avant la suppression de l'individu !") % len(listeDonnees), _("Détachement impossible"), wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                DB.Close()
                return False
            ...
            listeDonnees = DB.ResultatReq()
            if len(listeDonnees) > 0 :
                dlg = wx.MessageDialog(self, _("Vous ne pouvez pas détacher cette fiche car cet individu est déjà inscrit à %d activité(s) sur cette fiche famille.\n\nSupprimez l'inscription avant la suppression de l'individu !") % len(listeDonnees), _("Détachement impossible"), wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                DB.Close()
                return False
            ...
            dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment annuler la création de cette nouvelle fiche ?"), _("Annulation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
