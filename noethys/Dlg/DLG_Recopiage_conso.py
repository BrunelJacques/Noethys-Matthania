...
class CTRL_Unite(wx.Choice):
    ...
    def GetListeDonnees(self):
        ...
        listeItems = [_("-- Sélectionnez une unité --")]
        self.dictDonnees = {}
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        # Bandeau
        titre = _("Recopier des consommations")
        self.SetTitle(titre)
        intro = _("Vous pouvez ici recopier les consommations d'une unité sur une autre unité. Sélectionnez les unités à impacter puis cliquez sur Ok.")
        ...
        # Unités
        self.staticbox_unites = wx.StaticBox(self, -1, _("Unités"))
        ...
        # Paramètres
        self.staticbox_parametres = wx.StaticBox(self, -1, _("Informations à recopier"))
        self.check_param_horaires = wx.CheckBox(self, -1, _("Horaires"))
        self.check_param_quantite = wx.CheckBox(self, -1, _("Quantité"))
        self.check_param_etiquettes = wx.CheckBox(self, -1, _("Etiquettes"))
        self.check_param_etat = wx.CheckBox(self, -1, _("Etat"))

        # Options
        self.staticbox_options = wx.StaticBox(self, -1, _("Options"))
        self.radio_lignes_affichees = wx.RadioButton(self, -1, _("Toutes les lignes affichées"), style=wx.RB_GROUP)
        self.radio_lignes_selectionnees = wx.RadioButton(self, -1, _("Toutes les lignes sélectionnées"))
    ...
    def __set_properties(self):
        self.ctrl_unite_origine.SetToolTip(wx.ToolTip(_("Sélectionnez l'unité à recopier")))
        self.ctrl_unite_destination.SetToolTip(wx.ToolTip(_("Sélectionnez l'unité sur laquelle effectuer le recopiage")))
        self.check_param_horaires.SetToolTip(wx.ToolTip(_("Cochez cette option pour recopier les horaires")))
        self.check_param_quantite.SetToolTip(wx.ToolTip(_("Cochez cette option pour recopier la quantité")))
        self.check_param_etiquettes.SetToolTip(wx.ToolTip(_("Cochez cette option pour recopier les étiquettes. Cette option n'est disponible que lorsque les unités sélectionnées appartiennent à la même activité.")))
        self.check_param_etat.SetToolTip(wx.ToolTip(_("Cochez cette option pour recopier l'état")))
        self.radio_lignes_affichees.SetToolTip(wx.ToolTip(_("Sélectionnez cette option pour appliquer le recopiage sur toutes les lignes affichées")))
        self.radio_lignes_selectionnees.SetToolTip(wx.ToolTip(_("Sélectionnez cette option pour appliquer le recopiage uniquement sur les lignes sélectionnées")))
    ...
    def OnBoutonOk(self, event):
        ...
        if self.ctrl_unite_origine.GetValeur() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner une unité à recopier !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        if self.ctrl_unite_destination.GetValeur() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner l'unité vers laquelle recopier !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        if self.ctrl_unite_origine.GetValeur() == self.ctrl_unite_destination.GetValeur()  :
            dlg = wx.MessageDialog(self, _("Les deux unités sélectionnées doivent être différentes !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
