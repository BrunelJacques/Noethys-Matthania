...
class CTRL_Modeles(wx.Choice):
    ...
    def GetListeDonnees(self):
        ...
        for IDmodele, nom, IDactivite, date_debut, date_fin, IDtarif, donnees in listeDonnees :
            ...
            label = _("%s - du %s au %s") % (nom, UTILS_Dates.DateDDEnFr(date_debut), UTILS_Dates.DateDDEnFr(date_fin))
...
class CTRL_Contrats(wx.Choice):
    ...
    def GetListeDonnees(self):
        ...
        for IDcontrat, date_debut, date_fin, nomActivite, montant, nomIndividu, prenomIndividu in listeDonnees :
            ...
            nomIndividu = "%s %s" % (nomIndividu, prenomIndividu)
            if type(montant) != float :
                montant = 0.0
            montantStr = "%.2f %s" % (montant, SYMBOLE)
            self.dictDonnees[index] = { "ID" : IDcontrat, "nomIndividu " : nomIndividu}
            label = _("%s - %s - %s - du %s au %s") % (nomIndividu, nomActivite, montantStr, UTILS_Dates.DateDDEnFr(date_debut), UTILS_Dates.DateDDEnFr(date_fin))
            listeItems.append(label)
            index += 1
        return listeItems
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDindividu=None, dictFamillesRattachees=None):
        ...
        # Type de contrat
        self.box_type_staticbox = wx.StaticBox(self, wx.ID_ANY, _("Type de contrat"))
        self.radio_type_classique = wx.RadioButton(self, wx.ID_ANY, _("Contrat classique"), style=wx.RB_GROUP)
        self.radio_type_psu = wx.RadioButton(self, wx.ID_ANY, _("Contrat P.S.U."))

        # Activité
        self.box_activite_staticbox = wx.StaticBox(self, wx.ID_ANY, _("Sélection de l'activité"))
        ...
        self.radio_option_vierge = wx.RadioButton(self, wx.ID_ANY, _("Contrat vierge"), style=wx.RB_GROUP)
        self.radio_option_modele = wx.RadioButton(self, wx.ID_ANY, _("Utiliser le modèle de contrat :"))
        ...
        self.radio_option_contrat = wx.RadioButton(self, wx.ID_ANY, _("Copier le contrat :"))
    ...
    def __set_properties(self):
        self.SetTitle(_("Saisie d'un nouveau contrat"))
        self.radio_type_classique.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner le type de contrat classique")))
        self.radio_type_psu.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner le type de contrat de type P.S.U.")))
        self.ctrl_inscriptions.SetToolTip(wx.ToolTip(_("Sélectionnez une activité pour laquelle créer le contrat")))
        self.radio_option_vierge.SetToolTip(wx.ToolTip(_("Cliquez ici pour créer un contrat vierge")))
        self.radio_option_modele.SetToolTip(wx.ToolTip(_("Cliquez ici pour créer un contrat basé sur un modèle de contrat")))
        self.ctrl_modele.SetToolTip(wx.ToolTip(_("Sélectionnez un modèle de contrat dans la liste")))
        self.bouton_modeles.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à la gestion des contrats")))
        self.radio_option_contrat.SetToolTip(wx.ToolTip(_("Cliquez ici pour créer un contrat basé sur un autre contrat")))
        self.ctrl_contrat.SetToolTip(wx.ToolTip(_("Sélectionnez le contrat à copier")))
    ...
    def OnBoutonOk(self, event): 
        ...
        if self.radio_option_modele.GetValue() == True and self.ctrl_modele.GetID() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner un modèle dans la liste proposée !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return

        if self.radio_option_contrat.GetValue() == True and self.ctrl_contrat.GetID() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner un contrat dans la liste proposée !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return

        if self.radio_type_psu.GetValue() == True and track_inscription.psu_activation != 1 :
            dlg = wx.MessageDialog(self, _("L'activité que vous avez sélectionné n'est pas compatible P.S.U. !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
