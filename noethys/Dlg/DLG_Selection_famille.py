...
class Dialog(wx.Dialog):
    ...
    def __set_properties(self):
        self.SetTitle(_("Sélection d'une famille"))
        self.ctrl_familles.SetToolTip(wx.ToolTip(_("Double-cliquez sur une ligne pour la sélectionner rapidement")))
    ...
    def OnBoutonOk(self, event=None):
        ...
        if self.ctrl_familles.GetID() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner une famille !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
