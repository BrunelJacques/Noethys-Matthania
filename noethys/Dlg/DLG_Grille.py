...
class Commandes(wx.Panel):
    def __init__(self, parent):
        ...
        self.bouton_lot = CTRL_Bouton_image.CTRL(self, texte=_("Traitement par lot"), cheminImage="Images/32x32/Magique.png")
        ...
        self.bouton_options.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder au menu des options")))
        self.bouton_outils.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder au menu des outils")))
        self.bouton_lot.SetToolTip(wx.ToolTip(_("Cliquez ici pour saisir, modifier ou supprimer un lot de consommations")))
    ...
    def OnBoutonAnnuler(self, event):
        ...
        if len(listeHistorique) > 0 :
            dlg = wx.MessageDialog(self, _("Vous avez effectué %d modification(s) dans la grille.\n\nSouhaitez-vous vraiment les annuler ?") % len(listeHistorique), _("Annulation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
...
class CTRL_titre(html.HtmlWindow):
    def __init__(self, parent, IDfamille, texte="", hauteur=18,  couleurFond=(255, 255, 255)):
        ...
        texte = _("coucou") # _("Famille de %s") % self.GetNomsTitulaires()
        self.SetTexte(texte)
...
class PanelGrille(wx.Panel):
    def __init__(self, parent, mode="individu", IDfamille=None):
        ...
        liste_tools = [
            (self.ID_MODE_RESERVATION, _("Réservation"), CTRL_Grille.COULEUR_RESERVATION, _("Mode de saisie 'Réservation'")),
            (self.ID_MODE_ATTENTE, _("Attente"), CTRL_Grille.COULEUR_ATTENTE, _("Mode de saisie 'Attente'")),
            (self.ID_MODE_REFUS, _("Refus"), CTRL_Grille.COULEUR_REFUS, _("Mode de saisie 'Refus'")),
        ]
...
class Notebook(aui.AuiNotebook):
    def __init__(self, parent):
        ...
        self.AddPage(self.grille, _("test1"))
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDfamille=None, selectionIndividus=[], selectionTous=False):
        ...
        self.SetTitle(_("Grille des consommations"))
        ...
        self._mgr.AddPane(self.panel_individus, aui.AuiPaneInfo().
                          Name("individus").Caption(_("Sélection des individus")).
        ...
        self._mgr.AddPane(self.panel_facturation, aui.AuiPaneInfo().
                          Name("facturation").Caption(_("Facturation")).
...
##        event.Skip() 
    ...
    def MenuOptions(self):
        ...
        for dictPerspective in self.perspectives:
            ...
            item = wx.MenuItem(menuPop, ID_PREMIERE_PERSPECTIVE + index, label, _("Afficher la disposition '%s'") % label, wx.ITEM_CHECK)
            menuPop.AppendItem(item)
        ...
        self.listePanneaux = [
            { "label" : _("Sélection de la période"), "code" : "periode", "IDmenu" : None },
            { "label" : _("Sélection des individus"), "code" : "individus", "IDmenu" : None },
            { "label" : _("Options d'affichage et de saisie"), "code" : "activites", "IDmenu" : None },
            { "label" : _("Facturation"), "code" : "facturation", "IDmenu" : None },
            { "label" : _("Légende"), "code" : "legende", "IDmenu" : None },
            { "label" : _("Touches raccourcis"), "code" : "raccourcis", "IDmenu" : None },
            ]
        ...
        item = wx.MenuItem(menuPop, ID_MASQUER_UNITES_FERMEES, _("Masquer les unités fermées"), _("Masquer les unités fermées"), wx.ITEM_CHECK)
        menuPop.AppendItem(item)
    ...
    def MenuOutils(self):
        ...
        item = wx.MenuItem(menuPop, ID_OUTILS_SAISIE_FORFAIT, _("Appliquer des forfaits datés"), _("Appliquer des forfaits datés"))
        ...
        item = wx.MenuItem(menuPop, ID_OUTILS_IMPRIMER_CONSO, _("Imprimer la liste des réservations"), _("Imprimer la liste des réservations affichées"))
        ...
        item = wx.MenuItem(menuPop, ID_OUTILS_ENVOYER_CONSO, _("Envoyer la liste des réservations par Email"), _("Envoyer la liste des réservations affichées par Email"))
    ...
    def On_affichage_perspective_save(self, event):
        ...
        dlg = wx.TextEntryDialog(self, _("Veuillez saisir un intitulé pour cette disposition :"), _("Sauvegarde d'une disposition"))
    ...
    def On_outils_saisie_forfait(self, event):
        ...
        if len(self.panel_grille.grille.listeHistorique) > 0 :
            dlg = wx.MessageDialog(self, _("Des modifications ont été effectuées dans la grille.\n\nVous devez d'abord les sauvegarder avant de saisir un forfait !"), _("Sauvegarde des modifications"), wx.YES_NO|wx.YES_DEFAULT|wx.ICON_EXCLAMATION)
            reponse = dlg.ShowModal() 
            dlg.Destroy()
            if reponse != wx.ID_YES :
                return 
    ...
    def On_outils_lot_saisie(self, event):
        dlg = wx.MessageDialog(self, _("Désolé, cette fonction n'est pas encore disponible !"), _("Fonction indisponible"), wx.OK | wx.ICON_EXCLAMATION)
        dlg.ShowModal()
        dlg.Destroy()
