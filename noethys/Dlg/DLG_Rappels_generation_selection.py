...
from Ol import OL_Textes_rappels
...
class Panel(wx.Panel):
    def __init__(self, parent):
        ...
        # Textes
        self.box_textes_staticbox = wx.StaticBox(self, -1, _("Textes de rappels"))
        self.box_rappels_staticbox = wx.StaticBox(self, -1, _("Lettres de rappel à générer"))
        ...
        self.bouton_ajouter_texte = wx.BitmapButton(self, -1, wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Ajouter.png"), wx.BITMAP_TYPE_ANY))
        self.bouton_modifier_texte = wx.BitmapButton(self, -1, wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Modifier.png"), wx.BITMAP_TYPE_ANY))
        self.bouton_supprimer_texte = wx.BitmapButton(self, -1, wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Supprimer.png"), wx.BITMAP_TYPE_ANY))

        # Rappels disponibles
        self.box_rappels_staticbox = wx.StaticBox(self, -1, _("Liste des impayés : clients avec dû"))
        self.ctrl_rappels = CTRL_Rappels_generation_selection.CTRL(self)
        self.bouton_apercu = wx.BitmapButton(self, -1, wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Apercu.png"), wx.BITMAP_TYPE_ANY))

        self.hyper_select_tout = Hyperlien(self, label=_("Tout cocher"), infobulle=_("Cliquez ici pour tout cocher"), URL="tout")
        self.label_separation = wx.StaticText(self, -1, "|")
        self.hyper_selection_rien = Hyperlien(self, label=_("Tout décocher"), infobulle=_("Cliquez ici pour tout décocher"), URL="rien")
    ...
    def __set_properties(self):
        self.bouton_apercu.SetToolTip(_("Cliquez ici pour créer un aperçu PDF de la lettre de rappel sélectionnée"))
        self.bouton_ajouter_texte.SetToolTip(_("Cliquez ici pour ajouter un texte de rappel"))
        self.bouton_modifier_texte.SetToolTip(_("Cliquez ici pour modifier le texte sélectionné dans la liste"))
        self.bouton_supprimer_texte.SetToolTip(_("Cliquez ici pour supprimer le texte sélectionné dans la liste"))
    ...
    def Validation(self):
        ...
        if nbreCoches == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune lettre de rappel à générer !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        # Validation du choix d'un texte
        #IDtexte = self.ctrl_textes.Appeler()
        #if IDtexte == None :
        #    return False

        # Demande de confirmation
        if nbreCoches == 1 :
            texte = _("Confirmez-vous la génération de 1 lettre de rappel ?")
        else :
            texte = _("Confirmez-vous la génération de %d rappels ?") % nbreCoches
        dlg = wx.MessageDialog(self, texte, _("Confirmation"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
    ...
    def SauvegardeRappels(self):
        ...
        dlgAttente = PBI.PyBusyInfo(_("Génération des rappels en cours..."), parent=None, title=_("Veuillez patienter..."), icon=wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Logo.png"), wx.BITMAP_TYPE_ANY))
        wx.Yield() 
        ...
        lstIDfamilles = self.parent.dictParametres["listeIDfamilles"]
        ...
        for IDcompte_payeur, dictCompte in self.ctrl_rappels.dictComptes.items() :
            if dictCompte["select"] == True and IDcompte_payeur in dictCoches :
                lstIDfamilles.append(IDcompte_payeur)
        ...
        if len(listeAnomalies) > 0  :
            del dlgAttente
            dlg = wx.MessageDialog(self, _("Il reste %d lettre(s) pour lesquelles vous n'avez pas attribué de texte !") % len(listeAnomalies), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        try :
            ...
            for nomTitulaires, IDcompte_payeur, dictCompte in listeComptes :
                ...
                self.EcritStatusbar(_("Génération de la lettre de rappel %d sur %d...") % (index+1, len(listeComptes)))
            ...
            dlg = wx.MessageDialog(self, _("Désolé, le problème suivant a été rencontré : \n\n%s") % err, _("Erreur"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            self.EcritStatusbar("")
            return False
        self.parent.dictParametres["listeIDfamilles"] = lstIDfamilles
        self.EcritStatusbar("")
