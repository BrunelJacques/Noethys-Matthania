...
LISTE_MODELES = [
    {"code" : "repartition_categories_debit_tresorerie", "label" : _("Répartition des opérations de trésorerie au débit"), "image" : "Repartition.png"},
    {"code" : "repartition_categories_credit_tresorerie", "label" : _("Répartition des opérations de trésorerie au crédit"), "image" : "Repartition.png"},
    {"code" : "repartition_categories_debit_budgetaires", "label" : _("Répartition des opérations budgétaires au débit"), "image" : "Repartition.png"},
    {"code" : "repartition_categories_credit_budgetaires", "label" : _("Répartition des opérations budgétaires au crédit"), "image" : "Repartition.png"},
    {"code" : "repartition_categories_debit_tresorerie_budgetaires", "label" : _("Répartition des opérations de trésorerie + budgétaires au débit"), "image" : "Repartition.png"},
    {"code" : "repartition_categories_credit_tresorerie_budgetaires", "label" : _("Répartition des opérations de trésorerie + budgétaires au crédit"), "image" : "Repartition.png"},
    {"code" : "tiers_debit", "label" : _("Dépenses par tiers"), "image" : "Barres.png"},
    {"code" : "tiers_credit", "label" : _("Recettes par tiers"), "image" : "Barres.png"},
    #{"code" : "repartition_depenses", "label" : _("Graphique 3"), "image" : "Courbes2.png"},
    ]
...
class CTRL_Exercice(wx.Choice):
    ...
    def GetListeDonnees(self):
        listeItems = [_("Tous les exercices"),]
...
class CTRL_Analytique(wx.Choice):
    ...
    def GetListeDonnees(self):
        listeItems = [_("Tous les postes analytiques"),]
...
class CTRL_Graphique(wx.Panel):
    ...
    def OnBoutonOptions(self, event):
        ...
        item = wx.MenuItem(menuPop, 10, _("Afficher les valeurs"), _("Afficher les valeurs"), wx.ITEM_CHECK)
    ...
    def Graphe_tiers(self, typeCategorie=""):
        ...
        def autolabel(rects):
            ...
            for rect in rects:
                ...
                ax.text(width + 10, rect.get_y()+rect.get_height()/2., "%.2f %s" % (int(width), SYMBOLE), ha='left', va='center', fontsize=8, color="grey")
...
##        
##        ax = self.figure.add_subplot(111)
##        barres = ax.bar(listeIndex, listeRealise, width=bar_width, alpha=opacity, color="g", label=_("Réel"))
##        barres = ax.bar(listeIndex + bar_width, listeBudgete, width=bar_width, alpha=opacity, color="b", label=_("Budgété"))
##
##        # Formatage des montants sur y
##        majorFormatter = FormatStrFormatter(SYMBOLE + " %d")
...
##        self.figure.subplots_adjust(left=0.12, bottom=0.40, right=None, wspace=None, hspace=None)
        ...
        ax = self.figure.add_subplot(111)
        barresRealise = ax.barh(listeIndex, listeRealise, height=bar_height, alpha=opacity, color="g", label=_("Réel"))
        barresBudgete = ax.barh(listeIndex + bar_height, listeBudgete, height=bar_height, alpha=opacity, color="b", label=_("Budgété"))
        ...
        def autolabel(rects):
            ...
            for rect in rects:
                width = rect.get_width()
                ax.text(width + 20, rect.get_y()+rect.get_height()/2., "%.2f %s" % (int(width), SYMBOLE), ha='left', va='center', fontsize=8, color="grey")
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        intro = _("Sélectionnez un modèle de graphique dans la liste proposée puis ajustez les paramètres si besoin.")
        titre = _("Graphiques")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Diagramme.png")

        # Modèle
        self.box_modele_staticbox = wx.StaticBox(self, wx.ID_ANY, _("Modèle"))
        ...
        self.label_exercice = wx.StaticText(self, wx.ID_ANY, _("Exercice :"))
        self.ctrl_exercice = CTRL_Exercice(self)
        self.label_analytique = wx.StaticText(self, wx.ID_ANY, _("Analytique :"))
    ...
    def __set_properties(self):
        self.ctrl_modele.SetToolTip(wx.ToolTip(_("Sélectionnez un modèle de graphique")))
        self.ctrl_exercice.SetToolTip(wx.ToolTip(_("Sélectionnez un exercice")))
        self.ctrl_analytique.SetToolTip(wx.ToolTip(_("Sélectionnez un poste analytique")))
        self.bouton_zoom.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder aux fonctions d'export et d'impression du graphique")))
        self.bouton_options.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder aux options du graphique")))
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez ici pour obtenir de l'aide")))
        self.bouton_ok.SetToolTip(wx.ToolTip(_("Cliquez ici pour fermer")))
