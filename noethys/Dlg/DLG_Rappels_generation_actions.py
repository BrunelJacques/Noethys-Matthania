...
class Panel(wx.Panel):
    def __init__(self, parent):
        ...
        # Liste de rappels
        self.box_rappels_staticbox = wx.StaticBox(self, -1, _("Lettres de rappel"))
        ...
        self.bouton_email_lot = CTRL_Bouton_image.CTRL(self, texte=_("Transmettre\npar Email"), tailleImage=(32, 32), margesImage=(4, 4, 0, 0), margesTexte=(-5, 1), cheminImage="Images/32x32/Emails_exp.png")
        self.bouton_imprimer_lot = CTRL_Bouton_image.CTRL(self, texte=_("Imprimer"), tailleImage=(32, 32), margesImage=(4, 0, 0, 0), margesTexte=(-5, 1), cheminImage="Images/32x32/Imprimante.png")
    ...
    def __set_properties(self):
        self.bouton_apercu.SetToolTip(wx.ToolTip(_("Cliquez ici pour afficher un aperçu de la lettre de rappel sélectionnée")))
        self.bouton_email.SetToolTip(wx.ToolTip(_("Cliquez ici envoyer la lettre de rappel sélectionnée par Email")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer la lettre de rappel sélectionnée ou les lettres de rappel cochées")))
        ...
        self.bouton_email_lot.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à l'envoi des rappels par Email")))
        self.bouton_imprimer_lot.SetToolTip(wx.ToolTip(_("Cliquez ici pour imprimer les lettres de rappel générées")))
    ...
    def OnBoutonImprimerLot(self, event): 
        ...
        filtres = [self.GetFiltreNumerosRappels(),]
        # Ouverture DLG pour un choix d'impression
        from Dlg import DLG_Rappels_impression
        dlg = DLG_Rappels_impression.Dialog(self, filtres=filtres,dictParametres=self.parent.dictParametres)
        dlg.ShowModal() 
        dlg.Destroy()

    def MAJ(self):
        self.lstIDfamilles = self.parent.dictParametres["listeIDfamilles"]
        self.ctrl_rappels.lstIDfamilles = self.lstIDfamilles
        if len(self.parent.listeRappelsGenerees) > 0 :
            ...
            self.ctrl_rappels.SetFiltres([{"type" : "IDrappel_intervalle",
                                           "IDrappel_min" : IDrappel_min,
                                           "IDrappel_max" : IDrappel_max},])
            self.box_rappels_staticbox.SetLabel(_("%d lettres de rappel générées") % len(self.parent.listeRappelsGenerees))
