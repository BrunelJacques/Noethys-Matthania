...
class Panel(wx.Panel):
    ...
    def __set_properties(self):
        self.radio_unique.SetToolTip(wx.ToolTip(_("Sélectionnez 'Agrément unique' si l'activité possède un agrément unique")))
        self.ctrl_agrement_unique.SetMinSize((120, -1))
        self.ctrl_agrement_unique.SetToolTip(wx.ToolTip(_("Saisissez ici le numéro d'agrément unique")))
        self.radio_multiples.SetToolTip(wx.ToolTip(_("Sélectionnez 'Agréments multiples' si l'activité peut avoir plusieurs agréments")))
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter un agrément")))
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier l'agrément selectionné dans la liste")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer l'agrément selectionnné dans la liste")))
    ...
    def Validation(self):
        ...
        if self.radio_aucun.GetValue() == True :
            ...
            if len(listeDonnees) > 0 :
                dlg = wx.MessageDialog(self, _("Vous avez sélectionné 'Aucun agrément' pour cette activité. Souhaitez-vous vraiment supprimer le ou les agréments précédemment saisis pour cette activité ?"), _("Suppression d'agréments"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
        ...
        if self.radio_unique.GetValue() == True :
            if self.ctrl_agrement_unique.GetValue() == "" :
                dlg = wx.MessageDialog(self, _("Vous avez sélectionné 'Agrément unique' sans saisir de numéro d'agrément !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_agrement_unique.SetFocus() 
                return False
        ...
        if self.radio_multiples.GetValue() == True :
            ...
            if nbreAgrements == 0 :
                dlg = wx.MessageDialog(self, _("Vous avez sélectionné 'Agréments multiples' sans saisir aucun numéro d'agrément !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        # Validation
