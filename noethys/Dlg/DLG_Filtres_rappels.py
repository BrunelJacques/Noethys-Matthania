...
from Dlg import DLG_Rappels_generation_parametres
        ...
        # Lots de lettres
         if filtre["type"] == "lot" :
            ...
            if len(listeDonnees) > 0 :
                listeTextes.append(_("Lot de lettres '%s'") % listeDonnees[0][1])
        
        # Date d'émission
         if filtre["type"] == "date_emission" :
            listeTextes.append(_("Date d'émission de %s à %s") % (DateEngFr(str(filtre["date_min"])), DateEngFr(str(filtre["date_max"]))))

        # Date de référence
         if filtre["type"] == "date_reference" :
            listeTextes.append(_("Date de référence de %s à %s") % (DateEngFr(str(filtre["date_min"])), DateEngFr(str(filtre["date_max"]))))

        # Numéros Intervalle
         if filtre["type"] == "numero_intervalle" :
            listeTextes.append(_("Numéros de lettres de %s à %s") % (filtre["numero_min"], filtre["numero_max"]))

        # Numéros Liste
         if filtre["type"] == "numero_liste" :
            listeTextes.append(_("Numéros de lettres suivants : %s") % ";".join(filtre["listeNumeros"]))

        # Solde
         if filtre["type"] == "solde" :
            operateur = filtre["operateur"]
            if operateur == "<>" : operateur = "&#60;&#62;"
            if operateur == "<" : operateur = "&#60"
            if operateur == ">" : operateur = "&#62;"
            listeTextes.append(_("Solde %s %.2f %s") % (operateur, filtre["montant"], SYMBOLE))

        # Email
         if filtre["type"] == "email" :
            if filtre["choix"] == True :
                listeTextes.append(_("Lettres nécessitant un envoi par Email"))
            else :
                listeTextes.append(_("Lettres ne nécessitant pas un envoi par Email"))
...
class CTRL_Filtres(wx.Panel):
    def __init__(self, parent, filtres=[], ctrl_rappels=None):
        ...
        self.bouton_parametres.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier les filtres de sélection des lettres de rappel")))
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        # Bandeau
        intro = _("Sélectionnez ici les filtres de sélection de votre choix à appliquer sur la liste des lettres de rappel.")
        titre = _("Filtres de sélection des lettres de rappel")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Filtre.png")
        
        # Filtres
        self.check_lot = wx.CheckBox(self, -1, _("Lot de rappels :"))
        dlg = DLG_Rappels_generation_parametres
        self.ctrl_lot = dlg.CTRL_Lot_rappels(self)
        
        self.check_emission = wx.CheckBox(self, -1, _("Date d'émission de"))
        self.ctrl_emission_min = CTRL_Saisie_date.Date2(self)
        self.label_emission_a = wx.StaticText(self, -1, "à")
        self.ctrl_emission_max = CTRL_Saisie_date.Date2(self)
        
        self.check_reference = wx.CheckBox(self, -1, _("Date de référence de"))
        self.ctrl_reference_min = CTRL_Saisie_date.Date2(self)
        self.label_reference_a = wx.StaticText(self, -1, "à")
        self.ctrl_reference_max = CTRL_Saisie_date.Date2(self)
        
        self.check_numeros_intervalle = wx.CheckBox(self, -1, _("Numéros de lettres de"))
        ...
        self.check_numeros_liste = wx.CheckBox(self, -1, _("Numéros de lettres suivants :"))
        self.ctrl_numeros_liste = wx.TextCtrl(self, -1, "")
        
        listeOperateurs = ("=", "<>", ">", "<", ">=", "<=")
        
        self.check_solde = wx.CheckBox(self, -1, _("Solde"))
        ...
        self.check_email = wx.CheckBox(self, -1, _("Envoi par Email demandé"))
        self.ctrl_email = wx.Choice(self, -1, choices=["Oui", _("Non")])
    ...
    def __set_properties(self):
        self.check_lot.SetToolTip(wx.ToolTip(_("Filtre Lot de lettres de rappel")))
        self.ctrl_lot.SetToolTip(wx.ToolTip(_("Sélectionnez un lot de lettres dans la liste")))
        ...
        self.check_reference.SetToolTip(wx.ToolTip(_("Filtre Date d'échéance")))
        self.ctrl_reference_min.SetToolTip(wx.ToolTip(_("Sélectionnez une date min")))
        self.ctrl_reference_max.SetToolTip(wx.ToolTip(_("Sélectionnez une date max")))
        self.check_numeros_intervalle.SetToolTip(wx.ToolTip(_("Filtre Intervalle de numéros de lettres")))
        self.ctrl_numeros_intervalle_min.SetToolTip(wx.ToolTip(_("Saisissez un numéro de lettre min")))
        self.ctrl_numeros_intervalle_max.SetToolTip(wx.ToolTip(_("Saisissez un numéro de lettre max")))
        self.check_numeros_liste.SetToolTip(wx.ToolTip(_("Filtre Liste de numéros de lettres")))
        self.ctrl_numeros_liste.SetToolTip(wx.ToolTip(_("Saisissez les numéros de lettres souhaités en les séparant par un point-virgule (;)")))
        self.check_solde.SetToolTip(wx.ToolTip(_("Filtre Solde")))
        self.ctrl_solde_operateur.SetToolTip(wx.ToolTip(_("Sélectionnez un opération de comparaison")))
        self.ctrl_solde_montant.SetToolTip(wx.ToolTip(_("Saisissez un montant")))
        self.check_email.SetToolTip(wx.ToolTip(_("Filtre Envoi par Email")))
        self.ctrl_email.SetToolTip(wx.ToolTip(_("Oui/Non")))
    ...
    def GetFiltres(self):
        ...
        if self.check_lot.GetValue() == True :
            ...
            if IDlot == None :
                dlg = wx.MessageDialog(self, _("Filtre Lot de lettres : Vous n'avez sélectionné aucun lot dans la liste proposée !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        ...
        if self.check_emission.GetValue() == True :
            ...
            if date_min == None or date_max == None :
                dlg = wx.MessageDialog(self, _("Filtre Date d'émission : Les dates saisies ne sont pas valides !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        ...
        if self.check_reference.GetValue() == True :
            ...
            if date_min == None or date_max == None :
                dlg = wx.MessageDialog(self, _("Filtre Date de référence : Les dates saisies ne sont pas valides !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        ...
        if self.check_numeros_liste.GetValue() == True :
            ...
            if listeTemp == "" :
                dlg = wx.MessageDialog(self, _("Filtre Liste de numéros : Vous n'avez saisi aucun numéro de lettre dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
            try :
                ...
                dlg = wx.MessageDialog(self, _("Filtre Liste de numéros : Les numéros de lettres saisis ne sont pas valides !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        ...
        if self.check_solde.GetValue() == True :
            ...
            if montant == None :
                dlg = wx.MessageDialog(self, _("Filtre Solde : Le montant saisi n'est pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
