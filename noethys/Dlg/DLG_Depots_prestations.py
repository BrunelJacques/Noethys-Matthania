...
class CTRL_Infos(html.HtmlWindow):
    ...
    def SetLabel(self, texte=""):
        if texte == "":
            texte = _("Sélectionnez un dépôt pour commencer...")
        self.texte = texte
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        # Bandeau
        intro = _("Sélectionnez un dépôt pour afficher le détail des prestations ventilées avec les règlements inclus dans ce dépôt. Il est ensuite possible d'exporter les résultats sous forme de PDF ou sous Ms Excel.")
    ...
    def __set_properties(self):
        self.bouton_rechercher.SetToolTip(wx.ToolTip(_("Cliquez ici pour rechercher un dépôt")))
        ...
        self.check_details.SetToolTip(wx.ToolTip(_("Cliquez ici pour afficher le détail par tarif unitaire")))
    ...
    def OnSelectionDepot(self, event=None):
        ...
        for IDdepot, date, nom, total, quantite in listeDepots:
            if not total:
                total = 0.0
            nom = "%s (%s - %.2f %s - %d règlements)" % (nom, UTILS_Dates.DateEngFr(date), total, SYMBOLE, quantite)
            listeDonnees.append({"IDdepot": IDdepot, "nom": nom})
            listeLabels.append(nom)

        dlg = wx.SingleChoiceDialog(None, _("Sélectionnez un dépôt dans la liste :"), _("Sélection d'un dépôt"), listeLabels, wx.CHOICEDLG_STYLE)
        dlg.SetSize((700, 700))
        dlg.CenterOnScreen()
