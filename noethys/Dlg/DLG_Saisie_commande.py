...
class OL_Totaux(FastObjectListView):
    ...
    def InitObjectListView(self):
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_("Nom"), "left", 170, "nom_colonne", typeDonnee="texte"),
            ColumnDefn(_("valeur"), "center", 50, "valeur", typeDonnee="texte"),
        ]
    ...
    def MAJ(self):
        ...
        if ("restaurateur_nom" in self.parent.dictDonnees) == True :
            ...
            if restaurateur_nom not in (None, "") :
                ...
                listeTemp.append(_("<B>Restaurateur<BR>non renseigné</B>"))
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDmodele=None, IDcommande=None):
        ...
        # Infos
        self.box_infos_staticbox = wx.StaticBox(self, wx.ID_ANY, _("Restaurateur"))
        self.ctrl_infos = CTRL_Infos(self)

        # Totaux
        self.box_totaux_staticbox = wx.StaticBox(self, wx.ID_ANY, _("Totaux"))
        ...
        # Repas
        self.box_repas_staticbox = wx.StaticBox(self, wx.ID_ANY, _("Données"))
        self.ctrl_repas = CTRL_Commande_repas.CTRL(self, IDmodele=IDmodele, IDcommande=IDcommande)
        self.ctrl_repas.SetMinSize((100, 380))

        # Légende
        self.listeLegende = [
            {"label": _("Cases modifiables"), "couleur": CTRL_Commande_repas.COULEUR_CASES_OUVERTES, "ctrl_label": None, "ctrl_img": None},
            {"label": _("Cases non modifiables"), "couleur": CTRL_Commande_repas.COULEUR_CASES_FERMEES, "ctrl_label": None, "ctrl_img": None},
            ]
        index = 0
        ...
        self.label_info = wx.StaticText(self, -1, _("Double-cliquez pour modifier ou faites un clic droit pour ouvrir le menu contextuel ou utilisez le copier-coller (Ctrl+C puis Ctrl+V)"))
        ...
        if self.IDcommande == None :
            self.SetTitle(_("Saisie d'une commande de repas"))
        else :
            self.SetTitle(_("Modification d'une commande de repas"))
    ...
    def __set_properties(self):
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez un nom pour cette commande (Ex : 'Vacances de Pâques 2018')")))
        self.ctrl_date_debut.SetToolTip(wx.ToolTip(_("Saisissez la date de début de la période")))
        self.ctrl_date_fin.SetToolTip(wx.ToolTip(_("Saisissez la date de fin de la période")))
        self.ctrl_observations.SetToolTip(wx.ToolTip(_("Saisissez des observations")))
        ...
        self.bouton_imprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour imprimer la commande au format PDF")))
        self.bouton_email.SetToolTip(wx.ToolTip(_("Cliquez ici pour envoyer la commande par Email")))
    ...
    def OnBoutonOutils(self, event):
        ...
        item = wx.MenuItem(menuPop, id, _("Convertir les suggestions"))
        ...
        # RAZ
        id = wx.Window.NewControlId()
        item = wx.MenuItem(menuPop, id, _("Vider les données"))
    ...
    def OnBoutonOk(self, event):
        ...
        if nom == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un nom pour cette commande !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.ctrl_date_debut.Validation() == False or date_debut == None:
            dlg = wx.MessageDialog(self, _("Vous devez saisir une date de début valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if date_debut > date_fin :
            dlg = wx.MessageDialog(self, _("Vous ne pouvez pas saisir une date de début supérieure à la date de fin !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_date_debut.SetFocus()
            return False
    ...
    def OnBoutonEmail(self, event):
        ...
        if restaurateur_mail in ("", None) :
            dlg = wx.MessageDialog(self, _("Vous devez d'abord renseigner l'adresse mail du restaurateur !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
    ...
    def CreationPDF(self, nomDoc=FonctionsPerso.GenerationNomDoc("COMMANDE_REPAS", "pdf"), afficherDoc=True):
        ...
        def Header():
            ...
            dataTableau.append( (_("Commande des repas"), _("%s\nEdité le %s") % (UTILS_Organisateur.GetNom(), dateDuJour)) )
            style = TableStyle([
        ...
        story.append(Paragraph(_("<para>%s - Du %s au %s</para>") % (nom_commande, UTILS_Dates.DateDDEnFr(date_debut), UTILS_Dates.DateDDEnFr(date_fin)), style_titre_commande))
        ...
        ligne = [Paragraph(_("Date"), style_entete),]
        for dictColonne in dictDonnees["liste_colonnes"] :
...
class CTRL_Options_impression(DLG_Options_impression_pdf.CTRL_Parametres):
    ...
    def Remplissage(self):
        ...
        #
        #     propriete = wxpg.StringProperty(label=_("Date de début"), name="date_debut", value=UTILS_Dates.DateDDEnFr(date_debut))
        #     propriete.SetEditor("EditeurDate")
        #     self.Append(propriete)
        #
        #     propriete = wxpg.StringProperty(label=_("Date de fin"), name="date_fin", value=UTILS_Dates.DateDDEnFr(date_fin))
        #     propriete.SetEditor("EditeurDate")
        #     self.Append(propriete)

        # Masquer dates anciennes
        propriete = wxpg.BoolProperty(label=_("Masquer les anciennes dates"), name="masquer_dates_anciennes", value=False)
        propriete.SetHelpString(_("Cochez cette case pour masquer les dates passées"))
        propriete.SetAttribute("UseCheckbox", True)
        self.Append(propriete)

        # Afficher observations
        propriete = wxpg.BoolProperty(label=_("Afficher les observations"), name="afficher_observations", value=False)
        propriete.SetHelpString(_("Cochez cette case pour afficher les observations"))
        propriete.SetAttribute("UseCheckbox", True)
        self.Append(propriete)
        ...
        # Couleur
        propriete = wxpg.ColourProperty(label=_("Fond entêtes"), name="couleur_fond_entetes", value=wx.Colour(230, 230, 230))
        propriete.SetHelpString(_("Sélectionnez une couleur"))
        propriete.SetAttribute("obligatoire", True)
        self.Append(propriete)

        # Couleur
        propriete = wxpg.ColourProperty(label=_("Fond colonne total"), name="couleur_fond_total", value=wx.Colour(245, 245, 245))
        propriete.SetHelpString(_("Sélectionnez une couleur"))
        propriete.SetAttribute("obligatoire", True)
        self.Append(propriete)

        self.Append( wxpg.PropertyCategory(_("Texte")) )

        # Taille police
        propriete = wxpg.IntProperty(label=_("Taille de texte"), name="taille_texte", value=7)
        ...
        # Taille police
        propriete = wxpg.IntProperty(label=_("Taille de texte du titre"), name="taille_texte_titre", value=9)
        propriete.SetHelpString(_("Saisissez une taille de texte (9 par défaut)"))
        propriete.SetAttribute("obligatoire", True)
        self.Append(propriete)
        self.SetPropertyEditor("taille_texte", "SpinCtrl")

        self.Append( wxpg.PropertyCategory(_("Colonnes")) )

        # Largeur automatique
        propriete = wxpg.BoolProperty(label=_("Largeur auto ajustée"), name="largeur_colonnes_auto", value=True)
        propriete.SetHelpString(_("Cochez cette case pour laisser Noethys ajuster la largeur des colonnes automatiquement"))
        propriete.SetAttribute("UseCheckbox", True)
        self.Append(propriete)

        # Largeur colonne labels
        propriete = wxpg.IntProperty(label=_("Largeur colonne date"), name="largeur_colonne_date", value=110)
        propriete.SetHelpString(_("Saisissez la largeur pour la colonne date (110 par défaut)"))
        propriete.SetAttribute("obligatoire", True)
        self.Append(propriete)
