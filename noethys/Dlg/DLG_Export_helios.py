...
class Dialog(wx.Dialog):
    def __init__(self, parent, filtres=[]):
        ...
        # Bandeau
        intro = _("Vous pouvez ici exporter les factures au format ROLMRE pour Hélios. Cochez les factures à inclure dans le rôle puis cliquez sur le bouton Fichier pour générer un fichier de transfert normalisé ou le bouton Impression pour imprimer des versions papier.")
        titre = _("Export vers Hélios (ROLMRE)")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Helios.png")
        
        # Factures
        self.box_factures_staticbox = wx.StaticBox(self, -1, _("Factures"))
        self.ctrl_factures = CTRL_Liste_factures.CTRL(self, filtres=filtres)
        
        # Paramètres
        self.box_parametres_staticbox = wx.StaticBox(self, -1, _("Paramètres du rôle"))
        self.label_date = wx.StaticText(self, -1, _("Date d'émission :"))
        self.ctrl_date = CTRL_Saisie_date.Date2(self)
        self.label_exercice = wx.StaticText(self, -1, _("Exercice :"))
        self.ctrl_exercice = wx.TextCtrl(self, -1, "")
        self.label_num_role = wx.StaticText(self, -1, _("Numéro de rôle :"))
        self.ctrl_num_role = wx.TextCtrl(self, -1, "")
        self.label_code_coll = wx.StaticText(self, -1, _("Code collectivité :"))
        self.ctrl_code_coll = wx.TextCtrl(self, -1, "")
        self.label_code_nat = wx.StaticText(self, -1, _("Code Nature :"))
        self.ctrl_code_nat = wx.TextCtrl(self, -1, "")
        self.label_code_rec = wx.StaticText(self, -1, _("Code Recette :"))
        self.ctrl_code_rec = wx.TextCtrl(self, -1, "")
        ...
        # Sortie
        self.box_fichier_staticbox = wx.StaticBox(self, -1, _("Fichier"))
        self.box_impression_staticbox = wx.StaticBox(self, -1, _("Impression"))
        self.bouton_fichier = CTRL_Bouton_image.CTRL(self, texte=_("Générer le fichier ROLMRE"), cheminImage="Images/32x32/Disk.png")
        self.label_type = wx.StaticText(self, -1, _("Type :"))
        self.label_type.Show(False)
        self.ctrl_type = CTRL_Type_impression(self, donnees=[ {"label" : _("Rôle détaillé"), "description" : _("Détail des prestations par famille"), "image" : wx.Bitmap(Chemins.GetStaticPath(u"Images/32x32/Facture.png"), wx.BITMAP_TYPE_ANY)},
                                                                    {"label" : _("Rôle simplifié"), "description" : _("Total des prestations par famille"), "image" : wx.Bitmap(Chemins.GetStaticPath(u"Images/32x32/Document_1.png"), wx.BITMAP_TYPE_ANY)},
                                                                    {"label" : _("Récapitulatif"), "description" : _("Total des prestations par activité"), "image" : wx.Bitmap(Chemins.GetStaticPath(u"Images/32x32/Document_2.png"), wx.BITMAP_TYPE_ANY)},
                                                                    ])
    ...
    def __set_properties(self):
        self.ctrl_date.SetToolTip(wx.ToolTip(_("Saisissez la date d'émission du rôle")))
        self.ctrl_code_coll.SetToolTip(wx.ToolTip(_("Saisissez ici le code Collectivité du rôle (ROLCOL)")))
        self.ctrl_exercice.SetToolTip(wx.ToolTip(_("Saisissez ici le code Exercice du rôle (ROLEX)")))
        self.ctrl_code_nat.SetToolTip(wx.ToolTip(_("Saisissez ici le code Nature du rôle (ROLNAT)")))
        self.ctrl_num_role.SetToolTip(wx.ToolTip(_("Saisissez ici le numéro de rôle du rôle (ROLROL)")))
        self.ctrl_code_rec.SetToolTip(wx.ToolTip(_("Saisissez ici le code Recette du rôle (ROLREC)")))
        self.bouton_fichier.SetToolTip(wx.ToolTip(_("Cliquez ici pour générer le fichier ROLMRE")))
        self.ctrl_type.SetToolTip(wx.ToolTip(_("Sélectionnez ici le type de l'impression")))
        self.bouton_apercu.SetToolTip(wx.ToolTip(_("Cliquez ici pour afficher le rôle au format PDF")))
    ...
    def __do_layout(self):
        ...
        grid_sizer_impression.Add(self.ctrl_type, 1, wx.EXPAND, 0)
##        grid_sizer_impression.Add((10, 10), 0, wx.EXPAND, 0)
    ...
    def ValidationSaisie(self):
        ...
        if len(tracks) == 0 : 
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune facture à inclure dans le rôle !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        for track in tracks :
            if -track.solde < 0.0 :
                dlg = wx.MessageDialog(self, _("La facture n°%s n'est pas valide car elle comporte un montant de %.2f %s, soit inférieure à 0.\n\nVeuillez la retirer de la sélection.") % (track.numero, -track.solde, SYMBOLE), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        ...
        if date_emission == None :
            dlg = wx.MessageDialog(self, _("Vous devez saisir une date d'émission pour ce rôle !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if code_col == "" :
            dlg = wx.MessageDialog(self, _("Vous devez saisir le code Collectivité du rôle !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if code_nat == "" :
            dlg = wx.MessageDialog(self, _("Vous devez saisir le code Nature du rôle !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if code_rec == "" :
            dlg = wx.MessageDialog(self, _("Vous devez saisir le code Recette du rôle !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if exercice == "" :
            dlg = wx.MessageDialog(self, _("Vous devez saisir l'exercice du rôle !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if num_role == "" :
            dlg = wx.MessageDialog(self, _("Vous devez saisir le numéro de rôle !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
    ...
    def OnBoutonFichier(self, event): 
        ...
        for track in tracks :
            ...
            dictDonnees = {
                ...
                "objet" : _("Facture n°%s") % track.numero, # Objet de la recette (64 caract max)
    ...
    def OnBoutonApercu(self, event): 
        ...
        def Header():
            ...
            dataTableau.append( (_("Rôle n°%s-%s") % (exercice, num_role), _("%s\nEdité le %s") % (UTILS_Organisateur.GetNom(), dateDuJour)) )
            style = TableStyle([
        ...
        # Intro
        story.append(Paragraph(_("Date d'émission : %s | Code Collectivité : %s | Code Nature : %s | Code Recette : %s") % (UTILS_Dates.DateDDEnFr(date_emission), code_col, code_nat, code_rec), styleTexte))
        story.append(Spacer(0, 20))
        ...
        if typeImpression in ("detail", "simple") :
            ...
            for track in tracks :
                ...
                dataTableau.append((
                    _("Facture n°%s") % track.numero,
                    "%.2f %s" % (-track.solde, SYMBOLE),
                ...
                dataTableau = [(Paragraph(_("Famille"), styleLabel), Paragraph(_("Adresse"), styleLabel), Paragraph(_("Prélèvement bancaire"), styleLabel)),]
                ...
                if typeImpression == "detail" and track.IDfacture in dictPrestations :
                    ...
                    dataTableau = [(Paragraph(_("Individu"), styleLabel), Paragraph(_("Activité"), styleLabel), Paragraph(_("Prestation"), styleLabel), Paragraph(_("Quantité"), styleLabel), Paragraph(_("Montant total"), styleLabel)),]
        ...
        story.append(Paragraph(_("%d factures | Montant total : %.2f %s") % (len(tracks), montantTotal, SYMBOLE), styleTexte))
