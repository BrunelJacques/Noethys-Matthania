...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDactivite=None, date_debut=None, date_fin=None, IDgroupe=None, listePeriodes=[]):
        ...
        self.radio_modele = wx.RadioButton(self, wx.ID_ANY, _("Le modèle de planning suivant :"), style=wx.RB_GROUP)
        ...
        self.radio_planning_detail = wx.RadioButton(self, wx.ID_ANY, _("Le planning suivant :"))
    ...
    def __set_properties(self):
        self.SetTitle(_("Génération de consommations"))
        ...
        self.radio_modele.SetToolTip(wx.ToolTip(_("Sélectionnez cette option pour utiliser un modèle de planning")))
        self.ctrl_modele.SetToolTip(wx.ToolTip(_("Sélectionnez un modèle de planning dans la liste")))
        self.bouton_modele.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à la gestion des modèles de planning")))
        self.radio_planning_detail.SetToolTip(wx.ToolTip(_("Sélectionnez cette option pour définir un planning manuellement")))
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter un paramètre")))
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier un paramètre")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer un paramètre")))
    ...
    def OnBoutonOk(self, event):
        if self.ctrl_date_debut.GetDate() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir la date de début d'application !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if self.ctrl_date_debut.GetDate() > self.ctrl_date_fin.GetDate() :
            dlg = wx.MessageDialog(self, _("La date de début doit être inférieure à la date de fin !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return

        if self.radio_modele.GetValue() == True and self.ctrl_modele.GetID() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner un modèle de planning !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if len(donnees) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement définir au moins un paramètre de planning !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Generation(self):
        ...
        if nbreConsoValides == 0 :
            dlg = wx.MessageDialog(self, _("Il n'y a aucune consommation valide à générer !"), _("Génération"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
