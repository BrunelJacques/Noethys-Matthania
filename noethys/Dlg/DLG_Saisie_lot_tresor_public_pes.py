...
class CTRL_Parametres(DLG_Saisie_lot_tresor_public.CTRL_Parametres):
    ...
    def Remplissage(self):
        ...
        propriete.SetHelpString(_("Saisissez l'ID du bordereau")) 
        self.Append(propriete)

        propriete = wxpg.StringProperty(label=_("ID Poste"), name="id_poste", value=u"")
        propriete.SetHelpString(_("Saisissez l'ID du bordereau")) 
        self.Append(propriete)
        
        propriete = wxpg.StringProperty(label=_("ID Collectivité"), name="id_collectivite", value=u"")
        propriete.SetHelpString(_("Saisissez l'ID de la collectivité")) 
        self.Append(propriete)
        ...
        propriete = wxpg.StringProperty(label=_("Code Etablissement"), name="code_etab", value=u"")
        propriete.SetHelpString(_("Saisissez le code Etablissement (3 caractères max). Nécessaire pour le datamatrix. Ce code apparaît dans les paramètres de flux dans Hélios."))
        self.Append(propriete)
        ...
        # Pièces jointes
        self.Append( wxpg.PropertyCategory(_("Pièces jointes")) )

        propriete = wxpg.BoolProperty(label=_("Inclure les factures en pièces jointes"), name="inclure_pieces_jointes", value=False)
        propriete.SetHelpString(_("Cochez cette case si vous souhaitez que Noethys intègre les factures en temps que pièces jointes au bordereau"))
        propriete.SetAttribute("UseCheckbox", True)
        self.Append(propriete)

        propriete = wxpg.StringProperty(label=_("Format des noms de fichiers"), name="format_nom_fichier", value=_("F{NUM_FACTURE}_{NOM_TITULAIRES_MAJ}"))
        propriete.SetHelpString(_("Saisissez le format des noms de fichiers (en majuscules et sans espaces ni accents). Vous pouvez personnaliser ce libellé grâce aux mots-clés suivants : {NUM_FACTURE} {NOM_TITULAIRES} {NOM_TITULAIRES_MAJ}."))
        self.Append(propriete)
...
class Dialog(DLG_Saisie_lot_tresor_public.Dialog):
    ...
    def OnBoutonFichier(self, event):
        ...
        txtMessage = _("Le fichier xml PES Recette ORMC a été créé avec succès.\n\nSouhaitez-vous visualiser son contenu maintenant ?")
...
"""
        ...
        dlg = DLG_Message_html.Dialog(self, texte=texte, titre=_("Information importante"), size=(510, 290), nePlusAfficher=True)
        dlg.CenterOnScreen()
        dlg.ShowModal()
