...
class CTRL(wx.Panel):
    ...
    def IsOuvert(self, IDunite=None, date=None):
        case = self.GetCase(IDunite, date)
        if case == None :
            return _("Cette case est inexistante.")
        return case.ouvert
    ...
    def SaisieConso(self, IDunite=None, mode="reservation", etat="reservation", heure_debut="defaut", heure_fin="defaut", date=None, quantite=None, badgeage_debut=None, badgeage_fin=None, conso=None):
        ...
        if case.ouvert == False :
            return _("Cette unité est fermée.")
        ...
        if case.HasPlaceDisponible(*conditions) == False :
            ...
            if mode == "reservation" and self.usage != "nomadhys":
                return _("Il n'y a plus de place le %s.") % UTILS_Dates.DateDDEnFr(date)
        ...
        if incompatibilite != None :
            ...
            return _("Action impossible car il existe déjà le %s une réservation sur l'unité '%s'.") %  (UTILS_Dates.DateDDEnFr(date), nomUniteIncompatible)
        ...
        # Si mode = attente
        #if hasPlaces == False :
        #    return _("Il n'y a plus de place le %s. Réservation en attente saisie.") % UTILS_Dates.DateDDEnFr(date)
    ...
    def SupprimeConso(self, IDunite=None, date=None):
        ...
        if case == None : 
            return _("Cette case est inexistante.")
        if case.etat == None :
            return _("Il n'existe aucune consommation à cette date et pour cette unité.")
        if case.IDfacture != None :
            return _("Interdit de supprimer une consommation déjà facturée.")
        if case.etat in ("present", "absenti", "absentj") :
            return _("Interdit de supprimer une consommation déjà pointée.")
    ...
    def ModifieMemo(self, date=None, texte=""):
        ...
        if case == None :
            return _("La case mémo journalier est inexistante.")
        case.SetTexte(texte)
        return True        
...
class Dialog(wx.Dialog):
    def __init__(self, parent, ):
        ...
        # Boutons de test
        bouton1 = wx.Button(self, -1, _("TEST 1 - fam 1 - Date unique"))
        bouton2 = wx.Button(self, -1, _("TEST 2 - fam 2- Plusieurs dates"))
        bouton3 = wx.Button(self, -1, _("TEST 3 - Sauvegarde "))
        bouton4 = wx.Button(self, -1, _("TEST 4 - saisie conso"))
