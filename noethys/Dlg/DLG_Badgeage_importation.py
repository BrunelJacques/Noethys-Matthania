...
def Purger():
    ...
    texte = _("La purge des archives de badgeages imprtés vous permet de réduire la taille de la base de données. Il est conseillé d'y procéder une fois que vous n'avez plus besoin des archives (soit quelques mois après).\n\nCommencez par sélectionner une date maximale de badgeage.")
    dlg = wx.MessageDialog(None, texte, _("Purge du journal de badgeage"), wx.OK|wx.CANCEL|wx.ICON_INFORMATION)
    ...
    dlg = wx.MessageDialog(None, _("Confirmez-vous la purge des archives de badgeage jusqu'au %s inclus ?") % DateEngFr(str(date)), _("Purge des archives de badgeage"), wx.YES_NO|wx.NO_DEFAULT|wx.ICON_EXCLAMATION)
    reponse = dlg.ShowModal()
    dlg.Destroy()
    ...
    # Fin
    dlg = wx.MessageDialog(None, _("Purge des archives de badgeage terminée."), _("Information"), wx.OK | wx.ICON_INFORMATION)
    dlg.ShowModal()
    dlg.Destroy()
...
class CTRL_Donnees(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ColumnDefn(_("Code-barres"), 'center', 135, "codebarres"),
            ColumnDefn(_("Date"), 'center', 200, "date", stringConverter=FormateDate),
            ColumnDefn(_("Heure"), 'center', 80, "heure"),
            ]
    ...
    def OnContextMenu(self, event):
        ...
        # Export Texte
        item = wx.MenuItem(menuPop, 60, _("Exporter au format Texte"))
        ...
        # Export Excel
        item = wx.MenuItem(menuPop, 70, _("Exporter au format Excel"))
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=_("Liste des données"), format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def ExportTexte(self, event):
        ...
        UTILS_Export.ExportTexte(self, titre=_("Liste des données"))
        
    def ExportExcel(self, event):
        from Utils import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=_("Liste des données"))
...
class CTRL_Choix_appareil(wx.Choice):
    ...
    def MAJ(self):
        listeDonnees = [
            ("cs1504", _("Symbol CS1504")),
            ("opn-2001", _("Opticon OPN-2001")),
            ]
        listeItems = []
        self.dictDonnees = {}
        index = 0
...
class Page_scanner(wx.Panel):
    def __init__(self, parent):
        ...
        self.check_vider = wx.CheckBox(self, -1, _("Vider après importation"))
        self.bouton_outils = wx.Button(self, -1, _("Outils"))
        self.label_appareil = wx.StaticText(self, -1, _("Modèle :"))
        self.ctrl_appareil = CTRL_Choix_appareil(self)
        self.check_heure_auto = wx.CheckBox(self, -1, _("Réglage de l'heure auto."))
        ...
        # Propriétés
        self.ctrl_port.SetToolTip(wx.ToolTip(_("Sélectionnez le port sur lequel est branché le scanner")))
        self.check_vider.SetToolTip(wx.ToolTip(_("Cochez cette case pour vider l'appareil après l'importation")))
        self.bouton_outils.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder aux commandes spéciales du scanner")))
        self.ctrl_appareil.SetToolTip(wx.ToolTip(_("Sélectionnez le nom du scanner")))
        self.check_heure_auto.SetToolTip(wx.ToolTip(_("Cochez cette case pour que le réglage de l'heure de l'appareil soit automatique")))
    ...
    def OnBoutonOutils(self, event): 
        ...
        menuPop.AppendItem(wx.MenuItem(menuPop, ID_VIDER_MEMOIRE, _("Vider la mémoire"), _("Vider la mémoire du scanner")))
        self.Bind(wx.EVT_MENU, self.OnOutil, id=ID_VIDER_MEMOIRE)

        ID_REGLER_HEURE = wx.Window.NewControlId()
        menuPop.AppendItem(wx.MenuItem(menuPop, ID_REGLER_HEURE, _("Régler l'horloge du scanner"), _("Régler l'horloge du scanner")))
    ...
    def Lire(self):
        ...
        try :
            ...
            dlg = wx.MessageDialog(self, _("Noethys n'arrive pas à lire les données du scanner. Essayez de le déconnecter et le reconnecter...\n\nErreur : %s") % err, _("Erreur"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            scanner.close() 
            return False
        ...
        for symbology, codebarres, timestamp in listeBarcodes :
            try :
                ...
                heure = "%02d:%02d" % (timestamp.hour, timestamp.minute)
                listeDonnees.append((codebarres, date, heure))
            except Exception as err:
                print("Erreur de lecture du code-barres %d :" % index+1, err)
                dlg = wx.MessageDialog(None, _("Erreur de lecture du code-barres %d : %s\n\nVoulez-vous quand-même continuer ?") % (index+1, err), _("Erreur"), wx.YES_NO|wx.YES_DEFAULT|wx.ICON_EXCLAMATION)
    ...
    def Connexion(self):
        ...
        if port == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner un port dans la liste proposée !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if appareil == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner un appareil dans la liste proposée !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if (appareil == "cs1504") or (appareil == "opn-2001") :
            try :
                ...
                dlg = wx.MessageDialog(self, _("Noethys n'arrive pas à se connecter à l'appareil."), _("Erreur"), wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                return False
    ...
    def ReglerHeure(self, scanner=None):
        try :
            ...
            dlg = wx.MessageDialog(self, _("Noethys n'arrive pas à se régler l'heure du scanner.\n\nErreur : %s") % err, _("Erreur"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return False

    def ViderMemoire(self, scanner=None):
        dlg = wx.MessageDialog(None, _("Souhaitez-vous vraiment vider la mémoire du scanner ?"), _("Vider le scanner"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        if dlg.ShowModal() == wx.ID_YES :
            try :
                scanner.clear_barcodes()
            except Exception as err:
                dlg = wx.MessageDialog(self, _("Noethys n'arrive pas à vider l'appareil.\n\nErreur : %s") % err, _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
        dlg.Destroy()
...
class Page_excel(wx.Panel):
    def __init__(self, parent):
        wx.Panel.__init__(self, parent, id=-1, style=wx.TAB_TRAVERSAL)
        self.label_fichier = wx.StaticText(self, -1, _("Fichier :"))
        wildcard = _("Fichiers Excel (*.xls)|*.xls|All files (*.*)|*.*")
        sp = wx.StandardPaths.Get()
        cheminDefaut = sp.GetDocumentsDir()
        self.ctrl_fichier = filebrowse.FileBrowseButton(self, -1, labelText=u"", buttonText=_("Sélectionner"), toolTip=_("Cliquez ici pour sélectionner un fichier de données"), dialogTitle=_("Sélectionner un fichier"), fileMask=wildcard, startDirectory=cheminDefaut)
        self.label_infos = wx.StaticText(self, -1, _("Versions acceptées : 2003, 2002, XP, 2000, 97, 95, 5.0, 4.0, 3.0."))
        self.label_infos.SetForegroundColour((130, 130, 130))
        
        # Propriétés
        self.ctrl_fichier.SetToolTip(wx.ToolTip(_("Sélectionnez le fichier source")))
    ...
    def Lire(self):
        ...
        try :
            ...
            dlg = wx.MessageDialog(self, _("Le fichier Excel ne semble pas valide !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        try :
            for num_ligne in range(feuille.nrows):
                ...
                heure = "%02d:%02d" % (dateTuple[3], dateTuple[4])
                # Mémorisation
                listeDonnees.append((codebarres, date, heure))
        except Exception as err :
            listeDonnees = []
            dlg = wx.MessageDialog(self, _("Noethys n'a pas réussi à lire les données !\n\nErreur : %s.\n\nNotez que les colonnes du fichier Excel doivent être les suivantes : 1. Code-barres 2. Date, 3. Heure.") % err, _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
...
class Page_csv(wx.Panel):
    def __init__(self, parent):
        wx.Panel.__init__(self, parent, id=-1, style=wx.TAB_TRAVERSAL)
        self.label_fichier = wx.StaticText(self, -1, _("Fichier :"))
        wildcard = _("Fichiers CSV ou TXT|*.csv;*.txt|Tous les fichiers (*.*)|*.*")
        sp = wx.StandardPaths.Get()
        cheminDefaut = sp.GetDocumentsDir()
        self.ctrl_fichier = filebrowse.FileBrowseButton(self, -1, labelText=u"", buttonText=_("Sélectionner"), toolTip=_("Cliquez ici pour sélectionner un fichier de données"), dialogTitle=_("Sélectionner un fichier"), fileMask=wildcard, startDirectory=cheminDefaut)
        self.label_delimiteur = wx.StaticText(self, -1, _("Délimiteur :"))
        self.ctrl_delimiteur = wx.Choice(self, -1, choices=[_("Virgule (,)"), _("Point-virgule (;)"), _("Tabulation")])
        ...
        # Propriétés
        self.ctrl_fichier.SetToolTip(wx.ToolTip(_("Sélectionnez le fichier source")))
        self.ctrl_delimiteur.SetToolTip(wx.ToolTip(_("Sélectionnez le symbole délimiteur des champs")))
        self.ctrl_delimiteur.SetSelection(1)
        self.ctrl_format.SetToolTip(wx.ToolTip(_("Saisissez le format de la ligne de la source : \n\n- CODE : Code-barre\n- AAAA : Année\n- MM : Mois\n- JJ : Jour\n- HH : Heures\n- MN : Minutes\n- SS : Secondes\n- X : Autre\n\nExemple : 'CODE;AAAA-MM-JJ HH:MN:SS;' \n")))
    ...
    def Lire(self):
        ...
        if selectionDelimiteur == -1 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner un symbole délimiteur pour ce fichier !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if len(format) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un format !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        
        # Décodage du format
        champs = [ ("AAAA", _("l'année")), ("MM", _("le mois")), ("JJ", _("le jour")), ("CODE", _("le code-barres")), ("HH", _("les heures")), ("MN", _("les minutes")) ]
        dictFormat = {}
        for code, label in champs :
            if code not in format :
                dlg = wx.MessageDialog(self, _("Le format doit obligatoirement comporter l'expression '%s' afin d'identifier %s dans les lignes !") % (code, label), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        ...
        try :
            for ligne in fichier :
                ...
                # Heure
                heure = "%02d:%02d" % (int(Decodage("HH")), int(Decodage("MN")))
                # Mémorisation
                listeDonnees.append((codebarres, date, heure))
        except Exception as err :
            listeDonnees = []
...
class Page_archives(wx.Panel):
    def __init__(self, parent):
        wx.Panel.__init__(self, parent, id=-1, style=wx.TAB_TRAVERSAL)
        self.label_date_archivage = wx.StaticText(self, -1, _("Date de l'archivage :"))
        self.ctrl_date_archivage = CTRL_Saisie_date.Date2(self)
        self.label_date_badgeage = wx.StaticText(self, -1, _("Date du badgeage :"))
...
class Notebook(wx.Notebook):
    def __init__(self, parent):
        ...
        self.listePages = [
            (_("scanner"), _("Scanner à mémoire"), _("Page_scanner(self)"), "Scanner_memoire.png"),
            (_("excel"), _("Fichier Excel"), _("Page_excel(self)"), "Excel.png"),
            (_("csv"), _("Fichier CSV"), _("Page_csv(self)"), "Facture.png"),
            (_("archives"), _("Archives"), _("Page_archives(self)"), "Database.png"),
            ]
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        # Bandeau
        intro = _("Pour importer des badgeages, commencez par sélectionner une source, cliquez sur le bouton 'Lire les données' puis sur le bouton 'Importer' pour importer les données cochées.")
        titre = _("Importation de badgeages")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Badgeage.png")
        
        # Source
        self.box_source_staticbox = wx.StaticBox(self, -1, _("Source des données"))
        self.box_donnees_staticbox = wx.StaticBox(self, -1, _("Données"))
        ...
        self.check_archiver = wx.CheckBox(self, -1, _("Archiver les données avant importation"))
        ...
        self.bouton_ok = CTRL_Bouton_image.CTRL(self, texte=_("Importer"), cheminImage="Images/32x32/Fleche_bas.png")
    ...
    def __set_properties(self):
        self.bouton_lire.SetToolTip(wx.ToolTip(_("Cliquer ici pour lire la source des données")))
        self.bouton_apercu.SetToolTip(wx.ToolTip(_("Cliquez ici pour créer un aperçu avant impression de la liste")))
        self.bouton_imprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour imprimer la liste")))
        self.bouton_excel.SetToolTip(wx.ToolTip(_("Cliquez ici pour faire un export Excel de la liste")))
        self.bouton_texte.SetToolTip(wx.ToolTip(_("Cliquez ici pour faire un export Texte de la liste")))
        self.check_archiver.SetToolTip(wx.ToolTip(_("Cochez cette case pour archiver les données")))
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez ici pour obtenir de l'aide")))
        self.bouton_ok.SetToolTip(wx.ToolTip(_("Cliquez ici pour lancer l'importation")))
    ...
    def ArchiverBadgeages(self):
        ...
        # Confirmation
        texte = _("%d codes-barres ont été archivés avec succès.") % len(listeTracksValides)
        if len(listeTracksNonValides) > 0 :
            texte += _("\n\nMais %d codes-barres déjà archivés ont été exclus.") % len(listeTracksNonValides)
        dlg = wx.MessageDialog(self, texte, _("Confirmation"), wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
    ...
    def OnBoutonOk(self, event): 
        ...
        if len(listeTracks) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune donnée à importer !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
