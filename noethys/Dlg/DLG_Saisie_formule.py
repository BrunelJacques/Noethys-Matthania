...
OPERATEURS = [
    ...
    (_("est supérieur à"), ">"),
    (_("est inférieur à"), "<"),
    (_("est supérieur ou égal à"), ">="),
    (_("est inférieur ou égal à"), "<="),
    (_("est vide"), "null"),
    (_("n'est pas vide"), "notnull"),
    ]
...
class Hyperlien(Hyperlink.HyperLinkCtrl):
    ...
    def OnLeftLink(self, event):
        dlg = wx.SingleChoiceDialog(None, _("Sélectionnez un champ à insérer :"), _("Insérer un champ"), GetLabelsChamps(self.parent.listeChamps), wx.CHOICEDLG_STYLE)
...
class Dialog(wx.Dialog):
    def __init__(self, parent, listeChamps=[]):
        ...
        self.box_formule_staticbox = wx.StaticBox(self, -1, _("Formule"))
        
        self.label_si = wx.StaticText(self, -1, _("Si :"))
        ...
        self.ctrl_condition = wx.TextCtrl(self, -1, "", size=(150, -1))
        
        self.label_afficher = wx.StaticText(self, -1, _("Afficher :"))
        self.ctrl_afficher = wx.TextCtrl(self, -1, "", style=wx.TE_MULTILINE)
    ...
    def __set_properties(self):
        self.SetTitle(_("Saisie d'une formule conditionnelle"))
        self.ctrl_champ.SetToolTip(wx.ToolTip(_("Sélectionnez ici un champ dans la liste")))
        self.ctrl_operateur.SetToolTip(wx.ToolTip(_("Sélectionnez un opérateur dans la liste")))
        self.ctrl_condition.SetToolTip(wx.ToolTip(_("Saisissez ici la valeur conditionnelle. Il peut s'agir \nd'un mot ou d'une phrase. Vous pouvez également \nsaisir ' OU ' entre plusieurs mots pour saisir plusieurs \nconditions avec l'opérateur =.")))
        self.ctrl_afficher.SetToolTip(wx.ToolTip(_("Saisissez ici le texte à afficher si la condition est vraie.\nIl peut s'agir d'un mot ou d'une phrase mais aussi \nd'un mot-clé. Ex : 'bonjour', '{INDIVIDU_NOM}',\n'123', etc...")))
    ...
    def OnBoutonOk(self, event): 
        ...
        if self.ctrl_champ.GetSelection() == -1 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner un champ SI dans la liste !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
    ...
    def GetFormule(self):
        ...
        if operateur == "notnull" :
            operateur = "<>"
            condition = ""
        formule = """[[SI %s%s%s->%s]]""" % (champ, operateur, condition, afficher)
        return formule
...
def ResolveurCalcul(texte=u"", dictValeurs={}):
    ...
    try :
        ...
        if resultatEuros == True :
            resultat = "%.02f %s" % (resultat, SYMBOLE)
        else :
...
if __name__ == "__main__":
    ...
    dictValeurs = {
        "{ORGANISATEUR_NOM}" : _("Association Noethys"),
        "{FAMILLE_VILLE}" : _("QUIMPER"),
        "{MONTANT}" : "2.00 €",
        }
...
##    app.MainLoop()
    ...
    resultat = ResolveurTexte(
                texte=_("Ceci est [[SI {FAMILLE_VILLE}<>->Bonjour brest !]] et voilà et aussi [[SI {FAMILLE_VILLE}=QUIMPER ->Salut quimper !]]. Je veux aussi résoudre le calcul suivant : [[1+2.0]] Euros"),
                listeChamps=listeChamps, 
                dictValeurs=dictValeurs)
    print((resultat,))
    
    # Test des formules de calcul
    resultat = ResolveurCalcul(texte=_("({MONTANT}*10)+2"), dictValeurs=dictValeurs)
    print((resultat,))