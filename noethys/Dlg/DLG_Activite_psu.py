...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDactivite=None, clsParametres=None):
        ...
        # Bandeau
        titre = _("Paramètres P.S.U.")
        intro = _("Vous pouvez ici renseigner les paramètres P.S.U. de l'activité.")
        self.SetTitle(titre)
    ...
    def __set_properties(self):
        self.radio_activation_oui.SetToolTip(wx.ToolTip(_("Cliquez ici pour activer le mode P.S.U. pour cette activité")))
        self.radio_activation_non.SetToolTip(wx.ToolTip(_("Cliquez ici pour désactiver le mode P.S.U. pour cette activité")))
        self.ctrl_unite_prevision.SetToolTip(wx.ToolTip(_("Sélectionnez l'unité de consommation qui doit être utilisée comme unité de prévision")))
        self.ctrl_unite_presence.SetToolTip(wx.ToolTip(_("Sélectionnez l'unité de consommation qui doit être utilisée comme unité de présence")))
        self.ctrl_tarif_forfait.SetToolTip(wx.ToolTip(_("Sélectionnez le tarif qui doit être utilisé comme forfait-crédit pour les mensualités")))
        self.ctrl_etiquette_rtt.SetToolTip(wx.ToolTip(_("Sélectionnez l'étiquette qui représente les absences RTT")))
    ...
    def OnBoutonOk(self, event):
        ...
        if psu_activation == 1 :
            if psu_unite_prevision == None :
                dlg = wx.MessageDialog(self.parent, _("Vous avez activé le mode P.S.U. mais sans sélectionner d'unité de prévision !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
            if psu_unite_presence == None :
                dlg = wx.MessageDialog(self.parent, _("Vous avez activé le mode P.S.U. mais sans sélectionner d'unité de présence !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
            if psu_tarif_forfait == None :
                dlg = wx.MessageDialog(self.parent, _("Vous avez activé le mode P.S.U. mais sans sélectionner de tarif forfait-crédit !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
