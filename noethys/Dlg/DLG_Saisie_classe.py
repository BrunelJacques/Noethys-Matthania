...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDecole=None, IDclasse=None):
        ...
        self.label_nom = wx.StaticText(self, -1, _("Nom :"), style=wx.ALIGN_RIGHT)
        self.ctrl_nom = wx.TextCtrl(self, -1, "")
        
        self.label_saison = wx.StaticText(self, -1, _("Saison :"), style=wx.ALIGN_RIGHT)
        self.label_du = wx.StaticText(self, -1, "Du", style=wx.ALIGN_RIGHT)
        self.ctrl_date_debut = CTRL_Saisie_date.Date2(self)
        self.label_au = wx.StaticText(self, -1, _("au"), style=wx.ALIGN_RIGHT)
        self.ctrl_date_fin = CTRL_Saisie_date.Date2(self)
        
        self.label_niveaux = wx.StaticText(self, -1, _("Niveaux :"), style=wx.ALIGN_RIGHT)
        ...
        if self.IDclasse == None:
            self.SetTitle(_("Saisie d'une classe"))
            DB = GestionDB.DB()
            ...
            self.SetTitle(_("Modification d'une classe"))
            self.Importation()

    def __set_properties(self):
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez ici le nom de la classe. Ex : 'CP - Mme PICHON'...")))
        self.ctrl_date_debut.SetToolTip(wx.ToolTip(_("Saisissez ici la date de début de saison")))
        self.ctrl_date_fin.SetToolTip(wx.ToolTip(_("Saisissez ici la date de fin de saison")))
        self.ctrl_niveaux.SetToolTip(wx.ToolTip(_("Cochez les niveaux scolaires de la classe")))
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez ici pour obtenir l'aide")))
    ...
    def Importation(self):
        ...
        if len(listeDonnees) > 0:
            ...
            dlg = wx.MessageDialog(self, _("Cette classe a déjà été attribuée à %d individus.\n\nVous ne pourrez donc pas modifier la période ni les niveaux suivants : %s.") % (len(listeDonnees), ", ".join(liste_noms_niveaux)), _("Avertissement"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
    ...
    def OnBoutonOk(self, event):
        ...
        if nom == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un nom pour cette classe !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if date_debut == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner une date de début de saison !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if date_fin == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner une date de fin de saison !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if date_debut > date_fin :
            dlg = wx.MessageDialog(self, _("La date de début ne peut pas être supérieure à celle de fin !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_date_debut.SetFocus()
            return
        ...
        if len(listeNiveaux) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun niveau scolaire pour cette classe. \n\nEtes-vous sûr de vouloir tout de même valider cette saisie ?"), _("Attention"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
        ...
        if len(self.niveaux_bloques):
            for IDniveau, niveau in self.niveaux_bloques.items():
                if IDniveau not in listeNiveaux:
                    dlg = wx.MessageDialog(self, _("Vous ne pouvez pas décocher le niveau '%s' car il est déjà utilisé par un individu !") % niveau["nom"], _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                    dlg.ShowModal()
                    dlg.Destroy()
                    return False
