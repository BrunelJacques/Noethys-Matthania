...
class Dialog(wx.Dialog):
    def __init__(self, parent,  nomUtilisateur="", nomHote="", nomBase=""):
        ...
        # Nom Utilisateur
        self.sizer_nom_staticbox = wx.StaticBox(self, -1, _("Nom de l'utilisateur"))
        self.label_nom = wx.StaticText(self, -1, "Nom :")
        self.ctrl_nom = wx.TextCtrl(self, -1, self.nomUtilisateur)
        
        # Mot de passe
        self.sizer_mdp_staticbox = wx.StaticBox(self, -1, _("Mot de passe"))
        ...
        # Hotes utilisateur
        self.sizer_hotes_staticbox = wx.StaticBox(self, -1, _("Hôtes de connexion"))
        self.radio_1 = wx.RadioButton(self, -1, _("Connexion depuis n'importe quel hôte (recommandé)"), style=wx.RB_GROUP)
        self.radio_2 = wx.RadioButton(self, -1, _("Connexion uniquement depuis le serveur principal"))
        self.radio_3 = wx.RadioButton(self, -1, _("Connexion uniquement depuis l'hôte suivant :"))
        self.ctrl_hote = wx.TextCtrl(self, -1, "")
        
        # Autorisation d'accès au fichier
        self.sizer_autorisation_staticbox = wx.StaticBox(self, -1, _("Autorisation d'accès au fichier '%s'") % self.nomBase)
        self.ctrl_autorisation = wx.CheckBox(self, -1, _("Cet utilisateur est autorisé à se connecter au fichier"))
        ...
        if self.nomUtilisateur == "" :
            self.mode = "creation"
            self.SetTitle(_("Création d'un utilisateur réseau"))
        else:
            self.mode = "modification"
            self.SetTitle(_("Modification d'un utilisateur réseau"))
    ...
    def __set_properties(self):
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez ici un nom pour l'utilisateur. \nIl est fortement conseillé de n'inclure aucun caractères spéciaux (accents, symboles...)")))
        self.radio_1.SetToolTip(wx.ToolTip(_("L'utilisateur ne peut se connecter à la base de données\n qu'à partir du serveur sur lequel se trouve la base de données.")))
        self.radio_2.SetToolTip(wx.ToolTip(_("L'utilisateur ne peut se connecter à la base de données\n qu'à partir des ordinateurs dont les adresses locales ou\n distantes sont cochées dans la liste suivante.")))
        self.radio_3.SetToolTip(wx.ToolTip(_("L'utilisateur ne peut se connecter à la base de données\n qu'à partir des ordinateurs dont les adresses locales ou\n distantes sont cochées dans la liste suivante.")))
        self.ctrl_mdp_1.SetToolTip(wx.ToolTip(_("Saisissez un mot de passe pour cet utilisateur")))
        self.ctrl_mdp_2.SetToolTip(wx.ToolTip(_("Confirmez le mot de passe en le tapant une seconde fois...")))
        self.ctrl_autorisation.SetToolTip(wx.ToolTip(_("Cochez cette case pour autoriser l'utilisateur à se connecter \nau fichier à partir de l'hôte indiqué ci-dessus.")))
    ...
    def RechercheAutorisation(self):
        ...
        req = "SELECT host, db, user FROM db WHERE user='%s' and db='%s' and host='%s';" % (self.nomUtilisateur, "%s_data" % self.nomBase, hote)
    ...
    def OnBoutonOk(self, event):
        ...
        if textNom == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement donner un nom pour cet utilisateur !"), "Information", wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        for caract in textNom :
            if caract in "ïîôûëê~&é(-è_çà)=~#{[|`\^@]}" :
                erreur = True
        if erreur == True :
            dlg = wx.MessageDialog(self, _("Le nom d'utilisateur ne devrait pas inclure de caractères spéciaux (accents, symboles...)"), "Information", wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_nom.SetFocus()
            return
        ...
        if mdp1 != mdp2 :
            dlg = wx.MessageDialog(self, _("Le mot de passe ne correspond pas à la confirmation."), "Information", wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if mdp1 == "" :
            txtMessage = unicode(_("Attention, vous n'avez saisi aucun mot de passe pour cet utilisateur.\nCela peut être risqué pour la sécurité des données.\n\nVoulez-vous quand même valider la création de cet utilisateur sans mot de passe ?\n\n(Cliquez sur Non ou sur Annuler pour saisir un mot de de passe)"))
            dlgConfirm = wx.MessageDialog(self, txtMessage, _("Confirmation de suppression"), wx.YES_NO|wx.CANCEL|wx.NO_DEFAULT|wx.ICON_QUESTION)
        ...
        if textHote == "" and self.radio_3.GetValue() == True :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un hote spécifique"), "Information", wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
    ...
    def Sauvegarde(self):
        ...
        donnees = DB.ResultatReq()
        if len(donnees) == 0 :
            req = "CREATE USER '%s'@'%s' IDENTIFIED BY '%s';" % (nom, hote, motdepasse)
            DB.ExecuterReq(req,MsgBox="ExecuterReq")
            print("L'utilisateur a ete cree.")
        else:
            print("L'utilisateur existe deja.")
            DB.Close()
            dlg = wx.MessageDialog(self, _("Cet utilisateur avec cet hôte existe déjà !"), "Information", wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return False
...
if __name__ == "__main__":
    ...
    dialog_1 = Dialog(None, nomUtilisateur=_("Bernard"), nomHote="%", nomBase="test1")
