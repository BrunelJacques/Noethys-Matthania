...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDscolarite=None, donneesScolarite=[]):
        ...
        # Classe
        self.box_scolarite_staticbox = wx.StaticBox(self, -1, _("Classe"))
        self.label_ecole = wx.StaticText(self, -1, _("Ecole :"))
        self.ctrl_ecole = CTRL_Ecole(self)
        self.label_classe = wx.StaticText(self, -1, _("Classe :"))
    ...
    def __set_properties(self):
        self.SetTitle(_("Saisie d'une étape de la scolarité"))
        self.ctrl_date_debut.SetToolTip(wx.ToolTip(_("Saisissez la date de debut")))
        self.ctrl_date_fin.SetToolTip(wx.ToolTip(_("Saisissez la date de fin")))
        self.ctrl_ecole.SetToolTip(wx.ToolTip(_("Sélectionnez une école")))
        self.ctrl_classe.SetToolTip(wx.ToolTip(_("Sélectionnez une classe")))
        self.ctrl_niveau.SetToolTip(wx.ToolTip(_("Sélectionnez un niveau scolaire")))
    ...
    def OnBoutonOk(self, event): 
        ...
        if date_debut > date_fin :
            dlg = wx.MessageDialog(self, _("La date de début ne peut pas être supérieure à la date de fin !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_date_debut.SetFocus()
            return
        ...
        if self.ctrl_ecole.GetEcole() == None :
            dlg = wx.MessageDialog(self, _("Vous devez sélectionner au moins une école !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.ctrl_classe.GetClasse() == None and self.ctrl_classe.IsEnabled() :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun classe. \nVoulez-vous quand même valider la saisie ?"), _("Attention"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
            if dlg.ShowModal() != wx.ID_YES :
                return

        if self.ctrl_niveau.GetNiveau() == None and self.ctrl_niveau.IsEnabled() :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun niveau scolaire. \nVoulez-vous quand même valider la saisie ?"), _("Attention"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
            if dlg.ShowModal() != wx.ID_YES :
                return
        ...
        for track in self.donneesScolarite :
            if track.IDscolarite != self.IDscolarite and date_debut <= track.dateFinDD and date_fin >= track.dateDebutDD :
                ...
                dlg = wx.MessageDialog(self, _("Vous ne pouvez pas effectuer cette inscription scolaire car\ncet individu est déjà inscrit sur une classe sur cette période :\n\nPériode : Du %s au %s\nEcole : %s\nClasse : %s") % (date_debut, date_fin, nomEcole, nomClasse), _("Erreur de saisie"), wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                return
