...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDregie=None):
        ...
        self.label_numclitipi = wx.StaticText(self, wx.ID_ANY, _("Numéro Client TIPI :"))
        self.ctrl_numclitipi = wx.TextCtrl(self, wx.ID_ANY, "")

        self.label_emailregisseur = wx.StaticText(self, wx.ID_ANY, _("Mail du régisseur :"))
        self.ctrl_emailregisseur = CTRL_Saisie_mail.Mail(self)

        self.label_compte_bancaire = wx.StaticText(self, wx.ID_ANY, _("Compte bancaire associé :"))
        ...
        if self.IDregie != None :
            self.SetTitle(_("Modification d'une régie de facturation"))
            self.Importation() 
        else :
            self.SetTitle(_("Saisie d'une régie de facturation"))

    def __set_properties(self):
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez le nom de la régie")))
        self.ctrl_numclitipi.SetToolTip(wx.ToolTip(_("Saisissez le numéro de client TIPI")))
        self.ctrl_emailregisseur.SetToolTip(wx.ToolTip(_("Saisissez l addresse mail du régisseur")))
        self.ctrl_compte_bancaire.SetToolTip(wx.ToolTip(_("Sélectionnez le compte bancaire associé a cette régie")))
    ...
    def Sauvegarde(self):
        ...
        #if numclitipi == "" :
        #    dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un numéro de client TIPI!"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
        #    dlg.ShowModal()
        #    dlg.Destroy()
        ...
        for caract in numclitipi :
            if caract not in "0123456789" :
                dlg = wx.MessageDialog(self, _("Le numéro de client TIPI ne peut comporter que des chiffres !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_numclitipi.SetFocus()
                return False
        ...
        if numclitipi != "" and emailregisseur == "":
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir l addresse mail du régisseur si vous avez saisi un numéro de client TIPI !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
