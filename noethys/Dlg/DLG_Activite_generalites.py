...
class Panel(wx.Panel):
    ...
    def __set_properties(self):
        self.ctrl_nom_complet.SetToolTip(wx.ToolTip(_("Saisissez ici le nom complet de l'activité")))
        self.ctrl_nom_abrege.SetMinSize((60, -1))
        self.ctrl_nom_abrege.SetToolTip(wx.ToolTip(_("Saisissez ici le nom abrégé de l'activité")))
        self.radio_coords_org.SetToolTip(wx.ToolTip(_("Selectionnez 'Autres coordonnées' si vous souhaitez attribuer des coordonnées différentes de celles de l'organisateur")))
        self.radio_coords_autres.SetToolTip(wx.ToolTip(_("Selectionnez 'Autres coordonnées' si vous souhaitez attribuer des coordonnées différentes de celles de l'organisateur")))
        self.ctrl_rue.SetToolTip(wx.ToolTip(_("Saisissez l'adresse de l'activité")))
        self.ctrl_site.SetToolTip(wx.ToolTip(_("Saisissez l'adresse du site internet de l'activité")))
        self.ctrl_responsables.SetToolTip(wx.ToolTip(_("Liste des responsables de l'activité")))
        self.bouton_ajouter_responsable.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter un nom de responsable")))
        self.bouton_modifier_responsable.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier le nom sélectionné dans la liste")))
        self.bouton_supprimer_responsable.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer le nom sélectionné dans la liste")))
        ...
        self.radio_illimitee.SetToolTip(wx.ToolTip(_("Selectionnez 'Illimitee' si l'activité est un accueil de loisirs")))
        self.radio_limitee.SetToolTip(wx.ToolTip(_("Saisissez une periode de validité si l'activite fonctionne uniquement sur une période donnée")))
        self.ctrl_validite_du.SetToolTip(wx.ToolTip(_("Saisissez une date de debut de validité")))
        self.ctrl_validite_au.SetToolTip(wx.ToolTip(_("Saisissez ici une date de fin de validité")))
        self.ctrl_public.SetToolTip(wx.ToolTip(_("Cochez les publics autorisés à être inscrit à cette activité")))
    ...
    def Validation(self):
        ...
        if self.ctrl_nom_complet.GetValue() == "" :
            dlg = wx.MessageDialog(self, _("Le nom de l'activité doit être obligatoirement saisi !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_nom_complet.SetFocus() 
            return False
        if self.ctrl_nom_abrege.GetValue() == "" :
            dlg = wx.MessageDialog(self, _("Le nom abrégé de l'activité doit être obligatoirement saisi !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        # if len(public) == 0 :
        #     dlg = wx.MessageDialog(self, _("Vous devez cocher au moins un type de public !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
        #     dlg.ShowModal()
        #     dlg.Destroy()
        #     return False
        # Dates de validité
        if self.radio_limitee.GetValue() == True :
            if self.ctrl_validite_du.Validation() == False :
                dlg = wx.MessageDialog(self, _("La date de début de validité ne semble pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_validite_du.SetFocus()
                return False
            if self.ctrl_validite_du.GetDate() == None :
                dlg = wx.MessageDialog(self, _("La date de début de validité n'a pas été spécifiée !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_validite_du.SetFocus()
                return False
            if self.ctrl_validite_au.Validation() == False :
                dlg = wx.MessageDialog(self, _("La date de fin de validité ne semble pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_validite_au.SetFocus()
                return False
            if self.ctrl_validite_au.GetDate() == None :
                dlg = wx.MessageDialog(self, _("La date de fin de validité n'a pas été spécifiée !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_validite_au.SetFocus()
                return False
            if self.ctrl_validite_du.GetDate() > self.ctrl_validite_au.GetDate() :
                dlg = wx.MessageDialog(self, _("La date de validité de début ne peut pas être supérieure à la date de fin !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_validite_du.SetFocus()
                return False
        ...
        if logo_org == 0 :
            if self.ctrl_logo.GetBuffer() == None :
                dlg = wx.MessageDialog(self, _("Vous avez sélectionné un logo personnalisé pour cette activité mais vous n'en avez sélectionné aucun !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        # Sauvegarde
