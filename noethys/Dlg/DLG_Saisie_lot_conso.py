...
class CTRL_Individus(wx.CheckListBox):
    ...
    def MAJ(self):
        ...
        FROM inscriptions
        WHERE (NOT inscriptions.statut LIKE 'ko%%') AND IDindividu IN %s;""" % condition
        DB.ExecuterReq(req,MsgBox="ExecuterReq")
        ...
        for IDindividu, nom, prenom in listeIndividus :
            ...
            if IDindividu in dictInscriptions :
                ...
                inscriptions = []
            listeValeurs.append({"IDindividu":IDindividu, "label": "%s %s" % (nom, prenom), "inscriptions":inscriptions, "selection":False})
...
class CTRL_Horaire(wx.Panel):
    def __init__(self, parent, treeListMainWindow=None, item=None, niveauUnite=None, labelUnite="", multihoraires=False, action=None):
        ...
        # Contrôles
        self.label_heure_debut = wx.StaticText(self, -1, _("Horaire :"))
        self.ctrl_heure_debut = CTRL_Saisie_heure.Heure(self)
        self.label_heure_fin = wx.StaticText(self, -1, "à")
    ...
    def Validation(self):
        ...
        if heure_debut == None :
            dlg = wx.MessageDialog(self, _("Vous devez saisir une heure de début pour l'unité '%s' !") % self.labelUnite, _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_heure_debut.SetFocus() 
            return False
        if heure_fin == None :
            dlg = wx.MessageDialog(self, _("Vous devez saisir une heure de fin pour l'unité '%s' !") % self.labelUnite, _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
...
class CTRL_Quantite(wx.Panel):
    def __init__(self, parent, item=None, labelUnite=""):
        ...
        # Contrôles
        self.label_quantite = wx.StaticText(self, -1, _("Quantité :"))
...
class CTRL_Unites(HTL.HyperTreeList):
    ...
    def Remplissage(self):
        ...
        for dictUnite in self.listeUnites :
            ...
            if self.parent.action not in ("suppression", "etat") :
                ...
                # CTRL Horaire
                if type == "Horaire" :
                    niveauHoraire = self.AppendItem(niveauUnite, "")
                ...
                # CTRL Quantité
                if type == "Quantite" :
                    niveauQuantite = self.AppendItem(niveauUnite, "")
    ...
    def RetirerHoraires(self, niveauUnite=None, ctrl=None, item=None):
        ...
        if len(data["controles"]) == 1 :
            dlg = wx.MessageDialog(self, _("Vous ne pouvez pas retirer le dernier horaire !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
...
class CTRL_Etat(wx.Choice):
    def __init__(self, parent):
        ...
        self.parent = parent
        self.listeEtats = [
            ("reservation", _("Réservation - Pointage en attente")),
            ("present", _("Réservation - Présent")),
            ("absentj", _("Réservation - Absence justifiée")),
            ("absenti", _("Réservation - Absence injustifiée")),
            ("attente", _("Attente")),
            ("refus", _("Refus")),
            ]
        self.MAJ() 
...
class Dialog(wx.Dialog):
    def __init__(self, parent, listeIndividus=[], date_debut=None, date_fin=None, IDactivite=None, mode_parametres=False):
        ...
        # Bandeau
        titre = _("Traitement par lot")
        if self.mode_parametres == True :
            titre = _("Sélectionner les paramètres de l'action")
        intro = _("Vous pouvez ici saisir, modifier ou supprimer un lot de consommations selon les critères de votre choix. Sélectionnez un type d'action puis renseignez les paramètres demandés.")
        self.SetTitle(titre)
        ...
        self.radio_saisie = wx.RadioButton(self, -1, _("Saisie"), style=wx.RB_GROUP)
        self.radio_modification = wx.RadioButton(self, -1, _("Modification"))
        self.radio_suppression = wx.RadioButton(self, -1, _("Suppression"))
        self.radio_etat = wx.RadioButton(self, -1, _("Changement d'état"))

        # Individus
        self.box_individus_staticbox = wx.StaticBox(self, -1, _("Individus"))
        ...
        self.label_semaines = wx.StaticText(self, -1, _("Semaines :"))
        ...
        # Unites
        self.box_unites_staticbox = wx.StaticBox(self, -1, _("Unités"))
        ...
        self.label_evenement = wx.StaticText(self, -1, _("Evènements :"))
        self.check_evenement = wx.CheckBox(self, -1, _("Uniquement les évènements dont le nom contient :"))
    ...
    def __set_properties(self):
        self.bouton_activite.SetToolTip(wx.ToolTip(_("Cliquez ici pour rechercher une activité")))
        self.radio_saisie.SetToolTip(wx.ToolTip(_("Cochez ici pour saisir un lot de consommations")))
        self.radio_modification.SetToolTip(wx.ToolTip(_("Cochez ici pour modifier un lot de consommations")))
        self.radio_suppression.SetToolTip(wx.ToolTip(_("Cochez ici pour supprimer un lot de consommations")))
        self.radio_etat.SetToolTip(wx.ToolTip(_("Cochez ici pour modifier l'état d'un lot de consommations")))
        self.ctrl_individus.SetToolTip(wx.ToolTip(_("Sélectionnez les individus visés")))
        self.ctrl_date_debut.SetToolTip(wx.ToolTip(_("Sélectionnez une date de début")))
        self.ctrl_date_fin.SetToolTip(wx.ToolTip(_("Sélectionnez une date de fin")))
        self.ctrl_semaines.SetToolTip(wx.ToolTip(_("Sélectionnez une fréquence")))
        self.ctrl_feries.SetToolTip(wx.ToolTip(_("Cochez cette case pour inclure les jours fériés dans le processus")))
        self.ctrl_activite.SetToolTip(wx.ToolTip(_("Sélectionnez une activité")))
        self.ctrl_etat.SetToolTip(wx.ToolTip(_("Sélectionnez une état pour les consommations")))
        self.check_evenement.SetToolTip(wx.ToolTip(_("Cocher cette case pour sélectionner uniquement les évènements dont le nom contient l'expression saisie")))
        self.ctrl_evenement.SetToolTip(wx.ToolTip(_("Saisissez un mot ou une expression présente dans le nom des évènements souhaités")))
    ...
    def OnBoutonOk(self, event): 
        ...
        if len(listeIndividus) == 0 and self.mode_parametres == False :
            dlg = wx.MessageDialog(self, _("Vous devez sélectionner au moins un individu !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if IDactivite == None :
            dlg = wx.MessageDialog(self, _("Vous devez sélectionner une activité !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.mode_parametres == False :
            for dictIndividu in listeIndividus :
                if IDactivite not in dictIndividu["inscriptions"] :
                    dlg = wx.MessageDialog(self, _("%s n'est pas inscrit à l'activité '%s' !") % (dictIndividu["label"], nomActivite), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                    dlg.ShowModal()
                    dlg.Destroy()
                    return
        ...
        if len(listeCoches) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez sélectionner au moins une unité !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if self.check_evenement.GetValue() == True :
            ...
            if len(expression) == 0 :
                dlg = wx.MessageDialog(self, _("Vous devez saisir une expression pour sélectionner les évènements !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_evenement.SetFocus()
                return
        ...
        if self.action == "saisie" : description = _("Saisie de consommations sur la période")
        if self.action == "modification" : description = _("Modification de consommations sur la période")
        if self.action == "suppression" : description = _("Suppression de consommations sur la période")
        if self.action == "etat" : description = _("Changement d'état des consommations de la période")
        description += " du %s au %s" % (UTILS_Dates.DateDDEnFr(date_debut), UTILS_Dates.DateDDEnFr(date_fin))
