...
from Ol import OL_Modeles_tickets
...
class CTRL_Categorie(wx.Choice):
    ...
    def GetListeDonnees(self):
        ...
        for dictCategorie in CATEGORIES :
            listeItems.append(dictCategorie["label"])
            index += 1
        return listeItems
    ...
    def SetCategorie(self, categorie=""):
        index = 0
        for dictCategorie in CATEGORIES :
            if dictCategorie["code"] == categorie :
...
class Dialog(wx.Dialog):
    def __init__(self, parent, categorie=None):
        wx.Dialog.__init__(self, parent, -1, name="DLG_modeles_tickets", style=wx.DEFAULT_DIALOG_STYLE|wx.RESIZE_BORDER|wx.MAXIMIZE_BOX|wx.MINIMIZE_BOX)
        self.parent = parent
        
        # Bandeau
        titre = _("Modèles de tickets")
        intro = _("Vous pouvez paramétrer ici des modèles de tickets qui sont utilisables uniquement avec des imprimantes de tickets.")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Ticket.png")
        ...
        self.ctrl_modeles = OL_Modeles_tickets.ListView(self, id=-1, style=wx.LC_REPORT|wx.SUNKEN_BORDER|wx.LC_SINGLE_SEL|wx.LC_HRULES|wx.LC_VRULES)
