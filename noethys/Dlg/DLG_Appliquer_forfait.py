...
class CTRL(HTL.HyperTreeList):
    def __init__(self, parent, IDfamille, listeActivites=[], listeIndividus=[], saisieManuelle=True, saisieAuto=False): 
        ...
        self.AddColumn(_("Individu/Activité/Prestation"))
        self.SetColumnWidth(0, 330)
        self.SetColumnAlignment(0, wx.ALIGN_LEFT)
        
        self.AddColumn(_("Période du forfait"))
                ...
                if IDactivite in dictForfaits and len(dictForfaits[IDactivite]["tarifs"])> 0 :
                    ...
                    for dictTarif in dictActivite["tarifs"] :
                        ...
                        if IDcategorie_tarif in dictTarif["categories_tarifs"] and dictTarif["forfait_saisie_manuelle"] == 1 :
                            ...
                            labelDates = _("Du %s au %s") % (date_debut_forfait, date_fin_forfait)
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDfamille=None, listeActivites=[], listeIndividus=[]):
        ...
        intro = _("Vous pouvez ici appliquer un ou plusieurs forfaits datés. Ceux-ci peuvent être paramétrés dans la tarification. Cochez un ou plusieurs forfaits pour l'individu et l'activité de votre choix puis cliquez sur Ok.")
        titre = _("Appliquer des forfaits datés")
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Forfait.png")
        self.SetTitle(titre)

        self.staticbox_forfaits = wx.StaticBox(self, -1, _("Cochez un ou plusieurs forfaits"))
        ...
        self.check_masquer_obsoletes = wx.CheckBox(self, -1, _("Masquer les forfaits obsolètes"))
        self.check_masquer_obsoletes.SetValue(True)

        self.bouton_aide = CTRL_Bouton_image.CTRL(self, texte=_("Aide"), cheminImage="Images/32x32/Aide.png")
        self.bouton_ok = CTRL_Bouton_image.CTRL(self, texte=_("Appliquer"), cheminImage="Images/32x32/Valider.png")
    ...
    def __set_properties(self):
        self.ctrl_forfaits.SetToolTip(wx.ToolTip(_("Double-cliquez sur un forfait pour l'appliquer")))
        self.check_masquer_obsoletes.SetToolTip(wx.ToolTip(_("Masquer les forfaits dont la date de fin de validité est inférieure à la date du jour")))
    ...
    def OnBoutonOk(self, event): 
        ...
        if len(listeTarifs) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez coché aucun forfait à appliquer !"), "Erreur", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
...
class DLG_Verrouillage(wx.Dialog):
    def __init__(self, parent):
        ...
        intro = _("La date de facturation du forfait que vous souhaitez créer se trouve dans une période de gestion verrouillée.\n\nSouhaitez-vous appliquer une autre date de facturation ?")
        self.label_ligne_2 = wx.StaticText(self, wx.ID_ANY, intro)
        self.ctrl_date = CTRL_Saisie_date.Date2(self)
        self.ctrl_date.SetDate(datetime.date.today())

        self.bouton_oui = CTRL_Bouton_image.CTRL(self, texte=_("Créer le forfait avec la date sélectionnée"), cheminImage="Images/32x32/Valider.png")
        self.bouton_non = CTRL_Bouton_image.CTRL(self, texte=_("Ne pas créer le forfait"), cheminImage="Images/32x32/Annuler.png")
    ...
    def __set_properties(self):
        ...
        self.bouton_oui.SetToolTip(wx.ToolTip(_("Cliquez ici pour appliquer la date de facturation sélectionnée")))
        self.bouton_non.SetToolTip(wx.ToolTip(_("Cliquez ici pour fermer et ne pas créer de forfait")))
    ...
    def OnBoutonOui(self, event):
        ...
        if self.ctrl_date.FonctionValiderDate() == False or date_facturation == None:
            dlg = wx.MessageDialog(self, _("La date de facturation ne semble pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.gestion.Verification("prestations", date_facturation, silencieux=True) == False:
            dlg = wx.MessageDialog(self, _("La date de facturation qaue vous avez sélectionné se trouve dans une période de gestion verrouillée !"), _("Verrouillage"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
...
class DLG_Date_facturation(wx.Dialog):
    def __init__(self, parent, date_facturation=None):
        ...
        self.SetTitle(_("Avertissement"))
        self.label_intro = wx.StaticText(self, wx.ID_ANY, _("La date de facturation du forfait à appliquer est ancienne. \n\nQue souhaitez-vous faire ?"))

        self.radio_conserver = wx.RadioButton(self, -1, _("Conserver la date (%s)") % UTILS_Dates.DateDDEnFr(self.date_facturation), style=wx.RB_GROUP)
        self.radio_modifier = wx.RadioButton(self, -1, _("Modifier la date :"))

        self.ctrl_date = CTRL_Saisie_date.Date2(self)
        self.ctrl_date.SetDate(datetime.date.today())

        self.label_conclusion = wx.StaticText(self, wx.ID_ANY, _("Notez que vous pouvez modifier la date de facturation par défaut depuis le paramétrage\ndu tarif du forfait, onglet type de tarif."))
    ...
    def OnBoutonOk(self, event):
        date_facturation = self.ctrl_date.GetDate()
        if self.ctrl_date.FonctionValiderDate() == False or date_facturation == None:
            dlg = wx.MessageDialog(self, _("La date de facturation saisie ne semble pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_date.SetFocus()
            return False
