...
def Autodetection(parent=None):
    ...
    dlg = wx.MessageDialog(parent, _("Des anomalies ont été détectées dans ce fichier de données.\n\nSouhaitez-vous lancer maintenant le correcteur d'anomalies ?"), _("Anomalies"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
...
class CTRL(CT.CustomTreeCtrl):
    ...
    def MAJ(self):
        ...
        # RAZ ctrl
        self.DeleteAllItems()
        if nbreAnomalies == 0 : texte = _("Aucune anomalie détectée")
        elif nbreAnomalies == 1 : texte = _("1 anomalie détectée :")
        else : texte = _("%d anomalies détectées :") % nbreAnomalies
        ...
        for labelAnomalie, labelCorrection, listeAnomalies in listeResultats :
            if len(listeAnomalies) > 0 :
                niveauRubrique = self.AppendItem(self.root, "%s (%d)" % (labelAnomalie, len(listeAnomalies)), ct_type=1)
    ...
    def Correction(self):
        ...
        if nbreCorrections == 0 :
            message = _("Aucune correction n'a été effectuée !")
        elif nbreCorrections == 1 :
            message = _("1 correction a bien été effectuée !")
        else :
            message = _("%d corrections ont bien été effectuées !") % nbreCorrections
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        intro = _("Cette fonction permet de détecter et corriger automatiquement certaines anomalies qui pourraient survenir après des erreurs de manipulation ou des bugs non identifiés. Cochez les éléments à corriger puis cliquez sur le bouton Corriger.")
        titre = _("Correcteur d'anomalies")
        self.SetTitle(titre)
        ...
        self.bouton_ok = CTRL_Bouton_image.CTRL(self, texte=_("Corriger les anomalies cochées"), cheminImage="Images/32x32/Depannage.png")
    ...
    def __set_properties(self):
        ...
        self.bouton_ok.SetToolTip(wx.ToolTip(_("Cliquez ici pour corriger les anomalies cochées")))
    ...
    def OnBoutonOk(self, event):
        ...
        if self.sauvegardeEffectuee == False :
            dlg = wx.MessageDialog(self, _("Souhaitez-vous sauvegarder votre fichier de données avant la correction (Recommandé) ?"), _("Annulation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
            reponse = dlg.ShowModal()
            dlg.Destroy()
