...
class CTRL_Page_texte(wx.Panel):
    def __init__(self, parent):
        ...
        self.parent = parent
        self.code = None
        self.radio_egal = wx.RadioButton(self, -1, _("Est égal à"), style=wx.RB_GROUP)
        self.ctrl_egal = wx.TextCtrl(self, -1, "", size=(200, -1))
        self.radio_different = wx.RadioButton(self, -1, _("Est différent de"))
        self.ctrl_different = wx.TextCtrl(self, -1, "", size=(200, -1))
        self.radio_contient = wx.RadioButton(self, -1, _("Contient"))
        self.ctrl_contient = wx.TextCtrl(self, -1, "", size=(200, -1))
        self.radio_contientpas = wx.RadioButton(self, -1, _("Ne contient pas"))
        self.ctrl_contientpas = wx.TextCtrl(self, -1, "", size=(200, -1))
        self.radio_commence = wx.RadioButton(self, -1, _("Commence par"))
        self.ctrl_commence = wx.TextCtrl(self, -1, "", size=(200, -1))
    ...
    def __do_layout(self):
        ...
        grid_sizer_commence = wx.FlexGridSizer(rows=1, cols=2, vgap=5, hgap=5)
        grid_sizer_commence.Add(self.radio_commence, 0, wx.ALIGN_CENTER_VERTICAL, 0)
        grid_sizer_commence.Add(self.ctrl_commence, 0, 0, 0)
    ...
    def GetValeur(self):
        ...
        if self.radio_commence.GetValue() == True :
            choix = "COMMENCE"
            criteres = self.ctrl_commence.GetValue()
    ...
    def Validation(self):
        ...
        if choix not in ("VIDE", "PASVIDE") and criteres == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un texte !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        return True
    ...
    def Reinit(self):
        # self.code est le nom de la colonne à filtrer
        self.radio_egal.SetValue(True)
...
class CTRL_Page_bool(wx.Panel):
    def __init__(self, parent):
        wx.Panel.__init__(self, parent, id=-1, style=wx.TAB_TRAVERSAL)
        self.radio_vrai = wx.RadioButton(self, -1, _("Est vrai"), style=wx.RB_GROUP)
        self.radio_faux = wx.RadioButton(self, -1, _("Est faux"))
...
class CTRL_Page_entier(wx.Panel):
    def __init__(self, parent):
        ...
        self.radio_sup = wx.RadioButton(self, -1, _("Est supérieur à"))
        self.ctrl_sup = wx.SpinCtrl(self, -1, "", min=0, max=999999)
        self.radio_supegal = wx.RadioButton(self, -1, _("Est supérieur ou égal à"))
        self.ctrl_supegal = wx.SpinCtrl(self, -1, "", min=0, max=999999)
        self.radio_inf = wx.RadioButton(self, -1, _("Est inférieur à"))
        self.ctrl_inf = wx.SpinCtrl(self, -1, "", min=0, max=999999)
        self.radio_infegal = wx.RadioButton(self, -1, _("Est inférieur ou égal à"))
        self.ctrl_infegal = wx.SpinCtrl(self, -1, "", min=0, max=999999)
        self.radio_compris = wx.RadioButton(self, -1, _("Est compris entre"))
...
class CTRL_Page_date(wx.Panel):
    ...
    def Validation(self):
        choix, criteres = self.GetValeur() 
        if criteres == None and False :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        return True
...
class CTRL_Page_inscrits(wx.Panel):
    def __init__(self, parent):
        ...
        self.check_presents.SetToolTip(wx.ToolTip(_("Cochez cette case pour saisir une période de présence")))
        self.ctrl_date_debut.SetToolTip(wx.ToolTip(_("Saisissez ici la date de début de période")))
        self.ctrl_date_fin.SetToolTip(wx.ToolTip(_("Saisissez ici la date de fin de période")))

        # Date d'inscription
        self.check_inscription = wx.CheckBox(self, -1, _("Et inscrits entre le"))
        self.ctrl_date_debut_inscription = CTRL_Saisie_date.Date2(self)
        self.label_au_inscription = wx.StaticText(self, -1, _(" et le"))
        self.ctrl_date_fin_inscription = CTRL_Saisie_date.Date2(self)

        self.check_inscription.SetToolTip(wx.ToolTip(_("Cochez cette case pour sélectionner les individus suivant leur date d'inscription à l'activité")))
        self.ctrl_date_debut_inscription.SetToolTip(wx.ToolTip(_("Saisissez ici la date de début d'inscription")))
        self.ctrl_date_fin_inscription.SetToolTip(wx.ToolTip(_("Saisissez ici la date de fin d'inscription")))

        # Date de naissance
        self.check_naissance = wx.CheckBox(self, -1, _("Et nés entre le"))
        self.ctrl_date_debut_naissance = CTRL_Saisie_date.Date2(self)
        self.label_au_naissance = wx.StaticText(self, -1, _(" et le"))
        self.ctrl_date_fin_naissance = CTRL_Saisie_date.Date2(self)

        self.check_naissance.SetToolTip(wx.ToolTip(_("Cochez cette case pour sélectionner les individus suivant leur date de naissance")))
        self.ctrl_date_debut_naissance.SetToolTip(wx.ToolTip(_("Saisissez ici une date minimale")))
        self.ctrl_date_fin_naissance.SetToolTip(wx.ToolTip(_("Saisissez ici la date maximale")))
    ...
    def Validation(self):
        ...
        if self.check_presents.GetValue() == True :
            if self.ctrl_date_debut.GetDate() == None or self.ctrl_date_debut.Validation() == False :
                dlg = wx.MessageDialog(self, _("Vous n'avez saisi aucune date de début de période !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
            
            if self.ctrl_date_fin.GetDate() == None or self.ctrl_date_fin.Validation() == False :
                dlg = wx.MessageDialog(self, _("Vous n'avez saisi aucune date de fin de période !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False

            if self.ctrl_date_debut.GetDate() > self.ctrl_date_fin.GetDate() :
                dlg = wx.MessageDialog(self, _("La date de début est supérieure à la date de fin !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        ...
        if self.check_inscription.GetValue() == True:
            if self.ctrl_date_debut_inscription.GetDate() == None or self.ctrl_date_debut_inscription.Validation() == False:
                dlg = wx.MessageDialog(self, _("Vous n'avez saisi aucune date de début d'inscription !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False

            if self.ctrl_date_fin_inscription.GetDate() == None or self.ctrl_date_fin_inscription.Validation() == False:
                dlg = wx.MessageDialog(self, _("Vous n'avez saisi aucune date de fin d'inscription !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False

            if self.ctrl_date_debut_inscription.GetDate() > self.ctrl_date_fin_inscription.GetDate():
                dlg = wx.MessageDialog(self, _("La date de début est supérieure à la date de fin d'inscription!"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        ...
        if self.check_naissance.GetValue() == True:
            if self.ctrl_date_debut_naissance.GetDate() == None or self.ctrl_date_debut_naissance.Validation() == False:
                dlg = wx.MessageDialog(self, _("Vous n'avez saisi aucune date minimale pour la date de naissance !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
            ...
            if self.ctrl_date_debut_naissance.GetDate() > self.ctrl_date_fin_naissance.GetDate():
                dlg = wx.MessageDialog(self, _("La date de minimale est supérieure à la date maximale de naissance!"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
...
class CTRL_Page_cotisations(wx.Panel):
    def __init__(self, parent):
        ...
        # Cotisations
        self.check_cotisations = wx.CheckBox(self, -1, _("Uniquement si au moins une des cotisations cochées est à jour :"))
...
class CTRL_Page_vide(wx.Panel):
    ...
    def Validation(self):
        dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun champ à filtrer !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
        dlg.ShowModal()
        dlg.Destroy()
        return False
...
class CTRL_Page(wx.Panel):
    def __init__(self, parent, typeDonnee=None, titre=""):
        wx.Panel.__init__(self, parent, id=-1, style=wx.SIMPLE_BORDER) 
        self.titre = titre
        self.parent = parent
        ...
        if typeDonnee == "texte" : 
            ...
            self.ctrl_contenu = CTRL_Page_vide(self, texte=_("Veuillez sélectionner un champ disponible dans la liste !"))
...
class CTRL_Filtres_archive(wx.Treebook):
    ...
    def MAJ(self):
        ...
        panel = CTRL_Page_vide(self, texte=_("Veuillez sélectionner un champ dans la liste !"))
        self.AddPage(panel, _("Champs disponibles"), imageId=self.dictImages["colonnes"]["ID"])
        index += 1
...
##                panel = CTRL_Page_vide(self, texte=_("Veuillez sélectionner une question dans la liste !"))
##                if type == "famille" :
##                    label = _("Questions familiales")
##                else :
##                    label = _("Questions individuelles")
...
class Dialog_archive(wx.Dialog):
    def __init__(self, parent, ctrl_listview=None):
        ...
        self.parent = parent      
        self.ctrl_listview = ctrl_listview
        self.SetTitle(_("Saisie d'un filtre"))  
    ...
    def SetCode(self, code=""):
        self.SetTitle(_("Modification d'un filtre"))  
...
class CTRL_Champs(wx.TreeCtrl):
    def __init__(self, parent, ctrl_listview=None):
        ...
        self.listeCategories = [
            ("speciaux", _("Filtres spéciaux")),
            ("colonnes", _("Filtres de colonnes")),
            ]
...
class CTRL_Filtres(wx.Panel):
    def __init__(self, parent):
        ...
        self.listeTypesDonnees = ["vide", "texte", "bool", "entier", "montant", "date", "inscrits"]
...
if __name__ == "__main__":
    ...
    # Test d'importation
    #dlg.SetCode("famille")
    #dlg.SetValeur("CONTIENT", _("Ceci est un test  !"))
