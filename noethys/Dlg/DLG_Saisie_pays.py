...
class Dialog_pays(wx.Dialog):
    def __init__(self, parent, typeSelection="pays"):
        ...
        if self.typeSelection == "pays" :
            intro = _("Vous pouvez sélectionner ici un pays en double-cliquant sur sa ligne. La barre de recherche peut vous permettre de retrouver rapidement un élément de la liste.")
            titre = _("Sélection d'un pays")
        else:
            intro = _("Vous pouvez sélectionner ici une nationalité en double-cliquant sur sa ligne. La barre de recherche peut vous permettre de retrouver rapidement un élément de la liste.")
            titre = _("Sélection d'une nationalité")
    ...
    def __set_properties(self):
        if self.typeSelection == "pays" :
            self.SetTitle(_("Sélection d'un pays"))
        else:
            self.SetTitle(_("Sélection d'une nationalité"))
    ...
    def OnBoutonOk(self, event):
        ...
        if len(selection) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez saisi aucun élément dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
