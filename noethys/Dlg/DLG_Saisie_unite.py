...
class CTRL_Raccourci(wx.Choice):
    ...
    def GetListeDonnees(self):
        ...
        listeItems = [ _("------------- Aucune touche de raccourci -------------"), ]
...
class CTRL_Type(CTRL_Ultrachoice.CTRL):
    def __init__(self, parent):
        donnees=[ 
            {"label" : _("Standard"), "description" : _("Pour saisir une conso simple par case"), "image" : wx.Bitmap(Chemins.GetStaticPath(u"Images/Special/Unite_standard.png"), wx.BITMAP_TYPE_ANY)},
            {"label" : _("Horaire"), "description" : _("Pour saisir un horaire dans chaque case"), "image" : wx.Bitmap(Chemins.GetStaticPath(u"Images/Special/Unite_horaire.png"), wx.BITMAP_TYPE_ANY)},
            {"label" : _("Multi-horaires"), "description" : _("Pour saisir plusieurs conso horaires par case"), "image" : wx.Bitmap(Chemins.GetStaticPath(u"Images/Special/Unite_multihoraires.png"), wx.BITMAP_TYPE_ANY)},
            {"label" : _("Evènementiel"), "description": _("Pour associer des évènements à une case"), "image": wx.Bitmap(Chemins.GetStaticPath(u"Images/Special/Unite_evenement.png"), wx.BITMAP_TYPE_ANY)},
            {"label" : _("Quantité"), "description" : _("Pour attribuer une quantité à une conso"), "image" : wx.Bitmap(Chemins.GetStaticPath(u"Images/Special/Unite_quantite.png"), wx.BITMAP_TYPE_ANY)},
            ]
...
class Dialog(wx.Dialog):
    ...
    def __set_properties(self):
        self.SetTitle(_("Saisie d'une unité"))
        self.SetSize((650, -1))
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez ici le nom complet de l'unité")))
        self.ctrl_abrege.SetMinSize((80, -1))
        self.ctrl_abrege.SetToolTip(wx.ToolTip(_("Saisissez ici le nom abrégé de l'unité")))
        self.ctrl_type.SetToolTip(wx.ToolTip(_("Sélectionnez un type d'unité")))
        self.ctrl_heure_debut.SetToolTip(wx.ToolTip(_("Saisissez l'heure minimale (Ex : 08:30)")))
        self.ctrl_heure_fin.SetToolTip(wx.ToolTip(_("Saisissez l'heure maximale (Ex : 09:30)")))
        self.ctrl_heure_debut_fixe.SetToolTip(wx.ToolTip(_("Cochez cette case si l'heure de début est obligatoirement celle-ci")))
        self.ctrl_heure_fin_fixe.SetToolTip(wx.ToolTip(_("Cochez cette case si l'heure de fin est obligatoirement celle-ci")))
        self.radio_groupes_tous.SetToolTip(wx.ToolTip(_("Cochez ici pour que tous les groupes bénéficient de cette unité")))
        self.radio_groupes_suivants.SetToolTip(wx.ToolTip(_("Cochez ici pour sélectionner certains groupes")))
        self.ctrl_repas.SetToolTip(wx.ToolTip(_("Cochez cette case si cette unité est ou comporte un repas")))
        self.ctrl_restaurateur.SetToolTip(wx.ToolTip(_("Selectionnez un restaurateur")))
        self.bouton_restaurateur.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à la gestion des restaurateurs")))
        self.ctrl_incompat.SetToolTip(wx.ToolTip(_("Cochez les unités qui sont incompatibles avec cette unité")))
        self.check_autogen.SetToolTip(wx.ToolTip(_("Cochez cette case pour activer l'auto-génération de cette unité de conommation")))
        self.bouton_autogen.SetToolTip(wx.ToolTip(_("Cliquez sur ce bouton pour renseigner les paramètres de l'auto-génération")))
        self.radio_illimitee.SetToolTip(wx.ToolTip(_("Cochez ici si l'unité est valable sur toute la durée de validité de l'activité")))
        self.radio_limitee.SetToolTip(wx.ToolTip(_("Cliquez ici pour définir une période de validité précise")))
        self.ctrl_date_debut.SetToolTip(wx.ToolTip(_("Saisissez une date de début")))
        self.ctrl_date_fin.SetToolTip(wx.ToolTip(_("Saisissez une date de fin")))
        self.ctrl_raccourci.SetToolTip(wx.ToolTip(_("La touche de raccourci est utile dans la grille de saisie des \nconsommations : Lorsque cette touche est maintenue enfoncée,\nune consommation de cette unité est automatiquement créée.")))
    ...
    def OnBoutonOk(self, event): 
        ...
        if self.IDunite != None :
            ...
            if len(listeDatesConso) > 0 and self.typeUnite == "Multihoraires" and self.ctrl_type.GetType() != "Multihoraires" :
                periode = _("entre le %s et le %s") % (UTILS_Dates.DateDDEnFr(listeDatesConso[0]), UTILS_Dates.DateDDEnFr(listeDatesConso[-1]))
                dlg = wx.MessageDialog(self, _("Des consommations multiples ont déjà été saisies sur %d dates (%s) !\n\nIl est donc impossible de convertir cette unité multihoraire en un autre type d'unité.") % (len(listeDatesConso), periode), _("Erreur"), wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                return
            ...
            if (self.ctrl_type.GetType() == "Evenement" and self.typeUnite != "Evenement") or (self.ctrl_type.GetType() != "Evenement" and self.typeUnite == "Evenement") :
                if len(listeDatesConso) > 0 :
                    periode = _("entre le %s et le %s") % (UTILS_Dates.DateDDEnFr(listeDatesConso[0]), UTILS_Dates.DateDDEnFr(listeDatesConso[-1]))
                    dlg = wx.MessageDialog(self, _("Il est impossible de convertir le type de cette unité car des consommations ont déjà été saisies sur %d dates (%s)") % (len(listeDatesConso), periode), _("Erreur"), wx.OK | wx.ICON_ERROR)
                    dlg.ShowModal()
                    dlg.Destroy()
                    return

                if len(listeDatesEvenements) > 0 :
                    periode = _("entre le %s et le %s") % (UTILS_Dates.DateDDEnFr(listeDatesEvenements[0]), UTILS_Dates.DateDDEnFr(listeDatesEvenements[-1]))
                    dlg = wx.MessageDialog(self, _("Il est impossible de convertir le type de cette unité car des évènements ont déjà été saisis sur %d dates (%s)") % (len(listeDatesEvenements), periode), _("Erreur"), wx.OK | wx.ICON_ERROR)
                    dlg.ShowModal()
                    dlg.Destroy()
                    return
    ...
    def Sauvegarde(self):
        ...
        if abrege == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un nom abrégé !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if heure_debut != None and self.ctrl_heure_debut.Validation() == False :
            dlg = wx.MessageDialog(self, _("L'heure de début n'est pas valide !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if heure_fin != None and self.ctrl_heure_fin.Validation() == False :
            dlg = wx.MessageDialog(self, _("L'heure de fin n'est pas valide !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if type == "Multihoraires" and (heure_debut == None or heure_fin == False) :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement spécifier une heure de début et une heure de fin pour les unités de type 'Multihoraire' !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if self.radio_illimitee.GetValue() == True :
            ...
            if date_debut == None :
                dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de début de validité !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
            date_fin = self.ctrl_date_fin.GetDate()
            if date_fin == None :
                dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de fin de validité !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
