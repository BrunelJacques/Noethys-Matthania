...
class Dialog(wx.Dialog):
    def __init__(self, parent, track=None, activeMontant=True):
        ...
        self.SetTitle(_("Saisie d'un prélèvement"))
        
        # Famille
        self.box_famille_staticbox = wx.StaticBox(self, -1, _("Destinataire"))
        ...
        self.label_etab = wx.StaticText(self, -1, _("Etab."))
        self.label_guichet = wx.StaticText(self, -1, _("Guichet"))
        self.label_numero = wx.StaticText(self, -1, _("Numéro"))
        self.label_cle = wx.StaticText(self, -1, _("Clé"))
        self.ctrl_code_etab = wx.TextCtrl(self, -1, "", style=wx.TE_CENTRE, validator = MyValidator())
        self.ctrl_code_guichet = wx.TextCtrl(self, -1, "", style=wx.TE_CENTRE, validator = MyValidator())
        self.ctrl_numero = wx.TextCtrl(self, -1, "", style=wx.TE_CENTRE, validator = MyValidator())
        self.ctrl_cle = wx.TextCtrl(self, -1, "", style=wx.TE_CENTRE, validator = MyValidator())
        ...
        self.label_titulaire = wx.StaticText(self, -1, _("Titulaire du compte"))
        self.ctrl_titulaire = wx.TextCtrl(self, -1, "")

        # Prélèvement
        self.box_prelevement_staticbox = wx.StaticBox(self, -1, _("Prélèvement"))

        self.label_type = wx.StaticText(self, -1, _("Type :"))
        self.ctrl_type = wx.StaticText(self, -1, _("Saisie manuelle"))

        self.label_libelle = wx.StaticText(self, -1, _("Libellé :"))
        ...
        self.label_etat = wx.StaticText(self, -1, _("Statut :"))
        self.radio_etat_attente = wx.RadioButton(self, -1, _("Attente"), style=wx.RB_GROUP)
        self.radio_etat_valide = wx.RadioButton(self, -1, _("Valide"))
        self.radio_etat_refus= wx.RadioButton(self, -1, _("Refus"))
        
        self.radio_etat_attente.Enable(False) 
        self.radio_etat_valide.Enable(False) 
        self.radio_etat_refus.Enable(False) 

        self.label_reglement = wx.StaticText(self, -1, _("Règlement :"))
        self.ctrl_reglement = wx.StaticText(self, -1, _("Non"))
    ...
    def __set_properties(self):
        ...
        self.ctrl_famille.SetToolTip(wx.ToolTip(_("Sélectionnez ici la famille à débiter")))
        self.ctrl_code_etab.SetToolTip(wx.ToolTip(_("Saisissez ici le code Etablissement")))
        self.ctrl_code_guichet.SetToolTip(wx.ToolTip(_("Saisissez ici le code Guichet")))
        self.ctrl_numero.SetToolTip(wx.ToolTip(_("Saisissez ici le numéro de compte")))
        self.ctrl_cle.SetToolTip(wx.ToolTip(_("Saisissez ici la clé du RIB")))
    ...
    def OnChoixFamille(self, event):
        ...
        if IDindividu != None :
            titulaireCompte = "%s %s" % (nomMembre, prenomMembre)
        else :
    ...
    def ControleRIB(self):
        ...
        bic = "%s%s%s%s" % (etab, guichet, numero, cle)
    ...
    def Importation(self):
        ...
        if self.track.type == "manuel" : labelType = _("Saisie manuelle")
        elif self.track.type == "facture" : labelType = _("Facture")
        else : labelType = ""
        ...
        if self.track.reglement == True :
            self.ctrl_reglement.SetLabel(_("Oui"))
        else :
            self.ctrl_reglement.SetLabel(_("Non"))
    ...
    def GetTrack(self):
        ...
        if IDfamille == None :
            dlg = wx.MessageDialog(self, _("Vous avez oublié de sélectionner une famille dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.ControleRIB() == False :
            dlg = wx.MessageDialog(self, _("Il est impossible d'activer le prélèvement :\nLes coordonnées bancaires ne sont pas valides !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if IDbanque == None :
            dlg = wx.MessageDialog(self, _("Il est impossible d'activer le prélèvement :\nVous n'avez sélectionné aucun établissement bancaire !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if titulaire == "" :
            dlg = wx.MessageDialog(self, _("Vous avez oublié de saisir un nom de titulaire pour le compte bancaire !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if libelle == "" :
            dlg = wx.MessageDialog(self, _("Vous avez oublié de saisir un libellé pour cette opération !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if montant == None or montant == 0.0 :
            dlg = wx.MessageDialog(self, _("Vous avez oublié de saisir un montant pour cette opération !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.ctrl_type.GetLabel() == _("Saisie manuelle") : track.type = "manuel"
        if self.ctrl_type.GetLabel() == _("Facture") : track.type = "facture"
