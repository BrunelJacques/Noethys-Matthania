...
def SelectionFichier():
    ...
    dlg = wx.FileDialog(None, message=_("Veuillez sélectionner le fichier de sauvegarde à restaurer"), defaultDir=rep, defaultFile="", wildcard=wildcard, style=wx.FD_OPEN)
    ...
    if fichier.endswith(".noc") == True :
        dlg = wx.PasswordEntryDialog(None, _("Veuillez saisir le mot de passe :"), _("Ouverture d'une sauvegarde cryptée"))
        if dlg.ShowModal() == wx.ID_OK:
...
class Dialog(wx.Dialog):
    def __init__(self, parent, fichier=""):
        ...
        # Données
        self.box_donnees_staticbox = wx.StaticBox(self, -1, _("Données à restaurer"))
