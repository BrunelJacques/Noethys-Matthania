...
from Ol import OL_Restaurateurs



class Dialog(wx.Dialog):
    def __init__(self, parent, mode="gestion"):
        wx.Dialog.__init__(self, parent, -1, name="DLG_restaurateur", style=wx.DEFAULT_DIALOG_STYLE|wx.RESIZE_BORDER|wx.MAXIMIZE_BOX|wx.MINIMIZE_BOX)
        self.parent = parent
        self.mode = mode
        
        if self.mode == "selection" :
            intro = _("Vous pouvez ici sélectionner un restaurateur. Double-cliquez sur une ligne pour effectuer rapidement la sélection.")
            titre = _("Sélection d'un restaurateur")
            self.SetTitle(_("Sélection d'un restaurateur"))
        else:
            intro = _("Vous pouvez ici saisir, modifier ou supprimer des restaurateurs. Ceux-ci pourront ensuite être associés à des modèles de commande de repas.")
            titre = _("Gestion des restaurateurs")
            self.SetTitle(_("Gestion des restaurateurs"))
        ...
        self.ctrl_recherche = OL_Restaurateurs.CTRL_Outils(self, listview=self.ctrl_listview)
    ...
    def __set_properties(self):
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter un restaurateur")))
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier le restaurateur sélectionné dans la liste")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer le restaurateur sélectionné dans la iste")))
    ...
    def GetIDrestaurateur(self):
        selection = self.ctrl_listview.Selection()
        if len(selection) == 0 :
            return None
        else:
            return selection[0].IDrestaurateur
