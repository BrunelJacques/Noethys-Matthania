...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        # Bandeau
        titre = _("Enregistrement")
        intro = _("Vous pouvez saisir ici votre code d'enregistrement personnel afin d'activer votre abonnement Classic ou Premium. Effectuez ce paramétrage sur chacun de vos postes.")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Cle.png")
        
        # Saisie code
        self.staticbox_code_staticbox = wx.StaticBox(self, -1, _("Code d'enregistrement"))
        ...
        self.bouton_validite = wx.Button(self, -1, _("Vérifier la validité"))
        self.ctrl_image = wx.StaticBitmap(self, -1, wx.Bitmap(Chemins.GetStaticPath("Images/16x16/absenti.png"), wx.BITMAP_TYPE_ANY))
        self.label_validite = wx.StaticText(self, -1, _("Veuillez saisir un code."))
    ...
    def __set_properties(self):
        self.ctrl_identifiant.SetToolTip(wx.ToolTip(_("Saisissez votre identifiant")))
        self.ctrl_code.SetToolTip(wx.ToolTip(_("Saisissez votre code d'enregistrement")))
        self.bouton_validite.SetToolTip(wx.ToolTip(_("Cliquez ici pour vérifier la validité de votre code d'enregistrement")))
    ...
    def OnBoutonValidite(self, event): 
        ...
        if len(identifiant) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez saisi aucun identifiant !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_identifiant.SetFocus()
            return
        if " " in identifiant :
            dlg = wx.MessageDialog(self, _("L'identifiant saisi n'est pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_identifiant.SetFocus()
            return
        ...
        if " " in code :
            dlg = wx.MessageDialog(self, _("Le code saisi n'est pas complet !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
    ...
    def VerifieEtat(self, identifiant="", code=""):
        ...
        dlgAttente = wx.BusyInfo(_("Vérification du code en cours..."), None)
        ...
        # Affiche l'état
        if date == None :
            texte = _("Vous n'avez saisi aucun code valide")
            image = "pasok"
        else :
            nbreJoursRestants =  (date - datetime.date.today()).days
            if nbreJoursRestants < 0 :
                texte = _("Votre licence est périmée depuis le %s") % DateEngFr(str(date))
                image = "pasok"
            elif nbreJoursRestants <= 30 :
                texte = _("Votre licence est valide jusqu'au %s (%d jours restants)") % (DateEngFr(str(date)), nbreJoursRestants)
                image = "attention"
            else :
                texte = _("Votre licence est valide jusqu'au %s") % DateEngFr(str(date))
                image = "ok"
    ...
    def OnBoutonFermer(self, event): 
        ...
        if identifiant != None and code != None and self.derniereValidation != (identifiant, code) and "impossible" not in self.label_validite.GetLabel() :
            dlg = wx.MessageDialog(self, _("Vous avez modifié votre saisie depuis la dernière vérification.\n\nVeuillez vérifier maintenant la validité du code en cliquant sur le bouton 'Vérifier la validité' !"), _("Validation"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
