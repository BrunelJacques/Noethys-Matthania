...
class Dialog(wx.Dialog):
    def __init__(self, parent, mode_debug=False):
        ...
        # Bandeau
        intro = _("Vous pouvez ici effectuer des procédures de badgeage. Commencez par sélectionner la procédure souhaitée dans la liste pour cliquez sur le bouton 'Ok' pour lancer la procédure. Vous pourrez interrompre celle-ci en appuyant sur CTRL+SHIFT+Q.")
        titre = _("Badgeage")
        self.SetTitle(titre)
        ...
        self.label_procedure = wx.StaticText(self, -1, _("Procédure :"))
        ...
        self.check_dateauto = wx.CheckBox(self, -1, _("Date auto."))
        ...
        self.hyper_consommations = Hyperlien(self, label=_("Consulter la liste des consommations badgées"), infobulle=_("Cliquez ici pour consulter la liste des consommations badgées"), URL="consommations")
        self.label_separation1 = wx.StaticText(self, -1, "|")
        self.hyper_journal = Hyperlien(self, label=_("Consulter l'historique du journal"), infobulle=_("Cliquez ici pour consulter l'historique du journal"), URL="historique")
        self.label_separation2 = wx.StaticText(self, -1, "|")
        self.hyper_purger = Hyperlien(self, label=_("Purger l'historique"), infobulle=_("Cliquez ici pour purger l'historique du journal"), URL="purger")

        # Boutons
        self.bouton_aide = CTRL_Bouton_image.CTRL(self, texte=_("Aide"), cheminImage="Images/32x32/Aide.png")
        self.bouton_vocal = CTRL_Bouton_image.CTRL(self, texte=_("Synthèse vocale"), cheminImage="Images/32x32/Vocal.png")
        self.bouton_importation = CTRL_Bouton_image.CTRL(self, texte=_("Importer"), cheminImage="Images/32x32/Fleche_bas.png")
        self.bouton_ok = CTRL_Bouton_image.CTRL(self, texte=_("Commencer le badgeage"), cheminImage="Images/32x32/Badgeage.png")
...
##        wx.CallAfter(self.AfficheAvertissement)
    ...
    def __set_properties(self):
        self.ctrl_procedure.SetToolTip(wx.ToolTip(_("Sélectionnez une procédure")))
        self.bouton_procedures.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à la gestion des procédures")))
        self.ctrl_date.SetToolTip(wx.ToolTip(_("Sélectionnez une date (Par défaut la date du jour)")))
        self.check_dateauto.SetToolTip(wx.ToolTip(_("Cochez cette case pour que Noethys sélectionne automatiquement la date du jour (A minuit, la date changera automatiquement)")))
        self.bouton_log_apercu.SetToolTip(wx.ToolTip(_("Cliquez ici pour afficher un aperçu avant impression")))
        self.bouton_log_imprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour imprimer")))
        self.bouton_log_excel.SetToolTip(wx.ToolTip(_("Cliquez ici pour exporter la liste au format Excel")))
        self.bouton_log_texte.SetToolTip(wx.ToolTip(_("Cliquez ici pour exporter la liste au format Texte")))
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez ici pour obtenir de l'aide")))
        self.bouton_vocal.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder au paramétrage de la synthèse vocale")))
        self.bouton_importation.SetToolTip(wx.ToolTip(_("Cliquez ici pour importer des badgeages manuellement")))
        self.bouton_ok.SetToolTip(wx.ToolTip(_("Cliquez ici pour lancer la procédure de badgeage en temps réél")))
    ...
    def LancementProcedure(self, importationManuelle=False) :
        ...
        IDprocedure = self.ctrl_procedure.GetID() 
        if IDprocedure == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner une procédure !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if date != datetime.date.today() :
            dlg = wx.MessageDialog(self, _("Vous avez sélectionné une date différente d'aujourd'hui. Confirmez-vous ce choix ?"), _("Date"), wx.YES_NO|wx.YES_DEFAULT|wx.ICON_EXCLAMATION)
        ...
        try :
            ...
            if vocal.VerifieSiVirginieInstallee() == False :
                if UTILS_Parametres.Parametres(mode="get", categorie="ne_plus_afficher", nom="voix_virginie", valeur=False) == False:
                    introduction = _("Souhaitez-vous installer la voix française Virginie pour la synthèse vocale ?")
                    dlg = DLG_Messagebox.Dialog(self, titre=_("Avertissement"), introduction=introduction, icone=wx.ICON_EXCLAMATION, boutons=[_("Oui"), _("Pas maintenant"), _("Ne plus demander")], defaut=0)
                    reponse = dlg.ShowModal()
                    dlg.Destroy()
                    if reponse == 0:
                ...
                if vocal.VerifieSiVirginieDefaut() == False :
                    dlg = wx.MessageDialog(self, _("La voix française Virginie est bien installée sur votre ordinateur mais n'a pas été sélectionnée comme voix par défaut.\n\nSouhaitez-vous le faire maintenant (conseillé) ?"), _("Synthèse vocale"), wx.YES_NO|wx.YES_DEFAULT|wx.ICON_EXCLAMATION)
                    reponse = dlg.ShowModal()
                    dlg.Destroy()
                    if reponse == wx.ID_YES:
                        self.OnBoutonVocal(None)
        ...
        self.log.AjouterAction(action=_("Lancement de la procédure '%s'") % nomProcedure)
        
        # Rappel de la combinaison de touches pour quitter la fenêtre
        if importationManuelle == False and modeDebug == False :
            dlg = wx.MessageDialog(self, _("Appuyez sur CTRL+SHIFT+Q pour\nquitter l'interface de badgeage."), _("Information"), wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        self.log.AjouterAction(action=_("Arrêt de la procédure '%s'") % nomProcedure)
    ...
    def OnBoutonImportation(self, event):
        ...
        if IDprocedure == None : 
            dlg = wx.MessageDialog(self, _("Vous devez d'abord sélectionner une procédure de badgeage !"), _("Erreur"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        for IDaction, ordre, action, action_demande, action_message, action_question in listeActions :
            ...
            if action_demande == "1" : 
                dlg = wx.MessageDialog(self, _("Une des actions de cette procédure contient une question de type 'Heure de début ou heure de fin ?' incompatible avec le processus d'importation."), _("Information"), wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                return False
    
        if nbreMessages > 0 : 
            dlg = wx.MessageDialog(self, _("Notez que les messages ou questions programmées dans les actions ne seront pas prises en compte (%d actions).") % nbreMessages, _("Remarque"), wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()

        # Demande de confirmation
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment importer les %d badgeages sélectionnés avec la procédure '%s' ?") % (len(listeDonnees), nomProcedure), _("Confirmation"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
