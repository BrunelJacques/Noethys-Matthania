...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        # Bandeau
        titre = _("Horaires du soleil")
        intro = _("Cette fonctionnalité permet d'afficher les heures de lever et de coucher du soleil d'une ville sur un mois donné. Cette fonction utilise la géolocalisation GPS de la ville et un agorithme intégré de calcul de la position du soleil.")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Soleil.png")
        
        # Paramètres
        self.box_ville_staticbox = wx.StaticBox(self, -1, _("Ville"))
        self.radio_ville = wx.RadioButton(self, -1, _("Selon une ville :"), style=wx.RB_GROUP)
        self.label_ville = wx.StaticText(self, -1, _("CP :"))
        self.ctrl_ville = CTRL_Saisie_adresse.Adresse(self)
        self.radio_gps = wx.RadioButton(self, -1, _("Selon une position GPS :"))
        self.label_lat = wx.StaticText(self, -1, _("Lat. :"))
        self.ctrl_lat = wx.TextCtrl(self, -1, "")
        self.label_long = wx.StaticText(self, -1, _("Long. :"))
        self.ctrl_long = wx.TextCtrl(self, -1, "")
        
        self.box_mois_staticbox = wx.StaticBox(self, -1, _("Mois"))
        ...
        self.ctrl_annee = wx.SpinCtrl(self, -1, "", min=1970, max=2999)
        ...
        # Résultats
        self.ctrl_resultats = wx.TextCtrl(self, -1, "", style=wx.TE_MULTILINE)
    ...
    def __set_properties(self):
        ...
        self.bouton_actualiser.SetToolTip(wx.ToolTip(_("Cliquez ici pour actualiser la liste en fonction des paramètres")))
    ...
    def GetGPS(self):
        ...
        if self.radio_ville.GetValue() == True :
            ...
            if cp == "" or cp == None or ville == "" or ville == None :
                dlg = wx.MessageDialog(self, _("Vous n'avez pas renseigné correctement la ville !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
            # recherche GPS
            ...
            if dictGPS == None : 
                dlg = wx.MessageDialog(self, _("Les coordonnées GPS n'ont pas été détectées !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
            else :
        ...
        # COORDS
            ...
            if lat == "" or lat == None or long == "" or long == None :
                dlg = wx.MessageDialog(self, _("Vous n'avez pas renseigné correctement les coordonnées GPS !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
            else :
