...
from Ol import OL_Medecins



class Dialog(wx.Dialog):
    def __init__(self, parent, mode="gestion"):
        wx.Dialog.__init__(self, parent, -1, name="DLG_medecin", style=wx.DEFAULT_DIALOG_STYLE|wx.RESIZE_BORDER|wx.MAXIMIZE_BOX|wx.MINIMIZE_BOX)
        self.parent = parent
        self.mode = mode
        
        if self.mode == "selection" :
            intro = _("Vous pouvez ici sélectionner un médecin traitant pour l'individu. Double-cliquez sur une ligne pour effectuer rapidement la sélection.")
            titre = _("Sélection d'un médecin")
            self.SetTitle(_("Sélection d'un médecin"))
        else:
            intro = _("Vous pouvez ici saisir, modifier ou supprimer des médecins.")
            titre = _("Gestion des médecins")
            self.SetTitle(_("Gestion des médecins"))
        ...
        self.ctrl_recherche = OL_Medecins.CTRL_Outils(self, listview=self.ctrl_listview)
    ...
    def __set_properties(self):
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter un médecin")))
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier le médecin sélectionné dans la liste")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer le médecin sélectionné dans la iste")))
    ...
    def GetIDmedecin(self):
        ...
        if len(selection) == 0 :
            ...
            return selection[0].IDmedecin
        
    def OnBouton_ok(self, event):
        IDmedecin = self.GetIDmedecin()
        if IDmedecin == None :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun médecin dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
...
if __name__ == "__main__":
    ...
    dialog_1 = Dialog(None, mode="selection")
