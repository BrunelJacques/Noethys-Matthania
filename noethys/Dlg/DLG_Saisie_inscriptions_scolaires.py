...
class CTRL_Ecole(wx.Choice):
    ...
    def GetListeDonnees(self):
        ...
        if len(listeDonnees) > 0 :
            listeItems = [_("-- Toutes les écoles --")]
...
class CTRL_Classe(wx.Choice):
    ...
    def GetListeDonnees(self):
        ...
        for IDclasse, nom, date_debut, date_fin, niveaux in listeDonnees :
            ...
            # Formatage du nom
            nom = _("%s   (Du %s au %s)") % (nom, DateEngFr(str(date_debut)), DateEngFr(str(date_fin)))
        ...
        if len(listeClasses) > 0 :
            listeItems = [_("-- Toutes les classes --")]
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDecole=None, IDclasse=None):
        ...
        self.box_param_staticbox = wx.StaticBox(self, -1, _("Paramètres de l'inscription"))
        ...
        self.label_niveau = wx.StaticText(self, -1, _("Niveau :"))
        self.ctrl_niveau = CTRL_Niveau(self)

        # Individus
        self.box_individus_staticbox = wx.StaticBox(self, -1, _("Sélection des individus"))
        
        self.label_filtre_periode = wx.StaticText(self, -1, _("Période :"))
        self.label_filtre_du = wx.StaticText(self, -1, "du")
        self.ctrl_filtre_date_debut = CTRL_Saisie_date.Date2(self)
        self.label_filtre_au = wx.StaticText(self, -1, _("au"))
        self.ctrl_filtre_date_fin = CTRL_Saisie_date.Date2(self)
        
        self.label_filtre_ecole = wx.StaticText(self, -1, _("Ecole :"))
        self.ctrl_filtre_ecole = CTRL_Ecole(self)
        self.label_filtre_classe = wx.StaticText(self, -1, _("Classe :"))
        self.ctrl_filtre_classe = CTRL_Classe(self)
        self.label_filtre_niveau = wx.StaticText(self, -1, _("Niveau :"))
    ...
    def __set_properties(self):
        self.SetTitle(_("Inscrire des individus"))
        self.ctrl_niveau.SetToolTip(wx.ToolTip(_("Sélectionnez ici le niveau scolaire")))
        self.ctrl_date_debut.SetToolTip(wx.ToolTip(_("Saisissez ici la date de début de scolarisation")))
        self.ctrl_date_fin.SetToolTip(wx.ToolTip(_("Saisissez ici la date de fin de scolarisation")))
        self.ctrl_filtre_date_debut.SetToolTip(wx.ToolTip(_("Saisissez ici un filtre de période")))
        self.ctrl_filtre_date_fin.SetToolTip(wx.ToolTip(_("Saisissez ici un filtre de période")))
        self.ctrl_filtre_ecole.SetToolTip(wx.ToolTip(_("Vous pouvez sélectionner ici un filtre sur les écoles")))
        self.ctrl_filtre_classe.SetToolTip(wx.ToolTip(_("Vous pouvez sélectionner ici un filtre sur les classes")))
        self.ctrl_filtre_niveau.SetToolTip(wx.ToolTip(_("Vous pouvez sélectionner ici un filtre sur les niveaux scolaires")))
        self.ctrl_individus.SetToolTip(wx.ToolTip(_("Cochez les individus à inscrire")))
    ...
    def SetNomClasse(self, nom=""):
        self.SetTitle(_("Inscription d'individus dans la classe de %s") % nom)
    ...
    def OnBoutonOk(self, event): 
        ...
        if IDniveau == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner un niveau scolaire !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if len(listeIndividus) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun individu !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        
        # Demande de confirmation d'enregistrement
        dlg = wx.MessageDialog(self, _("Confirmez-vous l'inscription de %d individus ?") % len(listeIndividus), _("Confirmation"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        ...
        # Fenêtre d'attente
        dlgAttente = wx.BusyInfo(_("Veuillez patienter durant la procédure d'inscription..."), None)
        ...
        nomClasse = _("%s (Du %s au %s)") % (nomClasse, DateEngFr(date_debut_classe), DateEngFr(date_fin_classe))
        ...
        for IDindividu in listeIndividus :
            ...
            for track in donneesScolarite :
                if date_debut <= track.dateFinDD and date_fin >= track.dateDebutDD :
                    ...
                    nomIndividu_temp = "%s %s" % (track.prenom, track.nom)
                    listeProblemes.append(_("- %s (déjà dans la classe de %s du %s au %s)\n") % (nomIndividu_temp, nomClasse_temp, date_debut_temp, date_fin_temp))
                    valide = False
                    break
            ...
            if valide == True :
                ...
                UTILS_Historique.InsertActions([{
                    ...
                    "IDfamille" : None,
                    "IDcategorie" : 30, 
                    "action" : _("Inscription scolaire du %s au %s. Ecole : '%s'. Classe : '%s'. Niveau : '%s'") % (DateEngFr(str(date_debut)), DateEngFr(str(date_fin)), nomEcole, nomClasse, nomNiveau)
                    },])
        ...
        if len(listeProblemes) > 0 :
            message = _("%d inscriptions ont été enregistrées sauf pour les individus suivants :\n\n%s") % (nbreValides, "".join(listeProblemes))
        else:
            message = _("%d inscriptions ont été enregistrées avec succès.") % nbreValides
        dlg = wx.MessageDialog(self, message, _("Information"), wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
