...
class Panel(wx.Panel):
    ...
    def __set_properties(self):
        self.ctrl_pieces.SetToolTip(wx.ToolTip(_("Cochez les pièces que l'individu inscrit à cette activité doit fournir")))
        self.ctrl_vaccins.SetToolTip(wx.ToolTip(_("Cochez cette case si l'individu inscrit à cette activité doit obligatoirement justifier de ses vaccins à jour")))
        self.ctrl_infos.SetToolTip(wx.ToolTip(_("Cochez les informations que l'individu inscrit à cette activité doit obligatoirement renseigner dans son dossier")))
        self.ctrl_cotisations.SetToolTip(wx.ToolTip(_("Cochez les cotisations que l'individu inscrit à cette activité doit avoir à jour (au moins l'une d'elle)")))
    ...
    def Validation(self):
        ...
        if self.ctrl_check_cotisations.GetValue() == True :
            if len(self.ctrl_cotisations.GetIDcoches()) == 0 :
                dlg = wx.MessageDialog(self, _("Vous avez coché 'Cotisation obligatoire' mais sans cocher de cotisation dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
