...
class CTRL_Adresse_auto(wx.Choice):
    def __init__(self, parent, IDindividu=None, size=(-1, -1)):
        ...
        self.SetToolTip(wx.ToolTip(_("Sélectionnez une adresse de rattachement ou saisissez une adresse")))
    ...
    def GetListeDonnees(self):
        ...
        listeNoms = [_("L'adresse suivante")]
        index = 1
        for IDindividu, nom, prenom, rue, cp, ville, tel_domicile in listeDonnees:
            if IDindividu != self.parent.IDindividu:
                if rue != "" and cp != "" and ville != "":
                    nomComplet = "L'adresse de %s %s" % (nom, prenom)
...
class Adresse(wx.Panel):
    def __init__(self, parent, IDindividu=None, size=(-1, -1)):
        ...
        self.label_cp = wx.StaticText(self, -1, _("CP :"))
...
class CTRL_Renseignements(ULC.UltimateListCtrl):
    ...
    def Remplissage(self):
        ...
        self.InsertColumn(0, _("Champ"), width=180)
        self.InsertColumn(1, _("Valeur"), width=largeurColonneDonnee+5, format=ULC.ULC_FORMAT_LEFT)

        # Création des lignes
        self.liste_lignes = [
            {"code": "nom", "label": _("Nom de famille"), "ctrl": wx.TextCtrl(self, -1, "", size=(largeurColonneDonnee, -1))},
            {"code": "prenom", "label": _("Prénom"), "ctrl": wx.TextCtrl(self, -1, "", size=(largeurColonneDonnee, -1))},

            {"code": "date_naiss", "label": _("Date de naissance"), "ctrl": CTRL_Saisie_date.Date2(self, size=(largeurColonneDonnee, -1))},
            {"code": "ville_naiss", "label": _("Ville de naissance"), "ctrl": CTRL_Saisie_adresse.Adresse(self, size=(largeurColonneDonnee, -1))},

            {"code": "adresse", "label": _("Adresse"), "ctrl": Adresse(self, IDindividu=self.IDindividu, size=(largeurColonneDonnee, 100))},

            {"code": "tel_domicile", "label": _("Tél. Domicile"), "ctrl": CTRL_Saisie_tel.Tel(self, intitule=_("Domicile"), size=(largeurColonneDonnee, -1))},
            {"code": "tel_mobile", "label": _("Tél. Mobile"), "ctrl": CTRL_Saisie_tel.Tel(self, intitule=_("Mobile"), size=(largeurColonneDonnee, -1))},
            {"code": "mail", "label": _("Email"), "ctrl": CTRL_Saisie_mail.Mail(self, size=(largeurColonneDonnee, -1))},

            {"code": "profession", "label": _("Profession"), "ctrl": wx.TextCtrl(self, -1, "", size=(largeurColonneDonnee, -1))},
            {"code": "employeur", "label": _("Employeur"), "ctrl": wx.TextCtrl(self, -1, "", size=(largeurColonneDonnee, -1))},
            {"code": "travail_tel", "label": _("Tél. Pro."), "ctrl": CTRL_Saisie_tel.Tel(self, intitule=_("Tél.Pro."), size=(largeurColonneDonnee, -1))},
            {"code": "travail_mail", "label": _("Email Pro."), "ctrl": CTRL_Saisie_mail.Mail(self, size=(largeurColonneDonnee, -1))},
            ]
...
class Dialog(wx.Dialog):
    def __init__(self, parent, track=None):
        ...
        # Bandeau
        intro = _("Vous pouvez gérer ici la demande de façon manuelle. Commencez par cliquer sur le bouton 'Appliquer la demande' pour voir apparaître les modifications demandées sur le portail. Vous pouvez alors effectuer manuellement d'éventuelles modifications avant de valider.")
        titre = _("Traitement manuel des renseignements")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Cotisation.png")

        # Détail demande
        self.box_demande_staticbox = wx.StaticBox(self, wx.ID_ANY, _("Détail de la demande"))
        self.ctrl_demande = CTRL_Html(self, couleurFond=self.GetBackgroundColour())
        self.ctrl_demande.SetMinSize((275, 100))

        # Renseignements
        self.box_renseignements_staticbox = wx.StaticBox(self, wx.ID_ANY, _("Renseignements"))
        self.ctrl_renseignements = CTRL_Renseignements(self)

        # Journal
        self.box_journal_staticbox = wx.StaticBox(self, wx.ID_ANY, _("Journal d'évènements"))
        self.ctrl_log = CTRL_Log(self)
        self.ctrl_log.SetMinSize((100, 80))
        self.bouton_traiter = CTRL_Bouton_image.CTRL(self, texte=_("Appliquer la demande"), cheminImage="Images/32x32/Fleche_bas.png")
        self.bouton_reinit = CTRL_Bouton_image.CTRL(self, texte=_("Annuler les modifications"), cheminImage="Images/32x32/Actualiser.png")

        # Boutons
        self.bouton_aide = CTRL_Bouton_image.CTRL(self, texte=_("Aide"), cheminImage="Images/32x32/Aide.png")
        self.bouton_ok = CTRL_Bouton_image.CTRL(self, texte=_("Ok"), cheminImage="Images/32x32/Valider.png")
        self.bouton_fermer = CTRL_Bouton_image.CTRL(self, texte=_("Annuler"), cheminImage="Images/32x32/Annuler.png")
    ...
    def __set_properties(self):
        self.bouton_traiter.SetToolTip(wx.ToolTip(_("Cliquez ici pour appliquer la demande")))
        self.bouton_reinit.SetToolTip(wx.ToolTip(_("Cliquez ici pour annuler les modifications")))
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez ici pour obtenir de l'aide")))
        self.bouton_ok.SetToolTip(wx.ToolTip(_("Cliquez ici pour commencer le traitement des demandes")))
    ...
    def OnBoutonTraiter(self, event=None):
        self.parent.EcritLog(_("Application de la demande de modification de renseignements"), self.ctrl_log)
        ...
        for champ, valeur in listeRenseignements:
            ...
            try :
                ...
                self.parent.EcritLog(_("Le champ '%s' n'a pas pu être importé. Erreur : %s") % (DICT_RENSEIGNEMENTS[champ], err), self.ctrl_log)
                listeAnomalies.append(champ)

        # Création du texte de réponse
        if len(liste_champs_importes) == 0 :
            reponse = _("Aucun renseignement modifié")
        elif len(liste_champs_importes) == 1 :
            reponse = _("1 renseignement modifié")
        else :
            reponse = _("%d renseignements modifiés") % len(liste_champs_importes)

        if len(liste_champs_importes) > 0 :
            reponse += " (%s)" % ", ".join([DICT_RENSEIGNEMENTS[champ] for champ in liste_champs_importes])

        self.parent.EcritLog(_("Réponse : %s") % reponse, self.ctrl_log)
        ...
        for champ, ctrl in self.ctrl_renseignements.GetCoches() :
            ...
            if hasattr(ctrl, "Validation") :
                if ctrl.Validation() == False :
                    dlg = wx.MessageDialog(self, _("La valeur du champ '%s' semble incorrecte !") % DICT_RENSEIGNEMENTS[champ], _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                    dlg.ShowModal()
                    dlg.Destroy()
                    return False
