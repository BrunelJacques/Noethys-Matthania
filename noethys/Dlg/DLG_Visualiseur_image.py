...
phrase1 = _("Ramener l'image à sa taille d'origine")
phrase2 = _("Opération interdite")
...
class MyFrame(wx.Frame):
    def __init__(self, parent, imgPIL=None, imgWX=None):
        ...
        self.SetIcon(_icon)
        self.SetTitle(_("Aperçu d'image"))
...
##        self.SetMenuBar(menuBarre)
    ...
    def OnOpen(self, evt):
        if self.imgORIG != None :
            dlg = wx.MessageDialog(self, _("Vous devez d'abord fermer l'image en cours d'utilisation"),
                       phrase2, style = wx.OK)
            retour = dlg.ShowModal()
            dlg.Destroy()
        else:
            dlg = wx.FileDialog(self, _("Choisissez un fichier"),
                    wildcard = "*.*",
                    style = wx.FD_OPEN)
            retour = dlg.ShowModal()
            chemin = dlg.GetPath()
            ...
            if retour == wx.ID_OK and fichier != "":
                ...
                self.bmpRESU = self.imgORIG.ConvertToBitmap()
                self.panneau.Affiche(self.bmpRESU, self.ratio)
                self.SetTitle(_("Aperçu d'image [%s]")% fichier)
                self.barre.SetStatusText("(%s, %s) %s %%"%(self.imgORIX, self.imgORIY, self.ratio), 1)
    ...
    def OnClose(self, evt):
        if self.imgORIG != None :
            ...
            self.bmpRESU = None
            self.ratio = 100
            self.SetTitle(_("Aperçu d'images"))
