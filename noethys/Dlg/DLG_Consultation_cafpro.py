...
class CTRL_Drag(wx.TextCtrl):
    def __init__(self, parent, texte=""):
        ...
        # Propriétés
        self.SetToolTip(wx.ToolTip(_("Faites un glisser-déposer pour envoyer cette information dans le formulaire de votre navigateur internet")))
...
class MyTextDropTarget(wx.TextDropTarget):
    ...
    def OnDropText(self, x, y, text):
        try :
            ...
            dlg = wx.MessageDialog(self.window, _("La donnée que vous avez déposé ne semble pas valide. Il ne peut s'agit que d'un nombre entier !"), _("Erreur de saisie"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return
...
class CTRL_Drop_archive(wx.TextCtrl):
    def __init__(self, parent, texte=""):
        ...
        # Propriétés
        self.SetToolTip(wx.ToolTip(_("Sélectionnez le quotient dans votre navigateur internet et glissez-le jusqu'ici")))
...
class CTRL_Drop(wx.StaticBitmap):
    ...
    def OnDLeftDown(self, event):
        dlg = wx.TextEntryDialog(self, _("Veuillez modifier la valeur :"), _("Modification"))
        if self.texte != None :
        ...
        if texte == "" :
            ...
            try :
                ...
                dlg = wx.MessageDialog(self, _("La donnée que vous avez saisi ne semble pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                return
        self.SetTexte(texte)
...
class Frame(wx.Frame):
    def __init__(self, parent, IDfamille=None, dictDonnees={}):
        ...
        # Bandeau
        titre = _("Consultation CAF-CDAP")
        intro = _("Glissez-déposez le numéro d'allocataire et le nom vers le formulaire de CDAP. Puis sélectionnez et glissez-déposez le QF ou le revenu ci-dessous.")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self.panel, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Caf.png")

        # Informations
        self.box_informations = wx.StaticBox(self.panel, -1, _("Informations"))
        self.label_nom = wx.StaticText(self.panel, -1, _("Nom de famille :"))
        self.ctrl_nom = CTRL_Drag(self.panel, texte="")
        self.label_numero = wx.StaticText(self.panel, -1, _("Numéro d'allocataire :"))
        self.ctrl_numero = CTRL_Drag(self.panel, texte="")

        # Informations
        self.box_quotient = wx.StaticBox(self.panel, -1, _("Quotient familial/Revenu"))
        self.ctrl_quotient = CTRL_Drop(self.panel, taille=(180, 80), texte_defaut=_("Sélectionnez et glissez-déposez\nle quotient familial ici"), tip=_("Double-cliquez sur le quotient dans votre navigateur internet pour le sélectionner et glissez-le jusqu'ici.\n\nVous pouvez également faire un double-clic pour saisir ou modifier la valeur manuellement."))
        self.ctrl_revenu = CTRL_Drop(self.panel, taille=(180, 80), texte_defaut=_("Sélectionnez et glissez-déposez\nle revenu annuel ici"), tip=_("Double-cliquez sur le revenu dans votre navigateur internet pour le sélectionner et glissez-le jusqu'ici.\n\nVous pouvez également faire un double-clic pour saisir ou modifier la valeur manuellement."))
        ...
        # Boutons
        self.bouton_aide = CTRL_Bouton_image.CTRL(self.panel, texte=_("Aide"), cheminImage="Images/32x32/Aide.png")
        self.bouton_fermer = CTRL_Bouton_image.CTRL(self.panel, texte=_("Fermer"), cheminImage="Images/32x32/Fermer.png")
        self.bouton_ok = CTRL_Bouton_image.CTRL(self.panel, texte=_("Enregistrer"), cheminImage="Images/32x32/Sauvegarder.png")
    ...
    def __set_properties(self):
        ...
        self.bouton_ok.SetToolTip(wx.ToolTip(_("Cliquez ici pour enregistrer le nouveau quotient familial dans la fiche famille")))
    ...
    def GetTexteDernierQF(self):
        ...
        if len(listeDonnees) > 0 :
            ...
            if quotient != None and revenu == None :
                texte = _("Dernier quotient familial : %d du %s au %s (%s)") % (quotient, UTILS_Dates.DateDDEnFr(date_debut), UTILS_Dates.DateDDEnFr(date_fin), nom_quotient)
            elif quotient == None and revenu != None :
                texte = _("Dernier revenu : %d du %s au %s (%s)") % (revenu, UTILS_Dates.DateDDEnFr(date_debut), UTILS_Dates.DateDDEnFr(date_fin), nom_quotient)
            elif quotient != None and revenu != None :
                texte = _("Dernier quotient/revenu : %s/%s du %s au %s (%s)") % (quotient, revenu, UTILS_Dates.DateDDEnFr(date_debut), UTILS_Dates.DateDDEnFr(date_fin), nom_quotient)
        return texte
    ...
    def OnBoutonOk(self, event):
        ...
        dlg.SetTitle(_("Saisie d'un quotient familial/revenu"))
