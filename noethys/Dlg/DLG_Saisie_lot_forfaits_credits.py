    ...
    def run(self): 
        try: 
            ...
            for track in self.tracks :
                ...
                # Affichage
                texte = "[%d/%d] %s..." % (self.index+1, len(self.tracks), track.nomCompletIndividu)
            ...
            if self.succes == True :
                self.parent.EcritLog(_("Traitement terminé"))
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        # Bandeau
        titre = _("Saisir un lot de forfaits-crédits")
        intro = _("Vous pouvez ici saisir des forfaits-crédits individuels pour un ensemble d'individus. Sélectionnez une activité, le forfait-crédit à appliquer puis la période à impacter. Cochez les individus souhaités puis cliquez sur le bouton Commencer pour lancer le traitement.")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Euro.png")
        
        # Paramètres
        self.box_parametres_staticbox = wx.StaticBox(self, -1, _("Paramètres du forfait"))
        self.label_periode = wx.StaticText(self, -1, "Période :")
        ...
        self.label_forfait = wx.StaticText(self, -1, "Forfait :")
        ...
        # Grille des conso
        self.box_grille_staticbox = wx.StaticBox(self, -1, _("Journal"))
        ...
        self.bouton_ok = CTRL_Bouton_image.CTRL(self, texte=_("Commencer"), cheminImage="Images/32x32/Valider.png")
        self.bouton_fermer = CTRL_Bouton_image.CTRL(self, texte=_("Fermer"), cheminImage="Images/32x32/Annuler.png")
    ...
    def __set_properties(self):
        self.ctrl_activite.SetToolTip(wx.ToolTip(_("Sélectionnez l'activité pour laquelle vous souhaitez recalculer les prestations")))
        ...
        self.bouton_fermer.SetToolTip(wx.ToolTip(_("Cliquez ici pour annuler")))
    ...
    def OnBoutonActualiser(self, event): 
        ...
        date_debut = self.ctrl_date_debut.GetDate()
        if date_debut == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de début !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
    ...
    def Arreter(self, forcer=False):
        ...
        if TraitmentEnCours:
            if forcer == True :
                ...
                self.traitement.abort()
                time.sleep(0.2)
                self.EcritLog(_("Vous avez interrompu le traitement."))
                return True
    ...
    def OnBoutonOk(self, event): 
        ...
        if TraitmentEnCours:
            ...
            if len(tracks) == 0 :
                dlg = wx.MessageDialog(self, _("Vous devez cocher au moins un individu dans la liste !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False

            # Demande confirmation de lancement
            dlgConfirm = wx.MessageDialog(self, _("Souhaitez-vous saisir le forfait pour les %d individus cochés ?\n\nLe traitement peut prendre quelques minutes...") % len(tracks), _("Confirmation"), wx.YES_NO|wx.CANCEL|wx.YES_DEFAULT|wx.ICON_QUESTION)
            reponse = dlgConfirm.ShowModal()
            dlgConfirm.Destroy()
            ...
            # Lancer traitement
            self.EcritLog(_("Lancement du traitement"))
