...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        intro = _("Vous pouvez saisir ici un lot de cotisations. Sélectionnez les paramètres des cotisations à créer, cochez les individus ou les familles puis cliquez sur le bouton Ok. Utilisez la commande de Filtrage de liste pour effectuer une sélection rapide des individus ou des familles.")
        titre = _("Saisir un lot de cotisations")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Cotisation.png")

        # Liste
        self.staticbox_individus_staticbox = wx.StaticBox(self, -1, _("Individus / Familles"))

        self.label_info = wx.StaticText(self, -1, _("Double-cliquez sur une ligne pour modifier le numéro de cotisation à générer."))
        self.label_info.SetFont(wx.Font(7, wx.SWISS, wx.NORMAL, wx.NORMAL))
        self.check_masquer = wx.CheckBox(self, -1, _("Masquer si la cotisation existe déjà"))
        ...
        self.bouton_ok = CTRL_Bouton_image.CTRL(self, texte=_("Générer les cotisations"), cheminImage="Images/32x32/Valider.png")
    ...
    def __set_properties(self):
        self.check_masquer.SetToolTip(wx.ToolTip(_("Masquez les familles ou les individus qui possèdent déjà la cotisation sélectionnée")))
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez ici pour obtenir de l'aide")))
        self.bouton_ok.SetToolTip(wx.ToolTip(_("Cliquez ici pour inscrire les individus cochés")))
    ...
    def OnBoutonAide(self, event): 
        ...
        UTILS_Aide.Aide("Saisirunlotdecotisations")
    ...
    def Sauvegarde(self):
        ...
        if len(listeCotisationsExistantes) > 0 :
            ...
            for IDcotisation, IDfamille, IDindividu, numero, prenom_individu, nom_individu in listeCotisationsExistantes :
                ...
                if IDfamille != None and IDfamille in dictTitulaires:
                    ...
                    liste_details.append(_("Famille de %s : Cotisation n°%s") % (nom_famille, numero))
                # Si cotisation individuelle
                if IDindividu != None :
                    if prenom_individu == None : prenom_individu = ""
                    liste_details.append(_("%s %s : Cotisation n°%s") % (nom_individu, prenom_individu, numero))
            # Demande si on continue ou non
            intro = _("Les cotisations suivantes existent déjà avec le même type et la même unité de cotisation :")
            conclusion = _("Souhaitez-vous quand même les générer une nouvelle fois ?")
            detail = "\n".join(liste_details)
            dlg = DLG_Messagebox.Dialog(self, titre=_("Avertissement"), introduction=intro, detail=detail, conclusion=conclusion, icone=wx.ICON_EXCLAMATION, boutons=[_("Oui"), _("Non"), _("Annuler")])
        ...
        # Confirmation
        dlg = wx.MessageDialog(self, _("Confirmez-vous la génération de %d cotisations ?") % len(liste_tracks), _("Confirmation"), wx.YES_NO | wx.NO_DEFAULT | wx.CANCEL | wx.ICON_INFORMATION)
        reponse = dlg.ShowModal()
        dlg.Destroy()
        ...
        dlgprogress = wx.ProgressDialog(_("Génération des cotisations"), _("Veuillez patienter..."), maximum=len(liste_tracks), parent=None, style=wx.PD_SMOOTH | wx.PD_AUTO_HIDE | wx.PD_APP_MODAL)
        index = 1
        for track in liste_tracks :

            dlgprogress.Update(index, _("Génération de la cotisation %d sur %d") % (index, len(liste_tracks)))
            ...
            UTILS_Historique.InsertActions([{
                ...
                "IDfamille": track.IDfamille,
                "IDcategorie": 21,
                "action": _("Saisie de la cotisation ID%d '%s' pour la période du %s au %s") % (IDcotisation, label_prestation, date_debut_periode, date_fin_periode),
            }, ])
        ...
        # Succès
        dlg = wx.MessageDialog(self, _("Les %d cotisations ont été générées avec succès.") % len(liste_tracks), _("Fin"), wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
