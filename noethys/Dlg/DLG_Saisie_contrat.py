...
class CTRL_Tarif(CTRL_Ultrachoice.CTRL):
    ...
    def MAJ(self):
        ...
        for dictTarif in self.listeTarifs :
            ...
            if date_debut == None and date_fin == None : label = _("%s (Sans période de validité)") % nom
            if date_debut == None and date_fin != None : label = _("%s (Jusqu'au %s)") % (nom, UTILS_Dates.DateDDEnFr(date_fin))
            if date_debut != None and date_fin == None : label = _("%s (A partir du %s)") % (nom, UTILS_Dates.DateDDEnFr(date_debut))
            if date_debut != None and date_fin != None : label = _("%s (Du %s au %s)") % (nom, UTILS_Dates.DateDDEnFr(date_debut), UTILS_Dates.DateDDEnFr(date_fin))
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDindividu=None, IDinscription=None, IDcontrat=None, IDmodele=None, mode_modele=False, IDactivite=None, copie=None, copie_conso=True):
        ...
        self.label_tarif = wx.StaticText(self, wx.ID_ANY, _("Tarif de base :"))
        ...
        self.bouton_consommations = CTRL_Bouton_image.CTRL(self, texte=_("Générer des consommations"), cheminImage="Images/32x32/Magique.png")
        ...
        if self.mode_modele == True :
            ...
            if self.IDmodele == None :
                self.SetTitle(_("Saisie d'un modèle de contrat pour l'activité '%s'") % self.nomActivite)
            else :
                self.SetTitle(_("Modification d'un modèle de contrat pour l'activité '%s'") % self.nomActivite)
                self.Importation_modele() 
        
        else :
            # Contrat
            if self.IDcontrat == None :
                self.SetTitle(_("Saisie d'un contrat pour l'activité '%s'") % self.nomActivite)
                if copie != None :
                    self.Importation(IDcontrat=copie, mode_copie=True, copie_conso=copie_conso) 
                if self.IDmodele != None :
                    self.Importation_modele() 
            else :
                self.SetTitle(_("Modification d'un contrat pour l'activité '%s'") % self.nomActivite)
                self.Importation(IDcontrat=self.IDcontrat) 
        

    def __set_properties(self):
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez un nom pour ce modèle de contrat")))
        self.ctrl_date_debut.SetToolTip(wx.ToolTip(_("Saisissez la date de début de contrat")))
        self.ctrl_date_fin.SetToolTip(wx.ToolTip(_("Saisissez la date de fin de contrat")))
        self.ctrl_observations.SetToolTip(wx.ToolTip(_("Saisissez des observations sur ce contrat")))
        self.ctrl_tarif.SetToolTip(wx.ToolTip(_("Sélectionnez un tarif de base dans cette liste des tarifs de type 'forfait crédit' disponibles")))
        self.bouton_assistant.SetToolTip(wx.ToolTip(_("Cliquez ici pour générer automatiquement les périodes")))
        self.bouton_ajouter_periode.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter une période")))
        self.bouton_modifier_periode.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier la période sélectionnée")))
        self.bouton_supprimer_periode.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer la période sélectionnée")))
        ...
        self.bouton_consommations.SetToolTip(wx.ToolTip(_("Cliquez ici pour générer automatiquement des consommations")))
    ...
    def Importation(self, IDcontrat=None, mode_copie=False, copie_conso=True):
        ...
        for IDprestation, date_debut, date_fin, label, montant, date_prestation, IDfacture, IDprefixe, prefixe, numFacture, nbreConso in listeDonnees :
            ...
            if IDprefixe != None :
                numFacture = "%s-%06d" % (prefixe, numFacture)
            else :
                numFacture = "%06d" % numFacture
    ...
    def Sauvegarde(self):
        ...
        if self.mode_modele == True and nom == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement renseigner le nom du modèle !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        date_debut = self.ctrl_date_debut.GetDate() 
        if date_debut == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement renseigner la date de début !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        date_fin = self.ctrl_date_fin.GetDate()
        if date_fin == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement renseigner la date de fin !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if date_fin < date_debut :
            dlg = wx.MessageDialog(self, _("La date de début ne doit pas être supérieure à la date de fin !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if IDtarif == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner un tarif dans la liste proposée !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if len(listePeriodes) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir au moins une période !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        for dictPeriode in listePeriodes :
            if dictPeriode["date_debut"] < date_debut :
                dlg = wx.MessageDialog(self, _("La période '%s' comporte une date de début antérieure à la date de début du contrat !") % dictPeriode["label_prestation"], _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
            if dictPeriode["date_fin"] > date_fin :
                dlg = wx.MessageDialog(self, _("La période '%s' comporte une date de fin supérieure à la date de fin du contrat !") % dictPeriode["label_prestation"], _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
            ...
            for dictConso in dictPeriode["listeConso"] :
                if dictConso["IDconso"] == None :
                    ...
                    if ((dictConso["date"], dictConso["IDunite"], self.IDgroupe) in dictOuvertures) == False :
                        listeAnomalies.append(_("%s : Unité %s fermée pour le groupe %s") % (dateFr, dictUnites[dictConso["IDunite"]]["nom"], self.nomGroupe))
                        valide = False
                    ...
                    if dictConso["date"] in dictConsommations :
                        for dictConsoTemp in dictConsommations[dictConso["date"]] :
                            ...
                            if self.VerifieCompatibilitesUnites(dictUnites, dictConsoTemp["IDunite"], dictConso["IDunite"]) == False :
                                listeAnomalies.append(_("%s : Unité %s incompatible avec unité %s déjà présente") % (dateFr, nomUnite1, nomUnite2))
                                valide = False
                            ...
                            if dictConso["IDunite"] == dictConsoTemp["IDunite"] :
                                if dictUnites[dictConso["IDunite"]]["type"] == "Multihoraire" :
                                    if dictConso["heure_fin"] > dictConsoTemp["heure_debut"] and dictConso["heure_debut"] < dictConsoTemp["heure_fin"] :
                                        listeAnomalies.append(_("%s : L'unité multihoraires %s chevauche une consommation d'une unité identique") % (dateFr, nomUnite1))
                                        valide = False
                                else :
                                    listeAnomalies.append(_("%s : Unité %s déjà présente") % (dateFr, nomUnite1))
                                    valide = False
                    
        # Signalement des anomalies
        if len(listeAnomalies) :
            message1 = _("Validation du contrat impossible.\n\nLes %d anomalies suivantes ont été trouvées :") % len(listeAnomalies)
            message2 = "\n".join(listeAnomalies)
            dlg = dialogs.MultiMessageDialog(self, message1, caption = _("Génération des consommations"), msg2=message2, style = wx.ICON_EXCLAMATION | wx.YES|wx.YES_DEFAULT, btnLabels={wx.ID_YES : _("Ok")})
...
##                    listeAnomalies.append(_("%s : Unité %s fermée pour le groupe %s") % (UTILS_Dates.DateDDEnFr(dictConso["date"]), dictUnites[dictConso["IDunite"]]["nom"], self.nomGroupe))
##
##        # Signalement des anomalies
##        if len(listeAnomalies) :
##            message1 = _("Impossible de créer ce contrat : Les %d anomalies suivantes ont été trouvées :") % len(listeAnomalies)
##            message2 = "\n".join(listeAnomalies)
##            dlg = dialogs.MultiMessageDialog(self, message1, caption = _("Génération de consommations"), msg2=message2, style = wx.ICON_EXCLAMATION | wx.YES|wx.YES_DEFAULT, btnLabels={wx.ID_YES : _("Ok")})
##            reponse = dlg.ShowModal() 
##            dlg.Destroy() 
##            return False
##        
...
class Dialog_selection_activite(wx.Dialog):
    def __init__(self, parent):
        ...
        self.parent = parent
        self.SetTitle(_("Sélection d'une activité"))
        
        self.label_intro = wx.StaticText(self, wx.ID_ANY, _("Sélectionnez l'activité pour laquelle vous souhaitez créer un modèle de contrat puis cliquez sur OK :"))
    ...
    def OnBoutonOk(self, event):  
        if len(self.ctrl_activites.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner une activité dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        self.EndModal(wx.ID_OK)
