...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDfamille=None, IDutilisateur=None, mode_virtuel=False):
        ...
        # Bandeau
        intro = _("Sélectionnez les paramètres du compte internet.")
        titre = _("Paramètres du compte internet")
        self.SetTitle(titre)
        ...
        self.label_activation = wx.StaticText(self, -1, _("Compte activé :"))
        ...
        # Codes
        self.box_codes_staticbox = wx.StaticBox(self, -1, _("Codes d'accès"))
    ...
    def __set_properties(self):
        self.radio_activation_oui.SetToolTip(wx.ToolTip(_("Cliquez ici pour activer le compte internet")))
        self.radio_activation_non.SetToolTip(wx.ToolTip(_("Cliquez ici pour désactiver le compte internet")))
        self.bouton_identifiant_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour autoriser la modification manuelle de l'identifiant")))
        self.bouton_mdp_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour autoriser la modification manuelle du mot de passe")))
        self.bouton_identifiant_regenerer.SetToolTip(wx.ToolTip(_("Cliquez ici pour régénérer automatiquement un nouvel identifiant")))
        self.bouton_mdp_regenerer.SetToolTip(wx.ToolTip(_("Cliquez ici pour régénérer automatiquement un nouveau mot de passe")))
    ...
    def ActiveMdp(self, event):
        if "********" in self.ctrl_mdp.GetValue():
            dlg = wx.MessageDialog(self, _("Attention, ce mot de passe a déjà été personnalisé par l'usager ! \n\nSouhaitez-vous vraiment modifier ce mot de passe ?"), _("Avertissement"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
    ...
    def RegenererIdentifiant(self, event):
        dlg = wx.MessageDialog(self, _("Vous êtes vraiment sûr de vouloir générer un nouvel identifiant ?"), _("Avertissement"), wx.YES_NO | wx.NO_DEFAULT | wx.CANCEL | wx.ICON_EXCLAMATION)
        reponse = dlg.ShowModal()
        dlg.Destroy()
        if reponse != wx.ID_YES:
            return False
    ...
    def RegenererMdp(self, event):
        ...
        dlg = wx.MessageDialog(self, _("Vous êtes vraiment sûr de vouloir générer un nouveau mot de passe ?"), _("Avertissement"), wx.YES_NO | wx.NO_DEFAULT | wx.CANCEL | wx.ICON_EXCLAMATION)
        reponse = dlg.ShowModal()
        dlg.Destroy()
        if reponse != wx.ID_YES:
            return False
    ...
    def OnBoutonOk(self, event):
        ...
        if len(dictDonnees["internet_identifiant"]) < 7 and dictDonnees["internet_identifiant"] != "demo" :
            dlg = wx.MessageDialog(self, _("L'identifiant est vraiment court.\n\nSouhaitez-vous tout de même le valider ?"), _("Avertissement"), wx.YES_NO | wx.NO_DEFAULT | wx.CANCEL | wx.ICON_EXCLAMATION)
            reponse = dlg.ShowModal()
            dlg.Destroy()
            if reponse != wx.ID_YES:
                return False
        ...
        if len(dictDonnees["internet_mdp"]) < 7 and dictDonnees["internet_mdp"] != "demo" :
            dlg = wx.MessageDialog(self, _("Le mot de passe est vraiment court.\n\nSouhaitez-vous tout de même le valider ?"), _("Avertissement"), wx.YES_NO | wx.NO_DEFAULT | wx.CANCEL | wx.ICON_EXCLAMATION)
            reponse = dlg.ShowModal()
            dlg.Destroy()
            if reponse != wx.ID_YES:
                return False
        ...
        DB = GestionDB.DB()
        if self.IDfamille != None :
            req = """SELECT IDfamille, internet_identifiant
            FROM familles
            ...
            if len(listeDonnees) > 0 :
                dlg = wx.MessageDialog(self, _("Cet identifiant a déjà été attribué à %d famille(s) !") % len(listeDonnees), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False

        if self.IDutilisateur != None :
            req = """SELECT IDutilisateur, internet_identifiant
            FROM utilisateurs
            ...
            DB.Close()
            if len(listeDonnees) > 0 :
                dlg = wx.MessageDialog(self, _("Cet identifiant a déjà été attribué à %d utilisateur(s) !") % len(listeDonnees), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
