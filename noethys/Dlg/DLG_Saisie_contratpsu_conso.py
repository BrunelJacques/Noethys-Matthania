...
class Panel_planning(wx.Panel):
    def __init__(self, parent, clsbase=None):
        ...
        # Généralités
        self.box_generalites_staticbox = wx.StaticBox(self, wx.ID_ANY, _("Période d'application"))
    ...
    def Validation(self):
        ...
        if self.ctrl_date_debut.GetDate() < self.date_debut :
            dlg = wx.MessageDialog(self, _("La date de début ne peut pas être inférieure à la date de début du contrat !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False

        if self.ctrl_date_fin.GetDate() > self.date_fin :
            dlg = wx.MessageDialog(self, _("La date de fin ne peut pas être supérieure à la date de fin du contrat !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
...
class Panel_calendrier(wx.Panel):
    ...
    def Validation(self):
        ...
        listeDates.sort()
        if len(listeDates) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner au moins une date dans le calendrier !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
