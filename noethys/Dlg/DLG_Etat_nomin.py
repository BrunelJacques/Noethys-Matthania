...
class CTRL_Periode(wx.Panel):
    ...
    def Validation(self):
        if self.ctrl_date_debut.GetDate() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de début de validité !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_date_debut
            return False
        if self.ctrl_date_fin.GetDate() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de fin de validité !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_date_debut
            return False
...
class CTRL_Caisses(wx.CheckListBox):
    ...
    def Importation(self):
        listeCaisses = [(_("Caisse non spécifiée"), 0),]
        dictCaisses = {"nom":_("Caisse non spécifiée"), "IDcaisse":0}
...
class CTRL_Age(wx.Panel):
    def __init__(self, parent):
        ...
        self.radio_tous = wx.RadioButton(self, -1, _("Tous les individus"), style=wx.RB_GROUP)
        self.radio_dates = wx.RadioButton(self, -1, _("Nés entre le"))
        self.ctrl_date_debut = CTRL_Saisie_date.Date(self)
        self.label_separation1 = wx.StaticText(self, -1, _("et le"))
        self.ctrl_date_fin = CTRL_Saisie_date.Date(self)
        self.radio_age = wx.RadioButton(self, -1, _("Age entre"))
        self.ctrl_age_min = wx.SpinCtrl(self, -1, "", min=0, max=100)
        self.ctrl_age_min.SetMinSize((60, -1))

        self.label_separation2 = wx.StaticText(self, -1, _("et"))
    ...
    def Validation(self):
        if self.radio_dates.GetValue() == True :
            if self.ctrl_date_debut.GetDate() == None or self.ctrl_date_fin.GetDate() == None :
                dlg = wx.MessageDialog(self, _("L'intervalle de dates de naissance semble incorrect !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        return True
...
class CTRL_QF(wx.Panel):
    def __init__(self, parent):
        ...
        # Contrôles
        self.radio_tous = wx.RadioButton(self, -1, _("Tous les quotients"), style=wx.RB_GROUP)
        self.radio_sans = wx.RadioButton(self, -1, _("Sans quotient"))
        self.radio_qf = wx.RadioButton(self, -1, _("Entre"))
        self.ctrl_qf_min = wx.SpinCtrl(self, -1, "", min=0, max=99999)
        self.ctrl_qf_min.SetMinSize((70, -1))
        self.label_separation = wx.StaticText(self, -1, _("et"))
...
class CTRL_Champs(wx.Panel):
    def __init__(self, parent):
        ...
        self.label_profil = wx.StaticText(self, -1, _("Profil :"))
        ...
        self.label_champs = wx.StaticText(self, -1, _("Champs :"))
        ...
        # Properties
        self.ctrl_profil.SetToolTip(wx.ToolTip(_("Selectionnez un profil de liste")))
        self.bouton_profils.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à la gestion des profils")))
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier la liste des champs")))
        self.bouton_monter.SetToolTip(wx.ToolTip(_("Cliquez ici pour monter le champ sélectionné")))
        self.bouton_descendre.SetToolTip(wx.ToolTip(_("Cliquez ici pour descendre le champ sélectionné")))
...
class CTRL_Filtres(wx.Notebook):
    def __init__(self, parent):
        ...
        self.listePages = [
            {"code": "groupes", "ctrl": Page_Groupes(self), "label": _("Groupe")},
            {"code": "categories", "ctrl": Page_Categories(self), "label": _("Catégorie de tarif")},
            {"code": "age", "ctrl": Page_Age(self), "label": _("Age")},
            {"code": "caisse", "ctrl": Page_Caisse(self), "label": _("Caisse")},
            {"code": "quotient", "ctrl": Page_Quotient(self), "label": _("Quotient familial")},
            {"code": "etats", "ctrl": Page_Etat(self), "label": _("Etat")},
        ]
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        # Champs
        self.box_champs = wx.StaticBox(self, -1, _("Champs"))
        self.ctrl_champs = CTRL_Champs(self)
        self.ctrl_champs.SetMinSize((200, 220))

        # Filtres
        self.box_filtres = wx.StaticBox(self, -1, _("Filtres"))
    ...
    def __set_properties(self):
        ...
        self.bouton_ok.SetToolTip(wx.ToolTip(_("Cliquez ici pour afficher les resultats")))
    ...
    def OnBoutonOk(self, event):
        ...
        if len(listeGroupes) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun groupe !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if len(listeCategories) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune catégorie de tarif !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if len(listeChamps) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun champ !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if len(listeChampsIndisponibles) > 0 :
            message = _("Attention, les %d champs suivants ne sont pas disponibles avec les activités sélectionnées : %s. Ils seront donc exclus des résultats.\n\nSouhaitez-vous quand même continuer ?") % (len(listeChampsIndisponibles), ", ".join(listeChampsIndisponibles))
            dlg = wx.MessageDialog(self, message, _("Avertissement"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
