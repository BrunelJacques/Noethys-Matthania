...
##        self.bouton_banques = wx.BitmapButton(self, -1, wx.Bitmap(Chemins.GetStaticPath(u"Images/16x16/Mecanisme.png"), wx.BITMAP_TYPE_ANY))
        ...
        self.label_sequence = wx.StaticText(self, -1, _("Séquence :"))
...
##        self.bouton_banques.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à la gestion des établissements bancaires")))
        ...
        self.ctrl_sequence.SetToolTip(wx.ToolTip(_("Sélectionnez la séquence de l'opération (Si vous n'êtes pas sûr, laissez ce que Noethys a sélectionné automatiquement)")))
...
##        box_rib.Add(grid_sizer_banque, 1, wx.LEFT|wx.RIGHT|wx.EXPAND, 10)
...
##        if self.track.prelevement_banque != None : self.ctrl_banque.SetID(self.track.prelevement_banque)
    ...
    def GetTrack(self):
        ...
        iban = self.ctrl_iban.GetValue()
        bic = self.ctrl_bic.GetValue()
        IDbanque = None #self.ctrl_banque.GetID()
...
##            return False
        ...
        if libelle == "" :
            dlg = wx.MessageDialog(self, _("Vous avez oublié de saisir un libellé pour cette opération ! \n\n(Ex : 'Facture n°346573', 'Décembre 2013', etc...)"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
