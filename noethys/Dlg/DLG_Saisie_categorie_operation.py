...
class CTRL_Compte(wx.Choice):
    ...
    def GetListeDonnees(self):
        ...
        for IDcompte, nom, numero in listeDonnees :
            ...
            label = "%s%s  -  %s" % (" "*len(numero)*2, numero, nom)
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDcategorie=None, typeCategorie="credit"):
        ...
        self.label_abrege = wx.StaticText(self, wx.ID_ANY, _("Abrégé :"))
        self.ctrl_abrege = wx.TextCtrl(self, wx.ID_ANY, "")
        ...
        self.label_journal = wx.StaticText(self, wx.ID_ANY, _("Journal :"))
        self.ctrl_journal = wx.TextCtrl(self, wx.ID_ANY, "")
        ...
        if self.IDcategorie != None :
            titre = _("Modification d'une catégorie")
            self.Importation() 
        else :
            titre = _("Saisie d'une catégorie")
        if self.typeCategorie == "credit" : titre += _(" de crédit")
        if self.typeCategorie == "debit" : titre += _(" de débit")
        self.SetTitle(titre)

    def __set_properties(self):
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez le nom de la catégorie")))
        self.ctrl_abrege.SetToolTip(wx.ToolTip(_("Saisissez le nom abrégé de la catégorie")))
        self.ctrl_compte.SetToolTip(wx.ToolTip(_("Sélectionnez un compte comptable")))
        self.bouton_comptes.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à la gestion des comptes comptables")))
        self.ctrl_journal.SetToolTip(wx.ToolTip(_("Saisissez le code du journal")))
    ...
    def Sauvegarde(self):
        ...
        if IDcompte == None :
            dlg = wx.MessageDialog(self, _("Vous n'avez pas renseigné le compte comptable.\n\nSouhaitez-vous tout de même valider ?"), _("Avertissement"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
            reponse = dlg.ShowModal() 
            dlg.Destroy()
            if reponse != wx.ID_YES :
        ...
        if journal == "" :
            dlg = wx.MessageDialog(self, _("Vous n'avez pas renseigné le code journal.\n\nSouhaitez-vous tout de même valider ?"), _("Avertissement"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
            reponse = dlg.ShowModal() 
            dlg.Destroy()
            if reponse != wx.ID_YES :
