...
class CTRL(HTL.HyperTreeList):
    def __init__(self, parent): 
        ...
        listeColonnes = [
            ( _("Individu/Date"), 230, wx.ALIGN_LEFT),
            ( _("Unité"), 170, wx.ALIGN_LEFT),
            ( _("Activité"), 170, wx.ALIGN_LEFT),
            ( _("Etat"), 90, wx.ALIGN_LEFT),
            ]
        numColonne = 0
    ...
    def Remplissage(self):
        ...
        for IDindividu, dictIndividu in dictResultats.items() :
            nomCompletIndividu = "%s %s" % (dictIndividu["nom"], dictIndividu["prenom"])
    ...
    def OnContextMenu(self, event):
        ...
        if dictItem == None :
            dlg = wx.MessageDialog(self, _("Double-cliquez sur le nom de l'individu pour accéder à sa fiche famille !"), _("Astuce"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return            
    ...
    def OnDoubleClick(self, event):
        ...
        if len(listeFamilles) == 1 :
            ...
            dlg = wx.SingleChoiceDialog(self, _("Cet individu est rattaché à %d familles.\nLa fiche de quelle famille souhaitez-vous ouvrir ?") % len(listeFamilles), _("Rattachements multiples"), listeNoms, wx.CHOICEDLG_STYLE)
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        intro = _("Vous pouvez ici consulter la liste des consommations qui ne possèdent pas de prestations associées. Il s'agit donc de consommations 'gratuites' qui n'ont pas été facturées. Double-cliquez sur une ligne pour ouvrir la fiche famille correspondante.")
        titre = _("Consommations sans prestations")
        self.SetTitle(titre)
