    ...
    def SetListeDonnees(self, type=""):
        ...
        if type == "famille" :
            ...
            if self.GetGrandParent().IDfamille != None :
                ...
                if nbreTitulaires == 2 : 
                    nomTitulaires = _("%s et %s") % (listeTitulaires[0], listeTitulaires[1])
                if nbreTitulaires > 2 :
                    nomTitulaires = ""
...
class CTRL_Activites(wx.TextCtrl):
    ...
    def Modifier(self):
        ...
        for dictTemp in self.listeActivites :
            ...
            index += 1
        dlg = wx.MultiChoiceDialog(self, _("Cochez les activités à associer à la cotisation :"), _("Associer des activités"), listeLabels)
        dlg.SetSize((500, 550))
...
class CTRL_Parametres(wx.Panel):
    ...
    def __set_properties(self):
        self.ctrl_type.SetToolTip(wx.ToolTip(_("Sélectionnez ici le type de cotisation")))
        self.ctrl_unite.SetToolTip(wx.ToolTip(_("Sélectionnez ici l'unité de cotisation")))
        self.ctrl_beneficiaire.SetToolTip(wx.ToolTip(_("Sélectionnez ici la famille ou l'individu qui bénéficie de cette cotisation")))
        self.ctrl_date_debut.SetToolTip(wx.ToolTip(_("Saisissez ici la date de début de validité")))
        self.ctrl_date_fin.SetToolTip(wx.ToolTip(_("Saisissez ici la date de fin de validité")))
        self.bouton_activites.SetToolTip(wx.ToolTip(_("Cliquez ici pour associer une ou plusieurs activités à cette cotisation")))
        self.ctrl_observations.SetToolTip(wx.ToolTip(_("Saisissez un texte libre (optionnel)")))
        self.ctrl_creation.SetToolTip(wx.ToolTip(_("Selectionnez OUI si une carte d'adhérent a ete créee")))
        self.ctrl_numero.SetMinSize((70, -1))
        self.ctrl_numero.SetToolTip(wx.ToolTip(_("Saisissez ici le numéro de la carte d'adhérent")))
        self.ctrl_date_creation.SetToolTip(wx.ToolTip(_("Saisissez ici la date de création de la carte d'adhérent")))
        self.ctrl_depot.SetMinSize((70, -1))
        self.ctrl_depot.SetToolTip(wx.ToolTip(_("Numero et date du dépôt")))
        self.ctrl_facturer.SetToolTip(wx.ToolTip(_("Selectionnez OUI si la carte doit etre facturée")))
        self.ctrl_date_prestation.SetToolTip(wx.ToolTip(_("Saisissez ici la date de la prestation à facturer")))
        self.ctrl_montant.SetMinSize((70, -1))
        self.ctrl_montant.SetToolTip(wx.ToolTip(_("Saisissez ici le montant a facturer")))
        self.ctrl_label.SetToolTip(wx.ToolTip(_("Saisissez ici le label de la prestation a créer")))
        self.ctrl_payeur.SetToolTip(wx.ToolTip(_("Sélectionnez la famille à facturer")))
    ...
    def Validation(self):
        ...
        if self.ctrl_type.GetID() == None:
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner un type de cotisation !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if self.ctrl_unite.GetID() == None:
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner une unité de cotisation !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if self.ctrl_creation.GetValue() == True:
            ...
            if self.ctrl_date_creation.FonctionValiderDate() == False or date_creation == None:
                dlg = wx.MessageDialog(self, _("Vous devez saisir une date de création de carte valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_date_creation.SetFocus()
                return False
            ...
            if self.ctrl_numero.GetValue() == "" and (self.mode_lot == False or self.radio_numero_manuel.GetValue() == True) :
                dlg = wx.MessageDialog(self, _("Etes-vous sûr de ne pas vouloir saisir de numéro d'adhérent ?"), _("Confirmation"), wx.YES_NO | wx.NO_DEFAULT | wx.CANCEL | wx.ICON_INFORMATION)
        ...
        if self.ctrl_facturer.GetValue() == True:
            ...
            if self.ctrl_date_prestation.FonctionValiderDate() == False or date_prestation == None:
                dlg = wx.MessageDialog(self, _("Vous devez saisir une date de facturation valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
            ...
            if self.ctrl_montant.GetMontant() == 0.00:
                dlg = wx.MessageDialog(self, _("Etes-vous sûr de ne pas vouloir saisir de montant ?"), _("Confirmation"), wx.YES_NO | wx.NO_DEFAULT | wx.CANCEL | wx.ICON_EXCLAMATION)
                reponse = dlg.ShowModal()
                dlg.Destroy()
                if reponse != wx.ID_YES:
                    return False
            ...
            if self.ctrl_label.GetValue() == "":
                dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un label pour la facturation !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
            ...
            if self.IDprestation != None:
                dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer la prestation qui a déjà été créée pour cette cotisation ?"), _("Suppression"), wx.YES_NO | wx.NO_DEFAULT | wx.CANCEL | wx.ICON_INFORMATION)
    ...
    def Sauvegarde(self):
        ...
        UTILS_Historique.InsertActions([{
            ...
            "action": _("%s de la cotisation ID%d '%s' pour la période du %s au %s") % (type, self.IDcotisation, label_prestation, date_debut_periode, date_fin_periode),
        }, ])
    ...
    def Importation(self):
        ...
        if IDdepot_cotisation == None:
            self.ctrl_depot.SetValue(_("Non déposée"))
        else:
            self.ctrl_depot.SetValue(_("Déposée sur le dépôt n°%d") % IDdepot_cotisation)
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDcotisation=None, IDfamille=None, IDindividu=None, dictFamillesRattachees={}):
        ...
        if self.IDcotisation == None :
            self.SetTitle(_("Saisie d'une cotisation"))
        else:
            self.SetTitle(_("Modification d'une cotisation"))
