...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDcategorie=None):
        ...
        if self.IDcategorie == None :
            self.SetTitle(_("Saisie d'une catégorie de message"))
        else:
            self.Importation()
            self.SetTitle(_("Modification d'une catégorie de message"))

    def __set_properties(self):
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez un nom pour cette catégorie")))
    ...
    def OnBoutonOk(self, event): 
        ...
        if len(self.ctrl_nom.GetValue()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un nom pour cette catégorie !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
