...
class CTRL_Caisse(wx.Choice):
    ...
    def GetListeDonnees(self):
        listeItems = []
        self.dictDonnees = {}
        self.dictDonnees[0] = { "ID" : None, "nom " : _("- Aucune caisse -") }
        listeItems.append(_("- Aucune caisse -"))
        db = GestionDB.DB()
...
class CTRL_Beneficiaires(wx.CheckListBox):
    ...
    def MAJ(self):
        ...
        for IDindividu, nom, prenom in listeDonnees :
            ...
            listeValeurs.append((IDindividu, "%s %s" % (nom, prenom), False))
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDaide=None, IDfamille=None):
        ...
        self.bouton_importer = CTRL_Bouton_image.CTRL(self, texte=_("Importer un modèle"), cheminImage="Images/32x32/Fleche_bas.png")
        ...
        if self.IDaide != None :
            if self.IDfamille == None :
                self.SetTitle(_("Modification d'un modèle d'aide"))
            else:
                self.SetTitle(_("Modification d'une aide"))
            self.Importation()
        else:
            if self.IDfamille == None :
                self.SetTitle(_("Saisie d'un modèle d'aide"))
            else:
                self.SetTitle(_("Saisie d'une aide"))
    ...
    def __set_properties(self):
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez ici le nom de l'aide (Exemple : 'Bons vacances CAF 2017')")))
        self.ctrl_activite.SetToolTip(wx.ToolTip(_("Sélectionnez ici l'activité")))
        self.ctrl_caisse.SetToolTip(wx.ToolTip(_("Sélectionnez ici une caisse")))
        ...
        self.ctrl_beneficiaires.SetToolTip(wx.ToolTip(_("Cochez ici les bénéficiaires de l'aide")))
        self.checkbox_plafond_montant.SetToolTip(wx.ToolTip(_("Cliquez ici pour appliquer un montant maximal")))
        self.ctrl_plafond_montant.SetToolTip(wx.ToolTip(_("Saisissez ici le montant maximal")))
        self.checkbox_plafond_quantite.SetToolTip(wx.ToolTip(_("Cliquez ici pour appliquer un nombre de dates maximal")))
        self.ctrl_plafond_quantite.SetToolTip(wx.ToolTip(_("Cliquez ici le nombre maximal de dates")))
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter un montant")))
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier le montant sélectionné dans la liste")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer le montant sélectionné dans la liste")))
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez ici pour obtenir de l'aide")))
        self.bouton_importer.SetToolTip(wx.ToolTip(_("Cliquez ici pour importer un modèle d'aide prédéfini")))
    ...
    def OnBoutonOk(self, event):
        ...
        IDactivite = self.ctrl_activite.GetID()
        if IDactivite == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner une activité !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
...
##        if IDcaisse == None :
##            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une caisse !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
##            dlg.ShowModal()
##            dlg.Destroy()
...
##            return
        ...
        if date_fin < date_debut :
            dlg = wx.MessageDialog(self, _("Vous avez saisi une date de début supérieure à la date de fin !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if self.checkbox_plafond_montant.GetValue() == True :
            ...
            if plafond_montant == None or plafond_montant == 0.0 :
                dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un montant plafond !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return
        else:
        ...
        if self.checkbox_plafond_quantite.GetValue() == True :
            ...
            if nbre_plafond_quantite == 0 :
                dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un de nombre de dates maximal !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return
        else:
...
##            self.checkbox_plafond_quantite.SetValue(True) 
        ...
        if self.IDfamille != None :
            if len(listeBeneficiaires) == 0 :
                dlg = wx.MessageDialog(self, _("Vous sélectionner au moins un bénéficiaire dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return
        ...
        if len(listeMontants) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez saisi aucun montant !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
