...
def FiltreCar(texte):
    ...
    if new != texte:
        wx.MessageBox("Les caractères spéciaux ne sont pas autorisés dans les noms!")
    return new
...
class Sieste(wx.Choice):
    def __init__(self, parent):
        ...
        self.parent = parent
        self.SetToolTip(wx.ToolTip(_("Sélectionnez les infos particulières pour cet individu")))
        self.MAJ() 
    ...
    def GetListeDonnees(self):
        ...
        db.Close()
        self.dictDonnees = { 0 : (None, _("A préciser")) }
        listeNoms = [ _("A préciser")]
        index = 1
...
class Panel_identite(wx.Panel):
    def __init__(self, parent, IDindividu=None):
        ...
        self.label_civilite = wx.StaticText(self.staticbox_etatcivil_staticbox, -1, _("Civilité/genre :"))
        self.ctrl_civilite = CTRL_Saisie_civilite.Civilite(self.staticbox_etatcivil_staticbox)
        self.label_nomjfille = wx.StaticText(self.staticbox_etatcivil_staticbox, -1, _("Nom de jeune fille :"))
        self.ctrl_nomjfille = wx.TextCtrl(self.staticbox_etatcivil_staticbox, -1, "")
        self.label_nom = wx.StaticText(self.staticbox_etatcivil_staticbox, -1, _("Nom :"))
        self.ctrl_nom = wx.TextCtrl(self.staticbox_etatcivil_staticbox, -1, "")
        self.label_prenom = wx.StaticText(self.staticbox_etatcivil_staticbox, -1, _("Prénom :"))
        self.ctrl_prenom = wx.TextCtrl(self.staticbox_etatcivil_staticbox, -1, "")
        self.label_numsecu = wx.StaticText(self.staticbox_etatcivil_staticbox, -1, _("Num. sécu. :"))
        self.ctrl_numsecu = CTRL_Saisie_numSecu.NumSecu(self.staticbox_etatcivil_staticbox)
        self.label_nationalite = wx.StaticText(self.staticbox_etatcivil_staticbox, -1, _("Nationalité :"))
        self.ctrl_nationalite = CTRL_Saisie_pays.SaisiePays(self.staticbox_etatcivil_staticbox, mode="nationalite")
        
        # Naissance
        self.staticbox_naiss_staticbox = wx.StaticBox(self, -1, _("Naissance"))
        self.label_datenaiss = wx.StaticText(self.staticbox_naiss_staticbox, -1, _("Date :"))
        self.ctrl_datenaiss = CTRL_Saisie_date.Date(self.staticbox_naiss_staticbox)
        self.ctrl_age = wx.TextCtrl(self.staticbox_naiss_staticbox, -1, "", style=wx.TE_CENTRE, size=(46,-1))
        self.label_paysnaiss = wx.StaticText(self.staticbox_naiss_staticbox, -1, _("Pays de naissance :"))
        self.ctrl_paysnaiss = CTRL_Saisie_pays.SaisiePays(self.staticbox_naiss_staticbox, mode="pays")
        self.label_lieunaiss = wx.StaticText(self.staticbox_naiss_staticbox, -1, _("Lieu | C.P. :"))
        self.ctrl_adressenaiss = CTRL_Saisie_adresse.Adresse(self.staticbox_naiss_staticbox)
        
        # Décès
        self.staticbox_deces = wx.StaticBox(self, -1, _("Changement famille"))
        self.ctrl_deces = wx.CheckBox(self.staticbox_deces, -1, "")
        self.label_deces = wx.StaticText(self.staticbox_deces, -1, _("Année :"))
        self.ctrl_annee_deces = wx.TextCtrl(self.staticbox_deces, -1, "")
        
        # Sieste
        self.staticbox_sieste = wx.StaticBox(self, -1, _("Complément info"))
        self.ctrl_sieste = Sieste(self.staticbox_sieste)
        self.bouton_sieste = wx.Button(self.staticbox_sieste, -1, "...", size=(20, 20))
    ...
    def __set_properties(self):
        ...
        self.ctrl_prenom.SetToolTip(wx.ToolTip(_("Saisissez ici le prénom de l'individu")))
        ...
        self.ctrl_deces.SetToolTip(wx.ToolTip(_("Cochez cette case si l'individu a changé de famille")))
        self.ctrl_annee_deces.SetToolTip(wx.ToolTip(_("Saisissez ici l'année de séparation")))
        self.ctrl_annee_deces.SetMinSize((40, -1))
        self.ctrl_sieste.SetToolTip(wx.ToolTip(_("Sélectionnez ici des infos complémentaires pour cet individu")))
        self.bouton_sieste.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à la fenêtre de gestion des infos complémentaires")))
    ...
    def OnMaxLen(self,event):
        mess = "Limite du nombre de caractères atteinte"
        wx.MessageBox(mess)
        if event != None :
            event.Skip()
...
##            return
        ...
        if dateFR == None : 
            ...
            if sexe == "M" : texte = _("Né le %s (%d ans)") % (dateFR, age)
    ...
    def OnChoiceCivilite(self, event):
        ...
        # Vérifie la présence d'une désignation famille change désignation si elle est l'individu
...
##        self.ctrl_deces.Enable(etat)
    ...
    def ValidationData(self):
        ...
        # Vérification de la possibilité NOM + prénom sur deux lignes si disponibles
        if self.Parent.page2.lstContacts:
            ...
            if not adresseCourte:
                ...
                if len(nom) + len(prenom) >= 37:
                    dlg = wx.MessageDialog(self, _("Avec une adresse déjà longue, le prénom + le nom doit tenir sur 39 caractères!"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                    dlg.ShowModal()
                    dlg.Destroy()
                    return False
        ...
        if self.ctrl_civilite.GetID() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner une civilité !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if self.ctrl_civilite.GetRubrique() != "AUTRE" and self.ctrl_prenom.GetValue() == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un prénom !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if self.ctrl_deces.GetValue() == True :
            if self.ctrl_annee_deces.GetValue() != "" :
                try :
                    ...
                    dlg = wx.MessageDialog(self, _("L'année de séparation que vous avez saisi n'est pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                    dlg.ShowModal()
                    dlg.Destroy()
                    return False
