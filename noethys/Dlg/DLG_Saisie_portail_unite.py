...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDunite=None, IDactivite=None, ordre=1):
        ...
        # Nom
        self.staticbox_nom_staticbox = wx.StaticBox(self, -1, _("Nom de l'unité de réservation"))
        self.ctrl_nom = wx.TextCtrl(self, -1, "")

        # Caractéristiques
        self.staticbox_caract_staticbox = wx.StaticBox(self, -1, _("Caractéristiques"))

        self.label_unites_principales = wx.StaticText(self, -1, _("Unités de conso\nprincipales :"))
        self.ctrl_unites_principales = Liste_unites_conso(self, IDactivite=self.IDactivite)
        self.ctrl_unites_principales.MAJ()

        self.label_unites_secondaires = wx.StaticText(self, -1, _("Unités de conso\nsecondaires :"))
    ...
    def __set_properties(self):
        if self.IDunite == None :
            self.SetTitle(_("Saisie d'une unité de réservation"))
        else :
            self.SetTitle(_("Modification d'une unité de réservation"))
        self.SetSize((650, -1))
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez ici le nom de l'unité de réservation")))
    ...
    def OnBoutonOk(self, event): 
        ...
        if nom == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un nom pour cette unité de réservation !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if len(listeCochesUnitesPrincipales) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement cocher au moins une unité de consommation principale !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if len(listeUniteConflit) :
            ...
            dlg = wx.MessageDialog(self, _("Les unités secondaires ne peuvent pas être identiques aux unités secondaires. Veuillez décocher les unités secondaires suivantes :\n\n > %s") % ", ".join(listeNoms), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
