...
class Dialog(wx.Dialog):
    def __init__(self, parent, track_mensualite=None, titre_detail=""):
        ...
        intro = _("Vous pouvez consulter ici le détail de la mensualité sélectionnée avec un regroupement par date.")
        titre = _("Détail de la mensualité")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Contrat.png")

        # dates
        self.box_dates_staticbox = wx.StaticBox(self, -1, _("Détail de la mensualité de %s pour %s" % (titre_detail, track_mensualite.individu_nom_complet)))
        ...
        self.Bind(wx.EVT_BUTTON, self.ctrl_dates.Imprimer, self.bouton_imprimer)
        self.Bind(wx.EVT_BUTTON, self.ctrl_dates.ExportTexte, self.bouton_texte)
        self.Bind(wx.EVT_BUTTON, self.ctrl_dates.ExportExcel, self.bouton_excel)
    ...
    def __do_layout(self):
        ...
        # Dates
        box_dates = wx.StaticBoxSizer(self.box_dates_staticbox, wx.VERTICAL)
        ...
        box_dates.Add(grid_sizer_contenu, 1, wx.ALL|wx.EXPAND, 10)
        grid_sizer_base.Add(box_dates, 1, wx.LEFT|wx.RIGHT|wx.EXPAND, 10)
