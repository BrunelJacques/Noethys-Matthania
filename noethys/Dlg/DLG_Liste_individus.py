...
##        self.check_concernes = wx.CheckBox(self, -1, _("Ne pas afficher les dossiers complets"))
...
class Parametres(wx.Panel):
    def __init__(self, parent):
        ...
        # Période
        self.stbPeriode = wx.StaticBox(self, -1, _("Date de référence"))
        self.ctrl_date = CTRL_Saisie_date.Date(self.stbPeriode)
        self.periode = (datetime.date.today(),datetime.date.today())
        self.ctrl_date.SetDate(datetime.date.today())
        self.bouton_date = wx.BitmapButton(self.stbPeriode, -1, wx.Bitmap(Chemins.GetStaticPath(u"Images/16x16/Calendrier.png"), wx.BITMAP_TYPE_ANY))
        
        # Activités
        self.stbActiv = wx.StaticBox(self, -1, _("Activités"))
        self.ctrl_activites = CTRL_Selection_activites_groupes.CTRL(self.stbActiv, periode=self.periode)
...
##        self.Bind(wx.EVT_BUTTON, self.OuvrirFiche, self.bouton_ouvrir_fiche)
    ...
    def __set_properties(self):
        self.SetTitle(_("Liste des individus"))
##        self.bouton_ouvrir_fiche.SetToolTip(wx.ToolTip(_("Cliquez ici pour ouvrir la fiche famille de l'individu sélectionné dans la liste")))
        ...
        self.bouton_configuration.SetToolTip(wx.ToolTip(_("Cliquez ici pour configurer la liste")))
    ...
    def GetLabelParametres(self):
        ...
        if presents == None :
            listeParametres.append(_("Tous les inscrits"))
        else :
            listeParametres.append(_("Uniquement les présents du %s au %s") % (DateEngFr(str(presents[0])), DateEngFr(str(presents[1]))))

        # Archivés
        if self.ctrl_parametres.ctrl_options.GetArchives() == True :
            listeParametres.append(_("Individus archivés inclus"))
