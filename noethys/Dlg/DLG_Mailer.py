...
class Dialog(wx.Dialog):
    def __init__(self, parent, categorie="saisie_libre", afficher_confirmation_envoi=True):
        ...
        # Bandeau
        intro = _("Vous pouvez ici expédier des Emails par lot. Sélectionnez une adresse d'expéditeur, un objet, un ou plusieurs destinataires avant de rédiger votre texte. Les Emails sont envoyés par lots afin de contourner les protections anti-spam des opérateurs.")
        titre = _("Editeur d'Emails")
        self.SetTitle(titre)
        ...
        # Paramètres
        self.box_param_staticbox = wx.StaticBox(self, -1, _("Paramètres"))
        self.label_exp = wx.StaticText(self, -1, _("Exp. :"))
    ...
    def __set_properties(self):
        ...
        self.ctrl_objet.SetToolTip(wx.ToolTip(_("Saisissez l'objet du message")))
        self.bouton_modifier_dest.SetToolTip(wx.ToolTip(_("Cliquez pour ajouter ou supprimer des destinataires")))
        self.bouton_ajouter_piece_spec.SetToolTip(wx.ToolTip(_("Cliquez pour ajouter une pièce jointe personnelle au destinataire sélectionné dans la liste")))
        self.bouton_retirer_piece_spec.SetToolTip(wx.ToolTip(_("Cliquez pour retirer une pièce jointe personnelle au destinataire sélectionné dans la liste")))
        self.bouton_ajouter_piece.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter une pièce jointe")))
        self.bouton_suppr_piece.SetToolTip(wx.ToolTip(_("Cliquez ici pour retirer la pièce jointe sélectionnée dans la liste")))
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez pour obtenir de l'aide")))
    ...
    def OnBoutonOutils(self, event):
        ...
        for motcle, label in listeMotscles :
            ...
            index += 1
        menuPop.AppendMenu(10, _("Insérer un mot-clé"), sousMenuMotscles)

        # Aperçu de la fusion
        item = wx.MenuItem(menuPop, 60, _("Aperçu de la fusion"))
        ...
        # Effacer le texte
        item = wx.MenuItem(menuPop, 50, _("Effacer le texte"))
        ...
        item = wx.MenuItem(menuPop, 30, _("Gestion des modèles"))
        item.SetBitmap(wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Emails_modele.png"), wx.BITMAP_TYPE_PNG))
        menuPop.AppendItem(item)
        ...
        item = wx.MenuItem(menuPop, 40, _("Envoyer un Email de test"))
    ...
    def EnvoyerTest(self, event):
        ...
        dlg = wx.TextEntryDialog(self, _("Saisissez une adresse Email et cliquez sur Ok :"), _("Envoi d'un Email de test"), adresse)
        ...
        if len(listeDestinataires) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez sélectionner au moins un destinataire !\n\n(En cas de fusion, les données du premier destinataire seront utilisés)"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        # Envoi du mail test
    ...
    def Envoyer(self, listeDestinataires=[], adresseTest=None):
        ...
        if dictExp == None :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune adresse d'expéditeur !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if len(sujet) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un objet pour ce message !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            if self.IsShown() == False : self.ShowModal()
            self.ctrl_objet.SetFocus()
        ...
        # Destinataires
        if len(listeDestinataires) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez sélectionner au moins un destinataire !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            if self.IsShown() == False : self.ShowModal()
            return
        ...
        for dest in listeDestinataires :
            if dest.adresse == None :
                nbreAnomalies += 1
        if nbreAnomalies > 0 :
            dlg = wx.MessageDialog(self, _("%d adresse(s) Email ne sont pas renseignées !") % nbreAnomalies, _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        # ajout JB pour éliminer les marges par défaut
        texteHTML = texteHTML.replace("<head>","<head><style>p { margin-top: 0; margin-bottom: 0; } </style>")
        ...
        if adresseTest == None and self.afficher_confirmation_envoi == True :
            dlg = wx.MessageDialog(self, _("Confirmez-vous l'envoi de ce message pour %d destinataires ?\n\nAttention, l'envoi peut prendre quelques minutes...") % len(listeDestinataires), _("Confirmation"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        ...
        dlg_progress = wx.ProgressDialog(_("Envoi des mails"), _("Connexion au serveur de messagerie..."), maximum=len(liste_messages)+1, parent=None)
        dlg_progress.SetSize((450, 140))
        dlg_progress.CenterOnScreen()

        try :
            messagerie = UTILS_Envoi_email.Messagerie(backend=dictExp["moteur"],
                                                      hote=dictExp["smtp"],
                                                      port=dictExp["port"],
    ...
    def MemorisationHistorique(self, adresse="", sujet=""):
        ...
        for IDindividu, IDfamille in listeDonnees :
            UTILS_Historique.InsertActions([{
                ...
                "IDfamille" : IDfamille,
                "IDcategorie" : 33,
                "action" : _("Envoi de l'Email '%s'") % sujet,
                },])
        DB.Close()
    ...
    def VerifieFusion(self, texteHTML="", listeDestinataires=[]):
        ...
        if len(listeResultats) > 0 :
            message = _("Certains mots-clés semblent ne pas avoir été remplacés lors de la fusion des données. Est-ce normal ?\n\n")
            affichageMax = 10
            for adresse, listeAnomalies in listeResultats[:affichageMax] :
                message += "   - %s : %s.\n" % (adresse, ", ".join(listeAnomalies))
            if len(listeResultats) > affichageMax :
                message += _("   - Ainsi que %d autres...\n") % (len(listeResultats) - affichageMax)
            message += _("\nSouhaitez-vous tout de même continuer l'envoi ?")
            dlgErreur = wx.MessageDialog(self, message, _("Anomalies"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
