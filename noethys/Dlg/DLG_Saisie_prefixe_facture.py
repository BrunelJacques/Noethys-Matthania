...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDprefixe=None):
        ...
        self.label_prefixe = wx.StaticText(self, wx.ID_ANY, _("Préfixe :"))
        self.ctrl_prefixe = wx.TextCtrl(self, wx.ID_ANY, "")
        ...
        if self.IDprefixe != None :
            self.SetTitle(_("Modification d'un préfixe de facture"))
            self.Importation() 
        else :
            self.SetTitle(_("Saisie d'un préfixe de facture"))

    def __set_properties(self):
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez le nom du préfixe (interne au logiciel)")))
        self.ctrl_prefixe.SetToolTip(wx.ToolTip(_("Saisissez le préfixe (en majuscules uniquement)")))
    ...
    def Sauvegarde(self):
        ...
        if prefixe == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un préfixe !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        for caract in prefixe :
            if caract not in "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ" :
                dlg = wx.MessageDialog(self, _("Le préfixe ne peut comporter que des chiffres et des lettres majuscules !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_prefixe.SetFocus()
                return False
