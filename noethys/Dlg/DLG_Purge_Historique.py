...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDutilisateur=None, IDfamille=None, IDindividu=None, IDcategorie=None):
        ...
        # Bandeau
        intro = _("Vous pouvez ici purger tout ou une partie de l'historique de Noethys. Cette fonction peut vous permettre d'alléger la base de données au bout de quelques années d'utilisation.")
        titre = _("Purge de l'historique")
    ...
    def __set_properties(self):
        self.SetTitle(_("Historique"))
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez ici pour obtenir de l'aide")))
        self.bouton_ok.SetToolTip(wx.ToolTip(_("Cliquez ici pour lancer la purge")))
        self.bouton_fermer.SetToolTip(wx.ToolTip(_("Cliquez ici pour fermer")))
        self.ctrl_date.SetToolTip(wx.ToolTip(_("Saisissez ici la date limite jusqu'à laquelle les actions seront supprimées")))
    ...
    def OnBoutonOk(self, event):
        ...
        if len(listeCategories) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez sélectionner un moins une catégorie !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if date == None :
            dlg = wx.MessageDialog(self, _("Vous devez saisir une date limite jusqu'à laquelle les actions seront effacées !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        dlg = wx.MessageDialog(self, _("Etes-vous sûr de vouloir purger l'historique ?"), _("Demande de confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
