...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        titre = _("Liste des pièces manquantes")
        ...
        self.bouton_email = CTRL_Bouton_image.CTRL(self, texte=_("Envoyer des rappels par Email"), cheminImage="Images/32x32/Emails_exp.png")
    ...
    def __set_properties(self):
        self.SetTitle(_("Liste des pièces manquantes"))
        ...
        self.bouton_email.SetToolTip(wx.ToolTip(_("Cliquez ici pour envoyer un email de rappel aux familles cochées")))
    ...
    def GetLabelParametres(self):
        ...
        if date == None : date = "---"
        listeParametres.append(_("Situation au %s") % DateEngFr(str(date)))
        ...
        if self.ctrl_parametres.ctrl_options.GetConcernes() == True :
            listeParametres.append(_("Uniquement les dossiers incomplets"))
    ...
    def EnvoyerEmail(self, event):
        ...
        if len(tracks) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune famille dans la liste !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
