...
class Dialog(wx.Dialog):
    def __init__(self, parent, lignes_calcul=[], label=u"", montant=0.0, titre=None):
        ...
        # Bandeau
        intro = _("Vous pouvez ici sélectionner dans la liste un montant et un label pour la prestation qui va être créée. Vous pouvez également double-cliquer dans la liste pour valider aussitôt.")
        titreDefaut = _("Tarif au choix")
        if titre == None :
            titre = titreDefaut
        self.SetTitle(titre)
        ...
        # Détail prestation
        self.box_prestation_staticbox = wx.StaticBox(self, -1, _("Détail de la prestation"))
        ...
        self.ctrl_montant = CTRL_Saisie_euros.CTRL(self, font=wx.Font(13, wx.FONTFAMILY_DEFAULT, wx.NORMAL, wx.BOLD, 0, ""), style=wx.TE_RIGHT|wx.TE_PROCESS_ENTER)
    ...
    def __set_properties(self):
        self.ctrl_label.SetToolTip(wx.ToolTip(_("Vous pouvez modifier ici un label personnalisé pour la prestation")))
        self.ctrl_montant.SetToolTip(wx.ToolTip(_("Saisissez ici le montant de la prestation et tapez sur la touche Entrée pour valider rapidement")))
    ...
    def OnBoutonOk(self, event): 
        ...
        if validation == False :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un montant valide pour cette prestation !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.ctrl_label.GetValue() == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un label pour cette prestation !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
...
if __name__ == "__main__":
    ...
    lignes_calcul = [
        {"label" : "Tarif 1", "montant_unique" : 1.0},
        {"label" : "Tarif 2", "montant_unique" : 2.0},
        {"label" : "Tarif 3", "montant_unique" : 3.0},
        ]
