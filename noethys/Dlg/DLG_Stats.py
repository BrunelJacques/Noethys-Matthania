...
class CTRL_Parametres(wx.html.HtmlWindow):
    ...
    def MAJ(self):
        ...
        if self.dictParametres["mode"] == "inscrits" :
            html += _("<U><B>Période :</B></U> Aucune")
        else:
            html += _("<U><B>Période :</B></U> %s") % self.dictParametres["periode"]["label"]
        html += "<BR><BR>"
        
        # Activités
        listeActivites = self.dictParametres["listeActivites"]
        if len(listeActivites) == 0 :
            html += _("<U><B>Activités :</B></U> Aucune")
        else :
            html += _("<U><B>Activités : </B></U><UL>")
            for IDactivite in listeActivites :
                nomActivite = self.dictParametres["dictActivites"][IDactivite]
                html += "<LI>%s</LI>" % nomActivite
            html += "</UL>"
        ...
        html = _("<html><head><title>Paramètres</title></head><body><FONT SIZE=-1>%s</FONT></body></html>") % html
        self.SetPage(html)
...
####Liste d'objets ici
        self.listeObjets = [
            ...
            {"nom" : _("Individus"), "code" : "individus", "image" : None, "ctrl_notebook" : None, "visible" : True, "pages" : [
            
                    {"nom" : _("Nombre"), "code" : "individus_nombre", "image" : None, "ctrl_html" : None, "visible" : True, "objets" : [
                            INDIVIDUS.Texte_nombre_individus(),
                            INDIVIDUS.Tableau_nombre_individus(),
                            INDIVIDUS.Graphe_nombre_individus(),
                            ]},

                    {"nom" : _("Ancienneté"), "code" : "individus_anciennete", "image" : None, "ctrl_html" : None, "visible" : True, "objets" : [
                            INDIVIDUS.Tableau_nouveaux_individus(),
                            INDIVIDUS.Graphe_nouveaux_individus(),
                            INDIVIDUS.Graphe_arrivee_individus(),
                            ]},

                    {"nom" : _("Genre"), "code" : "individus_genre", "image" : None, "ctrl_html" : None, "visible" : True, "objets" : [
                            INDIVIDUS.Tableau_repartition_genre(),
                            INDIVIDUS.Graphe_repartition_genre(),
                            ]},

                    {"nom" : _("Âge"), "code" : "individus_age", "image" : None, "ctrl_html" : None, "visible" : True, "objets" : [
                    ...
                    {"nom" : _("Coordonnées"), "code" : "individus_coordonnees", "image" : None, "ctrl_html" : None, "visible" : True, "objets" : [
                            INDIVIDUS.Tableau_repartition_villes(),
                            INDIVIDUS.Graphe_repartition_villes(),
                            ]},

                    {"nom" : _("Scolarité"), "code" : "individus_scolarite", "image" : None, "ctrl_html" : None, "visible" : True, "objets" : [
                    ...
                    {"nom" : _("Profession"), "code" : "individus_profession", "image" : None, "ctrl_html" : None, "visible" : True, "objets" : [
            ...
            {"nom" : _("Familles"), "code" : "familles", "image" : None, "ctrl_notebook" : None, "visible" : True, "pages" : [

                    {"nom" : _("Nombre"), "code" : "familles_nombre", "image" : None, "ctrl_html" : None, "visible" : True, "objets" : [
                            FAMILLES.Texte_nombre_familles(),
                            FAMILLES.Tableau_nombre_familles(),
                            FAMILLES.Graphe_nombre_familles(),
                            ]},

                    {"nom" : _("Caisse"), "code" : "familles_caisse", "image" : None, "ctrl_html" : None, "visible" : True, "objets" : [
                            FAMILLES.Tableau_repartition_caisses(),
                            FAMILLES.Graphe_repartition_caisses(),
                            ]},

                    {"nom" : _("Composition"), "code" : "familles_composition", "image" : None, "ctrl_html" : None, "visible" : True, "objets" : [
                            FAMILLES.Tableau_nombre_membres(),
                            FAMILLES.Graphe_nombre_membres(),
                            ]},

                    {"nom" : _("Quotient familial"), "code" : "familles_qf", "image" : None, "ctrl_html" : None, "visible" : True, "objets" : [
        ...
        self.hyper_parametres = Hyperlien(self, label=_("Modifier les paramètres"), infobulle=_("Modifier les paramètres"), URL="parametres")
        ...
        self.hyper_selectionner = Hyperlien(self, label=_("Tout sélectionner"), infobulle=_("Tout sélectionner"), URL="selectionner")
        self.label_separation = wx.StaticText(self, -1, "|")
        self.hyper_deselectionner = Hyperlien(self, label=_("Tout dé-sélectionner"), infobulle=_("Tout dé-sélectionner"), URL="deselectionner")
    ...
    def __set_properties(self):
        ...
        self.bouton_imprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour imprimer")))
    ...
    def OnBoutonImprimer(self, event): 
        ...
        item = wx.MenuItem(menuPop, 10, _("Tout"))
        ...
        item = wx.MenuItem(menuPop, 20, _("La rubrique affichée"))
        item.SetBitmap(wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Imprimante.png"), wx.BITMAP_TYPE_PNG))
        menuPop.AppendItem(item)
        self.Bind(wx.EVT_MENU, self.Imprimer, id=20)

        item = wx.MenuItem(menuPop, 30, _("La page affichée"))
        item.SetBitmap(wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Imprimante.png"), wx.BITMAP_TYPE_PNG))
        menuPop.AppendItem(item)
    ...
    def Imprimer(self, event=None):
        ...
        # Imprimer tout
        dlgAttente = wx.BusyInfo(_("Création du rapport..."), None)
        ...
        if len(listeCodes) == 0 or len(html) <= 50 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune information à imprimer !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
...
##        preview_window = UTILS_Printer.PreviewFrame(preview, None, _("Aperçu avant impression"))
...
##        print self.listeObjets
    ...
    def MAJpage(self, indexRubrique=None, indexPage=None):
        ...
        dlgAttente = wx.BusyInfo(_("Actualisation des données..."), None)
        if 'phoenix' not in wx.PlatformInfo:
            wx.Yield()
