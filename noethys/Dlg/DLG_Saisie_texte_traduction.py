...
class Dialog(wx.Dialog):
    def __init__(self, parent, texte="", traduction=""):
        ...
        self.label_texte = wx.StaticText(self, wx.ID_ANY, _("Texte original :"))
        self.ctrl_texte = wx.TextCtrl(self, wx.ID_ANY, "", style=wx.TE_MULTILINE | wx.TE_READONLY)
        self.ctrl_texte.SetBackgroundColour((245, 245, 245)) 
        
        self.label_traduction = wx.StaticText(self, wx.ID_ANY, _("Traduction :"))
        self.ctrl_traduction = wx.TextCtrl(self, wx.ID_ANY, "", style=wx.TE_MULTILINE)
    ...
    def __set_properties(self):
        ...
        self.ctrl_texte.SetToolTip(wx.ToolTip(_("Texte original")))
        self.ctrl_traduction.SetToolTip(wx.ToolTip(_("Saisissez une traduction")))
    ...
    def OnBoutonOk(self, event): 
        ...
        for balise in balises :
            ...
            if nbreOriginal != nbreTraduction :
                ...
                detail = "%d fois dans le texte original et %d fois dans le texte traduit" % (nbreOriginal, nbreTraduction)
                differences.append(u"%s  -> %s" % (balise, detail))
                
        if len(differences) > 0 :
            dlg = DLG_Messagebox.Dialog(self, titre=_("Avertissement"), introduction=_("Attention, les éléments suivants du texte original n'apparaissent pas\ndans la traduction : "), detail="\n".join(differences), conclusion=_("Il est très fortement conseillé de les insérer dans la traduction !"), icone=wx.ICON_EXCLAMATION, boutons=[_("Valider quand même"), _("Annuler")])
