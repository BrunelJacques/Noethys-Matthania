...
TEXTE_INTRO = _("Vous pouvez saisir ici les paramètres d'un contrat P.S.U.")
...
class Base(object) :
    ...
    def InitPages(self, parent=None):
        ...
        self.listePages = [
            {"code" : "generalites", "label" : _("Généralités"), "ctrl" : DLG_Contratpsu_generalites.Panel(parent, clsbase=self), "image" : "Maison.png"},
            {"code" : "previsions", "label" : _("Prévisions"), "ctrl" : DLG_Contratpsu_calendrier.Panel(parent, clsbase=self), "image" : "Calendrier.png"},
            {"code" : "tarification", "label" : _("Tarification"), "ctrl" : DLG_Contratpsu_tarification.Panel(parent, clsbase=self), "image" : "Calculatrice.png"},
            {"code" : "mensualisation", "label" : _("Mensualisation"), "ctrl" : DLG_Contratpsu_mensualisation.Panel(parent, clsbase=self), "image" : "Euro.png"},
            {"code" : "recapitulatif", "label" : _("Récapitulatif"), "ctrl" : DLG_Contratpsu_recapitulatif.Panel(parent, clsbase=self), "image" : "Information.png"},
            ]
    ...
    def Calculer(self, mode_test=False, dict_valeurs={}):
        ...
        #             present = True
        #     if present == False :
        #         listeAnomalies.append(_("La prestation du %s ne peut pas être supprimée car elle apparaît déjà sur la facture n°%s." % (UTILS_Dates.DateDDEnFr(dictPrestation["date_facturation"]), dictPrestation["num_facture"])))
        ...
        if nbreConsoHorsPeriode > 0 :
            listeAnomalies.append(_("%d consommations prévues sont en dehors de la période du contrat." % nbreConsoHorsPeriode))

        # Si mode test, renvoie la liste des anomalies
        if mode_test == True :
            if len(listeAnomalies) > 0 :
                from Dlg import DLG_Messagebox
                introduction =_("Votre saisie ne peut pas être validée en raison des erreurs suivantes :")
                detail = "\n".join(listeAnomalies)
                conclusion =_("Veuillez modifier les données saisies.")
                dlg = DLG_Messagebox.Dialog(None, titre=_("Erreur"), introduction=introduction, detail=detail, conclusion=conclusion, icone=wx.ICON_INFORMATION, boutons=[_("Ok"),])
                dlg.ShowModal()
                dlg.Destroy()
                return False
            else :
        ...
        if self.IDcontrat != None :
            ...
            if len(listeDonnees) > 0 :
                ...
                if individu_prenom != None :
                    dictValeurs["individu_nom_complet"] = "%s %s" %(individu_nom, individu_prenom)
                else :
...
class Assistant(wx.Dialog, Base):
    def __init__(self, parent, IDcontrat=None, IDinscription=None):
        ...
        titre = _("Création d'un contrat PSU")
        self.SetTitle(titre)
    ...
    def Annuler(self):
        ...
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment abandonner la saisie de ce nouveau contrat ?\n\nLes éventuelles données saisies seront perdues."), _("Annulation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDcontrat=None, IDinscription=None):
        ...
        titre = _("Modification d'un contrat PSU")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=TEXTE_INTRO, hauteurHtml=30, nomImage="Images/32x32/Contrat.png")
    ...
    def OnBoutonAnnuler(self, event=None):
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment annuler ?\n\nLes éventuelles modifications que vous avez effectué seront perdues."), _("Annulation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        reponse = dlg.ShowModal()
        dlg.Destroy()
        if reponse != wx.ID_YES :
            return
