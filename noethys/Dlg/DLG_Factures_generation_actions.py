...
class Panel(wx.Panel):
    def __init__(self, parent):
        ...
        # Actions
        self.box_actions_staticbox = wx.StaticBox(self, -1, _("Autres actions possibles"))
        ...
        self.bouton_imprimer = CTRL_Bouton_image.CTRL(self, texte=_("Imprimer"), tailleImage=(32, 32), margesImage=(4, 0, 0, 0), margesTexte=(-5, 1), cheminImage="Images/32x32/Imprimante.png")
    ...
    def __set_properties(self):
        ...
        self.bouton_email_facture.SetToolTip(wx.ToolTip(_("Cliquez ici envoyer la facture sélectionnée par Email")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer la facture sélectionnée ou les factures cochées")))
        self.bouton_apercu_liste.SetToolTip(wx.ToolTip(_("Cliquez ici pour afficher un aperçu avant impression de la liste")))
        self.bouton_imprimer_liste.SetToolTip(wx.ToolTip(_("Cliquez ici pour imprimer la liste")))
        self.bouton_export_texte.SetToolTip(wx.ToolTip(_("Cliquez ici pour exporter la liste au format texte")))
        self.bouton_export_excel.SetToolTip(wx.ToolTip(_("Cliquez ici pour exporter la liste au format Excel")))
        self.bouton_prelevements.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à la gestion des prélèvements automatiques")))
        self.bouton_email.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à l'envoi des factures par Email")))
        self.bouton_helios.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à l'export HELIOS des factures")))
        self.bouton_imprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour imprimer les factures générées")))
    ...
    def OnBoutonPrelevements(self, event): 
        ...
        dlg = wx.MessageDialog(self, _("Souhaitez-vous utiliser l'assistant de préparation des lots de prélèvements automatiques (Conseillé) ?\n\nSinon, le gestionnaire des prélèvements sera simplement ouvert."), _("Prélèvement automatique"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
    ...
    def OnBoutonHelios(self, event):
        ...
        if choix == "pes" :
            ...
            # Demande d'application automatique de filtres
            dlg = wx.MessageDialog(self, _("Souhaitez-vous utiliser l'assistant de préparation des bordereaux (Conseillé) ?\n\nSinon, le gestionnaire des bordereaux sera simplement ouvert."), _("Trésor public"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
            reponse = dlg.ShowModal() 
            dlg.Destroy()
            if reponse == wx.ID_CANCEL :
                return
    ...
    def OnBoutonEmail(self, event): 
        ...
        if filtre_prefixe != None :
            filtres.append(filtre_prefixe)
        # Demande d'application automatique de filtres
        dlg = wx.MessageDialog(self, _("Souhaitez-vous que Noethys sélectionne automatiquement les factures dont les familles souhaitent recevoir leurs factures par Email ?\n\n(Si non, notez que vous pouvez toujours effectuer cette sélection ultérieurement avec les filtres de sélection)"), _("Application automatique de filtres"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        reponse = dlg.ShowModal() 
        dlg.Destroy()
        if reponse == wx.ID_CANCEL :
            return
    ...
    def OnBoutonImprimer(self, event): 
        ...
        if filtre_prefixe != None :
            filtres.append(filtre_prefixe)
        # Demande d'application automatique de filtres
        dlg = wx.MessageDialog(self, _("Souhaitez-vous que Noethys sélectionne automatiquement les factures dont les familles ne souhaitent pas recevoir leurs factures par Email ?\n\n(Si non, notez que vous pouvez toujours effectuer cette sélection ultérieurement avec les filtres de sélection)"), _("Application automatique de filtres"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        reponse = dlg.ShowModal() 
        dlg.Destroy()
        if reponse == wx.ID_CANCEL :
            return
    ...
    def MAJ(self):
        if len(self.parent.listeFacturesGenerees) > 0 :
            ...
            self.box_factures_staticbox.SetLabel(_("%d factures générées") % len(self.parent.listeFacturesGenerees))
