...
class CTRL_Choix_unites(HTL.HyperTreeList):
    def __init__(self, parent, IDindividu=None): 
        ...
        self.SetToolTip(wx.ToolTip(_("Cochez les unités conditionnelles")))
...
class CTRL_Saisie_prog(wx.Panel):
    def __init__(self, parent, IDtransport=None, IDindividu=None):
        ...
        # Validité
        self.box_validite_staticbox = wx.StaticBox(self, -1, _("Validité"))
        self.label_date_debut = wx.StaticText(self, -1,u"Du :")
        self.ctrl_date_debut = CTRL_Saisie_date.Date2(self)
        self.label_date_fin = wx.StaticText(self, -1,_("Au :"))
        self.ctrl_date_fin = CTRL_Saisie_date.Date2(self)
        self.label_actif = wx.StaticText(self, -1,_("Actif :"))
        self.ctrl_actif = wx.CheckBox(self, -1, "")
        self.ctrl_actif.SetValue(True) 
        
        # Conditions
        self.box_conditions_staticbox = wx.StaticBox(self, -1, _("Conditions"))

        # Périodes scolaires
        self.label_periodes_scolaires = wx.StaticText(self, -1,_("Périodes scolaires :"))
        ...
        self.label_periodes_vacances = wx.StaticText(self, -1,_("Périodes de vacances :"))
        ...
        # Unités
        self.label_unites = wx.StaticText(self, -1,_("Unités de consommation :"))
    ...
    def Validation(self):
        ...
        if dictData["date_debut"] == None or self.ctrl_date_debut.Validation() == False :
                dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de début de période !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
        ...
        if dictData["date_fin"] == None or self.ctrl_date_fin.Validation() == False :
                dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de fin de période !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_date_fin.SetFocus()
                return False
        if len(dictData["jours_scolaires"]) == 0 and len(dictData["jours_vacances"]) == 0 :
                dlg = wx.MessageDialog(self, _("Vous devez cocher au moins un jour de la semaine !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        if len(dictData["unites"]) == 0 :
                dlg = wx.MessageDialog(self, _("Vous devez cocher au moins une unité de consommation !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        return True
...
class CTRL_Mode_multiple(wx.Choicebook):
    def __init__(self, parent):
        ...
        self.listePanels = [
            ("CALENDRIER", _("Selon les dates sélectionnées"), CTRL_Calendrier.CTRL(self, afficheBoutonAnnuel=False) ),
            ("PLANNING", _("Selon le planning suivant"), CTRL_Planning(self) ),
            ]
    ...
    def Validation(self):
        ...
        if dictData["mode"] == "CALENDRIER" :
            if dictData["dates"] == None or len(dictData["dates"]) == 0 :
                dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune date dans le calendrier !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
...
class CTRL_Choix_activite(wx.Choice):
    def __init__(self, parent, IDindividu=None):
        ...
        self.IDindividu = IDindividu
        self.MAJ() 
        self.SetToolTip(wx.ToolTip(_("Sélectionnez ici une activité")))
...
class CTRL_Saisie_multiple(wx.Panel):
    def __init__(self, parent, IDindividu=None):
        ...
        # Mode
        self.box_mode_staticbox = wx.StaticBox(self, -1, _("Mode de saisie"))
        self.radio_unique = wx.RadioButton(self, -1, _("Saisie unique"), style=wx.RB_GROUP)
        self.radio_multiple = wx.RadioButton(self, -1, _("Saisie multiple"))
        
        # Saisie multiple
        self.box_multiple_staticbox = wx.StaticBox(self, -1, _("Saisie multiple"))
        self.ctrl_multiple = CTRL_Mode_multiple(self)
        
        # Options
        self.box_options_staticbox = wx.StaticBox(self, -1, _("Options"))
        self.check_feries = wx.CheckBox(self, -1, _("Inclure les jours fériés"))
        self.check_activite = wx.CheckBox(self, -1, _("Uniquement si individu inscrit sur l'activité :"))
    ...
    def __set_properties(self):
        self.radio_unique.SetToolTip(wx.ToolTip(_("Sélectionnez ce mode pour saisir un seul transport")))
        self.radio_multiple.SetToolTip(wx.ToolTip(_("Sélectionnez ce mode pour saisir plusieurs transports à la fois")))
        self.check_feries.SetToolTip(wx.ToolTip(_("Cochez cette case pour saisir également des transports sur les jours fériés")))
        self.check_activite.SetToolTip(wx.ToolTip(_("Cochez cette case pour saisir ce transport uniquement les jours sur lesquels l'individu est inscrit sur une activité donnée")))
...
class Dialog_prog(wx.Dialog):
    def __init__(self, parent, IDtransport=None, IDindividu=None):
        ...
        # Bandeau
        titre = _("Programmation d'un transport")
        intro = _("Vous pouvez ici programmer un transport. Celui-ci sera ainsi créé automatiquement par Noethys à chaque fois qu'une consommation sera saisie pour cet individu en fonction des paramètres que vous aurez précisé ci-dessous.")
        self.SetTitle(titre)
...
class Dialog_multiple(wx.Dialog):
    def __init__(self, parent, IDindividu=None):
        ...
        # Bandeau
        titre = _("Saisie d'un ou plusieurs transports")
        intro = _("Vous pouvez ici sélectionner toutes les caractéristiques du ou des transports à créer. Pour saisir un lot de transports identiques sur un ensemble de dates, sélectionnez le mode Saisie multiple.")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Transport.png")
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDtransport=None, IDindividu=None, modeVirtuel=False, dictDonnees={}, verrouilleBoutons=False):
        ...
        # Bandeau
        if IDtransport == None :
            titre = _("Saisie d'un transport")
        else:
            titre = _("Modification d'un transport")
        intro = _("Vous pouvez ici sélectionner toutes les caractéristiques du transport.")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Transport.png")
