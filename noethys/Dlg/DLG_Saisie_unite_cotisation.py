...
class Dialog(wx.Dialog):
    def __init__(self, parent, nomTypeCotisation=u"", dictDonnees={}):
        ...
        self.radio_periode = wx.RadioButton(self, -1, _("La période suivante : "), style=wx.RB_GROUP)
        ...
        self.radio_duree = wx.RadioButton(self, -1, _("La durée suivante : "))
        ...
        # Prestation
        self.staticbox_prestation_staticbox = wx.StaticBox(self, -1, _("Prestation"))
        self.label_montant = wx.StaticText(self, -1, _("Montant :"))
        self.ctrl_montant = CTRL_Saisie_euros.CTRL(self)
        self.label_label_prestation = wx.StaticText(self, -1, _("Label de la\nprestation :"), style=wx.ALIGN_RIGHT)
        self.radio_label_defaut = wx.RadioButton(self, -1, _("Par défaut :"), style=wx.RB_GROUP)
        self.ctrl_label_defaut = wx.StaticText(self, -1, "")
        self.radio_label_personnalise = wx.RadioButton(self, -1, _("Personnalisé :"))
        self.ctrl_label_personnalise = wx.TextCtrl(self, -1, "")
        ...
        if len(dictDonnees) == 0 :
            self.SetTitle(_("Saisie d'une nouvelle unité de cotisation"))
            self.SetLabelPrestationDefaut() 
        else:
            self.SetTitle(_("Modification d'une unité de cotisation"))
            self.Importation()
    ...
    def __set_properties(self):
        ...
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez ici le nom de l'unité")))
        ...
        self.ctrl_montant.SetToolTip(wx.ToolTip(_("Saisissez ici le montant en euros de l'unité de cotisation")))
        self.radio_label_defaut.SetToolTip(wx.ToolTip(_("Cochez cette case pour utiliser le label de prestation par défaut")))
        self.radio_label_personnalise.SetToolTip(wx.ToolTip(_("Cochez cette case pour utiliser un label de prestation personnalisé")))
        self.ctrl_label_personnalise.SetToolTip(wx.ToolTip(_("Saisissez ici un label personnalisé")))
    ...
    def OnBoutonOk(self, event):         
        ...
        if nom == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un nom pour cette unité de cotisation !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.radio_periode.GetValue() == True :
            ...
            if self.ctrl_date_debut.FonctionValiderDate() == False or date_debut == None:
                dlg = wx.MessageDialog(self, _("La date de début de validité n'est pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
            ...
            if self.ctrl_date_fin.FonctionValiderDate() == False or date_fin == None:
                dlg = wx.MessageDialog(self, _("La date de fin de validité n'est pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
            ...
            if date_debut > date_fin:
                dlg = wx.MessageDialog(self, _("La date de début de validité est supérieure à la date de fin !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        ...
        if self.radio_duree.GetValue() == True :
            ...
            if jours == 0 and mois == 0 and annees == 0 :
                dlg = wx.MessageDialog(self, _("Vous devez donc saisir un nombre de jours et/ou de mois et/ou d'années."), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
    ...
    def SetLabelPrestationDefaut(self):
        if self.radio_label_defaut.GetValue() == True :
            label = "'%s - %s'" % (self.nomTypeCotisation, self.ctrl_nom.GetValue())
