...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDmode=None):
        ...
        self.ctrl_type_comptable= wx.Choice(self, -1, choices=[_("Regroupé par dépôt lors du transfert"),
        ...
        self.radio_numero_numerique = wx.RadioButton(self, -1, _("Numérique"))
        self.ctrl_check_caract = wx.CheckBox(self, -1, _("Nbre caractères max :"))
...
##        self.Bind(wx.EVT_BUTTON, self.OnSupprimerEmetteur, self.bouton_supprimer_emetteur)
        ...
        if self.IDmode != None :
            self.Importation()
            self.SetTitle(_("Modification d'un mode de règlement"))
        else:
            self.SetTitle(_("Création d'un mode de règlement"))
    ...
    def __set_properties(self):
        ...
        self.ctrl_label.SetToolTip(wx.ToolTip(_("Saisissez ici le nom du mode de règlement")))
        self.bouton_ajouter_image.SetToolTip(wx.ToolTip(_("Cliquez ici pour importer une image pour ce mode de règlement")))
        self.bouton_supprimer_image.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer l'image active")))
        self.radio_numero_aucun.SetToolTip(wx.ToolTip(_("Cochez ici si le règlement de nécessite aucun numéro de pièce")))
        self.radio_numero_alpha.SetToolTip(wx.ToolTip(_("Cochez ici si le numéro peut contenir des lettres et des chiffres")))
        self.radio_numero_numerique.SetToolTip(wx.ToolTip(_("Cochez ici si le numéro ne peut contenir que des chiffres")))
        self.ctrl_check_caract.SetToolTip(wx.ToolTip(_("Cochez cette case si le numéro a un nombre maximal de chiffres")))
        self.ctrl_nbre_caract.SetToolTip(wx.ToolTip(_("Saisissez ici le nombre de chiffres du numéro")))
        self.radio_frais_aucun.SetToolTip(wx.ToolTip(_("Cochez ici si aucun frais de gestion n'est applicable pour ce mode de règlement")))
        ...
        self.ctrl_frais_prorata.SetToolTip(wx.ToolTip(_("Saisissez ici le pourcentage du montant du règlement")))
        self.ctrl_frais_arrondi.SetToolTip(wx.ToolTip(_("Selectionnez une méthode de calcul de l'arrondi")))
        self.ctrl_frais_label.SetToolTip(wx.ToolTip(_("Vous avez ici la possibilité de modifier le label de la prestation qui sera créée pour les frais de gestion")))
    ...
    def OnBoutonOk(self, event):
        ...
        if self.radio_numero_numerique.GetValue() == True :
            ...
            if self.ctrl_check_caract.GetValue() == True :
                ...
                if nbre_chiffres == 0 :
                    dlg = wx.MessageDialog(self, _("Vous avez sélectionné l'option 'Nbre limité de caractères' sans saisir de chiffre !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                    dlg.ShowModal()
                    dlg.Destroy()
        ...
        if self.ctrl_type_comptable.GetSelection() == 0 :
            ...
            #détail par règlement avec date du dépôt
