    ...
    for filtre in filtres :
        ...
        # Type de cotisation
         if filtre["type"] == "type" :
            ...
            if len(listeDonnees) > 0 :
                listeTextes.append(_("Type de cotisation '%s'") % listeDonnees[0][1])
        ...
        # Unité de cotisation
         if filtre["type"] == "unite" :
            ...
            DB.Close()
            if len(listeDonnees) > 0 :
                listeTextes.append(_("Unité de cotisation '%s'") % listeDonnees[0][1])

##        # Date d'échéance
##         if filtre["type"] == "date_echeance" :
##            listeTextes.append(_("Date d'échéance de %s à %s") % (UTILS_Dates.DateEngFr(str(filtre["date_min"])), UTILS_Dates.DateEngFr(str(filtre["date_max"]))))

        # Numéros Intervalle
         if filtre["type"] == "numero_intervalle" :
            listeTextes.append(_("Numéros de cotisations de %d à %d") % (filtre["numero_min"], filtre["numero_max"]))

        # Numéros Liste
         if filtre["type"] == "numero_liste" :
            listeTextes.append(_("Numéros de cotisations suivants : %s") % ";".join([str(x) for x in filtre["liste"]]))
        ...
        # Montant
         if filtre["type"] == "montant" :
            ...
            if operateur == "<" : operateur = "&#60;"
            if operateur == ">" : operateur = "&#62;"
            listeTextes.append(_("Montant facturé %s %.2f %s") % (operateur, filtre["montant"], SYMBOLE))
        ...
        # Solde actuel
         if filtre["type"] == "solde_actuel" :
            ...
            if operateur == "<" : operateur = "&#60;"
            if operateur == ">" : operateur = "&#62;"
            listeTextes.append(_("Montant réglé %s %.2f %s") % (operateur, filtre["montant"], SYMBOLE))

        # Carte
         if filtre["type"] == "carte" :
            if filtre["choix"] == True :
                listeTextes.append(_("Cotisations pour lesquelles une carte a été créée"))
            else :
                listeTextes.append(_("Cotisations pour lesquelles aucune carte n'a été créée"))

        # Facturée
         if filtre["type"] == "facturee" :
            if filtre["choix"] == True :
                listeTextes.append(_("Cotisations facturées"))
            else :
                listeTextes.append(_("Cotisations non facturées"))
        ...
        # Dépôt
         if filtre["type"] == "depot" :
            ...
            DB.Close()
            if len(listeDonnees) > 0 :
                listeTextes.append(_("Dépôt de cotisations '%s'") % listeDonnees[0][1])
...
class CTRL_Types_cotisations(wx.Choice):
    ...
    def MAJ(self):
        ...
        listeDonnees = DB.ResultatReq()
        DB.Close()
        listeItems = [_("Aucun type de cotisation"),]
        self.dictDonnees = {}
        self.dictDonnees[0] = { "ID" : 0, "nom" : _("Inconnu")}
        index = 1
...
class CTRL_Unites_cotisations(wx.Choice):
    ...
    def MAJ(self):
        ...
        listeDonnees = DB.ResultatReq()
        DB.Close()
        listeItems = [_("Aucune unité de cotisation"),]
        self.dictDonnees = {}
        self.dictDonnees[0] = { "ID" : 0, "nom" : _("Inconnu")}
        index = 1
...
class CTRL_Depots_cotisations(wx.Choice):
    ...
    def MAJ(self):
        ...
        listeDonnees = DB.ResultatReq()
        DB.Close()
        listeItems = [_("Aucun dépôt de cotisations"),]
        self.dictDonnees = {}
        self.dictDonnees[0] = { "ID" : 0, "nom" : _("Inconnu")}
        index = 1
        for ID, nom in listeDonnees :
...
class CTRL_Filtres(wx.Panel):
    def __init__(self, parent, filtres=[], ctrl_cotisations=None):
        ...
        self.ctrl_html = MyHtml(self, texte=_("Aucun filtre de sélection."), couleurFond=couleurFond, hauteur=25)
        ...
        self.bouton_parametres.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier les filtres de sélection des cotisations")))
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        # Bandeau
        intro = _("Sélectionnez ici les filtres de sélection de votre choix à appliquer sur la liste des cotisations")
        titre = _("Filtres de sélection des cotisations")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Filtre.png")
        
        # Filtres
        self.check_type = wx.CheckBox(self, -1, _("Type de cotisation :"))
        self.check_type.SetToolTip(wx.ToolTip(_("Filtre type de cotisation")))
        self.ctrl_type = CTRL_Types_cotisations(self)
        self.ctrl_type.SetToolTip(wx.ToolTip(_("Sélectionnez un type de cotisation dans la liste")))
        self.Bind(wx.EVT_CHECKBOX, self.OnCheck, self.check_type)

        self.check_unite = wx.CheckBox(self, -1, _("Unité de cotisation :"))
        self.check_unite.SetToolTip(wx.ToolTip(_("Filtre unité de cotisation")))
        self.ctrl_unite = CTRL_Unites_cotisations(self)
        self.ctrl_unite.SetToolTip(wx.ToolTip(_("Sélectionnez une unité de cotisation dans la liste")))
        self.Bind(wx.EVT_CHECKBOX, self.OnCheck, self.check_unite)
        
##        self.check_echeance = wx.CheckBox(self, -1, _("Date d'échéance de"))
...
##        self.ctrl_echeance_max = CTRL_Saisie_date.Date2(self)
        ...
        self.check_numeros_intervalle = wx.CheckBox(self, -1, _("Numéros de cotisations de"))
        self.check_numeros_intervalle.SetToolTip(wx.ToolTip(_("Filtre Intervalle de numéros de cotisation")))
        self.ctrl_numeros_intervalle_min = wx.SpinCtrl(self, -1, "", min=0, max=1000000)
        self.ctrl_numeros_intervalle_min.SetMinSize((70, -1))
        self.label_numeros_intervalle_a = wx.StaticText(self, -1, "à")
        self.ctrl_numeros_intervalle_max = wx.SpinCtrl(self, -1, "", min=0, max=1000000)
        self.ctrl_numeros_intervalle_max.SetMinSize((70, -1))
        self.ctrl_numeros_intervalle_min.SetToolTip(wx.ToolTip(_("Saisissez un numéro de cotisation min")))
        self.ctrl_numeros_intervalle_max.SetToolTip(wx.ToolTip(_("Saisissez un numéro de cotisation max")))
        self.Bind(wx.EVT_CHECKBOX, self.OnCheck, self.check_numeros_intervalle)
        
        self.check_numeros_liste = wx.CheckBox(self, -1, _("Numéros de cotisations suivants :"))
        self.check_numeros_liste.SetToolTip(wx.ToolTip(_("Filtre Liste de numéros de cotisations")))
        self.ctrl_numeros_liste = wx.TextCtrl(self, -1, "")
        self.ctrl_numeros_liste.SetToolTip(wx.ToolTip(_("Saisissez les numéros de cotisations souhaités en les séparant par un point-virgule (;)")))
        self.Bind(wx.EVT_CHECKBOX, self.OnCheck, self.check_numeros_liste)
        
        listeOperateurs = (u"=", "<>", ">", "<", ">=", "<=")
        
        self.check_montant = wx.CheckBox(self, -1, _("Montant facturé"))
        self.check_montant.SetToolTip(wx.ToolTip(_("Filtre montant facturé")))
        self.ctrl_montant_operateur = wx.Choice(self, -1, choices=listeOperateurs)
        self.ctrl_montant_operateur.SetSelection(0)
        self.ctrl_montant_operateur.SetToolTip(wx.ToolTip(_("Sélectionnez un opérateur de comparaison")))
        self.ctrl_montant_montant = CTRL_Saisie_euros.CTRL(self)
        self.ctrl_montant_montant.SetToolTip(wx.ToolTip(_("Saisissez un montant")))
        ...
        self.ctrl_solde_actuel_operateur.SetToolTip(wx.ToolTip(_("Sélectionnez un opérateur de comparaison")))
        ...
        self.check_carte = wx.CheckBox(self, -1, _("Carte créée"))
        self.check_carte.SetToolTip(wx.ToolTip(_("Filtre carte créée")))
        self.ctrl_carte = wx.Choice(self, -1, choices=["Oui", _("Non")])
        self.ctrl_carte.SetSelection(0)
        self.ctrl_carte.SetToolTip(wx.ToolTip(_("Oui/Non")))
        self.Bind(wx.EVT_CHECKBOX, self.OnCheck, self.check_carte)
        
        self.check_facturee = wx.CheckBox(self, -1, _("Cotisation facturée"))
        self.check_facturee.SetToolTip(wx.ToolTip(_("Filtre cotisation facturée")))
        self.ctrl_facturee = wx.Choice(self, -1, choices=["Oui", _("Non")])
        self.ctrl_facturee.SetSelection(0)
        self.ctrl_facturee.SetToolTip(wx.ToolTip(_("Oui/Non")))
        self.Bind(wx.EVT_CHECKBOX, self.OnCheck, self.check_facturee)
        
        self.check_depot = wx.CheckBox(self, -1, _("Dépôt de cotisations :"))
        self.check_depot.SetToolTip(wx.ToolTip(_("Filtre dépôt de cotisations")))
        self.ctrl_depot = CTRL_Depots_cotisations(self)
        self.ctrl_depot.SetToolTip(wx.ToolTip(_("Sélectionnez un dépôt de cotisations dans la liste")))
    ...
    def GetFiltres(self):
        ...
        if self.check_type.GetValue() == True :
            ...
            if IDtype_cotisation == None :
                dlg = wx.MessageDialog(self, _("Filtre Type de cotisation : Vous n'avez sélectionné aucun type dans la liste proposée !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        ...
        if self.check_unite.GetValue() == True :
            ...
            if IDunite_cotisation == None :
                dlg = wx.MessageDialog(self, _("Filtre Unité de cotisation : Vous n'avez sélectionné aucune unité dans la liste proposée !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
...
##                dlg = wx.MessageDialog(self, _("Filtre Date d'échéance : Les dates saisies ne sont pas valides !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
##                dlg.ShowModal()
##                dlg.Destroy()
##                return False
##        
##            filtres.append({"type" : "date_echeance", "date_min" : date_min, "date_max" : date_max})
        ...
        if self.check_numeros_liste.GetValue() == True :
            ...
            if listeTemp == "" :
                dlg = wx.MessageDialog(self, _("Filtre Liste de numéros : Vous n'avez saisi aucun numéro de facture dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
            try :
                ...
                dlg = wx.MessageDialog(self, _("Filtre Liste de numéros : Les numéros de factures saisis ne sont pas valides !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        ...
        if self.check_montant.GetValue() == True :
            ...
            if montant == None :
                dlg = wx.MessageDialog(self, _("Filtre Montant facturé : Le montant saisi n'est pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        ...
        if self.check_solde_actuel.GetValue() == True :
            ...
            if montant == None :
                dlg = wx.MessageDialog(self, _("Filtre Solde actuel : Le montant saisi n'est pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        ...
        if self.check_depot.GetValue() == True :
            ...
            if IDdepot_cotisation == None :
                dlg = wx.MessageDialog(self, _("Filtre Dépôt de cotisations : Vous n'avez sélectionné aucun dépôt dans la liste proposée !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
