...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDtexte=None):
        ...
        # Attribution
        self.ctrl_attribution = wx.CheckBox(self, -1, "")
        self.label_attribution_1 = wx.StaticText(self, -1, _("Attribuer par défaut lorsque le nombre de jours de retard est entre"))
        self.ctrl_retard_min = wx.SpinCtrl(self, -1, "", min=0, max=9000)
        self.label_attribution_2 = wx.StaticText(self, -1, _("et"))
        self.ctrl_retard_max = wx.SpinCtrl(self, -1, "", min=0, max=9000)
        ...
        if self.IDtexte != None :
            self.Importation()
            self.SetTitle(_("Modification d'un texte de rappel"))
        else:
            self.SetTitle(_("Saisie d'un texte de rappel"))
        
        # Init contrôles
        self.OnCheckAttribution(None)

    def __set_properties(self):
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez un nom pour ce texte")))
        self.ctrl_titre.SetToolTip(wx.ToolTip(_("Saisissez le titre qui apparaîtra en entête du document")))
        self.ctrl_couleur.SetToolTip(wx.ToolTip(_("Sélectionnez une couleur")))
        self.ctrl_attribution.SetToolTip(wx.ToolTip(_("Cochez cette case pour que ce texte soit automatiquement attribué en fonction du nombre de jours de retard du paiement")))
        self.ctrl_retard_min.SetMinSize((60, -1))
        self.ctrl_retard_max.SetMinSize((60, -1))
        self.ctrl_motscles.SetToolTip(wx.ToolTip(_("Double-cliquez sur un mot-clé pour l'insérer dans le texte\nou recopiez-le directement (avec ses accolades).")))
...
##        source = re.sub(r'face=".*?"', "", source)
    ...
    def OnBoutonOk(self, event):     
        ...
        if label == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un nom !"), "Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.couleur == (255, 255, 255):
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner une couleur en cliquant sur le bouton couleur !"), "Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if attribution == True :
            ...
            if retard_min >= retard_max :
                dlg = wx.MessageDialog(self, _("Les conditions d'attribution par défaut semblent erronées !"), "Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return   
        else:
        ...
        if titre == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un titre !"), "Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
