...
class CTRL_Options(wx.CheckListBox):
    def __init__(self, parent):
        ...
        self.dictIndex = {}
        self.listeOptions = [
            ("abonnements", _("Listes de diffusion et abonnements")),
            ("scolarite", _("Données de scolarité : étapes, écoles, classes...")),
            ("questionnaire", _("Questionnaires familiaux et individuels")),
            ("pieces", _("Pièces et types de pièces")),
            ("messages", _("Messages de type famille, individuel ou accueil")),
            ("quotients", _("Quotients familiaux des familles")),
            ("mandats", _("Mandats SEPA des familles")),
            ]
        self.MAJ() 
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        # Bandeau
        titre = _("Importer des familles depuis un fichier Noethys")
        ...
        # Mode
        self.box_mode_staticbox = wx.StaticBox(self, -1, _("Mode"))
        self.radio_local = wx.RadioButton(self, -1, _("Local"), style=wx.RB_GROUP)
        self.radio_reseau = wx.RadioButton(self, -1, _("Réseau"))
        
        # Codes d'accès
        self.box_codes_staticbox = wx.StaticBox(self, -1, _("Codes d'accès réseau"))
        self.label_port = wx.StaticText(self, -1, _("Port :"))
        self.ctrl_port = wx.TextCtrl(self, -1, "3306", style=wx.TE_CENTRE)
        self.label_hote = wx.StaticText(self, -1, _("Hôte :"))
        ...
        self.label_motdepasse = wx.StaticText(self, -1, _("Mot de passe :"))
        self.ctrl_motdepasse = wx.TextCtrl(self, -1, "", style=wx.TE_PASSWORD)
        self.bouton_valider_codes = wx.Button(self, -1, _("Valider"), style=wx.BU_EXACTFIT)
        
        # Liste fichiers
        self.box_fichiers_staticbox = wx.StaticBox(self, -1, _("1. Sélection du fichier"))
        self.ctrl_fichiers = CTRL_Liste_fichiers.CTRL(self, mode="local")
        self.ctrl_fichiers.SetMinSize((-1, 200))

        # Options
        self.box_options_staticbox = wx.StaticBox(self, -1, _("2. Sélection des données optionnelles"))
    ...
    def OnBoutonOk(self, event): 
        ...
        if nbreFamilles > 0 :
            dlg = wx.MessageDialog(self, _("Attention, %d fiches familles sont déjà saisies dans ce fichier. Si vous utilisez la fonction d'importation, TOUTES les fiches actuelles seront écrasées !!!\n\nSouhaitez-vous tout de même continuer ?") % nbreFamilles, _("Avertissement"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
            if dlg.ShowModal() != wx.ID_YES :
                dlg.Destroy()
                return False
            dlg.Destroy()
        
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment lancer l'importation des familles du fichier '%s' ?\n\nAttention, toutes les données actuelles seront écrasées !") % titre, _("Confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
        if dlg.ShowModal() != wx.ID_YES :
            dlg.Destroy()
            return False
        dlg.Destroy()
        ...
        # Fin
        nbreFamilles = self.GetNbreFamilles() 
        dlg = wx.MessageDialog(self, _("La procédure d'importation est terminée !\n\n%d familles ont été importées avec succès.") % nbreFamilles, _("Confirmation"), wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
