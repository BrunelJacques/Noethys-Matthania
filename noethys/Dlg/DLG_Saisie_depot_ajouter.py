...
class Dialog(wx.Dialog):
    ...
    def __set_properties(self):
        ...
        self.ctrl_compte.SetToolTip(wx.ToolTip(_("Sélectionnez un filtre de compte")))
        self.ctrl_mode.SetToolTip(wx.ToolTip(_("Sélectionnez un filtre de mode de règlement")))
        self.ctrl_tri.SetToolTip(wx.ToolTip(_("Sélectionnez le critère de tri")))
        self.ctrl_ordre.SetToolTip(wx.ToolTip(_("Sélectionnez l'ordre de tri")))
        self.bouton_bas_tout.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter tous les règlements dans le dépôt")))
        self.bouton_bas.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter le règlement disponible selectionné dans le dépôt")))
        ...
        self.bouton_haut.SetToolTip(wx.ToolTip(_("Cliquez ici pour retirer le règlement sélectionné du dépôt")))
        self.bouton_haut_tout.SetToolTip(wx.ToolTip(_("Cliquez ici pour retirer tous les règlements du dépôt")))
    ...
    def OnBoutonBasTout(self, event): 
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment ajouter tous les règlements ?"), _("Confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
    ...
    def OnBoutonHautTout(self, event):
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment retirer tous les règlements ?"), _("Confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
        reponse = dlg.ShowModal()
        dlg.Destroy()
        if reponse ==  wx.ID_YES :
    ...
    def OnBoutonOk(self, event): 
        ...
        for track in self.tracks :
            if track.inclus == True :
                ...
                label = _("Règlement ID%d du %s payé en %s par %s") % (track.IDreglement, DateEngFr(str(track.date)), track.nom_mode, track.nom_payeur)
                
                # Vérifie si pas de règlement en attente
                if track.encaissement_attente == True :
                    dlg = wx.MessageDialog(self, _("Vous avez sélectionné le règlement suivant alors qu'il a été saisi avec l'option 'Encaissement en attente' :\n\n> %s\n\nIl vous est donc impossible de l'inclure dans ce dépôt !") % label, _("Règlement en attente"), wx.OK | wx.ICON_ERROR)
                    dlg.ShowModal()
                    dlg.Destroy()
                    return False
        ...
        if len(tracks_differes) > 0 :
            ...
            for track in tracks_differes:
                detail.append(_("Règlement ID%d du %s payé en %s par %s") % (track.IDreglement, DateEngFr(str(track.date)), track.nom_mode, track.nom_payeur))
            introduction = _("Vous avez sélectionné des règlements alors qu'ils comportent une date d'encaissement différé supérieure à la date du jour :")
            conclusion = _("Souhaitez-vous tout de même les inclure dans ce dépôt ?")
