...
def Supprime_accent(texte):
    liste = [ (u"é", "e"), (u"è", "e"), (u"ê", "e"), (u"ë", "e"), (u"à", "a"), (u"û", "u"), (u"ô", "o"), (u"ç", "c"), (u"î", "i"), (u"ï", "i"),]
    for a, b in liste :
...
class CTRL_Parametres(CTRL_Propertygrid.CTRL):
    ...
    def Remplissage(self):
        # Généralités
        self.Append(wxpg.PropertyCategory(_("Généralités")))

        propriete = CTRL_Propertygrid.Propriete_choix(label=_("Format"), name="format", liste_choix=[("prive", _("Secteur privé")), ("public_dft", _("Secteur public DFT"))], valeur="prive")
        propriete.SetEditor("EditeurChoix")
        propriete.SetHelpString(_("Sélectionnez le format"))
        ...
        # Secteur public
        self.Append(wxpg.PropertyCategory(_("Secteur public")))

        propriete = wxpg.EnumProperty(label=_("Perception"), name="perception")
        propriete.SetHelpString(_("Sélectionnez une perception (uniquement pour le secteur public)"))
        propriete.SetEditor("EditeurComboBoxAvecBoutons")
        self.Append(propriete)
        self.MAJ_perceptions()

        propriete = wxpg.StringProperty(label=_("Motif du prélèvement"), name="motif", value=u"")
        propriete.SetHelpString(_("Saisissez le motif du prélèvement. Ex : 'Garderie Novembre 2019' (uniquement pour le secteur public)"))
        self.Append(propriete)

        propriete = wxpg.StringProperty(label=_("Identifiant de service"), name="identifiant_service", value=u"")
        propriete.SetHelpString(_("Saisissez l'identifiant de service (uniquement pour compte DFT). Exemple : TGDFT027 (numéro de département sur 3 chiffres)"))
        self.Append(propriete)

        propriete = wxpg.StringProperty(label=_("Codique DDFiP"), name="poste_comptable", value=u"")
        propriete.SetHelpString(_("Saisissez le codique de la DDFiP de rattachement sur 6 caractères (uniquement pour compte DFT). Exemple : 027000"))
        self.Append(propriete)

        propriete = wxpg.IntProperty(label=_("Numéro de séquence dans la journée"), name="numero_sequence", value=1)
        propriete.SetEditor("SpinCtrl")
        propriete.SetHelpString(_("Numéro de séquence du fichier dans une même journée. Exemple: 1 = premier fichier de la journée"))
        ...
        propriete.SetHelpString(_("Cochez cette case si vous souhaitez que Noethys enregistre un règlement dans Noethys pour chaque prélèvement valide"))
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDlot=None, typePrelevement="national"):
        ...
        # Généralités
        self.box_generalites_staticbox = wx.StaticBox(self, -1, _("Caractéristiques"))
        ...
        self.label_date = wx.StaticText(self, -1, _("Date du prélèv. :"))
        ...
        # Prélèvements
        self.box_prelevements_staticbox = wx.StaticBox(self, -1, _("Prélèvements"))
        ...
        self.label_actions = wx.StaticText(self, -1, _("Actions sur prélèvements cochés :"))

        self.hyper_etat_attente = Hyperlien(self, label=_("Attente"), infobulle=_("Cliquez ici pour mettre en attente les prélèvements cochés"), URL="etat_attente")
        self.label_separation_1 = wx.StaticText(self, -1, "|")
        self.hyper_etat_valide = Hyperlien(self, label=_("Valide"), infobulle=_("Cliquez ici pour valider les prélèvements cochés"), URL="etat_valide")
        self.label_separation_2 = wx.StaticText(self, -1, "|")
        self.hyper_etat_refus = Hyperlien(self, label=_("Refus"), infobulle=_("Cliquez ici pour refuser les prélèvements cochés"), URL="etat_refus")

        self.hyper_reglements_tout = Hyperlien(self, label=_("Régler"), infobulle=_("Cliquez ici pour régler les prélèvements cochés"), URL="reglements_tout")
        self.label_separation_3 = wx.StaticText(self, -1, "|")
        self.hyper_reglements_rien = Hyperlien(self, label=_("Ne pas régler"), infobulle=_("Cliquez ici pour ne pas régler les prélèvements cochés"), URL="reglements_rien")


        self.hyper_selection_tout = Hyperlien(self, label=_("Tout cocher"), infobulle=_("Cliquez ici pour tout cocher la sélection"), URL="selection_tout")
        self.label_separation_4 = wx.StaticText(self, -1, "|")
        self.hyper_selection_rien = Hyperlien(self, label=_("Tout décocher"), infobulle=_("Cliquez ici pour tout décocher la sélection"), URL="selection_rien")
        ...
        self.bouton_fichier_national = CTRL_Bouton_image.CTRL(self, texte=_("Générer un fichier AFB-CFONB"), cheminImage="Images/32x32/Disk.png")
        self.bouton_fichier_SEPA = CTRL_Bouton_image.CTRL(self, texte=_("Générer un fichier SEPA"), cheminImage="Images/32x32/Disk.png")
    ...
    def __set_properties(self):
        if self.typePrelevement == "sepa" :
            self.SetTitle(_("Saisie d'un lot de prélèvements - format SEPA"))
        else :
            self.SetTitle(_("Saisie d'un lot de prélèvements - format National"))
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez un nom pour ce lot (Ex : 'Janvier 2013', etc...)")))
        self.ctrl_date.SetToolTip(wx.ToolTip(_("Saisissez la date prévue du prélèvement")))
        self.ctrl_verrouillage.SetToolTip(wx.ToolTip(_("Cochez cette case pour verrouiller le lot lorsqu'il a été communiqué à la banque")))
        self.ctrl_observations.SetToolTip(wx.ToolTip(_("Saisissez ici des observations sur ce lot de prélèvements")))
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter un prélèvement")))
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier le prélèvement sélectionné dans la liste")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour retirer le prélèvement sélectionné dans la liste")))
        self.bouton_apercu.SetToolTip(wx.ToolTip(_("Cliquez ici pour afficher un aperçu avant impression de la liste des prélèvements")))
        self.bouton_imprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour imprimer la liste des prélèvements de ce lot")))
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez ici pour obtenir de l'aide")))
        self.bouton_fichier_national.SetToolTip(wx.ToolTip(_("Cliquez ici pour générer un fichier normalisé AFB-CFONB destiné à votre banque")))
        self.bouton_fichier_SEPA.SetToolTip(wx.ToolTip(_("Cliquez ici pour générer un fichier normalisé SEPA destiné à votre banque")))
        self.bouton_ok.SetToolTip(wx.ToolTip(_("Cliquez ici pour valider les données")))
...
##        self.ctrl_prelevements.Saisie_factures()
        ...
        item = wx.MenuItem(menuPop, 20, _("Ajouter une ou plusieurs factures"))
        ...
        item = wx.MenuItem(menuPop, 10, _("Ajouter un prélèvement manuel"))
        item.SetBitmap(wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Ajouter.png"), wx.BITMAP_TYPE_PNG))
        menuPop.AppendItem(item)
    ...
    def OnBoutonFichierSEPA(self, event): 
        ...
        if len(self.ctrl_prelevements.GetObjects()) != len(tracks) :
            txtMessage = "Souhaitez-vous vraiment prendre uniquement compte les lignes filtrées (%d/%d lignes) ?\n\nSi vous répondez Non, toutes les lignes seront intégrées dans le fichier." % (len(tracks), len(self.ctrl_prelevements.GetObjects()))
            dlgConfirm = wx.MessageDialog(None, txtMessage, _("Confirmation"), wx.YES_NO|wx.CANCEL|wx.NO_DEFAULT|wx.ICON_QUESTION)
            reponse = dlgConfirm.ShowModal()
            dlgConfirm.Destroy()
        ...
        for sequence in listeSequences :
            ...
            for track in tracks :
                if sequence == track.sequence :
                    ...
                    if track.prelevement_reference_mandat in ("", None) : listeAnomalies.append(_("%s : Référence Unique du Mandat manquant") % track.titulaires)
                    if track.prelevement_date_mandat in ("", None) : listeAnomalies.append(_("%s : Date du Mandat manquante") % track.titulaires)
                    if track.prelevement_iban in ("", None) : listeAnomalies.append(_("%s : Numéro IBAN du compte bancaire manquant") % track.titulaires)
                    if track.prelevement_bic in ("", None) : listeAnomalies.append(_("%s : Numéro BIC du compte bancaire manquant") % track.titulaires)
                    if track.titulaire in ("", None) : listeAnomalies.append(_("%s : Nom du titulaire du compte bancaire manquant") % track.titulaires)
            ...
            if len(listeTransactions) > 0 :
                ...
                dictLot = {
                    "lot_nom" : "%s %s" % (remise_nom, sequence),
        ...
        if len(listeAnomalies) > 0 :
            intro = _("Le fichier SEPA ne peut être généré en raison des anomalies suivantes :")
            detail = "\n".join(listeAnomalies)
            dlg = DLG_Messagebox.Dialog(self, titre=_("Génération impossible"), introduction=intro, detail=detail, icone=wx.ICON_ERROR, boutons=[_("Ok"), ], defaut=0)
            dlg.ShowModal()
            dlg.Destroy() 
            return
        ...
        if valide != True :
            ...
            dlg = DLG_Messagebox.Dialog(self, titre=_("Validation XSD"), introduction=_("Les %d anomalies suivantes ont été détectées :") % len(liste_erreurs),
                                        detail=u"\n".join(liste_erreurs), conclusion=_("Le fichier ne semble pas valide. Souhaitez-vous continuer quand même ?"),
                                        icone=wx.ICON_EXCLAMATION, boutons=[_("Oui"), _("Non"), _("Annuler")])
        ...
        UTILS_Historique.InsertActions([{
            "IDcategorie" : 35, 
            "action" : _("Génération du fichier XML SEPA '%s'") % remise_nom,
            },])

        # Confirmation de création du fichier et demande d'ouverture directe
        txtMessage = _("Le fichier SEPA a été créé avec succès.\n\nSouhaitez-vous visualiser son contenu maintenant ?")
        dlgConfirm = wx.MessageDialog(None, txtMessage, _("Confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.ICON_QUESTION)
        reponse = dlgConfirm.ShowModal()
        dlgConfirm.Destroy()
        if reponse == wx.ID_NO:
    ...
    def GetLabelParametres(self):
        ...
        texte = _("Prélèvement : '%s' (%s)") % (nom, date)
        return texte
    ...
    def ValidationDonnees(self):
        ...
        for caract in nom :
            if caract in ("_",) :
                dlg = wx.MessageDialog(self, _("Le caractère '%s' n'est pas autorisé dans le nom du lot de prélèvements !") % caract, _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_nom.SetFocus() 
                return False
        ...
        DB.Close()
        if len(listeDonnees) > 0 :
            dlg = wx.MessageDialog(self, _("Ce nom de lot a déjà été attribué à un autre lot de prélèvements.\n\nChaque lot doit avoir un nom unique. Changez le nom."), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_nom.SetFocus() 
            return False
        ...
        if date == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de prélèvement !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if raison == "" or raison == None :
            dlg = wx.MessageDialog(self, _("Vous n'avez pas renseigné la raison sociale du compte créditeur !\n\nModifiez les paramètres du compte bancaire."), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if self.typePrelevement == "sepa" :
            ...
            if iban == "" or iban == None :
                dlg = wx.MessageDialog(self, _("Vous n'avez pas renseigné le IBAN du compte créditeur !\n\nModifiez les paramètres du compte bancaire."), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
            
            if bic == "" or bic == None :
                dlg = wx.MessageDialog(self, _("Vous n'avez pas renseigné le BIC du compte créditeur !\n\nModifiez les paramètres du compte bancaire."), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
            
            if code_ics == "" or code_ics == None :
                dlg = wx.MessageDialog(self, _("Vous n'avez pas renseigné le ICS du compte créditeur !\n\nModifiez les paramètres du compte bancaire."), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        ...
        if self.typePrelevement == "national" :
            ...
            if numero == "" or numero == None :
                dlg = wx.MessageDialog(self, _("Vous n'avez pas renseigné le numéro du compte créditeur !\n\nModifiez les paramètres du compte bancaire."), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False

            if code_etab == "" or code_etab == None :
                dlg = wx.MessageDialog(self, _("Vous n'avez pas renseigné le code Etablissement créditeur !\n\nModifiez les paramètres du compte bancaire."), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False

            if code_guichet == "" or code_guichet == None :
                dlg = wx.MessageDialog(self, _("Vous n'avez pas renseigné le code Guichet du compte créditeur !\n\nModifiez les paramètres du compte bancaire."), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False

            if code_nne == "" or code_nne == None :
                dlg = wx.MessageDialog(self, _("Vous n'avez pas renseigné le code NNE du compte créditeur !\n\nModifiez les paramètres du compte bancaire."), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        ...
        if type_remise == "public_dft":
            ...
            if not dft_titulaire:
                dlg = wx.MessageDialog(self, _("Vous n'avez pas renseigné le nom du titulaire du compte DFT !\n\nModifiez les paramètres du compte bancaire."), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
            ...
            if not parametres["motif"]:
                dlg = wx.MessageDialog(self, _("Vous n'avez pas renseigné le motif du prélèvement !\n\nModifiez les paramètres du lot."), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False

            if not parametres["perception"]:
                dlg = wx.MessageDialog(self, _("Vous n'avez pas sélectionné de perception.\n\nModifiez les paramètres du lot."), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False

            if not parametres["identifiant_service"] or parametres["identifiant_service"] == "":
                dlg = wx.MessageDialog(self, _("Vous devez renseigner l'identifiant de service dans le cadre Paramètres."), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False

            if not parametres["poste_comptable"] or len(parametres["poste_comptable"]) != 6:
                dlg = wx.MessageDialog(self, _("Vous devez saisir un codique DFiP valide (sur 6 caractères) dans le cadre Paramètres."), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
                ...
                if track.sequence in ("OOFF", "FRST") :
                    ...
                    if key in listeTemp1 :
                        if track.sequence == "OOFF" :
                            listeErreurs.append(_("- Prélèvement '%s' de %s : Le mandat '%s' de type ponctuel a déjà été utilisé une fois !") % (track.libelle, track.titulaires, track.prelevement_reference_mandat))
                        if track.sequence == "FRST" : 
                            listeErreurs.append(_("- Prélèvement '%s' de %s : Mandat '%s' déjà initialisé. La séquence doit être définie sur 'RCUR' !") % (track.libelle, track.titulaires, track.prelevement_reference_mandat))
                    listeTemp1.append(key)
            
        if len(listeErreurs) > 0 :
            message1 = _("Le lot de prélèvements ne peut être validé en raison des erreurs suivantes :")
            detail = "\n".join(listeErreurs)
            dlg = DLG_Messagebox.Dialog(self, titre=_("Erreur"), introduction=message1, detail=detail, icone=wx.ICON_EXCLAMATION, boutons=[_("Ok"), ], defaut=0)
            dlg.ShowModal()
            dlg.Destroy()
            return False
    ...
    def OnBoutonOk(self, event): 
        ...
        if self.ctrl_verrouillage.GetValue() == False :
            dlg = wx.MessageDialog(self, _("Pour clôturer le traitement d'un lot de prélèvements, vous devez valider ou refuser les prélèvements puis verrouiller le lot.\n\nSouhaitez-vous le faire maintenant ?!"), _("Information"), wx.YES_NO|wx.CANCEL|wx.NO_DEFAULT|wx.ICON_INFORMATION)
            reponse = dlg.ShowModal()
            dlg.Destroy()
            if reponse != wx.ID_NO :
                return
