...
class Parametres(wx.Panel):
    ...
    def __set_properties(self):
        ...
        self.ctrl_villes.SetToolTip(wx.ToolTip(_("Saisissez les noms de villes en les séparant d'un point-virgule")))
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        intro = _("Vous pouvez ici consulter et imprimer la liste des prestations par ville. Sélectionnez un ou plusieurs groupes d'activités ou certaines activités en particulier, puis saisissez une liste de villes à sélectionner, avant de cliquer sur le bouton 'Rafraîchir la liste' pour afficher les résultats.")
        titre = _("Liste des prestations par famille")
        self.SetTitle(titre)
    ...
    def __set_properties(self):
        ...
        self.bouton_apercu.SetToolTip(wx.ToolTip(_("Cliquez ici pour créer un aperçu PDF de la liste")))
    ...
    def GetLabelParametres(self):
        ...
        if presents == None :
            ...
            listeParametres.append(_("Tous les présents du %s au %s") % (DateEngFr(str(presents[0])), DateEngFr(str(presents[1]))))
        
        # Villes
        villes = self.ctrl_parametres.ctrl_villes.GetValue()
        villes = villes.replace(";", ", ")
        if len(villes) > 0 :
            listeParametres.append(_("Uniquement les villes : %s") % villes)
