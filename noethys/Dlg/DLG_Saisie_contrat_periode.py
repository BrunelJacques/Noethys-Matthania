...
##        self.Bind(wx.EVT_BUTTON, self.ctrl_conso.Modifier, self.bouton_modifier_conso)
        ...
        # Init
        if self.track == None :
            self.SetTitle(_("Saisie d'une période de facturation"))
        else :
            self.SetTitle(_("Modification d'une période de facturation"))
            self.Importation()
    ...
    def __set_properties(self):
        ...
        self.ctrl_label_prestation.SetToolTip(wx.ToolTip(_("Saisissez le label de la prestation")))
        self.ctrl_date_prestation.SetToolTip(wx.ToolTip(_("Saisissez la date de la prestation")))
        self.ctrl_montant_prestation.SetToolTip(wx.ToolTip(_("Saisissez le montant de la prestation")))
...
##        self.bouton_generer_conso.SetToolTip(wx.ToolTip(_("Cliquez ici pour générer automatiquement des consommations")))
##        self.bouton_ajouter_conso.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter une consommation")))
##        self.bouton_modifier_conso.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier la consommation sélectionnée")))
        self.bouton_supprimer_conso.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer les consommations sélectionnées ou cochées")))
        self.bouton_cocher_conso.SetToolTip(wx.ToolTip(_("Cliquez ici pour tout cocher")))
        self.bouton_decocher_conso.SetToolTip(wx.ToolTip(_("Cliquez ici pour tout décocher")))
...
##        grid_sizer_boutons_conso.Add(self.bouton_modifier_conso, 0, 0, 0)
    ...
    def OnBoutonOk(self, event):  
        ...
        if self.ctrl_label_prestation.GetValue() == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement renseigner le label de la prestation !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return

        if self.ctrl_date_prestation.GetDate() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement renseigner la date de la prestation !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        
        if self.ctrl_montant_prestation.GetMontant() == 0.0 :
            dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment appliquer un montant de 0.00 ?"), _("Avertissement"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
        ...
        for track in self.listeTracks :
            if track != self.track and date_fin >= track.date_debut and date_debut <= track.date_fin :
                dlg = wx.MessageDialog(self, _("Création impossible !\n\nCette période chevauche la période existante '%s' (du %s au %s).") % (track.label_prestation, UTILS_Dates.DateDDEnFr(track.date_debut), UTILS_Dates.DateDDEnFr(track.date_fin)), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return
