...
class Panel(wx.Panel):
    ...
    def __set_properties(self):
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour créer un utilisateur réseau")))
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier l'utilisateur réseau sélectionné dans la liste")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer l'utilisateur réseau sélectionné dans la liste")))
    ...
    def OnBoutonSupprimer(self, event):
        ...
        if index == -1:
            dlg = wx.MessageDialog(self, _("Vous devez d'abord sélectionner un utilisateur à supprimer dans la liste."), "Information", wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Remplissage(self, select=None):
        ...
        for user, host, autorisation in self.listeDonnees :
            ...
            if 'phoenix' in wx.PlatformInfo:
                ...
                self.SetStringItem(index, 1, user)
            if host == "%" : host = _("Connexion depuis n'importe quel hôte")
            elif host == "localhost" : host = _("Connexion uniquement depuis le serveur principal")
            else : host = _("Connexion uniquement depuis l'hôte %s") % host
            if 'phoenix' in wx.PlatformInfo:
