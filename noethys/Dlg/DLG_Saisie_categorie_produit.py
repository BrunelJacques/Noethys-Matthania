...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDcategorie=None):
        ...
        if self.IDcategorie == None :
            self.SetTitle(_("Saisie d'une catégorie de produits"))
        else :
            self.SetTitle(_("Modification d'une catégorie de produits"))
        ...
        # Questionnaire
        self.staticbox_questionnaire_staticbox = wx.StaticBox(self, -1, _("Questionnaire"))
    ...
    def __set_properties(self):
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez ici le nom de la catégorie de produits")))
        self.ctrl_observations.SetToolTip(wx.ToolTip(_("Saisissez ici des observations éventuelles")))
        self.ctrl_logo.SetToolTip(wx.ToolTip(_("Image de la catégorie de produits")))
    ...
    def OnBoutonOk(self, event):
        ...
        if len(nom) == 0:
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un nom pour cette catégorie de produits !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_nom.SetFocus()
            return
