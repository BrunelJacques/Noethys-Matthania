...
class CTRL_Interface(wx.Panel):
    def __init__(self, parent):
        ...
        # Interface
        self.box_interface_staticbox = wx.StaticBox(self, -1, _("Interface"))
        self.label_style = wx.StaticText(self, -1, _("Style :"))
        self.ctrl_style = CTRL_Style(self)
        self.label_theme = wx.StaticText(self, -1, _("Thème :"))
        self.ctrl_theme = CTRL_Theme(self)
        self.label_image = wx.StaticText(self, -1, _("Image :"))
        self.ctrl_image = wx.TextCtrl(self, -1, "")
        self.bouton_image = wx.Button(self, -1, "...", size=(20, 20))
        
        # Identification
        self.box_identification_staticbox = wx.StaticBox(self, -1, _("Système d'identification"))
        self.radio_barre = wx.RadioButton(self, -1, _("Barre numérique"), style=wx.RB_GROUP)
        self.radio_clavier = wx.RadioButton(self, -1, _("Barre et clavier numérique"))
        self.radio_liste = wx.RadioButton(self, -1, _("Liste des individus"))
        self.check_activites = wx.CheckBox(self, -1, _("Activer uniquement les inscrits aux activités :"))
    ...
    def __set_properties(self):
        self.ctrl_style.SetToolTip(wx.ToolTip(_("Sélectionnez un style d'interface")))
        self.ctrl_theme.SetToolTip(wx.ToolTip(_("Sélectionnez un thème")))
        self.radio_barre.SetToolTip(wx.ToolTip(_("Recommandé pour la saisie avec lecteur de code-barres")))
        self.radio_clavier.SetToolTip(wx.ToolTip(_("Recommandé pour la saisie avec écrans tactiles")))
        self.radio_liste.SetToolTip(wx.ToolTip(_("Recommandé pour la saisie avec écrans tactiles")))
        self.check_activites.SetToolTip(wx.ToolTip(_("Cochez cette case pour activer uniquement les inscrits aux activités sélectionnées")))
        self.ctrl_activites.SetToolTip(wx.ToolTip(_("Cochez les activités")))
        self.bouton_image.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner une image personnalisée")))
    ...
    def Validation(self):
        if self.ctrl_theme.GetID() == "personnalise" :
            ...
            if chemin == "" :
                dlg = wx.MessageDialog(self, _("Vous avez sélectionné le thème 'personnalisé' mais vous n'avez sélectionné aucune image !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
##            try :
##                testBmp = wx.Bitmap(Chemins.GetStaticPath(chemin), wx.BITMAP_TYPE_ANY)
##            except :
##                dlg = wx.MessageDialog(self, _("L'image personnalisée que vous avez sélectionné ne semble pas valide !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
##                dlg.ShowModal()
##                dlg.Destroy()
##                return False
        
        if self.check_activites.GetValue() and len(self.ctrl_activites.GetIDcoches()) == 0 :
                dlg = wx.MessageDialog(self, _("Vous avez sélectionné l'activation des inscrits mais sans sélectionner d'activités !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDprocedure=None, defaut=None):
        ...
        # Généralités
        self.box_generalites_staticbox = wx.StaticBox(self, -1, _("Généralités"))
        ...
        self.check_vocal = wx.CheckBox(self, -1, _("Activer la synthèse vocale"))
        self.check_vocal.SetValue(True)
        self.check_tutoiement = wx.CheckBox(self, -1, _("Activer le tutoiement"))
        self.check_confirmation = wx.CheckBox(self, -1, _("Demander une confirmation de l'identité"))

        # Actions
        self.box_actions_staticbox = wx.StaticBox(self, -1, _("Actions"))
    ...
    def __set_properties(self):
        self.SetTitle(_("Saisie d'une procédure"))
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez un nom pour cette procédure")))
        self.check_vocal.SetToolTip(wx.ToolTip(_("Cochez cette case pour activer la synthèse vocale dans cette procédure")))
        self.check_tutoiement.SetToolTip(wx.ToolTip(_("Cochez cette case pour utiliser le tutoiement au lieu du vouvoiement")))
        self.check_confirmation.SetToolTip(wx.ToolTip(_("Noethys demandera la confirmation de l'identité de l'individu après son identification")))
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter une action")))
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier l'action sélectionnée")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer l'action sélectionnée")))
        self.bouton_monter.SetToolTip(wx.ToolTip(_("Cliquez ici pour faire monter l'action sélectionnée")))
        self.bouton_descendre.SetToolTip(wx.ToolTip(_("Cliquez ici pour faire descendre l'action sélectionnée")))
    ...
    def OnBoutonOk(self, event): 
        ...
        if nom_procedure == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un nom de procédure !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if len(listeActions) == 0 :
            dlg = wx.MessageDialog(self, _("Etes-vous sûr de ne pas vouloir saisir d'action ?"), _("Confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
