...
class Dialog(wx.Dialog):
    ...
    def __set_properties(self):
        self.ctrl_agrement.SetToolTip(wx.ToolTip(_("Saisissez ici un numéro d'agrément Par exemple : '098GHJ25542'")))
    ...
    def OnBoutonOk(self, event):
        ...
        if self.ctrl_agrement.GetValue() == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement donner un numéro d'agrément !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_agrement.SetFocus()
            return
        if self.ctrl_date_debut.GetDate() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement donner une date de début de validité !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_date_debut.SetFocus()
            return
        if self.ctrl_date_fin.GetDate() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement donner une date de fin de validité !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
