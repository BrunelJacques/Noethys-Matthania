...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        # Bandeau
        titre = _("Transférer des tables")
        intro = _("Cette fonctionnalité permet de transférer le contenu des tables du fichier de données chargé vers un fichier de données local.")
        ...
        self.ctrl_destination = filebrowse.FileBrowseButton(self, -1, labelText=u"", buttonText=_("Sélectionner"), toolTip=_("Cliquez ici pour sélectionner un fichier de données"), dialogTitle=_("Sélectionner un fichier"), fileMask=wildcard)
        
        self.label_destination_copy = wx.StaticText(self, -1, _("Tables à transférer :"))
    ...
    def __set_properties(self):
        self.ctrl_destination.SetToolTip(wx.ToolTip(_("Sélectionnez un fichier de destination")))
        self.ctrl_tables.SetToolTip(wx.ToolTip(_("Sélectionnez les tables à transférer")))
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez ici pour obtenir de l'aide")))
        self.bouton_ok.SetToolTip(wx.ToolTip(_("Cliquez ici pour lancer la procédure")))
    ...
    def OnBoutonOk(self, event): 
        ...
        if len(listeTables) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement cocher au moins une table à exporter !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        
        # Demande de confirmation
        dlg = wx.MessageDialog(None, _("Souhaitez-vous vraiment transférer %d tables ?") % len(listeTables), _("Confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        ...
        # Confirmation
        dlg = wx.MessageDialog(self, _("Procédure de transfert terminée !"), _("Fin"), wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
