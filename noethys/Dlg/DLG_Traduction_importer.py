...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        # Bandeau
        titre = _("Import/Export de traductions au format texte")
        intro = _("Sélectionnez une langue dans la liste puis exportez ou importez les textes au format TXT.")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Traduction.png")
        
        # Langue
        self.box_langue_staticbox = wx.StaticBox(self, -1, _("Langue"))
        self.ctrl_langues = CTRL_Langue(self)
        
        # Exporter
        self.box_exporter_staticbox = wx.StaticBox(self, -1, _("Exporter"))
        self.check_nontraduits = wx.CheckBox(self, -1, _("Inclure uniquement les textes non traduits"))
        self.check_nontraduits.SetValue(True)
        self.bouton_exporter = CTRL_Bouton_image.CTRL(self, texte=_("Exporter"), cheminImage="Images/32x32/Fleche_haut.png")
        self.bouton_exporter.SetMinSize((400, -1))
        
        # Importer
        self.box_importer_staticbox = wx.StaticBox(self, -1, _("Importer"))
        
        wildcard = _("Fichiers texte|*.txt|Tous les fichiers (*.*)|*.*")
        sp = wx.StandardPaths.Get()
        cheminDefaut = sp.GetDocumentsDir()
        self.ctrl_fichier_importer_original = filebrowse.FileBrowseButton(self, -1, labelText=_("Fichier des textes originaux :"), buttonText=_("Sélectionner"), toolTip=_("Cliquez ici pour sélectionner un fichier"), dialogTitle=_("Sélectionner un fichier"), fileMask=wildcard, startDirectory=cheminDefaut)
        self.ctrl_fichier_importer_traduction = filebrowse.FileBrowseButton(self, -1, labelText=_("Fichier des textes traduits :"), buttonText=_("Sélectionner"), toolTip=_("Cliquez ici pour sélectionner un fichier"), dialogTitle=_("Sélectionner un fichier"), fileMask=wildcard, startDirectory=cheminDefaut)
        self.bouton_importer = CTRL_Bouton_image.CTRL(self, texte=_("Importer"), cheminImage="Images/32x32/Fleche_bas.png")
    ...
    def __set_properties(self):
        self.ctrl_langues.SetToolTip(wx.ToolTip(_("Sélectionnez la langue souhaitée")))
        self.check_nontraduits.SetToolTip(wx.ToolTip(_("Cochez cette case pour inclure uniquement les textes non traduits")))
        self.bouton_exporter.SetToolTip(wx.ToolTip(_("Cliquez ici pour exporter les textes originaux au format txt")))
        self.bouton_importer.SetToolTip(wx.ToolTip(_("Cliquez ici pour importer les traductions du fichier txt")))
    ...
    def OnBoutonExporter(self, event):
        ...
        if dictLangue == None :
            dlg = wx.MessageDialog(self, _("Vous devez sélectionner une langue dans la liste !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        nom_fichier = "Textes_originaux.txt"
        wildcard = "Fichier Texte (*.txt)|*.xml| Tous les fichiers (*.*)|*.*"
        sp = wx.StandardPaths.Get()
        cheminDefaut = sp.GetDocumentsDir()
        dlg = wx.FileDialog(
        ...
        # Confirmation fin
        dlg = wx.MessageDialog(self, _("Le fichier a été généré avec succès (%d textes) !") % nbreTextes, _("Génération"), wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
    ...
    def OnBoutonImporter(self, event):
        ...
        if fichier_original == "" :
            dlg = wx.MessageDialog(self, _("Vous devez sélectionner un fichier de textes originaux !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if fichier_traduction == "" :
            dlg = wx.MessageDialog(self, _("Vous devez sélectionner un fichier de textes traduits !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if len(lignesOriginal) !=  len(lignesTraduction) :
            dlg = wx.MessageDialog(self, _("Le nombre de lignes des deux fichiers doit être identique !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    
        # Demande de confirmation
        dlg = wx.MessageDialog(None, _("Confirmez-vous l'insertion de %d textes traduits dans le fichier '%s' ?") % (len(lignesOriginal), nom_langue), "Confirmation", wx.YES_NO | wx.YES_DEFAULT | wx.ICON_EXCLAMATION)
        if dlg.ShowModal() == wx.ID_NO :
            return False
            dlg.Destroy()
        else:
        ...
        # Confirmation
        dlg = wx.MessageDialog(self, _("L'importation des traductions s'est déroulée avec succès !"), _("Succès"), wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
