...
class CTRL_Produit(wx.TextCtrl):
    ...
    def SetIDproduit(self, IDproduit=None):
        ...
        if len(listeDonnees) > 0 :
            ...
            label = "%s (%s)" % (nomProduit, nomCategorie)
        else :
            IDcategorie = None
            label = ""
            observations = ""
...
class CTRL_Parametres(wx.Notebook):
    def __init__(self, parent, IDlocation=None):
        ...
        self.listePages = [
            {"code": "questionnaire", "ctrl": Page_Questionnaire(self, self.IDlocation), "label": _("Questionnaire"), "image": "Questionnaire.png"},
            {"code": "facturation", "ctrl": Page_Facturation(self, self.IDlocation), "label": _("Prestations"), "image": "Euro.png"},
            {"code": "historique", "ctrl": Page_Historique(self, self.IDlocation), "label": _("Historique"), "image": "Historique.png"},
        ]
...
class Page_Facturation(wx.Panel):
    def __init__(self, parent, IDlocation=None):
        ...
        # Properties
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter une prestation")))
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier la prestation sélectionnée dans la liste")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer la prestation sélectionnée dans la liste")))
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDlocation=None, IDfamille=None, IDproduit=None):
        ...
        if self.IDlocation == None :
            self.SetTitle(_("Saisie d'une location"))
        else :
            self.SetTitle(_("Modification d'une location"))

        # Généralités
        self.staticbox_generalites_staticbox = wx.StaticBox(self, -1, _("Généralités"))
        self.label_loueur = wx.StaticText(self, -1, _("Loueur :"))
        ...
        self.bouton_loueur = wx.Button(self, -1, _("Sélectionner"))

        self.label_produit = wx.StaticText(self, -1, _("Produit :"))
        ...
        self.bouton_produit = wx.Button(self, -1, _("Sélectionner"))
        ...
        self.label_partage = wx.StaticText(self, -1, _("Partage :"))
        self.ctrl_partage = wx.CheckBox(self, -1, _("Le loueur autorise le partage du produit avec d'autres loueurs"))
        self.label_partage.Show(False)
        self.ctrl_partage.Show(False)

        # Logo
        self.staticbox_logo_staticbox = wx.StaticBox(self, -1, _("Image du produit"))
        ...
        # Période
        self.staticbox_periode_staticbox = wx.StaticBox(self, -1, _("Période de location"))
        self.label_date_debut = wx.StaticText(self, -1, _("Début :"))
        self.ctrl_date_debut = CTRL_Saisie_date.Date2(self)
        self.ctrl_heure_debut = CTRL_Saisie_heure.Heure(self)
        self.check_date_fin = wx.CheckBox(self, -1, _("Fin :"))
        self.ctrl_date_fin = CTRL_Saisie_date.Date2(self)
        self.ctrl_heure_fin = CTRL_Saisie_heure.Heure(self)

        self.check_recurrence = wx.CheckBox(self, -1, _("Récurrence"))
        ...
        # Quantité
        self.staticbox_quantite_staticbox = wx.StaticBox(self, -1, _("Quantité"))
    ...
    def __set_properties(self):
        self.ctrl_loueur.SetToolTip(wx.ToolTip(_("Nom du loueur")))
        self.ctrl_produit.SetToolTip(wx.ToolTip(_("Nom du produit")))
        self.bouton_loueur.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner un loueur")))
        self.bouton_produit.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner un produit")))
        self.ctrl_description.SetToolTip(wx.ToolTip(_("Saisissez ici une description pour cette location (Ex : Location de salle pour mariage...)")))
        self.ctrl_observations.SetToolTip(wx.ToolTip(_("Saisissez ici des observations éventuelles")))
        self.ctrl_partage.SetToolTip(wx.ToolTip(_("Cliquez ici pour autoriser le partage entre plusieurs usagers")))
        self.ctrl_logo.SetToolTip(wx.ToolTip(_("Image du produit")))
        self.bouton_visualiser.SetToolTip(wx.ToolTip(_("Cliquez ici pour visualiser l'image actuelle en taille réelle")))

        self.ctrl_date_debut.SetToolTip(wx.ToolTip(_("Saisissez la date de début de location")))
        self.ctrl_heure_debut.SetToolTip(wx.ToolTip(_("Saisissez l'heure de début de location")))
        self.check_date_fin.SetToolTip(wx.ToolTip(_("Cochez cette case pour définir la date de fin de location")))
        self.ctrl_date_fin.SetToolTip(wx.ToolTip(_("Saisissez la date de fin de location")))
        self.ctrl_heure_fin.SetToolTip(wx.ToolTip(_("Saisissez l'heure de fin de location")))
        self.ctrl_quantite.SetToolTip(wx.ToolTip(_("Saisissez une quantité")))
        self.bouton_recurrence.SetToolTip(wx.ToolTip(_("Cliquez ici pour définir les paramètres de la récurrence")))
    ...
    def OnBoutonOutils(self, event):
        ...
        # Item Mesurer
        item = wx.MenuItem(menuPop, 10, _("Mesurer une distance"))
        ...
        if UTILS_Customize.GetValeur("referentiel", "url", None, ajouter_si_manquant=False) != None:
            item = wx.MenuItem(menuPop, 20, _("Contrôle référentiel"))
    ...
    def OnBoutonOk(self, event):
        ...
        IDfamille = self.ctrl_loueur.GetIDfamille()
        if IDfamille == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner un loueur pour ce produit !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if IDproduit == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner un produit !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if self.check_recurrence.GetValue() == False:
            ...
            if date_debut == None:
                dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de début de location !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
            ...
            if heure_debut == None or self.ctrl_heure_debut.Validation() == False:
                dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une heure de début valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
            ...
            if self.check_date_fin.GetValue() == True :
                ...
                if date_fin == None:
                    dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de fin de location !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                    dlg.ShowModal()
                    dlg.Destroy()
                    self.ctrl_date_fin.SetFocus()
                    return
                ...
                if heure_fin == None or self.ctrl_heure_fin.Validation() == False:
                    dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une heure de fin valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                    dlg.ShowModal()
                    dlg.Destroy()
            ...
            if date_fin != None and date_debut > date_fin:
                dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de fin supérieure à la date de début !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_date_fin.SetFocus()
                return
        ...
        if self.check_recurrence.GetValue() == True:
            ...
            if not self.recurrence:
                dlg = wx.MessageDialog(self, _("Vous n'avez pas renseigné les paramètres de la récurrence !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return
            ...
            introduction = _("Confirmez-vous la création des %d locations suivantes :") % len(liste_locations)
            liste_locations_txt = []
            for dict_location in liste_locations:
                date_debut = dict_location["date_debut"]
                date_fin = dict_location["date_fin"]
                liste_locations_txt.append(_("%s - %s") % (date_debut.strftime("%d/%m/%Y %H:%M:%S"), date_fin.strftime("%d/%m/%Y %H:%M:%S") if (date_fin and date_fin.year != 2999) else _("Illimitée")))
            dlg = DLG_Messagebox.Dialog(self, titre=_("Confirmation"), introduction=introduction, detail=u"\n".join(liste_locations_txt), icone=wx.ICON_EXCLAMATION, boutons=[_("Oui"), _("Non"), _("Annuler")])
            ...
            if liste_prestations:
                dlg = wx.MessageDialog(self, _("Souhaitez-vous que la date de chaque prestation soit la date de début de chaque location ?"), _("Question"), wx.YES_NO | wx.NO_DEFAULT | wx.CANCEL | wx.ICON_QUESTION)
                reponse = dlg.ShowModal()
                dlg.Destroy()
        ...
        for dict_location in liste_locations:
            ...
            date_debut = dict_location["date_debut"]
            date_fin = dict_location["date_fin"]
            periode_str = _("%s - %s") % (date_debut.strftime("%d/%m/%Y %H:%M:%S"), date_fin.strftime("%d/%m/%Y %H:%M:%S") if (date_fin and date_fin.year != 2999) else _("Illimitée"))
            ...
            for periode, valeurs in dictPeriodes.items() :
                if valeurs["disponible"] < quantite :
                    ...
                    liste_periode_non_dispo.append(_("Stock disponible du %s au %s : %d produits") % (debut, fin, valeurs["disponible"]))
            if len(liste_periode_non_dispo) > 0:
                liste_anomalies.append(u"Location du %s : Produit indisponible. %s." % (periode_str, ", ".join(liste_periode_non_dispo)))
                valide = False
        ...
        if len(liste_anomalies) > 0:
            introduction = _("%d anomalies ont été détectées :") % len(liste_anomalies)
            if len(liste_valides) > 0:
                conclusion = _("Souhaitez-vous quand même continuer avec les %d locations possibles ?") % len(liste_valides)
                dlg = DLG_Messagebox.Dialog(self, titre=_("Anomalies"), introduction=introduction, detail=u"\n".join(liste_anomalies), conclusion=conclusion, icone=wx.ICON_EXCLAMATION, boutons=[_("Oui"), _("Non"), _("Annuler")])
                reponse = dlg.ShowModal()
                dlg.Destroy()
                if reponse in (1, 2):
                    return False
            else:
                dlg = DLG_Messagebox.Dialog(self, titre=_("Anomalies"), introduction=introduction, detail=u"\n".join(liste_anomalies), icone=wx.ICON_EXCLAMATION, boutons=[_("Fermer")])
                reponse = dlg.ShowModal()
                dlg.Destroy()
                return False
        ...
        for dict_location in liste_valides:
            ...
            periode = _("%s - %s") % (date_debut.strftime("%d/%m/%Y %H:%M:%S"), date_fin.strftime("%d/%m/%Y %H:%M:%S") if (date_fin and date_fin.year != 2999) else _("Illimitée"))
            ...
            if self.IDlocation == None :
                ...
                texte_historique = _("Saisie de la location ID%d : %s %s") % (IDlocation, nom_produit, periode)
                ...
                texte_historique = _("Modification de la location ID%d : %s %s") % (IDlocation, nom_produit, periode)
