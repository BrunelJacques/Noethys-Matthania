...
class PAGE_Criteres(wx.Panel):
    def __init__(self, parent, IDdemande=None):
        ...
        self.label_categories = wx.StaticText(self, -1, _("Catégories :"))
        self.ctrl_categories = CTRL_Categories(self)
        self.bouton_categories = wx.Button(self, -1, _("Sélectionner"))

        self.label_produits = wx.StaticText(self, -1, _("Produits :"))
        self.ctrl_produits = CTRL_Produits(self)
        self.bouton_produits = wx.Button(self, -1, _("Sélectionner"))

        self.label_filtres = wx.StaticText(self, -1, _("Questions :"), style=wx.ALIGN_RIGHT)
        ...
        # Properties
        self.ctrl_categories.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner des catégories")))
        self.bouton_categories.SetToolTip(wx.ToolTip(_("Choix de catégories")))
        self.ctrl_produits.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner des produits")))
        self.bouton_produits.SetToolTip(wx.ToolTip(_("Choix de catégories")))
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour saisir un filtre sur les questionnaires")))
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier le filtre sélectionné")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer le filtre sélectionné")))
...
class Notebook(wx.Notebook):
    def __init__(self, parent, IDdemande=None):
        ...
        self.listePages = [
            ("criteres", _("Critères"), PAGE_Criteres(self, IDdemande=IDdemande)),
            ("questionnaire", _("Questionnaire"), PAGE_Questionnaire(self, IDdemande=IDdemande)),
        ]
...
class PAGE_Statut_attente(wx.Panel):
    def __init__(self, parent):
        ...
        # Propositions
        self.label_propositions = wx.StaticText(self, -1, _("Produits proposés :"))
        ...
        # Options
        self.radio_produits_disponibles = wx.RadioButton(self, -1, _("Les produits disponibles"), style=wx.RB_GROUP)
        self.radio_produits_possibles = wx.RadioButton(self, -1, _("Les produits potentiels"))

        self.radio_produits_disponibles.SetToolTip(wx.ToolTip(_("Afficher les produits disponibles répondant aux critères")))
        self.radio_produits_possibles.SetToolTip(wx.ToolTip(_("Afficher tous les produits répondant aux critères donnés")))

        # Bouton Attribuer
        self.bouton_attribuer = CTRL_Bouton_image.CTRL(self, texte=_("Attribuer le produit sélectionné"), cheminImage="Images/32x32/Location.png")
        self.bouton_attribuer.SetToolTip(wx.ToolTip(_("Cliquez ici pour attribuer le produit sélectionné dans la liste")))
    ...
    def OnBoutonAttribuer(self, event):
        ...
        if IDproduit == None:
            dlg = wx.MessageDialog(self, _("Vous devez sélectionner un produit disponible dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
...
class PAGE_Statut_refusee(wx.Panel):
    def __init__(self, parent):
        ...
        self.label_motif = wx.StaticText(self, -1, _("Motif du refus :"))
...
class CTRL_Attribution(html.HtmlWindow):
    ...
    def SetIDlocation(self, IDlocation):
        ...
        texte_debut = _("%s à %s") % (date_debut_temp, heure_debut_temp)
        ...
        if date_fin != None :
            ...
            texte_fin = _("%s à %s") % (date_fin_temp, heure_fin_temp)
        else :
            texte_fin = _("Non définie")

        if len(observations) > 0 :
            texte_observations = _("<BR>Notes : %s") % observations
        else :
            texte_observations = ""
...
class PAGE_Statut_attribuee(wx.Panel):
    def __init__(self, parent):
        ...
        self.label_attribution = wx.StaticText(self, -1, _("Location attribuée :"))
...
class CTRL_Statut(wx.Choicebook):
    ...
    def __init__(self, parent):
        ...
        self.parent = parent
        self.SetToolTip(wx.ToolTip(_("Sélectionnez ici le statut de la demande")))

        self.listePanels = [
            ("attente", _("En attente"), PAGE_Statut_attente(self)),
            ("refusee", _("Demande refusée"), PAGE_Statut_refusee(self)),
            #("attribuee", _("Demande satisfaite"), PAGE_Statut_attribuee(self)),
        ]
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDdemande=None, IDfamille=None):
        ...
        if self.IDdemande == None :
            self.SetTitle(_("Saisie d'une demande"))
        else :
            self.SetTitle(_("Modification d'une demande"))
        ...
        self.label_date_demande = wx.StaticText(self, -1, _("Date :"))
        ...
        # Statut
        self.staticbox_statut_staticbox = wx.StaticBox(self, -1, _("Statut"))
    ...
    def __set_properties(self):
        ...
        self.ctrl_date_demande.SetToolTip(wx.ToolTip(_("Date de la demande")))
        self.ctrl_heure_demande.SetToolTip(wx.ToolTip(_("Heure de la demande")))
    ...
    def Mesurer_distance(self, event):
        ...
        if len(categories) == 0 and len(produits) == 0:
            dlg = wx.MessageDialog(self, _("Vous devez d'abord sélectionner renseigner une catégorie ou un produit dans les critères !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if len(categories) > 1 or len(produits) > 1 :
            ...
            dlg = wx.SingleChoiceDialog(self, _("Sélectionnez la donnée qui vous intéresse :"), _("Sélection d'une donnée"), liste_labels, wx.CHOICEDLG_STYLE)
            if dlg.ShowModal() == wx.ID_OK:
                dlg.Destroy()
    ...
    def OnBoutonOk(self, event):
        ...
        if date_demande == None:
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date pour cette demande !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if heure_demande == None or self.ctrl_heure_demande.Validation() == False:
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une heure pour cette demande !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        IDfamille = self.ctrl_loueur.GetIDfamille()
        if IDfamille == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner un loueur pour cette demande !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
