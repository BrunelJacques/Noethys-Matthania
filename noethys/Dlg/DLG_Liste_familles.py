...
from Ol import OL_Liste_familles
...
##        self.check_concernes = wx.CheckBox(self, -1, _("Ne pas afficher les dossiers complets"))
        ...
        self.check_archives = wx.CheckBox(self, -1, _("Afficher les familles archivées"))
...
class Dialog(wx.Dialog):
    ...
    def __set_properties(self):
        self.SetTitle(_("Liste des familles"))
    ...
    def GetLabelParametres(self):
        ...
        if presents == None :
            ...
            listeParametres.append(_("Uniquement les familles dont un des membres est présent du %s au %s") % (DateEngFr(str(presents[0])), DateEngFr(str(presents[1]))))

        # Archivés
        if self.ctrl_parametres.ctrl_options.GetArchives() == True :
            listeParametres.append(_("Familles archivées incluses"))
