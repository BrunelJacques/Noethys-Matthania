    ...
    def MAJ(self):
        ...
        if len(listeFichiersReseau) > 0  and self.parent.check_reseau.GetValue() == True :
            ...
            for nomFichier in listeFichiersReseau :
                ...
                for nomCategorie, codeCategorie in LISTE_CATEGORIES :
                    fichier = "%s_%s" % (nomFichier, codeCategorie.lower())
                    brancheFichier = self.AppendItem(brancheNom, nomCategorie, ct_type=1)
                    self.SetPyData(brancheFichier, fichier)
...
##            return [], []
    ...
    def AfficheNbreElements(self):
        ...
        texte = _("Données à sauvegarder (%d éléments cochés)") % nbreElements
...
class CTRL_Parametres(wx.Panel) :
    def __init__(self, parent, labelControleNom=_("Nom")) :
        wx.Panel.__init__(self, parent, id=-1, style=wx.TAB_TRAVERSAL)
        self.labelControleNom = labelControleNom
        
        # Nom
        self.box_nom_staticbox = wx.StaticBox(self, -1, labelControleNom)
        self.label_nom = wx.StaticText(self, -1, "%s :" % labelControleNom)
        self.ctrl_nom = wx.TextCtrl(self, -1, "")
        
        # Protection
        self.box_mdp_staticbox = wx.StaticBox(self, -1, _("Protection"))
        self.label_cryptage = wx.StaticText(self, -1, _("Cryptage :"))
        self.check_cryptage = wx.CheckBox(self, -1, "")
        ...
        self.check_repertoire = wx.CheckBox(self, -1, _("Répertoire :"))
        ...
        self.check_email = wx.CheckBox(self, -1, _("Envoi par Email :"))
        self.ctrl_email = wx.TextCtrl(self, -1, "")
        
        # Données
        self.box_donnees_staticbox = wx.StaticBox(self, -1, _("Données à sauvegarder"))
        self.ctrl_donnees = CTRL_Donnees(self)
        self.ctrl_donnees.SetMinSize((300, -1))
        self.check_locaux = wx.CheckBox(self, -1, _("Fichiers locaux"))
        self.check_reseau = wx.CheckBox(self, -1, _("Fichiers réseau"))
    ...
    def __set_properties(self):
        self.check_cryptage.SetToolTip(wx.ToolTip(_("Cochez cette case pour crypter le fichier de sauvegarde (utile pour un stockage en ligne)")))
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez ici le %s du fichier de sauvegarde") % self.labelControleNom.lower()))
        self.ctrl_mdp.SetToolTip(wx.ToolTip(_("Si vous souhaitez protéger le fichier de sauvegarde avec un mot de passe, tapez-le ici")))
        self.ctrl_confirmation.SetToolTip(wx.ToolTip(_("Confirmez le mot de passe")))
        self.check_repertoire.SetToolTip(wx.ToolTip(_("Cliquez ici pour enregistrer le fichier de sauvegarde dans le répertoire donné")))
        self.ctrl_repertoire.SetMinSize((180, -1))
        self.ctrl_repertoire.SetToolTip(wx.ToolTip(_("Saisissez ici le répertoire de destination")))
        self.bouton_repertoire.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner un répertoire de destination")))
        self.check_email.SetToolTip(wx.ToolTip(_("Cochez cette case pour envoyer la sauvegarde par Email")))
        self.ctrl_email.SetToolTip(wx.ToolTip(_("Saisissez ici une ou plusieurs adresses Email (separées par des points-virgules)")))
        self.check_locaux.SetToolTip(wx.ToolTip(_("Cochez cette case pour afficher les fichiers locaux")))
        self.check_reseau.SetToolTip(wx.ToolTip(_("Cochez cette case pour afficher les fichiers réseau")))
    ...
    def OnCheckReseau(self, event):
        if self.dictConnexion == None :
            ...
            intro = _("Pour accéder à la liste des fichiers réseau disponibles, un accès MySQL \nest nécessaire. Veuillez saisir vos paramètres de connexion réseau :")
    ...
    def GetDonnees(self):
        ...
        if nom == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un %s pour cette sauvegarde !") % self.labelControleNom.lower(), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.check_cryptage.GetValue() == True :
            ...
            if motdepasse == "" :
                dlg = wx.MessageDialog(self, _("Vous n'avez saisi aucun mot de passe !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_mdp.SetFocus()
                return False
            if motdepasse != confirmation :
                dlg = wx.MessageDialog(self, _("Le mot de passe n'a pas été confirmé à l'identique !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
        ...
        if self.check_email.GetValue() == True :
            ...
            if listeEmails == "" :
                dlg = wx.MessageDialog(self, _("Vous devez saisir au moins une adresse Email !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
        ...
        if self.check_repertoire.GetValue() == False and self.check_email.GetValue() == False :
            dlg = wx.MessageDialog(self, _("Vous devez sélectionner au moins une destination !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if listeFichiersLocaux == None and listeFichiersReseau == None :
            dlg = wx.MessageDialog(self, _("Vous devez cocher au moins un élément à sauvegarder dans la liste !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
...
class Dialog(wx.Dialog):
    def __init__(self, parent, dictDonnees=None):
        ...
        intro = _("Vous pouvez créer ici une sauvegarde des données afin d'en la conservation ou de les transférer sur un autre ordinateur. Il est possible de crypter le fichier de sauvegarde et de l'enregistrer sur le disque dur ou une clé USB et de l'expédier par Email.")
        titre = _("Sauvegarde")
        self.SetTitle(titre)
    ...
    def __set_properties(self):
        ...
        self.bouton_ok.SetToolTip(wx.ToolTip(_("Cliquez ici pour lancer la sauvegarde")))
    ...
    def Importation(self):
        ...
        dictSauvegarde = {
            "sauvegarde_nom" : _("Noethys_%s") % datetime.datetime.now().strftime("%Y%m%d_%H%M"),
    ...
    def OnBoutonOk(self, event): 
        ...
        # Fin du processus
        dlg = wx.MessageDialog(self, _("La procédure de sauvegarde est terminée."), _("Sauvegarde"), wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
