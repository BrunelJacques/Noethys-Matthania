...
class Parametres(wx.Panel):
    def __init__(self, parent):
        ...
        # Séparation
        self.staticbox_dateNaiss_staticbox = wx.StaticBox(self, -1, _("Limite d'âge"))
        self.check_dateNaiss = wx.CheckBox(self, -1, _("Date de naissance min. :"))
    ...
    def __set_properties(self):
        ...
        self.ctrl_dateNaiss.SetToolTip(wx.ToolTip(_("Saisissez une date de naissance maximale")))
    ...
    def OnBoutonActualiser(self, event): 
        ...
        if self.check_dateNaiss.GetValue() == True :
            ...
            if self.ctrl_dateNaiss.FonctionValiderDate() == False or dateNaiss == None :
                dlg = wx.MessageDialog(self, _("La date de naissance semble incorrecte !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_dateNaiss.SetFocus()
                return False
        ...
        for IDactivite in listeActivites :
            if UTILS_Utilisateurs.VerificationDroitsUtilisateurActuel("facturation_attestations", "creer", IDactivite=IDactivite, afficheMessage=False) == False : 
                dlg = wx.MessageDialog(self, _("Vous n'avez pas l'autorisation de générer des attestations pour l'ensemble des activités sélectionnées !"), _("Action non autorisée"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
...
class Panel(wx.Panel):
    def __init__(self, parent):
        ...
        # CTRL Prestations
        self.staticbox_prestations_staticbox = wx.StaticBox(self, -1, _("Prestations"))
        self.label_commentaires = wx.StaticText(self, -1, _("Double-cliquez dans la colonne 'Ajustement' pour créer un ajustement sur chaque prestation (Ex : '+3.5' ou '-2.5')."))
        ...
        for track in listePrestations :
            if track.ajustement not in ("", None) :
                try :
                    ...
                    dlg = wx.MessageDialog(self, _("L'ajustement que vous avez paramétré pour la prestation '%s' semble erroné !\n\nVous pouvez uniquement saisir des valeurs du type '-2.5' ou '+5'.") % track.label, _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                    dlg.ShowModal()
                    dlg.Destroy()
                    return False
