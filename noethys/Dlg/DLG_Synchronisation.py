...
def AnalyserFichier(nomFichier="", tailleFichier=None, typeTransfert=None):
    ...
    if tailleFichier != None :
        ...
        if tailleFichier != tailleFinaleFichier :
            listeAnomalies.append((nomFichier, _("Le fichier n'a pas été téléchargé en intégralité (%d/%d)") % (tailleFichier, tailleFinaleFichier)))
    ...
    if zipfile.is_zipfile(nouveauCheminFichier) == False :
        listeAnomalies.append((nomFichier, _("Le fichier compressé ne semble pas valide.")))
        return False        
...
class CTRL_Mode(CTRL_Ultrachoice.CTRL):
    def __init__(self, parent):
        donnees=[ 
            {"label" : _("Transfert par FTP"), "description" : _("Pour transférer les données par FTP depuis ou vers un hébergement internet"), "image" : wx.Bitmap(Chemins.GetStaticPath(u"Images/32x32/Ftp.png"), wx.BITMAP_TYPE_ANY)},
            {"label" : _("Transfert manuel"), "description" : _("Pour sélectionner un fichier sur un support physique (disque dur, clé USB, etc...)"), "image" : wx.Bitmap(Chemins.GetStaticPath(u"Images/32x32/Sauvegarde_param.png"), wx.BITMAP_TYPE_ANY)},
            ]
...
class Page_serveur(wx.Panel):
    def __init__(self, parent):
        ...
        self.label_activer = wx.StaticText(self, -1, _("Activer :"))
        self.check_activer = wx.CheckBox(self, -1, "")
        self.label_info = wx.StaticText(self, -1, _("(Relancez Noethys pour appliquer les modifications)"))
        ...
        self.label_port = wx.StaticText(self, -1, _("Port :"))
        self.ctrl_port = wx.TextCtrl(self, -1, "8000", size=(50, -1))
        self.label_ip_autorisees = wx.StaticText(self, -1, _("IP autorisées :"))
        self.ctrl_ip_autorisees = wx.TextCtrl(self, -1, "")
        self.label_ip_interdites = wx.StaticText(self, -1, _("IP interdites :"))
        self.ctrl_ip_interdites = wx.TextCtrl(self, -1, "")

        # Propriétés
        self.check_activer.SetToolTip(wx.ToolTip(_("Cochez cette case pour activer/désactiver le serveur WIFI/Direct au prochain démarrage de Noethys")))
        self.ctrl_port.SetToolTip(wx.ToolTip(_("Sélectionnez le port de communication")))
        legende_texte_ip = _("""
...
'192.168.1.*;192.*;192.168.*.*;192.*.1'     
        """)
        self.ctrl_ip_autorisees.SetToolTip(wx.ToolTip(_("Saisissez une suite d'adresses IP autorisées à se connecter (séparées par des points-virgules). Laissez vide pour tout autoriser.  %s") % legende_texte_ip))
        self.ctrl_ip_interdites.SetToolTip(wx.ToolTip(_("Saisissez une suite d'adresses IP interdites de se connecter (séparées par des points-virgules). Laissez vide pour tout autoriser.  %s") % legende_texte_ip))
        ...
        sizer2.Add(self.label_activer, 0, wx.ALIGN_CENTER_VERTICAL, 0)
        sizer2.Add(self.check_activer, 0, wx.LEFT, 10)
        sizer2.Add(self.label_info, 0, wx.EXPAND|wx.LEFT, 10)
    ...
    def Validation(self):
        if self.check_activer.GetValue() == True and self.ctrl_port.GetValue() == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un numéro de port !"), "Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
...
class Page_ftp(wx.Panel):
    def __init__(self, parent):
        ...
        self.label_hote = wx.StaticText(self, -1, _("Adresse hôte :"))
        self.ctrl_hote = wx.TextCtrl(self, -1, "")
        self.label_identifiant = wx.StaticText(self, -1, _("Identifiant :"))
        ...
        self.label_repertoire = wx.StaticText(self, -1, _("Répertoire :"))
        self.ctrl_repertoire = wx.TextCtrl(self, -1, "www/")
        self.bouton_test = wx.Button(self, -1, _("Tester la\nconnexion"))
        
        self.Bind(wx.EVT_BUTTON, self.OnBoutonTest, self.bouton_test)
        
        # Propriétés
        self.ctrl_hote.SetToolTip(wx.ToolTip(_("Saisissez l'adresse de l'hôte")))
        self.ctrl_identifiant.SetToolTip(wx.ToolTip(_("Saisissez l'identifiant")))
        self.ctrl_mdp.SetToolTip(wx.ToolTip(_("Saisissez le mot de passe")))
        self.ctrl_repertoire.SetToolTip(wx.ToolTip(_("Saisissez le chemin du répertoire. Exemple : 'www/mesfichiers")))
        self.bouton_test.SetToolTip(wx.ToolTip(_("Cliquez ici pour tester les paramètres de connexion")))
...
class Page_cryptage(wx.Panel):
    def __init__(self, parent):
        ...
        self.label_activer = wx.StaticText(self, -1, _("Activer :"))
        self.check_activer = wx.CheckBox(self, -1, "")
        self.label_info = wx.StaticText(self, -1, _("(Les données envoyées seront cryptées avec le mot de passe)"))
        self.label_info.SetFont(wx.Font(8, wx.SWISS, wx.NORMAL, wx.NORMAL, False))
        self.label_info.SetForegroundColour(wx.Colour(150, 150, 150))
        self.label_mdp = wx.StaticText(self, -1, _("Mot de passe :"))
        ...
        # Propriétés
        self.check_activer.SetToolTip(wx.ToolTip(_("Cochez cette case pour activer/désactiver le cryptage des données envoyées")))
        self.ctrl_mdp.SetToolTip(wx.ToolTip(_("Saisissez le mot de passe qui sera utiliser pour crypter les données envoyées et décrypter les données reçues. Recommandé si transfert sur internet.")))
    ...
    def Validation(self):
        if self.check_activer.GetValue() == True and self.ctrl_mdp.GetValue() == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un mot de passe valide pour le cryptage !"), "Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
...
class Page_archivage(wx.Panel):
    def __init__(self, parent):
        ...
        self.label_activer = wx.StaticText(self, -1, _("Activer :"))
        self.check_activer = wx.CheckBox(self, -1, "")
        self.label_info = wx.StaticText(self, -1, _("(Au-delà du nombre de jours indiqués, les fichiers de synchronisation archivés seront supprimés)"))
        self.label_info.SetFont(wx.Font(8, wx.SWISS, wx.NORMAL, wx.NORMAL, False))
        self.label_info.SetForegroundColour(wx.Colour(150, 150, 150))
        self.label_delai = wx.StaticText(self, -1, _("Nbre jours :"))
        self.ctrl_delai = wx.SpinCtrl(self, -1, "30")
        
        # Propriétés
        self.check_activer.SetToolTip(wx.ToolTip(_("Cochez cette case pour activer la suppression automatique des fichiers de synchronisation archivés obsolètes")))
        self.ctrl_delai.SetToolTip(wx.ToolTip(_("Sélectionnez un nombre de jours")))
    ...
    def Validation(self):
        if self.check_activer.GetValue() == True and self.ctrl_delai.GetValue() == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un nombe de jours !"), "Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
...
class Notebook(wx.Notebook):
    def __init__(self, parent):
        ...
        self.listePages = [
            (_("serveur"), _("Serveur WIFI/Internet"), _("Page_serveur(self)"), "Server.png"),
            (_("ftp"), _("FTP"), _("Page_ftp(self)"), "Ftp.png"),
            (_("cryptage"), _("Cryptage"), _("Page_cryptage(self)"), "Cle.png"),
            (_("archivage"), _("Archivage"), _("Page_archivage(self)"), "Poubelle.png"),
            ]
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        # Bandeau
        intro = _("Vous pouvez ici synchroniser les données avec Nomadhys, l'application nomade pour Noethys. Choisissez une méthode de synchronisation (Serveur direct WIFI/Internet ou FTP ou Manuel) puis renseignez les paramètres. Cliquez sur le bouton 'Lire les données' pour importer les données.")
        titre = _("Synchroniser Nomadhys")
        self.SetTitle(titre)
        ...
        # Transfert
        self.box_transfert = wx.StaticBox(self, -1, _("Transfert manuel"))
        self.ctrl_mode = CTRL_Mode(self)
        self.bouton_envoyer = CTRL_Bouton_image.CTRL(self, texte=_("Envoyer"), cheminImage="Images/32x32/Fleche_haut.png")
        self.bouton_recevoir = CTRL_Bouton_image.CTRL(self, texte=_("Recevoir"), cheminImage="Images/32x32/Fleche_bas.png")

        # Lecture
        self.box_lecture = wx.StaticBox(self, -1, _("Importation des données"))
        ...
        self.check_archives = wx.CheckBox(self, -1, _("Afficher les archives"))
        ...
        self.bouton_googleplay = CTRL_Bouton_image.CTRL(self, texte=_("Télécharger Nomadhys"), cheminImage="Images/32x32/Googleplay.png")
    ...
    def __set_properties(self):
        self.ctrl_mode.SetToolTip(wx.ToolTip(_("Sélectionnez le mode de transfert souhaité pour envoyer/recevoir des données manuellement")))
        self.check_archives.SetToolTip(wx.ToolTip(_("Cochez cette case pour afficher également dans la liste les fichiers de synchronisation archivés")))
        self.bouton_envoyer.SetToolTip(wx.ToolTip(_("Cliquez ici pour envoyer manuellement le fichier de données")))
        self.bouton_recevoir.SetToolTip(wx.ToolTip(_("Cliquez ici pour recevoir manuellement un fichier de données")))
        self.bouton_actualiser.SetToolTip(wx.ToolTip(_("Cliquez ici pour actualiser la liste des fichiers à synchroniser")))
        self.bouton_lecture.SetToolTip(wx.ToolTip(_("Cliquez ici pour lire le contenu des fichiers sélectionnés")))
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez ici pour obtenir de l'aide")))
        self.bouton_googleplay.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à la page de téléchargement de Nomadhys sur Google Play")))
        self.bouton_outils.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à des outils")))
    ...
    def OnBoutonOutils(self, event):
        ...
        item = wx.MenuItem(menuPop, 10, _("Purger le répertoire FTP"), _("Supprimer uniquement les fichiers liés à ce fichier de données du répertoire FTP"))
    ...
    def On_outils_purger_ftp(self, event):
        ...
        try :
            ...
            print(err)
            dlg = wx.MessageDialog(self, _("La connexion FTP n'a pas pu être établie !\n\nVérifiez les paramètres de connexion FTP dans les paramètres de synchronisation."), "Erreur ", wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        
        dlg = wx.MessageDialog(self, _("%d fichiers ont été supprimés dans le répertoire FTP !") % nbreFichiersSupprimes, "Suppression ", wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
    ...
    def RecevoirFichier(self):
        ...
        wildcard = _("Fichiers de synchronisation (*%s, *%s)|*%s;*%s|Tous les fichiers (*.*)|*.*") % (UTILS_Export_nomade.EXTENSION_CRYPTE, UTILS_Export_nomade.EXTENSION_DECRYPTE, UTILS_Export_nomade.EXTENSION_CRYPTE, UTILS_Export_nomade.EXTENSION_DECRYPTE)
        dlg = wx.FileDialog(None, message=_("Sélectionnez un fichier de synchronisation"), defaultDir=standardPath.GetDocumentsDir(),  wildcard=wildcard, style=wx.FD_OPEN)
        chemin = None
        ...
        if chemin != None :
            ...
            # Analyse du fichier
            dlgAttente = wx.BusyInfo(_("Analyse du fichier synchronisation..."), self)
            resultat = AnalyserFichier(nomFichier, typeTransfert="manuel") 
            del dlgAttente
            if resultat == True :
                dlg = wx.MessageDialog(self, _("Le fichier a été réceptionné avec succès !"), "Succès", wx.OK | wx.ICON_INFORMATION)
                dlg.ShowModal()
                dlg.Destroy()
    ...
    def RecevoirFTP(self):
        ...
        listeFichiersRecus = []
        dlgAttente = wx.BusyInfo(_("Récupération des fichiers de synchronisation depuis un répertoire FTP..."), self)
        try :
            ftp = ftplib.FTP(hote, identifiant, mdp)
            ftp.cwd(repertoire)
            # Récupère la liste des fichiers de synchronisation présents sur le répertoire FTP
    ...
    def OnBoutonLecture(self, event):
        ...
        if len(tracks) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun fichier de synchronisation à lire !"), "Erreur ", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
