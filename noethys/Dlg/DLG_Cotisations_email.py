...
from Ctrl import CTRL_Liste_cotisations
from Ctrl import CTRL_Cotisations_options
...
class Dialog(wx.Dialog):
    def __init__(self, parent, filtres=[]):
        ...
        # Bandeau
        intro = _("Cochez les cotisations à envoyer par Email puis cliquez sur le bouton 'Transférer vers l'éditeur d'Emails'.")
        titre = _("Envoi de cotisations par Email")
        self.SetTitle(titre)
        ...
        # Cotisations
        self.box_cotisations_staticbox = wx.StaticBox(self, -1, _("Liste des cotisations"))
        self.ctrl_liste_cotisations = CTRL_Liste_cotisations.CTRL(self, filtres=filtres)
        
        # Options
        self.box_options_staticbox = wx.StaticBox(self, -1, _("Options des cotisations"))
    ...
    def __set_properties(self):
        ...
        self.bouton_ok.SetToolTip(wx.ToolTip(_("Cliquez ici pour transférer les cotisations vers l'éditeur d'Emails")))
    ...
    def OnBoutonOk(self, event): 
        ...
        if len(tracks) == 0 : 
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune cotisation à envoyer par Email !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
