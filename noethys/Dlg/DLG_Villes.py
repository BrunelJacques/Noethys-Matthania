...
class Dialog(wx.Dialog):
    def __init__(self, parent, modeImportation=False):
        ...
        titre = _("Gestion villes et codes postaux")
        if modeImportation == True :
            intro = _("Vous pouvez ici rechercher une ville ou un code postal grâce à la barre de recherche. Cliquez sur 'Choisir' pour importer la ville saisie ou sélectionnée.")
        else:
            intro =_("Vous pouvez ici rechercher une ville ou un code postal. La barre de recherche vous permet d'effectuer une recherche sur une partie du nom ou du code postal.")
        ...
        self.bouton_importer = CTRL_Bouton_image.CTRL(self, texte=_("Choisir la ville"), cheminImage="Images/32x32/Fleche_bas.png")
    ...
    def __set_properties(self):
        ...
        self.ctrl_cp.SetToolTip(wx.ToolTip(_("Saisissez ici un code postal")))
        self.ctrl_ville.SetToolTip(wx.ToolTip(_("Saisissez ici le nom d'une ville")))
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter une ville")))
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier la ville sélectionnée dans la liste")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer la ville sélectionnée dans la liste")))
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez ici pour obtenir de l'aide")))
        self.bouton_importer.SetToolTip(wx.ToolTip(_("Cliquez ici pour importer la ville saisie manuellement ou sélectionnée dans la liste")))
    ...
    def OnBoutonChoisir(self, event):
        ...
        if saisie_ville == "" and saisie_cp == "" and len(selectionListe) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez effectué aucune saisie manuelle ou sélection dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        if saisie_ville != "" and saisie_cp == "":
            dlg = wx.MessageDialog(self, _("Vous avez saisi le nom d'une ville sans saisir le code postal !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        if saisie_ville == "" and saisie_cp != "":
            dlg = wx.MessageDialog(self, _("Vous avez saisi le code postal sans saisir le nom de la ville !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
