...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDtype_cotisation=None, premiereCotisation=False):
        ...
        self.ctrl_type = wx.Choice(self, -1, choices=[_("Cotisation familiale"), _("Cotisation individuelle")])
        self.ctrl_type.Select(0)
        self.ctrl_carte = wx.CheckBox(self, -1, _("Est représentée par une carte d'adhérent"))
        ...
        if self.IDtype_cotisation == None :
            self.SetTitle(_("Saisie d'un type de cotisation"))
            self.ctrl_unites.MAJ() 
        else:
            self.SetTitle(_("Modification d'un type de cotisation"))
            self.Importation()
            
            

    def __set_properties(self):
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez ici le nom de la cotisation")))
        self.ctrl_type.SetToolTip(wx.ToolTip(_("Sélectionnez ici le type de la cotisation")))
        self.ctrl_carte.SetToolTip(wx.ToolTip(_("Cochez cette case si la cotisation est représentée par une carte d'adhérent")))
        self.bouton_ajouter_unite.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter une unité")))
        self.bouton_modifier_unite.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier l'unité sélectionnée dans la liste")))
        self.bouton_supprimer_unite.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer l'unité sélectionnée dans la liste")))
        self.bouton_defaut.SetToolTip(wx.ToolTip(_("Cliquez ici pour définir l'unité de cotisation sélectionnée comme celle par défaut")))
        self.ctrl_code_comptable.SetToolTip(wx.ToolTip(_("Saisissez un code comptable pour cette cotisation si vous souhaitez exporter les écritures comptables vers des logiciels de compta")))
    ...
    def OnBoutonOk(self, event):
        ...
        if nom == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un nom pour ce type de cotisation !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        for dictUnite in listeUnites :
            if dictUnite["etat"] != "SUPPR" :
                nbreUnites += 1
        if nbreUnites == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir au moins une unité de cotisation !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
