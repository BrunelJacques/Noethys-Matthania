...
class Track_excel(object):
    def __init__(self, parent=None, ctrl_log=None, dict_champs={}, dict_ligne={}):
        ...
        for num_colonne, valeur in dict_ligne.items():
            ...
            # OD Matricule
            if "Matricule" in nom_colonne:
                self.od_matricule = valeur

            # OD Civilité
            if "Libellé civilité" in nom_colonne:
                if "Monsieur" in valeur:
                    self.od_civilite = 1
                if "Mademoiselle" in valeur:
                    self.od_civilite = 2
                if "Madame" in valeur:
                    self.od_civilite = 3

            # OD Nom et prénom
            if "Nom+Prénom OD" in nom_colonne:
                valeur_temp = valeur.split("  ")
                if len(valeur_temp) >= 2:
                    self.od_nom = valeur_temp[0]
                    self.od_prenom = valeur_temp[1]
                    self.od_nom_complet = "%s %s" % (self.od_nom, self.od_prenom)

            # OD Date de naissance
            if "Date de naissance OD" in nom_colonne:
                self.od_date_naiss = valeur

            # OD Rue
            if "Adresse domicile 1" in nom_colonne:
                if len(valeur) > 0:
                    self.od_rue += valeur
            if "Adresse domicile 2" in nom_colonne:
                if len(valeur) > 0:
                    self.od_rue += " - " + valeur
            if "Adresse domicile 3" in nom_colonne:
                if len(valeur) > 0:
                    self.od_rue += " - " + valeur
            self.od_rue = self.od_rue.replace('"', "")

            # OD Code postal
            if "Code postal domicile" in nom_colonne:
                self.od_code_postal = valeur

            # OD Ville
            if "Ville domicile" in nom_colonne:
                self.od_ville = valeur

            # OD Téléphone 1
            if nom_colonne == "Téléphone personnel":
                valeur = valeur.replace(" ", "").replace(".", "")
                if len(valeur) == 10:
                    self.od_telephone_1 = "%s.%s.%s.%s.%s." % (valeur[0:2], valeur[2:4], valeur[4:6], valeur[6:8], valeur[8:10])

            # OD Téléphone 2
            if "Téléphone personnel 2" in nom_colonne:
                valeur = valeur.replace(" ", "").replace(".", "")
                if len(valeur) == 10:
                    self.od_telephone_2 = "%s.%s.%s.%s.%s." % (valeur[0:2], valeur[2:4], valeur[4:6], valeur[6:8], valeur[8:10])

            # OD Mail personnel
            if "E-Mail personnel" in nom_colonne:
                self.od_mail_perso = valeur

            # OD Mail interne
            if "E-Mail interne" in nom_colonne:
                self.od_mail_travail = valeur

            # ------------------------------ AD -------------------------------------
            # AD Nom et prénom
            if "Nom+Prénom AD" in nom_colonne:
                valeur_temp = valeur.split("  ")
                if len(valeur_temp) >= 2:
                    self.ad_nom = valeur_temp[0]
                    self.ad_prenom = valeur_temp[1]
                    self.ad_nom_complet = "%s %s" % (self.ad_nom, self.ad_prenom)

            # AD Civilité
            if "Sexe Ayant-droit" in nom_colonne or "Sexe AD" in nom_colonne:
                if "M" in valeur:
                    self.ad_civilite = 4
                if "F" in valeur:
                    self.ad_civilite = 5
            ...
            if "Date de naissance AD" in nom_colonne:
...
class CTRL_Categories(wx.CheckListBox):
    ...
    def MAJ(self):
        ...
        liste_categories = [
            ("ajouter_famille", "Ajouter une famille"),
            ("modifier_civilite",  "Modifier la civilité"),
            ("modifier_rue",  "Modifier la rue"),
            ("modifier_cp",  "Modifier le code postal"),
            ("modifier_ville",  "Modifier la ville"),
            ("modifier_tel_domicile", "Modifier le tél domicile"),
            ("modifier_tel_portable",  "Modifier le tél portable"),
            ("modifier_mail_perso",  "Modifier le mail perso"),
            ("modifier_mail_travail",  "Modifier le mail pro"),
            ("modifier_internet_identifiant", "Modifier l'identifiant internet"),
            ("modifier_internet_mdp",  "Modifier le mot de passe internet"),
            ("ajouter_individu",  "Ajouter un individu AD"),
            ("modifier_enfant", "Modifier un individu AD"),
        ]
        self.data = []
        index = 0
...
class CTRL_Donnees(FastObjectListView):
    ...
    def InitObjectListView(self):
        ...
        liste_colonnes = [
            ColumnDefn(_("Ouvrant-droit"), 'left', 200, "od_nom_complet", typeDonnee="texte"),
            ColumnDefn(_("Action"), 'left', 270, "label", typeDonnee="texte"),
            ColumnDefn(_("Ancienne valeur"), 'left', 180, "ancienne_valeur", typeDonnee="texte", stringConverter=FormateValeur),
            ColumnDefn(_("Nouvelle valeur"), 'left', 180, "nouvelle_valeur", typeDonnee="texte", stringConverter=FormateValeur),
            ]
    ...
    def OnCheck(self, event=None):
        texte = _("Sélection des modifications à appliquer (%d)") % len(self.GetTracksCoches())
    ...
    def OnContextMenu(self, event):
        ...
        item = wx.MenuItem(menuPop, 10, _("Ouvrir la fiche famille correspondante"))
        ...
        self.GenerationContextMenu(menuPop, titre=_("Liste des modifications"))
    ...
    def Analyse_fichier(self):
        ...
        if len(self.liste_categories) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement cocher au moins une catégorie !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        try:
            ...
            dlg = wx.MessageDialog(self, _("Impossible d'ouvrir ce fichier Excel !"), _("Erreur"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        champs_obligatoires = [
            "Matricule Ouvrant-droit", "Libellé civilité", "Nom+Prénom OD", "Date de naissance OD", "Adresse domicile 1",
            "Adresse domicile 2", "Adresse domicile 3", "Code postal domicile", "Ville domicile",
            "Téléphone personnel", "Téléphone personnel 2", "E-Mail personnel", "E-Mail interne",
            "Nom+Prénom AD", "Sexe AD", "Date de naissance AD",
            ]
        champs_manquants = []
        ...
        if len(champs_manquants) > 0:
            self.ctrl_log.Ecrit(u"[ERREUR] Il manque les champs suivants dans le fichier Excel : %s." % ", ".join(champs_manquants))
            dlg = wx.MessageDialog(self, _("Il manque les champs suivants dans le fichier Excel :\n\n- %s") % "\n- ".join(champs_manquants), _("Erreur"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        for num_ligne in range(1, feuille.nrows):
            ...
            if len(track.liste_anomalies) == 0:
                ...
                self.ctrl_log.Ecrit(u"%d anomalies sur la ligne %d : %s" % (len(track.liste_anomalies), num_ligne, " / ".join(track.liste_anomalies)))
        ...
        for IDindividu, IDcivilite, nom, prenom, date_naiss, adresse_auto, rue_resid, cp_resid, ville_resid, tel_domicile, tel_mobile, mail, travail_tel, travail_mail in liste_individus:
            ...
            nom_complet = nom
            if prenom != None: nom_complet += " " + prenom
        ...
        for track in dict_tracks.values():
            ...
            if IDindividu_od == None:
                label = "Création d'une nouvelle famille"
            ...
            if IDindividu_od != None:
                ...
                if dict_individu["adresse_auto"] == None and dict_individu["cp_resid"] != track.GetValeur("od_code_postal"):
                    label = "Changement de code postal"
                ...
                if dict_individu["tel_domicile"] != track.GetValeur("od_telephone_1") and track.GetValeur("od_telephone_1") != "":
                    label = "Changement de téléphone fixe"
                ...
                if dict_individu["tel_mobile"] != track.GetValeur("od_telephone_2") and track.GetValeur("od_telephone_2") != "":
                    label = "Changement de téléphone mobile"
                ...
                if dict_individu["mail"] != track.GetValeur("od_mail_perso") and track.GetValeur("od_mail_perso") != "":
                    label = "Changement de mail personnel"
                ...
                if dict_individu["travail_mail"] != track.GetValeur("od_mail_travail") and track.GetValeur("od_mail_travail") != "":
                    label = "Changement de mail professionnel"
                ...
                if IDfamille != None:
                    ...
                    if internet_identifiant != track.GetValeur("od_matricule"):
                        label = "Changement d'identifiant internet"
                ...
                if IDfamille != None:
                    ...
                    if internet_mdp.startswith("custom") == False and internet_mdp != mdp_defaut:
                        label = "Changement de mot de passe internet"
                ...
                if IDfamille != None:
                    for dict_ad in track.liste_ad:
                        ...
                        if IDindividu_ad == None:
                            label = "Création de l'individu %s" % dict_ad["nom_complet"]
...
class Panel(wx.Panel):
    def __init__(self, parent):
        ...
        self.box_fichier_staticbox = wx.StaticBox(self, -1, _("Sélection du fichier"))
        self.label_fichier = wx.StaticText(self, -1, _("Cliquez sur le bouton 'Sélectionner' pour sélectionner le fichier de données :"))
        wildcard = _("Fichiers Excel (*.xls)|*.xls|Tous les fichiers (*.*)|*.*")
        sp = wx.StandardPaths.Get()
        self.ctrl_fichier = filebrowse.FileBrowseButton(self, -1, labelText=_("Fichier à importer :"), buttonText=_("Sélectionner"), toolTip=_("Cliquez ici pour sélectionner un fichier de données"), dialogTitle=_("Sélectionner un fichier"), fileMask=wildcard, startDirectory=sp.GetDocumentsDir(), changeCallback=self.OnSelectionFichier)

        # Catégories des modifications
        self.box_categories_staticbox = wx.StaticBox(self, -1, _("Catégories des modifications"))
        self.ctrl_categories = CTRL_Categories(self)
        self.ctrl_categories.SetMinSize((250, 20))
        self.ctrl_appliquer_categories = wx.Button(self, -1, "Appliquer")

        # Liste de données
        self.box_donnees_staticbox = wx.StaticBox(self, -1, _("Sélection des modifications à appliquer"))
        self.label_donnees = wx.StaticText(self, -1, _("Cochez les données à modifier puis cliquez sur le bouton Appliquer :"))
        ...
        # Log
        self.box_log_staticbox = wx.StaticBox(self, -1, _("Journal"))
...
class CTRL_Log(wx.TextCtrl):
    def __init__(self, parent):
        wx.TextCtrl.__init__(self, parent, wx.ID_ANY, "", style=wx.TE_MULTILINE | wx.TE_READONLY | wx.TE_RICH2)
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        # Intro
        intro = _("Cet assistant permet d'importer des individus depuis un fichier Excel.")
        ...
        self.bouton_ok = CTRL_Bouton_image.CTRL(self, texte=_("Synchroniser"), cheminImage="Images/32x32/Valider.png")
    ...
    def __set_properties(self):
        ...
        self.bouton_fermer.SetToolTip(wx.ToolTip(_("Cliquez pour annuler")))
        self.bouton_ok.SetToolTip(wx.ToolTip(_("Cliquez pour lancer la synchronisation")))
    ...
    def OnBoutonOk(self, event):
        ...
        dlg = wx.MessageDialog(self, _("Confirmez-vous la synchronisation de %d informations ?\n\nAttention, il est conseillé de procéder à une sauvegarde des données auparavant.") % len(tracks), _("Confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
