...
class Dialog(wx.Dialog):
    ...
    def __set_properties(self):
        self.ctrl_label.SetToolTip(wx.ToolTip(_("Saisissez un label pour cette étiquette")))
        self.ctrl_couleur.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner une couleur pour cette étiquette")))
        self.ctrl_active.SetToolTip(wx.ToolTip(_("Décochez cette case pour empêcher l'utilisateur de sélectionner cette étiquette. Cette option peut par exemple servir pour créer une étiquette de regroupement pour des sous-étiquettes ou pour désactiver une étiquette obsolète.")))
    ...
    def OnBoutonOk(self, event): 
        ...
        if len(self.ctrl_label.GetValue()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un label pour cette étiquette !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
...
##            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner une couleur en cliquant sur le bouton couleur."), "Information", wx.OK | wx.ICON_INFORMATION)
##            dlg.ShowModal()
##            dlg.Destroy()
...
##            return
        ...
        if self.GetIDparent() == (None, None) :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une étiquette parente !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
    ...
    def SetLabel(self, texte=""):
        self.SetTitle(_("Modification d'une étiquette"))
        if texte != None :
