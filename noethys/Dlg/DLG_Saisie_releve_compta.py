...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDreleve=None, IDcompte_bancaire=None):
        ...
        if self.IDreleve != None :
            self.SetTitle(_("Modification d'un relevé bancaire"))
            self.Importation() 
        else :
            self.SetTitle(_("Saisie d'un relevé bancaire"))
    ...
    def __set_properties(self):
        self.ctrl_compte.SetToolTip(wx.ToolTip(_("Sélectionnez le compte bancaire associé")))
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez le nom du relevé bancaire (Ex : 'Janvier 2014')")))
        self.ctrl_date_debut.SetToolTip(wx.ToolTip(_("Saisissez la date de début du relevé")))
        self.ctrl_date_fin.SetToolTip(wx.ToolTip(_("Saisissez la date de fin du relevé")))
    ...
    def Sauvegarde(self):
        ...
        if IDcompte == None : 
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner un compte bancaire !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if date_debut > date_fin :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de fin supérieure à la date de début !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_date_fin.SetFocus()
            return False
        ...
        if len(listeOperations) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez rapproché aucune opération.\n\nSouhaitez-vous tout de même valider ?"), _("Avertissement"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
