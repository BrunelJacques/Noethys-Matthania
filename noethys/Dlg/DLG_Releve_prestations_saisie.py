...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        # Type
        self.box_type_staticbox = wx.StaticBox(self, -1, _("Type"))
        self.radio_type_prestations = wx.RadioButton(self, -1, _("Prestations"), style=wx.RB_GROUP)
        self.radio_type_factures = wx.RadioButton(self, -1, _("Factures"))
        
        # Options
        self.box_options_staticbox = wx.StaticBox(self, -1, _("Options"))
        self.check_impayes = wx.CheckBox(self, -1, _("Uniquement les impayés"))
        self.check_conso = wx.CheckBox(self, -1, _("Détailler les consommations"))
        self.check_regroupement = wx.CheckBox(self, -1, _("Regrouper par"))
        self.ctrl_regroupement_date = wx.Choice(self, -1, choices=["Date", _("Mois"), _("Année")])
        
        # Période
        self.box_periode_staticbox = wx.StaticBox(self, -1, _("Période"))
        self.radio_tout = wx.RadioButton(self, -1, _("Toutes les périodes"), style=wx.RB_GROUP)
        self.radio_dates = wx.RadioButton(self, -1, "Du")
        self.ctrl_date_debut = CTRL_Saisie_date.Date2(self)
        self.label_au = wx.StaticText(self, -1, _("au"))
        self.ctrl_date_fin = CTRL_Saisie_date.Date2(self)
        self.radio_apartirde = wx.RadioButton(self, -1, _("A partir du"))
        self.ctrl_date_apartirde = CTRL_Saisie_date.Date2(self)
        self.radio_jusquau = wx.RadioButton(self, -1, _("Jusqu'au"))
        self.ctrl_date_jusquau = CTRL_Saisie_date.Date2(self)
        self.radio_mois = wx.RadioButton(self, -1, _("Mois de"))
        ...
        self.ctrl_annee = wx.SpinCtrl(self, -1, "2012", min=1900, max=2099)
        self.radio_vacances = wx.RadioButton(self, -1, _("Vacances de"))
        self.ctrl_vacances = wx.Choice(self, -1, choices=[_("Février"), _("Pâques"), _("Eté"), _("Toussaint"), _("Noël")])
        self.ctrl_vacances_annee = wx.SpinCtrl(self, -1, "2012", min=1900, max=2099)
    ...
    def __set_properties(self):
        self.SetTitle(_("Saisie d'une période"))
        self.radio_type_prestations.SetToolTip(wx.ToolTip(_("Sélectionnez le type de données à afficher")))
        self.radio_type_factures.SetToolTip(wx.ToolTip(_("Sélectionnez le type de données à afficher")))
        self.check_impayes.SetToolTip(wx.ToolTip(_("Uniquement les impayés")))
        self.check_regroupement.SetToolTip(wx.ToolTip(_("Regroupement des prestations")))
        self.ctrl_regroupement_date.SetSelection(0)
        self.check_conso.SetToolTip(wx.ToolTip(_("Détailler les consommations")))
    ...
    def GetPeriode(self):
        ...
        if self.radio_tout.GetValue() == True :
            ...
            parametres = {"code":"tout", "date_debut":date_debut, "date_fin":date_fin, "label":_("Toutes les périodes")}

        # Dates
        if self.radio_dates.GetValue() == True :
            date_debut = self.ctrl_date_debut.GetDate()
            date_fin = self.ctrl_date_fin.GetDate()
            parametres = {"code":"dates", "date_debut":date_debut, "date_fin":date_fin, "label":_("Du %s au %s") % (DateEngFr(str(date_debut)), DateEngFr(str(date_fin))) }
        
        # A partir de
        if self.radio_apartirde.GetValue() == True :
            date_debut = self.ctrl_date_apartirde.GetDate() 
            date_fin = datetime.date(2999, 1, 1)
            parametres = {"code":"apartirde", "date_debut":date_debut, "date_fin":date_fin, "label":_("A partir du %s") % DateEngFr(str(date_debut)) }
            
        # Jusqu'au
        if self.radio_jusquau.GetValue() == True :
            date_debut = datetime.date(1900, 1, 1)
            date_fin = self.ctrl_date_jusquau.GetDate() 
            parametres = {"code":"jusquau", "date_debut":date_debut, "date_fin":date_fin, "label":_("Jusqu'au %s") % DateEngFr(str(date_fin)) }
        ...
        if self.radio_mois.GetValue() == True :
            ...
            parametres = {"code":"mois", "mois":mois, "annee":annee, "date_debut":date_debut, "date_fin":date_fin, "label":_("Mois de %s %d") % (LISTE_MOIS[mois-1], annee) }
        ...
        if self.radio_annee.GetValue() == True :
            ...
            parametres = {"code":"annee", "annee":annee, "date_debut":date_debut, "date_fin":date_fin, "label":_("Année %d") % annee }
        ...
        if self.radio_vacances.GetValue() == True :
            ...
            parametres = {"code":"vacances", "nom":nom, "annee":annee, "date_debut":date_debut, "date_fin":date_fin, "label":_("Vacances de %s %d") % (nom, annee)}
    ...
    def OnBoutonOk(self, event): 
        ...
        if parametres["date_debut"] == None or parametres["date_fin"] == None :
            dlg = wx.MessageDialog(self, _("La période sélectionnée ne semble pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
