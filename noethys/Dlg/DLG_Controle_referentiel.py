...
class Dialog(wx.Dialog):
    def __init__(self, parent, dictParametres={}):
        ...
        # Bandeau
        intro = _("La recherche est totalement automatisée mais vous pouvez effectuer une recherche manuelle en modifiant les paramètres de recherche puis en cliquant sur Rechercher.")
        titre = _("Contrôle des coordonnées")
        self.SetTitle(titre)
        ...
        self.label_prenom = wx.StaticText(self, wx.ID_ANY, _("Prénom :"))
        self.ctrl_prenom = wx.TextCtrl(self, -1, "")
        self.label_adresse = wx.StaticText(self, wx.ID_ANY, _("Adresse :"))
        self.ctrl_adresse = wx.TextCtrl(self, -1, "")

        self.bouton_valider = wx.Button(self, -1, _("Rechercher"))
    ...
    def __set_properties(self):
        self.bouton_valider.SetToolTip(wx.ToolTip(_("Cliquez ici pour valider les paramètres de la recherche")))
    ...
    def Rechercher(self, event=None):
        ...
        if nom == "" :
            dlg = wx.MessageDialog(self, _("Vous devez renseigner le nom !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if prenom == "" :
            dlg = wx.MessageDialog(self, _("Vous devez renseigner le prénom !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if adresse == "" :
            dlg = wx.MessageDialog(self, _("Vous devez renseigner l'adresse !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if url == None :
            dlg = wx.MessageDialog(self, _("Vous devez renseigner l'URL dans le fichier Customize !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        url = "%s/%s* %s*" % (url, nom, prenom)
        ...
        if reponse.status_code != 200:
            self.ctrl_resultats.SetTexte(_("Accès au référentiel impossible (Erreur %s)") % reponse.status_code)
            return
        ...
        for dictTemp in listeResultats :
            texte1 = "%s#%s#%s" % (dictTemp["nom"], dictTemp["prenom"], dictTemp["adresse"])
            texte2 = "%s#%s#%s" % (nom, prenom, adresse)
        ...
        if nbreResultats > 0 :
            texte = _("<FONT SIZE=5><B>%d résultats :</B><BR></FONT><BR>") % nbreResultats
        else :
            texte = _("<FONT SIZE=5><B>Aucun résultat</B><BR></FONT><BR>")
