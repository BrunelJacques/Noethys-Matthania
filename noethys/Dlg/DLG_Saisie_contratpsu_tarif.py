...
class CTRL_Montant(wx.TextCtrl):
    ...
    def Validation(self):
        ...
        try :
            ...
            message = _("Le montant que vous avez saisi n'est pas valide.")
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDfamille=None):
        ...
        self.staticbox_periode_staticbox = wx.StaticBox(self, -1, _("Date d'application"))
        self.label_date_debut = wx.StaticText(self, -1, _("A partir du :"))
        self.ctrl_date_debut = CTRL_Saisie_date.Date2(self)

        # Ressources
        self.staticbox_ressources_staticbox = wx.StaticBox(self, -1, _("Ressources"))
        self.label_revenu = wx.StaticText(self, -1, _("Revenu :"))
        self.ctrl_revenu = CTRL_Saisie_euros.CTRL(self)
        self.label_qf = wx.StaticText(self, -1, _("Quotient familial :"))
        self.ctrl_qf = wx.TextCtrl(self, -1, "")

        # Tarifs
        self.staticbox_tarifs_staticbox = wx.StaticBox(self, -1, _("Tarifs"))
        self.label_suggere = wx.StaticText(self, -1, _("Suggéré"))
        self.label_retenu = wx.StaticText(self, -1, _("Retenu"))

        self.label_suggere.SetForegroundColour((180, 180, 180))
        self.label_retenu.SetForegroundColour((180, 180, 180))

        self.label_taux = wx.StaticText(self, -1, _("Taux d'effort :"))
        self.ctrl_taux_suggere = CTRL_Montant(self)
        self.ctrl_taux_retenu = CTRL_Montant(self)
        self.ctrl_taux_suggere.Enable(False)

        self.label_base = wx.StaticText(self, -1, _("Tarif de base :"))
        self.ctrl_base_suggere = CTRL_Montant(self)
        self.ctrl_base_retenu = CTRL_Montant(self)
        self.ctrl_base_suggere.Enable(False)

        self.label_depassement = wx.StaticText(self, -1, _("Tarif dépassement :"))
    ...
    def __set_properties(self):
        self.SetTitle(_("Saisie d'un tarif de contrat"))
        self.ctrl_date_debut.SetToolTip(wx.ToolTip(_("Saisissez la date de début d'application")))
        self.ctrl_revenu.SetToolTip(wx.ToolTip(_("Saisissez le revenu pris en compte")))
        self.ctrl_qf.SetToolTip(wx.ToolTip(_("Saisissez le quotient familial pris en compte")))
        self.ctrl_taux_suggere.SetToolTip(wx.ToolTip(_("Taux calculé automatiquement")))
        self.ctrl_taux_retenu.SetToolTip(wx.ToolTip(_("Saisissez le taux retenu")))
        self.ctrl_base_suggere.SetToolTip(wx.ToolTip(_("Tarif de base calculé automatiquement")))
        self.ctrl_base_retenu.SetToolTip(wx.ToolTip(_("Saisissez le tarif de base")))
        self.ctrl_depassement_suggere.SetToolTip(wx.ToolTip(_("Tarif de dépassement calculé automatiquement")))
        self.ctrl_depassement_retenu.SetToolTip(wx.ToolTip(_("Saisissez le tarif de dépassement")))
    ...
    def __do_layout(self):
        ...
        grid_sizer_tarifs.Add(self.ctrl_taux_suggere, 0, wx.EXPAND, 0)
        grid_sizer_tarifs.Add(self.ctrl_taux_retenu, 0, wx.EXPAND, 0)

        grid_sizer_tarifs.Add(self.label_base, 0, wx.ALIGN_RIGHT|wx.ALIGN_CENTER_VERTICAL, 0)
        grid_sizer_tarifs.Add(self.ctrl_base_suggere, 0, wx.EXPAND, 0)
        grid_sizer_tarifs.Add(self.ctrl_base_retenu, 0, wx.EXPAND, 0)

        grid_sizer_tarifs.Add(self.label_depassement, 0, wx.ALIGN_RIGHT|wx.ALIGN_CENTER_VERTICAL, 0)
        grid_sizer_tarifs.Add(self.ctrl_depassement_suggere, 0, wx.EXPAND, 0)
        grid_sizer_tarifs.Add(self.ctrl_depassement_retenu, 0, wx.EXPAND, 0)
    ...
    def OnBoutonOk(self, event):
        if self.ctrl_date_debut.Validation() == False or self.ctrl_date_debut.GetDate() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de début d'application valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.ctrl_revenu.Validation() == False :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement un montant de revenu valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if len(self.ctrl_qf.GetValue()) > 0 :
            try :
                ...
                dlg = wx.MessageDialog(self, _("Le quotient familial que vous avez saisi ne semble pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
        ...
        #         dlg = wx.MessageDialog(self, _("Attention, si vous modifiez la date de début de ce tarif, la période du contrat ne disposera pas de tarif disponible au premier jour du contrat !\n\nSouhaitez-vous tout de même modifier ce tarif ?"), _("Avertissement"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
