...
class Notebook(wx.Notebook):
    def __init__(self, parent, IDactivite=None, IDtarif=None, nouveauTarif=False, choix_pages=None, cacher_dates=False, track_tarif=None):
        ...
        self.listePages = [
            ("generalites", _("Généralités"), "CTRL_Tarification_generalites.Panel(self, IDactivite=IDactivite, IDtarif=IDtarif, cacher_dates=cacher_dates)", "Information.png"),
            ("conditions", _("Conditions d'application"), "CTRL_Tarification_conditions.Panel(self, IDactivite=IDactivite, IDtarif=IDtarif)", "Filtre.png"),
            ("type", _("Type de tarif"), "CTRL_Tarification_type.Panel(self, IDactivite=IDactivite, IDtarif=IDtarif, nouveauTarif=nouveauTarif)", "Outils.png"),
            ("calcul", _("Calcul du tarif"), "CTRL_Tarification_calcul.Panel(self, IDactivite=IDactivite, IDtarif=IDtarif, track_tarif=track_tarif)", "Calculatrice.png"),
            ]
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDactivite=None, IDtarif=None, IDnom_tarif=None, nom_tarif=None, choix_pages=None, cacher_dates=False, track_tarif=None):
        ...
        intro = _("Renseignez les caractéristiques générales, les conditions d'application, le type de tarif puis le mode de calcul du tarif. Vous pouvez consulter le Guide de démarrage ou le manuel de référence pour en savoir davantage...")
        titre = _("Paramétrage du tarif '%s'") % self.nom_tarif
        if self.IDtarif != None :
            titre += " (ID%d)" % self.IDtarif
        self.SetTitle(titre)
    ...
    def GetNomTarif(self):
        ...
        if self.IDnom_tarif == None : return ""
        db = GestionDB.DB()
    ...
    def Sauvegarde(self):
        ...
        if typeTarif != None and typeTarif not in tarifsCompatibles and self.track_tarif == None :
            dlg = wx.MessageDialog(self, _("La méthode de calcul sélectionnée n'est pas compatible avec le type de tarif sélectionné !"), _("Information"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
    ...
    def OnBoutonAnnuler(self, event): 
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment annuler ?"), _("Annulation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
