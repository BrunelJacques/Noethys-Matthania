...
class CTRL_Parametres(DLG_Saisie_lot_tresor_public.CTRL_Parametres):
    ...
    def Remplissage(self):
        ...
        propriete.SetHelpString(_("Saisissez l'objet du bordereau (Ex : 'Cantine décembre 2020')"))
        self.Append(propriete)
        ...
        propriete = wxpg.StringProperty(label=_("Poste par défaut"), name="id_poste", value=u"")
        propriete.SetHelpString(_("Saisissez le poste par défaut. C'est celui qui sera utilisé si le champ 'code comptable' n'est pas renseigné dans la prestation ou dans le paramétrage de l'activité."))
        self.Append(propriete)

        propriete = wxpg.StringProperty(label=_("Code Produit Local par défaut"), name="code_prodloc", value=u"")
        propriete.SetHelpString(_("Saisissez le code Produit Local. C'est celui qui sera utilisé si le champ 'code produit local' n'est pas renseigné dans la prestation ou dans le paramétrage de l'activité."))
        self.Append(propriete)

        propriete = wxpg.StringProperty(label=_("Opération"), name="operation", value=u"")
        propriete.SetHelpString(_("Saisissez le code de l'opération."))
        self.Append(propriete)

        propriete = wxpg.StringProperty(label=_("Service"), name="service", value=u"")
        propriete.SetHelpString(_("Saisissez le code du service."))
        self.Append(propriete)

        propriete = wxpg.StringProperty(label=_("Fonction"), name="fonction", value=u"")
        propriete.SetHelpString(_("Saisissez le code de la fonction."))
        self.Append(propriete)

        # Options
        self.Append(wxpg.PropertyCategory(_("Options")))

        propriete = wxpg.BoolProperty(label=_("Titre payable par internet"), name="tipi", value=True)
        propriete.SetHelpString(_("Cochez cette case si l'usager peut payer sur internet avec TIPI (PayFip)"))
        propriete.SetAttribute("UseCheckbox", True)
        self.Append(propriete)

        choix = [
            ("", _("ASAP non dématérialisé")),
            ("01", _("01-ASAP dématérialisé à éditer par le centre éditique")),
            ("02", _("02-ASAP dématérialisé à destination d'une entité publique référencée dans Chorus Pro")),
            ("03", _("03-ASAP ORMC Chorus Pro")),
            ("04", _("04-ASAP sans traitement DGFIP")),
        ]
        propriete = CTRL_Propertygrid.Propriete_choix(label=_("Edition ASAP"), name="edition_asap", liste_choix=choix, valeur="01")
        propriete.SetEditor("EditeurChoix")
        propriete.SetHelpString(_("Indiquez si l'ASAP doit être édité ou non par le centre éditique (Balise Edition dans bloc pièce du PES Titre)"))
        self.Append(propriete)

        propriete = wxpg.BoolProperty(label=_("Inclure le détail des factures"), name="inclure_detail", value=False)
        propriete.SetHelpString(_("Cochez cette case si vous souhaitez que Noethys intègre le détail des prestations de chaque facture"))
        propriete.SetAttribute("UseCheckbox", True)
        self.Append(propriete)

        propriete = wxpg.BoolProperty(label=_("Inclure les factures au format PDF"), name="inclure_pieces_jointes", value=False)
        propriete.SetHelpString(_("Cochez cette case si vous souhaitez que Noethys intègre les factures au format PDF"))
        propriete.SetAttribute("UseCheckbox", True)
        self.Append(propriete)

        propriete = wxpg.StringProperty(label=_("Nom du tribunal de recours"), name="tribunal", value=_("le tribunal administratif"))
        propriete.SetHelpString(_("Saisissez le nom du tribunal"))
        self.SetPropertyMaxLength(propriete, 100)
        self.Append(propriete)

        propriete = wxpg.BoolProperty(label=_("Utiliser code comptable familial comme code tiers"), name="code_compta_as_alias", value=True)
        propriete.SetHelpString(_("Utiliser le code comptable de la famille (Fiche famille > Onglet Divers) comme code tiers (ou alias). Sinon un code de type FAM000001 sera généré automatiquement."))
        propriete.SetAttribute("UseCheckbox", True)
        self.Append(propriete)
...
class Dialog(DLG_Saisie_lot_tresor_public.Dialog):
    ...
    def ValidationDonnees(self):
        ...
        listeVerifications = [
            ...
            (tribunal, "tribunal", _("le tribunal de recours")),
            ]
        ...
        for donnee, code, label in listeVerifications :
            ...
            if code == "id_bordereau" :
                try :
                    ...
                    dlg = wx.MessageDialog(self, _("Vous devez saisir une valeur numérique valide pour le paramètre de bordereau 'ID Bordereau' !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                    dlg.ShowModal()
                    dlg.Destroy()
                    return False
    ...
    def OnBoutonFichier(self, event):
        ...
        dlg = wx.MessageDialog(self, _("Le fichier a été créé avec succès.\n\nPensez à décompresser ce fichier avant l'import dans Magnus."), _("Information"), wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
    ...
    def GenerationPiecesJointes(self, repertoire=""):
        ...
        nomFichierUnique = "F{NUM_FACTURE}_{NOM_TITULAIRES_MAJ}"
    ...
    def Generer_fichier(self, dict_donnees={}, repertoire=""):
        ...
        for IdEcriture, piece in enumerate(dict_donnees["pieces"], start=1):
            ...
            if piece["IDfacture"] in dict_donnees["codes"]:
                for (IDposte, code_produit_local), montant in dict_donnees["codes"][piece["IDfacture"]].items():
                    ...
                    # Civilité - Texte (32)
                    if piece["titulaire_civilite"] == "M.": ligne[69] = ConvertToTexte("M")
                    if piece["titulaire_civilite"] == "Mme": ligne[69] = ConvertToTexte("MME")
                    if piece["titulaire_civilite"] == "Melle": ligne[69] = ConvertToTexte("MLLE")
                    ...
                    if piece["prelevement"] == 1:
                        ...
                        IDcompte = self.ctrl_parametres.GetPropertyValue("IDcompte")
                        if not IDcompte:
                            dlg = wx.MessageDialog(self, _("Au moins un prélèvement SEPA est demandé. Vous devez donc sélectionner un compte bancaire créditeur dans les paramètres !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                            dlg.ShowModal()
                            dlg.Destroy()
                            return False
                        ...
                        if not dict_compte["code_ics"]:
                            dlg = wx.MessageDialog(self, _("Au moins un prélèvement SEPA est demandé. Vous devez donc renseigner le numéro ICS dans les paramètres du compte bancaire créditeur !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                            dlg.ShowModal()
                            dlg.Destroy()
                            return False
                    ...
                    # Civilité - Texte (32)
                    if piece["titulaire_civilite"] == "M.": ligne[93] = ConvertToTexte("M")
                    if piece["titulaire_civilite"] == "Mme": ligne[93] = ConvertToTexte("MME")
                    if piece["titulaire_civilite"] == "Melle": ligne[93] = ConvertToTexte("MLLE")
        ...
        if lignes:
            ...
            with open(os.path.join(repertoire, "WTAMC001.txt"), 'w') as fichier:
                if six.PY2:
                    contenu_lignes = contenu_lignes.encode('cp1252')
        ...
        if lignes_detail:
            ...
            with open(os.path.join(repertoire, "WTAMC001AS.txt"), 'w') as fichier:
                if six.PY2:
                    contenu_lignes_detail = contenu_lignes_detail.encode('cp1252')
        ...
        if lignes_pj:
            ...
            with open(os.path.join(repertoire, "WTAMC001PJ.txt"), 'w') as fichier:
                if six.PY2:
                    contenu_lignes_pj = contenu_lignes_pj.encode('cp1252')
