...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDmodele=None, IDactivite=None):
        ...
        # Importation
        if self.IDmodele == None :
            self.SetTitle(_("Saisie d'un modèle de planning"))
        else :
            self.SetTitle(_("Modification d'un modèle de planning"))
            self.Importation() 
    ...
    def OnBoutonOk(self, event):
        ...
        if len(self.ctrl_planning.GetDonnees()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un élément de planning !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
...
class Dialog_selection_activite(wx.Dialog):
    def __init__(self, parent):
        ...
        self.label_intro = wx.StaticText(self, wx.ID_ANY, _("Sélectionnez l'activité pour laquelle vous souhaitez créer un modèle de planning puis cliquez sur OK :"))
