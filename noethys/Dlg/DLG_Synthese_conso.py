...
class CTRL_Choix_valeurs(wx.Choice):
    ...
    def MAJ(self):
        self.listeLabels = [_("Quantité"), _("Temps de présence"), _("Temps facturé")]
...
class CTRL_Choix_colonnes(wx.Choice):
    ...
    def MAJ(self):
        self.listeLabels = [_("Unités"), _("Unités + groupes"), _("Unités + événements")]
        self.SetItems(self.listeLabels)
        self.Select(0)
...
class CTRL_Choix_lignes(wx.Choice):
    def __init__(self, parent):
        ...
        self.listeDonnees = [
            ...
            {"label" : _("Evènement"), "code": "evenement"},
            {"label" : _("Evènement (avec date)"), "code": "evenement_date"},
            ...
            {"label" : _("Catégorie de travail"), "code" : "categorie_travail"},
            {"label" : _("Catégorie de travail du père"), "code" : "categorie_travail_pere"},
            {"label" : _("Catégorie de travail de la mère"), "code" : "categorie_travail_mere"},
            ]
...
class CTRL_Choix_mode(wx.Choice):
    ...
    def MAJ(self):
        self.listeLabels = [_("Réservation"), _("Attente"), _("Refus")]
        self.SetItems(self.listeLabels)
        self.Select(0)
...
class CTRL_Choix_etat(wx.CheckListBox):
    ...
    def MAJ(self):
        self.listeLabels = [_("Pointage en attente"), _("Présent"), _("Absence justifiée"), _("Absence injustifiée")]
        self.SetItems(self.listeLabels)
        self.Check(0)
        self.Check(1)
    ...
    def GetLabels(self):
        ...
        for code in self.GetValeur() :
            if code == "reservation" : listeLabels.append(_("Réservation"))
            if code == "present" : listeLabels.append(_("Présent"))
            if code == "absentj" : listeLabels.append(_("Absence justifiée"))
            if code == "absenti" : listeLabels.append(_("Absence injustifiée"))
        return ", ".join(listeLabels)
...
class Parametres(wx.Panel):
    def __init__(self, parent, listview=None):
        ...
        # Activité
        self.box_activite_staticbox = wx.StaticBox(self, -1, _("Activité"))
    ...
    def __set_properties(self):
        ...
        self.ctrl_groupes.SetToolTip(wx.ToolTip(_("Cochez les groupes à prendre en compte")))
        self.ctrl_valeurs.SetToolTip(wx.ToolTip(_("Sélectionnez le type de données à afficher")))
        self.ctrl_lignes.SetToolTip(wx.ToolTip(_("Sélectionnez le type de données par ligne")))
        self.ctrl_colonnes.SetToolTip(wx.ToolTip(_("Sélectionnez le type de données par colonne")))
    ...
    def OnBoutonActualiser(self, event): 
        ...
        if self.ctrl_date_debut.GetDate()  == None :
            dlg = wx.MessageDialog(self, _("Vous n'avez saisi aucune date de début !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if len(self.ctrl_etat.GetValeur()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez cocher au moins un état !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
    ...
    def Actualiser(self, event=None):
        ...
        # Label Paramètres
        listeParametres = [ 
            _("Période du %s au %s") % (DateEngFr(str(date_debut)), DateEngFr(str(date_fin))),
            _("Activité : %s") % self.ctrl_activite.GetNomActivite(),
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        intro = _("Vous pouvez ici éditer une synthèse des consommations pour une période et une activité données. Commencez par sélectionner une date de début et de fin puis choisissez une activité. Vous pouvez affiner vos résultats et modifier l'affichage des données grâce aux options proposées.")
        titre = _("Synthèse des consommations")
        self.SetTitle(titre)
    ...
    def __set_properties(self):
        self.bouton_apercu.SetToolTip(wx.ToolTip(_("Cliquez ici pour créer un aperçu de la liste (PDF)")))
