...
from Ol import OL_Liste_non_transferts
...
class Dialog(wx.Dialog):
    def __init__(self, parent,dateFin=None):
        ...
        intro = _("Vous pouvez consulter ici le détail par nature des écritures non transférées en compta.")
        titre = _("Récapitulatif non transféré en compta")
        self.SetTitle("DLG_Liste_non_transferts")
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/22x22/Smiley_nul.png")
        
        self.listviewAvecFooter = OL_Liste_non_transferts.ListviewAvecFooter(self, kwargs={"dateFin":self.dateFin})
        self.ctrl_comptes = self.listviewAvecFooter.GetListview()

        self.ctrl_recherche = OL_Liste_non_transferts.CTRL_Outils(self, listview=self.ctrl_comptes)
