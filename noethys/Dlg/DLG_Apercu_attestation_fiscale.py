...
class Dialog(wx.Dialog):
    def __init__(self, parent, provisoire=False, titre=_("Aperçu d'une attestation fiscale"), intro=_("Vous pouvez ici créer un aperçu PDF du document sélectionné.")):
        wx.Dialog.__init__(self, parent, -1, style=wx.DEFAULT_DIALOG_STYLE)
        self.parent = parent
        
        # Bandeau
        if provisoire == True :
            intro += _(" <FONT COLOR = '#FF0000'>Attention, il ne s'agit que d'un document provisoire avant génération !</FONT>")
        self.SetTitle(titre)
...
##        self.check_coupons = wx.CheckBox(self, -1, _("Insérer les coupons-réponses"))
##        self.check_codesbarres = wx.CheckBox(self, -1, _("Insérer les codes-barres"))
...
##        self.check_codesbarres.SetValue(UTILS_Config.GetParametre("impression_rappels_codeBarre", defaut=1))
    ...
    def __set_properties(self):
        self.ctrl_modele.SetToolTip(wx.ToolTip(_("Sélectionnez ici le modèle de document")))
##        self.check_coupons.SetToolTip(wx.ToolTip(_("Cochez cette case pour insérer les coupons-réponses")))
##        self.check_codesbarres.SetToolTip(wx.ToolTip(_("Cochez cette case pour insérer les codes-barres")))
