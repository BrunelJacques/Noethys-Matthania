...
FORMATS = [
    {"code": "pes", "label": _("PES v2 ORMC Recette"), "description": _("Export vers le logiciel Hélios du Trésor Public diffusé par la DGFIP.")},
    {"code": "magnus", "label": _("Magnus Berger-Levrault"), "description": _("Export vers le logiciel Magnus de la société Berger-Levrault.")},
    {"code": "jvs", "label": _("Millesime Online JVS"), "description": _("Export vers le logiciel Millesime Online de la société JVS-MAIRISTEM.")},
]
...
class DLG_Choix_format(wx.Dialog):
    def __init__(self, parent):
        ...
        # Bandeau
        titre = _("Sélection du format")
        intro = _("Cliquez sur le format d'export souhaité.")
        self.SetTitle(titre)
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDlot=None, format=None, ctrl_parametres=None):
        ...
        # Pièces
        self.box_pieces_staticbox = wx.StaticBox(self, -1, _("Pièces"))
        ...
        # Boutons
        self.bouton_aide = CTRL_Bouton_image.CTRL(self, texte=_("Aide"), cheminImage="Images/32x32/Aide.png")
        self.bouton_fichier = CTRL_Bouton_image.CTRL(self, texte=_("Générer le fichier d'export %s") % GetFormatByCode(self.format)["label"], cheminImage="Images/32x32/Disk.png")
    ...
    def __set_properties(self):
        ...
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez un nom pour ce lot (Ex : 'Janvier 2013', etc...). Nom interne à Noethys.")))
        self.ctrl_verrouillage.SetToolTip(wx.ToolTip(_("Cochez cette case pour verrouiller le lot lorsque vous considérez qu'il est finalisé")))
        self.ctrl_observations.SetToolTip(wx.ToolTip(_("Saisissez ici des observations sur ce lot")))
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter une pièce")))
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier la pièce sélectionnée dans la liste")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour retirer la pièce sélectionnée dans la liste")))
        self.bouton_apercu.SetToolTip(wx.ToolTip(_("Cliquez ici pour afficher un aperçu avant impression de la liste des pièces")))
        self.bouton_imprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour imprimer la liste des pièces de ce lot")))
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez ici pour obtenir de l'aide")))
        self.bouton_fichier.SetToolTip(wx.ToolTip(_("Cliquez ici pour générer un fichier au format %s") % GetFormatByCode(self.format)["label"]))
    ...
    def OnBoutonOk(self, event): 
        ...
        if self.ctrl_verrouillage.GetValue() == False :
            dlg = wx.MessageDialog(self, _("Pour clôturer le traitement d'un lot, vous devez valider ou refuser les pièces puis verrouiller le lot.\n\nSouhaitez-vous le faire maintenant ?"), _("Information"), wx.YES_NO|wx.CANCEL|wx.NO_DEFAULT|wx.ICON_INFORMATION)
            reponse = dlg.ShowModal()
            dlg.Destroy()
            if reponse != wx.ID_NO :
                return
