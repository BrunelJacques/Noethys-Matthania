...
def DateComplete(dateDD):
    ...
    listeMois = (
        _("janvier"), _("février"), _("mars"), _("avril"), _("mai"),
        _("juin"), _("juillet"), _("août"), _("septembre"),
...
class Commandes(wx.Panel):
    def __init__(self, parent):
        ...
        self.bouton_options = CTRL_Bouton_image.CTRL(self, texte=_("Options"), cheminImage="Images/32x32/Configuration2.png")
        ...
        self.bouton_aide.SetToolTip(wx.ToolTip(
            _("Cliquez ici pour obtenir de l'aide")
        ))
        self.bouton_options.SetToolTip(wx.ToolTip(
            _("Cliquez ici pour définir les paramètres d'affichage de la fenêtre")
        ))
        self.bouton_outils.SetToolTip(wx.ToolTip(
            _("Cliquez ici pour accéder aux outils")
        ))
        self.bouton_annuler.SetToolTip(wx.ToolTip(
            _("Cliquez ici pour fermer")
        ))
...
##                    dlg = wx.MessageDialog(self, _("L'individu que vous avez sélectionné est déjà dans la grille des présences !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
##                    dlg.ShowModal()
##                    dlg.Destroy()
##                    return
...
##
##        # Ajouter individu
##        self.hyper_ajouter_individu = Hyperlien(self, label=_("Ajouter un individu"), infobulle=_("Cliquez ici pour ajouter un individu à la liste afichée"), URL="AJOUTER")
##        self.label_separation_1 = wx.StaticText(self, -1, "|")
##        self.hyper_tous_inscrits = Hyperlien(self, label=_("Afficher tous les inscrits"), infobulle=_("Cliquez ici pour afficher tous les inscrits aux activités et groupes sélectionnés"), URL="INSCRITS")
##        # Mode de saisie
##        self.label_mode = wx.StaticText(self, -1, _("Mode de saisie :"))
##        self.radio_reservation = wx.RadioButton(self, -1, _("Réservation"), style = wx.RB_GROUP )
##        self.radio_attente = wx.RadioButton(self, -1, _("Attente") )
##        self.radio_refus = wx.RadioButton(self, -1, _("Refus") )
##        self.radio_reservation.SetValue(True)
##
##        self.radio_reservation.SetToolTip(wx.ToolTip(_("Le mode Réservation permet de saisir une réservation")))
##        self.radio_attente.SetToolTip(wx.ToolTip(_("Le mode Attente permet de saisir une place sur liste d'attente")))
##        self.radio_refus.SetToolTip(wx.ToolTip(_("Le mode de refus permet de saisir une place sur liste d'attente qui a été refusée par l'individu. Cette saisie est juste utilisée à titre statistique")))
##
...
class PanelGrille(wx.Panel):
    def __init__(self, parent):
        ...
        AddTool(self.barreOutils, ID_AJOUTER_INDIVIDU, label=_("Ajouter un individu"), bitmap=wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Femme.png"), wx.BITMAP_TYPE_PNG))
        AddTool(self.barreOutils, ID_AFFICHER_TOUS_INSCRITS, label=_("Afficher tous les inscrits"), bitmap=wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Famille.png"), wx.BITMAP_TYPE_PNG))
        ...
        AddTool(self.barreOutils, self.ID_MODE_RESERVATION, label=_("Réservation"), bitmap=CTRL_Grille.CreationImage(10, 20, CTRL_Grille.COULEUR_RESERVATION), kind=wx.ITEM_RADIO)
        AddTool(self.barreOutils, self.ID_MODE_ATTENTE, label=_("Attente"), bitmap=CTRL_Grille.CreationImage(10, 20, CTRL_Grille.COULEUR_ATTENTE), kind=wx.ITEM_RADIO)
        AddTool(self.barreOutils, self.ID_MODE_REFUS, label=_("Refus"), bitmap=CTRL_Grille.CreationImage(10, 20, CTRL_Grille.COULEUR_REFUS), kind=wx.ITEM_RADIO)
    ...
    def AfficherTousInscrits(self, event=None):
        ...
        req += """
        WHERE (NOT inscriptions.statut LIKE 'ko%%') AND {0}
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        self._mgr.AddPane(self.panel_commandes, (
            aui.AuiPaneInfo()
               .Name("commandes").Caption(_("Commandes"))
        ...
        self._mgr.AddPane(self.panel_calendrier, (
            aui.AuiPaneInfo()
               .Name("calendrier").Caption(_("Sélection de la date"))
        ...
        self._mgr.AddPane(self.panel_activites, (
            aui.AuiPaneInfo()
               .Name("activites").Caption(_("Sélection des activités"))
        ...
        self._mgr.AddPane(self.panel_legende, (
            aui.AuiPaneInfo()
               .Name("legende").Caption(_("Légende"))
        ...
        self._mgr.AddPane(self.panel_raccourcis, (
            aui.AuiPaneInfo()
               .Name("raccourcis").Caption(_("Touches raccourcis"))
        ...
        self._mgr.AddPane(self.panel_ecoles, (
            aui.AuiPaneInfo()
               .Name("ecoles").Caption(_("Écoles/classes"))
        ...
        self._mgr.AddPane(self.panel_etiquettes, (
            aui.AuiPaneInfo()
               .Name("etiquettes").Caption(_("Etiquettes"))
        ...
        self._mgr.AddPane(self.panel_forfaits, (
            aui.AuiPaneInfo()
               .Name("forfaits").Caption(_("Forfaits crédits"))
        ...
        self.SetTitle(_("Gestionnaire des consommations"))
...
#        self.panel_totaux.MAJ()
    ...
    def Annuler(self):
        if len(self.panel_grille.grille.listeHistorique) > 0:
            dlg = wx.MessageDialog(self, _("Des modifications ont été effectuées dans la grille.\n\nSouhaitez-vous enregistrer ces modifications avant de fermer cette fenêtre ?"), _("Sauvegarde des modifications"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
    ...
    def MenuOutils(self):
        ...
        #        item = wx.MenuItem(menuPop, ID_OUTILS_SAISIE_FORFAIT, _("Appliquer un forfait daté"), _("Appliquer un forfait daté"))
...
#        menuPop.AppendSeparator()
        ...
        item = wx.MenuItem(menuPop, ID_OUTILS_IMPRIMER_CONSO, _("Imprimer la liste des consommations"), _("Imprimer la liste des consommations"))
        ...
        item = wx.MenuItem(menuPop, ID_OUTILS_RECALCUL, _("Recalculer toutes les prestations"), _("Recalculer toutes les prestations"))
        ...
        item = wx.MenuItem(menuPop, ID_OUTILS_CONVERTIR_ETAT, _("Convertir l'état des consommations"), _("Convertir l'état des consommations"))
        ...
        item = wx.MenuItem(menuPop, ID_OUTILS_RECOPIAGE, _("Recopier des consommations"), _("Recopier des consommations"))
        item.SetBitmap(wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Calendrier_modification.png"), wx.BITMAP_TYPE_PNG))
        menuPop.AppendItem(item)
        ...
        item = wx.MenuItem(menuPop, ID_OUTILS_TOUT_SELECTIONNER, _("Sélectionner toutes les lignes"), _("Sélectionner toutes les lignes"))
        ...
        item = wx.MenuItem(menuPop, ID_OUTILS_TOUT_DESELECTIONNER, _("Désélectionner toutes les lignes"), _("Désélectionner toutes les lignes"))
    ...
    #     if nbre == 0 :
    #         dlg = wx.MessageDialog(self, _("Il n'y a aucune consommation affichée ayant cet état !"), _("Annulation"), wx.OK | wx.ICON_EXCLAMATION)
    #         dlg.ShowModal()
    #         dlg.Destroy()
    #         return
    #     dlg = wx.MessageDialog(self, _("Confirmez-vous le changement d'état '%s' en '%s' pour %d consommations ?") % (labelEtat1, labelEtat2, nbre), _("Changement d'état"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
    #     reponse = dlg.ShowModal()
    #     dlg.Destroy()
    #     if reponse != wx.ID_YES :
    #         return
    ...
    def On_outils_imprimer(self, event):
        if len(self.panel_grille.grille.listeHistorique) > 0:
            dlg = wx.MessageDialog(self, _("Des modifications ont été effectuées dans la grille.\n\nSouhaitez-vous les enregistrer maintenant afin qu'elles apparaissent dans le document ?"), _("Sauvegarde des modifications"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
            reponse = dlg.ShowModal()
            dlg.Destroy()
            if reponse != wx.ID_YES:
                return
    ...
    def On_outils_recalculer(self, event):
        if UTILS_Utilisateurs.VerificationDroitsUtilisateurActuel("consommations_conso", "modifier") is False:
            return
        dlg = wx.MessageDialog(self, _("Confirmez-vous le recalcul des prestations de toutes les consommations affichées ?"), _("Recalcul des prestations"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        reponse = dlg.ShowModal()
        dlg.Destroy()
        if reponse != wx.ID_YES:
            return
    ...
    def MenuOptions(self):
        ...
        item = wx.MenuItem(menuPop, ID_AFFICHAGE_PERSPECTIVE_DEFAUT, _("Disposition par défaut"), _("Afficher la disposition par défaut"), wx.ITEM_CHECK)
        menuPop.AppendItem(item)
        ...
        for dictPerspective in self.perspectives:
            ...
            item = wx.MenuItem(menuPop, self.ID_PREMIERE_PERSPECTIVE + index, label, _("Afficher la disposition '%s'") % label, wx.ITEM_CHECK)
            menuPop.AppendItem(item)
        ...
        item = wx.MenuItem(menuPop, ID_AFFICHAGE_PERSPECTIVE_SAVE, _("Sauvegarder la disposition actuelle"), _("Sauvegarder la disposition actuelle de la page d'accueil"))
        ...
        item = wx.MenuItem(menuPop, ID_AFFICHAGE_PERSPECTIVE_SUPPR, _("Supprimer des dispositions"), _("Supprimer des dispositions de page d'accueil sauvegardée"))
        ...
        self.listePanneaux = [
            {"label": _("Sélection de la date"),
             "code": "calendrier", "IDmenu": None},
            {"label": _("Sélection des activités"),
             "code": "activites", "IDmenu": None},
            {"label": _("Légende"),
             "code": "legende", "IDmenu": None},
            {"label": _("Touches raccourcis"),
        ...
        for dictPanneau in self.listePanneaux:
            ...
            item = wx.MenuItem(menuPop, dictPanneau["IDmenu"], label, _("Afficher le panneau '%s'") % label, wx.ITEM_CHECK)
            menuPop.AppendItem(item)
        ...
        item = wx.MenuItem(sousMenuLabelLigne, self.ID_FORMAT_LABEL_LIGNE_1, _("Nom Prénom"), _("Format du label : 'Nom Prénom'"), wx.ITEM_RADIO)
        ...
        item = wx.MenuItem(sousMenuLabelLigne, self.ID_FORMAT_LABEL_LIGNE_2, _("Prénom Nom"), _("Format du label : 'Prénom Nom'"), wx.ITEM_RADIO)
        sousMenuLabelLigne.AppendItem(item)
        ...
        item = wx.MenuItem(sousMenuLabelLigne, self.ID_FORMAT_LABEL_LIGNE_3, _("Nom Prénom (ID)"), _("Format du label : 'Nom Prénom (ID)'"), wx.ITEM_RADIO)
        sousMenuLabelLigne.AppendItem(item)
        ...
        item = wx.MenuItem(sousMenuLabelLigne, self.ID_FORMAT_LABEL_LIGNE_4, _("Prénom Nom (ID)"), _("Format du label : 'Prénom Nom (ID)'"), wx.ITEM_RADIO)
        sousMenuLabelLigne.AppendItem(item)
        ...
        item = menuPop.AppendMenu(ID_FORMAT_LABEL_LIGNE_MENU, _("Format du label de la ligne"), sousMenuLabelLigne)

        menuPop.AppendSeparator()

        item = wx.MenuItem(menuPop, ID_AFFICHE_COLONNE_MEMO, _("Afficher la colonne Mémo journalier"), _("Afficher la colonne Mémo journalier"), wx.ITEM_CHECK)
        menuPop.AppendItem(item)
        ...
        item = wx.MenuItem(menuPop, ID_AFFICHE_COLONNE_TRANSPORTS, _("Afficher la colonne Transports"), _("Afficher la colonne Transports"), wx.ITEM_CHECK)
        menuPop.AppendItem(item)
        ...
        item = wx.MenuItem(menuPop, ID_MASQUER_UNITES_FERMEES, _("Masquer les unités non ouvertes"), _("Masquer les unités non ouvertes"), wx.ITEM_CHECK)
        menuPop.AppendItem(item)
        ...
        item = wx.MenuItem(menuPop, ID_AFFICHAGE_PARAMETRES, _("Définir hauteur et largeurs des cases"), _("Définir la hauteur des lignes et la largeur des cases"))
        ...
        item = wx.MenuItem(menuPop, ID_AFFICHE_SANS_PRESTATION, _("Afficher le symbole 'Sans prestation'"), _("Affiche le symbole 'Sans prestation' dans les cases si aucune prestation n'est rattachée"), wx.ITEM_CHECK)
        menuPop.AppendItem(item)
        ...
        item = wx.MenuItem(menuPop, ID_COCHER_ACTIVITES, _("Cocher les activités automatiquement"), _("Cocher automatiquement toutes les activités automatiquement dans le cadre 'Selection des activités'"), wx.ITEM_CHECK)
        menuPop.AppendItem(item)
        ...
        item = wx.MenuItem(menuPop, ID_BLOCAGE_SI_COMPLET, _("Blocage si capacité maximale atteinte"), _("Empêche l'utilisateur de saisir une consommation si la capacité maximale est atteinte (case rouge)"), wx.ITEM_CHECK)
        menuPop.AppendItem(item)
    ...
    def On_affichage_perspective_save(self, event):
        ...
        dlg = wx.TextEntryDialog(self, _("Veuillez saisir un intitulé pour cette disposition :"), "Sauvegarde d'une disposition")
    ...
    def On_affichage_perspective_suppr(self, event):
        ...
        dlg = wx.MultiChoiceDialog( self, _("Cochez les dispositions que vous souhaitez supprimer :"), _("Supprimer des dispositions"), listeLabels)
        if dlg.ShowModal() == wx.ID_OK:
    ...
    def On_affichage_largeur_unite(self, event):
        ...
        dlg = wx.TextEntryDialog(self, _("Veuillez saisir une largeur en pixels (50 par défaut) :"), "Largeur d'une colonne unité")
        ...
        if reponse == wx.ID_OK:
            ...
            try:
                ...
                dlg2 = wx.MessageDialog(self, _("La valeur saisie semble incorrecte !"), _("Erreur de saisie"), wx.OK | wx.ICON_INFORMATION)
                dlg2.ShowModal()
                dlg2.Destroy()
                dlg.Destroy()
                return
            if newLargeur < 30 or newLargeur > 300:
                dlg2 = wx.MessageDialog(self, _("La valeur doit être comprise entre 30 et 300 !"), _("Erreur de saisie"), wx.OK | wx.ICON_INFORMATION)
                dlg2.ShowModal()
                dlg2.Destroy()
                return
