...
class Dialog(wx.Dialog):
    def __init__(self, parent, mode="gestion"):
        ...
        if self.mode == "selection" :
            intro = _("Vous pouvez ici sélectionner un établissement bancaire. Double-cliquez sur une ligne pour effectuer rapidement la sélection.")
            titre = _("Sélection d'un établissement bancaire")
            self.SetTitle(_("Sélection d'un établissement bancaire"))
        else:
            intro = _("Vous pouvez ici saisir, modifier ou supprimer des établissements bancaires. Ceux-ci sont utilisés lors de la saisie des coordonnées bancaires des comptes des familles pour les prélèvements automatiques.")
            titre = _("Gestion des établissements bancaires")
            self.SetTitle(_("Gestion des établissements bancaires"))
        ...
        self.ctrl_recherche = OL_Banques.CTRL_Outils(self, listview=self.ctrl_listview)
    ...
    def __set_properties(self):
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter un établissement bancaire")))
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier l'établissement sélectionné dans la liste")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer l'établissement sélectionné dans la iste")))
    ...
    def OnBouton_ok(self, event):
        ...
        if IDmedecin == None :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun établissement bancaire dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
