...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDproduit=None, liste_produits=[], selection_multiple=False, selection_obligatoire=True, coche_uniquement_disponibles=True):
        ...
        self.check_disponibles = wx.CheckBox(self, -1, _("Afficher uniquement les produits disponibles"))
    ...
    def __set_properties(self):
        self.SetTitle(_("Sélection d'un produit"))
        self.ctrl_produits.SetToolTip(wx.ToolTip(_("Double-cliquez sur une ligne pour la sélectionner rapidement")))
        self.check_disponibles.SetToolTip(wx.ToolTip(_("Cochez cette case pour afficher uniquement les produits disponibles")))
        ...
        if self.selection_obligatoire == True :
            ...
            if self.selection_multiple == False:
                ...
                if len(self.ctrl_produits.GetListeProduits()) == 0 :
                    dlg = wx.MessageDialog(self, _("Vous devez obligatoirement cocher au moins un produit !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                    dlg.ShowModal()
                    dlg.Destroy()
                    return
