...
class CTRL_champ(wx.Panel):
    ...
    def __init__(self,parent,nom,label,aide="",valdef="",choices=[""],btntip="Gestion",btnonclic=None):
        wx.Panel.__init__(self, parent, -1, style=wx.DEFAULT_DIALOG_STYLE|wx.RESIZE_BORDER|wx.MAXIMIZE_BOX|wx.MINIMIZE_BOX)
        self.parent = parent
        self.choices = choices
        self.label = wx.StaticText(self, -1, label + " :", style=wx.ALIGN_RIGHT,size=(140,20))
        if nom == 'ville':
    ...
    def __do_layout(self):
        ...
        gridsizer_base.Add(self.ctrl, 1,  wx.EXPAND, 5)
...
class PnlAdresse(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        if self.parent.mode == "familles":
            lstLignes = [
            ['designation', "Désignation de la famille",   "Libellé de l'adresse du courrier pour la famille", "Obligatoire"],
            ['designIndiv', "Individu correspondant", "Individu dont l'adresse est utilisée pour correspondre", "Obligatoire",
             ["Choisir..."], "Changement du correspondant de la famille", self.OnClicCorrespondant],
            ]
            self.posAdresse = 2
        else:
            lstLignes = []
        ...
        lstLignes += [
            ['appart',  "Appart,Esc,Etage|Service",
             "Réservé au numéro de l'appartement, escalier ou étage\nPour une administration le nom du service est accepté"],
            ['batiment',"Bâtiment, Résidence", "Un numéro une lettre doivent être précédés de BATIMENT, zone optionnelle"],
            ['rue',     "No et nom de rue",   "Aucun caractère spécial ou ponctuation n'est accepté", "Obligatoire"],
            ['bp',      "BP, CS, TSA, lieu dit",     "Uniquement pour préciser une distribution spéciale ou un lieu dit"],
            ['cp', "Code Postal", "Saisir le début du code postal", "Obligatoire"],
            ['ville', "Ville", "Choix de la ville ou gestion par '...'", "Obligatoire",["Choisir..."],
            "Gestion des villes et des codes postaux",self.OnClicVille],
            ['pays', "Pays étranger",
                    "Seulement pour l'étranger, à blanc pour le pays national\nGestion des pays par les villes", "",],
            ['forcer', "Forcer la saisie", "Permet de passer outre les contrôles de sortie, procédure exceptionnelle!", ],
        ]
    ...
    def GetVillesDuCp(self,filtrecp,mute=False):
        ...
        if len(lstChoix)>1 and not mute:
            ...
            dlg = CTRL_ChoixListe.Dialog(self,titre="Précisez votre choix",intro="Sinon allez choisir la ville par '...'",
                                        listeOriginale=lstChoix)
            ret = dlg.ShowModal()
    ...
    def GetVilleDuNom(self,ville):
        ...
        if len(lstChoix)>1:
            ...
            dlg = CTRL_ChoixListe.Dialog(self,listeOriginale=lstChoix,LargeurCode=180,titre="Précisez la ville",
                                          intro="Sinon allez choisir la ville par '...'")
            ret = dlg.ShowModal()
            if ret == wx.ID_OK and dlg.GetChoix():
                ix = lstChoix.index(dlg.GetChoix())
                ville = self.lstVilles[ix]
    ...
    def SetAdresse(self,adresse=[]):
        ...
        if len(adresse) != 7:
            mess = "PnlAdresse.SetAdresse: Adresse n'ayant pas 7 lignes!\n\n%s"%str(adresse)
            wx.MessageBox(mess)
            return
        cp = adresse[4]
        self.lstCpVillesPays = UTILS_SaisieAdresse.GetVilles("cp", cp)
    ...
    def OnClicCorrespondant(self,event):
        event.Skip()
        ret  = UTILS_SaisieAdresse.GetDBCorrespondant(self,self.parent.IDfamille)
        if ret == wx.ID_ABORT:
            return
        ...
        lstAdresse, IDindividuLu,(nom,prenom) = UTILS_SaisieAdresse.GetDBadresse(IDindividu,retNom=True)
...
class DlgSaisieAdresse(wx.Dialog):
    ...
    def __init__(self,ID,mode='individus',LargeurCode=180,LargeurLib=100,minSize=(180, 350),
                 titre="Saisie d'une adresse normalisée",intro="Lignes limitées à 38 caractères"):
        wx.Dialog.__init__(self, None, -1, style=wx.DEFAULT_DIALOG_STYLE|wx.RESIZE_BORDER|wx.MAXIMIZE_BOX|wx.MINIMIZE_BOX)
        self.SetTitle("DLG_SaisieAdresse")
        self.mode = mode
        self.choix= None
        if self.mode == 'familles':
            self.dicCorrespondant = UTILS_SaisieAdresse.GetDBfamille(ID)
            self.IDfamille = ID
        ...
        self.lstAdresse, self.IDindividuLu,(nom,prenom) = UTILS_SaisieAdresse.GetDBadresse(self.IDindividu,retNom=True)
        ...
        # Boutons
        self.bouton_remonte = CTRL_Bouton_image.CTRL(self, texte="Remonte\nex adr.", cheminImage=Chemins.GetStaticPath("Images/32x32/Actualiser.png",))
        self.bouton_ok = CTRL_Bouton_image.CTRL(self, texte="Valider", cheminImage=Chemins.GetStaticPath("Images/32x32/Valider.png"))
        self.bouton_fermer = CTRL_Bouton_image.CTRL(self, texte="Abandon", cheminImage=Chemins.GetStaticPath("Images/32x32/Annuler.png"))
    ...
    def __set_properties(self):
        ...
        self.bouton_remonte.SetToolTip("Cliquez ici pour remonter la première saisie")
        self.bouton_fermer.SetToolTip("Cliquez ici pour abandonner la saisie")
        self.bouton_ok.SetToolTip("Cliquez ici pour enregistrer l'adresse")
        # Binds
    ...
    def OnClicRemonte(self, event):
        event.Skip()
        self.lstAdresse = UTILS_SaisieAdresse.GetDBoldAdresse(IDindividu=self.IDindividu)
        if not self.lstAdresse :
            wx.MessageBox("Pas d'adresse antérieure stockée!")
        else:
            self.panelAdresse.SetAdresse(self.lstAdresse)
    ...
    def Enregistre(self):
        ...
        ret = UTILS_SaisieAdresse.SetDBadresse(None,self.IDindividu,self.lstAdresse)
        if ret != "ok":
            wx.MessageBox(self, "Pas d'écriture possible !\nAbandon de la modification\n%s"%ret)
            self.EndModal(wx.ID_ABORT)
        else:
            # archive cette adresse s'il n'y en a pas d'autre
            exadresse = UTILS_SaisieAdresse.GetDBoldAdresse(self.IDindividu)
            if not exadresse:
                ret = UTILS_SaisieAdresse.SetDBoldAdresse(None,self.IDindividu, self.lstAdresse)
            self.EndModal(wx.ID_OK)
        if self.mode == 'familles':
            ret = UTILS_SaisieAdresse.SetDBcorrespondant(self.dicCorrespondant)
            if ret != "ok":
                wx.MessageBox(self, "Pas d'écriture possible du correspondant !\n%s"%ret)
    ...
    def OnClicOk(self, event):
        ...
        if not forcer:
            self.lstAdresse = UTILS_SaisieAdresse.TransposeAdresse(saisie)
            self.panelAdresse.SetAdresse(self.lstAdresse)
        ...
        validee = False
        if not forcer:
            ret = UTILS_SaisieAdresse.Validation(self.lstAdresse)
            if ret == wx.ID_OK:
                validee = True
        ...
        if (validee and nonmodifiee) or forcer:
            ...
            mesdlg = wx.MessageDialog(None,
                                ...
                                caption="DLG_SaisieAdresse: Validation",
            ...
            wx.MessageBox("Pour sortir quand même sans abandonner, vous pouvez cocher la case 'Forcer la saisie'")

if __name__ == "__main__":
    app = wx.App(0)
    dlg = wx.Frame(None)
    dlg.individu = 1780
    dlg2 = DlgSaisieAdresse(dlg.individu, titre="Adresse de %d"%dlg.individu)
    dlg2.ShowModal()
    app.MainLoop()
