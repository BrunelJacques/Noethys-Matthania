...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDactivite=None, listeUnites=[]):
        ...
        # Date
        self.staticbox_date = wx.StaticBox(self, -1, _("Date"))
        ...
        # Unites
        self.staticbox_unites = wx.StaticBox(self, -1, _("Unités à combiner"))
    ...
    def __set_properties(self):
        self.SetTitle(_("Saisie d'une combinaison de forfait"))
        self.ctrl_date.SetToolTip(wx.ToolTip(_("Saisissez ici la date de la consommation")))
        self.bouton_calendrier.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner la date dans un calendrier")))
        self.ctrl_unites.SetToolTip(wx.ToolTip(_("Cochez les unités à combiner")))
    ...
    def OnBoutonOk(self, event): 
        date = self.GetDate()
        if date == None :
            dlg = wx.MessageDialog(self, _("La date que vous avez saisi ne semble pas valide !"), "Erreur", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if len(listeUnites) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez cocher au moins une unité !"), "Erreur", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
