...
LISTE_DONNEES = [
    { "nom" : _("Attestation"), "champs" : [
        { "code" : "numero", "label" : _("Numéro")},
        { "code" : "date", "label" : _("Date d'édition")},
        { "code" : "lieu", "label" : _("Lieu d'édition")},
        ] },
    { "nom" : _("Destinataire"), "champs" : [
        { "code" : "nom", "label" : _("Nom")},
        { "code" : "rue", "label" : _("Rue")},
        { "code" : "ville", "label" : _("CP + Ville")},
        ] },
    { "nom" : _("Organisme"), "champs" : [
        { "code" : "siret", "label" : _("Numéro SIRET")},
        { "code" : "ape", "label" : _("Code APE")},
        ] },
    ]

TEXTE_INTRO = _("Je soussigné{GENRE} {NOM}, {FONCTION}, atteste avoir accueilli {ENFANTS} sur la période du {DATE_DEBUT} au {DATE_FIN} selon le détail suivant :")
...
class CTRL_Individus(wx.CheckListBox):
    ...
    def Importation(self):
        ...
        AND ((inscriptions.statut = 'ok') OR (inscriptions.statut Is Null)) AND consommations.date>='%s' AND consommations.date<='%s'
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDfamille=None, date_debut=None, date_fin=None):
        ...
        # Bandeau
        intro = _("Vous pouvez ici éditer une attestation de présence au format PDF. Saisissez une période, sélectionnez les éléments de votre choix, puis cliquez sur 'Aperçu'.")
        titre = _("Edition d'une attestation de présence")
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Imprimante.png")
        
        # Période
        self.staticbox_periode_staticbox = wx.StaticBox(self, -1, _("Période"))
    ...
    def __set_properties(self):
        ...
        self.bouton_date_debut.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner la date de début")))
        self.ctrl_date_fin.SetToolTip(wx.ToolTip(_("Saisissez la date de fin")))
        self.bouton_date_fin.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner la date de fin")))
        self.ctrl_individus.SetToolTip(wx.ToolTip(_("Cochez les individus")))
        self.ctrl_activites.SetToolTip(wx.ToolTip(_("Cochez les activités")))
        self.ctrl_unites.SetToolTip(wx.ToolTip(_("Cochez les unites")))
        self.ctrl_donnees.SetToolTip(wx.ToolTip(_("Vous pouvez modifier ici les données de base")))
        self.ctrl_afficher_conso.SetToolTip(wx.ToolTip(_("Cochez cette case pour afficher uniquement les consommations dans la liste")))
    ...
    def Sauvegarder(self):
        ...
        UTILS_Historique.InsertActions([{
                ...
                "IDcategorie" : 27, 
                "action" : _("Edition d'une attestation de présence pour la période du %s au %s pour un total de %.02f € et un solde de %.02f €") % (DateEngFr(self.dictSave["date_debut"]), DateEngFr(self.dictSave["date_fin"]), self.dictSave["total"], self.dictSave["solde"] ),
                },])
    ...
    def CreationPDF(self, nomDoc=FonctionsPerso.GenerationNomDoc("ATTESTATION", "pdf") , afficherDoc=True):
        ...
        if len(listePrestations) == 0 :
            dlg = wx.MessageDialog(self, _("Il n'existe aucune prestation avec les paramètres donnés !"), _("Annulation"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
                ...
                if IDindividu in dictIndividus :
                    ...
                    if dateNaiss != None : 
                        if DICT_CIVILITES[IDcivilite]["sexe"] == "M" :
                            texteDateNaiss = _(", né le %s") % DateEngFr(str(dateNaiss))
                        else:
                            texteDateNaiss = _(", née le %s") % DateEngFr(str(dateNaiss))
                    else:
                        texteDateNaiss = ""
        ...
        dictChampsFusion["{SOLDE}"] = "%.2f %s" % (self.dictSave["solde"], SYMBOLE)
