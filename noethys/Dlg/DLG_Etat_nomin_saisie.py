...
class Dialog(wx.Dialog):
    def __init__(self, parent, listeChamps=[], IDchamp=None, code="", nom="", formule="", titre=u""):
        ...
        self.label_code_info = wx.StaticText(self, -1, _("(En majuscules et sans espaces)"))
        
        self.label_nom = wx.StaticText(self, -1, _("Description :"))
        self.ctrl_nom = wx.TextCtrl(self, -1, nom)

        self.label_titre = wx.StaticText(self, -1, _("Titre de colonne :"))
        self.ctrl_titre = wx.TextCtrl(self, -1, titre)

        self.label_formule = wx.StaticText(self, -1, _("Formule :"))
    ...
    def __set_properties(self):
        self.SetTitle(_("Saisie d'un champ personnalisé"))
        self.ctrl_code.SetToolTip(wx.ToolTip(_("Saisissez un code pour ce champ. Ex : 'MONCHAMP1' [OBLIGATOIRE]")))
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez un nom pour ce champ. Ex : 'Total des présences' [OBLIGATOIRE]")))
        self.ctrl_titre.SetToolTip(wx.ToolTip(_("Saisissez un titre de colonne pour ce champ [OBLIGATOIRE]")))
        self.ctrl_formule.SetToolTip(wx.ToolTip(_("Saisissez une formule pour ce champ [OPTIONNEL]")))
    ...
    def OnBoutonOk(self, event): 
        ...
        if code == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un code !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.ctrl_nom.GetValue() == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un descriptif !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.ctrl_titre.GetValue() == "" :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un titre de colonne !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        for IDchamp, code in listeDonnees :
            if self.IDchamp != IDchamp : 
                dlg = wx.MessageDialog(self, _("Ce code a déjà été attribué à un autre champ. Veuillez le modifier !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_code.SetFocus()
                return
...
if __name__ == "__main__":
    ...
    dialog_1 = Dialog(None, listeChamps=[(u"CODE1", _("Label du champ 1")),])
