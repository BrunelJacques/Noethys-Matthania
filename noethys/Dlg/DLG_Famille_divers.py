...
LISTE_CATEGORIES_TIERS = [
    ("01", _("Personne physique")),
    ("20", _("Etat ou établissement public national")),
    ("21", _("Région")),
    ("22", _("Département")),
    ("23", _("Commune")),
    ("24", _("Groupement de collectivités")),
    ("25", _("Caisse des écoles")),
    ("26", _("CCAS")),
    ("27", _("Etablissement public de santé")),
    ("28", _("Ecole nationale de la santé publique")),
    ("29", _("Autre établissement publique ou organisme international")),
    ("50", _("Personne morale de droit privé autre qu'organisme social")),
    ("60", _("Caisse de sécurité sociale régime général")),
    ("61", _("Caisse de sécurité sociale régime agricole")),
    ("62", _("Sécurité sociale des travailleurs non salariés et professions non agricoles")),
    ("63", _("Autre régime obligatoire de sécurité sociale")),
    ("64", _("Mutuelle ou organisme d'assurance")),
    ("65", _("Autre tiers payant")),
    ("70", _("CNRACL")),
    ("71", _("IRCANTEC")),
    ("72", _("ASSEDIC")),
    ("73", _("Caisse mutualiste de retraite complémentaire")),
    ("74", _("Autre organisme social")),
    ]

LISTE_NATURES_JURIDIQUES = [
    ("00", _("Inconnu")),
    ("01", _("Particulier")),
    ("02", _("Artisan / commerçant / agriculteur")),
    ("03", _("Société")),
    ("04", _("CAM ou Caisse appliquant les mêmes règles")),
    ("05", _("Caisse complémentaire")),
    ("06", _("Association")),
    ("07", _("Etat ou organisme d'état")),
    ("08", _("Etablissement public national")),
    ("09", _("Collectivité territoriale / EPL / EPS")),
    ("10", _("Etat étranger")),
    ("11", _("CAF")),
    ]
...
class CTRL_Parametres(wxpg.PropertyGrid) :
    def __init__(self, parent, IDfamille=None):
        ...
        self.Append( wxpg.PropertyCategory(_("Données tiers pour Hélios")) )
        
        propriete = wxpg.EnumProperty(label=_("Titulaire"), name="titulaire_helios")
        propriete.SetHelpString(_("Sélectionnez le titulaire du compte pour Hélios (Trésor Public)"))
        self.Append(propriete)
##        self.MAJ_titulaire_helios() 

        propriete = wxpg.StringProperty(label=_("Identifiant national"), name="idtiers_helios", value=u"")
        propriete.SetHelpString(_("[Facultatif] Saisissez l'identifiant national (SIRET ou SIREN ou FINESS ou NIR)"))
        self.Append(propriete)

        listeLabels, listeID = GetDonneesListe(LISTE_TYPES_ID_TIERS)
        propriete = wxpg.EnumProperty(label=_("Type d'identifiant national"), name="natidtiers_helios", labels=listeLabels, values=listeID, value=9999)
        propriete.SetHelpString(_("[Facultatif] Sélectionnez le type d'identifiant national du tiers pour Hélios (Trésor Public)"))
        self.Append(propriete)

        propriete = wxpg.StringProperty(label=_("Référence locale"), name="reftiers_helios", value="")
        propriete.SetHelpString(_("[Facultatif] Saisissez la référence locale du tiers"))
        self.Append(propriete)

        listeLabels, listeID = GetDonneesListe(LISTE_CATEGORIES_TIERS)
        propriete = wxpg.EnumProperty(label=_("Catégorie"), name="cattiers_helios", labels=listeLabels, values=listeID, value=1)
        propriete.SetHelpString(_("Sélectionnez la catégorie de tiers pour Hélios (Trésor Public)"))
        self.Append(propriete)

        listeLabels, listeID = GetDonneesListe(LISTE_NATURES_JURIDIQUES)
        propriete = wxpg.EnumProperty(label=_("Nature juridique"), name="natjur_helios", labels=listeLabels, values=listeID, value=1)
        propriete.SetHelpString(_("Sélectionnez la nature juridique du tiers pour Hélios (Trésor Public)"))
        self.Append(propriete)


        # Facturation
        self.Append( wxpg.PropertyCategory(_("Facturation")))

        # Autre adresse de facturation
        propriete = wxpg.BoolProperty(label=_("Autre adresse de facturation"), name="autre_adresse_facturation", value=False)
        propriete.SetHelpString(_("Cochez cette case pour activer une autre adresse de facturation"))
        propriete.SetAttribute("UseCheckbox", True)
        self.Append(propriete)

        propriete = wxpg.StringProperty(label=_("Autre adresse - Nom"), name="adresse_nom", value="")
        propriete.SetHelpString(_("Saisissez un nom de destinataire"))
        self.Append(propriete)

        propriete = wxpg.StringProperty(label=_("Autre adresse - Rue"), name="adresse_rue", value="")
        propriete.SetHelpString(_("Saisissez la rue de l'adresse"))
        self.Append(propriete)

        propriete = wxpg.StringProperty(label=_("Autre adresse - Code postal"), name="adresse_cp", value="")
        propriete.SetHelpString(_("Saisissez le code postal de l'adresse"))
        self.Append(propriete)

        propriete = wxpg.StringProperty(label=_("Autre adresse - Ville"), name="adresse_ville", value="")
        propriete.SetHelpString(_("Saisissez la ville de l'adresse"))
        self.Append(propriete)


        # Comptabilité
        self.Append( wxpg.PropertyCategory(_("Comptabilité")) )

        propriete = wxpg.StringProperty(label=_("Code comptable"), name="code_comptable", value=u"")
        propriete.SetHelpString(_("Saisissez le code comptable de la famille (Utilisé pour les exports vers logiciels de compta)"))
        self.Append(propriete)
...
class Panel(wx.Panel):
    ...
    def __set_properties(self):
        ...
        self.bouton_historique.SetToolTip(wx.ToolTip(_("Consulter et traiter les demandes de la famille")))
...
##            return False
        ...
        if titulaire_helios != None :
            ...
            for IDrattachement, IDindividu, IDcategorie, titulaire in listeRattachements:
                if titulaire_helios == IDindividu and titulaire == 0 :
                    dlg = wx.MessageDialog(self, _("Attention, le titulaire Hélios doit être obligatoirement un titulaire du dossier !"), "Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
                    dlg.ShowModal()
                    dlg.Destroy()
                    return False
    ...
    def Envoyer_email(self, event):
        ...
        if listeAdresses == False or len(listeAdresses) == 0 :
            dlg = wx.MessageDialog(self, _("Il n'y a aucune adresse email !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
