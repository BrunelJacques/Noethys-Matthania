...
from Ol import OL_Perceptions



class Dialog(wx.Dialog):
    def __init__(self, parent, mode="gestion"):
        wx.Dialog.__init__(self, parent, -1, name="DLG_Perceptions", style=wx.DEFAULT_DIALOG_STYLE|wx.RESIZE_BORDER|wx.MAXIMIZE_BOX|wx.MINIMIZE_BOX)
        self.parent = parent
        self.mode = mode
        
        if self.mode == "selection" :
            intro = _("Vous pouvez ici sélectionner une perception. Double-cliquez sur une ligne pour effectuer rapidement la sélection.")
            titre = _("Sélection d'une perception")
            self.SetTitle(_("Sélection d'une perception"))
        else:
            intro = _("Vous pouvez ici saisir, modifier ou supprimer des perceptions. Celles-ci sont utilisées dans le cadre des prélèvements automatiques pour les collectivités publiques.")
            titre = _("Gestion des perceptions")
            self.SetTitle(_("Gestion des perceptions"))
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Banque.png")
        self.ctrl_listview = OL_Perceptions.ListView(self, id=-1, style=wx.LC_REPORT|wx.SUNKEN_BORDER|wx.LC_SINGLE_SEL|wx.LC_HRULES|wx.LC_VRULES)
        self.ctrl_listview.MAJ()
        self.ctrl_recherche = OL_Perceptions.CTRL_Outils(self, listview=self.ctrl_listview)
    ...
    def __set_properties(self):
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter une perception")))
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier la perception sélectionnée dans la liste")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer la perception sélectionnée dans la iste")))
    ...
    def GetIDperception(self):
        selection = self.ctrl_listview.Selection()
        if len(selection) == 0 :
            return None
        else:
            return selection[0].IDperception
        
    def OnBouton_ok(self, event):
        IDperception = self.GetIDperception()
        if IDperception == None :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune perception dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
