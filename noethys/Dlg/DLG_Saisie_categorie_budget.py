...
class Hyperlien(Hyperlink.HyperLinkCtrl):
    ...
    def OnLeftLink(self, event):
        ...
        for IDunite, nomUnite, nomActivite in listeDonnees :
            ...
            listeLabels.append(_("%s : Quantité de '%s' %s") % (nomActivite, nomUnite, nomChamp))
...
class Dialog(wx.Dialog):
    def __init__(self, parent, typeCategorie="credit", track=None):
        ...
        self.label_categorie = wx.StaticText(self, wx.ID_ANY, _("Catégorie :"))
        ...
        self.label_valeur = wx.StaticText(self, wx.ID_ANY, _("Plafond :"))
        
        self.radio_montant = wx.RadioButton(self, -1, _("Montant :"), style=wx.RB_GROUP)
        self.ctrl_montant = CTRL_Saisie_euros.CTRL(self)
        self.radio_formule = wx.RadioButton(self, -1, _("Formule de calcul :"))
        self.ctrl_formule = wx.TextCtrl(self, wx.ID_ANY, "", style=wx.TE_MULTILINE)
        self.ctrl_formule.SetMinSize((400, 100)) 
        
        self.hyper_champ = Hyperlien(self, label=_("Insérer un champ"), infobulle=_("Cliquez ici pour insérer un champ"), URL="")
        ...
        # Init contrôles
        if self.track != None :
            titre = _("Modification d'une catégorie budgétaire")
            self.Importation()
        else :
            titre = _("Saisie d'une catégorie budgétaire")
        if self.typeCategorie == "credit" : titre += _(" au crédit")
        if self.typeCategorie == "debit" : titre += _(" au débit")
        self.SetTitle(titre)
        self.OnRadioValeur(None)

    def __set_properties(self):
        self.ctrl_categorie.SetToolTip(wx.ToolTip(_("Sélectionnez la catégorie comptable à associer")))
        self.bouton_categories.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à la gestion des catégories comptables")))
        self.ctrl_montant.SetToolTip(wx.ToolTip(_("Saisissez un montant plafonf (Ex : 200.00)")))
        self.ctrl_formule.SetToolTip(wx.ToolTip(_("Saisissez une formule de calcul du plafond (Ex : {NBRE_UNITE_1}*3.00)")))
    ...
    def OnBoutonOk(self, event): 
        IDcategorie = self.ctrl_categorie.GetID()
        if IDcategorie == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner une catégorie !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.radio_montant.GetValue() == True :
            ...
            if valeur == "" or valeur == None :
                dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une formule de calcul !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_formule.SetFocus()
                return False
