...
class CaseOuverture():
    ...
    def GetTexteInfobulle(self):
        if self.ouvert == False :
            pied = _("Cliquez pour ouvrir cette unité")
            texte = _("Cette unité est fermée.\n")
        else :
            texte = _("Cette unité est ouverte.\n")
            pied = _("Cliquez pour fermer cette unité")

            # Conso associées
            if self.nbre_conso > 0 :
                texte += _("Vous ne pouvez pas la fermer car %d consommations y ont déjà été associées\n") % self.nbre_conso
        ...
        dictDonnees = {
            "couleur" : couleur,
            "titre" : "%s - %s" % (label_date, self.nomGroupe),
            "texte" : texte + "\n",
            "pied" : pied,
            }
        return dictDonnees
...
class Calendrier(gridlib.Grid, glr.GridWithLabelRenderersMixin): 
    ...
    def OnLabelRightClick(self, event):
        ...
        # Item Copier
        item = wx.MenuItem(menuPop, 2000+numLigne, _("Copier"))
        ...
        # Item Coller
        item = wx.MenuItem(menuPop, 3000+numLigne, _("Coller"))
        ...
        # Item Reinitialiser
        item = wx.MenuItem(menuPop, 4000+numLigne, _("Réinitialiser cette date"))
    ...
    def SaisieLot(self):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            try :
                dlgAttente = wx.BusyInfo(_("Veuillez patienter durant l'opération..."), None)
                ...
                dlg2 = wx.MessageDialog(self, _("Désolé, le problème suivant a été rencontré dans le traitement par lot : \n\n%s") % err, _("Erreur"), wx.OK | wx.ICON_ERROR)
                dlg2.ShowModal()
                dlg2.Destroy()
        dlg.Destroy()
    ...
    def ReInitDate(self, event):
        ...
        # dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment réinitialiser les paramètres de la date du %s ?") % DateComplete(dateDD), _("Réinitialisation"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
        ...
        if (numLigne, numColonne) in self.dictCases :
            ...
            if infoCase["type"] == "ouverture" and infoCase["actif"] == True :
                ...
                if len(case.liste_evenements) > 0 :
                    if silencieux == False :
                        dlg = wx.MessageDialog(None, _("%s évènement(s) sont déjà associé(s) à cette unité de consommation.\n\nConfirmez-vous leur suppression ?") % len(case.liste_evenements), _("Avertissement"), wx.YES_NO | wx.NO_DEFAULT | wx.CANCEL | wx.ICON_QUESTION)
    ...
    def GetListeGroupes(self, DB):
        ...
        if len(listeGroupes) == 0 :
            listeGroupes.append({ "IDgroupe" : 0, "nom" : _("Sans groupe") })
        # Tous les groupes
        if self.afficherTousGroupes == True :
            listeGroupes.append({ "IDgroupe" : None, "nom" : _("Total max.") })
        return listeGroupes
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDactivite=None):
        ...
        intro = _("Ce calendrier vous permet de définir les jours d'ouverture de l'activité et le nombre maximal de places. Cliquez sur les cases pour indiquer que l'unité est fonctionnelle et saisissez directement les effectifs maximals en double-cliquant sur les cases blanches. <U>Important :</U> Cliquez avec le bouton droit de la souris sur les cases Dates pour utiliser le Copier-Coller.")
        titre = _("Calendrier des ouvertures et des évènements")
        ...
        self.listeLegende = [
            ...
            { "label" : _("Vacances"), "couleur" : COULEUR_VACANCES, "ctrl_label" : None, "ctrl_img" : None },
            { "label" : _("Férié"), "couleur" : COULEUR_FERIE, "ctrl_label" : None, "ctrl_img" : None },
            { "label" : _("Effectifs max."), "couleur" : (255, 255, 255), "ctrl_label" : None, "ctrl_img" : None },
            ]
        index = 0
        ...
        self.bouton_annuler = CTRL_Bouton_image.CTRL(self, id=wx.ID_CANCEL, texte=_("Annuler"), cheminImage="Images/32x32/Fermer.png")
    ...
    def __set_properties(self):
        self.SetTitle(_("Calendrier des ouvertures"))
        self.ctrl_mois.SetToolTip(wx.ToolTip(_("Sélectionnez un mois")))
        self.ctrl_annee.SetMinSize((70, -1))
        self.ctrl_annee.SetToolTip(wx.ToolTip(_("Sélectionnez une année")))
        self.check_tous_groupes.SetToolTip(wx.ToolTip(_("Cochez cette case pour afficher les lignes 'Total max.' afin de saisir des limitations d'effectifs globales")))
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez ici pour obtenir de l'aide")))
        self.bouton_saisie_lot.SetToolTip(wx.ToolTip(_("Cliquez ici saisir ou supprimer un lot d'ouvertures")))
    ...
    def OnBoutonOk(self, event):
        ...
        try :
            dlgAttente = wx.BusyInfo(_("Veuillez patienter durant la sauvegarde des données..."), None) # .PyBusyInfo(_("Veuillez patienter durant la sauvegarde des données..."), parent=None, title=_("Enregistrement"), icon=wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Logo.png"), wx.BITMAP_TYPE_ANY))
            if 'phoenix' not in wx.PlatformInfo:
                wx.Yield()
            ...
            del dlgAttente
            traceback.print_exc(file=sys.stdout)
            dlg = wx.MessageDialog(self, _("Désolé, le problème suivant a été rencontré dans la sauvegarde des ouvertures : \n\n%s") % err, _("Erreur"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
