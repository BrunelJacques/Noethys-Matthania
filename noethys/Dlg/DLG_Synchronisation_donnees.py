...
class Dialog(wx.Dialog):
    def __init__(self, parent, listeFichiers=None):
        ...
        # Bandeau
        intro = _("Cliquez simplement sur le bouton Importer pour importer dans Noethys les données cochées.")
        ...
        # Données
        self.box_donnees_staticbox = wx.StaticBox(self, wx.ID_ANY, _("Données à importer"))
        ...
        self.check_cacher_doublons = wx.CheckBox(self, -1, _("Cacher les doublons"))
        self.check_cacher_doublons.SetValue(True)
        self.label_regroupement = wx.StaticText(self, -1, _("Regrouper par"))
        self.ctrl_regroupement = wx.Choice(self, -1, choices=[_("Catégorie"), _("Action"), _("Individu")])
        self.ctrl_regroupement.Select(0) 
        
        # Journal
        self.box_journal_staticbox = wx.StaticBox(self, wx.ID_ANY, _("Journal d'évènements"))
        self.ctrl_journal = wx.TextCtrl(self, wx.ID_ANY, "", style=wx.TE_MULTILINE | wx.TE_READONLY)
    ...
    def __set_properties(self):
        self.check_cacher_doublons.SetToolTip(wx.ToolTip(_("Cochez cette case pour cacher les doublons")))
        self.bouton_apercu.SetToolTip(wx.ToolTip(_("Cliquez ici pour afficher un aperçu avant impression de la liste")))
        ...
        self.bouton_enregistrer.SetToolTip(wx.ToolTip(_("Cliquez ici pour enregistrer le contenu du journal dans un fichier")))
        self.ctrl_regroupement.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner une colonne de regroupement")))
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez ici pour obtenir de l'aide")))
        self.bouton_ok.SetToolTip(wx.ToolTip(_("Cliquez ici pour lancer l'importation des données")))
    ...
    def OnBoutonEnregistrer(self, event):
        ...
        wildcard = _("Tous les fichiers (*.*)|*.*")
        dlg = wx.FileDialog(None, message=_("Sélectionnez un répertoire et un nom de fichier"), defaultDir=standardPath.GetDocumentsDir(),  defaultFile="journal.txt", wildcard=wildcard, style=wx.FD_SAVE)
        nomFichier = None
    ...
    def OnBoutonOk(self, event):  
        ...
        if len(listeTracks) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez coché aucune action à importer !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return 
    ...
    def ArchiverFichiers(self):
        ...
        if nbreErreurs > 0 or nbreNonTraites > 0 :
            if nbreErreurs > 0 and nbreNonTraites == 0 : intro = _("%d erreurs ont été trouvées lors de l'importation. ") % nbreErreurs
            if nbreErreurs == 0 and nbreNonTraites > 0 : intro = _("%d actions n'ont pas été traitées. ") % nbreNonTraites
            if nbreErreurs > 0 and nbreNonTraites > 0 : intro = _("%d actions n'ont pas été traitées et %d erreurs ont été trouvées. ") % (nbreErreurs, nbreNonTraites)
            dlg = wx.MessageDialog(self, _("%s\n\nConfirmez-vous tout de même l'archivage des fichiers de synchronisation traités ?\n(Si vous choisissez Non, les fichiers seront conservés)") % intro, _("Archivage"), wx.YES_NO|wx.CANCEL|wx.YES_DEFAULT|wx.ICON_QUESTION)
...
class Traitement(Thread): 
    ...
    def run(self):
        ...
        try: 
                ...
                if track.anomalie != False :
                    ...
                    if track.categorie == "consommation" :
                        ...
                        if resultat == True :
                            ...
                            self.parent.parent.EcritLog(track.detail + " -> ok")
            ...
            if self.succes == True :
                self.parent.label_intro.SetLabel(_("Traitement terminé"))
                self.parent.parent.EcritLog(_("Traitement terminé"))
                self.parent.Fermer(forcer=True) 
            else:
                self.parent.label_intro.SetLabel(_("Traitement interrompu par l'utilisateur"))
                self.parent.parent.EcritLog(_("Traitement interrompu par l'utilisateur"))
        ...
        if len(listeAnomalies) > 0 :
            introduction = _("%d actions ont été importées avec succès et %d anomalies ont été trouvées :") % (nbre_tracks - len(listeAnomalies), len(listeAnomalies))
            conclusion = ""
            dlg = DLG_Messagebox.Dialog(None, titre=_("Information"), introduction=introduction, detail=u"\n".join(listeAnomalies), conclusion=conclusion, icone=wx.ICON_EXCLAMATION, boutons=[_("Ok"),])
            reponse = dlg.ShowModal() 
            dlg.Destroy() 
        else :
            dlg = wx.MessageDialog(None, _("Les %d actions ont été importées avec succès !") % nbre_tracks, _("Information"), wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
...
class Dialog_Traitement(wx.Dialog):
    def __init__(self, parent, listeTracks=[], debug=False):
        ...
        self.label_intro = wx.StaticText(self, -1, _("Initialisation..."))
    ...
    def __set_properties(self):
        self.SetTitle(_("Importation des données"))
        self.bouton_ok.SetToolTip(wx.ToolTip(_("Cliquez ici pour lancer l'importation des données")))
        self.bouton_fermer.SetToolTip(wx.ToolTip(_("Cliquez ici pour fermer")))
        if self.debug == False :
    ...
    def __do_layout(self):
        ...
        grid_sizer_base.Add(self.label_intro, 0, wx.EXPAND|wx.ALIGN_CENTER_VERTICAL|wx.LEFT|wx.RIGHT|wx.TOP, 10)
    ...
    def Fermer(self, forcer=False):
        ...
        if TraitmentEnCours:
            if forcer == True :
                ...
                dlgConfirm = wx.MessageDialog(self, _("Souhaitez-vous vraiment arrêter le traitement ?"), _("Confirmation d'arrêt"), wx.YES_NO|wx.NO_DEFAULT|wx.ICON_QUESTION)
                ...
                self.traitement.abort()
                self.label_intro.SetLabel(_("Vous avez interrompu le traitement."))
    ...
    def Demarrer(self):
        self.parent.EcritLog(_("Lancement du traitement"))
