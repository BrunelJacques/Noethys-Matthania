...
class CTRL_Options(CTRL_Propertygrid.CTRL):
    ...
    def Remplissage(self):
        ...
        propriete.SetHelpString(_("Nombre de lignes vierges à afficher en fin de liste"))
        propriete.SetAttribute("obligatoire", True)
        propriete.SetAttribute("Min", 0)
        self.Append(propriete)

        # Insérer saut de page après groupe
        propriete = wxpg.BoolProperty(label=_("Insérer saut de page après groupe"), name="saut_page_groupe", value=True)
        propriete.SetHelpString(_("Cochez cette case pour insérer un saut de page après chaque groupe"))
        propriete.SetAttribute("UseCheckbox", True)
        self.Append(propriete)

        # Couleur du fond d'entête de colonne
        propriete = wxpg.ColourProperty(label=_("Couleur ligne des entêtes"), name="couleur_fond_entetes", value=wx.Colour(208, 208, 208) )
        propriete.SetHelpString(_("Sélectionnez la couleur de fond de la ligne des entêtes"))
        propriete.SetAttribute("obligatoire", True)
        self.Append(propriete)
        ...
        propriete.SetEditor("EditeurChoix")
        propriete.SetHelpString(_("Sélectionnez la largeur de la colonne Nom de l'individu (en pixels)"))
        propriete.SetAttribute("obligatoire", True)
        self.Append(propriete)

        # Catégorie
        self.Append(wxpg.PropertyCategory(_("Colonne Âge")))

        # Afficher l'âge des individus
        propriete = wxpg.BoolProperty(label=_("Afficher la colonne"), name="afficher_age", value=True)
        propriete.SetHelpString(_("Cochez cette case pour afficher de la colonne de l'âge des individus"))
        propriete.SetAttribute("UseCheckbox", True)
        self.Append(propriete)
        ...
        propriete.SetEditor("EditeurChoix")
        propriete.SetHelpString(_("Sélectionnez la largeur de la colonne Âge de l'individu (en pixels)"))
        propriete.SetAttribute("obligatoire", True)
        self.Append(propriete)
...
class CTRL_Parametres(wx.Notebook):
    def __init__(self, parent):
        ...
        self.listePages = [
            {"code": "generalites", "ctrl": Page_Generalites(self), "label": _("Paramètres"), "image": "Calendrier.png"},
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        intro = _("Vous pouvez ici imprimer une liste au format PDF des informations médicales des individus présents sur la période de votre choix. Pour une liste standard, sélectionnez simplement une période puis cliquez sur 'Aperçu'.")
        titre = _("Impression de la liste des informations médicales")
        ...
        self.ctrl_profil = CTRL_profil_perso(self.stbProfil, categorie="impression_infos_medicales", dlg=self)
    ...
    def __set_properties(self):
        self.SetTitle(_("Impression de la liste des informations médicales"))
    ...
    def OnBoutonOk(self, event):
        ...
        if not dictParametres["colonnes"]:
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement créer au moins une colonne !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        def Header():
            ...
            dataTableau.append( (_("Informations médicales"), _("%s\nEdité le %s") % (UTILS_Organisateur.GetNom(), dateDuJour)) )
            style = TableStyle([
        ...
        for IDactivite in listeActivites :
            ...
            if IDactivite in dictOuvertures :
                ...
                for IDgroupe in dictOuvertures[IDactivite] :
                    ...
                    labelsColonnes.append(_("Nom - prénom"))
