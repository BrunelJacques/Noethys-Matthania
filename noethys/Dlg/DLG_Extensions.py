...
class CTRL_Extensions(wx.ListBox):
    ...
    def Importer(self, event=None):
        ...
        wildcard = _("Module python (*.py)|*.py|Tous les fichiers (*.*)|*.*")
        sp = wx.StandardPaths.Get()
        dlg = wx.FileDialog(
            self, message=_("Choisissez une extension à importer"),
        ...
        if "def Extension(" not in texte:
            dlg = wx.MessageDialog(self, _("Cette extension ne semble pas valide.\n\nUne extension est un module Python qui doit obligatoirement contenir une fonction Extension()."), _("Erreur"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return False
    ...
    def Supprimer(self, event=None):
        if self.GetSelection() == -1:
            dlg = wx.MessageDialog(self, _("Aucune extension n'a été sélectionnée dans la liste !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False

        nom_fichier = self.GetStringSelection()
        dlg = wx.MessageDialog(self, _("Souhaitez-vous vraiment supprimer l'extension '%s' ?") % nom_fichier, _("Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        intro = _("Vous pouvez ici exécuter, importer ou supprimer des extensions. Une extension est un module Python qui apporte des fonctionnalités supplémentaires. Une extension doit contenir une fonction Extension().")
        titre = _("Extensions")
        self.SetTitle(titre)
    ...
    def __set_properties(self):
        self.bouton_importer.SetToolTip(wx.ToolTip(_("Cliquez ici pour importer une extension")))
        self.bouton_executer.SetToolTip(wx.ToolTip(_("Cliquez ici pour exécuter l'extension sélectionnée dans la liste ou double-cliquez sur la ligne correspondante dans la liste")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer l'extension sélectionnée dans la liste")))
