...
class Dialog(wx.Dialog):
    def __init__(self, parent, titre=_("Géolocalisation GPS")):
        wx.Dialog.__init__(self, parent, -1, style=wx.DEFAULT_DIALOG_STYLE|wx.RESIZE_BORDER|wx.MAXIMIZE_BOX|wx.MINIMIZE_BOX)
        self.parent = parent   
        
        # Bandeau
        intro = _("Cette fonctionnalité vous permet d'obtenir les coordonnées GPS d'un lieu. Saisissez une adresse complète ou une ville avant de cliquer sur Rechercher.")
        titre = _("Géolocalisation GPS")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Carte.png")
        
        # Recherche
        self.box_recherche_staticbox = wx.StaticBox(self, -1, _("Recherche"))
        ...
        self.bouton_rechercher = CTRL_Bouton_image.CTRL(self, texte=_("Rechercher"), cheminImage="Images/32x32/Loupe.png")
        
        # Résultats
        self.box_resultats_staticbox = wx.StaticBox(self, -1, _("Résultats"))
    ...
    def __set_properties(self):
        self.ctrl_rue.SetToolTip(wx.ToolTip(_("Saisissez un nom de rue [Optionnel]")))
        self.bouton_rechercher.SetToolTip(wx.ToolTip(_("Cliquez ici pour lancer la recherche")))
    ...
    def OnBoutonRechercher(self, event): 
        ...
        if ville == None or ville == "" :
            dlg = wx.MessageDialog(self, _("Vous devez saisir obligatoirement un nom de ville !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if resultats == None :
            texte = _("Aucun résultat \n")
        else :
            texte = _("Lat : %s  Long : %s \n") % (str(resultats["lat"]), str(resultats["long"]))
