...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        self.SetEmptyListMsg(_("Aucun visage détecté"))
...
##        self.SetSortColumn(self.columns[0])
    ...
    def Modifier(self, event):
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune photo à identifier dans la liste"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        intro = _("Vous pouvez ici importer un lot de photos individuelles depuis une photo de groupe. Chargez la photo et Noethys se chargera de détecter les visages. Vous devrez ensuite identifier chaque photo : Double-cliquez sur une photo pour l'associer à un individu. Cliquez sur Enregistrer pour terminer.")
    ...
    def OnBoutonFichier(self, event):
        ...
        if nbreIdentifiees > 0 :
            dlg = wx.MessageDialog(None, _("Souhaitez-vous vraiment charger une nouvelle image ?\n\nAttention, les %d photo(s) identifiée(s) non enregistrées seront annulées.") % nbreIdentifiees, _("Avertissement"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
    ...
    def OnBoutonEnregistrer(self, event):
        ...
        if len(tracks) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez identifié aucune photo dans la liste !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if DB.echec == 1 : 
            dlg = wx.MessageDialog(self, _("La base de données PHOTOS n'a pas été trouvée !"), _("Erreur"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            DB.Close() 
            return False
        ...
        dlg = wx.MessageDialog(self, _("Les %d photos identifiées ont été enregistrées avec succès dans les fiches individuelles !") % len(tracks), _("Confirmation"), wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
