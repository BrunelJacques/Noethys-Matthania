...
class Parametres(wx.Panel):
    def __init__(self, parent):
        ...
        # Mode
        self.staticbox_mode_staticbox = wx.StaticBox(self, -1, _("Type de règlements"))
        self.radio_saisis = wx.RadioButton(self, -1, _("Saisis"), style = wx.RB_GROUP)
        self.radio_deposes = wx.RadioButton(self, -1, _("Déposés"))
        self.radio_nondeposes = wx.RadioButton(self, -1, _("Non déposés"))
        ...
        self.radio_consommations = wx.CheckBox(self, -1, _("Conso."))
        self.radio_locations = wx.CheckBox(self, -1, _("Locations"))
        self.radio_autres = wx.CheckBox(self, -1, _("Autres"))
        self.radio_avoirs = wx.CheckBox(self, -1, _("Avoirs"))
...
##        self.ctrl_activites.SetMinSize((-1, 90))
    ...
    def __set_properties(self):
        ...
        self.radio_saisis.SetToolTip(wx.ToolTip(_("Cochez cette case pour considérer les règlements saisis dans Noethys sur la période de référence")))
        self.radio_deposes.SetToolTip(wx.ToolTip(_("Cochez cette case pour considérer les règlements déposés en banque sur la période de référence")))
        self.radio_deposes.SetToolTip(wx.ToolTip(_("Cochez cette case pour considérer les règlements pas encore déposés en banque à ce jour")))
        ...
        self.radio_avoirs.SetToolTip(wx.ToolTip(_("Cochez cette case pour afficher les avoirs (règlements non ventilés)")))
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        # Bandeau
        intro = _("Vous pouvez ici afficher la répartition des modes de règlements par activité. Commencez par sélectionner un mode de calcul, saisir une période de référence puis sélectionner les activités souhaitées. Il est ensuite possible d'exporter les résultats sous forme de PDF ou sous Ms Excel.")
        titre = _("Synthèse des modes de règlements")
        ...
        # Commandes
        self.label_ventilation = wx.StaticText(self, -1, _("Filtre de ventilation :"))
    ...
    def __set_properties(self):
        ...
        self.check_details.SetToolTip(wx.ToolTip(_("Cliquez ici pour afficher les détails dans les résultats")))
        self.ctrl_ventilation.SetToolTip(wx.ToolTip(_("Vous pouvez filtrer ici les résultats par période de ventilation")))
    ...
    def InitChoixAnneeVentilation(self):
        ...
        listeAnnees.sort()
        listeTemp = [_("Aucun"), _("Non ventilé"), _("La période suivante")]
    ...
    def GetLabelParametres(self):
        ...
        if mode == "saisis" : listeParametres.append(_("Règlements saisis du %s au %s") % (DateEngFr(str(date_debut)), DateEngFr(str(date_fin))))
        if mode == "deposes" : listeParametres.append(_("Règlements déposés du %s au %s") % (DateEngFr(str(date_debut)), DateEngFr(str(date_fin))))
        if mode == "nondeposes" : listeParametres.append(_("Règlements non déposés"))
        ...
        if ventilation != None :
            if ventilation == 0 : 
                listeParametres.append(_("Uniquement des règlements non ventilés"))
            elif type(ventilation) == list :
                listeParametres.append(_("Uniquement des règlements ventilés sur la période du %s au %s") % (UTILS_Dates.DateDDEnFr(ventilation[0]), UTILS_Dates.DateDDEnFr(ventilation[1])))
            else :
                listeParametres.append(_("Uniquement des règlements ventilés sur l'année %d") % ventilation)
