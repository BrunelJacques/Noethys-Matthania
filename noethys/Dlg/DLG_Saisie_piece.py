...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDpiece=None, IDfamille=None, IDindividu=None, dictFamillesRattachees={}):
        ...
        # Liste des pièces
        self.sizer_type_staticbox = wx.StaticBox(self, -1, _("Type de pièce"))
        if IDfamille != None :
            texte = _("la famille")
        else:
            texte = _("l'individu")
        self.radio_pieces_1 = wx.RadioButton(self, -1, _("Dans la liste des pièces que %s doit fournir :") % texte, style = wx.RB_GROUP)
        ...
        self.radio_pieces_2 = wx.RadioButton(self, -1, _("Dans la liste des autres types de pièces prédéfinis :"))
        self.ctrl_pieces_autres = Choix_Piece_autre(self, self.listePiecesObligatoires, self.dictTypesPieces)

        # Un type de pièce libre
        self.radio_pieces_3 = wx.RadioButton(self, -1, _("Un autre type de pièce :"))
        self.label_titre_piece = wx.StaticText(self, -1, "Titre :")
        self.ctrl_titre_piece = wx.TextCtrl(self, -1, "")

        # Date de début
        self.sizer_date_debut_staticbox = wx.StaticBox(self, -1, _("Date de début"))
        ...
        self.radio_date_fin_2 = wx.RadioButton(self, -1, _("Validité illimitée"))
        self.ctrl_date_fin = CTRL_Saisie_date.Date(self)
        
        # Pages capturées
        self.sizer_pages_staticbox = wx.StaticBox(self, -1, _("Documents associés"))
        ...
        if IDpiece == None :
            ...
            self.SetTitle(_("Modification d'une pièce"))
            self.Importation()
    ...
    def __set_properties(self):
        ...
        self.radio_pieces_2.SetToolTip(wx.ToolTip(_("Cliquez ici si la pièce que vous souhaitez enregistrer n'est pas dans la liste des pièces obligatoires à fournir")))
        self.radio_pieces_3.SetToolTip(wx.ToolTip(_("Cliquez ici si la pièce que vous souhaitez enregistrer n'est pas un type de pièce prédéfini")))
        self.ctrl_titre_piece.SetToolTip(wx.ToolTip(_("Saisissez un titre pour cette pièce")))
        self.ctrl_pieces_obligatoires.SetToolTip(wx.ToolTip(_("Sélectionnez un type de pièce en cliquant sur son nom")))
        self.ctrl_date_debut.SetToolTip(wx.ToolTip(_("Saisissez la date de début de validité.\nRemarque : Il s'agit bien de la date d'emission de la pièce \n(par exemple, la date d'obtention d'un diplôme) et non la date à laquelle vous avez reçue la pièce")))
        self.ctrl_date_fin.SetToolTip(wx.ToolTip(_("Saisissez la date d'expiration de la pièce")))
        self.radio_date_fin_1.SetToolTip(wx.ToolTip(_("Cliquez ici si la pièce a une durée de validité limitée dans le temps")))
        self.radio_date_fin_2.SetToolTip(wx.ToolTip(_("Cliquez ici si la pièce que vous souhaitez enregistrer a une durée de validité illimitée")))
        self.bouton_ajouter_page.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter un document")))
        self.bouton_supprimer_page.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer le document sélectionné")))
        self.bouton_visualiser_page.SetToolTip(wx.ToolTip(_("Cliquez ici pour visualiser le document sélectionné")))
        self.bouton_zoom_plus.SetToolTip(wx.ToolTip(_("Cliquez ici pour agrandir les vignettes")))
        self.bouton_zoom_moins.SetToolTip(wx.ToolTip(_("Cliquez ici pour réduire les vignettes")))
    ...
    def Sauvegarde(self):
        ...
        selection = self.GetSelectionPiece()
        if selection == None:
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun type de pièce !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if self.radio_date_fin_1.GetValue() == True:
            if self.ctrl_date_debut.GetDate() > self.ctrl_date_fin.GetDate():
                dlg = wx.MessageDialog(self, _("Attention, la date de début est supérieure à la date de fin !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_date_fin.SetFocus()
                return False
        ...
        if self.radio_pieces_3.GetValue() == True:
            ...
            if not titre:
                dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un titre pour cette pièce !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
        ...
        UTILS_Historique.InsertActions([{
            ...
            "action": _("%s de la pièce ID%d '%s' %s") % (type, self.IDpiece, nomPiece, texteDetail),
        }, ])
