...
##        self.GetMainWindow().Bind(wx.EVT_RIGHT_UP, self.OnContextMenu)
    ...
    def Initialisation(self):      
        ...
        self.AddColumn(_("Individu/Forfait"))
        self.SetMainColumn(0)
        self.SetColumnWidth(0, 230)
        self.AddColumn(_("Activité"))
        self.SetColumnWidth(1, 60)
        self.AddColumn(_("Tarif"))
            ...
            if IDfamille == self.IDfamille :
                ...
                for IDindividu, dictIndividu in dictFamille["individus"].items() :
                    ...
                    if len(dictIndividu["forfaits"]) > 0 :
                        label = "%s %s" % (dictIndividu["nom"], dictIndividu["prenom"])
    ...
    def CreationBrancheTarif(self, brancheParente, dictTarif):
        ...
        self.SetItemText(branche, "%.2f %s " % (montant, SYMBOLE), 2)
        return branche
...
class Dialog(wx.Dialog):
    def __init__(self, parent, grille=None, listeFamilles=[]):
        ...
        self.label_dates = wx.StaticText(self, -1, _("Validité du :"))
        self.ctrl_date_debut = CTRL_Saisie_date.Date2(self)
        self.label_au = wx.StaticText(self, -1, _("au"))
        self.ctrl_date_fin = CTRL_Saisie_date.Date2(self, activeCallback=False)

        # Forfaits
        self.box_forfaits_staticbox = wx.StaticBox(self, -1, _("Forfaits disponibles"))
        self.ctrl_forfaits = CTRL_Forfaits(self, grille=self.grille)

        # Prestation
        self.box_prestation_staticbox = wx.StaticBox(self, -1, _("Prestation"))
    ...
    def __set_properties(self):
        self.SetTitle(_("Saisie d'un forfait crédit"))
        self.ctrl_forfaits.SetToolTip(wx.ToolTip(_("Sélectionnez un forfait à créer")))
...
##        grid_sizer_base.Add(grid_sizer_famille, 1, wx.LEFT|wx.RIGHT|wx.TOP|wx.EXPAND, 10)
    ...
    def OnBoutonOk(self, event=None):
        ...
        if self.ctrl_famille.GetIDfamille() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner une famille !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if dictTarif == None :
            dlg = wx.MessageDialog(self, _("Vous devez sélectionner un tarif dans la liste proposée !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_forfaits.SetFocus()
            return False
        ...
        if len(self.ctrl_label.GetValue()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez saisir un label pour cette prestation !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.ctrl_date_prestation.GetDate() == None :
            dlg = wx.MessageDialog(self, _("Vous devez saisir une date pour cette prestation !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.ctrl_montant.GetMontant() == None :
            dlg = wx.MessageDialog(self, _("Vous devez saisir un montant pour cette prestation !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
