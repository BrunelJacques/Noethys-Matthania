...
class CTRL_Vacances(wx.Choice):
    def __init__(self, parent):
        self.listePeriodes = (_("Février"), _("Pâques"), _("Eté"), _("Toussaint"), _("Noël"))
        wx.Choice.__init__(self, parent, -1, choices=self.listePeriodes)
        self.parent = parent
        self.Select(0)
    
    def SetPeriode(self, nom=_("Février")):
...
class CTRL_Mode(wx.Panel):
    def __init__(self, parent):
        ...
        self.radio_presents = wx.RadioButton(self, -1, _("Présents sur la période suivante :"))
        
        self.radio_vacances = wx.RadioButton(self, -1, _("Vacances :"), style=wx.RB_GROUP)
        self.ctrl_vacances = CTRL_Vacances(self)
        self.ctrl_vacances_annee = CTRL_Annee(self)
        
        self.radio_mois = wx.RadioButton(self, -1, _("Mois :"))
        ...
        self.radio_dates = wx.RadioButton(self, -1, _("Dates :"))
        self.ctrl_date_debut = CTRL_Saisie_date.Date2(self)
        self.label_separation_dates = wx.StaticText(self, -1, "à")
    ...
    def __set_properties(self):
        self.radio_inscrits.SetToolTip(wx.ToolTip(_("Cochez ici pour baser les calculs sur les inscrits")))
        self.radio_presents.SetToolTip(wx.ToolTip(_("Cochez ici pour baser les calculs sur les présents sur une période donnée")))
    ...
    def GetPeriodeVacances(self):
        ...
        DB = GestionDB.DB()        
        req = _("SELECT nom, annee, date_debut, date_fin FROM vacances ORDER BY date_debut;")
        ...
        if self.radio_presents.GetValue() == True :
            ...
            if self.radio_vacances.GetValue() == True :
                ...
                if date_debut == None or date_fin == None :
                    dlg = wx.MessageDialog(self, _("La période de vacances que vous avez sélectionné n'a pas été paramétré !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                    dlg.ShowModal()
                    dlg.Destroy()
                    return False
                label = _("Les vacances de %s %d (du %s au %s)") % (vacances, annee, DateEngFr(str(date_debut)), DateEngFr(str(date_fin)))
            ...
            if self.radio_mois.GetValue() == True :
                ...
                if mois == None :
                    dlg = wx.MessageDialog(self, _("L'année que avez saisi n'est pas valide !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                    dlg.ShowModal()
                    dlg.Destroy()
                    return False
                label = _("Le mois de %s %d (du %s au %s)") % (LISTE_MOIS[mois-1], annee, DateEngFr(str(date_debut)), DateEngFr(str(date_fin)))
            ...
            if self.radio_annee.GetValue() == True :
                ...
                if annee == None :
                    ...
                    dlg.Destroy()
                    return False
                label = _("L'année %d (du %s au %s)") % (annee, DateEngFr(str(date_debut)), DateEngFr(str(date_fin)))
            ...
            if self.radio_dates.GetValue() == True :
                ...
                if date_debut == None or self.ctrl_date_debut.Validation() == False :
                    dlg = wx.MessageDialog(self, _("La date de début que avez saisi n'est pas valide !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                    dlg.ShowModal()
                    dlg.Destroy()
                    return False
                if date_fin == None or self.ctrl_date_fin.Validation() == False :
                    dlg = wx.MessageDialog(self, _("La date de fin que avez saisi n'est pas valide !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                    dlg.ShowModal()
                    dlg.Destroy()
                    return False
                if date_debut > date_fin :
                    dlg = wx.MessageDialog(self, _("La date de début est supérieure à la date de fin !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                    dlg.ShowModal()
                    dlg.Destroy()
                    return False
                label = _("Du %s au %s") % (DateEngFr(str(date_debut)), DateEngFr(str(date_fin)))
...
##        self.label_presents = wx.StaticText(self, -1, _("Méthode de calcul :"))
##        self.radio_presents_1 = wx.RadioButton(self, -1, _("Présents"), style = wx.RB_GROUP)
##        self.radio_presents_2 = wx.RadioButton(self, -1, _("Inscrits"))
...
##        self.ctrl_date_fin.SetDate("31/12/%d" % dateJour.year)
    ...
    def __set_properties(self):
        self.SetTitle(_("Sélection des paramètres"))
