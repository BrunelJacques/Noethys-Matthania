...
class MyDialog(wx.Dialog):
    def __init__(self, parent):
        ...
        self.label_intro = wx.StaticText(self, -1, _("""Cette fonctionnalité vous permet de générer automatiquement les jours fériés variables\nd'une ou plusieurs années selon des algorithmes de calcul intégrés. Saisissez une\nannée de départ, renseignez le nombre d'années à générer puis cochez les fériés à créer."""))

        self.label_nbre = wx.StaticText(self, -1, _("Nombre d'années à générer :"))
        self.ctrl_nbre = wx.SpinCtrl(self, -1, "", min=1, max=50)

        self.label_annee = wx.StaticText(self, -1, _("Depuis l'année :"))
        self.ctrl_annee = wx.SpinCtrl(self, -1, "", min=1970, max=2999)
        self.ctrl_annee.SetValue(datetime.date.today().year)

        self.label_jours = wx.StaticText(self, -1, _("Cochez les fériés à générer :"))
        listeJours = [_("Lundi de Pâques"), _("Jeudi de l'ascension"), _("Lundi de Pentecôte")]
    ...
    def __set_properties(self):
        self.SetTitle(_("Saisie automatique des jours fériés variables"))
        self.ctrl_nbre.SetToolTip(wx.ToolTip(_("Saisissez le nombre d'années à générer")))
        self.ctrl_annee.SetToolTip(wx.ToolTip(_("Saisissez l'année de départ")))
        self.ctrl_jours.SetToolTip(wx.ToolTip(_("Cochez les jours fériés à créer")))
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez ici pour obtenir de l'aide")))
        self.bouton_ok.SetToolTip(wx.ToolTip(_("Cliquez ici pour créer les jours fériés")))
    ...
    def OnBoutonOk(self, event): 
        ...
        if len(listeCoches) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement cocher au moins un jour férié à créer !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        for annee in listeAnnees :
            ...
            if 0 in listeCoches : SauvegarderDate(_("Lundi de Pâques"), lundi_paques)
            
            # Ascension
            ascension = dimanche_paques + relativedelta(days=+39)
            if 1 in listeCoches : SauvegarderDate(_("Jeudi de l'Ascension"), ascension)

            # Pentecote
            pentecote = dimanche_paques + relativedelta(days=+50)
            if 2 in listeCoches : SauvegarderDate(_("Lundi de Pentecôte"), pentecote)
