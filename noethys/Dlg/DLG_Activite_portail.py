...
class Page_periodes(wx.Panel):
    def __init__(self, parent, IDactivite=None):
        ...
        self.staticbox_periodes_staticbox = wx.StaticBox(self, -1, _("Périodes de réservations"))
        ...
        # Properties
        self.bouton_periodes_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour créer une période de réservations")))
        self.bouton_periodes_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier la période de réservations sélectionnée dans la liste")))
        self.bouton_periodes_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer la période de réservations selectionnée dans la liste")))
...
class Page_unites(wx.Panel):
    def __init__(self, parent, IDactivite=None):
        ...
        self.staticbox_unites_staticbox = wx.StaticBox(self, -1, _("Unités de réservations"))
        ...
        self.check_unites_multiples = wx.CheckBox(self, -1, _("Sélection multiple d'unités de réservations autorisée"))
        ...
        # Properties
        self.bouton_unites_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour créer une unité de réservation")))
        self.bouton_unites_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier l'unité de réservation sélectionnée dans la liste")))
        self.bouton_unites_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer l'unité de réservation sélectionnée dans la liste")))
        self.bouton_unites_monter.SetToolTip(wx.ToolTip(_("Cliquez ici pour monter l'unité de réservation dans la liste")))
        self.bouton_unites_descendre.SetToolTip(wx.ToolTip(_("Cliquez ici pour descendre l'unité de réservation dans la liste")))
        self.check_unites_multiples.SetToolTip(wx.ToolTip(_("Cochez cette case pour autoriser l'internaute à cocher plusieurs unités à la fois")))
...
class CTRL_Date_limite(wx.Choice):
    ...
    def MAJ(self):
        self.liste_donnees = [
            (1000, _("Lundi précédent")),
            (1001, _("Mardi précédent")),
            (1002, _("Mercredi précédent")),
            (1003, _("Jeudi précédent")),
            (1004, _("Vendredi précédent")),
            (1005, _("Samedi précédent")),
            (1006, _("Dimanche précédent")),
            (2000, _("Lundi de la semaine précédente")),
            (2001, _("Mardi de la semaine précédente")),
            (2002, _("Mercredi de la semaine précédente")),
            (2003, _("Jeudi de la semaine précédente")),
            (2004, _("Vendredi de la semaine précédente")),
            (2005, _("Samedi de la semaine précédente")),
            (2006, _("Dimanche de la semaine précédente")),
            (0, _("Jour J")),
        ]
        for x in range(1, 31):
            self.liste_donnees.append((x, _("Jour J-%d") % x))
...
class Page_options(wx.Panel):
    def __init__(self, parent, IDactivite=None):
        ...
        liste_jours = [_("Jour J"),]
        for x in range(1, 31):
            liste_jours.append(liste_jours[0] + str(-x))

        # Date limite de modification
        self.staticbox_date_limite = wx.StaticBox(self, -1, _("Limite de modification"))
        self.check_date_limite = wx.CheckBox(self, -1, _("Une réservation peut être ajoutée, modifiée ou supprimée jusqu'à"))
        self.ctrl_date_limite = CTRL_Date_limite(self)
        self.ctrl_heure_limite = CTRL_Saisie_heure.Heure(self)
        self.check_limite_weekends = wx.CheckBox(self, -1, _("Exclure les week-ends"))
        self.check_limite_feries = wx.CheckBox(self, -1, _("Exclure les jours fériés"))

        # Absence injustifiée
        self.staticbox_absenti = wx.StaticBox(self, -1, _("Absence injustifiée"))
        self.check_absenti = wx.CheckBox(self, -1, _("L'état 'Absence injustifiée' est attribué aux réservations modifiées ou supprimées après"))
    ...
    def Validation(self):
        if self.check_date_limite.GetValue() == True :
            if self.ctrl_heure_limite.GetHeure() == None or self.ctrl_heure_limite.Validation() == False :
                dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une heure limite valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
...
class Panel(wx.Panel):
    def __init__(self, parent, IDactivite=None, nouvelleActivite=False):
        ...
        # Inscriptions
        self.box_inscriptions_staticbox = wx.StaticBox(self, -1, _("Inscriptions sur le portail"))
        self.radio_inscriptions_non = wx.RadioButton(self, -1, _("Ne pas autoriser"), style=wx.RB_GROUP)
        self.radio_inscriptions_oui = wx.RadioButton(self, -1, _("Autoriser"))
        self.radio_inscriptions_dates = wx.RadioButton(self, -1, _("Autoriser uniquement du"))
        ...
        self.label_inscriptions_date_fin = wx.StaticText(self, -1, _("au"))
        self.ctrl_inscriptions_date_fin = CTRL_Saisie_date.Date2(self)
        self.ctrl_inscriptions_heure_fin = CTRL_Saisie_heure.Heure(self)

        # Réservations
        self.box_reservations_staticbox = wx.StaticBox(self, -1, _("Réservations sur le portail"))
        self.radio_reservations_non = wx.RadioButton(self, -1, _("Ne pas autoriser"), style=wx.RB_GROUP)
        self.radio_reservations_oui = wx.RadioButton(self, -1, _("Autoriser"))
        ...
        self.notebook_reservations.AddPage(self.page_periodes, _("Périodes"))
        self.notebook_reservations.AddPage(self.page_unites, _("Unités"))
        self.notebook_reservations.AddPage(self.page_options, _("Options"))
    ...
    def __set_properties(self):
        self.radio_inscriptions_oui.SetToolTip(wx.ToolTip(_("Sélectionnez cette option pour autoriser l'inscription à cette activité sur le portail")))
        self.radio_inscriptions_dates.SetToolTip(wx.ToolTip(_("Sélectionnez cette option pour autoriser l'inscription à cette activité sur le portail uniquement entre les dates souhaitées")))
        self.radio_inscriptions_non.SetToolTip(wx.ToolTip(_("Sélectionnez cette option pour ne pas autoriser l'inscription à cette activité sur le portail")))
        self.radio_reservations_oui.SetToolTip(wx.ToolTip(_("Sélectionnez cette option pour autoriser les réservations à cette activité sur le portail")))
        self.radio_reservations_non.SetToolTip(wx.ToolTip(_("Sélectionnez cette option pour ne pas autoriser les réservations à cette activité sur le portail")))
    ...
    def Validation(self):
        if self.radio_inscriptions_dates.GetValue() == True :
            ...
            if affichage_date_debut == None :
                dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de début !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
            ...
            if affichage_heure_debut == None or self.ctrl_inscriptions_heure_debut.Validation() == False :
                dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une heure de début valide pour l'affichage !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
            ...
            if affichage_date_fin == None :
                dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de fin !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
            ...
            if affichage_heure_fin == None or self.ctrl_inscriptions_heure_fin.Validation() == False :
                dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une heure de fin valide pour l'affichage !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
