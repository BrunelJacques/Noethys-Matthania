...
from Ol import OL_Saisie_lot_inscriptions
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        intro = _("Vous pouvez saisir ici un lot d'inscriptions. Sélectionnez les paramètres de l'inscription, cochez les individus à inscrire puis cliquez sur le bouton Ok. Utilisez la commande de Filtrage de liste pour effectuer une sélection rapide des individus.")
        titre = _("Saisir un lot d'inscriptions")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Activite.png")

        # Paramètres
        self.staticbox_parametres_staticbox = wx.StaticBox(self, -1, _("Paramètres de l'inscription"))
        ...
        self.label_categorie = wx.StaticText(self, -1, _("Catégorie de tarif :"))
        self.ctrl_categorie = CTRL_Choix(self, mode="categories")
        self.ctrl_categorie.SetMinSize((150, -1))
                
        # Liste
        self.staticbox_individus_staticbox = wx.StaticBox(self, -1, _("Individus à inscrire"))
    ...
    def OnBoutonOk(self, event):
        ...
        if IDactivite == None :
            dlg = wx.MessageDialog(self, _("Vous devez sélectionner une activité dans la liste proposée !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return

        if IDgroupe == None :
            dlg = wx.MessageDialog(self, _("Vous devez sélectionner un groupe dans la liste proposée !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return

        if IDcategorie_tarif == None :
            dlg = wx.MessageDialog(self, _("Vous devez sélectionner une catégorie de tarif dans la liste proposée !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
