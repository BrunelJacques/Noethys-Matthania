...
TEXTE_INTRO = _("Veuillez trouver ci-dessous le montant réglé à notre organisme sur la période du {DATE_DEBUT} au {DATE_FIN} pour la garde de votre ou vos enfants de moins de 7 ans :")
...
class CTRL_Options(wx.Panel):
    def __init__(self, parent):
        ...
        self.ctrl_montant = wx.Choice(self, -1, choices=[_("Réglé"), _("Facturé")])
...
##        self.radio_condense = wx.RadioButton(self, -1, _("Condensé"), style=wx.RB_GROUP)
##        self.radio_detaille = wx.RadioButton(self, -1, _("Détaillé"))

        # Répertoire
        self.label_repertoire = wx.StaticText(self, -1, _("Copie :"))
        self.checkbox_repertoire = wx.CheckBox(self, -1, _("Enregistrer une copie unique dans le répertoire :"))
    ...
    def __set_properties(self):
        self.ctrl_modele.SetToolTip(wx.ToolTip(_("Sélectionnez un modèle de documents")))
        ...
        self.checkbox_repertoire.SetToolTip(wx.ToolTip(_("Cochez cette case pour enregistrer un exemplaire de chaque attestation de rappel au format PDF dans le répertoire indiqué")))
        self.bouton_repertoire.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner un répertoire de destination")))
        self.ctrl_montant.SetToolTip(wx.ToolTip(_("Sélectionnez le montant à afficher. Réglé par défaut.")))
...
class Panel(wx.Panel):
    ...
    def __set_properties(self):
        ...
        self.bouton_email.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à l'envoi des attestations fiscales par Email")))
        self.bouton_imprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour imprimer les attestations fiscales générées")))
...
##        self.Layout()
    ...
    def MAJ(self):
        ...
        dlgAttente = wx.BusyInfo(_("Recherche des données..."), None)
    ...
    def Sauvegarder(self):
        ...
        dlgAttente = wx.BusyInfo(_("Sauvegarde des attestations en cours..."), None)
        if 'phoenix' not in wx.PlatformInfo:
            wx.Yield()
            ...
            for IDcompte_payeur, dictCompte in self.donnees.items() :
                if dictCompte["select"] == True :
                    ...
                    UTILS_Historique.InsertActions([{
                            "IDfamille" : IDfamille,
                            "IDcategorie" : 27, 
                            "action" : _("Edition d'une attestation de présence pour la période du %s au %s pour un total de %.02f € et un solde de %.02f €") % (DateEngFr(str(self.date_debut)), DateEngFr(str(self.date_fin)), total, solde),
                            },])
    ...
    def Apercu(self, event=None): 
        ...
        if len(tracks) == 0 : 
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune attestation à imprimer !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
