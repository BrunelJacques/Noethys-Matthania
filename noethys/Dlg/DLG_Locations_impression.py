...
class Dialog(wx.Dialog):
    def __init__(self, parent, filtres=[]):
        ...
        # Bandeau
        intro = _("Cochez les locations à imprimer puis cliquez sur le bouton 'Aperçu' pour visualiser le ou les documents dans votre lecteur PDF.")
        titre = _("Impression de locations")
        self.SetTitle(titre)
    ...
    def OnBoutonApercu(self, event): 
        ...
        if len(tracks) == 0 : 
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune location à imprimer !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        x.Impression(listeLocations=listeIDlocation, afficherDoc=True, dictOptions=dictOptions, repertoire=dictOptions["repertoire"])
