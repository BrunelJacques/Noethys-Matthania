...
class Options(wx.Panel):
    def __init__(self, parent):
        ...
        self.label_date = wx.StaticText(self, -1, _("Date de situation :"))
        ...
        self.radio_toutes = wx.RadioButton(self, -1, _("Toutes les familles"), style=wx.RB_GROUP)
        self.radio_avec = wx.RadioButton(self, -1, _("Uniquement familles avec QF"))
        self.radio_sans = wx.RadioButton(self, -1, _("Uniquement familles sans QF"))
        
        self.radio_inscrits = wx.RadioButton(self, -1, _("Tous les inscrits"), style=wx.RB_GROUP)
        self.radio_presents = wx.RadioButton(self, -1, _("Uniquement les présents"))
    ...
    def __set_properties(self):
        self.ctrl_date.SetToolTip(wx.ToolTip(_("Saisissez la date de situation")))
        self.bouton_date.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner la date de situation dans un calendrier")))
        self.ctrl_date_debut.SetToolTip(wx.ToolTip(_("Saisissez ici une date de début")))
        self.bouton_date_debut.SetToolTip(wx.ToolTip(_("Cliquez ici pour saisir une date de début")))
        self.ctrl_date_fin.SetToolTip(wx.ToolTip(_("Saisissez ici une date de fin")))
        self.bouton_date_fin.SetToolTip(wx.ToolTip(_("Cliquez ici pour saisir une date de fin")))
    ...
    def GetPresents(self):
        if self.radio_inscrits.GetValue() == True :
            ...
            if self.ctrl_date_debut.FonctionValiderDate() == False or date_debut == None :
                dlg = wx.MessageDialog(self, _("La date de début ne semble pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
            ...
            if self.ctrl_date_fin.FonctionValiderDate() == False or date_fin == None :
                dlg = wx.MessageDialog(self, _("La date de fin ne semble pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
...
class Parametres(wx.Panel):
    def __init__(self, parent):
        ...
        # Type de quotient
        self.staticbox_type_quotient_staticbox = wx.StaticBox(self, -1, _("Type de quotient"))
    ...
    def __set_properties(self):
        ...
        self.ctrl_type_quotient.SetToolTip(wx.ToolTip(_("Sélectionnez un type de quotient dans la liste")))
    ...
    def OnBoutonAfficher(self, event):
        ...
        if IDtype_quotient == None :
            dlg = wx.MessageDialog(self, _("Vous devez sélectionner obligatoirement un type de quotient dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.ctrl_options.ctrl_date.FonctionValiderDate() == False or date_reference == None :
            dlg = wx.MessageDialog(self, _("La date de situation ne semble pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        titre = _("Liste des quotients familiaux/revenus")
        self.SetTitle(titre)
    ...
    def __set_properties(self):
        self.bouton_ouvrir_fiche.SetToolTip(wx.ToolTip(_("Cliquez ici pour ouvrir la fiche de la famille sélectionnée dans la liste")))
        self.bouton_apercu.SetToolTip(wx.ToolTip(_("Cliquez ici pour créer un aperçu de la liste")))
    ...
    def GetLabelParametres(self):
        ...
        listeParametres.append(_("Type de quotient : %s") % nom_type_quotient)

        # Date
        date = self.ctrl_parametres.ctrl_options.ctrl_date.GetDate()
        if date != None :
            listeParametres.append(_("Situation au %s") % UTILS_Dates.DateEngFr(str(date)))
        ...
        if option == "AVEC" : listeParametres.append(_("Uniquement les familles avec QF"))
        if option == "SANS" : listeParametres.append(_("Uniquement les familles sans QF"))

        # Présents
        presents = self.ctrl_parametres.ctrl_options.GetPresents()
        if presents == None :
            listeParametres.append(_("Toutes les familles dont un des membres est inscrit"))
        else :
            listeParametres.append(_("Uniquement les familles dont un des membres est présent du %s au %s") % (UTILS_Dates.DateEngFr(str(presents[0])), UTILS_Dates.DateEngFr(str(presents[1]))))
