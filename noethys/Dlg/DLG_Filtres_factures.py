    ...
    for filtre in filtres :
        ...
        # Numéros Intervalle
         if filtre["type"] == "numero_intervalle" :
            listeTextes.append(_("Numéros de factures de %s à %s") % (filtre["numero_min"], filtre["numero_max"]))
        ...
        # Solde actuel
         if filtre["type"] == "solde_actuel" :
            ...
            if operateur == "<" : operateur = "&#60;"
            if operateur == ">" : operateur = "&#62;"
            listeTextes.append(_("Solde actuel %s %.2f %s") % (operateur, filtre["montant"], SYMBOLE))

        # Prélèvement
         if filtre["type"] == "prelevement" :
            if filtre["choix"] == True :
                listeTextes.append(_("Familles demandant un prélèvement automatique"))
            else :
                listeTextes.append(_("Familles ne demandant pas un prélèvement automatique"))

        # Email
         if filtre["type"] == "email" :
            if filtre["choix"] == True :
                listeTextes.append(_("Factures nécessitant un envoi par Email"))
            else :
                listeTextes.append(_("Factures ne nécessitant pas un envoi par Email"))
...
class CTRL_Filtres(wx.Panel):
    def __init__(self, parent, filtres=[], ctrl_factures=None):
        ...
        self.bouton_parametres.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier les filtres de sélection des factures")))
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        self.ctrl_numeros_intervalle_max = wx.SpinCtrl(self, -1, "", min=0, max=9999999)
        self.ctrl_numeros_intervalle_max.SetMinSize((70, -1))

        self.check_numeros_liste = wx.CheckBox(self, -1, _("Numéros de factures suivants :"))
        ...
        self.check_prelevement = wx.CheckBox(self, -1, _("Prélèvement automatique demandé"))
        self.ctrl_prelevement = wx.Choice(self, -1, choices=["Oui", _("Non")])
    ...
    def __set_properties(self):
        ...
        self.check_prelevement.SetToolTip(wx.ToolTip(_("Filtre Prélèvement automatique")))
        self.ctrl_prelevement.SetToolTip(wx.ToolTip(_("Oui/Non")))
