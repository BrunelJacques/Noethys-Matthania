...
class Dialog(wx.Dialog):
    ...
    def __set_properties(self):
        self.SetTitle(_("Sélection des attestations à éditer"))
        self.bouton_aide.SetToolTip(_("Cliquez ici pour obtenir de l'aide"))
        self.bouton_liste.SetToolTip(_("Cliquez ici pour imprimer la liste affichée"))
        self.bouton_ok.SetToolTip(_("Cliquez ici pour créer l'aperçu des attestations (PDF)"))
    ...
    def OnBoutonOk(self, event):
        ...
        if len(dictComptes) == 0 : 
            dlg = wx.MessageDialog(self, _("Il n'y a aucune attestation à créer !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if repertoire not in (None, "") :
            ...
            try :
                ...
                for IDcompte_payeur, dictCompte in dictComptes.items() :
                    ...
                    self.EcritStatusbar(_("Génération de l'attestation %d/%d : %s") % (index, len(dictComptes), nomFichier))
                ...
                dlg = wx.MessageDialog(self, _("Désolé, le problème suivant a été rencontré dans la génération des attestations : \n\n%s") % err, _("Erreur"), wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                return False
