...
# Module : Matthania possibilité de nul et négatif, désactivation frais gestion
...
class CTRL_Payeurs(wx.ListBox):
    ...
    def Modifier(self):
        IDpayeur = self.GetID()
        if IDpayeur == None :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun payeur à modifier dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self):
        IDpayeur = self.GetID()
        if IDpayeur == None :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun payeur à supprimer dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
##        if self.Selection()[0].nbreTitulaires > 0 :
##            dlg = wx.MessageDialog(self, _("Il est impossible de supprimer une catégorie déjà assignée à un ou plusieurs individus !"), _("Suppression impossible"), wx.OK | wx.ICON_INFORMATION)
##            dlg.ShowModal()
##            dlg.Destroy()
##            return
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDcompte_payeur=None, IDreglement=None):
        ...
        # Bandeau
        if self.IDreglement == None:
            regl = _("Ajout d'un règlement")
        else:
            regl =  _("Saisie du règlement ID : %s")%str(self.IDreglement)
...
##        self.panel_infos.SetBackgroundColour("#F0FBED")
    ...
    def __set_properties(self):
        ...
        self.ctrl_mode.SetToolTip(_("Sélectionnez le mode de règlement"))
        self.ctrl_emetteur.SetToolTip(_("Sélectionnez l'émetteur du règlement (banque ou organisme)"))
        self.ctrl_numero.SetToolTip(_("Saisissez le numéro de la pièce"))
        self.ctrl_montant.SetToolTip(_("Saisissez un montant en %s") % SYMBOLE)
        self.bouton_calendrier.SetToolTip(_("Cliquez ici pour sélectionner la date dans un calendrier"))
        self.bouton_mode.SetToolTip(_("Cliquez ici pour ajouter, modifier ou supprimer un mode de règlement"))
        self.bouton_emetteur.SetToolTip(_("Cliquez ici pour ajouter, modifier ou supprimer un émetteur de règlement"))
        self.ctrl_attente.SetToolTip(_("Cochez cette case si vous souhaitez laisser l'encaissement en attente"))
        self.ctrl_compte.SetToolTip(_("Sélectionner le compte bancaire sur lequel encaisser ce règlement"))
        self.ctrl_observations.SetToolTip(_("Saisissez ici les observations de votre choix"))
        self.ctrl_quittancier.SetToolTip(_("Saisissez ici le numéro de quittancier si vous en utilisez un"))
        self.ctrl_recu.SetToolTip(_("Cochez cette case si vous souhaitez éditer un reçu pour ce règlement juste après la validation de celui-ci"))
        self.bouton_ajouter_payeur.SetToolTip(_("Cliquez ici pour ajouter un payeur"))
        self.bouton_modifier_payeur.SetToolTip(_("Cliquez ici pour modifier le payeur sélectionné"))
        self.bouton_supprimer_payeur.SetToolTip(_("Cliquez ici pour supprimer le payeur sélectionné"))
        self.ctrl_check_differe.SetToolTip(_("Cochez cette case pour préciser une date d'encaissement ultérieure"))
        self.bouton_calendrier_differe.SetToolTip(_("Cliquez ici pour sélectionner une date d'encaissement différé dans un calendrier"))
        self.bouton_options.SetToolTip(_("Cliquez ici pour accéder aux options"))
        self.bouton_aide.SetToolTip(_("Cliquez ici pour obtenir de l'aide"))
        self.bouton_calculatrice.SetToolTip(_("Cliquez ici pour ouvrir la calculatrice installée par défaut sur votre ordinateur"))
        self.bouton_ok.SetToolTip(_("Cliquez ici pour valider"))
        self.bouton_annuler.SetToolTip(_("Cliquez ici pour annuler"))
        self.ctrl_differe.SetToolTip(_("Saisissez ici la date d'encaissement souhaitée"))
...
##        self.panel_infos.SetSizer(grid_sizer_infos)
    ...
    def MAJinfos(self):
        ...
        # IDrèglement
        if self.IDreglement != None :
            texte += _("Règlement n°%d<BR>") % self.IDreglement
        else:
            DB = GestionDB.DB()
            prochainID = DB.GetProchainID("reglements")
            DB.Close()
            if prochainID != None :
                texte += _("Règlement n°%d<BR>") % prochainID
        # Date saisie
        ...
        if self.IDdepot != None :
            texte += _("Dépôt n°%d<BR>") % self.IDdepot
        else :
            texte += _("Non déposé<BR>")
        # Dernier reçu
        if self.dernierRecu == None :
            texte += _("Aucun reçu édité<BR>")
        else :
            ...
            if self.IDutilisateur != None :
                ...
                nomUtilisateur = _("Utilisateur inconnu")
            texte += _("Reçu n°%s édité le %s par %s<BR>") % (numero, date_edition, nomUtilisateur)
        # Avis de dépôt
        if self.avis_depot != None :
            texte += _("Avis de dépôt envoyé par Email le %s<BR>") % DateEngFr(str(self.avis_depot))
...
##        self.ctrl_ventilation.Show(montant >= 0.0)
    ...
    def OnBoutonOptions(self, event):
        ...
        item = wx.MenuItem(menuPop, ID_OPTION_BLOQUER_VENTILATION, _("Bloquer la ventilation lorsque le crédit est épuisé"), _("Bloquer la ventilation lorsque le crédit est épuisé"), wx.ITEM_CHECK)
        menuPop.Append(item)
    ...
    def OnBoutonOk(self, event): 
        ...
        if email_recus != None and self.nouveauReglement == True :
            ...
            dlg = wx.MessageDialog(None, _("Cette famille est abonnée au service d'envoi automatique des reçus de règlements par Email. \n\nConfirmez-vous l'envoi direct du reçu maintenant ?\n\nOUI = Envoi direct \nNON = Accéder aux paramètres d'envoi \nANNULER = Ne pas envoyer"), _("Envoi du reçu par Email"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
            ...
            if reponse != wx.ID_CANCEL :
                ...
                if adresse == None :
                    dlg = wx.MessageDialog(self, _("L'adresse Email enregistrée ne semble pas valide. Renseignez une nouvelle adresse valide pour cette famille..."), _("Erreur d'adresse Email"), wx.OK | wx.ICON_ERROR)
                    dlg.ShowModal()
                    dlg.Destroy()
                    return False
    ...
    def Sauvegarde(self):
        ...
        IDmode = self.ctrl_mode.GetID()
        if IDmode == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner un mode de règlement !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.ctrl_emetteur.IsEnabled() and IDemetteur == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner un émetteur !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.ctrl_numero.IsEnabled() :
            if numero_piece == "" :
                dlg = wx.MessageDialog(self, _("Etes-vous sûr de ne pas saisir de numéro de pièce ?"), _("Confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
                reponse = dlg.ShowModal()
                dlg.Destroy()
                if reponse !=  wx.ID_YES :
                    return False
        ...
        if IDpayeur == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner un payeur dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        IDcompte = self.ctrl_compte.GetID()
        if IDcompte == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner un compte bancaire dans la liste ! S'il n'y en aucun, vous devez en paramétrer un à partir du menu Paramétrage de la fenêtre d'accueil..."), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.ctrl_check_differe.GetValue() == True :
            ...
            if date_differe == None :
                dlg = wx.MessageDialog(self, _("La date d'encaissement différé n'est pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_differe.SetFocus()
                return False
        ...
        if donneesFrais[0] != None and donneesFrais[0] != 0.0 :
            dlg = wx.MessageDialog(self, _("Confirmez-vous pour ce règlement la facturation de frais de gestion d'un montant de %.2f %s ?") % (donneesFrais[0], SYMBOLE), _("Avertissement"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
            reponse = dlg.ShowModal() 
            dlg.Destroy()
            if reponse != wx.ID_YES :
                DB.Close()
        ...
        UTILS_Historique.InsertActions([{
            ...
            "action" : _("%s du règlement ID%d : %s en %s %spayé par %s") % (categorie, self.IDreglement, montant, texteMode, texteDetail, textePayeur),
            },])
