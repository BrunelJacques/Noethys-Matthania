...
class Panel(wx.Panel):
    def __init__(self, parent):
        ...
        # Factures à générer
        self.box_factures_staticbox = wx.StaticBox(self, -1, _("Factures à générer"))
        ...
        self.bouton_option_montant = wx.Button(self, -1, _("Cocher uniquement les factures dont le montant 'Dû période' est supérieur ou égal à"))
    ...
    def __set_properties(self):
        self.bouton_apercu.SetToolTip(wx.ToolTip(_("Cliquez ici pour créer un aperçu PDF d'une ou plusieurs factures")))
        self.bouton_option_montant.SetToolTip(wx.ToolTip(_("Cliquez ici pour cocher uniquement les factures dont le montant 'Dû Période' est supérieur ou égal au montant souhaité")))
    ...
    def Validation(self):
        ...
        if nbreCoches == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune facture à générer !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if nbreCoches < nbreTotalFactures :
            texteSupp = _("(Sur un total de %d factures proposées) ") % nbreTotalFactures
        else :
            texteSupp = ""
        if nbreCoches == 1 :
            texte = _("Confirmez-vous la génération de 1 facture %s?") % texteSupp
        else :
            texte = _("Confirmez-vous la génération de %d factures %s?") % (nbreCoches, texteSupp)
    ...
    def OnBoutonApercu(self, event):
        ...
        item = wx.MenuItem(menuPop, 40, _("Aperçu avant impression de la liste"))
    ...
    def SauvegardeFactures(self):
        ...
        # ProgressBar
        dlgProgress = wx.ProgressDialog(_("Génération des factures"), _("Initialisation..."), maximum=len(listeComptes), parent=None, style=wx.PD_SMOOTH | wx.PD_AUTO_HIDE | wx.PD_APP_MODAL)
        ...
        try :
            ...
            for nomTitulaires, IDcompte_payeur in listeComptes :
                ...
                texte = _("Génération de la facture %d sur %d...") % (index+1, len(listeComptes))
