    ...
    def GetListeDonnees(self):
        ...
        for IDindividu, IDcategorie, nom, prenom, mailPerso, mailTravail in listeDonnees :
            if mailPerso != None and mailPerso != "" :
                listeAdresses.append((_("%s (Adresse perso de %s)") % (mailPerso, prenom), mailPerso, IDindividu, "perso"))
            if mailTravail != None and mailTravail != "" :
                listeAdresses.append((_("%s (Adresse pro de %s)") % (mailTravail, prenom), mailTravail, IDindividu, "travail"))
        ...
        for IDindividu, IDcategorie, nom, prenom, mailPerso, mailTravail in listeDonnees:
            if mailPerso != None and mailPerso != "":
                listeAdresses.append(
                    (_("%s (Adresse perso de %s)") % (mailPerso, prenom), mailPerso, IDindividu, "perso"))
            if mailTravail != None and mailTravail != "":
                listeAdresses.append(
                    (_("%s (Adresse pro de %s)") % (mailTravail, prenom), mailTravail, IDindividu, "travail"))
...
class Panel_adresse_unique(wx.Panel):
    def __init__(self, parent, IDfamille=None):
        ...
        self.radio_membre = wx.RadioButton(self, -1, _("L'adresse du membre :"), style=wx.RB_GROUP)
        self.ctrl_membre = CTRL_Membre(self, IDfamille=IDfamille)
        self.ctrl_membre.SetMinSize((380, 100))
        self.radio_autre = wx.RadioButton(self, -1, _("L'adresse suivante :"))
        self.ctrl_autre = CTRL_Saisie_mail.Mail(self)

        # Properties
        self.radio_membre.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner l'adresse Email d'un membre de la famille")))
        self.ctrl_membre.SetToolTip(wx.ToolTip(_("Sélectionnez ici l'adresse Email d'un membre de la famille")))
        self.radio_autre.SetToolTip(wx.ToolTip(_("Cliquez ici pour saisir manuellement une adresse Email")))
        self.ctrl_autre.SetToolTip(wx.ToolTip(_("Saisissez une adresse Email")))
    ...
    def GetValeur(self):
        ...
        if self.radio_membre.GetValue() == True:
            ...
            if IDindividu == None:
                dlg = wx.MessageDialog(self, _("Activation impossible !\n\nVous devez sélectionner un membre."), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
            ...
            if ";" in adresse:
                dlg = wx.MessageDialog(self, _("Vous ne pouvez pas utiliser de point-virgule (;) dans l'adresse !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_autre.SetFocus()
                return
            ...
            if adresse == "":
                dlg = wx.MessageDialog(self, _("Activation impossible !\n\nVous devez saisir une adresse Email."), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_autre.SetFocus()
                return
            ...
            if valide == False:
                dlg = wx.MessageDialog(self, _("Activation impossible !\n\nL'adresse email saisie n'est pas valide."), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_autre.SetFocus()
                return False

        valeur = "%s;%s;%s" % (str(IDindividu), categorie, adresse)
        return valeur
...
class Panel_adresses_multiples(wx.Panel):
    def __init__(self, parent, IDfamille=None):
        ...
        self.label_membres = wx.StaticText(self, -1, _("Cochez une ou plusieurs adresses :"))
        self.ctrl_membres = CTRL_Membres(self, IDfamille=IDfamille)
        self.ctrl_membres.SetMinSize((380, 100))
        self.label_autres = wx.StaticText(self, -1, _("Et/ou saisissez une ou plusieurs autres adresses :"))
        self.ctrl_autres = wx.TextCtrl(self, -1, "")

        # Properties
        self.ctrl_membres.SetToolTip(wx.ToolTip(_("Cochez une ou plusieurs adresses de destination")))
        self.ctrl_autres.SetToolTip(wx.ToolTip(_("Saisissez directement d'autres adresses en les séparant d'un point-virgule (;)")))
    ...
    def GetValeur(self):
        ...
        for dict_membre in liste_membres :
            valeur = "%s;%s;" % (dict_membre["IDindividu"], dict_membre["categorie"])
        ...
        if len(autres_adresses) > 0 :
            ...
            for adresse in liste_autres :
                ...
                if re.match("^.+\\@(\\[?)[a-zA-Z0-9\\-\\.]+\\.([a-zA-Z]{2,3}|[0-9]{1,3})(\\]?)$", adresse) == None:
                    dlg = wx.MessageDialog(self, _("L'adresse Email saisie '%s' n'est pas valide !") % adresse, _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                    dlg.ShowModal()
                    dlg.Destroy()
        ...
        if len(liste_valeurs) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez cocher ou saisir au moins une adresse Email !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDfamille=None, champ="email_factures", intro=u"", titre=u"", adresses_multiples=False):
        ...
        self.label_activation = wx.StaticText(self, -1, _("Envoi par Email activé :"))
        ...
        if adresses_multiples == True :
            self.box_adresse_staticbox = wx.StaticBox(self, -1, _("Adresses Email"))
            self.panel_adresse = Panel_adresses_multiples(self, IDfamille)
        else :
            self.box_adresse_staticbox = wx.StaticBox(self, -1, _("Adresse Email"))
    ...
    def __set_properties(self):
        self.radio_activation_oui.SetToolTip(wx.ToolTip(_("Cliquez ici pour activer l'envoi par Email")))
        self.radio_activation_non.SetToolTip(wx.ToolTip(_("Cliquez ici pour désactiver l'envoi par Email")))
    ...
    def OnBoutonOk(self, event):
        ...
        if self.radio_activation_oui.GetValue() == True :
            ...
            # Pas d'activation
            valeur = None
            dlg = wx.MessageDialog(None, _("Vous confirmez que vous ne souhaitez pas activer cette fonctionnalité ?"), _("Confirmation"), wx.YES_NO | wx.YES_DEFAULT | wx.CANCEL | wx.ICON_QUESTION)
