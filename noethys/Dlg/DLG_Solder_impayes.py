...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        # Bandeau
        titre = _("Solder les impayés")
        intro = _("Cette fonctionnalité vous permet de laisser Noethys créer des règlements pour les prestations impayées d'une période donnée. Utile pour remettre les comptes à zéro par exemple.")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Impayes.png")

        # Règlement
        self.box_reglement_staticbox = wx.StaticBox(self, -1, _("Paramètres des règlements"))
    ...
    def __set_properties(self):
        self.ctrl_compte.SetToolTip(wx.ToolTip(_("Sélectionnez un compte bancaire à créditer")))
        self.bouton_compte.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à la gestion des comptes bancaires")))
        self.ctrl_mode.SetToolTip(wx.ToolTip(_("Sélectionnez un mode de règlement")))
        self.bouton_mode.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à la gestion des modes de règlements")))
        self.ctrl_emetteur.SetToolTip(wx.ToolTip(_("Sélectionnez un émetteur de règlement")))
        self.bouton_emetteur.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à la gestion des modes de règlements")))
    ...
    def __do_layout(self):
        ...
        grid_sizer_reglement.Add(self.ctrl_compte, 0, wx.EXPAND, 0)
        ...
        grid_sizer_reglement.Add(self.ctrl_mode, 0, wx.EXPAND, 0)
        ...
        grid_sizer_reglement.Add(self.ctrl_emetteur, 0, wx.EXPAND, 0)
    ...
    def OnBoutonOk(self, event):
        ...
        IDmode = self.ctrl_mode.GetID() 
        if IDmode == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner un mode de règlement !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        dlg = wx.MessageDialog(self, _("Confirmez-vous la création automatique de %d règlements pour un total de %.2f %s ?") % (len(tracks), montantTotal, SYMBOLE), _("Demande de confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        ...
        for track in tracks :
            ...
            if track.IDcompte_payeur in dictPayeurs :
                ...
                nomTitulaire = "%s %s" % (track.listeTitulaires[0]["nom"], track.listeTitulaires[0]["prenom"])
            ...
            listeDonnees = [
                ...
                ("IDpayeur", IDpayeur),
                ("observations", _("Règlement créé avec la fonction 'Solder les impayés'")),
        ...
        dlg = wx.MessageDialog(self, _("Les %d règlements ont été créés avec succès.") % len(tracks), _("Confirmation"), wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
