...
class CTRL_Parametres(wx.Notebook):
    def __init__(self, parent, IDproduit=None):
        ...
        self.listePages = [
            {"code": "questionnaire", "ctrl": Page_Questionnaire(self, self.IDproduit), "label": _("Questionnaire"), "image": "Questionnaire.png"},
            {"code": "stock", "ctrl": Page_Stock(self, self.IDproduit), "label": _("Stock"), "image": "Stock.png"},
            {"code": "tarification", "ctrl": Page_Tarification(self, self.IDproduit), "label": _("Tarification"), "image": "Euro.png"},
        ]
...
class Page_Stock(wx.Panel):
    def __init__(self, parent, IDproduit=None):
        ...
        self.label_quantite = wx.StaticText(self, -1, _("Quantité fixe :"))
        ...
        self.label_partage = wx.StaticText(self, -1, _("Autoriser partage :"))
        self.ctrl_partage = wx.CheckBox(self, -1)
        self.ctrl_partage.SetToolTip(wx.ToolTip(_("Autoriser le partage de la ressource entre plusieurs usagers")))
...
class Page_Tarification(wx.Panel):
    def __init__(self, parent, IDproduit=None):
        ...
        # Aucune
        self.radio_tarification_aucune = wx.RadioButton(self, -1, _("Aucune"), style=wx.RB_GROUP)

        # Tarification simple
        self.radio_tarification_montant = wx.RadioButton(self, -1, _("Tarification simple :"))
        self.label_montant = wx.StaticText(self, -1, _("Montant fixe :"))
        self.ctrl_montant = CTRL_Saisie_euros.CTRL(self)

        # Tarification avancée
        self.radio_tarification_tarif = wx.RadioButton(self, -1, _("Tarification avancée :"))
        ...
        # Properties
        self.radio_tarification_aucune.SetToolTip(wx.ToolTip(_("Aucune tarification spécifique pour ce produit")))
        self.radio_tarification_montant.SetToolTip(wx.ToolTip(_("Un montant fixe est associé à ce produit")))
        self.radio_tarification_tarif.SetToolTip(wx.ToolTip(_("Un ou plusieurs tarifs avancés sont associés à ce produit")))
        self.ctrl_montant.SetToolTip(wx.ToolTip(_("Saisissez un montant fixe pour ce produit")))
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter un tarif")))
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier le tarif sélectionné dans la liste")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer le tarif sélectionné dans la liste")))
    ...
    def Validation(self):
        ...
        if self.radio_tarification_tarif.GetValue() == True :
            ...
            if len(liste_tarifs) == 0 :
                dlg = wx.MessageDialog(self, _("Vous devez saisir au moins un tarif !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDproduit=None):
        ...
        if self.IDproduit == None :
            self.SetTitle(_("Saisie d'un produit"))
        else :
            self.SetTitle(_("Modification d'un produit"))
    ...
    def __set_properties(self):
        self.ctrl_nom.SetToolTip(wx.ToolTip(_("Saisissez ici le nom du produit")))
        self.ctrl_categorie.SetToolTip(wx.ToolTip(_("Sélectionnez la catégorie associée au produit")))
        self.bouton_categories.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à la gestion des catégories de produits")))
        ...
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter ou modifier l'image")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer l'image actuelle")))
    ...
    def OnBoutonOk(self, event):
        ...
        if len(nom) == 0:
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un nom pour ce produit !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_nom.SetFocus()
            return
        ...
        if IDcategorie == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner une catégorie pour ce produit !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
