...
class Page_SMTP(wx.Panel):
    def __init__(self, parent):
        ...
        # Serveur
        self.static_sizer_serveur_staticbox = wx.StaticBox(self, -1, _("Serveur de messagerie"))
        self.radio_predefini = wx.RadioButton(self, -1, "")
        self.label_predefini = wx.StaticText(self, -1, _("Serveur prédéfini :"))
        self.ctrl_predefinis = wx.Choice(self, -1, choices=listeServeursChoices)
        self.radio_personnalise = wx.RadioButton(self, -1, "")
        self.label_personnalise = wx.StaticText(self, -1, _("Serveur personnalisé :"))
        self.label_smtp = wx.StaticText(self, -1, _("Serveur SMTP :"))
        self.ctrl_smtp = wx.TextCtrl(self, -1, "")
        self.label_port = wx.StaticText(self, -1, _("Numéro de port :"))
        self.ctrl_port = wx.TextCtrl(self, -1, "")
        self.label_authentification = wx.StaticText(self, -1, _("Authentification :"))
        self.ctrl_authentification = wx.CheckBox(self, -1, "")
        self.label_startTLS = wx.StaticText(self, -1, _("TLS :"))
        self.ctrl_startTLS = wx.CheckBox(self, -1, "")

        # Adresse
        self.static_sizer_adresse_staticbox = wx.StaticBox(self, -1, _("Adresse de messagerie"))
        self.label_adresse = wx.StaticText(self, -1, _("Adresse d'envoi :"))
        self.ctrl_adresse = wx.TextCtrl(self, -1, "")
        self.label_nom_adresse = wx.StaticText(self, -1, _("Nom affiché :"))
        ...
        # Options
        self.static_sizer_options_staticbox = wx.StaticBox(self, -1, _("Options"))
        self.ctrl_notification = wx.CheckBox(self, -1, _("Accusé de réception"))
        ...
        # Properties
        self.radio_predefini.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner un serveur prédéfini dans la liste")))
        self.ctrl_predefinis.SetMinSize((350, -1))
        self.radio_personnalise.SetToolTip(wx.ToolTip(_("Cliquez ici pour saisir manuellement les caractéristiques du serveur de messagerie")))
        self.ctrl_smtp.SetToolTip(wx.ToolTip(_("Saisissez ici le nom du serveur SMPT (exemple : smtp.orange.fr)")))
        self.ctrl_port.SetMinSize((60, -1))
        self.ctrl_port.SetToolTip(wx.ToolTip(_("Saisissez ici le numéro de port (laissez la case vide pour utiliser le numéro de port par défaut)")))
        self.ctrl_authentification.SetToolTip(wx.ToolTip(_("Cliquez ici si le serveur de messagerie nécessite une authentification")))
        self.ctrl_adresse.SetToolTip(wx.ToolTip(_("Saisissez ici votre adresse mail d'envoi")))
        self.ctrl_nom_adresse.SetToolTip(wx.ToolTip(_("Saisissez ici le nom qui sera affiché pour l'adresse mail d'envoi")))
        self.ctrl_utilisateur.SetToolTip(wx.ToolTip(_("Saisissez ici votre nom d'utilisateur (il s'agit souvent de l'adresse d'envoi)")))
        self.ctrl_mdp.SetToolTip(wx.ToolTip(_("Saisissez ici le mot de passe s'il s'agit d'une connexion authentifiée")))
        self.ctrl_startTLS.SetToolTip(wx.ToolTip(_("Cochez cette case si votre messagerie utilise le protocole TLS")))
        self.ctrl_notification.SetToolTip(wx.ToolTip(_("Cochez cette case pour demander à recevoir un accusé de réception (Attention : Ne fonctionne qu'avec certaines messageries)")))
    ...
    def Validation(self):
        if self.radio_predefini.GetValue() == True:
            ...
            if self.ctrl_predefinis.GetSelection() == -1:
                dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucun serveur de messagerie dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                return False
            ...
            if self.ctrl_adresse.GetValue() == "":
                dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une adresse de messagerie !"), _("Erreur de saisie"), wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                return False
            ...
            if self.listeServeurs[self.ctrl_predefinis.GetSelection()][3] == True:
                if self.ctrl_mdp.GetValue() == "":
                    dlg = wx.MessageDialog(self, _("Vous avez omis de saisir le mot de passe de votre messagerie !"), _("Erreur de saisie"), wx.OK | wx.ICON_ERROR)
                    dlg.ShowModal()
                    dlg.Destroy()
                    return False

                if self.ctrl_utilisateur.GetValue() == "":
                    dlg = wx.MessageDialog(self, _("Vous avez omis de saisir le nom d'utilisateur de votre messagerie !"), _("Erreur de saisie"), wx.OK | wx.ICON_ERROR)
                    dlg.ShowModal()
                    dlg.Destroy()
                    return False
            ...
            if self.ctrl_smtp.GetValue() == "":
                dlg = wx.MessageDialog(self, _("Vous avez omis de saisir le nom du serveur SMTP !"), _("Erreur de saisie"), wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                return False
            ...
            if self.ctrl_port.GetValue() != "":
                try:
                    ...
                    dlg = wx.MessageDialog(self, _("Le numéro de port que vous avez saisi n'est pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_ERROR)
                    dlg.ShowModal()
                    dlg.Destroy()
                    return False
...
class Page_MAILJET(wx.Panel):
    def __init__(self, parent):
        ...
        self.static_sizer_adresse_staticbox = wx.StaticBox(self, -1, _("Paramètres Mailjet"))
        self.label_adresse = wx.StaticText(self, -1, _("Adresse d'envoi :"))
        self.ctrl_adresse = wx.TextCtrl(self, -1, "")
        self.label_nom_adresse = wx.StaticText(self, -1, _("Nom affiché :"))
        self.ctrl_nom_adresse = wx.TextCtrl(self, -1, "")
        self.label_api_key = wx.StaticText(self, -1, _("Clé API :"))
        self.ctrl_api_key = wx.TextCtrl(self, -1, "")
        self.label_api_secret = wx.StaticText(self, -1, _("Clé secrète :"))
        self.ctrl_api_secret = wx.TextCtrl(self, -1, "")

        self.static_sizer_infos_staticbox = wx.StaticBox(self, -1, _("Informations"))
        ...
        # Properties
        self.ctrl_adresse.SetToolTip(wx.ToolTip(_("Saisissez ici votre adresse mail d'envoi")))
        self.ctrl_nom_adresse.SetToolTip(wx.ToolTip(_("Saisissez ici le nom qui sera affiché dans le client de messagerie de votre destinataire")))
        self.ctrl_api_key.SetToolTip(wx.ToolTip(_("Saisissez ici votre clé API Mailjet")))
        self.ctrl_api_secret.SetToolTip(wx.ToolTip(_("Saisissez ici votre clé secrète Mailjet")))
    ...
    def Validation(self):
        ...
        if adresse == "":
            dlg = wx.MessageDialog(self, _("Vous devez saisir l'adresse d'expédition !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False

        if nom_adresse == "":
            dlg = wx.MessageDialog(self, _("Vous devez saisir le nom à afficher !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False

        if api_key == "":
            dlg = wx.MessageDialog(self, _("Vous devez saisir votre clé API Mailjet !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False

        if api_secret == "":
            dlg = wx.MessageDialog(self, _("Vous devez saisir votre clé secrète Mailjet !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDadresse=None):
        wx.Dialog.__init__(self, parent, -1, title=_("Saisie d'une adresse Email"))
        self.IDadresse = IDadresse
        self.defaut = False

        # Intro
        self.label_intro = wx.StaticText(self, -1, _("Sélectionnez un moteur d'envoi (SMTP par défaut) puis renseignez les paramètres obligatoires."))
        ...
        self.bouton_tester = CTRL_Bouton_image.CTRL(self, texte=_("Tester"), cheminImage="Images/32x32/Connexion.png")
    ...
    def __set_properties(self):
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez ici pour accéder à l'aide")))
        self.bouton_tester.SetToolTip(wx.ToolTip(_("Cliquez ici pour réaliser un test d'envoi d'email")))
    ...
    def InitLabelbook(self):
        self.listePages = [
            ("smtp", _("SMTP"), Page_SMTP(self), wx.Bitmap(Chemins.GetStaticPath('Images/32x32/Smtp.png'), wx.BITMAP_TYPE_PNG)),
            ("mailjet", _("Mailjet"), Page_MAILJET(self), wx.Bitmap(Chemins.GetStaticPath('Images/32x32/Mailjet.png'), wx.BITMAP_TYPE_PNG)),
        ]
    ...
    def OnBoutonTester(self, event):
        ...
        dlg = wx.TextEntryDialog(self, _("Saisissez une adresse Email de destination et cliquez sur Ok :"), _("Envoi d'un Email de test"), adresse)
        ...
        if UTILS_Envoi_email.ValidationEmail(adresse) == False :
            dlg = wx.MessageDialog(self, _("L'adresse saisie n'est pas valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        dlg = wx.MessageDialog(self, _("L'email de test a été envoyé avec succès."), _("Test de connexion"), wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
