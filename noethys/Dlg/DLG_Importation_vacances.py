...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ColumnDefn(_("Année"), 'left', 50, "annee"),
            ColumnDefn(_("Nom"), "left", 120, "nom"),
            ColumnDefn(_("Date de début"), "left", 190, "date_debut", stringConverter=FormateDate),
            ColumnDefn(_("Date de fin"), "left", 190, "date_fin", stringConverter=FormateDate),
            ]
        
        self.SetColumns(liste_Colonnes)
        self.CreateCheckStateColumn(0)
        
        self.SetEmptyListMsg(_("Aucune période de vacances"))
    ...
    def CocheSuggestions(self):
        ...
        # Texte de label
        if nbre == 0 : texte = _("Noethys n'a aucune suggestion d'importation...")
        elif nbre == 1 : texte = _("Noethys vous suggère d'importer 1 période de vacances...")
        else : texte = _("Noethys vous suggère d'importer %d périodes de vacances...") % nbre
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        intro = _("Vous pouvez ici importer des périodes de vacances depuis le site internet de l'Education Nationale. Sélectionnez votre zone géographique et cochez les périodes à importer.")
        titre = _("Importation de périodes de vacances")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Telecharger.png")
        
        # Zone
        self.box_zone_staticbox = wx.StaticBox(self, -1, _("1. Sélectionnez votre zone"))
        self.label_zone = wx.StaticText(self, -1, _("Zone géographique :"))
        self.ctrl_zone = wx.Choice(self, -1, choices=[u"Zone A", "Zone B", "Zone C"])
        self.hyper_zone = Hyperlien(self, label=_("Rechercher la zone de l'organisateur sur internet"), infobulle=_("Cliquez ici pour rechercher la zone de l'organisateur sur internet"), URL="zone")
        
        # Périodes
        self.box_periodes_staticbox = wx.StaticBox(self, -1, _("2. Cochez les périodes à importer"))
        self.label_periodes = wx.StaticText(self, -1, _("Sélectionnez une zone..."))
        ...
        self.hyper_suggestions = Hyperlien(self, label=_("Sélectionner les suggestions"), infobulle=_("Cliquez ici pour sélectionner uniquement les suggestions"), URL="suggestions")
    ...
    def __set_properties(self):
        self.ctrl_zone.SetToolTip(wx.ToolTip(_("Sélectionnez une zone")))
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez ici pour obtenir de l'aide")))
        self.bouton_ok.SetToolTip(wx.ToolTip(_("Cliquez ici pour importer les périodes sélectionnées")))
    ...
    def OnBoutonOk(self, event): 
        ...
        # Validation
        if len(tracks) == 0 :
            dlg = wx.MessageDialog(self, _("Vous n'avez coché aucune période à importer !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def ImportationZone(self):
        ...
        try :
            ...
            zone = None
        if zone == None :
            dlg = wx.MessageDialog(self, _("Désolé, Noethys n'a pas réussi à trouver la zone scolaire de l'organisateur sur internet !"), _("Erreur"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return
        
        self.SetZone(zone) 
        dlg = wx.MessageDialog(self, _("La ville de l'organisateur (%s) est située dans la zone %s.") % (ville, zone), _("Zone scolaire"), wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
