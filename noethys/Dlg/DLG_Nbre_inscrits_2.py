...
# module : variante en OLV et non en ultimatelistctrl
...
class CTRL(HTL.HyperTreeList):
    ...
    def CreationColonnes(self):
        ...
        liste_colonnes = [
            (_("Activité / Groupe"), 300, wx.ALIGN_LEFT),
            (_("Abrégé"), 80, wx.ALIGN_CENTER),
            (_("Inscrits"), 80, wx.ALIGN_CENTER),
            (_("Places max"), 90, wx.ALIGN_CENTER),
            (_("Places libres"), 90, wx.ALIGN_CENTER),
            (_("Places attente"), 90, wx.ALIGN_CENTER),
            (_("Places refusées"), 90, wx.ALIGN_CENTER),
        ]
        index = 0
    ...
    def OnContextMenu(self, event):
        ...
        # Item Actualiser
        item = wx.MenuItem(menuPop, 10, _("Consulter les tarifs de l'activité"))
    ...
    def OuvrirTarifs(self, event=None):
        ...
        if data == None :
            dlg = wx.MessageDialog(self, _("Vous n'avez sélectionné aucune activité dans la liste !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def MAJ(self, forcerActualisation=False):
        ...
        if parametres != None :
            liste = parametres.split("###")
            if liste != "" :
                strListe = str(liste)[1:-1].replace(";",",")
                condition = "WHERE activites.IDactivite IN (%s)" % strListe
                ...
                if self.filtre == None or (self.filtre.lower() in dictActivite["infos"].lower()) :
                    ...
                    if self.regroupement_groupe_activites == 0 or (self.regroupement_groupe_activites == 1 and IDactivite in dictGroupeParActivite and IDtype_groupe_activite in dictGroupeParActivite[IDactivite]) :
                        ...
                        for dictGroupe in dictActivite["liste_groupes"] :
                            ...
                            nbrMaxi = dictGroupe["nbrMaxi"]
                            if nbrMaxi == None :
                                texte_inscrits_max = ""
                            else :
                                texte_inscrits_max = str(nbrMaxi)
    ...
    def Imprimer(self, event=None):
        ...
        dataTableau.append( (_("Inscriptions"), _("%s\nEdité le %s") % (UTILS_Organisateur.GetNom(), dateDuJour)) )
        style = TableStyle([
...
class Panel(wx.Panel):
    def __init__(self, parent):
        ...
        self.bouton_attente.SetToolTip(wx.ToolTip(_("Cliquez ici pour afficher la liste des inscriptions en attente")))
        self.bouton_refus = wx.BitmapButton(self, -1, wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Interdit.png"), wx.BITMAP_TYPE_PNG))
        self.bouton_refus.SetToolTip(wx.ToolTip(_("Cliquez ici pour afficher la liste des inscriptions refusées")))
        ...
        self.bouton_export.SetToolTip(wx.ToolTip(_("Cliquez ici pour exporter la liste au format Excel")))
        ...
        self.bouton_parametres.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier les paramètres d'affichage")))
        ...
        self.bouton_tarifs.SetToolTip(wx.ToolTip(_("Cliquez ici pour consulter les tarifs de l'activité sélectionnée (ou double-cliquez sur une activité dans la liste)")))
