...
import GestionDB
import datetime
from Ol import OL_Contratspsu_tarifs
...
class Panel(wx.Panel):
    def __init__(self, parent, clsbase=None):
        ...
        self.label_regularisation = wx.StaticText(self, -1, _("Heures de régularisation :"))
        self.ctrl_regularisation = CTRL_Saisie_duree.CTRL(self, size=(50, -1))
        self.label_tolerance = wx.StaticText(self, -1, _("Tolérance dépassement :"))
    ...
    def __set_properties(self):
        self.bouton_tarifs_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter un tarif")))
        self.bouton_tarifs_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier le tarif sélectionné")))
        self.bouton_tarifs_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer le tarif sélectionné")))
        self.bouton_tarifs_apercu.SetToolTip(wx.ToolTip(_("Cliquez ici pour afficher un aperçu avant impression de la liste")))
        self.bouton_tarifs_imprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour imprimer la liste")))
        self.ctrl_regularisation.SetToolTip(wx.ToolTip(_("Saisissez ici le nombre d'heure de régularisation (en positif ou négatif)")))
        self.ctrl_tolerance.SetToolTip(wx.ToolTip(_("Saisissez ici la valeur de tolérance des dépassements. Une facturation ne sera ainsi appliquée que lorsque le dépassement sera supérieur à la valeur renseignée.")))
    ...
    def Validation(self):
        if len(self.ctrl_tarifs.GetTracks()) == 0 :
            dlg = wx.MessageDialog(self, _("Vous devez saisir obligatoirement au moins un tarif !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
