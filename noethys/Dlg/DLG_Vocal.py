...
class MyHtml(html.HtmlWindow):
    ...
    def OnLinkClicked(self, link):
        ...
        <FONT SIZE=-1>
        <CENTER>
            <IMG SRC="%s/32x32/Telecharger.png">
            <BR><BR>
            ...
            <BR>
            <A HREF="http://www4.ac-nancy-metz.fr/tice57/spip.php?article139">Page de téléchargement</A>.
            <BR><BR>
            <I>(Le fichier à télécharger "Voix de Virginie" se trouve en bas de la page).</I>
        </CENTER>
        <BR><BR>
        ...
        dlg = DLG_Message_html.Dialog(self, texte=texte, titre=_("Télécharger la voix française"), size=(360, 500))
        dlg.ShowModal()
        dlg.Destroy()
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        # Bandeau
        intro = _("Vous pouvez paramétrer et tester ici la synthèse vocale utilisée par Noethys. Attention, si ce n'est pas déjà fait, téléchargez dès à présent la voix française ci-dessous !")
        titre = _("Synthèse vocale")
        self.SetTitle(titre)
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Vocal.png")
        
        # Paramètres
        self.box_parametres_staticbox = wx.StaticBox(self, -1, _("Paramètres de la voix"))
        self.label_voix = wx.StaticText(self, -1, _("Voix actuelle :"))
        ...
        # Corrections
        self.box_corrections_staticbox = wx.StaticBox(self, -1, _("Corrections phoniques"))
        ...
        # Test
        self.box_test_staticbox = wx.StaticBox(self, -1, _("Test"))
        self.ctrl_test = wx.TextCtrl(self, -1, "")
    ...
    def __set_properties(self):
        self.ctrl_voix.SetToolTip(wx.ToolTip(_("Sélectionnez une voix dans la liste")))
        self.bouton_ajouter.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter une correction phonique. Exemple : Créez une correction 'Kévin'->'Kévine' pour que le moteur vocal lise correctement le son 'in'.")))
        self.bouton_modifier.SetToolTip(wx.ToolTip(_("Cliquez ici pour modifier la correction sélectionnée")))
        self.bouton_supprimer.SetToolTip(wx.ToolTip(_("Cliquez ici pour supprimer la correction sélectionnée")))
        self.ctrl_test.SetToolTip(wx.ToolTip(_("Tapez un texte à lire")))
        self.bouton_test.SetToolTip(wx.ToolTip(_("Cliquez ici pour lire le texte saisi")))
    ...
    def OnBoutonTest(self, event): 
        ...
        if len(texte) == 0 :
                dlg = wx.MessageDialog(self, _("Vous n'avez saisi aucun texte à lire !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
