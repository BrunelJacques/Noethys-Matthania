...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        # Bandeau
        intro = _("Vous pouvez ici capturer une photo à partir d'une webcam connectée. Cliquez sur le bouton 'Prendre une photo' pour capturer l'image puis sur 'Ok' pour valider et ouvrir l'éditeur photo qui vous permettra d'effectuer un recadrage avant l'insertion dans la fiche individuelle.")
        titre = _("Capture d'une photo")
        self.SetTitle(titre)
    ...
    def Initialisation(self):
        ...
        if etat == True :
            ...
            dlg = wx.MessageDialog(self, _("Noethys n'a pas réussi à se connecter à la caméra.\nVeuillez vérifier que celle-ci est bien installée..."), _("Erreur"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()

    def __set_properties(self):
        self.bouton_capture.SetToolTip(wx.ToolTip(_("Cliquez ici pour prendre la photo")))
        self.bouton_aide.SetToolTip(wx.ToolTip(_("Cliquez ici pour obtenir de l'aide")))
        self.bouton_options.SetToolTip(wx.ToolTip(_("Cliquez ici pour définir les propriétés de la capture vidéo")))
    ...
    def OnBoutonOptions(self, event): 
        ...
        for index in range(0, 10) :
            id = 10000 + index
            item = wx.MenuItem(sousMenuPort, id, _("Port n°%d") % index, _("Port n°%d") % index, wx.ITEM_CHECK)
            sousMenuPort.AppendItem(item)
            self.Bind(wx.EVT_MENU, self.Menu_port, id=id)
            if self.port == index : item.Check(True)
        menuPop.AppendMenu(10, _("Port de connexion"), sousMenuPort)

        sousMenuFPS = UTILS_Adaptations.Menu()
        for index in (5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 60, 70, 80, 90, 100) :
            id = 20000 + index
            item = wx.MenuItem(sousMenuFPS, id, "%d" % index, _("%d images par secondes") % index, wx.ITEM_CHECK)
            sousMenuFPS.AppendItem(item)
            self.Bind(wx.EVT_MENU, self.Menu_fps, id=id)
            if self.ctrl_video.fps == index : item.Check(True)
        menuPop.AppendMenu(20, _("Nombres d'images par secondes"), sousMenuFPS)

##        menuPop.AppendSeparator()
        
##        menuPop.AppendItem(wx.MenuItem(menuPop, 10, _("Propriétés du flux vidéo")))
##        self.Bind(wx.EVT_MENU, self.Menu_proprietes_pin, id=10)
        
##        menuPop.AppendItem(wx.MenuItem(menuPop, 20, _("Propriétés de la capture vidéo")))
##        self.Bind(wx.EVT_MENU, self.Menu_proprietes_filter, id=20)
    ...
    def Menu_proprietes_pin(self, event):
        if self.ctrl_video.IsRunning() == False :
            dlg = wx.MessageDialog(self, _("La caméra n'est pas connectée !"), _("Erreur"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return False
    ...
    def OnBoutonOk(self, event): 
        if self.image == None :
            dlg = wx.MessageDialog(self, _("Vous devez d'abord capturer une image !"), _("Erreur"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        self.FermeVideo() 
