...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        intro = _("Vous pouvez ici saisir, modifier ou supprimer des consommations pour un ensemble d'individus. Sélectionnez une activité ainsi que les paramètres de l'action à appliquer. Cochez les individus souhaités puis cliquez sur le bouton Commencer pour lancer le traitement.")
        self.SetTitle(titre)
        ...
        self.label_action = wx.StaticText(self, -1, "Action :")
        self.ctrl_action = wx.TextCtrl(self, -1, "")
        self.ctrl_action.Enable(False)
        self.bouton_action = wx.Button(self, -1, _("Sélectionner une action"))
    ...
    def __set_properties(self):
        ...
        self.bouton_action.SetToolTip(wx.ToolTip(_("Cliquez ici pour sélectionner les paramètres des actions")))
    ...
    def OnBoutonOk(self, event): 
        ...
        if TraitmentEnCours:
            ...
            if IDactivite == None :
                dlg = wx.MessageDialog(self, _("Vous devez sélectionner une activité dans la liste !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False

            if self.dictAction == None :
                dlg = wx.MessageDialog(self, _("Vous devez sélectionner une action en cliquant sur le bouton 'Sélectionner une action' !"), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
            ...
            dlgConfirm = wx.MessageDialog(self, _("Souhaitez-vous appliquer le traitement par lot pour les %d individus cochés ?\n\nLe processus peut prendre quelques minutes...") % len(tracks), _("Confirmation"), wx.YES_NO|wx.CANCEL|wx.YES_DEFAULT|wx.ICON_QUESTION)
            reponse = dlgConfirm.ShowModal()
            dlgConfirm.Destroy()
