...
class Dialog(wx.Dialog):
    def __init__(self, parent, mode):
        ...
        intro = ("Définit une ligne de  tarification appelée Article")
        ...
        self.staticbox_CARACTER = wx.StaticBox(self, -1, _("Caractéristiques de l'article"))
        ...
        self.label_comptable = wx.StaticText(self, -1, _("Compte comptablité :"))
        ...
        self.check_NiveauActivite = wx.CheckBox(self, 0, _("Proposé au niveau de l'activité"))
        self.check_NiveauFamille = wx.CheckBox(self, -1, _("Proposé au niveau de la famille"))
    ...
    def __set_properties(self):
        ...
        self.ctrl_code.SetToolTip(_("16carMaxi - Un code article commençant par '$' est proposé et précoché systématiquement, par 'x' poposé mais non précoché. "))
        self.ctrl_libelle.SetToolTip(_("128carMax - Saisissez ici une déscription de l'article évoquant ses options"))
        self.choice_conditions.SetToolTip(_("Sélectionnez les conditions  parmi la liste"))
        self.choice_modeCalcul.SetToolTip(_("Sélectionnez le format de Ligne  parmi la liste"))
        self.choice_typeLigne.Select(0)
        self.txt_typeLigne.SetValue(self.liste_typeLigne[0][1])
        self.choice_typeLigne.SetToolTip(_("Sélectionnez le type de Ligne  parmi la liste"))
        self.choice_comptable.SetToolTip(_("Sélectionnez le regroupement comptable parmi la liste"))
        self.check_NiveauActivite.SetToolTip(_("L'article est proposé au Niveau Activité"))
        self.check_NiveauFamille.SetToolTip(_("L'article est proposé au Niveau Famille"))
    ...
    def OnBoutonOk(self, event):
        ...
        try:
            ...
            if textCode == "":
                message = "Vous devez obligatoirement donner un code à cet article_ !"
            if len(textCode) > 16:
                message = "La longueur d'un code article est limitée à 16 caractères_ !"
                textCode = ""
        except:
            message = "Un code article ne doit pas avoir de caractère spécial ou un accent_ !"
            textCode = ""
        if textCode == "" :
    ...
    def OnBoutonConditions(self, event):
        intro = "Les conditions sont évaluées pour l'apparition de l'article au moment de la tarification"
    ...
    def OnBoutonModeCalcul(self, event):
        dlg2 = CTRL_ChoixListe.Dialog(self,LargeurCode= 100,LargeurLib= 200,minSize = (500,300), listeOriginale=self.liste_modeCalcul, titre = "Choisissez un mode de calcul", intro = "Ce calcul détermira le prix en fonction des circonstances  ")
    ...
    def OnBoutonTypeLigne(self, event):
        intro = "Le type de ligne détermine le comportement du regroupement des articles de la tarification"
        dlg = CTRL_ChoixListe.Dialog(self,LargeurCode= 120,LargeurLib= 300,minSize = (500,300), listeOriginale=self.liste_typeLigne, titre = "Choisissez un type de ligne", intro = "Ce type détermira le comportement de certains calculs")
        interroChoix = dlg.ShowModal()
        dlg.Destroy()
        if interroChoix == wx.ID_OK :
