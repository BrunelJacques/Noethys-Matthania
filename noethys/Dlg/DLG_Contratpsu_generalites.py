...
class Panel(wx.Panel):
    def __init__(self, parent, clsbase=None):
        ...
        # Période
        self.staticbox_periode_staticbox = wx.StaticBox(self, -1, _("Dates du contrat"))
        ...
        # Paramètres
        self.staticbox_parametres_staticbox = wx.StaticBox(self, -1, _("Autres paramètres"))
    ...
    def __set_properties(self):
        self.ctrl_date_debut.SetToolTip(wx.ToolTip(_("Saisissez la date de début du contrat")))
        self.ctrl_date_fin.SetToolTip(wx.ToolTip(_("Saisissez la date de fin du contrat")))
        self.ctrl_observations.SetToolTip(wx.ToolTip(_("Saisissez d'éventuelles observations")))
    ...
    def Validation(self):
        if self.ctrl_date_debut.Validation() == False or self.ctrl_date_debut.GetDate() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de début de contrat valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.ctrl_date_fin.Validation() == False or self.ctrl_date_fin.GetDate() == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir une date de fin de contrat valide !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.ctrl_date_debut.GetDate() > self.ctrl_date_fin.GetDate() :
            dlg = wx.MessageDialog(self, _("Vous ne pouvez pas avoir une date de début de contrat supérieure à la date de fin !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_date_fin.SetFocus()
            return False
