...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDcompte_payeur=None, solde=None):
        ...
        titre = _("Saisir un remboursement")
        intro = _("Cette fonctionnalité permet de rembourser un avoir. Deux prestations et un règlement fictif seront générés automatiquement afin de régulariser le compte.")
        self.SetTitle(titre)
    ...
    def __set_properties(self):
        self.ctrl_montant.SetToolTip(wx.ToolTip(_("Saisissez ici le montant du remboursement")))
        self.ctrl_observations.SetToolTip(wx.ToolTip(_("Saisissez ici des observations [Optionnel]. Vous pouvez par exemple saisir le numéro de chèque qui a été utilisé pour le remboursement, etc...")))
        self.ctrl_payeur.SetToolTip(wx.ToolTip(_("Sélectionnez le payeur qui sera utilisé dans l'enregistrement du règlement fictif")))
        self.bouton_payeurs.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter, modifier ou supprimer un payeur")))
        self.ctrl_mode.SetToolTip(wx.ToolTip(_("Sélectionnez le mode de règlement qui sera utilisé. Si un mode 'Remboursement' n'existe pas, créez-le.")))
        self.bouton_modes.SetToolTip(wx.ToolTip(_("Cliquez ici pour ajouter, modifier ou supprimer un mode de règlement")))
        self.ctrl_compte.SetToolTip(wx.ToolTip(_("Sélectionnez le compte bancaire qui sera utilisé pour le remboursement")))
    ...
    def CreerMode(self):
        ...
        if present == True :
            dlg = wx.MessageDialog(self, _("Il semblerait qu'un mode 'Remboursement' soit déjà présent.\n\nSouhaitez-vous quand même créer un nouveau mode ?"), _("Avertissement"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
        ...
        # Confirmation
        dlg = wx.MessageDialog(self, _("Confirmez-vous la création d'un mode de règlement 'Remboursement' ?"), _("Confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        reponse = dlg.ShowModal()
        dlg.Destroy()
        if reponse != wx.ID_YES :
            return
    ...
    def OnBoutonOk(self, event):
        ...
        if montant == 0.0 :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement saisir un montant !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if montant < 0.0 :
            dlg = wx.MessageDialog(self, _("Le montant doit obligatoirement être positif !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_montant.SetFocus()
            return
        ...
        if self.solde != None and FloatToDecimal(montant) > FloatToDecimal(self.solde) :
            dlg = wx.MessageDialog(self, _("Le montant du remboursement ne doit pas être supérieur au solde du compte !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_montant.SetFocus()
            return
        ...
        IDpayeur = self.ctrl_payeur.GetID()
        if IDpayeur == None :
            dlg = wx.MessageDialog(self, _("Vous devez obligatoirement sélectionner un payeur !"), _("Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
