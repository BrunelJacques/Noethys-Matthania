...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        # Bandeau
        intro = _("Cette fonction vous permet d'identifier rapidement les familles ayant des impayés sur une période donnée. Commencez par saisir une période puis sélectionnez un groupe d'activités. Les résultats peuvent être regroupés par activité ou par famille. Ceux-ci peuvent ensuite être édités sous forme de PDF ou exportés sous Excel.")
        titre = _("Synthèse des impayés")
        ...
        self.label_regroupement = wx.StaticText(self, -1, _("Regroupement :"))
        self.ctrl_regroupement = wx.Choice(self, -1, choices = [_("Activités"), _("Familles")])
        self.ctrl_regroupement.Select(0)
        
        self.label_periode = wx.StaticText(self, -1, _("Période :"))
        self.ctrl_periode = wx.Choice(self, -1, choices = [_("Mois"), _("Année")])
    ...
    def __set_properties(self):
        ...
        self.bouton_ouvrir_fiche.SetToolTip(wx.ToolTip(_("Cliquez ici pour ouvrir la fiche famille sélectionnée dans la liste")))
        self.ctrl_regroupement.SetToolTip(wx.ToolTip(_("Sélectionnez ici le mode de regroupement : Activités ou Familles")))
        self.ctrl_periode.SetToolTip(wx.ToolTip(_("Sélectionnez ici le mode d'affichage : Mois ou Année")))
