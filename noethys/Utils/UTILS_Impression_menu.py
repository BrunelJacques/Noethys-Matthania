...
class Titre():
    ...
    def Dessine_titre(self, texte=""):
        ...
        if afficher_mot_menu == True :
            ...
            self.canvas.drawCentredString(x_box + largeur_box / 2.0, y_box + hauteur_box / 2.0 - hauteur_font / 2.0, _("MENUS"))
            self.canvas.rotate(-2)
    ...
    def Dessine_legende(self, liste_legendes=[]):
        ...
        for dictLegende in liste_legendes :
            ...
            if self.parent.dictDonnees["legende_type"] == "numero" :
                texte = "<font color='%s'><super>%d</super></font>&nbsp;%s&nbsp;&nbsp;" % (couleur, numero, dictLegende["nom"])
            elif self.parent.dictDonnees["legende_type"] == "carre" :
                texte = "<font face='ZapfDingbats' color='%s'>1</font>&nbsp;%s&nbsp;&nbsp;" % (couleur, dictLegende["nom"])
            else :
                texte = ""
            ...
            for chaine in liste_legendes :
                ...
                if IDlegende in self.parent.dictDonnees["legendes"] and IDlegende in self.parent.dictNumerosLegendes:
                    ...
                    if self.parent.dictDonnees["legende_type"] == "numero":
                        ...
                        chaine_remplacement = "<font color='%s' size='-2'><super>%d</super></font>" % (couleur, numero)
                    elif self.parent.dictDonnees["legende_type"] == "carre":
                        chaine_remplacement = "<font face='ZapfDingbats' color='%s' size='-2'><super>X</super></font>" % couleur
                    else :
                        chaine_remplacement = ""
...
class Impression():
        ...
        for dict_page in liste_pages :
            ...
            if dictDonnees["titre_afficher"] == True:
                if dictDonnees["titre_texte"] != "" :
                    texte_titre = "%s %s" % (dictDonnees["titre_texte"], texte_titre)
            ...
            if dictDonnees["page_grille"] == True :
                ...
                canvas.drawString(10, 10, _("Largeur carreau=50"))
    ...
    def GetLabelSemaine(self, date_debut=None, date_fin=None):
        if date_debut.year == date_fin.year and date_debut.month == date_fin.month :
            ...
            fin = "%d %s %d" % (date_fin.day, UTILS_Dates.LISTE_MOIS[date_fin.month-1], date_fin.year)
        elif date_debut.year == date_fin.year :
            debut = "%d %s" % (date_debut.day, UTILS_Dates.LISTE_MOIS[date_debut.month-1])
            fin = "%d %s %d" % (date_fin.day, UTILS_Dates.LISTE_MOIS[date_fin.month-1], date_fin.year)
        else :
            debut = UTILS_Dates.DateDDEnFr(date_debut)
            fin = UTILS_Dates.DateDDEnFr(date_fin)
        return _("Semaine du %s au %s") % (debut, fin)
