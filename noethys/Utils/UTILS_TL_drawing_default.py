...
LISTE_JOURS = (_("Lundi"), _("Mardi"), _("Mercredi"), _("Jeudi"), _("Vendredi"), _("Samedi"), _("Dimanche"))
LISTE_JOURS_ABREGE = (_("Lun"), _("Mar"), _("Mer"), _("Jeu"), _("Ven"), _("Sam"), _("Dim"))
LISTE_MOIS = (_("Janvier"), _("Février"), _("Mars"), _("Avril"), _("Mai"), _("Juin"), _("Juillet"), _("Août"), _("Septembre"), _("Octobre"), _("Novembre"), _("Décembre"))
LISTE_MOIS_ABREGE = (_("Jan."), _("Fév."), _("Mars"), _("Avr."), _("Mai"), _("Juin"), _("Juil."), _("Août"), _("Sept."), _("Oct."), _("Nov."), _("Déc."))
...
class StripMonth(Strip):
    ...
    def label(self, time, major=False):
        if major:
            return "%s %s" % (LISTE_MOIS[time.month-1], time.year)
...
class StripWeek(Strip):
    ...
    def label(self, time, major=False):
        if major:
            ...
            return (_("Semaine %s (%s)")) % (time.isocalendar()[1], range_string)
        ...
        * 28 Jun 08-3 Jul 2009
        ...
        if time1.year == time2.year:
            if time1.month == time2.month:
                return "%s-%s %s %s" % (time1.day, time2.day, LISTE_MOIS[time1.month-1], time1.year)
            return "%s %s-%s %s %s" % (time1.day, LISTE_MOIS_ABREGE[time1.month-1], time2.day, LISTE_MOIS_ABREGE[time2.month-1], time1.year)
        return "%s %s %s-%s %s %s" % (time1.day, LISTE_MOIS_ABREGE[time1.month-1], time1.year, time2.day, LISTE_MOIS_ABREGE[time2.month-1], time2.year)
...
class StripDay(Strip):
    ...
    def label(self, time, major=False):
        if major:
            return "%s %s %s" % (time.day, LISTE_MOIS[time.month-1], time.year)
        return str(time.day)
...
class StripWeekday(Strip):
    ...
    def label(self, time, major=False):
        if major:
            return "%s %s %s %s" % (LISTE_JOURS[time.weekday()-1], time.day, LISTE_MOIS[time.month-1], time.year)
...
class StripHour(Strip):
    ...
    def label(self, time, major=False):
        if major:
            return "%s %s %s %s" % (time.day, LISTE_MOIS[time.month-1], time.year, time.hour)
