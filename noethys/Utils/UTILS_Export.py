...
def ExportExcel(listview=None, grid=None, titre=_("Liste"), listeColonnes=None, listeValeurs=None, autoriseSelections=True, onlyCheck = False):
    ...
    try :
        ...
        dlg = wx.MessageDialog(None, _("Il est impossible d'enregistrer le fichier Excel. Veuillez vérifier que ce fichier n'est pas déjà ouvert en arrière-plan."), "Erreur", wx.OK | wx.ICON_ERROR)
        dlg.ShowModal()
        dlg.Destroy()
        return
...
class DLG_Choix_action(wx.Dialog):
    def __init__(self, parent):
        ...
        # Bandeau
        intro = _("Sélectionnez l'action à effectuer. Vous pouvez enregistrer le fichier généré dans le répertoire souhaité ou l'envoyer par email.")
