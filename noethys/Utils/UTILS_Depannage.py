...
class CorrigerHeuresConso(Anomalie):
    def Correction(self, DB=None):
        ...
        if heure_debut != None : 
            dlg = wx.TextEntryDialog(None, _("Corrigez l'heure de début de la consommation du %s de %s :\n\n(Obligatoirement au format HH:MM)") % (date.strftime("%d/%m/%Y"), individu), _("Correction d'une heure"), heure_debut)
        ...
        if heure_fin != None : 
            dlg = wx.TextEntryDialog(None, _("Corrigez l'heure de fin de la consommation du %s de %s :\n\n(Obligatoirement au format HH:MM)") % (date.strftime("%d/%m/%Y"), individu), _("Correction d'une heure"), heure_fin)
            if dlg.ShowModal() == wx.ID_OK:
...
class Depannage():
    ...
    def GetTexte(self):
        ...
        for labelProbleme, labelCorrection, listeDonnees in self.listeResultats :
            texte += "<P><B>%s (%d) :</B><FONT SIZE=2><BR>" % (labelProbleme, len(listeDonnees))
            
            if len(listeDonnees) > 0 :
                for anomalie in listeDonnees :
                    texteCorrection = _("<A HREF='http://www.noethys.com'>Correction proposée : %s</A>") % labelCorrection
                    texte += "&nbsp;&nbsp;&nbsp;<IMG SRC='Static/Images/16x16/Interdit2.png'>&nbsp;%s. %s<BR>" % (anomalie.label, texteCorrection)
            else :
                    texte += "&nbsp;&nbsp;&nbsp;<IMG SRC='Static/Images/16x16/Ok4.png'>&nbsp;Aucune anomalie détectée...<BR>"
            texte += "</FONT></P>"
        return texte
    ...
    def RechercheDonnees(self):        
        self.listeResultats = []
        EcritStatusbar(_("Recherche d'anomalies en cours...   Veuillez patienter..."))
        try :
            dlgAttente = wx.BusyInfo(_("Recherche d'anomalies en cours. Veuillez patienter..."), self.parent)
    ...
    def InscriptionsSansIndividus(self):
        labelProbleme = _("Inscriptions sans individus associés")
        labelCorrection = _("Supprimer l'inscription")
        ...
        for IDinscription, IDactivite, nomActivite, dateInscription, IDindividu in listePrestations :
            label = _("Individu ID%d inscrit le %s à l'activité %s") % (IDindividu, UTILS_Dates.DateEngFr(dateInscription), nomActivite)
    ...
    def CotisationsSansIndividus(self):
        labelProbleme = _("Cotisations sans individus associés")
        labelCorrection = _("Supprimer la cotisation")
        ...
        for IDcotisation, nomType, nomUnite, dateSaisie, IDindividu, IDprestation in listeDonnees :
            label = _("Cotisation ID%d de type '%s - %s' saisie le %s pour l'individu ID%d") % (IDcotisation, nomType, nomUnite, UTILS_Dates.DateEngFr(dateSaisie), IDindividu)
    ...
    def CotisationsSansFamilles(self):
        labelProbleme = _("Cotisations sans familles associées")
        labelCorrection = _("Supprimer la cotisation")
        ...
        for IDcotisation, nomType, nomUnite, dateSaisie, IDfamille, IDprestation in listeDonnees :
            label = _("Cotisation ID%d de type '%s - %s' saisie le %s pour la famille ID%d") % (IDcotisation, nomType, nomUnite, UTILS_Dates.DateEngFr(dateSaisie), IDfamille)
    ...
    def PrestationsSansFamilles(self):
        labelProbleme = _("Prestations sans familles associées")
        labelCorrection = _("Supprimer la prestation")
        ...
        for IDprestation, date, label, IDfamille in listeDonnees :
            label = _("Prestation ID%d '%s' saisie le %s pour la famille ID%d") % (IDprestation, label, UTILS_Dates.DateEngFr(date), IDfamille)
    ...
    def VentilationsSansPrestations(self):
        labelProbleme = _("Ventilations sans prestations associées")
        labelCorrection = _("Supprimer la ventilation")
        ...
        for IDventilation, IDprestation, IDreglement, IDfamille in listeDonnees :
            if IDventilation and IDprestation and IDreglement and IDfamille:
                label = _("Ventilation ID%d pour la prestation ID%d et le règlement ID%d pour la famille ID%d") % (IDventilation, IDprestation, IDreglement, IDfamille)
    ...
    def VentilationsSansReglements(self):
        labelProbleme = _("Ventilations sans règlements associés")
        labelCorrection = _("Supprimer la ventilation")
    ...
    def IndividusIncomplets(self):
        labelProbleme = _("Fiches individuelles tronquées")
        labelCorrection = _("Modifier la civilité et/ou le nom de famille")
        ...
        for IDindividu, IDcivilite, nom, prenom in listeIndividus :
            label = _("Individu ID%d (%s %s)") % (IDindividu, nom, prenom)
    ...
    def FamillesIncompletes(self):
        labelProbleme = _("Fiches familles tronquées")
        labelCorrection = _("Réparer la fiche famille")
        ...
        for IDfamille, IDcompte_payeur in listeFamilles :
            label = _("Famille ID%d") % IDfamille
            listeTemp.append(FamillesIncompletes(label=label, IDfamille=IDfamille))
        self.listeResultats.append((labelProbleme, labelCorrection, listeTemp))

    def InscriptionsIncompletes(self):
        labelProbleme = _("Inscriptions tronquées")
        labelCorrection = _("Réparer la table des inscriptions")
        ...
        for IDinscription, IDfamille, IDindividu, IDcompte_payeur in listeInscriptions :
            label = _("Inscription ID%d (Famille ID%d Individu ID%d)") % (IDinscription, IDfamille, IDindividu)
    ...
    def ConsommationsIncompletes(self):
        labelProbleme = _("Consommations tronquées")
        labelCorrection = _("Réparer la table des consommations")
        ...
        for IDconso, IDinscription, IDindividu, IDcompte_payeur in listeInscriptions :
            label = _("Conso ID%d (IDinscription ID%d Individu ID%d)") % (IDconso, IDinscription, IDindividu)
    ...
    def FacturesErronees(self):
        labelProbleme = _("Factures erronées")
        labelCorrection = _("Recalculer le montant des factures")
        ...
        for IDfacture, numeroFacture, IDcompte_payeur, IDfamille, totalFacture, regleFacture, soldeFacture in listeFactures :
            ...
            if montantTotalPrestations != FloatToDecimal(totalFacture) :
                ...
                if IDfamille == None : 
                    label = _("Facture ID%d : Famille inconnue") % IDfacture
                    listeTemp.append(FacturesAsupprimer(label=label, IDfacture=IDfacture))
                elif len(listePrestationsFacture) == 0 :
                    label = _("Facture ID%d : Aucune prestation correspondante") % IDfacture
                    listeTemp.append(FacturesAsupprimer(label=label, IDfacture=IDfacture))
                elif montantTotalPrestations == FloatToDecimal(0.0) :
                    label = _("Facture ID%d : Montant de la facture égal à 0") % IDfacture
                    listeTemp.append(FacturesAsupprimer(label=label, IDfacture=IDfacture))
                else :
                    label = _("Facture ID%d : Total à recalculer") % IDfacture
    ...
    def ConsommationsErronees(self):
        labelProbleme = _("Consommations erronées")
        labelCorrection = _("Réparer la table des consommations")
        ...
        for IDconso, date, heure_debut, heure_fin, nomIndividu, prenomIndividu in listeConso :
            ...
            if heure_debut != None :
                if ValidationHeure(heure_debut) == False :
                    label = _("Conso ID%d de %s : Heure de début de consommation erronée ('%s')") % (IDconso, individu, heure_debut)
            ...
            if heure_fin != None :
                if ValidationHeure(heure_fin) == False :
                    label = _("Conso ID%d de %s : Heure de fin de consommation erronée ('%s')") % (IDconso, individu, heure_fin)
    ...
    def LiensTronques(self):
        labelProbleme = _("Liens sans individus associés")
        labelCorrection = _("Supprimer le lien dans la base")
        ...
        for liste in (listeSujets, listeObjets) :
            for IDlien, IDindividu in liste :
                label = _("Lien ID%d (Individu ID%d)") % (IDlien, IDindividu)
                listeTemp.append(LiensTronques(label=label, IDlien=IDlien, IDindividu=IDindividu))
        self.listeResultats.append((labelProbleme, labelCorrection, listeTemp))

    def RattachementsTronques(self):
        labelProbleme = _("Rattachements sans individus associés")
        labelCorrection = _("Supprimer le rattachement dans la base")
        ...
        for IDrattachement, IDindividu, IDfamille in listeDonnees :
            label = _("Rattachement ID%d (Famille ID%d - Individu ID%d)") % (IDrattachement, IDfamille, IDindividu)
    ...
    def LiensErrones(self):
        labelProbleme = _("Liens de parenté erronés")
        labelCorrection = _("Supprimer les liens erronés")
        ...
        for IDfamille in listeIDfamille :
            ...
            label = _("Famille ID%d : %d liens erronés") % (IDfamille, len(listeIDlien))
            listeTemp.append(LiensErrones(label=label, IDfamille=IDfamille, listeIDlien=listeIDlien))
        self.listeResultats.append((labelProbleme, labelCorrection, listeTemp))
        
    def VentilationExcessive(self):
        labelProbleme = _("Ventilations excessives")
        labelCorrection = _("Supprimer la ventilation excessive")
    ...
    def PrestationsFantomes(self):
        labelProbleme = _("Prestations fantômes")
        labelCorrection = _("Supprimer la prestation fantôme")
        ...
        for IDprestation, label, date, IDfamille, IDindividu, nom, prenom in listePrestations:
            ...
            if (IDprestation in dictPrestations) == False:
                if nom != None and prenom != None:
                    ...
                    nomIndividu = ""
                label = "Prestation ID%d du %s pour %s : %s" % (IDprestation, UTILS_Dates.DateDDEnFr(date), nomIndividu, label)
                listeTemp.append(PrestationsFantomes(label=label, IDprestation=IDprestation))

        self.listeResultats.append((labelProbleme, labelCorrection, listeTemp))

    def MySQL8(self):
        labelProbleme = _("MySQL 8 Server détecté")
        labelCorrection = _("Correction manuelle requise : L'utilisation de la version 8 de MySQL est déconseillée. Faites une sauvegarde, désinstallez MySQL 8, installez MySQL 5.7 et restaurez la sauvegarde.")
