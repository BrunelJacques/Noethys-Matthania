...
from Utils import UTILS_Impression_attestation_fiscale
...
class Attestations_fiscales():
    ...
    def GetDonneesImpression(self, tracks=[], dictOptions={}):
        ...
        for track in tracks :
            ...
            for prestation in track.listePrestations :
                ...
                if (IDindividu in dictEnfants) == False : 
                    ...
                    if prestation["IDcivilite"] != None :
                        ...
                        if sexe == "F" :
                            genre = "e"
                    nomComplet = "%s %s" % (prestation["nom"], prenom)
            ...
            dictDonnee = {
                ...
                "{MONTANT_FACTURE}" : "%.2f %s" % (track.montant_total, SYMBOLE),
                "{MONTANT_REGLE}" : "%.2f %s" % (track.montant_regle, SYMBOLE),
                "{MONTANT_IMPAYE}" : "%.2f %s" % (track.montant_impaye, SYMBOLE),
            ...
            for nomCompletIndividu, dictIndividu in listeIndividus :
                dictDonnee["TXT_ENFANT_%d" % index] = _("%.2f %s pour %s né%s le %s") % (dictIndividu["regle"], SYMBOLE, nomCompletIndividu, dictIndividu["genre"], dictIndividu["date_naiss"])
                index += 1
    ...
    def Impression(self, tracks=[], nomDoc=None, afficherDoc=True, dictOptions=None, repertoire=None, repertoireTemp=False):
        ...
        def CreationPDFunique(repertoireCible=""):
            dictPieces = {}
            dlgAttente = wx.BusyInfo(_("Génération des attestations fiscales à l'unité au format PDF..."), None)
            if 'phoenix' not in wx.PlatformInfo:
                wx.Yield()
            try :
                index = 0
        ...
        if repertoireTemp == False :
            dlgAttente = wx.BusyInfo(_("Création du PDF des attestations fiscales..."), None)
            try :
                if 'phoenix' not in wx.PlatformInfo:
                    wx.Yield()
            except :
