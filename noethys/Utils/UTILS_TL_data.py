...
class Event(object):
    ...
    def get_label(self):
        ...
        return "%s (%s)" % (self.text, self.time_period.get_label())
...
class TimePeriod(object):
    ...
    def get_label(self):
        ...
        def label_without_time(time):
            return "%s %s %s" % (time.day, local_to_unicode(calendar.month_abbr[time.month]), time.year)
        def time_label(time):
            return time.time().isoformat()[0:5]
        if self.is_period():
            if has_nonzero_time(self.start_time, self.end_time):
                label = "%s to %s" % (label_with_time(self.start_time),
                                      label_with_time(self.end_time))
            else:
                label = "%s to %s" % (label_without_time(self.start_time),
                                      label_without_time(self.end_time))
        else:
            if has_nonzero_time(self.start_time, self.end_time):
                label = "%s" % label_with_time(self.start_time)
            else:
                label = "%s" % label_without_time(self.start_time)
        return label
