...
class Cotisation():
    ...
    def GetDonneesImpression(self, listeCotisations=[]):
        ...
        for item in listeDonnees :
            ...
            nomCotisation = "%s - %s" % (nomTypeCotisation, nomUniteCotisation)
            
            # Type
            if typeTypeCotisation == "famille" :
                typeStr = _("Cotisation familiale")
            else:
                typeStr = _("Cotisation individuelle")
                        
            # Dépôt
            if IDdepot_cotisation == None :
                depotStr = _("Non déposée")
            else:
                depotStr = _("Dépôt n°%d") % IDdepot_cotisation
            ...
            montantStr = "%.02f %s" % (montant, SYMBOLE)
            regleStr = "%.02f %s" % (ventilation, SYMBOLE)
            soldeStr = "%.02f %s" % (solde, SYMBOLE)
    ...
    def Impression(self, listeCotisations=[], nomDoc=None, afficherDoc=True, dictOptions=None, repertoire=None, repertoireTemp=False):
        ...
        if dictOptions == None :
            if afficherDoc == False :
                dlg = DLG_Apercu_cotisation.Dialog(None, titre=_("Sélection des paramètres de la cotisation"), intro=_("Sélectionnez ici les paramètres d'affichage de la cotisation."))
        ...
        def CreationPDFunique(repertoireCible=""):
            dictPieces = {}
            dlgAttente = wx.BusyInfo(_("Génération des cotisations à l'unité au format PDF..."), None)
            if 'phoenix' not in wx.PlatformInfo:
                wx.Yield()
            try :
                index = 0
                for IDcotisation, dictCotisation in dictCotisations.items() :
                    if dictCotisation["select"] == True :
                        ...
                        nomFichier = _("Cotisation %s - %s") % (numero_cotisation, nomTitulaires)
                        cheminFichier = "%s/%s.pdf" % (repertoireCible, nomFichier)
                        dictComptesTemp = {IDcotisation : dictCotisation}
                        self.EcritStatusbar(_("Edition de la cotisation %d/%d : %s") % (index, len(dictCotisation), nomFichier))
                ...
                dlg = wx.MessageDialog(None, _("Désolé, le problème suivant a été rencontré dans l'édition des cotisations : \n\n%s") % err, _("Erreur"), wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        ...
        if repertoireTemp == False :
            dlgAttente = wx.BusyInfo(_("Création du PDF des cotisations..."), None)
            if 'phoenix' not in wx.PlatformInfo:
                wx.Yield()
            self.EcritStatusbar(_("Création du PDF des cotisations en cours... veuillez patienter..."))
            try :
