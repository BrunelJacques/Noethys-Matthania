...
class Export():
    def __init__(self):
        self.dictTables = {
            ...
            "parametres":[      ("IDparametre", "INTEGER PRIMARY KEY AUTOINCREMENT", _("ID du paramètre")),
                                            ("nom", "VARCHAR(400)", _("Nom du paramètre")),
                                            ("valeur", "VARCHAR(400)", _("Valeur du paramètre")),
                                            ],
            ...
            "titulaires":[           ("IDtitulaires", "INTEGER PRIMARY KEY AUTOINCREMENT", _("ID de la ligne")),
                                            ("IDfamille", "INTEGER", _("ID de la famille")),
                                            ("nom", "VARCHAR(450)", _("Nom des titulaires")),
                                            ],
            ...
            "organisateur":[      ("IDorganisateur", "INTEGER PRIMARY KEY AUTOINCREMENT", _("ID de l'organisateur")),
                                            ("nom", "VARCHAR(200)", _("Nom de l'organisateur")),
                                            ("logo", "BLOB", _("Logo de l'organisateur")),
                                            ],
    ...
    def GetChamps_DATA_Tables(self, nomTable=""):
        listeChamps = []
    ...
    def Run(self, afficherDlgAttente=False):
        ...
        if afficherDlgAttente == True :
            dlgAttente = wx.BusyInfo(_("Génération du fichier de données..."), None)
        ...
        try :
            ...
            if afficherDlgAttente == True :
                del dlgAttente
            dlg = wx.MessageDialog(None, _("Désolé, l'erreur suivante a été rencontrée : ") + str(err), "Erreur ", wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return None
    ...
    def EnvoyerVersRepertoire(self, nomFichier=""):
        ...
        wildcard = _("Fichiers de synchronisation Noethys (*%s, *%s)|*%s;*%s|Tous les fichiers (*.*)|*.*") % (EXTENSION_CRYPTE, EXTENSION_DECRYPTE, EXTENSION_CRYPTE, EXTENSION_DECRYPTE)
        dlg = wx.FileDialog(None, message=_("Choisissez un emplacement"), defaultDir=standardPath.GetDocumentsDir(),  defaultFile=os.path.basename(nomFichier), wildcard=wildcard, style=wx.FD_SAVE)
        chemin = None
    ...
    def EnvoyerVersFTP(self, nomFichier=""):
        ...
        # Envoyer le fichier
        dlgAttente = wx.BusyInfo(_("Envoi du fichier de données vers un répertoire FTP..."), None)
        try :
            ...
            print(err)
            del dlgAttente
            dlg = wx.MessageDialog(None, _("Le fichier n'a pas pu être envoyé !\n\nVérifiez les paramètres de connexion FTP dans les paramètres de synchronisation."), "Erreur ", wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        
        del dlgAttente
        dlg = wx.MessageDialog(None, _("Le fichier a été envoyé avec succès !"), "Succès", wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
        return True
