...
DICT_PROCEDURES = {
    "A3687" : _("Transformation des QF de tarifs_lignes unicode->float"),
    "A3688" : _("Suppression des ouvertures avec IDunite=5"),
    "X1234" : _("Exportation des données d'une table dans la table DEFAUT"),
    "S1290" : _("Modification du IDcategorie_tarif"),
    "D1051" : _("Ajout du champ type dans la table Documents"),
    "E4072" : _("Suppression des prestations sans consommations associées"),
    "X0202" : _("Ajout de du champ forfait_date_debut à la table Prestations"),
    "G2345" : _("Attribution d'un ordre aux groupes"),
    "A4567" : _("Suppression de toutes les perspectives de la page d'accueil"),
    "A5000" : _("Réalisation d'une requête"),
    "A5134" : _("Exécution de l'ancienne version de l'état global des consommations"),
    "A5200" : _("Correction des arrondis"),
    "A5300" : _("Nettoyage des groupes d'activités"),
    "A5400" : _("Réparation de l'autoincrement sqlite de la table tarifs_lignes"),
    "A5500" : _("Réparation des liens prestations/factures"),
    "A7650" : _("Renseignement automatique du titulaire Hélios dans toutes les fiches familles"),
    "A8120" : _("Renseignement automatique du type comptable du mode de règlement (banque ou caisse)"),
    "A8260" : _("Modification de la table Paramètres"),
    "A8452" : _("Nettoyage des liens superflus"),
    "A8574" : _("Mise à niveau de la base de données"),
    "A8623" : _("Remplacement des exercices comptables par les dates budgétaires"),
    "A8733" : _("Correction des IDinscription disparus"),
    "A8823" : _("Création de tous les index"),
    "A8836" : _("Suppression des consommations avec prestation disparue"),
    "A8941" : _("Remplit automatiquement le champ états de la table Tarifs"),
    "A8956" : _("Réparation des factures : Rapprochement des factures et des prestations détachées"),
    "A8967" : _("Transfert des numéros des numéros des factures vers le champ str"),
    "A8971" : _("Attribution du type de quotient CAF à tous les quotients existants"),
    "A9001" : _("Modification de la structure de la table Documents"),
    "A9023" : _("Correction des ouvertures avec IDgroupe=0"),
    "A9054" : _("Importation des modèles d'Emails depuis la base défaut"),
    "A9061" : _("Modification de la structure de la table Documents"),
    "A9073" : _("Cryptage des mots de passe utilisateurs"),
    "A9074" : _("Cryptage des mots de passe utilisateurs dans nouveau champ mdpcrypt"),
    "A9075" : _("Suppression des mots de passe utilisateurs en clair"),
    "A9078" : _("Suppression des suppressions des consommations avec IDinscription null"),
    "A9079" : _("Création de la table PHOTOS"),
    "A9080" : _("Création de la table DOCUMENTS"),
    "A9081" : _("Création du profil de configuration par défaut pour la liste des infos médicales"),
    "A9102" : _("Suppression des données personnelles"),
    "A9105" : _("Remplissage du champ statut de la table inscriptions"),
    "A9120" : _("Effacement de toutes les actions du portail"),
    "A9122" : _("Réparation des prestations : Rapprochement des prestations et des consommations détachées"),
    "A9130" : _("Remplissage du champ moteur de la table adresses_mail"),
    "A9135" : _("Vérifie le total des factures"),
    "A9006" : _("Custom 1"),
    "A9007" : _("Correction Module Custom 1"),
    "A9011" : _("Custom SMDH - Suppression inscriptions"),
    "A9012" : _("Custom SMDH - Saisie inscriptions"),
    "A9038" : _("Mise à jour de l'historique des locations"),
    "A9045" : _("Mise à jour du format des lots PES"),
    "A9050" : _("Cryptage des mots de passe des utilisateurs du portail"),
    "A9052" : _("Validation des actions du portail en doublon"),
}
...
def Procedure(code=""):
    ...
    if (code in DICT_PROCEDURES) == False :
        dlg = wx.MessageDialog(None, _("Désolé, cette procédure n'existe pas..."), _("Erreur"), wx.OK | wx.ICON_ERROR)
        dlg.ShowModal()
        dlg.Destroy()
        return
    titre = DICT_PROCEDURES[code]
    # Demande de confirmation de lancement
    dlg = wx.MessageDialog(None, _("Souhaitez-vous vraiment lancer la procédure suivante ?\n\n   -> %s   ") % titre, _("Lancement de la procédure"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
    ...
    try :
        ...
        dlg = wx.MessageDialog(None, _("Désolé, une erreur a été rencontrée :\n\n-> %s  ") % err, _("Erreur"), wx.OK | wx.ICON_ERROR)
        dlg.ShowModal()
        dlg.Destroy()
        return
    # Fin
    # dlg = wx.MessageDialog(None, _("La procédure s'est terminée avec succès."), _("Procédure terminée"), wx.OK | wx.ICON_INFORMATION)
    # dlg.ShowModal()
    # dlg.Destroy()
...
def X1234():
    ...
    for donnees in listeTablesTemp :
        listeTables.append(donnees)
    dlg = wx.MultiChoiceDialog(None, _("Cochez les tables à exporter :"), _("Exportation vers la table DEFAUT"), listeTables)
...
def E4072():
    ...
    for label, listePrestationsTemp in dictResultats.items() :
        texte = _("%s (%d prestations)") % (label, len(listePrestationsTemp))
    ...
    message = _("Cochez les prestations sans consommations associées que vous souhaitez supprimer.\n(Pensez à sauvegarder votre fichier avant d'effectuer cette procédure !)")
    dlg = wx.MultiChoiceDialog(None, message, _("Suppression des prestations sans consommations associées"), listeLabels)
    dlg.SetSize((450, 500))
    dlg.CenterOnScreen() 
    if dlg.ShowModal() == wx.ID_OK:
        message = _("Veuillez patienter durant la procédure...")
...
def A5000():
    ...
    dlg = wx.TextEntryDialog(None, _("Saisissez la requête :"), _("Requête"), "")
    if dlg.ShowModal() == wx.ID_OK:
        req = dlg.GetValue()
        dlg.Destroy()
    else :
...
def A5200():
    ...
    dlgAttente = wx.BusyInfo(_("Veuillez patienter durant la procédure... Celle-ci peut nécessiter quelques minutes..."), None)
    if 'phoenix' not in wx.PlatformInfo:
        wx.Yield()
    ...
    for IDprestation, montant in listePrestations :
        EcritStatusbar(_("Correction des arrondis... Merci de patienter...             -> %d %% effectués") % (index * 100 / total))
...
def A5500():
    ...
    dlg = wx.MessageDialog(None, _("Souhaitez-vous vraiment lancer cette procédure pour %d prestations ?\n\n(Vous pourrez suivre la progression dans la barre d'état)") % nbrePrestations, _("Confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
...
##                    print "Traitement en cours...   %d/%d ....  Prestation ID%d -> Facture ID%d" % (index, nbrePrestations, IDprestation, IDfacture)
                    EcritStatusbar(_("Traitement en cours...   %d/%d ....  Prestation ID%d -> Facture ID%d") % (index, nbrePrestations, IDprestation, IDfacture))
                    nbreSucces += 1
                    time.sleep(0.05)
    ...
    # Fin du traitement
    dlg = wx.MessageDialog(None, _("Procédure terminée !\n\n%d prestations sur %s ont été rattachées avec succès !") % (nbreSucces, nbrePrestations), _("Fin"), wx.OK | wx.ICON_INFORMATION)
    dlg.ShowModal()
    dlg.Destroy()
...
def A8733():
    ...
    if len(listeModifications) > 0 :
        DB.Executermany(_("UPDATE consommations SET IDinscription=? WHERE IDconso=?"), listeModifications, commit=False)
        DB.Commit() 
...
def A8956():
    ...
    # Message de fin
    dlg = wx.MessageDialog(None, _("Résultats :\n\nNombre de factures réparées = %d\nNombre de prestations réparées = %d" % (len(listeFacturesReparees), len(listePrestationsReparees))), _("Fin de la procédure"), wx.OK | wx.ICON_INFORMATION)
    dlg.ShowModal()
    dlg.Destroy()
...
def A8967():
    ...
    if len(listeModifications) > 0 :
        DB.Executermany(_("UPDATE factures SET numerostr=? WHERE IDfacture=?"), listeModifications, commit=False)
        DB.Commit()
...
def A8971():
    ...
    if len(listeTypesQuotients) == 0 :
        IDtype_quotient = DB.ReqInsert("types_quotients", [("nom", "CAF"),])
    else :
...
def A9079():
    ...
    if DB.echec == 1:
        dlg = wx.MessageDialog(None, _("Erreur dans la création du fichier de photos.\n\nErreur : %s") % DB.erreur, _("Erreur de création de fichier"), wx.OK | wx.ICON_ERROR)
        dlg.ShowModal()
        dlg.Destroy()
        return False
...
def A9080():
    ...
    if DB.echec == 1:
        dlg = wx.MessageDialog(None, _("Erreur dans la création du fichier de documents.\n\nErreur : %s") % DB.erreur, _("Erreur de création de fichier"), wx.OK | wx.ICON_ERROR)
        dlg.ShowModal()
        dlg.Destroy()
        return False
...
def A9081():
    ...
    DB = GestionDB.DB()
    IDprofil = DB.ReqInsert("profils", [("label", "Liste par défaut"), ("categorie", "impression_infos_medicales"), ("defaut", 1)])
...
def A9122():
    ...
    for IDindividu, nom, prenom in listeIndividus:
        ...
        dict_individus[IDindividu] = "%s %s" % (nom, prenom)
    ...
    # Message de fin
    dlg = wx.MessageDialog(None, _("Réparation terminée : %d prestations corrigées.") % len(liste_IDprestation_corrigees), _("Fin de la procédure"), wx.OK | wx.ICON_INFORMATION)
    dlg.ShowModal()
    dlg.Destroy()
...
def A9135():
    ...
    for IDfacture, IDfamille, date_debut, date_fin, total, nbre_prestations, total_prestations in listeFactures:
        ...
        if IDfamille in titulaires:
            ...
            famille = "Titulaires inconnus"
        if total != total_prestations:
            texte = "Famille %s : Facture du %s au %s (Montant initial = %s / Montant actuel = %s)" % (famille, date_debut, date_fin, total, total_prestations)
