...
def CreationIdentifiant(IDfamille=None, IDutilisateur=None, nbreCaract=8):
    ...
    if IDfamille != None :
        identifiant = "F%d" % (int(identifiant) + IDfamille)
    if IDutilisateur != None :
        identifiant = "U%d" % (int(identifiant) + IDutilisateur)
    return identifiant
...
def ImporteFichierReservations():
    ...
    if len(txtPresences) < 2 :
        etat =  _("> Aucune présence a télécharger.")
        return etat
    ...
    etat = _("> %d réservations trouvées / %d réservations enregistrées dans la base") % (nbreReservations, nbreEnregistrements)
    return etat
...
def UploadFichierIdentites():
    ...
    etat =  _("> %d identités mises en ligne") % nbreIdentites
    return etat
...
def GetPiecesAFournir(dictPieces, dictCotisations, dictTypesPieces, dictEnfants, IDfamille):
        ...
        for key, valeurs in dictTypesPieces.items():
            ...
            if attribution == "famille" and nomTypePiece == _("Cotisation annuelle") :
                etat = "pasok"
