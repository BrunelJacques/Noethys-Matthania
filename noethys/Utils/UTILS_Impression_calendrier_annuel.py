...
class Impression():
    def __init__(self, titre="", annee=2009,
        ...
        dataTableauTitre = [(_("Planning %d de %s") % (self.annee, self.titre), _("Edité le %s") % dateDuJour ),]
        ...
        if AFFICHER_VACANCES == True :
            dataTableauLegende[numLigne][numCol] = CaseLegende(0, 10, _("Vacances"), COULEUR_VACANCES, None)
            numLigne += 1
        if AFFICHER_WE == True :
            dataTableauLegende[numLigne][numCol] = CaseLegende(0, 10, _("Week-ends"), COULEUR_WE, None)
            numLigne += 1
        if AFFICHER_FERIES == True :
            dataTableauLegende[numLigne][numCol] = CaseLegende(0, 10, _("Jours fériés"), COULEUR_FERIES, None)
            numLigne += 1
        ...
        if nbre_legendes > 1 :
            ...
            legende = CaseLegende(0, 10, _("Total pour l'année"), None, total_heures)
            dataTableauLegende[numLigne][numCol] = legende
