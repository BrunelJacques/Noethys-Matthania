    ...
    def Impression(self, listeInscriptions=[], nomDoc=None, afficherDoc=True, dictOptions=None, repertoire=None, repertoireTemp=False):
        ...
        if dictOptions == None:
            if afficherDoc == False:
                dlg = DLG_Apercu_inscription.Dialog(None, titre=_("Sélection des paramètres de l'inscription"), intro=_("Sélectionnez ici les paramètres d'affichage de l'inscription."))
            ...
            try:
                ...
                for IDinscription, dictInscription in dictInscriptions.items():
                    if dictInscription["select"] == True:
                        ...
                        nomFichier = _("Inscription %d - %s") % (IDinscription, nomTitulaires)
                        cheminFichier = "%s/%s.pdf" % (repertoireCible, nomFichier)
                        dictComptesTemp = {IDinscription: dictInscription}
                        self.EcritStatusbar(_("Edition de l'inscription %d/%d : %s") % (index, len(dictInscription), nomFichier))
                ...
                dlg = wx.MessageDialog(None, _("Désolé, le problème suivant a été rencontré dans l'édition des inscriptions : \n\n%s") % err, _("Erreur"), wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        ...
        if repertoireTemp == False:
            ...
            self.EcritStatusbar(_("Création du PDF des inscriptions en cours... veuillez patienter..."))
            try:
                ...
                del dlgAttente
                traceback.print_exc(file=sys.stdout)
                dlg = wx.MessageDialog(None, "Désolé, le problème suivant a été rencontré dans l'édition des inscriptions : \n\n%s" % err, _("Erreur"), wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                return False
