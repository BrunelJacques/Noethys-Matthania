...
class DLG_Rapport(wx.Dialog):
    ...
    def __set_properties(self):
        ...
        self.ctrl_rapport.SetToolTip(wx.ToolTip(_("Ce rapport d'erreur a été copié dans le presse-papiers")))
    ...
    def Envoyer_mail(self, commentaires="", joindre_journal=False):
        ...
        if dictExp == None :
            dlg = wx.MessageDialog(self, _("Vous devez d'abord saisir une adresse d'expéditeur depuis le menu Paramétrage > Adresses d'expédition d'Emails. Sinon, postez votre rapport de bug dans le forum de Noethys."), _("Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        dlg = wx.MessageDialog(self, _("Le rapport d'erreur a été envoyé avec succès."), _("Rapport envoyé"), wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
