...
def Impression(lignes=[], imprimante=None, titre=_("Ticket"), nomPolice="Arial", taillePolice=15, interligne=5) :
    if len(lignes) == 0 : 
        return

    # Vérifie que les librairies windows ont bien été chargées
    if LIB_CHARGEES == False :
        dlg = wx.MessageDialog(None, _("L'outil d'impression de ticket ne peut pas être chargé. Ce problème sera sûrement résolu en téléchargeant le Package redistribuable Microsoft Visual C++ 2008 (x86) de Microsoft. Rendez-vous sur http://www.microsoft.com/fr-fr/download/details.aspx?id=29."), _("Erreur"), wx.OK | wx.ICON_ERROR)
        dlg.ShowModal()
        dlg.Destroy()
        return
...
def ImpressionModele(IDmodele=None, dictValeurs={}, titre=_("Ticket")):
    if IDmodele == None : 
        return []
...
if __name__ == "__main__":
    lignes = [
        _("Ceci est la ligne 1"),
        _("Ceci est la ligne 2"),
        _("Ceci est la ligne 3"),
        _("Ceci est la ligne 4"),
        _("Ceci est la ligne 5"),
        ]
    Impression(lignes=lignes, imprimante=None, titre=_("Ticket"))
