...
def ConversionLocalReseau(parent, nomFichier=""):
    ...
    dlg.SetTitle(_("Conversion d'un fichier local en fichier réseau"))
    ...
    if dictResultats["connexion"][0] == False :
        ...
        dlg = wx.MessageDialog(parent, _("La connexion au réseau MySQL est impossible. \n\nErreur : %s") % erreur, _("Erreur de connexion"), wx.OK | wx.ICON_ERROR)
        dlg.ShowModal()
        dlg.Destroy()
        return False
    ...
    if dictResultats["fichier"][0] == True :
        dlg = wx.MessageDialog(parent, _("Le fichier existe déjà."), _("Erreur de création de fichier"), wx.OK | wx.ICON_ERROR)
        dlg.ShowModal()
        dlg.Destroy()
        return False
    ...
    if resultat[0] == True :
        parent.SetStatusText(_("La conversion s'est terminée avec succès."))
        dlg = wx.MessageDialog(None, _("La conversion s'est terminée avec succès. Le nouveau fichier a été créé."), _("Information"), wx.OK | wx.ICON_INFORMATION)
    else :
...
def ConversionReseauLocal(parent, nomFichier=""):
    ...
    from Dlg import DLG_Nouveau_fichier
    dlg = DLG_Nouveau_fichier.MyDialog(parent)
    dlg.SetTitle(_("Conversion d'un fichier réseau en fichier local"))
    ...
    # Demande une confirmation pour la conversion
    message = _("Confirmez-vous la conversion du fichier réseau '%s' en fichier local portant le nom '%s' ? \n\nCette opération va durer quelques instants...\n\n(Notez que le fichier original sera toujours conservé)") % (nomFichierReseauFormate, nouveauFichier)
