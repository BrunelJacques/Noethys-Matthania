...
class Track_periode(object):
    def __init__(self, donnees):
        ...
        # Texte verrous
        liste_categories = []
        if self.verrou_consommations == 1 : liste_categories.append(_("consommations"))
        if self.verrou_prestations == 1 : liste_categories.append(_("prestations"))
        if self.verrou_factures == 1 : liste_categories.append(_("factures"))
        if self.verrou_reglements == 1 : liste_categories.append(_("règlements"))
        if self.verrou_depots == 1 : liste_categories.append(_("dépôts"))
        if self.verrou_cotisations == 1 : liste_categories.append(_("cotisations"))
...
class DLG_Verrouillage(wx.Dialog):
    def __init__(self, parent, liste_problemes=[], intro=None):
        ...
        if intro == None :
            intro = _("Vous ne pouvez pas modifier les données d'une période de gestion qui a été verrouillée.")
