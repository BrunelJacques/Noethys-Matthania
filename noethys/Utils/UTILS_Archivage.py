...
def GetNomCategorie(IDcategorie=1):
    if IDcategorie == 1: nom_categorie = _("représentant")
    elif IDcategorie == 2: nom_categorie = _("enfant")
    elif IDcategorie == 3: nom_categorie = _("contact")
    else: nom_categorie = ""
...
class Archivage():
    ...
    def Archiver_familles(self, etat="archiver"):
        ...
        liste_individus = self.SelectionIndividus(intro=_("Cochez les individus à %s :") % label)
        if liste_individus == False :
            return False

        # Demande de confirmation
        dlg = wx.MessageDialog(None, _("Souhaitez-vous vraiment %s %d fiches familles et %d fiches individuelles ?") % (label, len(self.liste_familles), len(liste_individus)), _("Confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
    ...
    def Archiver_individus(self, etat="archiver"):
        ...
        # Demande de confirmation
        dlg = wx.MessageDialog(None, _("Souhaitez-vous vraiment %s %d fiches individuelles ?") % (label, len(liste_individus)), _("Confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
        reponse = dlg.ShowModal()
        dlg.Destroy()
        if reponse != wx.ID_YES :
            return False
                        ...
                        if IDindividu in dict_rattachements_individus:
                            for IDfamilleTemp, IDcategorieTemp in dict_rattachements_individus[IDindividu]:
                                if IDfamilleTemp not in self.liste_familles:
                                    infos.append(_("Rattaché également à la famille de %s en tant que %s") % (self.dict_titulaires[IDfamilleTemp]["titulairesSansCivilite"], GetNomCategorie(IDcategorieTemp)))

                        if (IDindividu, IDfamille) in dictPrestations:
                            infos.append(_("Des prestations futures sont déjà enregistrées"))
    ...
    def Effacer_familles(self):
        ...
        # Sélection des individus
        liste_individus = self.SelectionIndividus(intro=_("Cochez les individus à effacer :"))
        if liste_individus == False :
            return False

        # Demande de confirmation
        dlg = wx.MessageDialog(None, _("Confirmez-vous l'effacement de %d fiches familles et de %d fiches individuelles ?\n\nAttention, cette action est irréversible !") % (len(self.liste_familles), len(liste_individus)), _("Confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
        reponse = dlg.ShowModal()
        dlg.Destroy()
        if reponse != wx.ID_YES :
            return False
        ...
        # Attente
        dlgAttente = wx.BusyInfo(_("Veuillez patienter durant l'effacement..."), None)
        ...
        # Payeurs
        liste_valeurs = [("nom", _("Payeur effacé")),]
        ...
        # Fin de procédure
        dlg = wx.MessageDialog(None, _("L'effacement a été effectué !"), _("Information"), wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
        return True
    ...
    def Effacer_individus(self, liste_individus=None):
        ...
        if liste_individus == None :
            ...
            # Demande de confirmation
            dlg = wx.MessageDialog(None, _("Confirmez-vous l'effacement de %d fiches individuelles ?\n\nAttention, cette action est irréversible !") % len(liste_individus), _("Confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
            reponse = dlg.ShowModal()
            dlg.Destroy()
            if reponse != wx.ID_YES :
                return False
...
class CTRL_Selection(HTL.HyperTreeList):
    def __init__(self, parent, liste_familles=None, liste_individus=None):
        ...
        if len(self.liste_familles) > 0 :
            self.AddColumn(_("Famille/Individu rattaché"))
        else :
            self.AddColumn(_("Individu/Famille rattachée"))
        self.SetColumnWidth(0, 300)
        self.AddColumn(_("Catégorie"))
        self.SetColumnWidth(1, 100)
        self.AddColumn(_("Avertissements"))
        ...
        if len(self.liste_individus) > 0 :
            for dictIndividu in self.liste_individus:
                ...
                for dictFamille in dictIndividu["liste_rattaches"] :
                    niveau2 = self.AppendItem(niveau1, _("Famille de %s") % dictFamille["nomTitulaires"])
        ...
        for index in range(0, self.GetChildrenCount(self.root)):
            ...
            if len(infos) > 0:
                if afficher_avertissements == True :
                    intro = _("Souhaitez-vous sélectionner %s malgré les avertissements suivants ?") % nom_complet
                    dlg = DLG_Messagebox.Dialog(self, titre=_("Avertissement"), introduction=intro, detail=u"\n".join(infos), icone=wx.ICON_EXCLAMATION, boutons=[_("Oui"), _("Non"), _("Annuler")], defaut=1)
                    reponse = dlg.ShowModal()
                    dlg.Destroy()
                    if reponse != 0 :
                        return False
...
class DLG_Selection(wx.Dialog):
    def __init__(self, parent, intro=_("Cochez les individus à effacer :"), liste_familles=None, liste_individus=None):
    ...
    def __set_properties(self):
        self.SetTitle(_("Sélection des individus"))
