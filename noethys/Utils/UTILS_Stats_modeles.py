...
def GetInfosActivites(DB=None, listeActivites=[]):
    ...
    req = _("SELECT IDactivite, nom, abrege, date_debut, date_fin FROM activites WHERE IDactivite IN %s ORDER BY date_debut;") % conditionActivites
...
def GetPeriodesComparatives(DB=None, dictParametres={}, date_min=None, date_max=None):
    ...
    if dictPeriode["type"] == "mois" :
        ...
        for annee in range(date_min.year, date_max.year+1) :
            ...
            label = "%s %d" % (LISTE_NOMS_MOIS[mois-1], annee)
    ...
    if dictPeriode["type"] == "dates" :
        ...
        for annee in range(date_min.year, date_max.year+1) :
            ...
            label = _("Du %s au %s") % (DateEngFr(str(date_debut_temp)), DateEngFr(str(date_fin_temp)))
    ...
    def GetHTML(self, rubrique=None, page=None, mode="affichage", selectionsCodes=[]):
        ...
        if mode == "impression" :
            ...
            if self.dictParametres["mode"] == "inscrits" :
                labelPeriode = _("Aucune période spécifique")
            else:
                ...
                labelPeriode = _("Période du %s au %s") % (date_debut, date_fin)
            
            # Label des activités
            listeLabels = []
            for IDactivite in self.dictParametres["listeActivites"] :
                listeLabels.append((self.dictParametres["dictActivites"][IDactivite]))
            labelActivites = _("Activités : ") + ", ".join(listeLabels)
            
            # Titre
            html += """<CENTER><TABLE bgcolor="%s" CELLSPACING=1 BORDER=0 COLS=1 WIDTH="100%%">
            ...
            for dictRubrique in self.liste_objets :
                if dictRubrique["code"] in selectionsCodes and (rubrique == None or rubrique == dictRubrique["code"]) :
                    html += """<U><B>%d. %s</B></U>""" % (numRubrique, dictRubrique["nom"].upper())
                    numPage = 0
                    for dictPage in dictRubrique["pages"] :
                        self.MAJ(page=dictPage["code"])
                        if dictPage["code"] in selectionsCodes and (page == None or page == dictPage["code"]) :
                            html += """<BLOCKQUOTE><U><B>%s. %s</B></U>""" % (alphabet[numPage], dictPage["nom"])
                            for objet in dictPage["objets"] :
...
class Tableau(Objet):
    ...
    def GetObjetHTML(self):
        ...
        html = """<CENTER><TABLE bgcolor="%s" CELLSPACING=1 BORDER=0 COLS=%d WIDTH="%s">""" % (couleurCadre, len(self.colonnes), largeurTableau)
        ...
        if len(self.colonnes) > 0 :
            ...
            for label, largeur in self.colonnes :
                html += """<TD bgcolor="%s" WIDTH="%s"><I>%s</I></TD>""" % (couleurFondLabel, largeur, label)
            html += """"""
        ...
        if len(self.lignes) > 0 :
            for ligne in self.lignes :
                html += """<TR ALIGN=CENTER>"""
                for label in ligne :
                    html += """<TD bgcolor="%s">%s</TD>""" % (couleurFondLigne, label)
                html += """</TR>"""
        ...
        if len(self.totaux) > 0 :
            ...
            for label in self.totaux :
                html += """<TD bgcolor="%s"><B>%s</B></TD>""" % (couleurFondTotaux, label)
            html += """</TR>"""
...
##LISTE_DONNEES = [
##
##    {"nom" : _("Individus"), "code" : "individus", "image" : None, "visible" : True, "pages" : [
##    
##            {"nom" : _("Nombre"), "code" : "nombre", "image" : None, "visible" : True, "objets" : [
...
##                    ]},
##
##            {"nom" : _("Nouveaux"), "code" : "nouveaux", "image" : None, "visible" : True, "objets" : [
##                    Graphe_repartition_genre(),
##                    Graphe_test_2(),
##                    Texte_nombre_individus(),
##                    ]},
...
class FrameTest(wx.Frame):
    ...
    def __init__(self, objet=None, dictParametres={}):
        ...
        html = """<HTML><BODY><FONT SIZE=-1><P>%s</P></FONT></BODY></HTML>""" % objet.GetObjetHTML()
