...
def Impression(dictDonnees={}, nomDoc=FonctionsPerso.GenerationNomDoc("RESERVATIONS", "pdf"), afficherDoc=True):
    ...
    def Header():
        ...
        dataTableau.append( (_("Réservations"), _("%s\nEdité le %s") % (UTILS_Organisateur.GetNom(), dateDuJour)) )
        style = TableStyle([
    ...
    if len(dictDonnees) == 0 :
        ...
        story.append(Paragraph("&nbsp;", paraStyle))
        story.append(Paragraph(_("<para align='centre'><b>Aucune réservation</b></para>"), paraStyle))
    ...
    for IDindividu, dictIndividu in dictDonnees.items() :
        ...
        if date_naiss != None :
            if sexe == "M" : 
                texteNaiss = _(", né le %s") % DateEngFr(str(date_naiss))
            else : 
                texteNaiss = _(", née le %s") % DateEngFr(str(date_naiss))
        else:
            texteNaiss = ""
        ...
        for IDactivite, dictActivite in dictIndividu["activites"].items() :
            ...
            dataTableau.append([_("Date"), _("Consommations"), _("Etat"), _("Prestations"), _("Total")])
                    ...
                    for dictUnite in liste_unites:
                        ...
                        if etat != None :
                            ...
                            if dictUnite["type"] == "Horaire" or (dictUnite["type"] == "Evenement" and dictUnite["heure_debut"] and dictUnite["heure_fin"]and dictUnite["heure_debut"] != "00:00" and dictUnite["heure_fin"] != "00:00"):
                                ...
                                labelUnite += _(" (%s-%s)") % (heure_debut, heure_fin)
            ...
            if len(dataTableau) == 1 :
                dlg = wx.MessageDialog(None, _("Il n'y a aucune consommation à imprimer !"), _("Erreur"), wx.OK | wx.ICON_INFORMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return
        ...
        dataTableau.append([Paragraph(_("<para align='right'>Total :</para>"), paraStyle), montantIndividu])
    ...
    if nbreIndividus > 1 :
        ...
        dataTableau.append([Paragraph(_("<para align='right'>TOTAL :</para>"), paraStyle), montantFamille])
        largeursColonnesTotal = [460, 60]
        tableau = Table(dataTableau, largeursColonnesTotal)
        tableau.setStyle(TableStyle(listeStyles))
        story.append(tableau)
    ...
    dictChampsFusion["{SOLDE}"] = "%.02f %s" % (totalFacturationFamille, SYMBOLE)
