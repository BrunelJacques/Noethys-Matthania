...
def Appliquer():
    ...
    for IDprestation, label, nomActivite in listeDonnees :
        if nomActivite == None :
            ...
            label = "%s (%s)" % (label, nomActivite)
        if (label in dictPrestations) == False :
            dictPrestations[label] = []
        dictPrestations[label].append(IDprestation)
    
    dlg = wx.MultiChoiceDialog(None, _("Sélectionnez les prestations à modifier :"), _("Selection des prestations"), list(dictPrestations.keys()))
    dlg.SetSize((500, 400))
    if dlg.ShowModal() == wx.ID_OK :
    ...
    # Choix du taux
    dlg = wx.TextEntryDialog(None, _("Quel taux souhaitez-vous appliquer ?"), _("Sélection du taux de TVA"), "0.00")
    if dlg.ShowModal() == wx.ID_OK:
        tva = dlg.GetValue()
        dlg.Destroy()
    else :
    ...
    try :
        ...
        dlg = wx.MessageDialog(None, _("Le taux saisi ne semble pas correct !"), _("Erreur de saisie"), wx.OK | wx.ICON_ERROR)
        dlg.ShowModal()
        dlg.Destroy()
        return
    ...
    dlg = wx.MessageDialog(None, _("Souhaitez-vous vraiment appliquer un taux de tva de %.2f %% aux %d prestations sélectionnées ?\n\n(PS : Cette modification n'aura aucun impact sur les montants facturés)") % (tva, nbrePrestations), _("Confirmation"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
    ...
    # Application
    dlgAttente = wx.BusyInfo(_("Traitement en cours..."), None)
    ...
    # Fin du traitement
    dlg = wx.MessageDialog(None, _("Le traitement s'est terminé avec succès !"), _("Fin"), wx.OK | wx.ICON_INFORMATION)
    dlg.ShowModal()
    dlg.Destroy()
