...
class Synchro():
    ...
    def Synchro_totale(self, full_synchro=False):
        t1 = time.time()
        self.nbre_etapes = 26
        self.log.EcritLog(_("Lancement de la synchronisation..."))
        ...
        if resultat == False :
            self.log.EcritLog(_("Synchronisation arrêtée."))
        else :
            secondes = time.time() - t1
            self.log.EcritLog(_("Synchronisation effectuée en %d secondes.") % secondes)
        self.log.EcritLog(_("Client de synchronisation prêt"))
        try :
            self.log.SetGauge(0)
        except :
            pass
    ...
    def Upload_config(self, ftp=None):
        ...
        self.log.EcritLog(_("Téléchargement du fichier de config"))
    ...
    def AutoReloadWSGI(self, ftp=None):
        ...
        self.log.EcritLog(_("Mise à jour du fichier WSGI pour auto reload"))
    ...
    def Connexion(self):
        ...
        if self.dict_parametres["hebergement_type"] == 1 :
            self.log.EcritLog(_("Connexion FTP..."))
            self.Pulse_gauge()
            ...
            try :
                ...
                self.log.EcritLog(_("[ERREUR] Connexion FTP impossible."))
                return False

        # Connexion SSH/SFTP
        if self.dict_parametres["hebergement_type"] == 2 :
            self.log.EcritLog(_("Connexion SSH/SFTP..."))
            self.Pulse_gauge()
            ...
            try :
                ...
                self.log.EcritLog(_("[ERREUR] Connexion SSH/SFTP impossible."))
                try:
                    self.log.EcritLog(_("[ERREUR] err: %s") % err.encode('ascii', 'ignore'))
                except:
                    self.log.EcritLog(_("[ERREUR] err: %s") % err)
                return False
    ...
    def Deconnexion(self, ftp=None):
        if self.dict_parametres["hebergement_type"] == 1 :
            self.log.EcritLog(_("Fermeture de la connexion FTP..."))
            ftp.quit()
        elif self.dict_parametres["hebergement_type"] == 2 :
            self.log.EcritLog(_("Fermeture de la connexion SSH/SFTP..."))
            ftp.close()
        else :
            pass

    def Upload_data(self, full_synchro=False) :
        self.log.EcritLog(_("Lancement de la synchronisation des données..."))
        t1 = time.time()
        ...
        self.log.EcritLog(_("Synchro du fichier de configuration..."))
        self.Pulse_gauge()
        resultat = self.Upload_config(ftp=ftp)
        if resultat == False :
            self.log.EcritLog(_("[ERREUR] Synchro du fichier de configuration impossible."))
            self.Deconnexion(ftp)
            return False

        # Récupération du fichier models
        self.log.EcritLog(_("Récupération des modèles de données..."))
        resultat = self.TelechargeFichier(ftp=ftp, nomFichier="models.py", repFichier="application")
        if resultat == False :
            self.log.EcritLog(_("[ERREUR] Récupération des modèles de données impossible."))
            self.Deconnexion(ftp)
            return False
        ...
        self.log.EcritLog(_("Récupération des données à exporter..."))
        self.Pulse_gauge()
        ...
        for profil, listeDonnees in [("famille", listeFamilles), ("utilisateur", listeUtilisateurs)]:
            for dictDonnee in listeDonnees :
                if dictDonnee["internet_identifiant"] not in ("", None) and dictDonnee["internet_mdp"] not in ("", None) :
                    ...
                    if profil == "utilisateur" :
                        nomDossier = "%s %s" % (dictDonnee["prenom"], dictDonnee["nom"])
        ...
        DB2.ExecuterReq(req,MsgBox="ExecuterReq")
        ...
        req = """
        SELECT IDgroupe, IDactivite, nom, ordre, nbrMaxi
        FROM groupes;"""
        DB.ExecuterReq(req,MsgBox="ExecuterReq")
        listeGroupes = DB.ResultatReq()

        for IDgroupe, IDactivite, nom, ordre, nbrMaxi in listeGroupes :
            m = models.Groupe(IDgroupe=IDgroupe, nom=nom, IDactivite=IDactivite, ordre=ordre, nbrMaxi=nbrMaxi)
            session.add(m)
        ...
        # Commit
        self.log.EcritLog(_("Enregistrement des données à exporter..."))
        self.Pulse_gauge()
        session.commit()

        # Compression du fichier
        self.log.EcritLog(_("Compression du fichier d'export..."))
        self.Pulse_gauge()
        ...
        # Cryptage du fichier
        self.log.EcritLog(_("Cryptage du fichier d'export..."))
        self.Pulse_gauge()
        ...
        self.log.EcritLog(_("Envoi du fichier de données..."))
        self.Pulse_gauge()
        time.sleep(0.5)
        ...
        self.log.EcritLog(_("Envoi de la requête de traitement du fichier d'export..."))
        self.Pulse_gauge()
        time.sleep(0.5)
        ...
        try :
            ...
            print(err)
            self.log.EcritLog(_("[ERREUR] Erreur dans le traitement du fichier"))
        ...
        if page != None and page != "True" and page != b"True" :
            ...
            self.log.EcritLog(_("[ERREUR] Erreur dans le traitement du fichier. Réponse reçue :"))
    ...
    def Download_data(self, full_synchro=False):
        ...
        self.log.EcritLog(_("Téléchargement des demandes..."))
        self.Pulse_gauge()
        ...
        try :
            ...
            if len(liste_actions) == 0 :
                self.log.EcritLog(_("Aucune demande non traitée trouvée..."))
            elif len(liste_actions) == 1 :
                self.log.EcritLog(_("1 demande non traitée trouvée..."))
            else :
                self.log.EcritLog(_("%s demandes non traitées trouvées...") % len(liste_actions))

        except Exception as err :
            print(err)
            self.log.EcritLog(_("Téléchargement des demandes impossible"))
            liste_actions = []
        ...
        if liste_actions != None and len(liste_actions) > 0 :
            ...
            if len(listePasswordsFamilles) > 0 :
                ...
                if len(listePasswordsFamilles) == 1:
                    self.log.EcritLog(_("1 mot de passe famille modifié..."))
                else:
                    self.log.EcritLog(_("%d mots de passe familles modifiés...") % len(listePasswordsFamilles))
            if len(listePasswordsUtilisateurs) > 0 :
                ...
                if len(listePasswordsUtilisateurs) == 1:
                    self.log.EcritLog(_("1 mot de passe administrateur modifié..."))
                else:
                    self.log.EcritLog(_("%d mots de passe administrateurs modifiés...") % len(listePasswordsUtilisateurs))

            DB.Commit()
            DB.Close()

            self.log.EcritLog(_("Téléchargement terminé"))

        return True

    def Valide_reception_demande(self, IDaction=None):
        self.log.EcritLog(_("COMING SOON:Validation de la reception de la demande %s") % IDaction)
        pass
    ...
    def Update_application(self):
        ...
        self.log.EcritLog(_("Recherche d'une mise à jour..."))
        ...
        try :
            ...
            self.log.EcritLog(_("[Erreur] Erreur dans la demande d'update"))
    ...
    def Upgrade_application(self):
        ...
        try :
            ...
            self.log.EcritLog(_("[Erreur] Erreur dans la demande d'upgrade"))
    ...
    def Repair_application(self):
        ...
        try :
            ...
            self.log.EcritLog(_("[Erreur] Erreur dans la demande de réparation DB"))
    ...
    def Clear_application(self):
        ...
        try :
            ...
            self.log.EcritLog(_("[Erreur] Erreur dans la demande d'effacement de la DB"))
    ...
    def TelechargeFichier(self, ftp=None, nomFichier="models.py", repFichier=None):
        ...
        if self.dict_parametres["hebergement_type"] == 0 :
            ...
            try :
                ...
                self.log.EcritLog(_("Récupération locale impossible du fichier '%s'") % nomFichier)
                self.log.EcritLog(_("err: %s") % err)
            ...
            try :
                ...
                self.log.EcritLog(_("Téléchargement FTP impossible du fichier '%s'") % nomFichier)
                self.log.EcritLog(_("err: %s") % err)
            ...
            try :
                ...
                if repFichier != None and "/" in repFichier :
                    ftp.chdir("wx.CURSOR_" * len(repFichier.split("/")))
            except Exception as err :
                if nomFichier == "config.py" :
                    self.log.EcritLog(_("Aucun fichier de configuration à télécharger"))
                else :
                    self.log.EcritLog(_("Téléchargement SSH/SFTP impossible du fichier '%s'") % nomFichier)
                    self.log.EcritLog(_("err: %s") % err)
                    print("Erreur dans telechargement du fichier '%s' : %s" % (nomFichier, str(err)))
                return False
        else:
        ...
        if self.dict_parametres["hebergement_type"] == 0 :
            if self.dict_parametres["hebergement_local_repertoire"] != None:
                try :
                    ...
                    self.log.EcritLog(_("[ERREUR] Envoi du fichier '%s' par copie locale impossible.") % nomFichier)
                    return False
        ...
        if self.dict_parametres["hebergement_type"] == 1 :
            if ftp != None :
                try :
                    ...
                    print(err)
                    self.log.EcritLog(_("[ERREUR] Envoi du fichier '%s' par FTP impossible.") % nomFichier)
                    return False
        ...
        if self.dict_parametres["hebergement_type"] == 2 :
            if ftp != None :
                try :
                    ...
                    if "/" in repDest :
                        ftp.chdir("wx.CURSOR_" * len(repDest.split("/")))
                except Exception as err :
                    print(u"Erreur upload fichier '%s' par SSH/SFTP :" % nomFichier)
                    print(err)
                    self.log.EcritLog(_("[ERREUR] Envoi du fichier '%s' par SSH/SFTP impossible.") % nomFichier)
                    return False
