...
class ServeurConnecthys():
    def __init__(self, parent):
        ...
        if self.dict_parametres["hebergement_type"] == 2 :
            if self.dict_parametres["ssh_utilisateur"] == "root" :
                dlg = wx.MessageDialog(None, _("L utilisation de l utilisateur root pour l accès SSH/SFTP est dangereuse !!!"), _("SSH/SFTP Warning"), wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                self.ssh = None
            else :
    ...
    def Demarrer_serveur(self, event):
        ...
        if self.GetServerStatus() == True :
            self.parent.EcritLog(_("Serveur démarré"))
            dlg = wx.MessageDialog(None, _("Le serveur Connecthys a bien été démarré !"), _("Serveur Connecthys"), wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
        else :
            self.parent.EcritLog(_("[ERREUR] Le serveur n'a pas pu être démarré"))
            dlg = wx.MessageDialog(None, _("Le serveur Connecthys n'a pas pu être démarré !"), _("Serveur Connecthys"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
    ...
    def Demarrer_serveurLocal(self) :
        ...
        if rep == "" or os.path.isfile(chemin_executable) == False :
            dlg = wx.MessageDialog(None, _("Le chemin du répertoire d'installation de Connecthys n'est pas valide !"), _("Accès impossible"), wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Arreter_serveur(self, event):
        self.parent.EcritLog(_("Arrêt du serveur Connecthys"))
        if self.dict_parametres["hebergement_type"] == 0 :
        ...
        if self.GetServerStatus() == False :
            self.parent.EcritLog(_("Serveur stoppé"))
            dlg = wx.MessageDialog(None, _("Le serveur Connecthys a bien été stoppé !"), _("Serveur Connecthys"), wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
        else :
            self.parent.EcritLog(_("[ERREUR] Le serveur n'a pas pu être stoppé"))
            dlg = wx.MessageDialog(None, _("Le serveur Connecthys n'a pas pu être stoppé !"), _("Serveur Connecthys"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
