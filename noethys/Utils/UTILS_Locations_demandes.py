    ...
    def Impression(self, listeDemandes=[], nomDoc=None, afficherDoc=True, dictOptions=None, repertoire=None, repertoireTemp=False):
        ...
        if dictOptions == None:
            if afficherDoc == False:
                dlg = DLG_Apercu_location_demande.Dialog(None, titre=_("Sélection des paramètres de la demande"), intro=_("Sélectionnez ici les paramètres d'affichage de la demande."))
        ...
        def CreationPDFunique(repertoireCible=""):
            ...
            try:
                ...
                dlg = wx.MessageDialog(None, _("Désolé, le problème suivant a été rencontré dans l'édition des demandes : \n\n%s") % err, _("Erreur"), wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        ...
        if repertoireTemp == False:
            ...
            self.EcritStatusbar(_("Création du PDF des demandes en cours... veuillez patienter..."))
            try:
                ...
                del dlgAttente
                traceback.print_exc(file=sys.stdout)
                dlg = wx.MessageDialog(None, "Désolé, le problème suivant a été rencontré dans l'édition des demandes : \n\n%s" % err, _("Erreur"), wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                return False
