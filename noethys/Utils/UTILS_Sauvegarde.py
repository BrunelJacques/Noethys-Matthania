...
LISTE_CATEGORIES = [
    (_("Données de base"), "DATA"),
    (_("Photos individuelles"), "PHOTOS"),
...
def Sauvegarde(listeFichiersLocaux=[], listeFichiersReseau=[], nom="", repertoire=None, motdepasse=None, listeEmails=None, dictConnexion=None):
    ...
    if repertoire != None :
        fichierDest = "%s/%s.%s" % (repertoire, nom, extension)
        if os.path.isfile(fichierDest) == True :
            dlg = wx.MessageDialog(None, _("Un fichier de sauvegarde portant ce nom existe déjà. \n\nVoulez-vous le remplacer ?"), "Attention !", wx.YES_NO | wx.NO_DEFAULT | wx.ICON_EXCLAMATION)
    ...
    if listeEmails != None :
        ...
        if dictAdresse == None :
            dlgErreur = wx.MessageDialog(None, _("Envoi par Email impossible :\n\nAucune adresse d'expéditeur n'a été définie. Veuillez la saisir dans le menu Paramétrage du logiciel..."), _("Erreur"), wx.OK | wx.ICON_ERROR)
            dlgErreur.ShowModal() 
            dlgErreur.Destroy()
            return False

    # Fenêtre de progression
    dlgprogress = wx.ProgressDialog(_("Sauvegarde"), _("Lancement de la sauvegarde..."), maximum=nbreEtapes, parent=None, style= wx.PD_SMOOTH | wx.PD_AUTO_HIDE | wx.PD_APP_MODAL)
    ...
    for nomFichier in listeFichiersLocaux :
        dlgprogress.Update(numEtape, _("Compression du fichier %s...") % nomFichier);numEtape += 1
        fichier = UTILS_Fichiers.GetRepData(nomFichier)
        if os.path.isfile(fichier) == True :
            fichierZip.write(fichier, nomFichier)
        else :
            dlgprogress.Destroy()
            dlgErreur = wx.MessageDialog(None, _("Le fichier '%s' n'existe plus sur cet ordinateur. \n\nVeuillez ôter ce fichier de la procédure de sauvegarde automatique (Menu Fichier > Sauvegardes automatiques)") % nomFichier, _("Erreur"), wx.OK | wx.ICON_ERROR)
            dlgErreur.ShowModal() 
            dlgErreur.Destroy()
            return False
    ...
    if len(listeFichiersReseau) > 0 and dictConnexion != None :
        ...
        if repMySQL == None :
            dlgprogress.Destroy()
            dlgErreur = wx.MessageDialog(None, _("Noethys n'a pas réussi à localiser MySQL sur votre ordinateur.\n\nNotez bien que MySQL doit être installé obligatoirement pour créer une sauvegarde réseau."), _("Erreur"), wx.OK | wx.ICON_ERROR)
            dlgErreur.ShowModal() 
            dlgErreur.Destroy()
            return False
        ...
        for nomFichier in listeFichiersReseau :
            dlgprogress.Update(numEtape, _("Compression du fichier %s...") % nomFichier);numEtape += 1
            fichierSave = "%s/%s.sql" % (repTemp, nomFichier)

            args = """"%sbin/mysqldump" --defaults-extra-file="%s" --column-statistics=0 --single-transaction --opt --databases %s > "%s" """ % (repMySQL, nomFichierLoginTemp, nomFichier, fichierSave)
            ...
            if out not in ("", b""):
                ...
                try :
                    ...
                    pass
                dlgprogress.Destroy()
                dlgErreur = wx.MessageDialog(None, _("Une erreur a été détectée dans la procédure de sauvegarde !\n\nErreur : %s") % out, _("Erreur"), wx.OK | wx.ICON_ERROR)
                dlgErreur.ShowModal() 
                dlgErreur.Destroy()
                return False
            ...
            try :
                fichierZip.write(fichierSave.encode('utf8'), "%s.sql" % nomFichier)
    ...
    # Cryptage du fichier
    if motdepasse != None :
        dlgprogress.Update(numEtape, _("Cryptage du fichier..."));numEtape += 1
        fichierCrypte = "%s.%s" % (nom, EXTENSIONS["crypte"])
    ...
    if repertoire != None :
        dlgprogress.Update(numEtape, _("Création du fichier dans le répertoire cible..."));numEtape += 1
        try :
    ...
    message = UTILS_Envoi_email.Message(destinataires=listeEmails, sujet=_("Sauvegarde Noethys : %s") % nom,
                                        texte_html=_("Envoi de la sauvegarde de Noethys"),
                                        fichiers=[UTILS_Fichiers.GetRepTemp(fichier=nomFichierTemp),])

    # Envoi par Email
    if listeEmails != None :
        dlgprogress.Update(numEtape, _("Expédition de la sauvegarde par Email..."));numEtape += 1
        try :
    ...
    dlgprogress.Update(numEtape, _("Suppression des fichiers temporaires..."));numEtape += 1
    ...
    # Fin du processus
    dlgprogress.Update(numEtape, _("Sauvegarde terminée avec succès !"))
    dlgprogress.Destroy()
...
def Restauration(parent=None, fichier="", listeFichiersLocaux=[], listeFichiersReseau=[], dictConnexion=None):
    ...
    if len(listeFichiersLocaux) > 0 :
        ...
        # Restauration
        nbreEtapes = len(listeFichiersLocaux)
        dlgprogress = wx.ProgressDialog(_("Merci de patienter"), _("Lancement de la restauration..."), maximum=nbreEtapes, parent=parent, style= wx.PD_SMOOTH | wx.PD_AUTO_HIDE | wx.PD_APP_MODAL)
        numEtape = 1

        for fichier in listeFichiersLocaux :
            dlgprogress.Update(numEtape, _("Restauration du fichier %s...") % fichier);numEtape += 1
            try :
                ...
                dlgprogress.Destroy()
                print(err)
                dlg = wx.MessageDialog(None, _("La restauration du fichier '%s' a rencontré l'erreur suivante : \n%s") % (fichier, err), "Erreur", wx.OK| wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                return False
    ...
    if len(listeFichiersReseau) > 0 :
        ...
        repMySQL = GetRepertoireMySQL(dictConnexion) 
        if repMySQL == None :
            dlgErreur = wx.MessageDialog(None, _("Noethys n'a pas réussi à localiser MySQL sur votre ordinateur.\nNotez bien que MySQL doit être installé obligatoirement pour créer une restauration réseau."), _("Erreur"), wx.OK | wx.ICON_ERROR)
            dlgErreur.ShowModal() 
            dlgErreur.Destroy()
            return False
        ...
        for fichier in listeFichiersReseau:
            ...
            if fichier not in listeFichiersExistants :
                nomFichier = "%s;%s;%s;%s[RESEAU]%s" % (dictConnexion["port"], dictConnexion["host"], dictConnexion["user"], dictConnexion["password"], fichier)
            ...
            fichierRestore = "%s/%s.sql" % (repTemp, fichier)
...
def GetRepertoireMySQL(dictValeurs={}):
                        ...
                        if "MySQL" in fichier2 :
                            ...
                            for fichier3 in listeFichiers3 :
                                if "MySQL Server" in fichier3 :
                                    chemin = "C:/%s/%s/%s/" % (fichier1, fichier2, fichier3)
                                    if os.path.isfile(chemin + "bin/mysql.exe") :
                                        return chemin
        except :
            pass
    ...
    try :
        if "linux" in sys.platform :
            message = _("Pour effectuer la sauvegarde de fichiers réseau, mysqlclient doit être installé. Sélectionnez ici le répertoire où se trouve 'mysqldump' sur votre ordinateur.")
        else :
            message = _("Pour effectuer la sauvegarde de fichiers réseau, Noethys \ndoit utiliser les outils de MySQL. Sélectionnez ici le répertoire qui se nomme 'MySQL Server...' sur votre ordinateur.")
