...
class Sauvegarde_auto():
    ...
    def Start(self):
        ...
        for dictSauvegarde in self.listeSauvegardes :
            ...
            dictSauvegarde["sauvegarde_nom"] = "%s_%s" % (dictSauvegarde["sauvegarde_nom"], datetime.datetime.now().strftime("%Y%m%d_%H%M"))
            ...
            if valide == True and dictSauvegarde["option_demander"] == "1" :
                ...
                message1 = _("Souhaitez-vous lancer la procédure de sauvegarde '%s' ?") % dictSauvegarde["nom"]
                dlg = dialogs.MultiMessageDialog(self.parent, message1, caption=_("Sauvegarde automatique"), msg2=None, style = wx.NO | wx.CANCEL | wx.YES | wx.YES_DEFAULT, icon=image, btnLabels={wx.ID_YES : _("Oui"), wx.ID_NO : _("Non"), wx.ID_CANCEL : _("Annuler")})
            ...
            if valide == True and dictSauvegarde["option_afficher_interface"] != "1" :
                ...
                if resultat == True and dictSauvegarde["option_confirmation"] == "1" :
                    dlg = wx.MessageDialog(self.parent, _("La procédure de sauvegarde '%s' s'est terminée avec succès.") % dictSauvegarde["nom"], _("Sauvegarde"), wx.OK | wx.ICON_INFORMATION)
                    dlg.ShowModal()
                    dlg.Destroy()

                if resultat == False :
                    dlg = wx.MessageDialog(self.parent, _("Echec de la procédure de sauvegarde '%s' !") % dictSauvegarde["nom"], _("Annulation"), wx.OK | wx.ICON_ERROR)
                    dlg.ShowModal()
                    dlg.Destroy()
                    return wx.ID_CANCEL
    ...
    def AnnonceRappel(self):
        ...
        try :
            image = wx.Bitmap(Chemins.GetStaticPath("Images/48x48/Sauvegarder.png"), wx.BITMAP_TYPE_ANY)
            message1 = _("Vous n'avez paramétré aucune sauvegarde automatique.\n\nSouhaitez-vous le faire maintenant ?")
            dlg = dialogs.MultiMessageDialog(self.parent, message1, caption=_("Rappel de sauvegarde"), msg2=None, style = wx.NO | wx.CANCEL | wx.YES | wx.YES_DEFAULT, icon=image, btnLabels={wx.ID_YES : _("Oui"), wx.ID_NO : _("Ne plus rappeler"), wx.ID_CANCEL : _("Pas maintenant")})
            reponse = dlg.ShowModal() 
            dlg.Destroy() 
        except :
            reponse = None
