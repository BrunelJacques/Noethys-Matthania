...
def Parser_voie(texte=""):
    if not texte:
        return {}
    texte = texte.replace(",", "")
    types_voies = [u"rue", "avenue", "boulevard", "bd", "bvd", "lieu dit", "lieu-dit", "route", "clos", "ldt", "square", "impasse", "cours", "esplanade", "allée", "résidence", "chemin", "place", "cité", "hameau", "coteau"]
    liste_types_voies = []
    for type_voie in types_voies:
        liste_types_voies.extend([u"%s des" % type_voie, "%s de la" % type_voie, "%s de" % type_voie, "%s du" % type_voie, type_voie])
...
def ConvertListeToPhrase(liste):
    if len(liste) == 0:
        ...
        return _(" et ").join(liste)
    else:
        return _("%s et %s") % (u", ".join(liste[:-1]), liste[-1])
