...
class Installer():
    ...
    def Telecharger(self):
        ...
        self.dlgprogress = wx.ProgressDialog(_("Veuillez patienter"), _("Lancement de l'installation..."), maximum=100, parent=self.parent, style= wx.PD_SMOOTH | wx.PD_ESTIMATED_TIME | wx.PD_REMAINING_TIME | wx.PD_CAN_ABORT | wx.PD_AUTO_HIDE | wx.PD_APP_MODAL)

        def _hook(nb_blocs, taille_bloc, taille_fichier):
            if nb_blocs % 5 == 0 :
                pourcentage = GetPourcentage(nb_blocs*taille_bloc, taille_fichier)
                keepGoing, skip = self.dlgprogress.Update(pourcentage, _("Téléchargement de Connecthys en cours... %s %%") % pourcentage)
                if keepGoing == False :
    ...
    def Dezipper(self, fichier_zip, chemin_dest=""):
        ...
        for i in liste_fichiers:
            ...
            keepGoing, skip = self.dlgprogress.Update(pourcentage, _("Décompression de Connecthys en cours... %s %%") % pourcentage)
        ...
        for name in os.listdir(path):
            ...
            if IsException(name) == False :
                ...
                if os.path.isfile(localpath):
                    ...
                    try :
                        keepGoing, skip = self.dlgprogress.Update(pourcentage, _("Installation : Transfert du fichier %s...") % name)
                    ...
                    if self.dict_parametres["hebergement_type"] == 1 :
                        ...
                        if self.dict_parametres["serveur_type"] == 1 and name == self.dict_parametres["serveur_cgi_file"] :
                            ...
                            if "mode=0755" not in ftp.sendcmd("MLST %s" % self.dict_parametres["serveur_cgi_file"]) :
                                message = "Attention, le fichier %s n'a pas les bonnes permissions." % self.dict_parametres["serveur_cgi_file"]
                    ...
                    if self.dict_parametres["hebergement_type"] == 2 :
                        ...
                        if self.dict_parametres["serveur_type"] == 1 and name == self.dict_parametres["serveur_cgi_file"] :
                            ...
                            if mode != 755 :
                                message = "Attention, le fichier %s n'a pas les bonnes permissions : %d au lieu de 0755." % (self.dict_parametres["serveur_cgi_file"], mode)
                                self.parent.EcritLog(message)
                                print(message)
    ...
    def Upload(self, source_repertoire=""):
        ...
        if self.dict_parametres["hebergement_type"] == 0 :
            keepGoing, skip = self.dlgprogress.Update(1, _("Préparation de la copie..."))
            ...
            keepGoing, skip = self.dlgprogress.Update(2, _("La copie va commencer..."))
            ftp = None

        # Initialisation pour transfert FTP
        if self.dict_parametres["hebergement_type"] == 1 :
            keepGoing, skip = self.dlgprogress.Update(1, _("Connexion FTP en cours..."))
            ...
            keepGoing, skip = self.dlgprogress.Update(2, _("Connexion FTP effectuée..."))

        # Initialisation pour un transfert SSH
        if self.dict_parametres["hebergement_type"] == 2 :
            keepGoing, skip = self.dlgprogress.Update(1, _("Connexion SSH/SFTP en cours..."))

            try :
                ftp = self.server_ctrl.ssh.open_sftp()
            except Exception as err:
                print(err)
                self.parent.EcritLog(_("[ERREUR] %s") % err)
                raise
            ...
            keepGoing, skip = self.dlgprogress.Update(2, _("Connexion SSH/SFTP effectuée..."))
        ...
        keepGoing, skip = self.dlgprogress.Update(97, _("Installation du fichier de configuration en cours..."))
        ...
        if self.server_ctrl != None and self.dict_parametres["serveur_type"] == 0 and self.server_ctrl.GetServerStatus() == False:
            keepGoing, skip = self.dlgprogress.Update(98, _("Tentative de lancement du serveur Connecthys..."))
            self.server_ctrl.Demarrer_serveur(event=None)

        # Demande un upgrade de l'application
        keepGoing, skip = self.dlgprogress.Update(99, _("Demande la mise à jour de l'application..."))
    ...
    def Installer(self):
        ...
        dlg = wx.MessageDialog(None, _("Confirmez-vous l'installation du portail internet Connecthys ?\n\nRemarque : Ce processus peut nécessiter plusieurs dizaines de minutes (selon votre connexion internet)"), _("Installation"), wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_QUESTION)
        ...
        try :
            ...
            self.parent.EcritLog(_("Téléchargement des fichiers source Connecthys"))
            self.Telecharger()

            # Dézippage du fichier
            self.parent.EcritLog(_("Décompression des fichiers source Connecthys"))
            self.Dezipper(self.nom_fichier_dest, UTILS_Fichiers.GetRepTemp())

            # Envoi des fichiers dans le répertoire d installation
            self.parent.EcritLog(_("Upload des fichiers Connecthys"))
            ...
            time.sleep(2)
            dlg = wx.MessageDialog(None, _("Procédure d'installation interrompue."), "Erreur", wx.OK | wx.ICON_EXCLAMATION)
            dlg.Raise()
            dlg.ShowModal()
            dlg.Destroy()
            return False
            ...
            time.sleep(2)
            dlg = wx.MessageDialog(None, _("Une erreur a été rencontrée !"), "Erreur", wx.OK | wx.ICON_ERROR)
            dlg.Raise()
            dlg.ShowModal()
            dlg.Destroy()
            return False
