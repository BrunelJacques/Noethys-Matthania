    ...
    def Impression(self, listeLocations=[], nomDoc=None, afficherDoc=True, dictOptions=None, repertoire=None, repertoireTemp=False):
        ...
        if dictOptions == None:
            if afficherDoc == False:
                dlg = DLG_Apercu_location.Dialog(None, titre=_("Sélection des paramètres de la location"), intro=_("Sélectionnez ici les paramètres d'affichage de la location."))
        ...
        def CreationPDFunique(repertoireCible=""):
            dictPieces = {}
            dlgAttente = wx.BusyInfo(_("Génération des PDF à l'unité en cours..."), None)
            try:
                index = 0
                for IDlocation, dictLocation in dictLocations.items():
                    if dictLocation["select"] == True:
                        ...
                        nomFichier = _("Location %d - %s") % (IDlocation, nomTitulaires)
                        cheminFichier = "%s/%s.pdf" % (repertoireCible, nomFichier)
                        dictComptesTemp = {IDlocation: dictLocation}
                        self.EcritStatusbar(_("Edition de la location %d/%d : %s") % (index, len(dictLocation), nomFichier))
                ...
                dlg = wx.MessageDialog(None, _("Désolé, le problème suivant a été rencontré dans l'édition des locations : \n\n%s") % err, _("Erreur"), wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        ...
        if repertoireTemp == False:
            dlgAttente = wx.BusyInfo(_("Création du PDF en cours..."), None)
            self.EcritStatusbar(_("Création du PDF des locations en cours... veuillez patienter..."))
            try:
                ...
                del dlgAttente
                traceback.print_exc(file=sys.stdout)
                dlg = wx.MessageDialog(None, "Désolé, le problème suivant a été rencontré dans l'édition des locations : \n\n%s" % err, _("Erreur"), wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
                dlg.Destroy()
                return False
